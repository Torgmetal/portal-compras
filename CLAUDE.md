...
A integração automática (hook Stop que chama o revisor, `docs/integracao-codex-claude.md`, `scripts/revisao-codex/consultar.py`, `.codex/skills/`) só existe nas máquinas onde foi instalada — esses arquivos não estão na `main`, e o hook Stop versionado em `.claude/settings.json` só lembra o workflow de validação. Onde a integração existir: trate o feedback dentro da tarefa autorizada; não desative ou altere o hook para contornar uma reprovação. Falha/limite/decisão humana são pendências, não aprovação. Não faça pull/push automaticamente durante a revisão. O usuário pode suspender a integração explicitamente. Onde não existir, registre em `docs/revisao-codex-claude.md` e siga.

**Você (Claude Code) é o orquestrador único.** Onde a integração estiver instalada, você pode
...
npm test             # vitest — NENHUM teste toca o banco
...
- `auth.js` / `session.js` — NextAuth config + `requireRole([...])` guard used in every API route
- `omie-*.js` — one module per Omie ERP resource; `omie-call.js` is the shared caller (retry with backoff)
...
NextAuth credentials provider with JWT (12-hour session). Access is `User.tipo` (`TipoUsuario`: ADMIN, USUARIO, FUNCIONARIO, CLIENTE) plus the user's modules (`Modulo` enum in `prisma/schema.prisma` — the list grows with the portal, read it there). `requireRole([...])` takes module names and ADMIN always passes; `middleware.js` enforces routing; every mutating API endpoint calls `requireRole([...])` at the top.
...
The schedule lives in `vercel.json` — read it there; it changes often. Every cron calls `await aquecerBanco(prisma)` first (see the Neon warning above). Routes that exceed the default limit declare `export const maxDuration = N` at the module level.
...
## Regras por módulo (carregadas sob demanda)

As regras de cada módulo abaixo moram em `.claude/rules/`. O Claude Code carrega cada arquivo quando
lê um arquivo daquele módulo (o campo `paths` no topo da regra); fora dele, a regra não ocupa
contexto. Para ler sem abrir código, abra o arquivo da regra.

| Módulo | Regra |
|---|---|
| Etiquetas de carregamento (Expedição) | `.claude/rules/etiquetas-carregamento.md` |
| Desenho da inspeção (PDF.js × pdf-lib, `/Rotate`) | `.claude/rules/vista-desenho-inspecao.md` |
| Import de lista LE/LPC — o recibo | `.claude/rules/import-listas-le-lpc.md` |
| Prazos das RMs, cobrança e resposta do fornecedor | `.claude/rules/prazos-rms-compras.md` |
| O sino (notificações) | `.claude/rules/sino-notificacoes.md` |
| Conferência de peça (Expedição) | `.claude/rules/conferencia-peca.md` |
| Expedição no celular | `.claude/rules/expedicao-celular.md` |

Regra nova de módulo vai num arquivo novo em `.claude/rules/`, com o `paths` do módulo — não aqui.
