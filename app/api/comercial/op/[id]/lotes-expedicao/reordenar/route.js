// POST — reordena a prioridade dos lotes. Body { ordem: [loteId, loteId, ...] }
// grava ordem = posição+1 (1 = primeiro a entregar).
...
export async function POST(req, { params }) {
  ...
  await prisma.$transaction(ids.map((id, i) => prisma.loteExpedicao.updateMany({ where: { id, opId: params.id }, data: { ordem: i + 1 } })));
