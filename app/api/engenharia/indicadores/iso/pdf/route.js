// GET /api/engenharia/indicadores/iso/pdf?ano= — PDF de acompanhamento dos indicadores ISO da
// Engenharia (padrão Torg), com a série mensal + acumulado do ano.
...
export async function GET(req) {
  ...
  const bytes = await gerarIndicadoresIsoPDF({ titulo: "Indicadores de Engenharia (ISO)", ano, indicadores, mesFim });
  return new NextResponse(Buffer.from(bytes), {
    headers: { "Content-Type": "application/pdf", "Content-Disposition": dispArquivo(`indicadores-engenharia-${ano}.pdf`, "inline") },
  });
}
