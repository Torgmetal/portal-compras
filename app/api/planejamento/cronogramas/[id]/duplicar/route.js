...
export async function POST(req, { params }) {
  ...
  const novo = await prisma.$transaction(async (tx) => {
    const cron = await tx.cronograma.create({
      data: {
        ...
        dataBase: null, // baseline zerado — a cópia começa sem validação
    ...
    if (origem.tarefas.length > 0) {
      await tx.cronogramaTarefa.createMany({
        data: origem.tarefas.map((t) => ({
          ...
          // Zera execução/realizado/baseline — OP nova começa do zero:
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entity: "Cronograma",
      entityId: novo.id,
      diff: { origemId: id, origemOp: origem.opNumero, novaOp: opNumero, titulo, tarefas: origem.tarefas.length, opVinculada: op?.numero || null },
    },
  }).catch(() => {});
