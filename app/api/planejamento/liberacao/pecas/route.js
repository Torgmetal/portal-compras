...
export async function GET(req) {
  ...
  // Os fardos completos são consultados ao abrir o seletor, sem aumentar a carga
  // inicial da planilha nem cortar recebimentos mais recentes.
  const materiais = [...(material?.porPerfil?.values() || [])].map((v) => {
    const dele = pecas.filter(x => String(x.perfil || "").trim().toUpperCase() === v.perfil.toUpperCase());
    return {
