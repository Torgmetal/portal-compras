...
// PRODUÇÃO mês a mês = ACABAMENTO + acumulado (YTD), avaliada contra a meta
// geral de 6.000 kg/dia útil.
//
...
// peso da peça 4-6× (inflava). Mede-se num único estágio, contando cada peça uma
// vez. Usa-se o ACABAMENTO (não o corte): o corte é a ENTRADA; a produção real é
// a SAÍDA, e o acabamento é o estágio quase-final mais universal — a pintura
// subconta porque nem toda peça é pintada (definido pelo Vitor). A meta segue
// 6.000 kg/dia útil (ritmo da linha).
//
...
// o offset jogava o apontamento da madrugada pro mês anterior).
...
const META_DIA = 6000; // kg/dia útil — meta geral de produção (ritmo da linha)
const SETOR_PRODUCAO = "ACABAMENTO"; // estágio que mede a produção (cada peça 1×)
...
export async function GET(req) {
  ...
  try {
    ...
    // 1 agregado por mês (só o Acabamento) — janela UTC-naïve, sem offset
    const aggs = await Promise.all(
      Array.from({ length: ultimoMes }, (_, i) => {
        ...
        return prisma.mesApontamento.aggregate({
          where: { AND: [whereSetorSyneco(SETOR_PRODUCAO), { dataInicio: { gte, lt } }] },
    ...
    const metaKgAcc = META_DIA * accDu;
