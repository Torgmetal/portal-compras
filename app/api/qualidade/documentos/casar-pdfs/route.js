...
export async function POST(req) {
  ...
  let res;
  try {
    res = await casarCertificados(body.url || null);
  } catch (e) {
    return NextResponse.json({ success: false, error: "Falha ao casar PDFs: " + e.message }, { status: 502 });
  }

  await prisma.auditLog
    .create({ data: { userId: user.id, action: "CASAR_PDFS_QUALIDADE", entity: "DocumentoQualidade", entityId: "-", diff: { casados: res.casados, totalPdfs: res.totalPdfs } } })
    .catch(() => {});

  return NextResponse.json({ success: true, casados: res.casados, totalPdfs: res.totalPdfs });
}
