...
// Painel de RMs de SERVIÇOS — Aluguel de Equipamentos e Medição de Montagem,
// em abas separadas. Esses tipos não aparecem nas listas de materiais/
// consumíveis (que filtram ENGENHARIA/INTERNA) e o controle aqui é por VALOR.
...
export default async function PainelServicos({ searchParams }) {
  await requireRole(["ADMIN", "COMPRAS"]);
  const tipo = searchParams?.tipo === "MONTAGEM" ? "MONTAGEM" : "ALUGUEL";
  ...
  const [rms, totaisAluguel, totaisMontagem] = await Promise.all([
    prisma.rM.findMany({
      where,
    ...
    prisma.rM.groupBy({ by: ["status"], where: { tipoRM: "ALUGUEL" }, _count: { _all: true } }),
    prisma.rM.groupBy({ by: ["status"], where: { tipoRM: "MONTAGEM" }, _count: { _all: true } }),
  ]);

  const contar = (totais, statuses) => totais.filter((t) => statuses.includes(t.status)).reduce((s, t) => s + t._count._all, 0);
  const ativasAluguel = contar(totaisAluguel, ["ABERTA", "EM_COTACAO", "COTADA"]);
  const ativasMontagem = contar(totaisMontagem, ["ABERTA", "EM_COTACAO", "COTADA"]);

  const totais = tipo === "ALUGUEL" ? totaisAluguel : totaisMontagem;
  ...
  const qs = (t, arq) => `/compras/servicos?tipo=${t}${arq ? "&arquivadas=1" : ""}`;

  return (
    <div className="space-y-6 max-w-7xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-3xl font-extrabold text-torg-dark tracking-tight">Aluguel & Montagem</h2>
          <p className="text-sm text-torg-gray mt-1">
            {tipo === "ALUGUEL"
              ? "Locação de equipamentos — diária × dias, vinculada à OP."
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="inline-flex bg-gray-100 rounded-lg p-1">
            <Link href={qs("ALUGUEL", verArquivadas)}
              className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                tipo === "ALUGUEL" ? "bg-white text-torg-orange shadow-sm" : "text-torg-gray hover:text-torg-dark"
              }`}>
              <Forklift size={15} /> Aluguel ({ativasAluguel})
            </Link>
            <Link href={qs("MONTAGEM", verArquivadas)}
              className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                tipo === "MONTAGEM" ? "bg-white text-emerald-700 shadow-sm" : "text-torg-gray hover:text-torg-dark"
              }`}>
              <Hammer size={15} /> Montagem ({ativasMontagem})
            </Link>
          </div>
          {/* Ativas / Histórico */}
          <div className="inline-flex bg-gray-100 rounded-lg p-1">
            <Link href={qs(tipo, false)}
      ...
      {rms.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-torg-gray text-lg">
            {verArquivadas ? "Nada no histórico" : `Nenhuma RM de ${tipo === "ALUGUEL" ? "aluguel" : "montagem"} ativa`}
          </p>
          {!verArquivadas && (
            <p className="text-sm text-torg-gray mt-2">
              O solicitante cria em <strong>/rm/nova</strong> escolhendo o tipo
              {tipo === "ALUGUEL" ? " “Aluguel de Equipamentos”" : " “Medição de Montagem”"}.
            </p>
          )}
        </div>
      ) : (
