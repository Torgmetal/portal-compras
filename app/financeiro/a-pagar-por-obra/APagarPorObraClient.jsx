...
import {
  ...
  Building2, CalendarClock, Wallet, AlertTriangle, Inbox,
} from "lucide-react";
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-torg-gray">
            {obra.numeroOp ? `OP-${obra.numeroOp} · ` : ""}{obra.qtd} título{obra.qtd !== 1 ? "s" : ""} a pagar
          </p>
        </div>
        {obra.vencido > 0 && (
            ...
            <div className="px-4 py-3 bg-gray-50/60 flex flex-wrap gap-2">
              {obra.meses.map((m) => (
                <div key={m.chave} className="bg-white border border-gray-100 rounded-lg px-3 py-1.5">
                  ...
                  <span className="ml-1 text-[10px] text-torg-gray">({m.qtd})</span>
                </div>
              ))}
            </div>
          )}
              ...
              <tbody className="divide-y divide-gray-50">
                {obra.titulos.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-1.5 font-mono text-torg-gray">{t.pedidoCompra || "—"}</td>
                    <td className="px-4 py-1.5 font-mono text-torg-gray">{t.nf || "—"}</td>
                    <td className="px-4 py-1.5 text-torg-gray">{t.parcela || "—"}</td>
...
export default function APagarPorObraClient() {
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      const res = await fetch("/api/financeiro/a-pagar-por-obra");
      if (!res.ok) {
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-torg-gray mt-0.5">
            Previsão de pagamentos das compras de fornecedor (não-diretas), agrupada por obra e mês de vencimento.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {data?.ultimoSync && (
            <span className="text-xs text-torg-gray">
              Sync: {new Date(data.ultimoSync).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
            </span>
          )}
          <button
            onClick={carregar}
      ...
      {data && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <KpiCard icon={Building2} label="Obras" valor={data.totais.obras} cor="text-torg-blue" />
          <KpiCard icon={CalendarClock} label="Títulos" valor={data.totais.qtd} cor="text-torg-gray" />
        </div>
      )}
      ...
      {loading && !data && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 flex items-center justify-center gap-3 text-torg-gray">
          <Loader2 size={18} className="animate-spin" /> Carregando contas a pagar...
        </div>
      )}
      ...
      {erro && (
        <div className="bg-white rounded-xl shadow-sm border border-red-100 p-6 flex flex-col items-center gap-3 text-center">
          ...
          <button onClick={carregar} className="text-sm font-medium text-torg-blue hover:underline">Tentar novamente</button>
        </div>
      )}
      ...
      {data && !loading && data.obras.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-10 flex flex-col items-center gap-3 text-center text-torg-gray">
          ...
          <p className="text-sm">Nenhuma conta a pagar de compras em aberto.</p>
          <p className="text-xs">As contas aparecem aqui conforme o pedido de compra é gerado e o sync do Omie roda.</p>
        </div>
      )}
      ...
      <p className="text-[11px] text-torg-gray text-center pt-2">
        Valores e vencimentos reais do Omie (espelho local, sincronizado pelo cron). Visão informativa — não altera o saldo do Fluxo de Caixa.
      </p>
    </div>
  );
}
