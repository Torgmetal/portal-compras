...
// módulo de Vendas/Faturamento do Omie em nova aba, para colar na busca.
const OMIE_TENANT = process.env.NEXT_PUBLIC_OMIE_TENANT || "torg-5mos4yik";
const OMIE_VENDAS_URL = `https://app.omie.com.br/gestao/${OMIE_TENANT}/#VEN`;
...
export default function ContasReceberClient() {
  ...
  // Default "abertos": sem filtro de data, mostra tudo que tem saldo a receber
  // (a maioria dos atrasados tem vencimento no passado).
  const [de, setDe]   = useState("");
  const [ate, setAte] = useState("");
  const [presetAtivo, setPreset] = useState("abertos");
  ...
  const carregar = async (d1 = de, d2 = ate) => {
    ...
    try {
      ...
      const res = await fetch(`/api/financeiro/contas-receber?${qs}`);
  ...
  const aplicarPreset = (p) => {
    const r = p === "abertos" ? { de: "", ate: "" }
      : p === "hoje" ? rangeHoje() : p === "semana" ? rangeSemana() : rangeMes();
  ...
  const sincronizar = async (manual = true) => {
    ...
    try {
      await fetch("/api/cron/contas-receber");
      await carregar();
  ...
  const totais = useMemo(() => {
    const sum = (arr) => arr.reduce((s, c) => s + (c.saldo || 0), 0);
  ...
  const resumoPor = (campo) => {
    ...
    for (const c of filtradas.filter((x) => x.aberta)) {
      ...
      cur.valor += c.saldo || 0; cur.qtd++; map.set(k, cur);
    }
  ...
  const exportarXlsx = async () => {
    ...
    try {
      ...
      const subtitulo = [
        (de || ate) ? `Período: ${fmtData(de + "T12:00")} a ${fmtData(ate + "T12:00")}` : "Todos em aberto",
      ...
      const { workbook, sheet: ws, linhaInicio } = await criarRelatorioTorg({
        ...
        subtitulo,
        nomePlanilha: "Contas a Receber",
        codigoDoc: "REL-FIN-002",
        totalColunas,
        kpis: [
          `Total a receber: ${fmtMoeda(totais.total)} (${totais.qtd} títulos)   |   Vencidas: ${fmtMoeda(totais.vencidas)} (${totais.qtdVencidas})   |   Vence hoje: ${fmtMoeda(totais.venceHoje)} (${totais.qtdVenceHoje})   |   A vencer: ${fmtMoeda(totais.aVencer)} (${totais.qtdAVencer})`,
        ],
      });

      const colWidths = [12, 32, 15, 15, 15, 22, 12, 12, 12];
      ...
      filtradas.forEach((c, idx) => {
        somaValor += c.valor || 0; somaReceb += c.valorRecebido || 0; somaSaldo += c.saldo || 0;
        const valores = [
          fmtData(c.vencimento), c.cliente || "—",
          Number((c.valor || 0).toFixed(2)), Number((c.valorRecebido || 0).toFixed(2)), Number((c.saldo || 0).toFixed(2)),
          c.categoria || "—", c.nf || "—", c.os || "—",
          c.situacao + (c.parcial ? " (parcial)" : ""),
        ];
        const fontColors = c.situacao === "VENCIDA" ? { 8: "D32F2F" } : {};
        adicionarLinhaTabela(ws, row, valores, {
          fillColor: idx % 2 === 1 ? "F8FAFC" : undefined,
          alinhamento: { 2: "right", 3: "right", 4: "right", 8: "center" }, fontColors, fontSize: 9, rowHeight: 16,
        });
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-3xl font-extrabold text-torg-dark tracking-tight flex items-center gap-2">
            <ArrowUpCircle size={26} className="text-green-600" /> Contas a Receber
          </h2>
          <p className="text-sm text-torg-gray mt-1">
            Títulos a receber do Omie por vencimento.
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-3 flex-wrap">
        ...
        {[["abertos", "Em aberto"], ["hoje", "Hoje"], ["semana", "Esta semana"], ["mes", "Este mês"]].map(([p, label]) => (
      ...
      {data && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <KpiCard label="Total a receber" valor={totais.total} sub={`${totais.qtd} títulos`} cor="green"
          ...
          <KpiCard label="A receber" valor={totais.aVencer} sub={`${totais.qtdAVencer} títulos`} cor="torg-blue"
      ...
      {data && verResumo && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <ResumoCard titulo="Por cliente" icone={Building2} itens={resumoPor("cliente")} />
        </div>
      )}
      ...
      {loading && !data ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-50 text-sm text-torg-dark flex items-center justify-between flex-wrap gap-2">
            <span><strong>{filtradas.length}</strong> títulos · {fmtMoeda(totais.total)} a receber</span>
            <span className="text-xs text-torg-gray">{de || ate ? `Período ${fmtData(de + "T12:00")} → ${fmtData(ate + "T12:00")}` : "Todos em aberto"}</span>
          </div>
          {filtradas.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50/60 border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-3 py-2.5 text-left text-xs font-medium text-gray-500 uppercase">Cliente</th>
                    <th className="px-3 py-2.5 text-right text-xs font-medium text-gray-500 uppercase whitespace-nowrap">Valor</th>
                    <th className="px-3 py-2.5 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">Recebido</th>
                    <th className="px-3 py-2.5 text-right text-xs font-medium text-gray-500 uppercase whitespace-nowrap">A receber</th>
                    ...
                    <th className="px-3 py-2.5 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">OS</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtradas.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-3 py-2 text-gray-700 max-w-[200px] truncate" title={c.cliente}>{c.cliente}</td>
                      <td className="px-3 py-2 text-right tabular-nums text-gray-600 whitespace-nowrap">{fmtMoeda(c.valor)}</td>
                      <td className="px-3 py-2 whitespace-nowrap"><PctRecebido pct={c.pctRecebido} valor={c.valorRecebido} /></td>
                      <td className="px-3 py-2 text-right tabular-nums font-semibold text-torg-dark whitespace-nowrap">{fmtMoeda(c.saldo)}</td>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap"><OmieCell valor={c.os} copiar={c.os} rotulo="OS" /></td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        ...
                        {c.parcial && <span className="ml-1 text-[10px] text-amber-600">parcial</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
// Mostra a % recebida (barra + texto), com o valor recebido no tooltip.
function PctRecebido({ pct, valor }) {
  const p = Math.max(0, Math.min(100, pct || 0));
  const cor = p >= 100 ? "bg-green-500" : p > 0 ? "bg-amber-500" : "bg-gray-200";
  return (
    <div className="flex items-center gap-1.5 min-w-[78px]" title={`Recebido ${fmtMoeda(valor)} (${p}%)`}>
      ...
      <span className="text-[11px] tabular-nums text-gray-500">{p}%</span>
    </div>
  );
}

// Célula clicável de NF / OS: copia o número (ou a chave NFe) e abre o
// módulo de Vendas do Omie em nova aba, pra colar na busca.
function OmieCell({ valor, copiar, rotulo }) {
  ...
  const onClick = () => {
    ...
    window.open(OMIE_VENDAS_URL, "_blank", "noopener,noreferrer");
  };
  return (
