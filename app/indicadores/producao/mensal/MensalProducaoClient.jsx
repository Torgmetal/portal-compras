...
import { CalendarRange, Loader2, RefreshCw, Factory } from "lucide-react";
    ...
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-torg-gray mt-0.5">Produção <b>mês a mês</b> e no <b>acumulado do ano</b>, avaliada contra a meta de preparação de {kg(dados?.metaDiaKg || 6000)} kg/dia.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-12 text-center text-sm text-torg-gray"><Factory size={34} className="mx-auto text-gray-300 mb-2" /> Sem apontamentos em {ano}.</div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table className="w-full text-[13px]">
              <thead className="bg-gray-50/70 text-torg-gray">
                <tr>
                  ...
                  <th className="text-right px-4 py-2.5 font-medium">Produzido (kg)</th>
                  <th className="text-right px-4 py-2.5 font-medium">kg/dia (fábrica)</th>
                  <th className="text-right px-4 py-2.5 font-medium">Corte kg/dia</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {meses.map((m) => (
                  <tr key={m.mes} className="hover:bg-gray-50/40">
                    ...
                    <td className="px-4 py-2 text-right text-torg-gray">{kg(m.kgDiaCorte)}</td>
              ...
              {ac && (
                <tfoot>
                  <tr className="bg-torg-blue-50/60 border-t-2 border-torg-blue-100 font-semibold text-torg-dark">
                    ...
                    <td className="px-4 py-2.5 text-right">{kg(ac.kgDiaCorte)}</td>
          ...
          <p className="text-[11px] text-torg-gray px-4 py-2.5 border-t border-gray-50">A nota é o atingimento da meta de preparação (Corte kg/dia ÷ {kg(dados?.metaDiaKg || 6000)}). No mês corrente, a meta considera só os dias úteis já decorridos.</p>
        </div>
      )}
    </div>
  );
