...
export function filtrarProjetos(itens, { busca='', perfil='', material='', soFalta=false, colunas={} }={}) {
  return itens.filter(i => (!busca || norm([i.m,i.pf,i.mt].join(' ')).includes(norm(busca)))
    ...
    && (!soFalta || !i.g)
    && Object.entries(colunas).every(([campo,valores]) => valores == null || valores.includes(i[campo] || '__SEM__')));
}
