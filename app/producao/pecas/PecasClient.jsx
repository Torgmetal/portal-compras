...
export default function PecasClient({ ops, pecasIniciais, userRole }) {
  ...
  // Apontamento via planilha
  const [modalApontamento, setModalApontamento] = useState(false);
  const [apontamentoRows, setApontamentoRows] = useState(null);
  const [apontamentoFile, setApontamentoFile] = useState("");
  const [importandoApontamento, setImportandoApontamento] = useState(false);
  const [resultadoApontamento, setResultadoApontamento] = useState(null);
  ...
  // Baixar modelo de apontamento (planilha Excel com pecas pre-preenchidas)
  async function baixarModeloApontamento() {
    const dadosOp = filtroOp ? pecas.filter((p) => p.opNumero === filtroOp) : pecas;
    if (dadosOp.length === 0) {
      alert("Nenhuma peça encontrada. Selecione uma OP ou importe peças primeiro.");
      return;
    }

    const { workbook, sheet: ws, linhaInicio } = await criarRelatorioTorg({
      titulo: "Apontamento de Producao",
      subtitulo: filtroOp ? `OP ${filtroOp}` : "Todas as OPs",
      kpis: [`Modelo para preenchimento manual — preencha as colunas em amarelo`],
      totalColunas: 10,
      nomePlanilha: "Apontamento",
      codigoDoc: "MOD-PRD-001",
    });
    ...
    ws.columns = [
      ...
      { width: 8 },  // Qte Total
      { width: 10 }, // Peso Total
      { width: 12 }, // Qte Produzida
      { width: 12 }, // Peso Produzido
      { width: 14 }, // Data Producao
      { width: 14 }, // Setor
    ];

    let row = linhaInicio;
    const headers = ["OP", "Marca", "Descricao", "Material", "Qte Total", "Peso Total (kg)", "Qte Produzida", "Peso Produzido (kg)", "Data Producao", "Setor"];
    adicionarHeaderTabela(ws, row, headers);
    // Marcar colunas editaveis (7-10) com fundo amarelo no header
    for (let c = 7; c <= 10; c++) {
      const cell = ws.getCell(row, c);
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFD700" } };
      cell.font = { name: "Arial", size: 9, bold: true, color: { argb: "333333" } };
    }
    row++;

    // Agrupar por OP
    const opsOrdenadas = [...new Set(dadosOp.map((p) => p.opNumero))].sort();
    for (const op of opsOrdenadas) {
      const pecasOp = dadosOp.filter((p) => p.opNumero === op);
      for (const p of pecasOp) {
        adicionarLinhaTabela(ws, row, [
          ...
          p.qte || 1,
          p.pesoTotalKg ? Number(p.pesoTotalKg.toFixed(1)) : 0,
          "", // Qte Produzida — usuario preenche
          "", // Peso Produzido — usuario preenche
          "", // Data Producao — usuario preenche
          "", // Setor — usuario preenche
        ]);
        // Proteger colunas fixas (cinza claro) e destacar editaveis (amarelo claro)
        for (let c = 1; c <= 6; c++) {
          ws.getCell(row, c).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "F5F5F5" } };
          ws.getCell(row, c).font = { name: "Arial", size: 9, color: { argb: "666666" } };
        }
        for (let c = 7; c <= 10; c++) {
          ws.getCell(row, c).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFDE7" } };
        }
        row++;
      }
    }

    // Data validation para coluna Setor (dropdown)
    const setores = ["CORTE", "MONTAGEM", "SOLDA", "ACABAMENTO", "JATO", "PINTURA", "EXPEDIDO"];
    for (let r = linhaInicio + 1; r < row; r++) {
      ws.getCell(r, 10).dataValidation = {
        type: "list",
        allowBlank: true,
        formulae: [`"${setores.join(",")}"`],
        showErrorMessage: true,
        errorTitle: "Setor invalido",
        error: "Selecione um setor da lista.",
      };
      // Data validation para data (texto livre, mas formato sugerido)
      ws.getCell(r, 9).note = "Formato: dd/mm/aaaa";
    }

    // Legenda de instrucoes
    row += 2;
    adicionarLegenda(ws, row, [
      { cor: "FFFDE7", label: "Amarelo = preencher manualmente" },
      { cor: "F5F5F5", label: "Cinza = dados do portal (nao alterar)" },
    ], 10);
    row++;
    ws.mergeCells(row, 1, row, 10);
    const cellInstr = ws.getCell(row, 1);
    cellInstr.value = "Setores validos: CORTE, MONTAGEM, SOLDA, ACABAMENTO, JATO, PINTURA, EXPEDIDO  |  Data no formato dd/mm/aaaa";
    cellInstr.font = { name: "Arial", size: 8, italic: true, color: { argb: CORES.TORG_GRAY } };

    const nomeArquivo = `Torg_Modelo_Apontamento_${filtroOp || "Todas"}_${new Date().toISOString().split("T")[0]}.xlsx`;
    await downloadWorkbook(workbook, nomeArquivo);
  }

  // Ler planilha de apontamento do usuario
  function handleApontamentoFile(e) {
    const file = e.target.files?.[0];
    if (!file) return;
    setApontamentoFile(file.name);
    setResultadoApontamento(null);
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Mapear colunas (flexivel — aceita variantes)
        const rows = [];
        for (const r of json) {
          const opNumero = String(r["OP"] || r["op"] || r["Op"] || "").trim();
          const marca = String(r["Marca"] || r["marca"] || r["MARCA"] || "").trim();
          const setor = String(r["Setor"] || r["setor"] || r["SETOR"] || r["Status"] || r["status"] || "").trim().toUpperCase();
          const qteProduzida = Number(r["Qte Produzida"] || r["QteProduzida"] || r["qte_produzida"] || r["Produzido"] || 0);
          const pesoProduzido = Number(r["Peso Produzido (kg)"] || r["PesoProduzido"] || r["peso_produzido"] || r["Peso Produzido"] || 0);
          let dataProducao = r["Data Producao"] || r["DataProducao"] || r["data_producao"] || r["Data Produção"] || r["Data"] || "";
          if (dataProducao instanceof Date) {
            dataProducao = dataProducao.toLocaleDateString("pt-BR");
          } else if (typeof dataProducao === "number") {
            // Excel serial date
            const d = new Date((dataProducao - 25569) * 86400000);
            dataProducao = d.toLocaleDateString("pt-BR");
          }
          dataProducao = String(dataProducao).trim();

          if (!opNumero || !marca || !setor) continue;
          if (!STATUS_PIPELINE.includes(setor)) continue;
          if (setor === "PENDENTE") continue; // Nao faz sentido apontar como pendente

          rows.push({ opNumero, marca, setor, qteProduzida: Math.max(0, Math.round(qteProduzida)), pesoProduzido: Math.max(0, pesoProduzido), dataProducao: dataProducao || null });
        }
        setApontamentoRows(rows);
      } catch (err) {
        alert("Erro ao ler planilha: " + err.message);
        setApontamentoRows(null);
      }
    };
    reader.readAsArrayBuffer(file);
  }

  // Enviar apontamento para API
  async function enviarApontamento() {
    if (!apontamentoRows || apontamentoRows.length === 0) return;
    setImportandoApontamento(true);
    try {
      const res = await fetch("/api/producao/importar-apontamento", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rows: apontamentoRows }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `Erro ${res.status}`);
      setResultadoApontamento(data);
      if (data.resumo?.atualizados > 0) {
        router.refresh();
      }
    } catch (e) {
      setResultadoApontamento({ error: e.message });
    } finally {
      setImportandoApontamento(false);
    }
  }
    ...
    <div className="space-y-4 max-w-7xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            onClick={baixarModeloApontamento}
            className="px-3 py-1.5 bg-amber-50 text-amber-700 text-xs rounded-lg hover:bg-amber-100 font-medium flex items-center gap-1.5 border border-amber-200"
          >
            <Download size={14} /> Modelo Apontamento
          </button>
          <button
            onClick={() => { setModalApontamento(true); setApontamentoRows(null); setResultadoApontamento(null); setApontamentoFile(""); }}
            className="px-3 py-1.5 bg-purple-600 text-white text-xs rounded-lg hover:bg-purple-700 font-medium flex items-center gap-1.5"
          >
            <FileSpreadsheet size={14} /> Importar Apontamento
          </button>
      ...
      {/* Modal Importar Apontamento via Planilha */}
      {modalApontamento && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !importandoApontamento && setModalApontamento(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-torg-dark">Importar Apontamento via Planilha</h3>
              </div>
              <button onClick={() => !importandoApontamento && setModalApontamento(false)} className="text-torg-gray hover:text-torg-dark">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
              {!resultadoApontamento && (
                <>
                  <div className="bg-purple-50 rounded-xl p-3 text-xs text-purple-800">
                    <p className="font-medium">Importe uma planilha com dados de apontamento de producao.</p>
                    <p className="mt-1 text-purple-600">
                      Use o botao "Modelo Apontamento" para baixar a planilha modelo com as pecas pre-preenchidas.
                      Preencha as colunas <strong>Qte Produzida</strong>, <strong>Peso Produzido</strong>, <strong>Data Producao</strong> e <strong>Setor</strong>.
                    </p>
                  </div>

                  <div>
                    <label className="text-xs font-semibold text-torg-dark block mb-2">Selecione o arquivo</label>
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept=".xlsx,.xls,.csv"
                      onChange={handleApontamentoFile}
                    ...
                    />
                    {apontamentoFile && (
                      <p className="text-[11px] text-torg-gray mt-1">Arquivo: {apontamentoFile}</p>
                    )}
                  </div>

                  {/* Preview das linhas parseadas */}
                  {apontamentoRows && apontamentoRows.length > 0 && (
                    <div className="border border-gray-200 rounded-xl overflow-hidden">
                      <div className="px-3 py-2 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                        <p className="text-[11px] font-semibold text-torg-dark uppercase tracking-wide">
                          Pre-visualizacao
                        </p>
                        <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">
                          {apontamentoRows.length} linha{apontamentoRows.length !== 1 ? "s" : ""} valida{apontamentoRows.length !== 1 ? "s" : ""}
                        </span>
                      </div>
                      <div className="overflow-x-auto max-h-[280px] overflow-y-auto">
                        <table className="w-full text-[11px]">
                          <thead className="bg-gray-50/80 sticky top-0">
                            <tr>
                              ...
                              <th className="px-2 py-1.5 text-right font-medium text-gray-500">Qte Prod.</th>
                              <th className="px-2 py-1.5 text-right font-medium text-gray-500">Peso Prod.</th>
                              <th className="px-2 py-1.5 text-left font-medium text-gray-500">Data</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-50">
                            {apontamentoRows.slice(0, 100).map((r, i) => (
                              <tr key={i} className="hover:bg-gray-50">
                                <td className="px-2 py-1 font-mono text-torg-blue">{r.opNumero}</td>
                                <td className="px-2 py-1 font-mono font-semibold text-torg-dark">{r.marca}</td>
                                <td className="px-2 py-1">
                                  <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${STATUS_COR[r.setor]?.bg || "bg-gray-100"} ${STATUS_COR[r.setor]?.text || "text-gray-600"}`}>
                                    {STATUS_LABEL[r.setor] || r.setor}
                                  </span>
                                </td>
                                <td className="px-2 py-1 text-right tabular-nums text-torg-dark font-semibold">{r.qteProduzida}</td>
                                <td className="px-2 py-1 text-right tabular-nums text-torg-gray">
                                  {r.pesoProduzido > 0 ? `${r.pesoProduzido.toFixed(1)} kg` : "—"}
                                </td>
                                <td className="px-2 py-1 text-torg-gray font-mono">{r.dataProducao || "—"}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        {apontamentoRows.length > 100 && (
                          <p className="text-center text-[10px] text-torg-gray py-2">
                            Mostrando 100 de {apontamentoRows.length} linhas...
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {apontamentoRows && apontamentoRows.length === 0 && (
                    <div className="bg-orange-50 rounded-xl p-3 text-xs text-orange-700 flex items-center gap-2">
                      <AlertCircle size={14} />
                      Nenhuma linha valida encontrada. Verifique se a planilha segue o modelo correto.
                    </div>
                  )}
                </>
              )}
              ...
              {resultadoApontamento && !resultadoApontamento.error && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-sm font-bold text-torg-dark">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div className="bg-emerald-50 rounded-xl p-3 text-center">
                      <p className="text-[10px] font-medium uppercase tracking-wide text-emerald-600">Atualizados</p>
                      <p className="text-2xl font-extrabold text-emerald-700 tabular-nums">{resultadoApontamento.resumo.atualizados}</p>
                    </div>
                    <div className="bg-orange-50 rounded-xl p-3 text-center">
                      <p className="text-[10px] font-medium uppercase tracking-wide text-orange-600">Nao Encontrados</p>
                      <p className="text-2xl font-extrabold text-orange-700 tabular-nums">{resultadoApontamento.resumo.naoEncontrados}</p>
                    </div>
                    <div className="bg-red-50 rounded-xl p-3 text-center">
                      <p className="text-[10px] font-medium uppercase tracking-wide text-red-600">Erros</p>
                      <p className="text-2xl font-extrabold text-red-700 tabular-nums">{resultadoApontamento.resumo.erros}</p>
                    </div>
                  </div>

                  {/* Detalhes */}
                  {resultadoApontamento.resultados?.length > 0 && (
                    <div className="border border-gray-200 rounded-xl overflow-hidden">
                      <div className="px-3 py-2 bg-gray-50 border-b border-gray-200">
                        <p className="text-[11px] font-semibold text-torg-dark uppercase tracking-wide">Resultados</p>
                      </div>
                      <div className="overflow-x-auto max-h-[200px] overflow-y-auto">
                        <table className="w-full text-[11px]">
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {resultadoApontamento.resultados.map((r, i) => (
                              <tr key={i}>
                                <td className="px-2 py-1 font-mono text-torg-blue">{r.op}</td>
                                <td className="px-2 py-1 font-mono font-semibold text-torg-dark">{r.marca}</td>
                                <td className="px-2 py-1">
                                  <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${
                                    r.status === "ATUALIZADO" ? "bg-emerald-100 text-emerald-700" :
                                    r.status === "NAO_ENCONTRADA" ? "bg-orange-100 text-orange-700" :
                                    "bg-red-100 text-red-700"
                                  }`}>
                                    {r.status === "ATUALIZADO" ? `Atualizado → ${STATUS_LABEL[r.setor] || r.setor}` :
                                     r.status === "NAO_ENCONTRADA" ? "Nao encontrada" :
              ...
              {/* Erro */}
              {resultadoApontamento?.error && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs text-red-700">
                  <p className="font-medium flex items-center gap-1.5">
                    <AlertCircle size={14} /> Erro na importacao
                  </p>
                  <p className="mt-1">{resultadoApontamento.error}</p>
                </div>
              )}
            </div>
            ...
            <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => { setModalApontamento(false); setApontamentoRows(null); setResultadoApontamento(null); }}
                  disabled={importandoApontamento}
                  className="px-4 py-2 text-sm text-torg-gray hover:text-torg-dark disabled:opacity-50"
                >
                  {resultadoApontamento ? "Fechar" : "Cancelar"}
                </button>
                {!resultadoApontamento && apontamentoRows && apontamentoRows.length > 0 && (
                  <button
                    onClick={enviarApontamento}
                    disabled={importandoApontamento}
                    className="px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 font-medium flex items-center gap-1.5 disabled:opacity-50"
                  >
                    {importandoApontamento ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}
                    Importar {apontamentoRows.length} linha{apontamentoRows.length !== 1 ? "s" : ""}
                  </button>
                )}
                {resultadoApontamento && !resultadoApontamento.error && (
                  <button
                    onClick={() => { setModalApontamento(false); setApontamentoRows(null); setResultadoApontamento(null); router.refresh(); }}
                    className="px-4 py-2 bg-torg-blue text-white text-sm rounded-lg hover:bg-torg-blue-700 font-medium"
                  >
                    Atualizar Pagina
                  </button>
