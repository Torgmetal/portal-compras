...
export default function PecasInformadasEditor({ pecas, onChange, disabled, quantidadesLista = {} }) {
  ...
  return <fieldset disabled={disabled} className="mt-2 min-w-0 max-w-2xl">
    <datalist id="marcas-lista-inspecao">{Object.keys(quantidadesLista).map(marca => <option key={marca} value={marca} />)}</datalist>
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-auto max-h-80">
      <table className="w-full text-sm"><thead className="bg-gray-50 sticky top-0"><tr><th className="text-left p-2">Marca</th><th className="text-left p-2">Quantidade</th><th><span className="sr-only">Remover</span></th></tr></thead>
        <tbody className="divide-y divide-gray-50">{pecas.map((p, i) => <tr key={i}>
          <td className="p-2"><input aria-label={`Marca ${i + 1}`} value={p.marca} maxLength={40} list="marcas-lista-inspecao" onChange={e => onChange(pecas.map((p, n) => n === i ? { marca: e.target.value, quantidade: quantidadesLista[e.target.value.trim().toUpperCase()] || "" } : p))} className="w-full min-w-32 border rounded-lg px-2 py-1.5 font-mono" /></td>
    ...
    <p className="text-xs text-torg-gray mt-2">As quantidades vêm das listas da OP. Altere apenas se a quantidade inspecionada for diferente e clique em Salvar. Marcas sem quantidade na lista ficam em branco.</p>
  </fieldset>;
}
