...
export default function AtaDetalheClient({ id }) {
  ...
  async function confirmarRecebimento() {
    setSalvando(true);
    try {
      const r = await fetch(`/api/reunioes/${id}/responder`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ acao: "confirmar" }) });
    ...
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          {gerente && !isRascunho && <button onClick={() => setModalRev(true)} className="px-3 py-1.5 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 text-torg-dark inline-flex items-center gap-1.5"><Pencil size={14} /> Revisar</button>}
          {gerente && isRascunho && <button onClick={enviar} disabled={salvando} className="px-4 py-1.5 bg-torg-blue text-white text-sm rounded-lg hover:bg-torg-dark font-medium inline-flex items-center gap-1.5 disabled:opacity-50">{salvando ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />} Enviar aos envolvidos</button>}
          {gerente && !isRascunho && <button onClick={enviar} disabled={salvando} className="px-3 py-1.5 border border-torg-blue text-torg-blue text-sm rounded-lg hover:bg-torg-blue-50 font-medium inline-flex items-center gap-1.5 disabled:opacity-50"><Send size={14} /> Reenviar</button>}
          {gerente && <button onClick={excluir} disabled={salvando} className="px-2.5 py-1.5 text-sm border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-200 hover:text-red-600 text-torg-gray"><Trash2 size={14} /></button>}
        </div>
      </div>
      ...
      {/* Aceite do recebimento pelo portal (ISO) — pra quem é envolvido e ainda não confirmou */}
      {!isRascunho && eu?.envolvido && !eu?.confirmadoEm && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-center gap-3 flex-wrap">
          <AlertCircle size={16} className="text-amber-600 flex-shrink-0" />
          <span className="text-sm text-amber-800 flex-1">Você está entre os envolvidos e ainda não confirmou o recebimento desta ata.</span>
          <button onClick={confirmarRecebimento} disabled={salvando} className="px-3.5 py-1.5 bg-amber-600 text-white text-sm rounded-lg hover:bg-amber-700 font-medium inline-flex items-center gap-1.5 disabled:opacity-50">{salvando ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Confirmar recebimento</button>
        </div>
      )}
...
function EnvolvidosView({ ata, confMap, onFlash, gerente }) {
        ...
        {envolvidos.map((e, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 text-[13px] py-1.5 border-b border-gray-50 last:border-0">
              ...
              {gerente && c?.token && <button onClick={() => copiar(c.token, e.email)} title="Copiar link da ata" className="text-gray-400 hover:text-torg-blue">{copiado === e.email ? <Check size={14} className="text-emerald-500" /> : <Link2 size={14} />}</button>}
            </div>
          );
        })}
      </div>
                        ...
                        {jaResp && !aberto && (
                          <div className="mt-3 pt-3 border-t border-gray-200/60 text-[13px] space-y-1.5">
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                              ...
                              <button onClick={() => abrirEdicao(a)} className="text-[11px] text-torg-blue hover:text-torg-dark font-medium underline">editar resposta</button>
                            </div>
                          </div>
                        )}
