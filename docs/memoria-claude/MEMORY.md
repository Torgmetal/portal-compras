...
- [Posição de estoque por local (Omie)](torg_omie_posicao_estoque.md) — sem filtro o `ListarPosEstoque` é SÓ o Almoxarifado; "TODOS" = linha por (produto, local); locais em `estoque/local/`; CMC por local; sem `cUnidade`; 100/página; Qtd = Almox+Fábrica (decisão pendente); a tela cortava em 1.000 de 2.500
