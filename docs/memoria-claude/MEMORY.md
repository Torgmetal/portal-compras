...
- [Filtrar depois de cortar esconde dado](torg_filtro_antes_do_corte.md) — `take:100` + filtro no cliente escondia 111 RMs e 11 obras SUMIAM do seletor; obra na URL filtra no banco sem teto, opções por `groupBy`, corte visível; `lib/rms-servico.js` ainda tem o mesmo padrão
