...
- [Relatório de ultrassom (RUS)](torg_relatorio_us.md) — tabela = indicação REPROVADA (15.1) + toda peça ensaiada (sem indicação → linha própria, laudo A); tela grava `marca`/`comprimento`, o PDF traduz; cabeçalho inteiro editável nas duas telas (`CAMPOS_CABECALHO_US`, listas = sugestão)
- [Data book: só relatório assinado](torg_databook_relatorio_assinado.md) — entra na ÚLTIMA assinatura; rodada reprovada e assinada acompanham, intermediária não; anexar à mão barrado; 13 vínculos tirados em 25/09
...
- [Posição de estoque por local (Omie)](torg_omie_posicao_estoque.md) — sem filtro o `ListarPosEstoque` é SÓ o Almoxarifado; "TODOS" = linha por (produto, local); locais em `estoque/local/`; CMC por local; sem `cUnidade`; 100/página; Qtd = Almox+Fábrica+Terceiro (Vitor, 26/09), patrimônio fora; a tela cortava em 1.000 de 2.500
...
- [Cadastro fiscal pelos produtos comprados](torg_omie_cadastro_fiscal.md) — imposto NÃO fica no produto (Cenário de Impostos); API grava NCM/CEST/origem/tipo/peso/família; 7306.30.00 NÃO existe; cantoneira ≥80 mm = 7216.40.10; CFOP de entrada não prova o tipo; escrita no Omie travada pelo modo automático
...
- [Rastreabilidade — o R por peça](torg_rastreio_corrida.md) — o R manda (puxa corrida/cert/NF); FIFO pela entrega mais antiga; só peça CORTADA ganha R; armadilha da chapa sem "ESPESSURA"; **dia** do recebimento conta (diaBRT) e corte provado pela etapa seguinte (`corteInferido`, 24/09); §04 do data book é retrato do "puxar" — os R da §02 têm de estar na §04 (OP-102, 25/09)
- [Certificado novo entra sozinho no data book](torg_databook_certificados_auto.md) — cron data-book vincula na §04/§05/§15 o que chegou DEPOIS da montagem (4 travas); o "puxar" era retrato do clique
...
- [Simulador de carga no portal](torg_simulador_carga_portal.md) — "Simular carga" no romaneio prévio; motor lib/carga em Web Worker; IFC medido no navegador; CargaSimulada com itensHash; **empilha pelo AÇO real** (caibro sobre aço, calço ≤15 cm; nem caixa envolvente, nem topoVazado — 24/09); **pacotes ≤1,14 m em fileiras com caibro entre elas, aço sempre sobre caibro no assoalho**; carga encostada na cabeceira (x cresce da cabine para trás); **travamento por volume** (vão >30 cm: escorar até 1,5 m, senão amarrar; cinta = fita, não placa); cabeceira com altura (graneleira 1,8 m, editável na Configuração da expedição; Res. CONTRAN 945/2022 art. 8º)
...
- [⚠ Busca do Graph = HTTP 500 desde 22/09](torg_graph_busca_500.md) — derrubou 6 pontos, só o cron do LQC avisava; TODOS corrigidos por `lib/sharepoint-arvore.js` (listar, nunca buscar); delta e varredura cega NÃO servem; revelou que o modelo em branco da LQC era escolhido por data
- [Cron sync-sharepoint do PCP: REMOVIDO 26/09](torg_sync_sharepoint_pcp.md) — abandonado (aba EAP congelada em junho, 87 dias velho); ⚠ `/producao/planejamento-semanal` é o PMP e FICOU; retry do Graph do CMR também ficou
...
- [Codex: aceito sempre](torg_codex_aceito_sempre.md) — pendência de terceiro/decisão humana não trava; defeito do Codex em código nosso: corrigir DIRETO, sem pedir autorização (24/09); P1 dos avulsos é do Vitor
...
- [Assistente Fiscal TORG](torg_assistente_fiscal.md) — chat com IA na Inteligência Fiscal; 3 camadas impedem invenção (lista fechada de ferramentas, blocos do servidor, conferirProsa); sem embeddings (298 dispositivos/37k tokens) com piso de relevância 4 medido; reserva de orçamento atômica provada 12→4; chave da Anthropic não vem pelo `vercel env pull`
- [Etapa com data futura](torg_etapa_data_futura.md) — 7 de 10 lançamentos eram "liberado para coleta" futuro; agora aparece como "coleta prevista"; "material recebido" futuro não conta como chegada (tirava da cobrança); lançar etapa NÃO remarca a previsão
- [Status da RM após pedido/estoque](torg_rm_status_apos_pedido.md) — sobrou só COTADO = pronta pra pedido; sobrou sem proposta = ABERTA; rota da OP recalcula só RMs com pedido gravado
- [Inteligência Fiscal enxuta](torg_fiscal_simples.md) — imposto da obra = OPReceita; IBS/CBS do ListarNF por NCM×CFOP (sem UF); cron soma só ontem; ipiDaTipi não tem .aliquota
