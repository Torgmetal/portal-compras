...
No recebimento, escolha uma vez o **Destino de todo o retorno**. Use **Selecionar todas as marcas** ou marque somente as que chegaram; a importação também seleciona as marcas conferidas. Confira a quantidade recebida. Somente nas exceções, use **Separar para outros setores** e informe as parcelas: o restante segue automaticamente para o destino geral. Ao trocar o destino geral, as exceções são preservadas; um conflito de setores precisa ser corrigido antes da confirmação. O servidor também verifica a soma por marca contra o saldo da remessa e recusa setores repetidos para a mesma marca. Cada distribuição mantém seu destino e lote de programação independente.
