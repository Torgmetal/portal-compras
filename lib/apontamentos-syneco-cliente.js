...
export async function baixarPlanilhaApontamentosSyneco({ opId = null, setor = null, nome = "" } = {}) {
  ...
  const wb = await criarExcelTabular({
    ...
    subtitulo: `${nome ? nome + " · " : ""}${j.total.pecas} peça(s) em ${j.total.linhas} marca(s) · gerado em ${DATA_BR(j.geradoEm)} — só baixa feita pelo setor; romaneio, terceiro e fechamento administrativo não entram`,
