 ...
 */
/*
 * ⚠⚠ DOIS SETORES NUNCA TÊM LINHA, E ISSO PARECIA PLANILHA QUEBRADA. Geraldo (18/09/2026) gerou a
 * planilha no posto dele e "não tem nada". Não estava quebrada:
 *   · PINTURA é a ÚLTIMA etapa da rota — não existe apontamento adiante para provar que a peça
 *     passou por ela, então ela jamais é cobrada aqui.
 *   · ACABAMENTO é opcional (a peça pode ir do Jato à Pintura sem passar), e por isso nunca é alvo.
 * Uma aba em branco não distingue "não há o que lançar" de "o portal falhou". Agora ela diz qual dos
 * dois é, com o nome do setor.
 */
export function motivoDoVazio(setorSyneco) {
  const s = String(setorSyneco || "").trim();
  if (!s) return "Nada a lançar: o Syneco está em dia com o que o portal sabe.";
  if (s === "Pintura") return "A Pintura é a última etapa da rota. Não existe apontamento adiante que prove a passagem por ela, então ela nunca aparece nesta lista — isto não é falha da planilha.";
  if (s === "Acabamento") return "O Acabamento é etapa opcional: a peça pode ir do Jato à Pintura sem passar por ele. Por isso ele nunca é cobrado nesta lista — isto não é falha da planilha.";
  return `Nada a lançar na ${s} agora: todo apontamento que o Syneco já tem está coerente com as etapas anteriores.`;
}

export function planilhaApontamentos({ portal = [], atras = [], nome = "", geradoEm = "", setorSyneco = "" } = {}) {
  const abas = [];
  ...
  if (!abas.length) {
    const motivo = motivoDoVazio(setorSyneco);
    abas.push({ nome: "Nada a lançar", subtitulo: motivo, headers: ["Situação"], linhas: [[motivo]], larguras: [110] });
  }
