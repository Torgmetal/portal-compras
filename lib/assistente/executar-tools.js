...
export async function executarTool(nome, args, user = null) {
  try {
    ...
    switch (nome) {
      ...
      case "listar_tarefas_planejamento":  return await listarTarefasPlanejamento(args);
...
async function listarTarefasPlanejamento({ setor, status, termo, semana, ano, limite = 20 }) {
  const where = {};
  ...
  if (tarefas.length === 0) return { mensagem: "Nenhuma tarefa encontrada com esses filtros." };
