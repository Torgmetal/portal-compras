...
const registro = log("executar-tools");
...
export async function executarTool(nome, args, user = null) {
  try {
    ...
    registro.erro(`[assistente] Erro na tool ${nome}:`, e.message);
...
async function consultarMesProducao({ obra, de, ate, setor }) {
  ...
  // ⚠⚠ O PESO DA OBRA NÃO É A SOMA DOS SETORES: a mesma peça é apontada em cada setor por onde
  // passa. Esta ferramenta devolvia `totalKg` somando todos — exatamente o número que o prompt do
  // Torguinho proíbe usar. Agora vai o kg do setor mais avançado com apontamento.
  const porObra = {};
  for (const g of grupos) {
    if (!porObra[g.obra]) porObra[g.obra] = { obra: g.obra, setores: [] };
  ...
  return {
    obras: Object.values(porObra).map(comPesoProduzido),
    totalApontamentos: totalApont,
  };
}

// Setores do Syneco na ordem da rota (Corte → … → Pintura), como `SETOR_SYNECO` os declara.
const ORDEM_MES = Object.values(SETOR_SYNECO).map((s) => s.toLowerCase());
const posicaoNoFluxo = (setor) => ORDEM_MES.indexOf(String(setor || "").trim().toLowerCase());

function comPesoProduzido(obra) {
  const setores = [...obra.setores].sort((a, b) => posicaoNoFluxo(a.setor) - posicaoNoFluxo(b.setor));
  const referencia = [...setores].reverse().find((s) => s.kg > 0 && posicaoNoFluxo(s.setor) >= 0) || null;
  return {
    obra: obra.obra,
    pesoProduzidoKg: referencia ? referencia.kg : 0,
    setorReferencia: referencia ? referencia.setor : null,
    setores,
  };
}
...
async function concluirTarefaPlanejamento({ tarefaId, termo, setor, observacao }, user) {
  ...
  } catch (e) { registro.erro("[torguinho] aviso de conclusão falhou:", e?.message); }
