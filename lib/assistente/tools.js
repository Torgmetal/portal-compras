...
export const TODAS_TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        status:  { type: "string",  enum: ["ABERTA", "EM_EXECUCAO", "ENCERRADA", "CANCELADA", "ATRASADA"], description: "Status da OP." },
  ...
  },
  {
    ...
    description:
      "Consulta dados de produção do MES (SKA Syneco) por obra: kg, unidades e número de " +
      "apontamentos em cada setor (na ordem do fluxo), e o peso produzido da obra — " +
      "pesoProduzidoKg, o kg do setor mais avançado com apontamento (setorReferencia). " +
      "O kg de setores diferentes não se soma: a mesma peça é apontada em cada setor por onde passa. " +
      "Use para perguntas sobre produção, peso produzido e apontamentos.",
    input_schema: {
      type: "object",
      properties: {
  ...
  // A camada lib/assistente/data-access.js controla o que cada usuário pode ler.
  {
    ...
    description:
      "Lista as tabelas/entidades do portal que você pode consultar, com uma descrição curta de cada. " +
      "Use quando a pergunta envolver dados que as ferramentas específicas não cobrem, ou quando " +
  ...
  },
  {
    ...
    description:
      "Consulta genérica de LEITURA em qualquer tabela liberada do portal: filtra, ordena, inclui relações " +
      "e limita resultados. Serve para o que as ferramentas específicas não cobrem, inclusive cruzando módulos. " +
      "Os filtros usam a sintaxe do Prisma (where). " +
      "Ex: { modelo: 'CronogramaTarefa', filtros: { dataFimPrevista: { lt: '2026-06-10' }, percentualRealizado: { lt: 100 } }, relacionar: { cronograma: { include: { op: true } } } }. " +
