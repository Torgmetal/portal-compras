...
export async function gerarAtaPDF(ata) {
  ...
  for (const [op, itens] of grupos) {
    ...
    const prog = `${nOk}/${itens.length} respondidas`;
    ...
    for (const a of itens) {
      ...
      // meta: setor · responsável · prazo
      const metaAtv = [sl(a.setor), a.responsavel || null, a.prazo ? `prazo ${fmtD(a.prazo)}` : null].filter(Boolean).join("  ·  ");
      txt(metaAtv, M + 8, y - 8, { size: 7.5, color: GRAY });
      // selo de status (à direita, na linha da descrição)
      const selo = done ? "RESPONDIDO" : "PENDENTE";
      txt(selo, A4[0] - M - 8 - wid(selo, bold, 7), topo - 9, { f: bold, size: 7, color: done ? GREEN : AMBER });
      y -= 11;
      ...
      // barra lateral do bloco
      page.drawRectangle({ x: M, y: y + 2, width: 2.5, height: topo - y - 2, color: done ? GREEN : LINE });
      y -= 8;
    }
    y -= 6;
  }
