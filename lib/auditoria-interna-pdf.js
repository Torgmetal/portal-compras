...
export async function gerarAuditoriaInternaPDF(a) {
  ...
  // Fotos: baixa e embute (JPG ou PNG), montando uma grade de 2 colunas.
  ...
  if (a.finalizadoEm) { espaco(18); txt(`Relatório finalizado em ${fmtD(a.finalizadoEm)} — ações do plano concluídas.`, M, y - 9, { f: bold, size: 8.5, color: GREEN }); y -= 17; }
  ...
  const xNum = M + 12;   // número — depois da barra colorida (evita a barra comer o dígito)
  const xBody = M + 32;  // rótulo do tipo + descrição, alinhados numa coluna só
  consts.forEach((c, i) => {
    ...
    txt(rot, xBody, y - 9, { f: bold, size: 8, color: cor });
    y -= 14;
  ...
  if (acoes.length) {
    ...
    for (const ac of acoes) {
      ...
      const respLines = ac.resposta ? wrap(`Resposta: ${ac.resposta}`, font, 8, W - 12) : [];
      const nEvid = Array.isArray(ac.evidencias) ? ac.evidencias.length : 0;
      espaco(oqueLines.length * 11 + 12 + respLines.length * 10 + (nEvid ? 10 : 0) + 10);
      ...
      // status da ação + resposta/evidência do responsável (acompanhamento)
      txt(done ? `• Concluída${ac.respondidoEm ? ` em ${fmtD(ac.respondidoEm)}` : ""}` : "• Em aberto", cAcao + 6, y - 8, { f: bold, size: 7.5, color: done ? GREEN : AMBER }); y -= 11;
      for (const ln of respLines) { txt(ln, cAcao + 6, y - 8, { size: 8, color: DARK }); y -= 10; }
      if (nEvid) { txt(`${nEvid} evidência(s) anexada(s) no portal`, cAcao + 6, y - 8, { size: 7.5, color: GRAY }); y -= 10; }
  ...
  pages.forEach((p, i) => {
    ...
    p.drawText(san(`${numRAI(a.numero)} · Torg Metal · auditoria interna${a.divulgadoEm ? ` · emitido em ${fmtDT(a.divulgadoEm)}` : ""}${a.finalizadoEm ? ` · finalizado em ${fmtDT(a.finalizadoEm)}` : ""} · documento controlado (ISO)`), { x: M, y: 19, size: 7, font, color: GRAY });
    const pg = `${i + 1}/${pages.length}`;
    p.drawText(pg, { x: A4[0] - M - font.widthOfTextAtSize(pg, 7), y: 19, size: 7, font, color: GRAY });
  });

  const bytes = await pdf.save();
  return { bytes, filename: nomeFORM(31, numRAI(a.numero)) };
}
