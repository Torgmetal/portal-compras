...
export async function gerarAuditoriaPortalPDF(a, { portalUrl, docBase } = {}) {
  ...
  // Link clicável (abre o portal, com ?doc pra pular direto no documento).
  const addLink = (pg, x1, y1, x2, y2, url) => {
    try {
      const annot = pdf.context.obj({
        Type: "Annot", Subtype: "Link", Rect: [x1, y1, x2, y2], Border: [0, 0, 0],
        A: pdf.context.obj({ Type: "Action", S: "URI", URI: PDFString.of(url) }),
      });
      pg.node.addAnnot(pdf.context.register(annot));
    } catch { /* sem link se a versão do pdf-lib não suportar */ }
  };
  const linhaDoc = (d) => {
    const lines = wrap(d.nome, font, 8.5, W - 44);
    espaco(11 * lines.length);
    page.drawCircle({ x: M + 24, y: y + 3, size: 1.5, color: ORANGE });
    const url = docBase && d.id ? `${docBase}/${encodeURIComponent(d.id)}?inline=1` : null;
    const cor = url ? BLUE : GRAY;
    for (let i = 0; i < lines.length; i++) {
      txt(lines[i], M + 32, y, { size: 8.5, color: cor });
      if (url) addLink(page, M + 30, y - 2, M + 32 + font.widthOfTextAtSize(san(lines[i]), 8.5), y + 9, url);
      y -= 11;
    }
  };
  ...
  // Documentos publicados, agrupados pelas seções criadas pelo usuário (itensAdicionais);
  // docs sem seção correspondente vão em "Outros documentos".
  const docs = (a.documentos || []).filter((d) => d.tipo === "EVIDENCIA" && d.publicar);
  const secoes = a.itensAdicionais || [];
  const secaoIds = new Set(secoes.map((s) => s.id));
  const grupos = secoes.map((s) => ({ titulo: s.titulo || "Documentos", descricao: s.descricao || null, docs: docs.filter((d) => d.requisito === s.id) })).filter((g) => g.docs.length);
  const outros = docs.filter((d) => !d.requisito || !secaoIds.has(d.requisito));
  if (outros.length) grupos.push({ titulo: "Outros documentos", docs: outros });

  for (const g of grupos) {
    cabSecao(g.titulo);
    if (g.descricao) {
      for (const ln of wrap(g.descricao, font, 8.5, W - 16)) { espaco(11); txt(ln, M + 8, y, { size: 8.5, color: GRAY }); y -= 11; }
      y -= 2;
    }
    for (const d of g.docs) linhaDoc(d);
    y -= 4;
  }
  if (!grupos.length) { espaco(16); txt("Nenhum documento publicado ainda.", M, y, { size: 9, color: GRAY }); }
