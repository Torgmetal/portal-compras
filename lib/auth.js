...
import { minutosDeBloqueio, registrarFalha, limparFalhas, ehSenhaDeCadastro } from "./login-tentativas";
...
//
// Portal interno: 12 horas, como sempre foi.
// Portal Qualidade Fábrica: 7 dias. Vitor (21/08/2026): "pode deixar o login de 7 dias sem
// problema". É celular de chão de fábrica — pedir senha todo dia, de luva, é o tipo de atrito que
// faz o inspetor voltar pro papel.
//
// ⚠ Só ganha os 7 dias quem tem SOMENTE o módulo de campo (os 5 inspetores). Quem também acessa o
// portal interno continua em 12 horas — senão a sessão longa do celular esticaria junto a do
// computador, que é onde estão comercial, financeiro e RH.
const HORAS_12 = 60 * 60 * 12;
const DIAS_7 = 60 * 60 * 24 * 7;

function duracaoDaSessao(token) {
  if (!token || token.tipo === "ADMIN") return HORAS_12;
  const mods = Array.isArray(token.modulos) ? token.modulos : [];
  const soCampo = mods.length > 0 && mods.every((m) => m === "QUALIDADE_CAMPO");
  return soCampo ? DIAS_7 : HORAS_12;
}

export const authOptions = {
  session: {
    strategy: "jwt",
    // teto do COOKIE (o JWT por dentro pode expirar antes — ver duracaoDaSessao)
    maxAge: DIAS_7,
  },
  jwt: {
    // O maxAge do `session` vale pro cookie; aqui é a validade real do token, por perfil.
    async encode({ token, secret }) {
      return jwtEncode({ token, secret, maxAge: duracaoDaSessao(token) });
    },
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // ⚠⚠ TRAVA ANTES DO BCRYPT. Comparar hash custa ~100ms de CPU — é esse custo que o ataque
        // de força bruta quer nos fazer pagar em escala. Conta trancada nem chega a comparar.
        // A contagem é POR CONTA (ver lib/login-tentativas.js): a fábrica inteira sai por um IP só.
        if (minutosDeBloqueio(user) > 0) return null;
        ...
        await limparFalhas(user);

        // Troca forçada na senha provisória (1º acesso) — a flag é setada ao habilitar acesso ou
        // resetar, e limpa quando a pessoa define a própria senha.
        //
        // ⚠ E TAMBÉM quando a senha digitada É a de cadastro ("Primeiro@2026!"). Vitor (29/08/2026):
        // "as contas que estiverem com as senhas iniciais vamos alterar". Quem nunca trocou entra
        // uma última vez com ela e sai obrigado a definir uma senha de verdade. A conferência é
        // com o texto que a pessoa acabou de digitar, não com o hash guardado.
        const naCadastro = ehSenhaDeCadastro(user, senha);
        const deveTrocarSenha = !!user.deveTrocarSenha || naCadastro;
        if (naCadastro && !user.deveTrocarSenha) {
          await prisma.user.update({ where: { id: user.id }, data: { deveTrocarSenha: true } }).catch(() => {});
        }
        ...
        return {
          ...
          podeCancelarRM: user.podeCancelarRM ?? false,
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.podeCancelarRM = user.podeCancelarRM ?? false;
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.podeCancelarRM = token.podeCancelarRM ?? false;
