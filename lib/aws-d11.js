 ...
 */
/** "só em estruturas ciclicamente carregadas e conexões tubulares" — em texto curto. */
export const ONDE_VALE = (criterio) =>
  (criterio.aplica || []).map((id) => TIPOS_ESTRUTURA.find((t) => t.id === id)?.nome.toLowerCase()).filter(Boolean).join(" / ");

/**
 * Os critérios que valem para um defeito.
 *
 * ⚠ SEM O TIPO, DEVOLVE TODOS — e é assim que a tela usa hoje. Vitor (21/08/2026): "acho melhor
 * tirar essa informação, vamos deixar apenas as anteriores; no relatório modelo não consta essa
 * informação". Está certo: o formulário dele não tem esse campo, e eu o havia acrescentado.
 *
 * A consequência é que o portal não sabe mais QUAL dos limites vale, então mostra os dois com o
 * rótulo de onde cada um se aplica. Escolher um sozinho seria pior: assumir "estática" por ser o
 * caso mais comum esconderia o limite quatro vezes mais rigoroso da cíclica (0,25 mm de mordedura
 * contra 1 mm) justamente na peça em que ele importa.
 */
export function criteriosDoDefeito(codigo, tipoEstrutura = null) {
