...
export async function baixasDeEtapaAnterior({ opId = null, setorSyneco = null } = {}) {
  ...
  const pecas = await prisma.pecaConjunto.findMany({
    ...
    select: { opId: true, marca: true, descricao: true, qte: true, pesoUnitKg: true, terceirizado: true, destinoTerceirizado: true, encaminhadoSetor: true },
  });
  const pecaPorChave = new Map();
  for (const p of pecas) { const k = `${p.opId}|${p.marca}`; if (!pecaPorChave.has(k)) pecaPorChave.set(k, p); }
