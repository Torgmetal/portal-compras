...
import { validarFracoesExportacao, feitoNasFracoes } from "./exportacao-fracoes";
 ...
 */
export async function listaBaixaSyneco(opNumero, setor, ids, fracoes) {
  ...
  const pecas = await prisma.pecaConjunto.findMany({
    ...
    select: { id: true, opId: true, opNumero: true, marca: true, descricao: true, perfil: true,
              ...
              ...(fracoes !== undefined && campos ? { [campos.dia]: true, [campos.recurso]: true } : {}) },
    orderBy: { marca: "asc" },
  });
  /* ⚠⚠ AQUI O LEITOR VAI NO CRU (`deduzirRota:false`). Esta planilha existe para regularizar o que
     NÃO foi apontado no Syneco; se ela deduzisse a baixa pela etapa da frente, a linha que precisa
     ser corrigida sumiria justamente do documento que veio corrigi-la. */
  const feito = await lerProduzidoPorSetor(pecas.map((p) => ({ opId: p.opId, marca: p.marca })), [setor], { deduzirRota: false });

  const selecao = validarFracoesExportacao(fracoes, pecas);
  const distribuicoes = selecao && campos
    ? await prisma.ganttDistribuicao.findMany({ where: { pecaId: { in: [...selecao.keys()] }, setor } }) : [];
  const porPeca = new Map(distribuicoes.map(d => [d.pecaId, d]));
  const linhas = pecas.filter(p => !selecao || selecao.has(p.id)).map((p) => {
    const total = Math.max(1, p.qte || 1);
    const bruto = Math.max(0, Math.min(feito({ opId: p.opId, marca: p.marca }, setor), total));
    const faixas = selecao?.get(p.id);
    const qte = faixas ? faixas.reduce((s, f) => s + f.quantidade, 0) : total;
    const partes = campos ? expandirPeca(p, campos, porPeca.get(p.id), bruto)
      : [{ inicioUnidade: 0, feitoDistribuido: bruto }];
    const jaTem = faixas ? feitoNasFracoes(faixas, partes) : bruto;
    return {
