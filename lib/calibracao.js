...
// Limite de erro admissível padrão (% do valor nominal) — PO-20 = 10%. Editável por
// equipamento; a norma (ISO 9001 §7.1.5 / ISO 10012) não fixa %, quem define é a Torg.
export const ERRO_MAX_PERCENT_SUGERIDO = 10;
...
export const CRITERIO_ACEITACAO_PADRAO =
  "Aprovado quando o erro de cada ponto (em % do valor nominal), somado à incerteza, for ≤ 10% (PO-20) " +
  "e os padrões utilizados na calibração estiverem na validade. " +
...
// Regra do Vitor (15/08): a FOTO do equipamento deixou de ser obrigatória — basta o
// relatório pra Aprovar/Reprovar. (Foto continua opcional/recomendada.)
export const temAnexos = (av) => !!(av?.relatorioUrl);
...
export function avaliarPontos(pontos = [], { limitePercent = null, empGlobalAbs = null, faixaMin = null, faixaMax = null, errosEmPercent = false } = {}) {
  const lista = Array.isArray(pontos) ? pontos : [];
  // Alguns certificados JÁ trazem o erro/incerteza em % (colunas "Erro %"/"U %").
  // Nesse caso usamos o valor direto — dividir pelo nominal seria contar o % duas vezes.
  const abs = (v) => { const n = num(v); return n == null ? null : Math.abs(n); };
  const emPercent = (v, nominal) => (errosEmPercent ? abs(v) : erroPercent(v, nominal, faixaMin, faixaMax));
  const out = lista.map((p) => {
    const eP = emPercent(p.erro, p.nominal);
    const uP = p.incerteza != null ? emPercent(p.incerteza, p.nominal) : null;
    const totalP = eP == null ? null : eP + (uP || 0);
    // limite efetivo do ponto: usuário → EMP do ponto → EMP global do cert (todos em %)
    const empPonto = errosEmPercent ? abs(p.emp ?? empGlobalAbs) : empPercentDoPonto(p.emp ?? empGlobalAbs, p.nominal, faixaMin, faixaMax);
