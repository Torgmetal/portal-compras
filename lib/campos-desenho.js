...
/** Os quatro lados de um retângulo, já no espaço da matriz. */
function ladosDoRetangulo(ctm, x, y, w, h) {
  const a = aplicar(ctm, x, y), b = aplicar(ctm, x + w, y);
  const c = aplicar(ctm, x + w, y + h), d = aplicar(ctm, x, y + h);
  return [[a, b], [b, c], [c, d], [d, a]];
}

export function horizontais(ol, OPS, base = [1, 0, 0, 1, 0, 0]) {
  const out = [];
  let ctm = [...base];
  const pilha = [];
  const seg = (p1, p2) => {
  ...
  for (let i = 0; i < ol.fnArray.length; i++) {
    ...
    // ⚠⚠ A `/Matrix` própria do Form XObject — ver a nota em `vista-desenho.js`. Sem ela, a
    // moldura e o carimbo que morem dentro de um form eram procurados no lugar errado, e
    // `regioes` separava a peça das tabelas com coordenadas que não batiam com o desenho.
      ...
      const [ops, args] = a;
      let ai = 0, cx = 0, cy = 0, ix = 0, iy = 0;
      for (const op of ops) {
        if (op === OPS.moveTo) { [cx, cy] = [args[ai], args[ai + 1]]; [ix, iy] = [cx, cy]; ai += 2; }
          ...
          // ⚠⚠ OS QUATRO LADOS, e quem classifica é `seg`, DEPOIS da matriz (achado do Codex,
          // 18/09/2026). Numa folha com `/Rotate 90` o lado vertical do retângulo CRU vira
          // horizontal na folha orientada: filtrar pelos lados crus perdia a moldura inteira.
          ladosDoRetangulo(ctm, x, y, w, h).forEach(([p1, p2]) => seg(p1, p2));
        } else if (op === OPS.closePath) {
          // ⚠⚠ Sem argumento, e fecha o subcaminho. Moldura escrita como `m l l l h` — que é como
          // o pdf-lib e vários CADs escrevem um retângulo — perdia o lado de fechamento, e o
          // `else ai += 2` ainda dessincronizava o resto do traçado (achado 18/09/2026).
          seg(aplicar(ctm, cx, cy), aplicar(ctm, ix, iy));
          [cx, cy] = [ix, iy];
        } else ai += 2;
      }
    }
...
export function verticais(ol, OPS, base = [1, 0, 0, 1, 0, 0]) {
  const out = [];
  let ctm = [...base];
  const pilha = [];
  const seg = (p1, p2) => {
    ...
    // `regioes` separava a peça das tabelas com coordenadas que não batiam com o desenho.
      ...
      for (const op of ops) {
        if (op === OPS.moveTo) { [cx, cy] = [args[ai], args[ai + 1]]; [ix, iy] = [cx, cy]; ai += 2; }
          ...
          const [x, y, w, h] = args.slice(ai, ai + 4); ai += 4;
          // ⚠⚠ ver a nota em `horizontais`: os quatro lados, classificados depois da matriz.
          ladosDoRetangulo(ctm, x, y, w, h).forEach(([p1, p2]) => seg(p1, p2));
        } else if (op === OPS.closePath) {
          // ⚠⚠ Sem argumento, e fecha o subcaminho. Moldura escrita como `m l l l h` — que é como
          // o pdf-lib e vários CADs escrevem um retângulo — perdia o lado de fechamento, e o
