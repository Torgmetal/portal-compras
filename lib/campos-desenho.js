...
export async function acharMoldura(pdfBytes) {
  ...
  for (let n = 1; n <= doc.numPages; n++) {
    ...
    const [yBase, yTopo] = ys;              // externa e interna, de baixo pra cima
    const x0 = xs[1] ?? xs[0];              // borda ESQUERDA da moldura interna
    const x1 = xs[xs.length - 2] ?? xs[xs.length - 1];
    if (yTopo - yBase < 9 || x1 - x0 < 100) continue; // faixa fina demais: não força
    const naFaixa = itens
      .filter((i) => i.str?.trim() && i.transform[5] >= yBase - 1 && i.transform[5] <= yTopo + 1 && i.transform[4] >= x0 - 5 && i.transform[4] <= x1)
      .map((i) => i.transform[5]);
    out.set(n, { x0, x1, yBase, yTopo, yTextoMin: naFaixa.length ? Math.min(...naFaixa) : yTopo });
  }
  return out;
}
