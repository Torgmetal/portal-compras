...
// passei" — e, no dia seguinte à medição, fechou a meta: "vamos usar 10 ton pelo menos para a
...
//   Ou seja: com a mistura de hoje as máquinas juntas ficam abaixo da meta. Por isso a tela mostra
//   o gargalo por máquina em vez de só somar — a cantoneira estoura muito antes de o total chegar
...
// preparação, pois temos 2 turnos, precisamos fazer". Antes havia dois números para a mesma coisa
// no portal — 6.000 em /pcp/carga-corte e 12.000 aqui —, e o mesmo lote dava dois prazos conforme
// a tela em que se olhasse. Agora é uma constante só, e é esta.
export const META_KG_DIA_PREPARACAO = 10000;
 ...
 *
 * ⚠ PURA, recebe os apontamentos já lidos: esta lib também roda no cliente (a tela usa `diasDoLote`
 * e `rotuloMaquina`), e importar o prisma aqui arrastaria o banco para o bundle do navegador.
 *
 * @param {{maquina: string, dataInicio: Date|string, produzidoKg: number}[]} aps
 * @returns {{ capacidade: Record<string, number>, amostra: Record<string, number>, totalKgDia: number }}
 */
export function capacidadePorMaquina(aps) {
