...
export function melhorArranjo(lista, perfil, ctx, pilha, veicBase = "carreta") {
  let melhor = null;
  const estrategias = ctx.modoGrupo === "grades" ? { "grades-area": ordemGrades, "grades-comp": ordemGradesComp }
    : ctx.sequencia ? { sequencia: ordemSequencia, "sequencia-comp": ordemSequenciaComp } : ESTRATEGIAS;
  const prefs = ctx.sequencia ? ["chao"] : ["chao", "empilhar"];
  ...
  const reservas = !ctx.sequencia && ctx.modoGrupo !== "grades" && altDelic ? [0, altDelic + MEDIDAS.MADEIRA] : [0];
  for (const [nomeOrdem, cmp] of Object.entries(estrategias)) for (const pref of prefs) for (const reserva of reservas) {
    ...
    if (!melhor || score < melhor.score) melhor = { nome, cargas, score, frete, snap: fotografar(lista.concat(pilha)) };
  }
