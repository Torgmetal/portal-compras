...
const pintura = () => new THREE.MeshPhysicalMaterial({ color: COR_CABINE, roughness: 0.25, metalness: 0.38, clearcoat: 0.85, clearcoatRoughness: 0.18 });
...
const VIDRO = new THREE.MeshPhysicalMaterial({ color: 0x183344, roughness: 0.12, metalness: 0.35, clearcoat: 1, clearcoatRoughness: 0.04 });
...
let PNEU = null, TABUAS = null, MARCA = null;
function materiaisMadeira() {
  if (TABUAS) return TABUAS;
  const cv = document.createElement("canvas"); cv.width = 128; cv.height = 256;
  const ctx = cv.getContext("2d"); ctx.fillStyle = "#bc9c74"; ctx.fillRect(0, 0, 128, 256);
  for (let i = 0; i < 100; i++) {
    ctx.strokeStyle = `rgba(72,43,20,${0.05 + (i % 5) * 0.03})`; ctx.lineWidth = i % 4 === 0 ? 2 : 0.6;
    ctx.beginPath(); const x = i * 1.37 % 128; ctx.moveTo(x, 0);
    ctx.bezierCurveTo(x + Math.sin(i) * 6, 85, x + Math.cos(i) * 9, 180, x, 256); ctx.stroke();
  }
  const textura = new THREE.CanvasTexture(cv); textura.colorSpace = THREE.SRGBColorSpace;
  TABUAS = [0xa58c6a, 0xb6a082].map((color) => mat(color, { map: textura, bumpMap: textura, bumpScale: 0.008, roughness: 0.88, metalness: 0 }));
  return TABUAS;
}
function materialIdentificacao() {
  if (MARCA) return MARCA;
  const cv = document.createElement("canvas"); cv.width = 512; cv.height = 160;
  const ctx = cv.getContext("2d"); ctx.fillStyle = "#ffffff"; ctx.font = "bold 96px Arial"; ctx.textAlign = "center"; ctx.fillText("TORG", 256, 100);
  ctx.fillStyle = "#f4801f"; ctx.fillRect(92, 120, 328, 9);
  MARCA = mat(0xffffff, { map: new THREE.CanvasTexture(cv), transparent: true, depthWrite: false, roughness: 0.45, metalness: 0 });
  MARCA.map.colorSpace = THREE.SRGBColorSpace; return MARCA;
}
...
export function montarCaminhao(scene, v) {
  ...
  const cabine = ({ xFrente, piso, teto, comp, larg, dormitorio }) => {
    ...
    // Limpadores acompanhando a inclinação do para-brisa, com braço e palheta separados.
    for (const lado of [-1, 1]) {
      const wz = zc + lado * larg * 0.22;
      add(box(0.028, len * 0.42, 0.022), ESCURO, mx + nx * (bev + 0.04), my - len * 0.18, wz, [0, 0, th + lado * 0.1]);
      add(box(0.03, len * 0.3, 0.038), ESCURO, mx + nx * (bev + 0.055), my - len * 0.03, wz, [0, 0, th - 0.25]);
    }
    // quebra-sol
    ...
    for (const s of [-1, 1]) { const zs = zc + s * (larg / 2), FRISO = mat(0x083a5e);
      ...
      add(new THREE.PlaneGeometry(comp * 0.34, comp * 0.105), materialIdentificacao(), xFrente + comp * 0.45, piso + h * 0.27, zs + s * 0.035, [0, s < 0 ? Math.PI : 0, 0]);
      // Vinco horizontal e saia sob a porta, com apoio metálico dos degraus.
      add(rbox(comp * 0.65, 0.07, 0.018, 0.008), pintura(), xFrente + comp * 0.56, piso + h * 0.1, zs + s * 0.012);
      for (let j = 0; j < 6; j++) add(box(0.032, 0.009, 0.36), ALUMINIO, xFrente + 0.45 + j * 0.06, piso - 0.282, zc + s * (larg / 2 - 0.22));
  ...
  const madeira = (i) => materiaisMadeira()[i % 2];
  ...
  // Ferragens e sinalização da carroceria: dobradiças, ganchos e lanternas laterais.
  for (const s of [-1, 1]) for (let x = 0.7; x < C; x += 1.8) {
    const z = zc + s * (L / 2 + 0.065);
    add(box(0.13, 0.07, 0.025), CHASSI, x, -0.075, z);
    add(cil(0.025, 0.18, 10), ALUMINIO, x, -0.07, z, [0, 0, Math.PI / 2]);
    add(new THREE.TorusGeometry(0.047, 0.012, 6, 12, Math.PI), CHASSI, x + 0.25, -0.14, z, [Math.PI, 0, 0]);
    add(rbox(0.1, 0.045, 0.025, 0.01), LARANJA, x + 0.4, -0.06, z);
  }
