...
function montarDeitada(u, malhas, cor) {
  ...
  if (u.tipo === "PALLET" || u.tipo === "CAIXA") { const b = mm(u.base || 150);
    ...
    else if (!u.semFechamento) { const cx2 = new THREE.Mesh(new THREE.BoxGeometry(mm(u.C), mm(u.A) - b, mm(u.L)), M(0xd9b382, { transparent: true, opacity: 0.22 })); cx2.position.set(mm(u.C) / 2, b + (mm(u.A) - b) / 2, mm(u.L) / 2); g.add(cx2); } }
