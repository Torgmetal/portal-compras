...
export function montarCaibros(u, madeira, baixo = []) {
  ...
  const apoios = baixo.filter((b) => Math.abs(b.y + b.A - topoBaixo) <= 300).map((b) => [b.x, b.x + (b.fx || b.C), b.z, b.z + (b.fz || b.L)]);
  const apoiado = (x) => !apoios.length || apoios.some(([x0, x1, z0, z1]) => x + 40 >= x0 && x - 40 <= x1 && z1 > u.z && z0 < u.z + fz);
  const usados = [];
  for (let k = 0; k < n; k++) {
...
export function rotuloVolume(u, tamanho = 0.05) {
  ...
  sp.position.set(mm(u.x + (u.fx || u.C) * fr), mm(u.y + u.A) + 0.12, mm(u.z + (u.fz || u.L) / 2));
  return sp;
}
