...
import { caibrosDoMotor } from "./apoio-motor";
...
// ⚠⚠ CINTA É FITA, NÃO PLACA: 32 mm de largura, bem fina, contornando o volume (em cima, embaixo e dos dois lados), na
// cor das cintas do desenho de referência do PDF. Desenhada como caixa de 5 cm × altura × largura, ela virava uma placa
// preta atravessando o pacote — Vitor (24/09/2026): "essas madeiras pretas que vc coloca (…) não vejo a forma de
// conseguirmos fazer aqui".
const COR_CINTA = 0x276653;
function cintar(grupo, x, y0, y1, z0, z1) {
  const w = 0.032, t = 0.008, mat = M(COR_CINTA, { roughness: 0.5 }), H = y1 - y0, W = z1 - z0;
  for (const [sy, sz, py, pz] of [[t, W + 2 * t, y1 + t / 2, (z0 + z1) / 2], [t, W + 2 * t, y0 - t / 2, (z0 + z1) / 2], [H, t, (y0 + y1) / 2, z0 - t / 2], [H, t, (y0 + y1) / 2, z1 + t / 2]]) {
    const f = new THREE.Mesh(new THREE.BoxGeometry(w, sy, sz), mat); f.position.set(x, py, pz); f.userData.cinta = true; grupo.add(f); }
}
...
function montarEmPe(u, malhas, cor) {
  ...
  if (e.tipo === "engradado") {
    ...
    for (let x = 0.4; x < C - 0.2; x += 1.2) cintar(P, x, b, b + h, L / 2 - mm(Wp) / 2, L / 2 + mm(Wp) / 2);
  }
...
function montarDeitada(u, malhas, cor) {
  ...
  // pacote: caibro entre as camadas (a cada ~1,5 m, como os de baixo) e as cintas por fora
  if (u.tipo === "PACOTE" && u.madeiraEntre) { const n = Math.max(2, Math.round(mm(u.C) / 1.5) + 1), e = mm(u.madeiraEntre), mat = M(0xb08650);
    for (const dy of [...new Set(u.membros.map((p) => p.dy || 0))].filter((v) => v > 0)) for (let k = 0; k < n; k++) {
      const w = new THREE.Mesh(new THREE.BoxGeometry(0.05, e, mm(u.L) + 0.04), mat); w.position.set(0.15 + (mm(u.C) - 0.3) * (k / (n - 1)), mm(dy) - e / 2, mm(u.L) / 2); g.add(w); } }
  if (u.tipo === "PACOTE") { for (let x = 0.3; x < mm(u.C); x += 1.2) cintar(g, x, 0, mm(u.A), 0, mm(u.L));
    // contorno do volume só como referência: claro e translúcido, para não ler como moldura de verdade
    const e = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(mm(u.C), mm(u.A), mm(u.L))), new THREE.LineBasicMaterial({ color: 0x9aa5ae, transparent: true, opacity: 0.45 })); e.position.set(mm(u.C) / 2, mm(u.A) / 2, mm(u.L) / 2); g.add(e); }
...
export function montarCaibros(u, madeira, baixo = []) {
  ...
  // ⚠⚠ Simulação nova: o motor já achou onde cada caibro assenta no AÇO de verdade e quanto calço vai embaixo
  // (lib/carga/perfil-apoio.js). Desenha exatamente isso — o caibro encostado na peça de cima, o calço sobre a de
  // baixo — para nada parecer "voando". Vitor (18/09/2026): "o que não pode é ficar voando as coisas".
  const doMotor = caibrosDoMotor(u, new Map(baixo.map((b) => [b.id, b])));
  if (doMotor) {
    // o caibro vai exatamente no trecho que o motor conferiu (z0..z1: a largura do volume e a folga) — além disso é o vizinho
    const matCalco = M(0x8a6a45);
    for (const cb of doMotor) {
      const m = new THREE.Mesh(new THREE.BoxGeometry(mm(madeira), mm(madeira), mm(cb.z1 - cb.z0)), matMad); m.position.set(mm(cb.x), mm(cb.y0 + madeira / 2), mm((cb.z0 + cb.z1) / 2)); m.userData.pilhaDe = u.id; m.castShadow = true; out.push(m);
      for (const [z0, z1, base] of cb.segs || []) { const h = cb.y0 - base, a = Math.max(z0, cb.z0), b = Math.min(z1, cb.z1); if (h < 5 || b - a < 10) continue;
        const k = new THREE.Mesh(new THREE.BoxGeometry(mm(madeira * 0.9), mm(h), mm(b - a)), matCalco); k.position.set(mm(cb.x), mm(base + h / 2), mm((a + b) / 2)); k.userData.pilhaDe = u.id; k.castShadow = true; out.push(k); }
    }
    return out;
  }
...
/**
 * Escoras do travamento (lib/carga/travamento.js): 2 caibros do volume até o da frente — ou até a cabeceira —, na
 * altura e no trecho em que os dois se encaram. O vão até 1,5 m se escora; além disso vai cinta e catraca, que o 3D não
 * inventa: fica no texto do volume. Vitor (24/09/2026), sobre o vão para as peças correrem: "concordo, podemos adotar".
 * @param {object} u  unidade; @param {number} madeira  seção do caibro (mm); @param {Map<string,object>} porId  volumes da carga
 */
export function montarEscoras(u, madeira, porId = new Map()) {
  const t = u.travamento; if (t?.tipo !== "escorar" || !(t.vao > 0)) return [];
  const w = t.contra ? porId.get(t.contra) : null, fy = u.fy || u.A, fz = u.fz || u.L;
  const y0 = w ? Math.max(u.y, w.y) : u.y, y1 = w ? Math.min(u.y + fy, w.y + (w.fy || w.A)) : u.y + Math.min(fy, 400);
  const z0 = w ? Math.max(u.z, w.z) : u.z, z1 = w ? Math.min(u.z + fz, w.z + (w.fz || w.L)) : u.z + fz;
  const y = Math.max(y0 + madeira / 2, Math.min(y1 - madeira / 2, (y0 + y1) / 2)), mat = M(0xb08650), out = [];
  for (const f of [0.25, 0.75]) {
    const m = new THREE.Mesh(new THREE.BoxGeometry(mm(t.vao), mm(madeira), mm(madeira)), mat);
    m.position.set(mm(u.x - t.vao / 2), mm(y), mm(z0 + (z1 - z0) * f)); m.userData.pilhaDe = u.id; m.userData.escora = true; m.castShadow = true; out.push(m);
  }
  return out;
}
