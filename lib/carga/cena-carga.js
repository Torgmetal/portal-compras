...
export function montarUnidade(u, malhas) {
  ...
  const g = (u.emPe || u.tipo === "ENGRADADO") ? montarEmPe(u, malhas, cor) : montarDeitada(u, malhas, cor);
  if (u.girada) { g.rotation.y = -Math.PI / 2; g.position.x += mm(u.L); } // atravessada: comprimento vira Z, largura vira X
  return g;
}

/** Caibros atravessados sob uma unidade que não está no chão. */
export function montarCaibros(u, madeira) {
  const out = [], matMad = M(0xb08650);
  if (!(u.y > 0)) return out;
  const fx = u.girada ? u.L : u.C, fz = u.girada ? u.C : u.L, n = Math.max(2, Math.round(mm(fx) / 1.5) + 1);
  for (let k = 0; k < n; k++) { const dx = 0.15 + (mm(fx) - 0.3) * (k / (n - 1));
    const m = new THREE.Mesh(new THREE.BoxGeometry(mm(madeira), mm(madeira), mm(fz) + 0.1), matMad); m.position.set(mm(u.x) + dx, mm(u.y) - mm(madeira) / 2, mm(u.z) + mm(fz) / 2); m.userData.pilhaDe = u.id; m.castShadow = true; out.push(m); }
  return out;
}

/** Bolinha com o número do volume, na cor da camada. */
export function rotuloVolume(u, escala) {
  ...
  ctx.beginPath(); ctx.arc(128, 128, 112, 0, Math.PI * 2); ctx.fillStyle = cor; ctx.fill(); ctx.lineWidth = 16; ctx.strokeStyle = "#ffffff"; ctx.stroke();
  ctx.fillStyle = "#ffffff"; ctx.font = "bold 150px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(String(u.volume), 128, 140);
  const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(cv), depthTest: false, depthWrite: false, transparent: true })); sp.renderOrder = 999;
  ...
  sp.position.set(mm(u.x + (u.fx || u.C) * fr), mm(u.y + u.A) + escala * 0.55, mm(u.z + (u.fz || u.L) / 2));
  return sp;
}
