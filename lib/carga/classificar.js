...
// ⚠ "CORRIMÃO" também é guarda-corpo: a DANPOWER (OP-085) chama os painéis de "CORRIMAO 2030MM" e os da
// escada de "ACESSOS - CORRIMAO DIREITO" — 46 marcas, 4,8 t, que sem isto viajariam como peça solta.
export const ehGC = (u) => /G\.?\s*C\b|GUARDA|CORRIM/i.test(u.desc || "");
...
// quadro VAZADO deitado (pórtico, treliça, escada, armação): a caixa é cheia, a peça não — o que subir nela cai no vão.
// Só outra peça da MESMA marca (que encaixa por cima, membro sobre membro) pode ir em cima. Vitor (13/09/2026), OP-107: feixe de colunas sobre um pórtico.
export const descVazada = (u) => /P[ÓO]RTICO|TRELI|ESCADA|QUADRO|ARMA[CÇ][AÃ]O|CONTRAVENT/i.test(u.desc || "") && u.L >= 600;
export const ehQuadroVazado = (u) => u.tipo === "PECA" && descVazada(u);
// volume com base própria de madeira (caixa, engradado, palete, em pé no engradado ou cavalete) assenta direto no
// assoalho; o AÇO nunca — Vitor (24/09/2026): "sempre com madeira para conseguir retirar com facilidade"
export const temBaseDeMadeira = (u) => u.tipo === "CAIXA" || u.tipo === "ENGRADADO" || u.tipo === "PALLET" || !!u.emPe;
