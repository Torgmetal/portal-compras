...
export const CAMPOS_VEICULO = ["nome", "C", "L", "alturaUtil", "pesoMax", "assoalho", "cabeceira"];
...
export function catalogoDeVeiculos(cfg) {
  ...
  for (const [k, padrao] of Object.entries(VEICULOS)) {
    ...
    veiculos[k] = { chave: k, nome: v.nome || padrao.nome, C: n(v.C, padrao.C), L: n(v.L, padrao.L), alturaUtil: n(v.alturaUtil, padrao.alturaUtil), pesoMax: n(v.pesoMax, padrao.pesoMax), assoalho: n(v.assoalho, padrao.assoalho),
      // altura do painel da frente acima do assoalho — sem ela, só a camada do assoalho encosta (lib/carga/travamento.js)
      cabeceira: n(v.cabeceira, padrao.cabeceira) ?? null };
...
export function linhasDeConfiguracao(cfg) {
  const porChave = new Map((cfg?.veiculos || []).map((v) => [v.chave, v]));
  return Object.values(VEICULOS).map((p) => { const v = porChave.get(p.chave) || {}; return { chave: p.chave, nome: v.nome || p.nome, C: v.C || p.C, L: v.L || p.L, alturaUtil: v.alturaUtil || p.alturaUtil, pesoMax: v.pesoMax || p.pesoMax, assoalho: v.assoalho || p.assoalho, cabeceira: v.cabeceira || p.cabeceira || null, frete: v.frete || (cfg?.frete || {})[p.chave] || FRETE[p.chave], ativo: v.ativo !== false || p.chave === "carreta" || p.chave === "carreta14", padrao: { ...p, frete: FRETE[p.chave] } }; });
}
