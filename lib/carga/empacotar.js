...
// tampa: 1 nada em cima · 2 só engradado · 3 só delicado · 4 só guarda-corpo · 5 só caixa/delicado
const bloqueia = (t, u, delic) => t === 1 || (t === 2 && u.tipo !== "ENGRADADO") || (t === 3 && !delic) || (t === 4 && !u.gc) || (t === 5 && !(u.tipo === "CAIXA" || delic));
...
function avaliarPosicao(c, u, perfil, ctx, pos, tetoUtil, pref, melhor) {
  ...
  for (let a = ix; a < ix + cx; a++) for (let b = iz; b < iz + cz; b++) { const k = a * NZ + b;
    if (bloqueia(c.tampa[k], u, delic)) return null;
  ...
  if (topo > 0 && nivelBase + 1 > perfil.niveisMax) return null;
...
export function tentaPor(c, u, perfil, ctx, pref = "chao") {
  ...
  for (let a = ix; a < ix + cx; a++) for (let b = iz; b < iz + cz; b++) { const k = a * NZ + b; c.alt[k] = y + u.A; c.dono[k] = u; c.nivel[k] = nivel + 1;
    if (u.emPe) c.tampa[k] = u.tipo === "ENGRADADO" && perfil.engradadoEmpilha && nivel === 0 ? 2 : 1;
