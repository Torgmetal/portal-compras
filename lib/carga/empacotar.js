...
export function novaCarga(veic, zona, ctx) {
  ...
  // apoio = topo do AÇO já carregado em cada célula (quem o pôs: donoApoio) · dono/tampa/nivel = o retângulo ocupado
  return { veicKey: veic.chave, veic, zonaGC: zona, nx, nz, cel, emCamadas: ctx.modoGrupo === "grades", apoio: new Float32Array(nx * nz), donoApoio: new Array(nx * nz).fill(null), dono: new Array(nx * nz).fill(null), tampa: new Uint8Array(nx * nz), nivel: new Uint8Array(nx * nz), itens: [], peso: 0, altura: 0, pilhas: [] };
}
...
// delicado numa carga mista vai para o alto (nada sobe nele)
const custoDaAltura = (y, pref, delicAlto) => (delicAlto ? -y * 1000 : pref === "empilhar" && y > 0 ? -1e6 + y * 100 : y * 1000);
...
function avaliarPosicao(c, u, perfil, ctx, pos, tetoUtil, pref, melhor) {
  ...
  for (let a = 0; a < cx; a++) for (let b = 0; b < cz; b++) { const k = (ix + a) * NZ + iz + b;
    ...
    const f = pg.fundo[a * cz + b]; if (f === Infinity) continue;
    const t = c.apoio[k];
    if (t > 0) { sobreAco = true; if (t - f + MADEIRA > y) { y = t - f + MADEIRA;
      // a altura só sobe no laço e o custo sobe com ela: se já perde para a melhor posição, nem termina a conta
      if (y + u.A > tetoUtil || (melhor && !delicAlto && custoDaAltura(y, pref, false) + extra >= melhor.custo)) return null; } } }
  // no assoalho, o retângulo tem de estar livre: peça do chão não se encaixa no vão de outra (vira quebra-cabeça)
  if (!sobreAco) { if (ocupado) return null; y = 0; }
  y = Math.ceil(y);
  if (y + u.A > tetoUtil) return null;
  if (y > 0 && (u.soChao || u.tipo === "PALLET" || (u.baseEmV && u.almaVertical !== false) || nivelBase + 1 > perfil.niveisMax)) return null; // ajuste "no chão": só no assoalho
  // custo provisório: se já não bate o melhor, nem olha os apoios
  let custo = custoDaAltura(y, pref, delicAlto) + extra;
  ...
  const baixo = [...donos];
  if (y === 0) return { ...pos, y, custo, baixo, nivel: 0 };
  ...
  const r = caibrosApoiados({ apoio: c.apoio, NZ, ix, iz, cx, cz, cel: c.cel, fx: pos.fx, fz: pos.fz, y, pg, rigido: u.tipo !== "PECA", minimo: c.emCamadas ? GRADE_CARGA.apoioMin : (perfil.apoioMin ?? 0.6) });
  if (!r.ok) return null;
...
export function tentaPor(c, u, perfil, ctx, pref = "chao") {
  ...
  const { ix, iz, y, cx, cz, fx, fz, girada, baixo, nivel, linhas, pg } = melhor;
...
export function consolidar(cargas, perfil, ctx, pref) {
  ...
  for (let i = cargas.length - 1; i >= 0; i--) {
    ...
    const snaps = alvos.map((x) => [x, { apoio: x.apoio.slice(), donoApoio: x.donoApoio.slice(), dono: x.dono.slice(), tampa: x.tampa.slice(), nivel: x.nivel.slice(), itens: x.itens.slice(), peso: x.peso, altura: x.altura }]);
