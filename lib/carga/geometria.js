...
export function expandirPecas(lista, geometria = {}) {
  ...
  for (const p of lista) {
    ...
    const dims = g?.obb?.dimsEixos || g?.dimsEixos, estimada = !dims ? caixaEstimada(p.desc, p.kgUn) : null;
    const o = dims ? orientarPeca(p.desc, dims, g.obb || null) : estimada ? orientarPeca(p.desc, estimada, null) : null;
    for (let i = 0; i < (p.qtd || 1); i++) {
      ...
      pecas.push(o ? { ...base, ...o, temGeo: !!g?.temGeo, aproximada: !g?.obb, estimada: !!estimada } : { ...base, semCaixa: true });
    }
  }
