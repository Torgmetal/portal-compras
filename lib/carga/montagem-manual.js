...
export function recalcularMontagem(carga, madeira=100) {
  ...
  for(const u of itens){
    ...
    if(u.y>1){
      ...
      if(!coberturaDaBase(u,apoios.filter(b=>!b.baseEmV&&!temGiroSemBaseConfirmada(b))).suficiente||temGiroSemBaseConfirmada(u))aviso('apoio',u,`Volume ${u.volume}: a base não tem apoio suficiente identificado na altura escolhida. Reposicione ou confira os apoios reais.`);
