...
export function recalcularMontagem(carga, madeira=100) {
  ...
  for(const u of itens){
    ...
    if(u.y>1){
      ...
      if(!coberturaDaBase(u,apoios.filter(b=>!b.baseEmV&&!b.topoVazado&&!temGiroSemBaseConfirmada(b))).suficiente||temGiroSemBaseConfirmada(u))aviso('apoio',u,`Volume ${u.volume}: a base não tem apoio suficiente identificado na altura escolhida. Reposicione ou confira os apoios reais.`);
  ...
  const romaneio=itens.map(u=>{const r=(carga.romaneio||[]).find(v=>v.id===u.id)||{id:u.id,volume:u.volume,rotulo:u.rotulo,pecas:u.membros?.length||1,dimsCm:[u.C,u.L,u.A].map(v=>Math.round(v/10))};const mad=madeiraDaUnidade(u,carga.veiculo);return {...r,madeira:{...pecasDeMadeira(mad),nCaibro:mad.nCaibro,compCaibro:arred(mad.compCaibro)}};});
