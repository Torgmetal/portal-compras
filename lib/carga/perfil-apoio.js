 ...
 * ⚠ Dois retratos, porque são duas perguntas: `topo`/`fundo` contam cada coluna de 5 cm só na célula do SEU centro —
 * é o APOIO (aço que só raspa o canto da célula não segura caibro); `topoMax`/`fundoMin` contam em toda célula que a
 * coluna toca — é a COLISÃO (o aço que invade 2 cm da célula vizinha também bate).
...
export function perfilNaGrade(u, perfis, { cel, cx, cz, girada, folga }) {
  const n = cx * cz, topo = new Float32Array(n).fill(-1), fundo = new Float32Array(n).fill(Infinity), topoMax = new Float32Array(n).fill(-1), fundoMin = new Float32Array(n).fill(Infinity);
  ...
  const celula = (lx, lz) => {
    ...
    return a < 0 || b < 0 || a >= cx || b >= cz ? -1 : a * cz + b;
  };
  const marcar = (k, t, f, T, F) => { if (k < 0) return; if (t > T[k]) T[k] = t; if (f < F[k]) F[k] = f; };
  // volume cheio (caixa, engradado, peça sem malha), célula a célula e exato: para a COLISÃO conta toda célula que ele
  // toca; para o APOIO, só a que ele cobre até o centro — 2 cm da borda de um engradado não seguram a ponta de um caibro.
  // ⚠ Amostrar de 25 em 25 a partir de +5 deixava a última faixa de fora, e a caixa vizinha entrava nela (OP-107,
  // perfil Vale: duas caixas sobre palete 30 mm uma dentro da outra).
  const bloco = (x0, z0, C, L, t, f) => {
    const [X0, X1] = girada ? [u.L - z0 - L, u.L - z0] : [x0, x0 + C], [Z0, Z1] = girada ? [x0, x0 + C] : [z0, z0 + L];
    const inicio = (v) => v + folga / 2; // célula a cobre [a·cel − folga/2, (a+1)·cel − folga/2) no referencial do volume
    for (let a = Math.max(0, Math.floor(inicio(X0) / cel)); a < cx && a * cel - folga / 2 < X1; a++) {
      const xc = (a + 0.5) * cel - folga / 2, noX = xc >= X0 && xc <= X1;
      for (let b = Math.max(0, Math.floor(inicio(Z0) / cel)); b < cz && b * cel - folga / 2 < Z1; b++) {
        const k = a * cz + b, zc = (b + 0.5) * cel - folga / 2;
        if ((a + 1) * cel - folga / 2 > X0 && (b + 1) * cel - folga / 2 > Z0) marcar(k, t, f, topoMax, fundoMin);
        if (noX && zc >= Z0 && zc <= Z1) marcar(k, t, f, topo, fundo);
      }
    }
  };
  if (cheio(u) || !perfis?.size) { bloco(0, 0, u.C, u.L, u.A, 0); return { topo, fundo, topoMax, fundoMin }; }
  for (const m of u.membros || [u]) {
    ...
    for (let ix = 0; ix < pf.nx; ix++) for (let iz = 0; iz < pf.nz; iz++) {
      ...
      const t = dy + pf.topo[k], f = dy + pf.fundo[k];
      marcar(celula(dx + (ix + 0.5) * pf.cel, dz + (iz + 0.5) * pf.cel), t, f, topo, fundo);
      for (const fx of [0.02, 0.98]) for (const fz of [0.02, 0.98]) marcar(celula(dx + (ix + fx) * pf.cel, dz + (iz + fz) * pf.cel), t, f, topoMax, fundoMin);
      marcar(celula(dx + (ix + 0.5) * pf.cel, dz + (iz + 0.5) * pf.cel), t, f, topoMax, fundoMin);
    }
  }
  return { topo, fundo, topoMax, fundoMin };
}
 ...
 * @param {{apoio:Float32Array, alto?:Float32Array, NZ:number, ix:number, iz:number, cx:number, cz:number, cel:number, fx:number, fz:number, y:number,
 *   pg:{topo:Float32Array, fundo:Float32Array}, rigido?:boolean, assoalho?:boolean, exigir?:boolean, minimo?:number, calco?:number}} o
 *   rigido = volume embalado (feixe cintado, pacote, caixa, engradado): o peso entra no meio da largura, não nos pontos
 *   em que encosta · assoalho = o piso também apoia (volume no chão, sobre caibro) · exigir = sem as duas pontas, não serve
 * @returns {{ ok:boolean, fracao:number, assentado:number, linhas:Array<{a:number, y0:number, celulas:number[]}> }}
 */
export function caibrosApoiados({ apoio, alto = apoio, NZ, ix, iz, cx, cz, cel, fx, fz, y, pg, rigido = false, assoalho = false, exigir = true, minimo = 0.6, calco = MEDIDAS.CALCO }) {
  ...
  const avaliar = (a) => { // faixa [a, a+largura) atravessada
    ...
    for (let da = 0; da < largura && a + da < cx; da++) for (let b = 0; b < nb; b++) {
      ...
      const t = alto[(ix + a + da) * NZ + iz + b]; if (t > maxT) maxT = t; // colisão: o retrato que conta o aço que raspa
    }
    ...
    for (let da = 0; da < largura && a + da < cx; da++) for (let b = 0; b < nb; b++) {
      ...
      if ((assoalho || apoio[k] > 0) && y0 - apoio[k] <= calco) { celulas.push(k); if (b < s0) s0 = b; if (b > s1) s1 = b; }
    }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (achou) { usados.push(achou.a); linhas.push(achou); } else if (exigir && (i === 0 || i === n - 1)) return { ok: false, fracao: 0, assentado: 0, linhas: [] };
  }
