...
export const VEICULOS = {
  ...
  carreta:     { chave: "carreta", nome: "Carreta graneleira 3 eixos", C: 12400, L: 2450, alturaUtil: 2900, pesoMax: 25000, assoalho: 1350, cabeceira: 1800 },
  carreta14:   { chave: "carreta14", nome: "Carreta especial 14 m", C: 14000, L: 2450, alturaUtil: 2900, pesoMax: 25000, assoalho: 1350 },
};
// cabeceira = altura do painel frontal acima do assoalho (mm): até onde ele segura a carga na frenagem (Resolução CONTRAN
// 945/2022, art. 8º — ver lib/carga/travamento.js). Vitor (24/09/2026): "usamos carretas graneleiras"; nas fichas das
// graneleiras de 3 eixos as tampas vão de 2 × 700 a 2 × 1.000 mm, e a combinação comum é 800 + 1.000 = 1,8 m. ⚠ Premissa a
// confirmar medindo a carreta. Veículo sem `cabeceira`: só a camada do assoalho conta como encostada nela.
...
// MADEIRA = caibro entre camadas E sob o volume no assoalho · FOLGA = entre unidade e vizinha/borda · GAP = entre peças
// lado a lado no mesmo pacote
...
// ⚠⚠ Vitor (24/09/2026): "precisamos fazer pacotes das peças com no máximo 1,2 de largura para facilitar o carregamento,
// travar com madeiras no meio, cintas, e nunca podemos colocar as peças diretamente no assoalho, sempre com madeira para
// conseguir retirar com facilidade". madeira = caibro de 5 cm entre as camadas do pacote (premissa).
export const PAC = { secaoMax: 500, compMin: 800, larguraMax: 1200, alturaMax: 600, kgMax: 2500, madeira: 50 };
/** Largura do pacote: até 1,2 m e DOIS lado a lado na carreta — cada um com a sua folga na grade de 10 cm
 *  (2,45 m → 1,14 m). Pacote de 1,2 m cheio não cabe em dupla, e a carreta passava a levar um por fileira. */
export const larguraDoPacote = (veic = VEICULOS.carreta) => Math.min(PAC.larguraMax, Math.floor(Math.floor(veic.L / MEDIDAS.CEL) / 2) * MEDIDAS.CEL - MEDIDAS.FOLGA);
