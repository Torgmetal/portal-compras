...
function saidaDaCarga(c, perfil) {
  ...
  const romaneio = ordem.map((u) => { const mad = madeiraDaUnidade(u, c.veic); return { volume: u.volume, id: u.id, tipo: tipoVolume(u), rotulo: u.rotulo, marcas: [...new Set((u.membros || [u]).map((m) => m.marca))], pecas: (u.membros || [u]).length, kgLiquido: Math.round((u.membros || [u]).reduce((t, m) => t + m.kg, 0)), kgBruto: Math.round(u.kg), dimsCm: [u.C, u.L, u.A].map((v) => Math.round(v / 10)), madeira: { ...pecasDeMadeira(mad), nCaibro: mad.nCaibro, compCaibro: Math.round(mad.compCaibro * 10) / 10 } }; });
  ...
  return { veiculo: c.veic, grupo: c.grupo, estrategia: c.estrategia, itens, peso: Math.round(c.peso), altura: c.altura, chao, usa, passos, alertas: c.itens.filter((u) => u.alerta).length,
 ...
 */
export function simularCarga({ lista, geometria = {}, perfil: chavePerfil = "recomendado", prefixo = "", opcoes = {} }) {
  const perfil = PERFIS[chavePerfil] || PERFIS.recomendado;
  const pecasBase = expandirPecas(lista, geometria);
  const semCaixa = pecasBase.filter((u) => u.semCaixa).map((u) => ({ marca: u.marca, desc: u.desc, kg: u.kg }));
  const rodar = (gcModo) => {
    const ctx = novoContexto({ ...opcoes, prefixo }); ctx.gradeSeparada = !!opcoes.gradeSeparada;
    ...
    return { gcModo, cargas: saida, especiais: especiais.map((u) => ({ id: u.id, rotulo: u.rotulo, transporte: u.transporte, aviso: u.aviso, C: u.C, L: u.L, A: u.A, kg: Math.round(u.kg), marcas: [...new Set((u.membros || [u]).map((m) => m.marca))] })),
      ...
      resumo: { viagens: cargas.length, frete, peso: Math.round(cargas.reduce((t, c) => t + c.peso, 0)), volumes: saida.reduce((t, c) => t + c.volumes, 0), alertas: saida.reduce((t, c) => t + c.alertas, 0), semLugar: unidades.filter((u) => u.semLugar).map((u) => u.rotulo), especiais: especiais.length, tempoMin: saida.reduce((t, c) => t + c.tempo.minutos, 0) } };
  };
  ...
  return { ...r, perfil: { chave: perfil.chave, nome: perfil.nome, resumo: perfil.resumo, madeiraTratada: perfil.madeiraTratada || 1 }, madeira: MEDIDAS.MADEIRA, semCaixa, pecas: pecasBase.length };
}
