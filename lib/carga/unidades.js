...
const { GAP } = MEDIDAS;
const faseDe = (fr) => `fase ${String(fr).split("/")[0]}`;
...
function caixasDeMiudos(pecas, ctx, perfil, unidades, seq) {
  ...
  for (const { CM, sel } of niveisCaixa) {
    const porMarca = agrupar(pecas.filter((x) => ehPequena(x) && sel(x)), (u) => u.marca);
...
function classificarTransporte(unidades, perfil, gcModo) {
  const veic = VEICULOS.carreta;
...
export function montarUnidades(pecasBase, perfil, gcModo, ctx) {
  ...
  const unidades = [];
  pacotesGuardaCorpo(pecas, ctx, gcModo, unidades, seq);
  pacotesGrade(pecas, ctx, unidades, seq);
  const barra = pacotesPlanosEFeixes(pecas, ctx, PAC_P, unidades, seq);
  caixasDeMiudos(pecas, ctx, perfil, unidades, seq);
  for (const u of pecas.filter((x) => !ehGC(x) && !ehGrade(x) && !ehPequena(x) && !barra(x) && !ehPlana(x))) unidades.push({ ...u, tipo: "PECA", rotulo: u.marca, membros: [u] });
  ...
  classificarTransporte(unidades, perfil, gcModo);
  return unidades;
}
