...
function pacotesPlanosEFeixes(pecas, ctx, PAC_P, unidades, seq, forcaFeixe = []) {
  ...
  const barras = agrupar(pecas.filter(barra).concat(forcaFeixe).sort((a, b) => (b.L - a.L) || (b.A - a.A) || (b.C - a.C)), (u) => `${frenteDe(u, ctx)}|${u.desc || "BARRA"}|${Math.ceil(u.C / 2000) * 2}`);
  for (const [k, lista] of barras) { const [fr, fam, comp] = k.split("|"); let at = null;
    for (const u of lista) {
      if (!at || at.membros.length >= at.cap || at.kg + u.kg > PAC_P.kgMax) { const pL = Math.max(...lista.map(p=>p.L)) + GAP, pA = Math.max(...lista.map(p=>p.A)) + GAP, cols = Math.max(1, Math.floor(PAC.larguraMax / pL)); at = { id: `k${seq.n++}`, tipo: "PACOTE", frente: fr, rotulo: `Feixe cintado ${faseDe(fr)} · ${fam} ~${comp} m`, membros: [], kg: 0, cap: cols * Math.max(1, Math.floor(PAC.alturaMax / pA)), cols, pL, pA, C: 0, L: 0, A: 0 }; unidades.push(at); }
