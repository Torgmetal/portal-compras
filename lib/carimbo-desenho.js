...
import { acharCampoConsumivel } from "./campo-consumivel";
...
export async function carimbarDesenho(pdfBytes, info) {
  ...
  for (const page of pdf.getPages()) {
    ...
    const alt = Math.round((ehConjunto
    ...
    const larg = ehConjunto ? Math.min(largCheia, Math.round(360 * k)) : largCheia;
    ...
    if (ehConjunto) {
      ...
      txt(m + 6 * k + Math.min(larguraTit, larg * 0.42) + 6 * k, y0 + fil + 4.5 * k, `— ${rodapeCurto}`,
    ...
    if (linhaConsumivel) { yAtual -= 11 * k; txt(ehConjunto ? m + 8 * k : xInfo, yAtual, linhaConsumivel, { size: 7.5 * k, f: bold, cor: NAVY }); }
    if (linhaJanela) { yAtual -= 10 * k; txt(ehConjunto ? m + 8 * k : xInfo, yAtual, linhaJanela, { size: 7 * k, f: bold, cor: LARANJA }); }
    ...
    if (!ehConjunto) txt(m + 8 * k, y0 + fil + 5 * k, rodape, { size: 6.5 * k, cor: CINZA });
  }
