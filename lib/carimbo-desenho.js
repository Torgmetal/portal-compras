...
export async function carimbarDesenho(pdfBytes, info) {
  ...
  for (const page of pdf.getPages()) {
    ...
    const alt = Math.round((anotar ? 74 : 60) * k);
    ...
    if (anotar) txt(m + 8 * k, y - 38 * k, "Nº R do material usado: ____________________   Corrida: ____________________   Visto: __________", { size: 7.5 * k, f: bold, cor: PRETO });
