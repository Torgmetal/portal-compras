...
export async function carimbarDesenho(pdfBytes, info) {
  ...
  // Ainda não soldado = PREVISTO, e o carimbo diz isso — previsão não pode passar por registro.
  const csPrev = cs?.origem === "prevista";
  const linhaConsumivel = cs
    ? `CONSUMÍVEL DE SOLDA${csPrev ? " (PREVISTO)" : ""}:  R ${cs.rastreio || "—"}${cs.lote ? `  ·  lote ${cs.lote}` : ""}${cs.certificado && cs.certificado !== cs.lote ? `  ·  cert. ${cs.certificado}` : ""}  ·  ${cs.material || ""}`
    : null;
