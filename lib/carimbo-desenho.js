...
// são o mesmo byte).
//
// Onde a corrida é INDEFINIDA (duas do mesmo material na fábrica no dia do corte) o carimbo não
// inventa: lista as candidatas e imprime um campo pra anotar a que foi usada. É a única evidência
// que o algoritmo não tem — quem separa a barra é que sabe.
...
function linhaRastreio(it) {
  const u = it?.usadas?.[0];
  const cand = [...new Set((it?.candidatas || []).map((c) => c.corrida).filter(Boolean))];
  switch (it?.situacao) {
    case "DEFINIDA":
      return {
        forte: `R ${u?.rastreio || "—"}  ·  corrida ${u?.corrida || "—"}`,
        fraco: [u?.certificado ? `cert. ${u.certificado}` : null, u?.material || null, u?.fornecedor || null].filter(Boolean).join("  ·  "),
        anotar: false,
      };
    case "INDEFINIDA":
      return {
        forte: `CORRIDA A CONFIRMAR — uma destas: ${it.candidatas.map((c) => `R ${c.rastreio || "—"} (corrida ${c.corrida})`).join("  ou  ")}`,
        fraco: `${u?.material || it.candidatas[0]?.material || ""}`,
        anotar: true,
      };
    case "SEM_CORRIDA":
      return { forte: `R ${u?.rastreio || "—"}  ·  SEM CORRIDA lançada no CMR`, fraco: u?.material || "", anotar: true };
    case "ESTOQUE":
      return { forte: `MATERIAL DE ESTOQUE — cortada antes de qualquer entrega desta OP${cand.length ? ` (entradas da OP: ${cand.join(", ")})` : ""}`, fraco: "", anotar: true };
    default:
...
export async function carimbarDesenho(pdfBytes, info) {
  ...
  if (itens.length === 1) {
    ...
    const c = { DEFINIDA: 0, INDEFINIDA: 0, SEM_CORRIDA: 0, ESTOQUE: 0, SEM_MATERIAL: 0 };
    for (const it of itens) c[it.situacao] = (c[it.situacao] || 0) + 1;
    const pend = itens.length - c.DEFINIDA;
    l1 = `CONJUNTO — ${itens.length} posições · ${c.DEFINIDA} com corrida definida${pend ? ` · ${pend} a confirmar` : ""}`;
    l2 = itens.filter((it) => it.situacao === "DEFINIDA").slice(0, 3)
      .map((it) => `${it.marca}: R ${it.usadas?.[0]?.rastreio || "—"}`).join("  ·  ") + (c.DEFINIDA > 3 ? "  ·  …" : "");
    anotar = pend > 0;
  } else {
  ...
  for (const page of pdf.getPages()) {
    ...
    txt(m + 8 * k, y - 12 * k, "RASTREABILIDADE DO MATERIAL", { size: 6.5 * k, f: bold, cor: CINZA, max: colRastreio - 10 * k });
    txt(m + colRastreio, y - 11 * k, l1, { size: 8.5 * k, f: bold, cor: PRETO });
    if (l2) txt(m + colRastreio, y - 22 * k, l2, { size: 7 * k, cor: CINZA });
    if (anotar) txt(m + 8 * k, y - 36 * k, "Corrida efetivamente usada: ______________________   Nº R: ____________   Visto: __________", { size: 7.5 * k, f: bold, cor: PRETO });
