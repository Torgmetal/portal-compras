...
export async function carimbarDesenho(pdfBytes, info) {
  ...
  for (const it of itens) {
    ...
    const rs = [...new Set((it?.usadas || []).map((u) => u?.rastreio).filter(Boolean))];
    if (k && rs.length) rPorMarca.set(k, rs);
  }
  ...
  for (const c of posicoes) {
    const rs = rPorMarca.get(String(c.marca).trim().toUpperCase());
    if (!rs?.length) continue;               // posição sem R: linha fica em branco, como hoje
    const page = pdf.getPage(c.pagina - 1);
    if (!page) continue;
    // ⚠ mais de um R na mesma posição acontece (a peça saiu de duas barras) — vai "R 1 / R 2".
    const alvo = rs.length === 1 ? `R ${rs[0]}` : `R ${rs.join(" / R ")}`;
    const { s: escrito, sz } = caber(alvo, c.larg, bold, Math.min(7, c.alt * 0.5), 4);
    page.drawText(escrito, {
      // alinhado à DIREITA: a descrição cresce da esquerda, o R encosta na borda da coluna
      x: c.x - bold.widthOfTextAtSize(escrito, sz),
      y: c.y - sz * 0.35,
