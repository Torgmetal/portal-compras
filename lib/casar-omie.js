...
/* ⚠⚠ CHAPA MÉTRICA DO MODELO × CHAPA COMPRADA EM POLEGADA — SÓ CHAPA. Vitor (25/09/2026), no caso da
   OP-118 levantado pelo Gabriel (Engenharia): "mais um caso daquele de chapa com espessura
   diferente (…) não consigo nem procurar o R pra colocar igual o da 9.50mm". O Tekla escreve 10 e
   12; a chapa que chega é a de 3/8" (9,50) e a de 1/2" (12,50). Sem o par, a janela "Selecionar R"
   abria vazia e a gravação recusava o R da própria obra.

   ⚠ NÃO ENTRA EM MESMA_BITOLA. Aquela tabela vale para todo perfil (é lida por `perto()`, que o tubo,
   a cantoneira e a barra também usam); lá, 9,5 e 10 são peças diferentes. Aqui só a espessura da
   CHAPA. Mesma regra de lá: par nomeado, com quem decidiu — o portal amplia os CANDIDATOS; quem
   declara o fardo continua sendo gente (TrocaRastreabilidade). */
const MESMA_CHAPA = [
  [9.50, 10.00],  // 3/8" — Vitor, 25/09/2026 (OP-118, T118B-P294 CH10.00X120)
  [12.00, 12.50], // 1/2" — Vitor, 25/09/2026 (OP-118, T118B-P17 CH12.00X118)
];
const mesmaChapaComercial = (a, b) =>
  MESMA_CHAPA.some((g) => g.some((x) => Math.abs(x - a) < 0.01) && g.some((x) => Math.abs(x - b) < 0.01));
...
function pontua(tipo, perfil, item) {
  ...
  switch (tipo) {
    case "CHAPA": {
      ...
      const esp = nP[0];
      const mesmaEspessura = (y) => perto(esp, y) || mesmaChapaComercial(esp, y);
      const espOmie = (norm(item.descricao).match(/ESPESSURA\s*(\d+(?:\.\d+)?)/) || [])[1];
      if (espOmie) return mesmaEspessura(Number(espOmie)) ? 3 : 0;
      ...
      if (cands.some(mesmaEspessura)) return 3;
      ...
      return emMm.some(mesmaEspessura) ? 3 : 0;
    }
    case "U": {
