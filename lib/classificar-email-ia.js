...
- confianca: 0.0 a 1.0.
- Um item por e-mail, com "i" = o número do e-mail na lista recebida.`;

export const ESQUEMA_CLASSIFICACAO = Esquema.objeto({
  itens: Esquema.lista(Esquema.objeto({ i: Esquema.inteiro, marco: Esquema.umDe(TAGS), confianca: Esquema.numero })),
});
...
export async function classificarLoteIA(emails) {
  ...
  const { dados } = await pedirJson(anthropic, {
    ...
    formato: ESQUEMA_CLASSIFICACAO,
  });
  const itens = Array.isArray(dados?.itens) ? dados.itens : [];
  return emails.map((_, idx) => {
    const it = itens.find((x) => Number(x.i) === idx + 1) || {};
    const marco = TAGS.find((t) => t === String(it.marco || "").toUpperCase()) || "OUTRO";
