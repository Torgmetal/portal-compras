...
export async function reconciliarCmr(prisma, ano, { userId = null } = {}) {
  ...
  for (const r of sheet) {
    ...
    for (const c of CAMPOS) {
      const novo = d[c];
      const velho = atual[c];
      // Vazio: nome placeholder; qtd/peso 0 conta como vazio (foram importados 0 antes de a
      // planilha ter o valor — sem isso o "13300KG" nunca entrava); demais, null/"".
      const velhoVazio = c === "nome"
        ? VAZIO_NOME(velho)
        : (c === "pesoKg" || c === "quantidade")
        ? (velho == null || Number(velho) === 0)
        : (velho == null || so(velho) === "");
