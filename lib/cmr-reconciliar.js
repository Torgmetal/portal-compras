 ...
 */
/**
 * A DESCRIÇÃO MUDOU DE MATERIAL, ou só foi CORRIGIDA?
 *
 * ⚠⚠ AS DUAS PRIMEIRAS "TROCAS" REAIS FORAM ALARME FALSO (medido na sincronização de 22/09/2026):
 ...
 *
 * Isso importa porque troca de material LIMPA os campos que a planilha não traz. Chamar uma
 * correção de descrição de "troca" apagaria o certificado de um material que está certo — o
 * defeito que tudo isto existe para impedir, com o sinal invertido.
 *
 * ⚠⚠ O CERTIFICADO É QUEM DESEMPATA. Se o número do certificado ou a corrida continuam os mesmos,
 * é a MESMA coisa física com outro nome — não importa o quanto o texto mudou.
 *
 * ⚠ Sem esse desempate, o texto responde por DUAS perguntas: mudou o SUBSTANTIVO (chapa × porca)
 * ou conflitam as MEDIDAS (200×26,6 × 250×25,3)? Qualquer uma das duas é troca. Acabamento e grafia
 * no resto continuam não sendo.
 */
export function ehOutroMaterial(antes, depois, rastro = {}) {
  if (VAZIO_NOME(antes) || VAZIO_NOME(depois) || ehProvisorio(antes)) return false;

  // ⚠⚠ A RASTREABILIDADE DESEMPATA PRIMEIRO, E ISSO É UMA MUDANÇA DE ORDEM (achado do Codex,
  // 22/09/2026). Antes o texto era consultado antes dela: bastava a primeira palavra coincidir para
  // a função dizer "mesmo material" sem nunca olhar o certificado. Certificado ou corrida iguais dos
  // dois lados provam que é a MESMA coisa física com outro nome — não há texto que derrube isso.
  // ⚠ Só conta quando a informação EXISTE dos dois lados: dois vazios não provam nada.
  const igual = (a, b) => so(a) !== "" && so(a) === so(b);
  if (igual(rastro.certificadoAntes, rastro.certificadoDepois)) return false;
  if (igual(rastro.corridaAntes, rastro.corridaDepois)) return false;

  // Sem esse desempate sobra o texto — e ele vale por DUAS perguntas, não uma.
  return substantivo(antes) !== substantivo(depois) || medidasConflitam(antes, depois);
}

const semAcento = (v) => so(v).toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");

/** O substantivo do material: é ele que separa uma CHAPA de uma PORCA. */
const substantivo = (v) => semAcento(v).replace(/[^A-Z0-9 ]/g, " ").trim().split(/\s+/)[0] || "";

/**
 * ⚠⚠ EM AÇO, A BITOLA É O MATERIAL (achado do Codex, 22/09/2026). A comparação era só pela primeira
 * palavra, então `PERFIL W 200X26,6` → `PERFIL W 250x25,3` passava como "mesmo material, nome
 * corrigido" — e, como troca é justamente o que LIMPA os campos que a planilha não traz, o perfil
 * novo herdava certificado, corrida e NF do anterior. É o híbrido do R 261547 voltando pela porta
 * estreita: mesma primeira palavra, outra peça.
 *
 * ⚠⚠ MAS SÓ CONFLITO CONTA — NÚMERO A MAIS É COMPLETAR, NÃO TROCAR. Exigir conjuntos idênticos
 * transformaria `CHAPA A-36` → `CHAPA A-36 ESPESSURA 4,75MM` numa troca, e o defeito voltaria com o
 * sinal invertido: apagaria a rastreabilidade CERTA de quem só detalhou a descrição. Conflito é cada
 * lado ter medida que o outro não tem; um lado contido no outro é a mesma peça, descrita com mais
 * cuidado.
 *
 * ⚠ Vírgula e ponto são a mesma coisa aqui ("4,75" = "4.75"): a planilha é digitada por gente.
 */
function medidasConflitam(antes, depois) {
  const a = medidas(antes), b = medidas(depois);
  return !subsequencia(a, b) && !subsequencia(b, a);
}

/**
 * AS MEDIDAS DA DESCRIÇÃO, COM A FRAÇÃO VALENDO UM NÚMERO SÓ.
 *
 * ⚠⚠ NUMERADOR E DENOMINADOR SOLTOS NÃO SERVEM (achado do Codex, 22/09/2026 — terceira rodada
 * sobre o mesmo defeito). `CANTONEIRA 2 X 2 X 1/4` dava [2,2,1,4] e `CANTONEIRA 2 X 2 X 1 1/4` dava
 * [2,2,1,1,4]: a primeira é subsequência da segunda, então a espessura indo de 1/4 para 1.1/4
 * passava como "detalhou a descrição" e a peça nova herdava certificado, corrida e NF da anterior.
 * Com a fração virando VALOR, viram [2,2,0.25] e [2,2,1.25] — conflito, que é o que é.
 *
 * ⚠⚠ E A FRAÇÃO MISTA SÓ CONTA SE O INTEIRO ESTIVER SOLTO. Sem isso, `PORCA A563 3/8` leria
 * "563 3/8" como 563,375 — e a MESMA porca escrita `A563 - 3/8"` leria [563; 0,375], virando troca
 * de material onde só mudou a pontuação. O `A` de A563 é o que separa a norma da bitola.
 */
function medidas(v) {
  const t = semAcento(v);
  const num = (x) => Number(String(x).replace(",", "."));
  const achados = [];
  const re = /(?<![A-Z0-9])(\d+)\s+(\d+)\/(\d+)|(\d+)\/(\d+)|\d+(?:[.,]\d+)?/g;
  for (const m of t.matchAll(re)) {
    if (m[1] !== undefined) achados.push(num(m[1]) + num(m[2]) / num(m[3]));   // 1 1/4
    else if (m[4] !== undefined) achados.push(num(m[4]) / num(m[5]));          // 3/8
    else achados.push(num(m[0]));                                             // 4,75
  }
  return achados;
}

/**
 * `curta` aparece em `longa` na MESMA ORDEM (podendo pular termos)?
 *
 * ⚠⚠ CONJUNTO NÃO SERVE, E O CONTRAEXEMPLO É ESTE (achado do Codex, 22/09/2026): `CANTONEIRA
 * 2 X 2 X 1/4` e `CANTONEIRA 2 X 4 X 1/4` têm o MESMO conjunto de números, {2, 1, 4} — são duas
 * cantoneiras diferentes que o `Set` declarava idênticas, e aí a segunda herdava certificado,
 * corrida e NF da primeira. Conjunto joga fora as duas coisas que a bitola usa para significar:
 * a ORDEM (2×2×1/4 ≠ 2×4×1/4) e a REPETIÇÃO (o 2 aparece duas vezes num e uma no outro).
 *
 * ⚠ Subsequência é o que separa as duas perguntas: `CHAPA A-36` [36] cabe em ordem dentro de
 * `CHAPA A-36 ESPESSURA 4,75MM` [36; 4,75] — detalhou. Já [2;2;1;4] não cabe em [2;4;1;4] nem o
 * contrário — trocou.
 */
function subsequencia(curta, longa) {
  let i = 0;
  for (const n of longa) if (i < curta.length && curta[i] === n) i += 1;
  return i === curta.length;
}

/**
 * ⚠ "A CONFIRMAR", "A DEFINIR" — o almoxarifado reserva o R e escreve isso enquanto não sabe o que
 * chegou. É casca com texto: preencher depois não é trocar de material.
 */
const ehProvisorio = (v) => /\bA\s+(CONFIRMAR|DEFINIR|COMBINAR)\b/i.test(so(v));
...
export async function reconciliarCmr(prisma, ano, { userId = null } = {}) {
  ...
  // ⚠⚠ OS ACUMULADORES NASCEM ANTES DE QUEM ESCREVE NELES (achado do Codex, 22/09/2026). Eles
  // ficavam declarados DEPOIS deste laço: o `catch` da criação chamava `falhasDeCriacao.push` na
  // zona morta do `const`, então a primeira falha ao criar um R virava `ReferenceError` e derrubava
  // a rodada INTEIRA — criação, patches e envio para a planilha. O `catch` que existia para não
  // perder um R era justamente o que perdia todos os outros. Entrou quando a criação subiu para
  // antes dos patches ("precisamos dos R para outros setores usarem") e a declaração ficou para
  // trás; nenhum teste pegou porque nenhum simulava criação que rejeita.
  const trocas = [];
  const falhas = [];
  ...
  for (const r of sheet) {
    ...
    const trocou = ehOutroMaterial(atual.nome, d.nome, {
      certificadoAntes: atual.numeroDocumento, certificadoDepois: d.numeroDocumento,
      corridaAntes: atual.numeroCorrida, corridaDepois: d.numeroCorrida,
    });
