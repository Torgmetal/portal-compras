...
export async function limparLinhaCmr(ano, indiceR) {
  ...
  try {
    ...
    const ur = await fetch(`${base}/worksheets('${enc}')/usedRange(valuesOnly=true)?$select=address`, { headers: H });
    const ultima = Number((String((await ur.json()).address).match(/:[A-Z]+(\d+)\s*$/) || [])[1]) || 4;
    if (ultima < 5) return { ok: true, encontrado: false };
    // Acha a linha pelo índice R (coluna B).
    ...
    let linha = -1;
    for (let i = 0; i < vals.length; i++) { if (so(vals[i][0]) === so(indiceR)) { linha = 5 + i; break; } }
    if (linha < 0) return { ok: true, encontrado: false };
    // Limpa A (R/RC) e C:N (dados) — mantém B (índice R) reservado.
    const c1 = await fetch(`${base}/worksheets('${enc}')/range(address='${encodeURIComponent(`A${linha}`)}')/clear`, { method: "POST", headers: H, body: JSON.stringify({ applyTo: "Contents" }) });
    const c2 = await fetch(`${base}/worksheets('${enc}')/range(address='${encodeURIComponent(`C${linha}:N${linha}`)}')/clear`, { method: "POST", headers: H, body: JSON.stringify({ applyTo: "Contents" }) });
    return { ok: c1.ok && c2.ok, encontrado: true, linha };
  } finally {
    await fetch(`${base}/closeSession`, { method: "POST", headers: H }).catch(() => {});
  }
}
