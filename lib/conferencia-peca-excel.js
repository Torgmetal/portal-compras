...
import { dataHoraBR, dataBR } from "./data-br";
...
/**
 * As 11 células da linha de uma marca.
 *
 ...
 *
 * ⚠ Marca que não foi lançada NESTA conferência sai em BRANCO nas duas últimas, não com "0" e
 * "—": o branco lê-se como "isto não é desta remessa", que é a pergunta que o PCP faz ao abrir a
 * segunda planilha da mesma obra.
 */
const celulasDaMarca = (l) => [
  l.marca, l.descricao || "—", l.previsto, l.conferido, l.saldo, rotuloSituacao(l.situacao),
  l.pesoUnitKg || "", l.pesoConferidoKg || "", l.observacoes || "",
  l.nestaRemessaQte || "", l.nestaRemessaEm ? dataBR(l.nestaRemessaEm) : "",
  l.conferidaEm ? dataBR(l.conferidaEm) : "",
];
...
export async function gerarConferenciaExcel(rel) {
  ...
  sheet.columns = [{ width: 22 }, { width: 44 }, { width: 11 }, { width: 12 }, { width: 10 },
    { width: 16 }, { width: 13 }, { width: 14 }, { width: 38 }, { width: 14 }, { width: 14 },
    { width: 14 }];
  let linha = linhaInicio;
  // ⚠⚠ AS DUAS ÚLTIMAS COLUNAS ENTRARAM NO FIM, E ISSO É REQUISITO. A ordem anterior é contrato
  // com o PCP (ver testes/lib/conferencia-excel-colunas.teste.js): eles copiam a coluna A e já
  // têm o olho nas posições das outras. Coluna nova no meio empurraria tudo sem avisar ninguém.
  adicionarHeaderTabela(sheet, linha, ["Marca", "Descrição", "Previsto", "Conferido", "Saldo",
    ...
    // ⚠⚠ ESTA É DA OBRA INTEIRA, e é o que permite a planilha ser lida sozinha: numa remessa
    // parcial, a marca que veio de três semanas atrás mostra a data dela aqui, em vez de deixar
    // o PCP abrir os arquivos anteriores para descobrir. As duas ao lado continuam dizendo o que
    // é NOVO — perguntas diferentes, colunas diferentes.
    "Conferida em"]);
  linha++;
  const primeira = linha;
  for (const l of rel.linhas) {
    adicionarLinhaTabela(sheet, linha, celulasDaMarca(l), {
      fillColor: FUNDO[l.situacao],
      alinhamento: { 2: "right", 3: "right", 4: "right", 5: "center", 6: "right", 7: "right",
        9: "right", 10: "center", 11: "center" },
    });
    linha++;
  }
  if (rel.linhas.length) {
    adicionarLinhaTotais(sheet, linha, ["TOTAL", "",
      ...
      { formula: `SUM(J${primeira}:J${linha - 1})` }, "", ""]);
  }
