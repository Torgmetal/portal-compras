...
export async function gerarConferenciaExcel(rel) {
  ...
  sheet.columns = [{ width: 22 }, { width: 44 }, { width: 11 }, { width: 12 }, { width: 10 }, { width: 16 }];
  let linha = linhaInicio;
  adicionarHeaderTabela(sheet, linha, ["Marca", "Descrição", "Previsto", "Conferido", "Saldo", "Situação"]);
  linha++;
  const primeira = linha;
  for (const l of rel.linhas) {
    adicionarLinhaTabela(sheet, linha, [l.marca, l.descricao || "—", l.previsto, l.conferido, l.saldo, rotuloSituacao(l.situacao)], {
      fillColor: FUNDO[l.situacao],
      alinhamento: { 2: "right", 3: "right", 4: "right", 5: "center" },
    });
    linha++;
  }
