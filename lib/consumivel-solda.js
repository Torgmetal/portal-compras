 ...
 * Origem devolvida:
 *   "apontamento" — soldado; o lote é o que estava valendo naquele dia (fato)
 *   "prevista"    — ainda não soldado; vale o lote de hoje, e o carimbo diz que é previsão
 * Quando a solda atravessa a troca de lote, devolve os DOIS em `janela` — inventar um só seria
 * exatamente o "provável" que não entra em documento de auditoria.
 */
export async function consumivelDoConjunto({ opId, marca, quando = new Date(), entradas = null }) {
  ...
  if (!datas.length) {
    ...
    return c ? { ...c, origem: "prevista", soldadoEm: null } : null;
  }
