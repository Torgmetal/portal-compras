...
/**
 * Espessura SECA de cálculo da demão, em µm.
 *
 * ⚠⚠ MÁXIMA PRIMEIRO. Vitor (07/09/2026): "sempre calcular com espessura máxima". Quando o plano
 * dá uma FAIXA, é a máxima que dimensiona a compra: calcular pela mínima subestima o consumo, e
 * tinta que falta no meio da obra para o galpão. Errar para cima sobra galão no almoxarifado;
 * errar para baixo para a pintura.
 *
 * ⚠ NÃO CONFUNDIR COM O QUE O INSPETOR MEDE. A mínima continua sendo a espessura que o medidor tem
 * de ENCONTRAR na peça — isso é critério de aceitação e vive no PLP. Aqui é outra pergunta: quanto
 * de tinta comprar e quanto o pente deve marcar no pior caso.
 *
 * ⚠ A MÍNIMA É O RESGATE porque nem todo PLP preenche a mesma coluna: a OP-112 tem 100 em
 * `espessuraMax` e nada em `espessuraMin`; a 094 e a 106 têm o contrário. Ler só uma das duas
 * mostrava "—" com o número gravado ao lado.
 */
export function espessuraSeca(demao) {
  return Number(demao?.espessuraMax) || Number(demao?.espessuraMin) || null;
}
...
export function consumoDoPlano(plano, areaM2, catalogos = {}) {
  ...
  // ⚠⚠ NENHUMA CAMADA FECHOU → O TOTAL É NULO, NÃO ZERO. A guarda por camada já devolvia `falta`
  // em vez de zero, mas o TOTAL somava nulos e virava 0 — e "0 galões" lê como "não precisa
  // comprar", que é o oposto de "não sabemos". Apareceu na OP-067: sem sólidos por volume, o
  // caderno dizia "0 L · falta 0 galões" para uma obra com 345 m² por pintar.
  const nenhumaFechou = camadas.length > 0 && camadas.every((c) => c.falta);
  return {
    camadas,
    ...
    total: {
      litros: nenhumaFechou ? null : Math.round(somar("litros") * 100) / 100,
      galoes: nenhumaFechou ? null : somar("galoes"),
      diluente: nenhumaFechou ? null : Math.round(somar("diluente") * 100) / 100,
