...
/**
 * ESTA CÉLULA É UMA OFERTA QUE DÁ PARA COMPRAR?
 *
 * ⚠⚠ "RESPONDEU" E "OFERTOU" SÃO COISAS DIFERENTES, e a tela do mapa misturava as duas: ela só
 * criava célula para item com preço, então quem clicou em "Não tenho" sumia e virava o mesmo "—"
 * de quem não respondeu nada (Matheus, 21/09/2026: "deve aparecer que ele respondeu NÃO TEM e não
 * ficar apenas sem valor"). Agora a recusa aparece, e é ESTA função que separa o que pode virar
 * pedido do que é só resposta.
 *
 * ⚠⚠ E É POR ISSO QUE ELA OLHA `semEstoque` ALÉM DO PREÇO. Preço zero sozinho não basta: o
 * fornecedor pode reenviar a cotação e marcar "não tenho" num item que JÁ estava vencedor — a rota
 * pública gravava a indisponibilidade sem limpar `vencedor` (achado do Codex, 21/09/2026). Medido
 * no mesmo dia: 0 casos no banco, mas o caminho existe, e um vencedor de preço zero entraria em
 * total e em "itens sem vencedor" como se fosse compra.
 *
 * @param {{semEstoque?:boolean, precoUnit?:number}|null|undefined} item
 */
export const ofertaValida = (item) => !item?.semEstoque && Number(item?.precoUnit) > 0;
