...
const ESPERADOS = [
  ...
  // ⚠ a cópia dos arquivos que só existiam no Blob (anexos do Data Book, fotos de inspeção) — 24/09/2026
  { job: "backup-arquivos",          label: "Backup de arquivos (SharePoint)", path: "/api/cron/backup-arquivos" },
  ...
  { job: "data-book",                label: "Data Book · certificados e volumes", path: "/api/cron/data-book" },
  ...
  { job: "omie-encerrados",          label: "Pedidos encerrados (Omie)",   path: "/api/cron/omie-encerrados" },
  { job: "produtos-omie",            label: "Produtos (Omie)",             path: "/api/compras/produtos-omie/sincronizar" },
  // ⚠ a pasta que alimenta o Tekla (Vitor, 24/09/2026) — planilha nova quando o Omie ganha perfil/parafuso
  { job: "materiais-tekla",          label: "Materiais OMIE → Tekla",      path: "/api/cron/materiais-tekla" },
  ...
  // ⚠ a LQC do SharePoint só existia como chamada manual e ninguém chamava (última importação 30/08;
  // Vitor, 16/09/2026: "as LQCs vc está atualizando?"). Agendada às :40, depois dos orçamentos.
  { job: "lqc-sharepoint",           label: "LQC do SharePoint",           path: "/api/comercial/estudos/importar-sharepoint" },
  { job: "conferir-listas",          label: "L.E. servidor × portal",      path: "/api/cron/conferir-listas" },
  { job: "fiscal-fontes",            label: "TIPI e NCM (Receita)",        path: "/api/cron/fiscal-tipi" },
  { job: "fiscal-ibs-cbs",           label: "Regras IBS/CBS (NFs do Omie)", path: "/api/cron/fiscal-ibs-cbs" },
  // ⚠ SEMANAL, então a tolerância é de semana: cobrar em 30 h alertaria seis dias por semana.
  { job: "fiscal-legislacao",        label: "Legislação fiscal (SEFAZ/SP)", path: "/api/cron/fiscal-legislacao", maxHoras: 24 * 8 },
];
