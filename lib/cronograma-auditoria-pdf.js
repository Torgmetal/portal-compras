...
import { dataBR, diaBRT, hojeBRT } from "./data-br";
...
export async function gerarCronogramaAuditoriaPDF({ ano = new Date().getUTCFullYear(), revisao = 0, auditorias = [], assinaturas = null }) {
  ...
  // ⚠ ATRASADA só depois que o DIA passa. Comparar com `new Date()` (o instante) marcava como
  // atrasada a auditoria marcada para HOJE — a data é gravada ao meio-dia UTC, então às 9h da manhã
  // de Brasília ela já ficava "no passado". Compara-se o dia-calendário BRT (Vitor, 27/08/2026).
  const hoje = hojeBRT();
  const atrasada = (a) => a.status === "AGENDADA" && a.dataAuditoria && (diaBRT(a.dataAuditoria) || "") < hoje;
  ...
  paginas.forEach((p, i) => {
    p.drawText(san(`Torg Metal · ${refFORM(29)} · Cronograma de Auditoria Interna ${ano} · ${fmtRev(revisao)}`), { x: M, y: 28, size: 7.5, font, color: GRAY });
