...
export async function gerarCronogramaAuditoriaPDF({ ano = new Date().getUTCFullYear(), revisao = 0, auditorias = [], assinaturas = null }) {
  ...
  const cols = [{ t: "RAI", w: 52 }, { t: "Data", w: 60 }, { t: "Setor / processo", w: W - 52 - 60 - 130 - 62 }, { t: "Responsável", w: 130 }, { t: "Status", w: 62 }];
  const drawHead = () => {
    espaco(24);
  ...
  ord.forEach((a, i) => {
    ...
    page.drawText(san(STATUS_LABEL[a.status] || a.status || "—"), { x: cx, y: y - 11, size: 7.5, font, color: GRAY });
    y -= rh;
