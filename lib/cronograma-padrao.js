...
import { lagDaEtapa, ETAPA_ANTERIOR } from "@/lib/cronograma-sobreposicao";
...
export async function criarCronogramaPadrao({ opId, opNumero, titulo, dataInicio, dataFim, comFd = false }) {
  ...
  for (let i = 1; i < seq.length; i++) {
    ...
    /* ⚠⚠ CRONOGRAMA NOVO NÃO NASCE EM FILA. Vitor (07-08/09/2026): "quando a montagem já tiver uma
       quantidade de peças prontas a solda já tem que começar… isso sempre foi um erro no nosso
       cronograma" e, no dia seguinte, "concordo com vc, precisamos implantar". O encadeamento
       finish-to-start puro é o que reproduzia o erro em TODA obra nova; a defasagem medida
       (lib/cronograma-sobreposicao.js) entra aqui, no nascimento.
       ⚠ O lag sai da duração da antecessora JÁ ENCAIXADA no prazo (a `dur()` acima encolhe o
       modelo quando a OP é curta) — usar a do template daria um lag que não corresponde à barra. */
    const anterior = ETAPA_ANTERIOR[t.nome];
    const lag = anterior && seq[i - 1]?.nome === anterior
      ? lagDaEtapa(t.nome, tarefas.find((x) => x.uidMpp === seq[i - 1].uidMpp)?.duracaoDias)
      : 0;
    if (ants.length || lag) {
      await prisma.cronogramaTarefa.update({
        where: { id: t.id },
        data: { ...(ants.length ? { antecessoraIds: ants } : {}), ...(lag ? { defasagemDias: lag } : {}) },
      });
    }
  }
