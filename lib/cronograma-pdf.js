...
export async function gerarCronogramaPDF(cronograma, tarefas, now = new Date()) {
  ...
  const t0 = cronograma.dataInicio ? utc0(cronograma.dataInicio) : (starts.length ? Math.min(...starts) : utc0(now));
  const t1 = cronograma.dataFim ? utc0(cronograma.dataFim) : (finishes.length ? Math.max(...finishes) : t0 + 30 * DIA);
  ...
  paginas.forEach((rows) => {
    ...
    rows.forEach((t, i) => {
      ...
      if (t.dataInicioPrevista && t.dataFimPrevista) {
        const bx0 = xOf(utc0(t.dataInicioPrevista)), bx1 = Math.max(bx0 + 2, xOf(utc0(t.dataFimPrevista) + DIA));
        const bw = bx1 - bx0;
        if ((t.duracaoDias || 0) === 0 || bw < 4) {
          page.drawRectangle({ x: bx0 - 3, y: cy - 3, width: 6, height: 6, color: NAVY, rotate: degrees(45) }); // marco
        } else if (t.isSummary) {
