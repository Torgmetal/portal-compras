...
export async function gerarCronogramaPDF(cronograma, tarefas, now = new Date()) {
  ...
  const legenda = () => {
    ...
    item(BLUE_LT, "Previsto"); item(BLUE, "Realizado"); item(NAVY, "Marco", true); item(RED, "Bloqueada");
  ...
  paginas.forEach((rows) => {
    ...
    rows.forEach((ln, i) => {
      ...
      page.drawText(fit(nomeTxt, nomeF, 8, wTarefa - 8 - ind), { x: cTarefa + 4 + ind, y: cy - 3, size: 8, font: nomeF, color: bloqueada ? RED : DARK });
