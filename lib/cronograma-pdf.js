...
export async function gerarCronogramaPDF(cronograma, tarefas, now = new Date()) {
  ...
  paginas.forEach((rows) => {
    ...
    rows.forEach((ln, i) => {
      ...
      if (ln.grupo) {
        const sub = ln.sub;
        page.drawRectangle({ x: 0, y: y - rowH, width: PW, height: rowH, color: sub ? rgb(0.98, 0.95, 0.86) : BLUE_LT });
        page.drawLine({ start: { x: 0, y: y - rowH }, end: { x: PW, y: y - rowH }, thickness: 0.5, color: sub ? rgb(0.85, 0.72, 0.4) : BLUE });
        page.drawText(san((sub ? "▸ " : "") + ln.grupo.toUpperCase()), { x: cTarefa + (sub ? 14 : 4), y: y - rowH + 5, size: sub ? 7.5 : 8, font: bold, color: sub ? rgb(0.5, 0.37, 0.05) : NAVY });
        y -= rowH;
        return;
      }
      const t = ln.t;
