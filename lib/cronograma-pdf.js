...
export async function gerarCronogramaPDF(cronograma, tarefas, now = new Date()) {
  ...
  for (const d of ORDEM_DEPT) {
    ...
    for (const t of doDept) { linhas.push({ t }); usados.add(t.id); }
  }
  ...
  paginas.forEach((rows) => {
    ...
    rows.forEach((ln, i) => {
      // faixa da ÁREA
      if (ln.grupo) {
        page.drawRectangle({ x: 0, y: y - rowH, width: PW, height: rowH, color: BLUE_LT });
        page.drawLine({ start: { x: 0, y: y - rowH }, end: { x: PW, y: y - rowH }, thickness: 0.5, color: BLUE });
        page.drawText(san(ln.grupo.toUpperCase()), { x: cTarefa + 4, y: y - rowH + 5, size: 8, font: bold, color: NAVY });
        y -= rowH;
        return;
      }
      const t = ln.t;
