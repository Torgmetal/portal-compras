...
const RED = rgb(0.80, 0.12, 0.12); // tarefa bloqueada

// Áreas na ordem do fluxo (mesma sequência da tela e do template do MS Project)
const ORDEM_DEPT = ["COMERCIAL", "ENGENHARIA", "SUPRIMENTOS", "FABRICACAO", "EXPEDICAO", "MONTAGEM"];
...
export async function gerarCronogramaPDF(cronograma, tarefas, now = new Date(), opcoes = {}) {
  const refinado = !!opcoes.refinado;
  const PW = refinado ? 1190.55 : 841.89, PH = refinado ? 841.89 : 595.28;
  ...
  if (opcoes.comparativo) starts.push(...ts.filter(t => t.dataInicioBase).map(t => utc0(t.dataInicioBase)));
  const finishes = comData.map((t) => utc0(t.dataFimPrevista));
  if (opcoes.comparativo) finishes.push(...ts.filter(t => t.dataFimBase).map(t => utc0(t.dataFimBase)));
  // A linha do tempo abraça EXATAMENTE as tarefas (menor início, maior fim). NÃO
  // une com o período gravado (dataInicio/dataFim): ele defasa — só alarga, nunca
  // encolhe — então ao reduzir durações o fim ficava preso numa data velha e o
  // "Período" saía lá na frente. O intervalo das tarefas datadas cobre todas por
  // definição (não corta) e não infla com dado velho. Gravado só entra de fallback
  // se não houver tarefa com data.
  const t0 = starts.length ? Math.min(...starts) : (cronograma.dataInicio ? utc0(cronograma.dataInicio) : utc0(now));
  const t1 = finishes.length ? Math.max(...finishes) : (cronograma.dataFim ? utc0(cronograma.dataFim) : t0 + 30 * DIA);
  const span = Math.max(DIA, t1 - t0 + (refinado ? DIA : 0));

  // % concluído geral (média ponderada por duração)
  const pesoTot = ts.reduce((a, t) => a + Math.max(1, t.duracaoDias || 1), 0);
  const pctCalculado = pesoTot > 0 ? ts.reduce((a, t) => a + Math.max(1, t.duracaoDias || 1) * (t.percentualRealizado || 0), 0) / pesoTot : 0;

  const pctGeral = refinado ? opcoes.percentual : pctCalculado;

  // Colunas
  const cTarefa = M, wTarefa = refinado ? 300 : 220;
  ...
  const bandH = refinado ? 62 : 46, topInfo = refinado ? 34 : 22, headH = refinado ? 32 : 26;
  const rowH = refinado ? 20 : 15;
  ...
  const chrome = (gridBottom) => {
    ...
    if (refinado) page.drawText(fit(cronograma.titulo, bold, 14, PW - 2 * M - 380), { x: M + 110, y: PH - 32, size: 14, font: bold, color: WHITE });
    ...
    const iy = PH - bandH - (refinado ? 21 : 15);
    const periodoTxt = san(`Período: ${periodo}  ·  ${pctGeral == null ? "Avanço sem duração definida" : `${pctGeral.toFixed(0)}% de avanço`}`);
    ...
    page.drawRectangle({ x: 0, y: yHeadTop - headH, width: PW, height: headH, color: refinado ? NAVY : LIGHT, borderColor: LINE, borderWidth: 0.5 });
    const hc = (t, x, w, right) => { const s = fit(t, bold, 8, w - 6); page.drawText(s, { x: right ? x + w - 4 - bold.widthOfTextAtSize(s, 8) : x + 4, y: yHeadTop - headH + 9, size: 8, font: bold, color: refinado ? WHITE : GRAY }); };
    ...
    while (+m <= t1) {
      ...
      if (x1 - x0 > 1) {
        if (alt && !refinado) page.drawRectangle({ x: x0, y: gridBottom, width: x1 - x0, height: yHeadTop - gridBottom, color: rgb(0.985, 0.99, 0.995) });
        ...
        if (x1 - x0 > wid(lbl, bold, 7.5) + 4) page.drawText(lbl, { x: x0 + (x1 - x0) / 2 - wid(lbl, bold, 7.5) / 2, y: yHeadTop - headH + 9, size: 7.5, font: bold, color: refinado ? WHITE : GRAY });
      }
      alt = !alt;
      m = new Date(mEnd);
    }
    // grade semanal (segunda a segunda)
    const wkStart = t0 - ((new Date(t0).getUTCDay() + 6) % 7) * DIA;
  ...
  const legenda = () => {
    ...
    item(BLUE_LT, refinado ? "Programado" : "Previsto"); item(BLUE, refinado ? "Avanço informado" : "Realizado");
    if (opcoes.comparativo) item(GRAY, "Linha de base"); item(NAVY, "Marco", true); item(RED, "Bloqueada");
    page.drawText(san("TORG METAL"), { x: PW - M - wid("TORG METAL", bold, 7.5), y: ly - 2, size: 7.5, font: bold, color: GRAY });
  };

  // Agrupa por ÁREA, como a tela do portal e o próprio MS Project (onde a área
  // é a tarefa-resumo de nível 1). Sem isso o PDF saía numa lista corrida por
  // UID, misturando Comercial, Engenharia e Suprimentos na mesma sequência.
  // As tarefas-resumo de nível <=1 viram o cabeçalho do grupo (não repetem como
  // linha); sub-resumos mais fundos continuam aparecendo.
  const linhas = [];
  const usados = new Set();
  const ehCabecalhoDeArea = (t) => t.isSummary && (t.outlineLevel || 0) <= 1;
  for (const d of ORDEM_DEPT) {
    const doDept = ts.filter((t) => t.departamento === d && !ehCabecalhoDeArea(t));
    if (!doDept.length) continue;
    linhas.push({ grupo: DEPT_LABEL[d] || d });
    // Sub-agrupa por Área (Setor → Área → Tarefa). Sem área = tarefas direto no setor.
    const mapa = new Map();
    ...
    const temArea = [...mapa.keys()].some((k) => k);
    for (const [area, arr] of mapa) {
      if (area) linhas.push({ grupo: area, sub: true });
      else if (temArea) linhas.push({ grupo: "Sem área", sub: true });
      for (const t of arr) linhas.push({ t });
    }
  }
  const sobra = ts.filter((t) => !usados.has(t.id) && !ehCabecalhoDeArea(t));
  if (sobra.length) {
    linhas.push({ grupo: "Sem área" });
    for (const t of sobra) linhas.push({ t });
  }

  const paginas = [];
  for (let i = 0; i < linhas.length; i += rowsPorPag) paginas.push(linhas.slice(i, i + rowsPorPag));
  ...
  paginas.forEach((rows) => {
    ...
    rows.forEach((ln, i) => {
      ...
      if (ln.grupo) {
        const sub = ln.sub;
        // Sub-cabeçalho de ÁREA: cor própria (mesma paleta do app). "Sem área" fica neutro.
        const cor = sub && ln.grupo !== "Sem área" ? corDaArea(ln.grupo, cronograma.areas) : null;
        const fill = cor ? rgb(...cor.rgb.bg) : (sub ? rgb(0.95, 0.95, 0.95) : BLUE_LT);
        const traco = cor ? rgb(...cor.rgb.border) : (sub ? rgb(0.8, 0.8, 0.8) : BLUE);
        const txtCor = cor ? rgb(...cor.rgb.text) : (sub ? rgb(0.4, 0.4, 0.4) : NAVY);
        page.drawRectangle({ x: 0, y: y - rowH, width: PW, height: rowH, color: fill });
        if (cor) page.drawRectangle({ x: 0, y: y - rowH, width: 4, height: rowH, color: traco }); // faixa lateral da cor
        page.drawLine({ start: { x: 0, y: y - rowH }, end: { x: PW, y: y - rowH }, thickness: 0.5, color: traco });
        // Sem prefixo "▸" (fora do WinAnsi da Helvetica → virava "?"); a faixa lateral colorida + o recuo já marcam a área.
        page.drawText(san(ln.grupo.toUpperCase()), { x: cTarefa + (sub ? 12 : 4), y: y - rowH + 5, size: sub ? 7.5 : 8, font: bold, color: txtCor });
        y -= rowH;
        return;
      }
      const t = ln.t;
      const rowTop = y, cy = rowTop - rowH / 2;
      // Bloqueada = tem motivo de bloqueio e ainda NÃO foi liberada (mesma regra da tela).
      const bloqueada = !t.isSummary && !!t.motivoBloqueio && !t.dataLiberacao;
      if (i % 2 === 1) page.drawRectangle({ x: 0, y: rowTop - rowH, width: tlX - 2, height: rowH, color: rgb(0.985, 0.988, 0.992) });
      if (bloqueada) page.drawRectangle({ x: 0, y: rowTop - rowH, width: 3.5, height: rowH, color: RED }); // faixa lateral vermelha
      // nome (indentado por outlineLevel) — bloqueada sai em vermelho com o motivo junto.
      const ind = Math.max(0, (t.outlineLevel || 1) - 1) * 9;
      const nomeF = t.isSummary ? bold : font;
      const nomeTxt = bloqueada ? `${t.nome} — BLOQUEADA: ${t.motivoBloqueio}` : t.nome;
      page.drawText(fit(nomeTxt, nomeF, refinado ? 9 : 8, wTarefa - 8 - ind), { x: cTarefa + 4 + ind, y: cy - 3, size: refinado ? 9 : 8, font: nomeF, color: bloqueada ? RED : DARK });
      ...
      if (t.dataInicioPrevista) {
        ...
        if (!isNaN(ini)) {
          if (isNaN(fim) || fim <= ini) {
            const dx = xOf(ini);
            page.drawRectangle({ x: dx - 3, y: cy - 3, width: 6, height: 6, color: bloqueada ? RED : NAVY, rotate: degrees(45) }); // marco
            ...
            if (t.isSummary) {
              ...
              page.drawRectangle({ x: bx0, y: cy - 4, width: bw, height: 8, color: bloqueada ? rgb(0.98, 0.86, 0.86) : BLUE_LT, borderColor: bloqueada ? RED : BLUE, borderWidth: 0.5 });
              if (pct > 0) page.drawRectangle({ x: bx0, y: cy - 4, width: bw * Math.min(100, pct) / 100, height: 8, color: bloqueada ? RED : BLUE });
            }
          }
        }
      }
      if (opcoes.comparativo && t.dataInicioBase && t.dataFimBase) {
        const bx = xOf(utc0(t.dataInicioBase)), be = xOf(utc0(t.dataFimBase) + DIA);
        if (be >= bx) page.drawRectangle({ x: bx, y: cy - 8, width: Math.max(2, be - bx), height: 1.5, color: GRAY });
      }
    ...
    // Obs.: `rows` traz faixas de área ({grupo}) e tarefas embrulhadas ({t}).
    rows.forEach((ln) => {
      const t = ln.t;
      if (!t) return; // faixa de área
      const dst = pos[t.id];
      if (!dst) return;
  ...
  pdf.getPages().forEach((p, i, pages) => { if (refinado) p.drawText(`${i + 1} / ${pages.length}`, { x: PW / 2, y: 10, size: 7, font, color: GRAY }); });
