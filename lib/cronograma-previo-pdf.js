...
const uteisParaCorridosPdf = (d) => Math.round(((Number(d) || 0) * 7) / 5);
...
export async function gerarCronogramaPrevioPDF(dados = {}) {
  const cronCru = dados.cron || montarCronogramaPrevio({ pesoKg: dados.pesoKg, cargas: dados.cargas }, dados.cfg || {});

  // ── O QUE VAI NA FOLHA ────────────────────────────────────────────────────────────────────
  //
  // ⚠⚠ NEM TUDO QUE O ESTUDO SABE VAI PARA O CLIENTE. Vitor (05/09/2026): "precisamos ter como
  // editar as informações, pois tem horas que não vamos querer usar a quantidade de cargas para
  // mostrar ao cliente". Número de cargas é munição de negociação — em obra que o cliente vai
  // querer retirar por conta, ou quando o transporte ainda não está fechado, dizer "46 cargas" é
  // abrir uma discussão que a proposta não precisa ter. O cálculo continua inteiro; muda o que a
  // folha mostra.
  const f = dados.folha || {};
  const ocultas = new Set(f.etapasOcultas || []);
  const mostrarCargas = f.mostrarCargas !== false;
  const cron = {
    ...cronCru,
    fases: cronCru.fases
      .filter((x) => !ocultas.has(x.key))
      .map((x) => ({ ...x, nome: f.rotulos?.[x.key] || x.nome })),
  };
  const r = cron.resumo;
  ...
  txt(f.titulo || "CRONOGRAMA PRELIMINAR DE FORNECIMENTO", M, PH - 86, { size: 13, f: bold, cor: WHITE });
  ...
  const tiles = [
    ...
    ...(mostrarCargas ? [
      ...
      { r: "Intervalo", v: r.intervaloEntregasCorridos > 1 ? `${numBR(r.intervaloEntregasCorridos)} dias` : (r.cargas > 1 ? "1 dia" : "única"), n: r.cargas > 1 ? "entre uma carga e outra" : "ao final da fabricação" },
    ] : [
      // ⚠ sem as cargas a folha não fica com dois buracos: entra o que o cliente também pergunta.
      { r: "Fabricação", v: `${numBR(uteisParaCorridosPdf(cronCru.fases.find((x) => x.key === "producao")?.dias))} dias`, n: "fabricação e pintura" },
      { r: "Primeira entrega", v: `dia ${numBR(r.primeiraEntregaCorridos)}`, n: "entregas parciais durante a fabricação" },
    ]),
  ];
  ...
  for (const f of cron.fases) {
    ...
    // ⚠ as linhas de material sob medida têm chave "compra:XXX" — são compras, e a cor tem de
    // dizer isso. Sem este casamento elas saíam em navy, parecendo uma quinta fase do fluxo.
    const cor = COR_FASE[f.key] || (String(f.key).startsWith("compra:") ? COR_FASE.compras : NAVY2);
    page.drawRectangle({ x: x0, y: y - alt + 4, width: larg, height: alt - 8, color: cor });
  ...
  txtDir(r.ritmoEntregas || "", PW - M, y, { size: 8, cor: GRAY });
  y -= 8;
  ...
  // ⚠ sem data de início a coluna "Data" só teria traços — sai da tabela. (Vitor, 05/09/2026:
  // "sem informar data, apenas com a representação da duração".)
  const cabecalhos = comData
    ? [["Carga", 52], ["Dia", 44], ["Data", 68], ["Peso", 62]]
    : [["Carga", 52], ["Dia a partir da assinatura", 130], ["Peso", 62]];
  const colX = [];
  let cx = M;
  ...
  for (const e of linhas) {
    ...
    if (comData) txt(fmtData(e.data), colX[2] + 4, y - 10, { size: 8 });
    txt(`${numBR(e.kg)} kg`, colX[comData ? 3 : 2] + 4, y - 10, { size: 8 });
    y -= 14;
  }
  ...
  // ── PREMISSAS ──
  y -= 14;
  // ⚠ o texto do prazo é gerado, mas quem assina a proposta pode reescrever: `premissas` no
  // `folha` substitui a lista inteira, e `observacao` acrescenta uma linha ao fim.
  const texto = textoDaProposta(mostrarCargas ? cron : { ...cron, resumo: { ...r, cargas: 0 } }, {});
  const premissas = Array.isArray(f.premissas) && f.premissas.length ? [...f.premissas] : [
    texto,
  ...
  if (f.observacao) premissas.push(String(f.observacao));
