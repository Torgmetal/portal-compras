...
export async function recalcularCronograma(cronogramaId, userId) {
  ...
  // Sincroniza o PERÍODO do cronograma com as tarefas — nos DOIS sentidos.
  // Antes só esticava o dataFim pra frente: o dataInicio nunca voltava atrás, e
  // uma tarefa que passou a começar antes ficava fora do período. Fica também
  // ANTES do early return, porque uma edição de data que não cascateia sucessora
  // nenhuma não gera updates mas ainda pode jogar a tarefa pra fora do período
  // (e aí ela sai espremida numa lasquinha na borda do gráfico/PDF).
  ...
  const periodo = {};
  if (minIniGlobal && (!cronograma.dataInicio || minIniGlobal < new Date(cronograma.dataInicio))) periodo.dataInicio = minIniGlobal;
  if (maxFimGlobal && (!cronograma.dataFim || maxFimGlobal > new Date(cronograma.dataFim))) periodo.dataFim = maxFimGlobal;
