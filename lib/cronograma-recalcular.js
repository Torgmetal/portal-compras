...
export async function recalcularCronograma(cronogramaId, userId) {
    ...
    include: {
      tarefas: {
        ...
        select: {
          ...
          dataLiberacao: true, motivoBloqueio: true,
        },
      },
    },
  });
  ...
  for (const tarefa of sorted) {
    ...
    //    Se tarefa tem dataLiberacao, ela NAO pode comecar antes dessa data
    //    O novo inicio = MAX(dia apos antecessora, dataLiberacao)
    let dataMinInicio = null;
    if (maxFimAntecessora) {
      dataMinInicio = nextWorkday(maxFimAntecessora);
    }
    if (temLiberacao) {
