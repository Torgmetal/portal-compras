 ...
 */
export async function situacaoSuprimentos(prisma, opId) {
  const itens = await prisma.rMItem.findMany({
    where: { rm: { opId }, canceladoEm: null },
    select: {
  ...
  for (const g of Object.values(porFamilia)) g.rms = g.rms.size;
...
export async function aplicarAvancoSuprimentos(prisma, opId) {
  ...
  const { porFamilia, fd } = await situacaoSuprimentos(prisma, opId);
  const linhas = [];
  let atualizadas = 0;
  ...
  for (const t of tarefas) {
    ...
    if (ehLinhaPedidoCliente(t.nome)) {
      ...
      const f = familiaDaLinha(t.nome) || "ACO";
      const g = porFamilia[f];
      ...
      dados = {
        ...
        obs: `${g.rms} RM(s) · ${g.itens} itens · ${Math.round(g.escopo)} ${g.unidade}` +
          (c.cresceu > 0 ? ` · escopo cresceu ${c.cresceu} ${g.unidade} depois do congelamento` : ""),
      };
    } else continue;
