...
// "Compra de Matéria Prima", "Compra de Tintas", "Compra de Parafusos" — o Planejamento escreve
// COMPRA onde o modelo diz COTAÇÃO. São a mesma linha: começa na RM e fecha quando vira pedido.
// Sem isso, 13 linhas de Suprimentos ficavam sem avanço nenhum. (Vitor 19/08/2026.)
export const ehLinhaCotacao = (nome) => /cota[çc]|compra/i.test(String(nome || ""));
...
export async function situacaoSuprimentos(prisma, opId, desde = null) {
  ...
  for (const it of itens) {
    ...
    // ⚠ ATENDIDO PELO ESTOQUE JÁ ESTÁ NA FÁBRICA: não passa pelo CMR, então nunca teria recebimento —
    // e a linha ficava presa abaixo de 100 % para sempre (OP-094, 14/09/2026: 94 kg de vergalhão do
    // estoque seguravam a linha em 84 %). Estoque conta como recebido por inteiro.
    const doEstoque = it.status === "ATENDIDO_ESTOQUE" || !!it.atendidoEstoqueEm;
    const pedido = Number(it.peso) || 0;
    const rec = doEstoque ? Math.max(pedido, val, 1) : (it.recebimentos || []).reduce((a, r) => a + (Number(r.qtdRecebida) || 0), 0);
    if (f === "ACO" && pedido > 0) {
...
export async function aplicarAvancoSuprimentos(prisma, opId) {
  ...
  const tarefas = await prisma.cronogramaTarefa.findMany({
    ...
    select: { id: true, nome: true, qtdePlanejada: true, percentualRealizado: true, dataInicioReal: true, dataFimReal: true, avancoManual: true },
  });
  ...
  for (const t of tarefas) {
    // ⚠ o número digitado à mão prevalece (Vitor, 14/09/2026: "deixa como o Guilherme fez")
    if (t.avancoManual) { linhas.push({ nome: t.nome, pct: t.percentualRealizado, manual: true }); continue; }
