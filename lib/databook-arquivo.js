...
import { procurarArquivoPorNome } from "./sharepoint-arvore";
import { prisma } from "./prisma";
import { resolveServidorDriveId } from "./projetos-databook";
import { isBlobUrlSegura } from "./blob-url";
import { pdfDoRelatorio, fonteDeInspecao, fonteDeCopiaArquivada } from "./relatorio-pdf-fonte";
import { fonteDePit, pdfDoPit } from "./pit-pdf-fonte";
...
export async function baixarDocumento(doc, servidorDriveId = null) {
  ...
  // a cópia do relatório na pasta da obra foi arquivada ANTES das assinaturas: sai o do portal
  const copia = await fonteDeCopiaArquivada(doc);
  if (copia) return (await pdfDoRelatorio(copia.relatorioId, { exigirOp: copia.exigirOp })).bytes;
  const pit = fonteDePit(doc);
  if (pit) return (await pdfDoPit(prisma, pit)).bytes;
  ...
  if (isBlobUrlSegura(url)) {
    ...
    // ⚠⚠ O BLOB NÃO TEM LIXEIRA. Arquivo que sumiu de lá ainda pode estar na cópia do backup
    // (lib/backup-arquivos.js, 24/09/2026), e o anexo do livro não precisa sumir junto: com
    // `sharepointItemId`, segue para a busca no SharePoint logo abaixo. Sem cópia, o erro de antes.
    if (!doc.sharepointItemId) throw new Error(`HTTP ${r.status}`);
