...
import { pdfDoRelatorio, refDeInspecao } from "./relatorio-pdf-fonte";
...
export async function baixarDocumento(doc, servidorDriveId = null) {
  ...
  const insp = refDeInspecao(url);
  if (insp) return (await pdfDoRelatorio(insp.relatorioId, { revisao: insp.revisao })).bytes;
