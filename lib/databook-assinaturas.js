...
// apelido histórico: o formato do Data Book é o mesmo do portal inteiro
export const fmtOPdb = fmtOP;
...
/**
 * E-mail avisando que alguém da cadeia PEDIU REVISÃO — o fluxo voltou e a revisão subiu.
 *
 * ⚠⚠ VAI PARA QUEM PERDEU A ASSINATURA, NÃO SÓ PARA QUEM CONSERTA.
 * Abrir revisão zera TODAS as assinaturas. Quem já tinha assinado precisa saber que a assinatura
 * dele deixou de valer — senão descobre depois, quando o link pedir a assinatura de novo, e fica
 * parecendo erro do portal.
 *
 * ⚠ O MOTIVO VAI INTEIRO NO CORPO. É o único lugar onde quem vai refazer o dossiê lê o que estava
 * errado; resumir aqui obrigaria a pessoa a entrar no portal para descobrir o que fazer.
 */
export async function enviarEmailRevisaoPedida({ email, nomeDest, op, obra, quemPediu, papelPediu, motivo, rotulo, jaAssinara, link }) {
  if (!email) return false;
  const label = PAPEL_LABEL[papelPediu] || "um dos assinantes";
  const corpo = `
    <p style="font-size:15px;margin:0 0 12px">${nomeDest ? `Olá, ${nomeDest}.` : "Olá."}</p>
    <p style="font-size:14px;line-height:1.6;margin:0 0 16px">
      <strong>${quemPediu}</strong> (${label}) pediu revisão do Data Book ${obra ? `da obra <strong>${obra}</strong> ` : ""}(<strong>${op}</strong>).
      O fluxo de assinaturas voltou ao início e o dossiê passou para a revisão <strong>${rotulo}</strong>.
    </p>
    <div style="background:#fff6e6;border:1px solid #f6c67a;border-radius:8px;padding:12px 16px;margin:0 0 16px">
      <p style="margin:0 0 4px;font-size:12px;font-weight:bold;color:#92400e;text-transform:uppercase;letter-spacing:.4px">Motivo</p>
      <p style="margin:0;font-size:14px;line-height:1.6;color:#92400e;white-space:pre-wrap">${motivo}</p>
    </div>
    ${jaAssinara ? `<p style="font-size:14px;line-height:1.6;margin:0 0 16px">Sua assinatura anterior deixou de valer: ela era da revisão passada. Quando o dossiê for reemitido, você receberá o link para assinar de novo.</p>` : ""}
    <p style="font-size:14px;line-height:1.6;margin:0 0 6px">A Qualidade vai corrigir o que foi apontado e reiniciar o fluxo.</p>
    ${link ? botao(link, "Abrir o Data Book") + fallback(link) : ""}`;
  await sendEmail({
    to: email,
    subject: `Data Book ${op}${obra ? " — " + obra : ""} — revisão pedida (${rotulo})`,
    html: molde("Data Book da Qualidade — revisão pedida", corpo),
    text: `Revisão pedida no Data Book ${op}${obra ? " — " + obra : ""} por ${quemPediu} (${label}). Motivo: ${motivo}`,
  });
  return true;
}
