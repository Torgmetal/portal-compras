...
// bitola do perfil laminado: SEÇÃO (W310) + PESO kg/m (o "X 21"). O peso diferencia
// W310X21 de W310X23,8 — mesma seção, perfis diferentes.
const bitolaCroqui = (perfil) => { const ns = numsOf(perfil); const sh = shapeOf(perfil); return { secao: ns.length ? sh + Math.round(ns[0]) : sh, peso: ns.length > 1 ? ns[1] : null }; };
// espec do certificado a partir do nome ("CHAPA … ESPESSURA 9,50MM" / "PERFIL W … DN. W310 X 21,0KG/M")
function certEspec(nome) {
  const N = String(nome || "").toUpperCase().normalize("NFD").replace(/[̀-ͯ]/g, "");
  const esp = N.match(/ESPESSURA\s*(\d+(?:[.,]\d+)?)/);
  const w = N.match(/\b(W|HP|H|I)\s*(\d{2,4})\s*X\s*(\d+(?:[.,]\d+)?)/);
  return { isChapa: /CHAPA/.test(N), thick: esp ? parseFloat(esp[1].replace(",", ".")) : null, prof: w ? { secao: w[1] + w[2], peso: parseFloat(w[3].replace(",", ".")) } : null };
}
...
export async function montarSecaoLpc(opNumeroBook) {
  ...
  const certsDaPosicao = (material, perfil) => {
    ...
    for (const c of certsMat) {
      ...
      if (rolado) {
        if (!c.espec.prof || c.espec.prof.secao !== bit.secao) continue;               // mesma seção (W310)
        if (bit.peso != null && c.espec.prof.peso != null && Math.abs(bit.peso - c.espec.prof.peso) > 0.5) continue; // mesmo peso (X21 ≠ X23,8)
      } else {
