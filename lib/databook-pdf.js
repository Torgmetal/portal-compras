...
import { desenharCapaTorg, desenharDivisoriaTorg } from "./databook-template-pdf";
...
import { resolverPitDataBook } from "./databook-pit";
...
export async function gerarDataBookPDF(dataBookId, opts = {}) {
  ...
  const opCapa = await prisma.oP
    .findFirst({
      ...
      select: { refCliente: true, pitPadrao: true, pitRevisao: true, referencias: { orderBy: { ordem: "asc" }, select: { rotulo: true, codigo: true } } },
    })
    .catch(() => null);
  ...
  const sec10 = book.secoes.find((s) => s.numero === "10");
  const pitLive = resolverPitDataBook(sec10?.conteudoJson, opCapa?.pitPadrao, opCapa?.pitRevisao);

  const pdf = await PDFDocument.create();
  const font = await pdf.embedFont(StandardFonts.Helvetica);
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
  // ⚠ nome de anexo com acento decomposto (NFD) derrubava a geração inteira — ver lib/pdf-winansi
  ...
  const templateNovo = usaTemplateNovo(book);
  ...
  const drawTabela = (colunas, linhas, opts = {}) => {
    ...
    const cab = () => {
      espaco(rowH + 6);
      page.drawRectangle({ x: M, y: y - rowH + 3, width: W, height: rowH, color: templateNovo ? BLUE : NAVY2 });
      let x = M + 5;
  ...
  const dividerPagina = (id, titulo, norma) => {
    novaPagina();
    if (templateNovo) {
      desenharDivisoriaTorg(page, { font, bold, logo: logoDarkImg, op: fmtOP(book.opNumero), identificacao: id, titulo, norma });
      return;
    }
    if (logoDarkImg) {
  ...
  if (templateNovo) {
    desenharCapaTorg(pdf, { font, bold, logo: logoDarkImg, book, codigo, revisao: rev, dataEmissao, emitido,
      referencias: opCapa?.referencias?.length ? opCapa.referencias : opCapa?.refCliente ? [{codigo:opCapa.refCliente}] : [] });
  } else {
  ...
  const temConteudo = (s) =>
    ...
    s.numero === "20" ||
    (s.numero === "10" && Array.isArray(pitLive?.itens) && pitLive.itens.length > 0) ||
  ...
  for (const g of grupos) {
   for (const it of g.itens) {
    ...
    if (s.numero === "20") {
      ...
      const itens = Array.isArray(pitLive?.itens) ? pitLive.itens : [];
      if (itens.length) {
  ...
  page = sumarioPage;
  if (templateNovo) { tituloSecao(page,bold,font,"01","SUMÁRIO",fmtOP(book.opNumero)); } else {
  ...
  novaPagina();
  if (templateNovo) { tituloSecao(page,bold,font,"20","RESPONSABILIDADE TÉCNICA E ASSINATURAS",null); } else {
  page.drawRectangle({ x: 0, y: A4[1] - 64, width: A4[0], height: 64, color: NAVY });
  ...
  function tituloSecao(pg, fb, fr, numero, titulo, norma) {
    if (templateNovo) {
      if (logoDarkImg) { const w=68,h=logoDarkImg.height/logoDarkImg.width*w;pg.drawImage(logoDarkImg,{x:M,y:A4[1]-24-h,width:w,height:h}); }
      pg.drawText(fit(titulo,fb,10,W-90),{x:M+90,y:A4[1]-34,size:10,font:fb,color:BLUE});
      if(norma)pg.drawText(fit(norma,fr,8,W-90),{x:M+90,y:A4[1]-50,size:8,font:fr,color:GRAY});
      pg.drawLine({start:{x:M,y:A4[1]-64},end:{x:M+W,y:A4[1]-64},thickness:.6,color:LIGHTBLUE});
      return;
    }
