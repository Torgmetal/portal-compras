...
export async function gerarDataBookPDF(dataBookId) {
  ...
  // ─── SEÇÕES 02..20 — fluem na página (várias por folha) ─────────────────
  let mergedPages = 0;
  let novaAntes = true; // 1ª seção começa em página nova (após a lista mestra)
  for (const s of book.secoes) {
    if (s.numero === "01") continue;
    if (s.estado === "NA") continue; // seção não compõe este data book

    if (novaAntes) { novaPagina(); novaAntes = false; }
    secaoHeader(s.numero, s.titulo, s.norma);
    ...
    if (pdf.getPages().length > antesPag) novaAntes = true; // anexou certs → próxima em página nova
    else y -= 14; // respiro entre seções no fluxo
  }
