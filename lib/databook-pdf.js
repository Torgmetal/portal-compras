...
export async function gerarDataBookPDF(dataBookId) {
  ...
  const temConteudo = (s) =>
    ...
    (s.numero === "02" && Array.isArray(s.conteudoJson?.conjuntos) && s.conteudoJson.conjuntos.length > 0);
  ...
  for (const g of grupos) {
   for (const it of g.itens) {
    ...
    if (s.numero === "20") {
      ...
      const conjuntos = Array.isArray(s.conteudoJson?.conjuntos) ? s.conteudoJson.conjuntos : [];
      if (conjuntos.length) {
        const cj = s.conteudoJson;
        page.drawText(`Rastreabilidade dos desenhos (LPC) — ${conjuntos.length} conjuntos · ${cj.totalPosicoes || 0} posições${cj.semCertificado ? ` · ${cj.semCertificado} sem certificado` : ""}`, { x: M, y, size: 9.5, font: bold, color: NAVY2 }); y -= 15;
        drawLpcSecao(conjuntos);
        y -= 8;
      } else {
        page.drawText('LPC não gerada — clique em "Gerar da LPC" na seção 02 do portal.', { x: M, y, size: 8.5, font, color: GRAY }); y -= 12;
      }
