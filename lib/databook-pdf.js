...
export async function gerarDataBookPDF(dataBookId) {
  ...
  // ── CERTIFICADO ANEXADO ↔ FICHA DO CMR ─────────────────────────────────────────────────────
  //
  // Vitor: "melhore o layout das páginas, principalmente onde você lista os certificados". O
  // layout era só metade do problema — o resto era a tabela não ter O QUE mostrar.
  //
  // Quem monta o data book anexa o PDF do certificado, e o arquivo se chama pelo índice R:
  // "R 260620.pdf". O documento nasce com `nome: "R 260620"` e mais nada — sem material, sem
  // corrida, sem número de certificado. Resultado na OP-070: a §04 (matéria-prima) saía como sete
  // linhas de "R 260620 | — | — | Sem validade", que não prova rastreabilidade nenhuma.
  //
  // Só que esses dados EXISTEM: o CMR importado do Almoxarifado tem, por R, o material, a corrida,
  // o nº do certificado, a norma e o fornecedor. Aqui a gente casa os dois pelo R e a tabela passa
  // a mostrar a ficha completa — que é o que o auditor lê.
  //
  // ⚠ Só casa quando o nome é MESMO um índice R (`R 260620`) ou o doc já tem `importRef`. Não dá
  // pra sair extraindo número de nome qualquer: "T70_-_ART_assinado" viraria o R 70 de outra obra.
  const RX_NOME_R = /^r[\s._-]*\d{4,}/i;
  const rDoDoc = (d) => {
  ...
  const rsBuscar = [...new Set(docs.map(rDoDoc).filter(Boolean))];
  const fichaPorR = new Map();
  if (rsBuscar.length) {
    const fichas = await prisma.documentoQualidade.findMany({
      where: { origem: "importacao_planilha", importRef: { in: rsBuscar }, ativo: true },
      select: { importRef: true, nome: true, numeroDocumento: true, numeroCorrida: true, norma: true, fornecedor: true, dataEmissao: true, dataValidade: true, opNumero: true },
    });
    for (const f of fichas) {
      // a ficha da PRÓPRIA OP ganha da de outra obra: o mesmo R pode ter sido usado em duas
      const atual = fichaPorR.get(f.importRef);
      if (!atual || (f.opNumero === book.opNumero && atual.opNumero !== book.opNumero)) fichaPorR.set(f.importRef, f);
    }
  }
  /** Documento com os campos que faltavam preenchidos pela ficha do CMR daquele R. */
  const comFicha = (d) => {
    const r = rDoDoc(d);
    const f = r ? fichaPorR.get(r) : null;
  ...
  for (const g of grupos) {
   for (const it of g.itens) {
    ...
    novaPagina();
    const docsSecao = s.documentos.map((ld) => docById.get(ld.documentoId)).filter(Boolean).map(comFicha);
