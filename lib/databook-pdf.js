...
export async function gerarDataBookPDF(dataBookId) {
  ...
  const drawLpcSecao = (conjuntos) => {
    ...
    const cols = [
      { t: "Posição", w: 78, bold: true },
      { t: "Material", w: 92 },
      { t: "Qtd", w: 34, align: "right" },
      { t: "Rastreab. (corrida)", w: 140 },
      { t: "Nº Certificado", w: 167 },
    ]; // soma = 511 = W
    const cab = () => {
      espaco(cabH + 16);
      page.drawRectangle({ x: M, y: y - cabH + 3, width: W, height: cabH, color: NAVY2 });
    ...
    for (const cj of conjuntos) {
      ...
      (cj.posicoes || []).forEach((p, ri) => {
        ...
        const rowH = Math.max(1, cs.length) * lineH + padV;
        if (y - rowH < M + 40) { novaPagina(); cab(); }
        ...
        page.drawText(fit(p.material || "—", font, fs, cols[1].w - 6), { x, y: y - 8, size: fs, font, color: DARK }); x += cols[1].w;
        const q = String(p.qtd ?? "—");
        page.drawText(q, { x: x + cols[2].w - 6 - font.widthOfTextAtSize(q, fs), y: y - 8, size: fs, font, color: DARK }); x += cols[2].w;
        const xCorr = x, xCert = x + cols[3].w;
        if (!cs.length) {
          page.drawText("—", { x: xCorr, y: y - 8, size: fs, font, color: ORANGE });
          page.drawText("sem certificado", { x: xCert, y: y - 8, size: fs, font, color: ORANGE });
        } else {
          cs.forEach((c, li) => {
            page.drawText(fit(c.corrida || "—", font, fs, cols[3].w - 6), { x: xCorr, y: y - 8 - li * lineH, size: fs, font, color: NAVY2 });
            page.drawText(fit(c.certificado || "—", font, fs, cols[4].w - 6), { x: xCert, y: y - 8 - li * lineH, size: fs, font, color: DARK });
          });
        }
        y -= rowH;
      });
  ...
  const temConteudo = (s) =>
    ...
    (s.numero === "02" && ((Array.isArray(s.conteudoJson?.conjuntos) && s.conteudoJson.conjuntos.length > 0) || lpcLive.conjuntos.length > 0));
  ...
  for (const g of grupos) {
   for (const it of g.itens) {
    ...
    if (s.numero === "20") {
      ...
      // Usa a versão salva (conteudoJson) se existir; senão, a LPC montada na hora.
      const saved = (s.conteudoJson?.tipo === "lpc" && Array.isArray(s.conteudoJson.conjuntos) && s.conteudoJson.conjuntos.length) ? s.conteudoJson : null;
      const lpc = saved || lpcLive;
      const conjuntos = lpc.conjuntos || [];
      if (conjuntos.length) {
        page.drawText(`Rastreabilidade dos desenhos (LPC) — ${conjuntos.length} conjuntos · ${lpc.totalPosicoes || 0} posições${lpc.semCertificado ? ` · ${lpc.semCertificado} sem certificado` : ""}`, { x: M, y, size: 9.5, font: bold, color: NAVY2 }); y -= 15;
