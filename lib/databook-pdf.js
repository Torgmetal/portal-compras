   ...
   for (const it of g.itens) {
        ...
        drawTabela(
          ...
          docsSecao.map((d) => {
            ...
            if (mapaDocs) {
              ...
              linha.push(onde ? `${String(onde.v).padStart(2, "0")} · ${onde.p}` : "—");
            }
            return linha;
          }),
        );
    ...
    if (!comAnexos) {
      if (anexaveis.length) {
        ...
        page.drawText(`Anexos desta seção: ${anexaveis.length} documento(s)${pags} — ${onde}.`,
          { x: M + 6, y: y - 9, size: 8.5, font: bold, color: NAVY });
        y -= 26;
      }
    } else {
