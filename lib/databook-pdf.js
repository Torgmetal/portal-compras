...
export async function gerarDataBookPDF(dataBookId) {
  ...
  const cwCapa = (A4[0] - 2 * M) / 3;
  [["CÓDIGO", codigo], ["REVISÃO", "00"], ["EMISSÃO", fmtData(new Date())]].forEach(([l, v], i) => {
    const cx = M + i * cwCapa;
    tracked(l, cx, fH - 30, 8, bold, GRAY, 1.5);
    page.drawText(v, { x: cx, y: fH - 52, size: 13, font: bold, color: NAVY });
  });
  tracked("STATUS", M, 26, 8, bold, GRAY, 1.5);
  page.drawText(emitido ? "EMITIDO" : "RASCUNHO", { x: M + trackedWidth("STATUS", 8, bold, 1.5) + 12, y: 23, size: 11, font: bold, color: emitido ? DARK : rgb(0.7, 0.45, 0) });
  if (book.tipo) { const tl = `Padrão: ${TIPO_DATABOOK_LABEL[book.tipo] || book.tipo}`; page.drawText(tl, { x: A4[0] - M - font.widthOfTextAtSize(tl, 9), y: 24, size: 9, font, color: GRAY }); }
  ...
  const incluidas = book.secoes.filter((s) => s.estado !== "NA" && s.numero !== "01");
  ...
  for (const g of grupos) {
    ...
    for (const it of g.itens) {
      ...
      page.drawText(fit(it.secao.titulo, font, 9.5, W - 170), { x: M + 58, y, size: 9.5, font, color: DARK });
  ...
  for (const g of grupos) {
   for (const it of g.itens) {
    ...
    if (s.numero === "20") {
      // Declaração de conformidade — fechamento do dossiê
      const decl = `A Torg Metal declara que o fornecimento da estrutura metálica referente à ${fmtOP(book.opNumero)}${book.obra ? " — " + book.obra : ""}${book.cliente ? " (cliente: " + book.cliente + ")" : ""} foi fabricado em conformidade com os projetos aprovados, as normas técnicas aplicáveis (ABNT NBR 8800 e AWS D1.1) e o Sistema de Gestão da Qualidade da empresa, certificado conforme ABNT NBR ISO 9001. A rastreabilidade dos materiais é comprovada pelos certificados de usina (com nº de corrida); a qualificação dos procedimentos de soldagem, dos soldadores e dos inspetores, bem como os ensaios e inspeções realizados, encontram-se evidenciados nas seções deste Data Book. Declaramos que os requisitos contratuais e normativos foram atendidos, atestando a conformidade do produto entregue.`;
      paragrafo(decl, 10);
      y -= 4;
      paragrafo("Este documento integra o dossiê da qualidade e deve acompanhar a entrega da obra.", 8.5, GRAY);
    } else if (s.numero === "10") {
  ...
  // Selo Bureau Veritas (ISO 9001) centralizado no rodapé — só se o arquivo
  // public/bureau-veritas.png existir (graceful: sem o arquivo, não aparece).
  let bvImg = null;
  try { bvImg = await pdf.embedPng(fs.readFileSync(path.join(process.cwd(), "public", "bureau-veritas.png"))); } catch {}
  ...
  paginas.forEach((p, i) => {
    ...
    if (bvImg) {
      const bw = 22, bh = (bvImg.height / bvImg.width) * bw;
      p.drawImage(bvImg, { x: A4[0] / 2 - bw / 2, y: 5, width: bw, height: bh });
    }
  });
