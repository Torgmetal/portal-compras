...
import { ESTADO_DATABOOK, GRUPOS_DATABOOK, grupoDaSecao, tipoPerfil } from "./databook-secoes";
  ...
  const drawLpcSecao = (conjuntos) => {
    ...
    for (const cj of conjuntos) {
      ...
      (cj.posicoes || []).forEach((p, ri) => {
        ...
        const perfilTxt = [tipoPerfil(p.perfil), p.perfil].filter(Boolean).join(" · ");
        const rowH = Math.max(cs.length, perfilTxt ? 2 : 1) * lineH + padV;
        if (y - rowH < M + 40) { novaPagina(); cab(); }
        ...
        // Material (grau) + tipo · perfil (2ª linha, cinza)
        page.drawText(fit(p.material || "—", font, fs, cols[1].w - 6), { x, y: y - 8, size: fs, font, color: DARK });
        if (perfilTxt) page.drawText(fit(perfilTxt, font, 6.8, cols[1].w - 6), { x, y: y - 8 - lineH, size: 6.8, font, color: GRAY });
        x += cols[1].w;
