...
export async function gerarDataBookPDF(dataBookId) {
  ...
  const drawLpcSecao = (conjuntos) => {
    ...
    for (const cj of conjuntos) {
      ...
      page.drawText(fit(`Conjunto ${cj.marca}${cj.descricao ? " — " + cj.descricao : ""}   ·   ${cj.qte}x`, bold, fs, W - 14 - wArame), { x: M + 4, y: y - grpH + 7, size: fs, font: bold, color: NAVY });
      if (txtArame) {
      ...
      (cj.posicoes || []).forEach((p, ri) => {
        ...
        if (!cs.length) {
          // ⚠ NÃO É TUDO "SEM CERTIFICADO". A peça sem R tem um motivo, e são motivos diferentes:
          // uma ainda não foi cortada (e de propósito não reivindica lote nenhum), outra saiu de
          // sobra de estoque, outra não tem material lançado no CMR. Escrever "sem certificado"
          // nas três dava a mesma cara de furo pra situação normal e pra pendência real.
          const MOTIVO = {
            AGUARDANDO_CORTE: { txt: "aguarda corte", cor: GRAY },
            ESTOQUE: { txt: "cortada antes da entrega (estoque)", cor: ORANGE },
            SEM_MATERIAL: { txt: "sem material no CMR", cor: ORANGE },
            SEM_DADO: { txt: "sem dado de rastreio", cor: GRAY },
          };
          const m = MOTIVO[p.situacao] || MOTIVO.SEM_DADO;
          page.drawText(m.txt, { x: xR, y: y - 8, size: fs, font, color: m.cor });
        } else {
          cs.forEach((c, li) => {
            ...
            // "troca" = o Almoxarifado registrou que separou outro fardo. Isso é FATO declarado, e
            // vale mais que o FIFO — quem lê o data book precisa saber que veio de lá.
            if (li === 0 && p.criterio === "troca") page.drawText("troca registrada", { x: xCorr, y: yy - lineH, size: 6.5, font, color: GRAY });
          });
        }
        y -= rowH;
      });
  ...
  for (const g of grupos) {
   for (const it of g.itens) {
    ...
    if (s.numero === "20") {
      ...
      if (conjuntos.length) {
        const sit = lpcLive.situacoes || {};
        page.drawText(`Rastreabilidade dos desenhos (LPC) — ${conjuntos.length} conjuntos · ${lpcLive.totalPosicoes || 0} posições`, { x: M, y, size: 9.5, font: bold, color: NAVY2 }); y -= 12;
        // ⚠ o quadro por situação vai em LINHA PRÓPRIA: junto do título estourava a largura e o
        // texto saía cortado no meio ("401 de esto").
        const partes = [
          sit.R_DEFINIDO ? `${sit.R_DEFINIDO} com R definido` : null,
          sit.AGUARDANDO_CORTE ? `${sit.AGUARDANDO_CORTE} aguardando corte` : null,
          sit.ESTOQUE ? `${sit.ESTOQUE} cortadas antes da entrega` : null,
          sit.SEM_MATERIAL ? `${sit.SEM_MATERIAL} sem material no CMR` : null,
        ].filter(Boolean);
        if (partes.length) { for (const ln of quebrar(partes.join("  ·  "), font, 8, W)) { page.drawText(ln, { x: M, y, size: 8, font, color: GRAY }); y -= 10; } }
        y -= 3;
