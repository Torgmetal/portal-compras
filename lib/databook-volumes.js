...
export async function processarGeracao(geracaoId) {
  ...
  const mapa = job.mapa && typeof job.mapa === "object"
    ...
    : { secoes: {}, docs: {} };
  const primeiroItem = roteiro[i];

  capaDoVolume(pdf, { font, bold, logo, book, volume, item: primeiroItem });
  paginas++;

  while (i < roteiro.length) {
    const it = roteiro[i];

    // ⚠ o corte do volume respeita a SEÇÃO só quando dá: uma seção com 1.336 desenhos
    // não cabe em volume nenhum, então ela atravessa volumes — e é o mapa que conta
    // isso para o leitor ("§02 — Volumes 2 a 5").
    if (secaoAberta !== it.secaoId) {
      if (paginas > 1 && estourou(bytesLidos, paginas, t0)) break;
      folhaDeRosto(pdf, { font, bold, logo, item: it, volume });
      paginas++;
      secaoAberta = it.secaoId;
    }

    const paginasAntes = paginas;
    try {
      ...
      const copiadas = await pdf.copyPages(ext, ext.getPageIndices());
      ...
      avisoNaFolha(pdf, { font, bold, item: it });
      paginas++;
    }

    const m = mapa.secoes[it.secaoId] || { volumeIni: volume, volumeFim: volume, paginas: 0, docs: 0 };
    m.volumeFim = volume;
    m.paginas = (m.paginas || 0) + (paginas - paginasAntes);
    m.docs = (m.docs || 0) + 1;
    mapa.secoes[it.secaoId] = m;
    // v = volume, p = página dentro do volume (1-based)
    mapa.docs[it.doc.id] = { v: volume, p: paginasAntes + 1 };

    i++;
    if (estourou(bytesLidos, paginas, t0)) break;
  }
...
function capaDoVolume(pdf, { font, bold, logo, book, volume, item }) {
  ...
  y -= 40;
  p.drawText("Este volume contém apenas anexos. O sumário e as listas mestras estão no Volume 01.",
