...
import { usaTemplateNovo } from "./databook-template";
import { desenharDivisoriaTorg } from "./databook-template-pdf";
...
import { blindarPdf } from "./pdf-winansi";
import fs from "fs";
...
export async function montarRoteiro(dataBookId) {
  const book = await prisma.dataBookQualidade.findUnique({
    where: { id: dataBookId },
    select: {
      id: true, templateVisual: true, opNumero: true, cliente: true, obra: true, revisao: true, emitidoEm: true, status: true,
  ...
  const docs = ids.length
    ? await prisma.documentoQualidade.findMany({
        where: { id: { in: ids } },
        select: { id: true, nome: true, arquivoNome: true, arquivoUrl: true, sharepointItemId: true, sharepointUrl: true, origem: true, numeroDocumento: true, opNumero: true },
      })
    : [];
  ...
  for (const s of book.secoes) {
    ...
    for (const v of s.documentos) {
      ...
      if (!ehAnexavelNoLivro(d)) continue; // CAD/planilha/página não é anexo do livro — ver databook-anexo
      vistos.add(chave);
...
export async function processarGeracao(geracaoId) {
  ...
  // ⚠ nome de anexo com acento decomposto (NFD) derrubava o volume inteiro — ver lib/pdf-winansi
  blindarPdf(pdf, [font, bold]);
  const logo = await embutirLogo(pdf, usaTemplateNovo(book));
  ...
  while (i < roteiro.length) {
    ...
    try {
      ...
      const ext = await abrirAnexoComoPdf(buf, it.doc);
...
// ─── PÁGINAS DE APOIO ─────────────────────────────────────────────────────────
async function embutirLogo(pdf, novo = false) {
  for (const n of (novo ? ["torg-logo.png"] : ["torg-logo-white.png", "torg-logo.png"])) {
...
function capaDoVolume(pdf, { font, bold, logo, book, volume, item, totalPaginas, docs }) {
  ...
  if (usaTemplateNovo(book)) {
    desenharDivisoriaTorg(p, { font, bold, logo, op: fmtOP(book.opNumero), identificacao: `VOLUME ${String(volume).padStart(2,"0")}`, titulo: "Anexos do data book", norma: `Revisão ${rotuloRevisao(book.revisao)}` });
    let y = 470;
    for (const [rotulo, valor] of [["CLIENTE", book.cliente], ["OBRA", book.obra], ["INÍCIO", item ? `Seção ${item.secaoNumero} - ${item.secaoTitulo}` : "Anexos"]]) {
      p.drawText(rotulo, {x:M,y,size:8,font:bold,color:GRAY}); y-=20;
      p.drawText(fit(valor || "Não informado",font,11,A4[0]-2*M),{x:M,y,size:11,font,color:DARK}); y-=45;
    }
    p.drawText(`${docs} documentos · ${totalPaginas} páginas`,{x:M,y:200,size:10,font,color:GRAY});
    p.drawText("Consulte o índice na página seguinte.",{x:M,y:180,size:10,font,color:GRAY});
    return;
  }
  ...
  if (item) {
    ...
    p.drawText(fit(`Seção ${item.secaoNumero} — ${item.secaoTitulo}`, bold, 11, A4[0] - 2 * M), { x: M, y, size: 11, font: bold, color: NAVY });
  }
  y -= 24;
  if (docs) {
...
// O título do volume tem que DISTINGUIR um volume do outro.
//
// Vitor (22/08/2026), olhando a lista: "não temos projetos duplicados não?". Não temos
// — 1.336 desenhos, 1.336 arquivos distintos. Mas dezessete linhas dizendo "Anexos ·
// Seção 02 Desenhos as-built" parecem o mesmo volume repetido, e não dão a menor pista
// de onde procurar. A FAIXA resolve as duas coisas: "T67A1 a T67A85" diz o que tem
// dentro e prova que cada volume é um pedaço diferente.
const curto = (nome, max = 26) => {
  const n = String(nome || "").trim();
  return n.length <= max ? n : n.slice(0, max - 1) + "…";
};

export function tituloVolume(primeiro, ultimo) {
  if (!primeiro) return "Anexos";
  if (ultimo && ultimo.secaoId !== primeiro.secaoId) {
    return `Anexos · Seções ${primeiro.secaoNumero} a ${ultimo.secaoNumero}`;
  }
  const a = curto(primeiro.doc?.nome);
  const b = curto(ultimo?.doc?.nome);
  const faixa = b && b !== a ? `${a} a ${b}` : a;
  return `Seção ${primeiro.secaoNumero} · ${faixa}`;
}
