...
import { consumivelVigenteEm, entradasConsumivelSolda } from "./consumivel-solda";
...
export async function emitirLoteDesenhos({ opNumero, marcas, setor = null, user = null, acao = "EMITIR" }) {
  ...
  for (const [marca, arq] of escolhido) {
    try {
      ...
      const consumivel = await consumivelVigenteEm(quando, entradasCons).catch(() => null);
  ...
  for (const [formato, grupo] of porFormato) {
    ...
    const pasta = escolhido.get(grupo.marcas[0])?.pasta
