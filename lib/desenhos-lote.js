...
import { amarracoesDaOp, amarracaoDoPerfil } from "./r-amarrado";
...
export async function emitirLoteDesenhos({ opNumero, marcas, setor = null, user = null, acao = "EMITIR" }) {
  ...
  for (const [marca, arq] of emOrdem) {
    try {
      ...
      if (am && semR) {
        itens = [...itens, { marca, perfil: perfilM, situacao: "R_INDICADO",
  ...
  if (acao === "IMPRIMIR") {
    for (const arqLote of arquivos) {
      for (const marca of arqLote.marcas) {
        ...
        if (jaTem) {
          await prisma.grdLiberacao.update({ where: { id: jaTem.id }, data: { impressoes: { increment: 1 }, ultimaImpressaoEm: quando, impressoItemId: arqLote.itemId, impressoUrl: arqLote.url, ...(rastreio.length ? { rastreio } : {}) } });
        } else {
