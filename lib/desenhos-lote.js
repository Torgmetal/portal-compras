  ...
  async function andar(caminho, pastaMae) {
    ...
    for (const it of value) {
      if (it.folder) await andar(`${caminho}/${it.name}`, it.name);
        ...
        out.push({ id: it.id, nome: it.name, formato });
      }
    }
  }
  await andar(raiz, "");
...
export async function emitirLoteDesenhos({ opNumero, marcas, setor = null, user = null, acao = "EMITIR" }) {
  ...
  for (const [marca, arq] of escolhido) {
    try {
      ...
      const carimbado = await carimbarDesenho(new Uint8Array(await res.arrayBuffer()), {
        ...
        usuario: user?.name || "—", quando, itens,
      });
  ...
  // sobe um arquivo por formato na pasta da OP
  const base = await acharPastaOp(num);
  const pasta = `${base || `/Ordem de Servico/01. OP/OP-${num}`}/2. Engenharia/2.5 Projetos/2.5.2 Fabricação/Impressos rastreados`;
  const carimboNome = dataHoraBR(quando).replace(/\/\d{4}/, "").replace(/[/:]/g, "-");
  const arquivos = [];
  for (const [formato, grupo] of porFormato) {
    grupo.pdf.setTitle(`Lote ${formato} — OP-${op.numero} (${grupo.marcas.length} desenhos)`);
    const bytes = await grupo.pdf.save();
    const up = await uploadFileToFolder({
      folderPath: pasta,
      fileName: `LOTE ${formato} - OP-${op.numero} - ${carimboNome} (${grupo.marcas.length} pc).pdf`,
