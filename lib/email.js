...
export async function sendEmail({ to, cc, subject, html, text, replyTo, attachments, fromName }) {
  // ⚠ DEMO: nada sai. Devolve ok para a tela seguir (cotação criada, cobrança "enviada"), e o
  // destinatário vai só para o console de quem está mostrando. Ver lib/modo-demo.js.
  if (emModoDemo()) {
    registro.info("[email][DEMO] não enviado:", { to, cc, subject });
    return { ok: true, id: `demo-${Date.now()}`, demo: true };
  }
  if (!RESEND_KEY) {
    registro.aviso("[email] RESEND_API_KEY nao configurado — pulando envio");
  ...
  try {
    ...
    if (result.error) {
      registro.erro("[email] resend retornou erro:", result.error);
      return { ok: false, error: result.error.message || "Resend error" };
    }
    return { ok: true, id: result.data?.id };
  } catch (e) {
    registro.erro("[email] excecao no envio:", e?.message);
    // ⚠⚠ EXCEÇÃO NÃO É RECUSA (achado do Codex, 18/09/2026). Um timeout depois de o provedor ter
    // aceitado é indistinguível de uma recusa daqui — e quem trata os dois como "falhou" reenvia
    // "por garantia", que é como a mesma mensagem chega duas vezes. `indeterminado` deixa quem
    // chama escolher; quem ignora o campo continua vendo `ok:false`, como antes.
    return { ok: false, indeterminado: true, error: e?.message || "erro desconhecido" };
  }
}
...
export async function notificarEvento({ evento, subject, html, text }) {
  ...
  try {
    ...
    registro.erro("[email] falha buscando inscritos:", e?.message);
