...
function lerItensComerciais(XLSX, wb) {
  ...
  return { aba: aba.nome, familias: familias.filter((f) => f.total > 0).map(({ col: _col, ...f }) => f) };
}
...
function lerIndustrializacao(XLSX, wb) {
  ...
  for (let i = cab.i + 1; i < linhas.length; i++) {
    ...
    const reg = {
      ...
      subtotal: sub,
      // ⚠ O subtotal é o valor BRUTO do pedido (o que o Compras paga). ICMS e PIS/COFINS são
      // CRÉDITO recuperável, não custo — conferido: 127.391,20 × 12% = 15.286,94 de ICMS, e o
      // PIS/COFINS de 9,25% incide sobre o valor JÁ SEM o ICMS.
      icms: cIcms >= 0 ? num(l[cIcms]) : null,
      pisCofins: cPis >= 0 ? num(l[cPis]) : null,
    };
 ...
 */
// ── BDI: IMPOSTOS, CRÉDITOS E O SPLIT TORG × FATURAMENTO DIRETO ────────────────────────────────
//
// Vitor (19/08/2026): "a receita do contrato seria o valor a ser faturado e itens de contrato
// seria o valor que o Compras deveria comprar — isso deve ser a confusão que está fazendo"; e
// "também não estou vendo os valores de impostos destacados".
//
// Ele está certo e a planilha resolve as duas coisas. A aba BDI tem:
//   · a TABELA DE % DE IMPOSTOS por CFOP (5101 venda dentro do estado, 6101 fora, 5125
//     industrialização, 701/702/1405 serviço) — é daí que sai o imposto de cada linha de receita;
//   · os VALORES DE FATURAMENTO, já com imposto por linha, o crédito recuperável e o total;
//   · a DISTRIBUIÇÃO, que separa o que a Torg fatura do que vai por FATURAMENTO DIRETO ao cliente.
//
// ⚠ Ler por RÓTULO, nunca por linha fixa: o bloco da direita começa em linhas diferentes conforme
// o estudo tenha ou não montagem/equipamentos.
const RX_TRIB = {
  pisPct: /tributos federais \(pis\)/,
  cofinsPct: /tributos federais \(cofins\)/,
  csllPct: /tributos federais \(csll\)/,
  irrfPct: /tributos federais \(irpj\)/,
  icmsPct: /tributos estaduais \(icms\)/,
  issPct: /tributos municipais \(iss\)/,
};

function lerBdi(XLSX, wb) {
  const aba = acharAba(wb, "BDI");
  if (!aba) return null;
  const L = XLSX.utils.sheet_to_json(aba.ws, { header: 1, defval: null });

  // acha a linha cujo rótulo (em qualquer coluna até `ateCol`) casa com o regex
  const acharLinha = (rx, ateCol = 8) => L.find((r) => (r || []).slice(0, ateCol).some((c) => rx.test(norm(c))));
  // valor logo à direita do rótulo
  const aposRotulo = (linha, rx) => {
    if (!linha) return null;
    const i = linha.findIndex((c) => rx.test(norm(c)));
    return i < 0 ? null : num(linha[i + 1]);
  };

  // 1) ALÍQUOTAS POR CFOP
  const lCfop = acharLinha(/^cfop ?\/ ?c[oó]d/, 12);
  const cfops = lCfop ? lCfop.map(num).map((n) => (n && n > 100 ? n : null)) : [];
  const aliquotas = {};
  for (const [campo, rx] of Object.entries(RX_TRIB)) {
    const lin = acharLinha(rx, 12);
    if (!lin) continue;
    cfops.forEach((cfop, col) => {
      if (!cfop) return;
      (aliquotas[cfop] ||= { cfop });
      aliquotas[cfop][campo] = num(lin[col]) ?? 0;
    });
  }

  // 2) VALORES DE FATURAMENTO — uma linha por natureza, com imposto e CFOP
  const faturamento = [];
  const iCab = L.findIndex((r) => (r || []).some((c) => norm(c) === "valor faturamento"));
  if (iCab >= 0) {
    const cDesc = (L[iCab] || []).findIndex((c) => norm(c) === "descricao");
    const cVal = (L[iCab] || []).findIndex((c) => norm(c) === "valor faturamento");
    for (let i = iCab + 1; i < L.length; i++) {
      const r = L[i] || [];
      const desc = String(r[cDesc] ?? "").trim();
      if (!desc) { if (faturamento.length) break; else continue; }
      ...
      faturamento.push({
        descricao: desc,
        valor,
        impostos: num(r[cVal + 1]),
        cfop: num(r[cVal + 2]) || null,
        // "TORG" ou "-" — quem emite a nota. Fora "TORG" é faturamento direto do fornecedor.
        faturadoPor: String(r[cVal + 3] ?? "").trim() || null,
      });
    }
  }

  // 3) TOTAIS
  const lTotGeral = L.find((r, i) => i > iCab && /^total geral$/.test(norm((r || [])[(L[iCab] || []).findIndex((c) => norm(c) === "descricao")])));
  const lCred = acharLinha(/^cr[eé]ditos$/, 12);
  const lVenda = acharLinha(/^valor total da venda$/, 4);
  const lImp = acharLinha(/^impostos$/, 4);
  const lBdi = acharLinha(/^bdi$/, 4);
  const lMargem = acharLinha(/^margem de lucro/, 4);

  // crédito fica NA MESMA linha do rótulo, uma coluna à ESQUERDA (layout da planilha)
  const credito = lCred ? num(lCred[lCred.findIndex((c) => /^cr[eé]ditos$/.test(norm(c))) - 1]) : null;

  const cDescR = (L[iCab] || []).findIndex((c) => norm(c) === "descricao");
  const totalFaturamento = lTotGeral ? num(lTotGeral[cDescR + 1]) : null;
  const totalImpostos = lTotGeral ? num(lTotGeral[cDescR + 2]) : null;

  const lTorg = acharLinha(/^total de faturamento torg$/, 8);
  const lDireto = acharLinha(/^total de faturamento direto$/, 8);
  const totalLinha = (lin) => {
    if (!lin) return null;
    const i = lin.findIndex((c) => /^total de faturamento/.test(norm(c)));
    // a linha é [rótulo, custos, ―, BDI, valor total]
    return num(lin[i + 4]);
  };
  ...
  return {
    aba: aba.nome,
    aliquotas,
    cfopPadrao: cfops.find(Boolean) || null,
    faturamento,
    venda: aposRotulo(lVenda, /^valor total da venda$/),
    impostos: aposRotulo(lImp, /^impostos$/),
    impostosPct: lImp ? num(lImp[lImp.findIndex((c) => /^impostos$/.test(norm(c))) + 2]) : null,
    bdi: aposRotulo(lBdi, /^bdi$/),
    bdiPct: lBdi ? num(lBdi[lBdi.findIndex((c) => /^bdi$/.test(norm(c))) + 2]) : null,
    margem: aposRotulo(lMargem, /^margem de lucro/),
    credito,
    totalFaturamento,
    // imposto LÍQUIDO da obra: o que sai nas notas menos o crédito recuperável
    totalImpostos,
    totalImpostosPct: totalFaturamento > 0 && totalImpostos > 0 ? totalImpostos / totalFaturamento : null,
    faturamentoTorg: totalLinha(lTorg),
    faturamentoDireto: totalLinha(lDireto),
  };
}
...
export async function lerEstudoComercial(buffer) {
  ...
  return { modelo, abas: wb.SheetNames, aco, pintura, familias, comercial, custos, bdi, cobertura, calhasRufos, faltando };
}
