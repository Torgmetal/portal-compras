...
function lerItensComerciais(XLSX, wb) {
  ...
  for (let i = iNomes + 1; i < linhas.length; i++) {
    const l = linhas[i] || [];
    const area = String(l[1] ?? l[0] ?? "").trim();
    const ehTotal = /^total$/i.test(norm(String(l[0] ?? "")));
    for (const f of familias) {
      const v = num(l[f.col]);
      if (!(v > 0)) continue;
      if (ehTotal) f.total = v; else f.porArea.push({ area, qtd: v });
    }
  }
...
export async function lerEstudoComercial(buffer) {
  ...
  return { modelo, abas: wb.SheetNames, aco, pintura, familias, cobertura, calhasRufos, faltando };
}
