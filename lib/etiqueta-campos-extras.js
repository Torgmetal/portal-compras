...
/**
 * As marcas marcadas como CAIXA cujas unidades têm destinos diferentes.
 *
 * ⚠⚠ A CAIXA COM DUAS TAGS SAI SEM TAG — E ISSO PRECISA SER DITO ANTES. `tagDaEtiqueta` recusa,
 * com razão, escolher entre o TC 4706 e o TC 4707 para uma caixa que leva peças dos dois; mas a
 * cobertura vê a marca inteiramente coberta e não pergunta nada, então o operador só descobre com
 * o adesivo na mão (Codex, 14/09/2026). Ou ele separa as caixas por destino, ou confirma.
 *
 * @param {{marca:string, qte:number, emCaixa?:boolean}[]} pecas
 * @param {Map<string, Map<number,string>>} porMarca  de `tagsPorUnidade`
 */
export function caixasComDestinosDiferentes(pecas, porMarca) {
  const achadas = [];
  for (const p of pecas) {
    if (!p.emCaixa) continue;
    const m = porMarca.get(chave(p.marca));
    if (!m?.size) continue;
    const qte = Math.max(1, Number(p.qte) || 1);
    const destinos = new Set();
    for (let u = 1; u <= qte; u++) if (m.has(u)) destinos.add(m.get(u));
    if (destinos.size > 1) achadas.push({ marca: p.marca, destinos: [...destinos] });
  }
  return achadas;
}
