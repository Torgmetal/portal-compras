...
import {
  A, L, MM, ajustarTexto, contagemDaEtiqueta, desenharEndereco, numeroDaEtiqueta, pesoBR, pincel,
} from "@/lib/etiqueta-pdf-base";
import { desenharEtiquetaQws } from "@/lib/etiqueta-qws-pdf";
import { descricaoComTag } from "@/lib/etiqueta-campos-extras";
...
export const GRADE = {
  ...
  // ⚠⚠ `yOP` É IGUAL A `yCabecalho`, E NÃO POR COINCIDÊNCIA. Matheus (14/09/2026): "preciso que o
  // QR CODE fique abaixo da linha vermelha para alinhar o cabeçalho com campo O.P.". Enquanto essa
  // divisória estava em 8, a célula do O.P. terminava sete milímetros acima da do endereço e o QR
  // começava DENTRO da faixa do cabeçalho — o traço que atravessa a etiqueta passava por cima dele.
  // Com as duas no mesmo y, o cabeçalho é uma faixa só de ponta a ponta. Ver `testes/etiqueta-grade`.
  yOP: 15,             // O.P.  /  QR
  yCliente: 24,
...
/**
 * O CORPO DE CADA COISA ESCRITA, em pontos. Um lugar só, porque eles se equilibram entre si.
 *
 * ⚠⚠ A REFERÊNCIA É A ETIQUETA DO BARTENDER, NÃO O ESPAÇO LIVRE. Matheus (11/09/2026), com as duas
 * lado a lado na mesma foto: "a parte operacional está funcionando, agora precisamos deixar mais
 * próximo da etiqueta da esquerda — ela é bem mais visível, maior, sem falhas". A nossa tinha os
 * dados em 7,5–8 pt onde a antiga usa o dobro; sobrava célula vazia em todo canto.
 *
 * ⚠ O TETO É A CÉLULA, E QUEM O RESPEITA É O `ajustarTexto`. Estes números são o MÁXIMO: cliente
 * comprido ou descrição longa encolhem sozinhos. Por isso dá para ser generoso aqui sem medo de
 * texto invadindo a célula vizinha — o que não pode é o contrário, que era o caso.
 *
 * ⚠ O RÓTULO NÃO ACOMPANHA O VALOR. Ele foi de 4,2 para 5,5 (também borrava), mas de propósito
 * continua muito menor: "CLIENTE:" é a legenda, quem lê no pátio procura o valor. Subir os dois
 * juntos devolveria a etiqueta cinzenta e sem hierarquia que a atual era.
 */
/**
 * AS LINHAS DE BASE de cada texto, em mm do topo. Um lugar só, e conferidas por teste.
 *
 * ⚠⚠ NÃO SÃO NÚMEROS DE ENFEITE. Quando o valor foi de 8 para 13 pt, as bases herdadas passaram a
 * cruzar o traço da célula — a vírgula de "41,40" descia 0,9 mm e saía por baixo da moldura. Cada
 * uma aqui foi calculada contra a altura REAL da fonte (ascendente e descendente do Helvetica
 * Bold), e `testes/lib/etiqueta-carregamento.teste.js` refaz essa conta: mexeu no corpo sem mexer
 * na base, o teste acusa antes de a etiqueta sair torta do rolo.
 */
export const BASES = {
  cliente: 21.2, obra: 30.2,
  // ⚠ A TAG e a QTDE dividem a MESMA linha de base, e não por estética: a 16 pt o descendente do
  // Helvetica Bold tem 1,17 mm, e a 40,8 a TAG cruzava o traço de 41,5. O teste pegou.
  rot1: 35.4, val1: 40.3, tag: 40.3,   // faixa de cima do rodapé (33 → 41,5)
  rot2: 43.8, val2: 47.7,              // faixa de baixo (41,5 → 48,8)
};

export const CORPOS = {
  rotulo: 5.5,       // CLIENTE:, OBRA:, QTDE. (PÇ):, TAG:, DESCRIÇÃO:
  campo: 13,         // o nome do cliente e o da obra
  celula: 12,        // os números de QTDE e PESO
  tag: 16,           // a marca — é o que se lê de longe
  descricao: 11,
  op: 10,            // T103, no alto à direita
  legendaQr: 6.5,
};
...
export async function gerarEtiquetasCarregamentoPDF({ cliente, obra, tagObra, opNumero, pecas, modelo = "padrao", calibragem }) {
  ...
  for (const peca of pecas) {
    ...
    // ⚠⚠ UM QR POR MARCA, NÃO POR ETIQUETA — e isto era um 504 em produção (14/09/2026). O QR
    // codifica a MARCA, então as 110 etiquetas da mesma marca carregavam 110 imagens IDÊNTICAS:
    // 110 gerações de PNG e 110 `embedPng`, cada um virando um objeto novo dentro do arquivo.
    // Medido na OP-105 (1.793 etiquetas): 29,3s e 10,3 MB — e a função da Vercel morria antes de
    // terminar. Gerado e embutido UMA vez, o mesmo objeto é desenhado em todas as páginas dela.
    const qr = await pdf.embedPng(
      await QRCode.toBuffer(String(peca.marca ?? ""), { type: "png", margin: 0, width: 320, errorCorrectionLevel: "M" }),
    );
...
function cabecalho(p, { opNumero, marca, qr, logo }) {
  ...
  // ⚠ `xMin` é onde o logo acaba (3,5 + LOGO) mais 1,5 mm de respiro: é o que impede o endereço
  // de crescer por cima dele.
  desenharEndereco(p, { xMin: 3.5 + LOGO + 1.5, xFim: colDir - 2, yIni: 4.9, entreLinhas: 4, tamMax: 6.5 });

  p.negrito("O.P.:", colDir + 1.5, 5.2, CORPOS.rotulo);
  p.encaixar({ s: numeroDaEtiqueta(opNumero), mmX: colDir + 9.5, mmY: 6.4, xFim: fimDir - 1.5, tamMax: CORPOS.op });

  // ⚠ O QR ENCOLHEU DE 14 PARA 12,5 mm quando a célula dele passou a começar em `yOP` (15) em vez
  // de 8: sobraram 18 mm de altura para o QR MAIS a legenda. Com 14 o código encostava no texto —
  // e encostar, na térmica, é o mesmo que borrar. A 203 dpi, 12,5 mm ainda são ~100 pontos.
  const lado = 12.5;
  p.imagem(qr, colDir + (fimDir - colDir - lado) / 2, GRADE.yOP + 1.2 + lado, lado);
  ...
  const cap = ajustarTexto(marca, p.bold, { xIni: colDir + 1, xFim: fimDir - 1, tamMax: CORPOS.legendaQr });
 ...
 * se vê.
 */
function linhaDeCampo(p, { rotulo, valor, xRot, xVal, mmY, xFim, tam = CORPOS.campo }) {
  p.negrito(rotulo, xRot, mmY, CORPOS.rotulo);
...
function celulaCentrada(p, { rotulo, valor, entre, yRot, yVal, tam }) {
  p.centro({ s: rotulo, entre, mmY: yRot, tam: CORPOS.rotulo, f: p.bold });
...
function corpo(p, { cliente, obra, tagObra, peca, indice, total }) {
  ...
  const xRot = 3.5;
  const xVal = xRot + Math.max(p.larg("CLIENTE:", CORPOS.rotulo, p.bold), p.larg("OBRA:", CORPOS.rotulo, p.bold)) + 2;
  ...
  // ⚠ As linhas de base saem do CENTRO ÓTICO da célula, não de um número herdado: o valor cresceu
  // de 8 para 13 pt e, mantido o 20,8 de antes, encostaria no traço de cima.
  linhaDeCampo(p, { rotulo: "CLIENTE:", valor: cliente || "—", xRot, xVal, mmY: BASES.cliente, xFim: colDir - 1.5 });
  linhaDeCampo(p, { rotulo: "OBRA:", valor: obraComTag(tagObra, obra), xRot, xVal, mmY: BASES.obra, xFim: colDir - 1.5 });

  celulaCentrada(p, { rotulo: "QTDE. (PÇ):", valor: contagemDaEtiqueta(indice, total),
                      entre: [B, colQtde], yRot: BASES.rot1, yVal: BASES.val1, tam: CORPOS.celula });
  celulaCentrada(p, { rotulo: "PESO (kg):", valor: pesoBR(peca.pesoUnitKg),
                      entre: [B, colQtde], yRot: BASES.rot2, yVal: BASES.val2, tam: CORPOS.celula });

  // TAG é o que se lê de longe no pátio: ocupa a célula inteira.
  p.negrito("TAG:", colQtde + 1.5, BASES.rot1, CORPOS.rotulo);
  p.encaixar({ s: peca.marca, mmX: colQtde + 12, mmY: BASES.tag, xFim: fimDir - 1.5, tamMax: CORPOS.tag });

  p.negrito("DESCRIÇÃO:", colQtde + 1.5, BASES.rot2, CORPOS.rotulo);
  const desc = descricaoComTag(peca, indice);
  const d = ajustarTexto(desc, p.bold, { xIni: colQtde + 1.5, xFim: fimDir - 1.5, tamMax: CORPOS.descricao });
  p.centro({ s: d.texto, entre: [colQtde, fimDir], mmY: BASES.val2, tam: d.tam, f: p.bold });
}
