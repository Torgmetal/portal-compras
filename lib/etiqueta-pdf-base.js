...
/**
 * A ESPESSURA DE TODO TRAÇO DA ETIQUETA, em pontos.
 *
 * ⚠⚠ 0,7 pt SAÍA FALHADO. Matheus (11/09/2026), comparando a nossa com a do BarTender: "ela é bem
 * mais visível, maior, sem falhas". A 203 dpi, 0,7 pt = 0,25 mm = **2 dots** — e um traço de 2 dots
 * na transferência térmica é justamente onde falha: basta um ponto frio da cabeça ou o adesivo
 * passar um pouco torto para a linha sair picotada. A 1,1 pt são ~3 dots, e o traço fecha.
 *
 * ⚠ Sobe junto com o corpo das letras, não sozinho: moldura grossa em volta de texto miúdo faz o
 * texto parecer ainda menor. Os dois mudaram na mesma passada.
 */
export const ESPESSURA = 1.1;
 ...
 *
 * ⚠ O TETO NÃO É LIMITAÇÃO DE ESPAÇO, É HIERARQUIA. No QWS caberiam 9 pt: o endereço ficaria do
 * tamanho da TAG PETROBRAS, que é o que o cliente precisa ler de longe. Endereço é informação de
 * apoio e tem que parecer uma.
 *
 * ⚠⚠ VAI EM NEGRITO, e isso importa mais que o corpo. Era o ÚNICO texto fino da etiqueta — na
 * transferência térmica a haste fina de uma Helvetica normal a 5 pt não fecha, e o que sai é a
 * letra esburacada da foto de 11/09/2026. O negrito é ~5% mais largo, então o corpo calculado cai
 * um pouco; ainda assim imprime melhor, porque o que borra é a espessura da haste, não a altura.
 *
...
export function desenharEndereco(p, { xMin, xFim, yIni, entreLinhas, tamMax = 5.5 }) {
  ...
  for (let t = 3; t <= tamMax; t += 0.25) {
    if (xFim - Math.max(...ENDERECO.map((l) => p.larg(l, t, p.bold))) >= xMin) tam = t;
  }
  ENDERECO.forEach((l, i) => p.negrito(l, xFim - p.larg(l, tam, p.bold), yIni + i * entreLinhas, tam));
  return tam;
}
...
export function pincel(pg, font, bold) {
  ...
  return {
    ...
    linha: (x1, y1, x2, y2) =>
      pg.drawLine({ start: { x: x(x1), y: dy(y1) }, end: { x: x(x2), y: dy(y2) }, thickness: ESPESSURA, color: PRETO }),
