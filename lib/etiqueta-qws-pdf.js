...
function cabecalho(p, { cliente, obra, opNumero, logo }) {
  ...
  desenharEndereco(p, { xMin: 3 + LOGO + 1.5, xFim: fimDir - 1.5, yIni: 4.2, entreLinhas: 3.2 });
  ...
  valorCentrado(p, { s: alvo, entre: [borda, colOP], mmY: yCabecalho + 6.9, tamMax: 8 });
  ...
  p.centro({ s: numeroDaEtiqueta(opNumero), entre: [colOP, fimDir], mmY: yCabecalho + 6.9, tam: 7.5, f: p.bold });
}
...
function faixaTag(p, { tagPetrobras, marca, qr }) {
  ...
  valorCentrado(p, { s: tagPetrobras, entre: [B, colQR], mmY: yTag - 2.2, tamMax: 13 });
  ...
  const cap = ajustarTexto(marca, p.bold, { xIni: colQR + 1, xFim: fimDir - 1, tamMax: 5.5 });
