import "server-only";
import { ajustarTexto, contagemDaEtiqueta, desenharEndereco, numeroDaEtiqueta, pesoBR, pincel } from "@/lib/etiqueta-pdf-base";
...
// do próprio valor. As linhas de base agora são explícitas, medidas a partir do topo do rodapé.
//
// ⚠⚠ O ESPAÇO ESTAVA TODO GASTO EM FOLGA. Matheus (11/09/2026) pediu a etiqueta "mais visível,
// maior, sem falhas", com a do BarTender ao lado. As faixas não mudaram de tamanho — o que mudou é
// que agora as quatro linhas de base são calculadas para o texto ENCOSTAR uma na outra com ~0,3 mm
// de respiro, em vez de 1 mm: rótulo 4,2 → 5 pt e valor 6,5 → 8 pt, tudo dentro dos mesmos 12,8 mm.
// Conferido com descendente ("Ç" do rótulo, vírgula de "41,40"), que é quem estoura primeiro.
export const RODAPE = { rot1: 2.4, val1: 5.9, rot2: 8.6, val2: 12.1, tamValor: 8, tamRotulo: 5 };

/**
 * Escreve o VALOR centrado na célula, encolhendo a fonte se precisar.
 *
 * ⚠ O RÓTULO NÃO ACOMPANHA. Matheus decidiu isso primeiro na O.P. (10/09/2026) e depois pediu o
 * mesmo para a TAG PETROBRAS, a QTDE e o PESO: o rótulo é etiqueta da célula e vive colado à
 * esquerda; o valor é o que se lê, e valor encostado na borda parece texto solto em vez de conteúdo
 * do campo. Ter uma função só evita as três células divergirem no próximo ajuste.
 *
 * ⚠ A folga de 1,5 mm de cada lado é o limite para ENCOLHER, não para centrar. O centro é o da
 * célula inteira: descontar a folga do centro empurraria o texto para a esquerda de novo.
 */
function valorCentrado(p, { s, entre: [x1, x2], mmY, tamMax }) {
  const r = ajustarTexto(String(s ?? "") || "—", p.bold, { xIni: x1 + 1.5, xFim: x2 - 1.5, tamMax });
  p.centro({ s: r.texto, entre: [x1, x2], mmY, tam: r.tam, f: p.bold });
}

/**
 * O RÓTULO no centro da célula, sobre o valor que também está centrado.
 *
 * ⚠ Não vale para toda célula: REFERÊNCIA e TAG / DESCRIÇÃO seguem à esquerda porque o valor delas
 * é texto longo, e texto longo centrado dá margem serrilhada dos dois lados. Centrar só faz sentido
 * onde o conteúdo é curto e a célula é larga — que é o caso destas.
 */
const rotuloCentrado = (p, { s, entre, mmY }) =>
  p.centro({ s, entre, mmY, tam: RODAPE.tamRotulo, f: p.bold });
...
function cabecalho(p, { cliente, obra, opNumero, logo }) {
  const { borda, colOP, fimDir, yCabecalho } = GRADE_QWS;
  const LOGO = 32;
  ...
  // ⚠ AQUI O TETO DE 5,5 pt PEGA DE VERDADE (no padrão quem manda é o logo). Nesta grade o
  // endereço teria 60 mm livres e chegaria a 9 pt — do tamanho da TAG PETROBRAS, que é o que o
  // cliente lê de longe. Em negrito a 5,5 pt ele já imprime limpo, que era o problema.
  desenharEndereco(p, { xMin: 3 + LOGO + 1.5, xFim: fimDir - 1.5, yIni: 4.3, entreLinhas: 3.4 });
  ...
  // ⚠ O NOME VAI CENTRADO, o rótulo não — Matheus (10/09/2026): "o nome do cliente/obra deixar
  // centralizado". A célula é larga (60 mm) e "QWS | Revamp" é curto: encostado à esquerda ele
  // deixava metade da célula vazia.
  p.negrito("CLIENTE / OBRA:", 3, yCabecalho + 2.8, RODAPE.tamRotulo);
  const alvo = [cliente, obra].map((s) => String(s ?? "").trim()).filter(Boolean).join(" | ") || "—";
  valorCentrado(p, { s: alvo, entre: [borda, colOP], mmY: yCabecalho + 7.1, tamMax: 10 });

  // ⚠ O NÚMERO VAI CENTRADO NA CÉLULA, o rótulo não. Matheus (10/09/2026), sobre a etiqueta
  // impressa. Recuado 8 mm da borda ele ficava nem encostado no rótulo nem no meio — parecia
  // desalinhado justamente por não seguir nenhuma das duas referências.
  p.negrito("O.P.:", colOP + 1.5, yCabecalho + 2.8, RODAPE.tamRotulo);
  p.centro({ s: numeroDaEtiqueta(opNumero), entre: [colOP, fimDir], mmY: yCabecalho + 7.1, tam: 10, f: p.bold });
}
...
function faixaTag(p, { tagPetrobras, marca, qr }) {
  ...
  // ⚠ RÓTULO E VALOR CENTRADOS. Matheus (10/09/2026): "deixe a TAG PETROBRAS mais para o centro do
  // campo dela" e, na volta seguinte, "faltou mover tbm os títulos desses campos". Encostados em
  // B+1,8 sobrava espaço à direita — e a TAG é o texto maior da etiqueta, então o desalinhamento
  // salta à vista. A faixa do rótulo tem a mesma largura da do valor, então os dois centram no
  // mesmo eixo e ficam aprumados um sobre o outro.
  rotuloCentrado(p, { s: "TAG PETROBRAS:", entre: [B, colQR], mmY: yCliente + 3.6 });
  valorCentrado(p, { s: tagPetrobras, entre: [B, colQR], mmY: yTag - 2.2, tamMax: 16 });
  ...
  // inteira sai no rodapé, no campo TAG / DESCRIÇÃO.
  //
  // ⚠ CENTRADA SOB O QR, não encostada na borda. Matheus (10/09/2026), sobre a etiqueta impressa:
  // colada à esquerda ela não parecia legenda do código, parecia texto solto na célula.
  const cap = ajustarTexto(marca, p.bold, { xIni: colQR + 1, xFim: fimDir - 1, tamMax: 6.5 });
  p.centro({ s: cap.texto, entre: [colQR, fimDir], mmY: yTag - 0.8, tam: cap.tam, f: p.bold });
}
...
function rodape(p, { peca, unidade, indice, total }) {
  ...
  // ⚠ RÓTULO E VALOR CENTRADOS. Matheus (10/09/2026): "as QTDE PC e PESO kg mais para centro do
  // campo deles" e depois "faltou mover tbm os títulos desses campos". A coluna tem 24,8 mm e
  // "1/1"/"13,26" são curtíssimos: encostados à esquerda sobrava quase a célula inteira vazia.
  rotuloCentrado(p, { s: "QTDE. (PÇ):", entre: [B, colRodape], mmY: yTag + rot1 });
  valorCentrado(p, { s: contagemDaEtiqueta(indice, total), entre: [B, colRodape], mmY: yTag + val1, tamMax: tamValor });

  rotuloCentrado(p, { s: "PESO (kg):", entre: [B, colRodape], mmY: yTag + rot2 });
  valorCentrado(p, { s: pesoBR(peca.pesoUnitKg), entre: [B, colRodape], mmY: yTag + val2, tamMax: tamValor });

  p.negrito("REFERÊNCIA:", colRodape + 1.8, yTag + rot1, RODAPE.tamRotulo);
  ...
  p.negrito("TAG / DESCRIÇÃO:", colRodape + 1.8, yTag + rot2, RODAPE.tamRotulo);
