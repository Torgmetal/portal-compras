...
function faixaTag(p, { tagPetrobras, marca, qr }) {
  ...
  // ⚠ CENTRADA NA CÉLULA. Matheus (10/09/2026), sobre a etiqueta impressa: "deixe a TAG PETROBRAS
  // mais para o centro do campo dela". Encostada em B+1,8 ela sobrava espaço à direita e lia como
  // se estivesse fora do lugar — é o texto maior da etiqueta, e o desalinhamento salta à vista.
...
function rodape(p, { peca, unidade, indice, total }) {
  ...
  // ⚠ RÓTULO À ESQUERDA, VALOR CENTRADO — a mesma regra da O.P. e da TAG. Matheus (10/09/2026):
  // "as QTDE PC e PESO kg mais para centro do campo deles". "1/1" e "13,26" são curtos, então
  // encostados em B+2,8 sobrava quase toda a célula vazia à direita.
  p.negrito("QTDE. (PÇ):", B + 1.8, yTag + rot1, RODAPE.tamRotulo);
  valorCentrado(p, { s: contagemDaEtiqueta(indice, total), entre: [B, colRodape], mmY: yTag + val1, tamMax: tamValor });

  p.negrito("PESO (kg):", B + 1.8, yTag + rot2, RODAPE.tamRotulo);
