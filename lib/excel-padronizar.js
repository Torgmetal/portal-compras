...
export async function padronizarPlanilha(buf, { titulo, subtitulo, codigoDoc, semPeso } = {}) {
  ...
  // ⚠⚠ PESO É PREÇO. Vitor (22/08/2026). O portal já publica a LPC e a LE sem peso quando a obra não
  // liberou; uma planilha da pasta com "PESO UNIT. (kg)" passaria por fora dessa regra inteira. Em
  // vez de barrar o documento — que é útil por tudo o mais que ele traz —, a coluna de peso sai na
  // hora de entregar. O arquivo no servidor continua intacto.
  const colsFora = new Set();
  if (semPeso) {
    cab.forEach((t, i) => { if (/\bpeso\b|\bweight\b/i.test(String(t))) colsFora.add(i); });
  }
  const nBrutas = Math.min(MAX_COLS, Math.max(...linhas.slice(iCab).map((l) => l.length), cab.length));
  const usadas = Array.from({ length: nBrutas }, (_, i) => i).filter((i) => !colsFora.has(i));
  const nCols = usadas.length;
  ...
  let l = linhaInicio;
  adicionarHeaderTabela(ws2, l, usadas.map((c) => cab[c] || "")); l++;
  ...
  for (const linha of dados) {
    ...
    adicionarLinhaTabela(ws2, l, usadas.map((c) => (limpar(linha[c]) ?? "")),
