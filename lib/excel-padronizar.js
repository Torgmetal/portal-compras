...
export async function padronizarPlanilha(buf,{titulo,subtitulo,codigoDoc,semPeso}={}){
 ...
 if(buf?.[0]===0x50&&buf?.[1]===0x4b){
  try{if(Object.keys(new PizZip(buf).files).some(n=>/^xl\/(drawings|charts|pivotTables|externalLinks|embeddings)\//.test(n)))return null;}catch{return null;}
 }
 let original;
 try{original=XLSX.read(buf,{type:'buffer',cellDates:true,cellFormula:true,bookVBA:true});}catch{return null;}
