...
export const LINHA_MAXIMA_CONGELAR=10;
...
export function refinarPlanilhaExcel(ws,{cabecalho,ultimaLinha,colunas,preservarAlturas=false}={}){
 ...
 // Congela o painel no cabeçalho SÓ quando ele está perto do topo — é tabela de dados, e a linha
 // de títulos precisa acompanhar a rolagem. Cabeçalho abaixo da linha 10 é folha-relatório (blocos
 // de premissas antes da tabela): congelar ali deixa oito linhas rolando num notebook. Vitor
 // (21/09/2026): "essas planilhas com esses painéis congelados não estão legais, fica difícil de
 // visualizar". A folha pode recusar de vez com `ws._torgSemCongelar = true`.
 const congelavel=tabelas.length===1&&tabelas[0].headerRow<=LINHA_MAXIMA_CONGELAR&&!ws._torgSemCongelar;
 if(congelavel&&!ws.views.some(v=>v.state==='frozen'||v.state==='split'))ws.views=[{...ws.views[0],state:'frozen',ySplit:tabelas[0].headerRow}];
 ...
 for(const t of tabelas){
  ...
  for(const r of t.rows){
   ...
   for(let c=1;c<=t.cols;c++){
    ...
    if(!ws._torgPreservarValores&&typeof cell.value==='string'&&/data|vencimento|validade|recebido em|emiss[aã]o|emitido em|pagamento|conferid[oa] em|[uú]ltim[oa]|in[ií]cio|fim|prevista/i.test(t.headers[c-1]||'')){
