...
export function refinarPlanilhaExcel(ws,{cabecalho,ultimaLinha,colunas,preservarAlturas=false}={}){
 ...
 for(const t of tabelas){
  for(let c=1;c<=t.cols;c++){
   ...
   cell.font={...cell.font,name:'Arial',size:Math.max(10,cell.font?.size||0),bold:true,color:{argb:'FFFFFF'}};
 ...
 if(tabelas.length===1){
  ...
  if(!ws.pageSetup.printTitlesRow)ws.pageSetup.printTitlesRow=`${t.headerRow}:${t.headerRow}`;
 }
 return ws;
}
