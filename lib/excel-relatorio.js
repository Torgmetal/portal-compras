...
export async function criarRelatorioTorg(opts) {
  ...
  // ⚠⚠ O CABEÇALHO PRECISA DE 4 COLUNAS. Logo ocupa 1-2, título começa na 3 e o bloco de controle
  // (Doc/Rev/Data) vem depois. Com 3 colunas o merge do controle saía invertido e a geração inteira
  // morria em "Cannot merge already merged cells" — a planilha não saía, e nenhum chamador tinha
  // como saber por quê. Quem passa menos ganha um cabeçalho de 4; os dados seguem no que pediu.
  const totalCols = Math.max(4, opts.totalColunas || 10);
...
export function adicionarLinhaTabela(ws, linha, valores, opts = {}) {
  ...
  valores.forEach((v, i) => {
    ...
    cell.alignment = {
      vertical: "middle",
      horizontal: opts.alinhamento?.[i] || (typeof v === "number" ? "right" : "left"),
...
export async function downloadWorkbook(wb, fileName) {
  // AutoFiltro do Excel em cabeçalho+dados de cada planilha (registrado em
  // adicionarHeaderTabela) — todo relatório sai com filtro nas colunas.
  for (const ws of wb.worksheets) {
    const f = ws._torgFiltro;
    if (f && f.cols > 0) {
      ws.autoFilter = {
        from: { row: f.headerRow, column: 1 },
        to: { row: Math.max(f.headerRow, f.lastRow), column: f.cols },
      };
    }
  }
