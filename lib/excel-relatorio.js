...
export async function adicionarFolhaTorg(wb, opts) {
  ...
  if(totalCols<6){
    for(let r=1;r<=3;r++)if(totalCols>1)ws.mergeCells(r,1,r,totalCols);
    ws.getCell(1,1).value="TORG METAL";
    ws.getCell(1,1).font={name:"Arial",size:13,bold:true,color:{argb:TORG_BLUE}};
    ws.getCell(1,1).alignment={horizontal:"right",vertical:"middle"};
    ws.getCell(2,1).value=opts.titulo||"Relatório";
    ws.getCell(2,1).font={name:"Arial",size:12,bold:true,color:{argb:TORG_DARK}};
    ws.getCell(3,1).value=`${codigoDoc} · Revisão ${opts.revisao||"00"} · ${dataEmissao} ${horaEmissao}${opts.form?" · "+refFORM(opts.form):""}`;
    ws.getCell(3,1).font={name:"Arial",size:8,color:{argb:TORG_GRAY}};
    ws.getRow(1).height=48;ws.getRow(2).height=30;ws.getRow(3).height=24;
    for(let c=1;c<=totalCols;c++)ws.getColumn(c).width=20;
    const logo=await carregarLogo();
    if(logo){const id=wb.addImage({buffer:logo,extension:"png"});ws.addImage(id,{tl:{col:0.1,row:0.1},ext:{width:90,height:50}});}
  }else{
  // === CABECALHO ISO 9001 ===
  // Linha 1-3: Logo (col 1-2) | Titulo (col 3 ao penultimo) | Controle (ultimas 2 cols)

  // ⚠⚠ O CONTROLE ENCOLHE, O TÍTULO NÃO. Vitor (01/09/2026): "a exportação de planilha anda saindo
  // tudo quebrado... tenho que ficar ajustando a formatação".
  //
  // Era `Math.max(totalCols - 2, 4)`: o bloco de controle sempre pegava 3 colunas, então numa
  // planilha de 5 ou 6 sobrava UMA coluna para o título — e um título como "Plano de Montagem por
  // Bancada" não cabe em 20 caracteres. Agora o controle é que se aperta (3 colunas a partir de 8,
  // 2 a partir de 6, 1 abaixo disso), porque ele é sempre texto curto: REL-XXX-000, Rev 00, data.
  const larguraControle = totalCols >= 8 ? 3 : totalCols >= 6 ? 2 : 1;
  const colControleInicio = Math.max(4, totalCols - larguraControle + 1);
  ...
  // Altura das linhas do header (mais altas pra caber logo proporcional)
  ws.getRow(1).height = 26;
  ws.getRow(2).height = 22;
  ws.getRow(3).height = 22;

  // Largura minima das colunas do logo (garante espaco pra centralizar)
  if (!ws.getColumn(1).width || ws.getColumn(1).width < 6) ws.getColumn(1).width = 6;
  if (!ws.getColumn(2).width || ws.getColumn(2).width < 14) ws.getColumn(2).width = 14;

  // Logo — imagem original eh 160x90 (ratio 1.78:1)
  // Usar proporcoes corretas pra nao distorcer
  const logoData = await carregarLogo();
  if (logoData) {
    const imgId = wb.addImage({ buffer: logoData, extension: "png" });
    ws.addImage(imgId, {
      tl: { col: 0.15, row: 0.15 },
      ext: { width: 120, height: 67 },
    });
  }
  ...
  // ⚠ a 3ª linha do bloco existe e estava só pintada. É nela que entra a identificação do
  // formulário do SGQ quando a planilha É um formulário — a ISO 9001 §7.5.2 pede isso NO documento,
  // e a planilha não tem rodapé desenhado como os PDFs têm.
  const ctrlLabels = [
    { label: "Codigo", valor: codigoDoc },
    { label: "Emissao", valor: `${dataEmissao} ${horaEmissao}` },
  ];
  const refForm = opts.form ? refFORM(opts.form) : "";
  if (refForm) ctrlLabels.push({ label: "Formulario", valor: refForm });
  ...
  // Linha 3 do controle: fundo uniforme (quando não levou o formulário)
  if (!refForm) {
    const cellCtrl3 = ws.getCell(3, colControleInicio);
    cellCtrl3.value = `Revisão: ${opts.revisao || "00"}`;
    cellCtrl3.font = { name: "Arial", size: 8, color: { argb: TORG_GRAY } };
    cellCtrl3.alignment = { vertical: "middle", horizontal: "right", wrapText: true };
