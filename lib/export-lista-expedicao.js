...
export async function exportarListaExpedicao({ op, frentes, marcasFiltradas, sufixo }) {
  ...
  const { workbook, sheet: ws, linhaInicio } = await criarRelatorioTorg({
    ...
    // A LE do portal traz colunas que o modelo não tem (Expedido, Pendente, Romaneio, Data
    // expedida). Vitor: "foi uma melhoria que fizemos" — é por isso que a revisão sobe.
    form: 21,
  });
  ...
  for (const m of todas) {
    ...
    adicionarLinhaTabela(ws, row, [
      ...
      Number((m.pesoTotal || 0).toFixed(2)),
      expShow, pend ?? "—", sit,
      m.romaneio || "—",
