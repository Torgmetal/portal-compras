...
Campo que o boletim não traz fica VAZIO (""). Números vão como texto, só o número (ex.: "72", "10,5").

Campos de cada produto:
- categoria: o que é o documento: "TINTA" (a tinta em si), "ENDURECEDOR" (endurecedor/catalisador/componente B vendido à parte) ou "DILUENTE" (diluente/thinner/redutor). Boletim de tinta bicomponente que descreve A e B juntos é "TINTA".
- fabricante: quem fabrica.
- produto: o nome comercial completo, como está no documento (ex.: "INDUSTHANE RHB DF", "W-POXI ZSP 315"). Sem a cor no nome quando ela vier separada.
- especificacao: a descrição técnica do tipo (ex.: "Tinta poliuretano dupla função, acrílico alifático à base de isocianato").
- tipo: a função no esquema: "PRIMER" (fundo/primer), "INTERMEDIARIA", "ACABAMENTO", "UNICA" (quando o próprio boletim diz que é de dupla função / demão única). Vazio se não der para saber.
- norma: a norma ou especificação citada (ex.: "N-2680", "ISO 12944").
- diluente: o diluente INDICADO por este produto, com o código quando houver (ex.: "Diluente 34.019"). Só para categoria TINTA.
- componenteA: o nome/código do componente A (base/resina), como o boletim nomeia. Vazio quando o produto é monocomponente.
- componenteB: o nome/código do componente B (endurecedor/catalisador). Vazio quando é monocomponente.
- proporcaoMistura: a proporção de mistura A:B como está escrita (ex.: "4:1 em volume", "3:1 em peso").
- potLife: a vida útil da mistura / pot life (ex.: "4 h a 25 °C").
- diluicaoMin / diluicaoMax: a FAIXA de diluição em %. Se o boletim der um valor único, use o mesmo nos dois.
- camadas: a tabela de espessuras, uma linha por condição de diluição — { diluicao (%), umida (µm), seca (µm) }. Se o boletim só der uma espessura sem falar de diluição, use uma linha com diluicao "0". Lista vazia se não houver.
- secaMin / secaMax: a espessura SECA recomendada por demão, em µm.
- secagemToque / secagemManuseio / secagemRepintura: os tempos, como o boletim escreve (ex.: "6 h a 25 °C").
...
- solidosVol: o teor de sólidos EM VOLUME, só o número (ex.: "72"). É o dado que permite calcular a espessura úmida por EPU = EPS × (100 + %diluição) / sólidos em volume. Se o documento não trouxer o número mas trouxer a tabela de espessuras úmidas, DEDUZA o sólidos em volume a partir dela e diga na observação que foi deduzido.
- observacoes: só o que for relevante para aplicar (intervalo entre demãos, temperatura e umidade limites, restrições). Curto.

REGRAS:
- Não invente: o que o boletim não disser fica vazio. Numa tinta, um número plausível e errado vira espessura errada na obra inteira.
...
const CATEGORIAS = ["TINTA", "ENDURECEDOR", "DILUENTE"];

// ⚠ AUSENTE É TEXTO VAZIO, NÃO NULL. O boletim tem mais de vinte campos que podem faltar, e a API
// aceita no máximo 16 campos "ou null" por schema. `txt()` e `num()` já transformam "" em null — e
// zero não serve de ausente: "0% de diluição" é valor.
export const ESQUEMA_BOLETIM = Esquema.objeto({
  produtos: Esquema.lista(Esquema.objeto({
    categoria: Esquema.umDe(CATEGORIAS),
    fabricante: Esquema.texto,
    produto: Esquema.texto,
    especificacao: Esquema.texto,
    componenteA: Esquema.texto,
    componenteB: Esquema.texto,
    proporcaoMistura: Esquema.texto,
    potLife: Esquema.texto,
    tipo: Esquema.umDe([...TIPOS, ""]),
    norma: Esquema.texto,
    diluente: Esquema.texto,
    diluicaoMin: Esquema.texto,
    diluicaoMax: Esquema.texto,
    camadas: Esquema.lista(Esquema.objeto({ diluicao: Esquema.texto, umida: Esquema.texto, seca: Esquema.texto })),
    secaMin: Esquema.texto,
    secaMax: Esquema.texto,
    secagemToque: Esquema.texto,
    secagemManuseio: Esquema.texto,
    secagemRepintura: Esquema.texto,
    rendimento: Esquema.texto,
    solidos: Esquema.texto,
    solidosVol: Esquema.texto,
...
export async function extrairBoletim({ data, contentType, texto, arquivo }) {
  ...
  const { dados: j } = await pedirJson(anthropic, { model: MODELO, max_tokens: 8000, system: SYSTEM, messages: [{ role: "user", content }], formato: ESQUEMA_BOLETIM });
  const lista = Array.isArray(j?.produtos) ? j.produtos : [];
...
function umProduto(j) {
  ...
  const cat = String(j.categoria || "").toUpperCase();
  return {
    categoria: CATEGORIAS.includes(cat) ? cat : "TINTA",
    fabricante: txt(j.fabricante, 80),
    produto: txt(j.produto, 160),
    especificacao: txt(j.especificacao, 300),
    componenteA: txt(j.componenteA, 160),
    componenteB: txt(j.componenteB, 160),
    proporcaoMistura: txt(j.proporcaoMistura, 60),
    potLife: txt(j.potLife, 60),
