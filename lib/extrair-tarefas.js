...
- Atribua o setor responsável, um destes: ${SETORES_TAREFA.join(", ")}.
...
- opNumero: a OP a que a tarefa pertence, só os dígitos (ex.: "085"). A OP costuma ser dita uma vez para a reunião/assunto inteiro (no título, no cabeçalho ou na primeira menção) e não é repetida em cada item. Então:
  ...
  Use só números de OP que aparecem nos documentos.
...
- resumo (da resposta): 1-2 frases do que foi a reunião.`;

export const ESQUEMA_TAREFAS = Esquema.objeto({
  resumo: Esquema.texto,
  tarefas: Esquema.lista(Esquema.objeto({
    titulo: Esquema.texto,
    setor: Esquema.umDe(SETORES_TAREFA),
    prioridade: Esquema.umDe(["ALTA", "MEDIA", "BAIXA"]),
    responsavel: Esquema.textoOuNulo,
    prazo: Esquema.textoOuNulo,
    opNumero: Esquema.textoOuNulo,
    descricao: Esquema.textoOuNulo,
    doCliente: Esquema.logico,
    clienteNome: Esquema.textoOuNulo,
  })),
});
...
export async function extrairTarefas({ documentos, hoje }) {
  ...
  const { dados } = await pedirJson(anthropic, {
    model: MODELO,
    max_tokens: 8000,
    system: SYSTEM,
    messages: [{ role: "user", content }],
    formato: ESQUEMA_TAREFAS,
  });
