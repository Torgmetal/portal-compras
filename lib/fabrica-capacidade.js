...
export async function capacidadeDaFabrica(forcar = false) {
  ...
  const serie = new Map(); // setor -> [{ mes, kg }] em ordem
  for (const [chave, kg] of porSetorMes) {
    const [setor, mes] = chave.split("|");
    if (!serie.has(setor)) serie.set(setor, []);
    serie.get(setor).push({ mes, kg });
  }
  const leitura = (setor) => {
    const linha = (serie.get(setor) || []).sort((a, b) => a.mes.localeCompare(b.mes));
    ...
    // ⚠ o trimestre precisa ser CONSECUTIVO: três bons meses espalhados não provam que a fábrica
    // sustenta o ritmo — provam que ela teve três bons meses.
    let tri = null;
    for (let i = 0; i + 2 < linha.length; i++) {
      const kg = (linha[i].kg + linha[i + 1].kg + linha[i + 2].kg) / 3;
      if (!tri || kg > tri.kgMes) tri = { kgMes: kg, de: linha[i].mes, ate: linha[i + 2].mes };
    }
    return {
