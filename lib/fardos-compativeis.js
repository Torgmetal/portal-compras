...
// ⚠ SÓ RECEBIMENTO ATIVO (25/09/2026). O R 261401 da OP-118 tem um duplicado desativado — sobra da
// confusão de 22/09, quando o índice também era uma PORCA — e a lista o oferecia ao lado do certo.
export async function buscarFardosCompativeis(perfil) {
  const nomes = await prisma.documentoQualidade.findMany({
    where: { categoria: 'MATERIAL', ativo: true, ...DO_CMR },
  ...
  const linhas = await prisma.documentoQualidade.findMany({
    where: { categoria: 'MATERIAL', ativo: true, ...DO_CMR, nome: {in: compativeis}, importRef: {not: null} },
