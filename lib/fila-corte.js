...
export async function buscarFilaCorte() {
  ...
  /* ⚠ CROQUI DE CONJUNTO QUE ESTÁ NO TERCEIRO NÃO É FILA DE CORTE. Quem fabrica fora recebe BARRA,
     não peça cortada — ver lib/fora-da-fabrica.js. Só sai o que ainda está pendente: o que já foi
     cortado continua aparecendo, porque a coluna dos 30 dias é histórico e apagá-lo esconderia
     produção que aconteceu. */
  return foraDaFabrica.size ? pecas.filter((p) => !(foraDaFabrica.has(p.id) && !pecaCortada(p))) : pecas;
}
