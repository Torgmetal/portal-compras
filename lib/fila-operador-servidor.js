...
import { BANCADAS_CONSULTA_MONTAGEM as MONTAGEM } from "./postos-operador";
...
let cache = null,
  pendente = null;
let geracao = 0;
export function invalidarFilaOperador() {
  cache = null;
  pendente = null;
  geracao++;
}
...
export async function carregarFilaOperador({ atualizar = false } = {}) {
  if (atualizar) invalidarFilaOperador();
  ...
  const consulta = (async () => {
    ...
    const [pecas, ops] = await Promise.all([
      ids.length
        ? prisma.pecaConjunto.findMany({
            where: { id: { in: ids } },
            select: { id: true, opId: true, destino: true, prioridade: true },
          })
        : [],
    ...
    const dados = {
      ...
      lotes: lotes.map((l) => ({
        ...
        itens: l.itens.map((i) => ({
          ...i,
          prioridade: porId.get(i.id)?.prioridade ?? null,
          impedimento: IMPEDIMENTOS[porId.get(i.id)?.destino] || null,
        })),
      })),
    };
    if (versao === geracao) cache = { em: Date.now(), dados };
    return dados;
  })();
  pendente = consulta;
  try {
    ...
    if (pendente === consulta) pendente = null;
  }
}
