 ...
 * alimenta. A SOLDA entra aqui mas NÃO no `ANTERIOR` exportado, de propósito: aquele mapa é o que
 * habilita /pcp/fila-<setor> e a rota de atribuição, e a solda já tem a tela dela (/pcp/fila-solda)
 * com regra própria. Aqui é só leitura para o quadro.
 */
const CASCATA_FILA = { SOLDA: "MONTAGEM", ACABAMENTO: "SOLDA", JATO: "ACABAMENTO", PINTURA: "JATO" };
const SETORES_COM_FILA = Object.keys(CASCATA_FILA);

// ⚠⚠ A MONTAGEM NÃO ENTRA NA CASCATA, e tentar encaixá-la produz número absurdo. Medido em
// 07/09/2026: "pronto no corte e não montado" devolvia 3.759 peças e 281 t — das quais 96% eram
// CROQUI (3.387 peças, 245 t). Croqui não vai para bancada: é componente do conjunto.
//
// O conjunto nunca é "cortado" no Syneco; quem é cortado são os croquis dele. A regra certa já
// existe em lib/prontidao-conjunto.js e é dura de propósito — Vitor (01/09/2026) pediu "por
// conjunto que tenha todas as sub peças prontas para iniciar a montagem". Com ela a fila real é
// 22 conjuntos e 9.166 kg, não 281 t.
const CAMPO_DIA_MONTAGEM = "montagemDiaProgramado";
...
export async function filasSemProgramacao() {
  const DIA = { ...Object.fromEntries(Object.entries(CAMPOS).map(([k, v]) => [k, v.dia])), SOLDA: "soldaDiaProgramado" };
  const campos = {};
  for (const s of SETORES_COM_FILA) Object.assign(campos, { [DIA[s]]: true });
  ...
  //
  // Consulta à parte porque precisa dos croquis de cada conjunto — a cascata acima trabalha só com
  // apontamento por marca e não teria como responder "todas as sub peças estão cortadas?".
  ...
  const feitoConj = await lerProduzidoPorSetor(
    conjuntos.map((c) => ({ opId: c.opId, marca: c.marca })), ["MONTAGEM", SAIDA]
  );
  const prontoConj = (c, s) => feitoConj({ opId: c.opId, marca: c.marca }, s) >= Math.max(1, c.qte || 1);

  saida.MONTAGEM = conjuntos
    .filter((c) => !prontoConj(c, SAIDA) && !prontoConj(c, "MONTAGEM"))
    // ⚠ `pronto`, não `podeLiberar`: a regra da metade serve para a fábrica COMEÇAR; para marcar o
    // dia o critério é ter tudo cortado. As duas perguntas são diferentes e a lib separa as duas.
