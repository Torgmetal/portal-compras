...
/** Setores cuja fila de entrada aparece na faixa "sem bancada" do quadro. */
const SETORES_COM_FILA = ["ACABAMENTO", "JATO", "PINTURA"];
...
export async function filasSemProgramacao() {
  ...
  const chaves = pecas.map((p) => ({ opId: p.opId, marca: p.marca }));
  const setoresLidos = [...new Set([SAIDA, ...SETORES_COM_FILA, ...SETORES_COM_FILA.map((s) => ANTERIOR[s])])];
