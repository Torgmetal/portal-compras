 ...
 */
export async function abrirExecucao({ conversaId, userId, userNome, pergunta, chave, tentativaHash = null }) {
  return prisma.$transaction(async (tx) => {
    ...
    if (anterior) {
      // ⚠⚠ MESMA CHAVE, OUTRO CONTEÚDO É CONFLITO, NÃO REENVIO (parecer de segurança do Codex,
      // 24/09/2026). Sem isto, trocar o arquivo e reenviar com a chave antiga devolveria em silêncio
      // a resposta de OUTRA nota — a pessoa leria a análise da remessa achando que era a da venda.
      if (anterior.tentativaHash && tentativaHash && anterior.tentativaHash !== tentativaHash) {
        return { conflito: true };
      }
      return { repetida: true, conversa: anterior.conversa, resposta: anterior };
    }
    ...
    const resposta = await tx.fiscalMensagem.create({
      data: { conversaId: conversa.id, seq: base + 1, papel: "ASSISTENTE", chave, estado: "EM_ANDAMENTO", tentativaHash },
    });
...
/**
 * Guarda o XML anexado — privado, no Postgres, único por (usuário, sha256).
 *
 * ⚠⚠ `prismaDirect`, NÃO o pooler: o conteúdo pode ter alguns MB, e statement grande pelo PgBouncer
 * é o caminho do OOM 53200 ("MessageContext") que este Neon já deu — regra do CLAUDE.md para
 * escrita grande.
 * ⚠ Reenviar o mesmo arquivo não duplica: o `upsert` devolve a linha que já existe.
 * ⚠ Falhar aqui NÃO derruba a resposta — mas a falha fica no carimbo (`gravado: false`), para a
 * rastreabilidade saber que aquele anexo não pode ser recuperado. Calar seria pior que falhar.
 */
export async function gravarAnexo({ userId, nome, tamanho, sha256, conteudo, parserVersao, chaveNfe }) {
  try {
    const a = await prismaDirect.fiscalAnexo.upsert({
      where: { userId_sha256: { userId, sha256 } },
      create: { userId, nome, tamanho, sha256, conteudo, parserVersao, chaveNfe: chaveNfe || null },
      update: {},
      select: { id: true },
    });
    return { gravado: true, anexoId: a.id };
  } catch (e) {
    return { gravado: false, erro: String(e?.message ?? e).slice(0, 200) };
  }
}
