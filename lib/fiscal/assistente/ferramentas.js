...
export const ESQUEMAS = [
  ...
  {
    name: "buscar_ncm",
    description: "Procura NCMs na TIPI pela DESCRIÇÃO do produto. Use quando o usuário descrever a peça mas não souber o código. Devolve candidatos — nunca escolhe por ele.",
    input_schema: { type: "object", properties: { termo: { type: "string" } }, required: ["termo"] },
  },
  {
    name: "consultar_cfop",
    description: "Consulta a tabela de CFOPs que a TORG utiliza, por código ou por descrição da operação.",
    input_schema: { type: "object", properties: { codigo: { type: "string" }, termo: { type: "string" } } },
  },
  {
  ...
  },
  {
    ...
    input_schema: { type: "object", properties: { cfop: { type: "string" } }, required: ["cfop"] },
  },
  {
    name: "simular_operacao",
    description: "Simulação fiscal determinística de uma operação de saída: IPI pela TIPI, ICMS de referência, conferência do CFOP contra as UFs e alertas. É o ÚNICO caminho para um número — nunca calcule de cabeça.",
    input_schema: {
      type: "object",
      properties: {
        ncm: { type: "string" }, cfop: { type: "string", description: "Código único ou par '5101/6101'." },
        ufOrigem: { type: "string" }, ufDestino: { type: "string" },
        valor: { type: "number" }, cstPretendido: { type: "string" },
      },
      required: ["ncm", "cfop"],
    },
  },
  {
    ...
    input_schema: { type: "object", properties: { descricao: { type: "string" }, codigoProduto: { type: "string" } }, required: ["descricao"] },
  },
];
...
async function situacaoDoCfop(codigo) {
...
const EXECUTORES = {
  async consultar_ncm({ ncm }) {
    const r = await detalharNcm(ncm);
  ...
  async buscar_ncm({ termo }) {
    const r = await buscarNcm(termo, { limite: 8 });
  ...
  async buscar_legislacao({ consulta }) {
    const r = await buscarLegislacao(consulta);
    if (!r.achados.length) {
      return {
  ...
  async consultar_regra_e_situacao({ cfop }) {
    const s = await situacaoDoCfop(cfop);
    ...
    return {
      ...
      bloco: { tipo: C.BLOCO.REGRA, titulo: "Conferência pela contabilidade", bloqueio: s.bloqueada ? s.motivo : null, fontes: [], linhas: s.regras.map((r) => ({ rotulo: r.titulo, valor: r.situacao, ressalva: r.motivo })) },
    };
  },

  async simular_operacao(entrada) {
    ...
    if (!versao) return vazio("Nenhuma versão da TIPI está ativa — a simulação fica indisponível.");
    const dados = {
    ...
    const validacao = escolhido ? await situacaoDoCfop(escolhido.codigo) : null;
  ...
  async consultar_classificacao({ descricao, codigoProduto }) {
