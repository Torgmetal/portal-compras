...
/**
 * Devolve uma reserva quando ficou COMPROVADO que nenhuma chamada foi feita (achado do Codex,
 * 24/09/2026).
 *
 * ⚠⚠⚠ NÃO É O INVERSO DA REGRA "RESERVA INTERROMPIDA NÃO VOLTA", É O COMPLEMENTO DELA. Execução
 * interrompida não devolve porque o custo real é DESCONHECIDO — pode ter havido consumo. Aqui é o
 * contrário: o caminho sai da rota ANTES de `responder()` existir, então é certo que a Anthropic
 * não foi chamada. Reter esses R$ 0,25 faria um reenvio de rede instável, ou um 404 de conversa
 * apagada, consumir o teto de quem não perguntou nada — e, repetido, esgotar o teto do portal.
 *
 * ⚠ Só pode ser chamada de um ponto que não chamou o modelo. Na dúvida, NÃO devolva.
 */
export async function devolver(userId, { dia, micros }) {
  await prisma.$executeRawUnsafe(
    `UPDATE "FiscalUsoIa"
        SET "custoMicros" = GREATEST(0, "custoMicros" - $3),
            "chamadas" = GREATEST(0, "chamadas" - 1), "atualizadoEm" = NOW()
      WHERE "userId" = ANY($1::text[]) AND "dia" = $2`,
    [userId, GLOBAL], dia, Math.round(micros),
  );
}
