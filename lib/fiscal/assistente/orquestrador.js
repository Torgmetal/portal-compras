...
import { prepararAnexo, mensagemComAnexo } from "@/lib/fiscal/assistente/anexo-contexto";
...
async function referenciasDaExecucao() {
  ...
  const referencias = {
    ...
    // ⚠ O BUILD carimba a versão do CÓDIGO inteiro — instrução, ferramentas, leitor do XML, motor de
    // auditoria. O Codex pediu "versão do motor e do prompt": o número de build cobre os dois sem
    // uma constante manual por módulo que alguém esqueceria de subir.
    build: process.env.NEXT_PUBLIC_BUILD_NUMERO ?? null,
 ...
 */
export async function responder({ historico, pergunta, aoProgredir = () => {}, ateMs = null, anexo = null }) {
  ...
  // ⚠⚠ O ANEXO É PREPARADO COM O CONTEXTO JÁ FIXADO — a auditoria da nota usa a mesma versão da TIPI
  // que as ferramentas vão usar.
  const preparado = anexo ? await prepararAnexo(anexo, contexto) : null;
  if (preparado) {
    aoProgredir({ etapa: "ferramenta", nome: "ler_documento_anexado" });
    referencias.anexo = preparado.registro;
  }

  const mensagens = [
    ...historico.map((m) => ({ role: m.papel === "USUARIO" ? "user" : "assistant", content: m.conteudo })),
    { role: "user", content: preparado ? mensagemComAnexo(pergunta, preparado) : pergunta },
  ];

  // ⚠ Os blocos do documento entram PRIMEIRO: o que a nota declara vem antes do que o portal conclui.
  const blocos = preparado ? [...preparado.blocos] : [];
  const executadas = [];
