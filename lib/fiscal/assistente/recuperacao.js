 ...
 */
export async function buscarLegislacao(consulta, { limite = 6, corpus: fixado = null } = {}) {
  // ⚠⚠ CORPUS FIXADO PELA EXECUÇÃO, quando ela já o resolveu (achado do Codex, 24/09/2026): o TTL
  // de 5 min do cache torna improvável que ele vire no meio de uma resposta, mas "improvável" não
  // é a garantia que o §22 do briefing pede. Recebendo o retrato, a resposta inteira fala da mesma
  // coleta — inclusive se uma importação de norma terminar entre duas ferramentas.
  const corpus = fixado ?? await corpusVigente();
