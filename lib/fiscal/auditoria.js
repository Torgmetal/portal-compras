...
import { conferirClassificacao, achadosDeAusencia } from "@/lib/fiscal/auditoria-classificacao";
import { GRAVIDADE, achado } from "@/lib/fiscal/achado";

export { GRAVIDADE } from "@/lib/fiscal/achado";
...
export function auditar(doc, tipi, referencia = {}) {
  ...
  // ⚠ `undefined` = a conferência não foi pedida; `null` = o cadastro não pôde ser lido. As duas
  // coisas são diferentes, e a segunda não pode virar "não há classificação".
  const { classificacoes: registro, ...semClassificacoes } = referencia;
  const semRegistro = [];
  ...
  for (const it of doc.itens ?? []) {
    ...
    // ── O REGISTRO DE CLASSIFICAÇÃO, ANTES DE QUALQUER COISA ────────────────
    //
    // ⚠⚠ ELE NÃO DEPENDE DA TIPI NEM DO CST (achado do Codex, 23/09/2026). Se ficasse depois do
    // `continue` abaixo, o item com NCM fora da TIPI — que é exatamente o item cuja classificação
    // está mais em dúvida — nunca seria comparado com a decisão que alguém registrou.
    conferirClassificacao({ it, onde, registro, achados, semRegistro });
  ...
  achados.push(...achadosDeAusencia(semRegistro));
  ...
  return {
    ...
    referencia: {
      // ⚠ O registro de classificação NÃO volta na resposta. Ele entra por `referencia` como
      // insumo do motor; devolvê-lo mandaria o cadastro inteiro no JSON de cada auditoria, e a
      // evidência que interessa já viaja dentro de cada achado.
      ...semClassificacoes,
