...
import { vigenciaDo } from "@/lib/fiscal/vigencia-tipi";
...
export async function referenciaAtiva() {
  ...
  // ⚠⚠ A VIGÊNCIA VEM DA DECLARAÇÃO AMARRADA AO sha256 DO ARTEFATO (ver lib/fiscal/vigencia-tipi.js).
  // Mudou o arquivo na origem, muda o hash, a declaração para de valer e a ressalva VOLTA — em vez
  // de a vigência de hoje se arrastar para uma tabela que ninguém conferiu.
  const dec = tipi ? vigenciaDo(tipi.arquivo.sha256) : null;
  return {
    tipi: tipi && {
      ...
      vigenciaInicio: tipi.vigenciaInicio ?? dec?.vigenciaInicio ?? null,
      vigenciaFundamento: tipi.vigenciaFundamento ?? dec?.norma ?? null,
      vigenciaDeclarada: Boolean(tipi.vigenciaInicio || dec),
      // ⚠ A PROCEDÊNCIA VIAJA JUNTO: quem declarou, quando, contra o quê — e o que a declaração
      // NÃO prova, que é o que impede a ressalva de virar carimbo.
      vigencia: dec && {
        norma: dec.norma, atualizadaAte: dec.atualizadaAte, atos: dec.atos,
        declaradoPor: dec.declaradoPor, declaradoEm: dec.declaradoEm,
        conferidoContra: dec.conferidoContra, naoProva: dec.naoProva,
      },
