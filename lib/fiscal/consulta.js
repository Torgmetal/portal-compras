...
import { atualizacaoDe } from "@/lib/fiscal/vigencia-tipi";
...
export async function referenciaAtiva() {
  ...
  // ⚠⚠ ATUALIZAÇÃO ≠ VIGÊNCIA, e confundir as duas foi um erro meu (ver lib/fiscal/vigencia-tipi.js).
  // Esta declaração diz até qual ato a tabela se identifica como atualizada; ela NÃO comprova qual
  // redação vigorava numa data passada, e por isso não mexe em `vigenciaDeclarada`.
  const atual = tipi ? atualizacaoDe(tipi.arquivo.sha256) : null;
  return {
    tipi: tipi && {
      ...
      vigenciaInicio: tipi.vigenciaInicio,
      vigenciaFundamento: tipi.vigenciaFundamento,
      // ⚠⚠ CONTINUA VINDO SÓ DO BANCO — que é o mesmo campo que a auditoria e o simulador leem.
      // Preenchê-lo pela declaração faria a Consulta dizer "vigência declarada" enquanto os outros
      // dois mantinham a ressalva: o módulo discordando de si sobre o próprio fundamento.
      vigenciaDeclarada: Boolean(tipi.vigenciaInicio),
      // ⚠ A PROCEDÊNCIA VIAJA JUNTO: quem declarou, quando, contra o quê — e o que a declaração
      // NÃO prova, que é o que impede ela de ser lida como vigência.
      atualizacao: atual && {
        norma: atual.norma, atualizadaAte: atual.atualizadaAte, atos: atual.atos,
        declaradoPor: atual.declaradoPor, declaradoEm: atual.declaradoEm,
        conferidoContra: atual.conferidoContra, naoProva: atual.naoProva,
      },
...
/**
 * ⚠⚠ `referencia` OPCIONAL: quem já resolveu as versões passa a sua, e a consulta NÃO reconsulta
 * (achado do Codex, 24/09/2026). O assistente fiscal resolve a referência uma vez por execução e
 * faz várias consultas em sequência; sem isto, uma sincronização da TIPI terminando no meio de uma
 * resposta faria duas ferramentas da MESMA resposta lerem versões diferentes — e a rastreabilidade
 * gravada apontaria para uma terceira. ⚠ Sem o parâmetro, nada muda para os chamadores antigos.
 */
export async function buscarNcm(termo, { limite = 25, referencia = null } = {}) {
  const t = String(termo ?? "").trim();
  if (t.length < 2) return { termo: t, resultados: [], motivo: "Digite ao menos 2 caracteres." };

  const ref = referencia ?? await referenciaAtiva();
 ...
 */
/** ⚠ `referencia` opcional pelo mesmo motivo de `buscarNcm` — ver o aviso lá. */
export async function detalharNcm(codigo, { referencia = null } = {}) {
