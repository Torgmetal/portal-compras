...
async function baixar(url, aceita) {
  ...
  if (!resp.ok) {
    // ⚠⚠ O SISCOMEX TEM TETO DE 3 ACESSOS POR HORA (código PUCX-ER1001, documentado por eles). É
    // limite conhecido, não falha — quem chama precisa distinguir para não alarmar à toa e para não
    // descartar a referência que já está ativa.
    const corpo = await resp.text().catch(() => "");
    if (resp.status === 422 && /limite de \d+ acessos/i.test(corpo)) {
      return { limite: true, erro: `Teto de acessos da fonte atingido (${resp.status}). A referência ativa continua valendo; a próxima verificação tenta de novo.` };
    }
    return { erro: `HTTP ${resp.status} em ${url}` };
  }
...
export async function baixarNcm() {
  ...
  if (r.erro) return r;   // ⚠ devolve inteiro para preservar o sinalizador `limite`
  let dados;
