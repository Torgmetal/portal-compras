...
export function icmsDeReferencia(ufOrigem, ufDestino, cfop = null) {
  ...
  if (cfop && FAMILIAS_SEM_REFERENCIA.has(cfop.familia)) {
    return { estado: ESTADO.NAO_DETERMINADO,
      motivo: `O CFOP ${cfop.codigoFormatado} é de ${cfop.familia.toLowerCase()}: a operação movimenta mercadoria e costuma ter tratamento próprio — a remessa para industrialização, por exemplo, corre com suspensão do ICMS (art. 402 do RICMS/SP), que tem condições e prazo. A alíquota interestadual não se aplica sem antes confirmar o tratamento.`,
      ...comum };
  }
