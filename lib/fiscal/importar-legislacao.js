 ...
 */
export async function baixarNorma(fonte, { timeoutMs = 30000 } = {}) {
  let resp;
  try {
    resp = await fetch(fonte.url, { headers: { "user-agent": AGENTE }, signal: AbortSignal.timeout(timeoutMs) });
  } catch (e) {
...
export async function importarNorma(chave) {
  ...
  // ⚠ Mesmo hash é "sem mudança", e isso PRECISA ser dito: silêncio aqui é indistinguível de falha.
  if (jaTem) return { chave, status: "SEM_MUDANCA", sha256: baixada.sha256, versaoId: jaTem.id, conferido: baixada.conferido };
 ...
 */
export async function importarLegislacao({ chaves = null, pausaMs = 800 } = {}) {
  const alvos = chaves?.length ? FONTES.filter((f) => chaves.includes(f.chave)) : FONTES;
  const resultados = [];
  for (const f of alvos) {
    resultados.push(await importarNorma(f.chave));
