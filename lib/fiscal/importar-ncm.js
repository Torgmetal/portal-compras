...
export function dataBr(v) {
  ...
  if (!m) return null;
  const [, d, mes, ano] = m;
  // ⚠ "31/12/9999" é o jeito do Siscomex dizer "sem fim" — vira null, não uma data no ano 9999.
  if (ano === "9999") return null;
  const dt = new Date(Date.UTC(Number(ano), Number(mes) - 1, Number(d)));
  // ⚠⚠ DATA INVÁLIDA NÃO VIRA `NaN` NO JAVASCRIPT — ELA ROLA. `Date.UTC(2022, 12, 32)` devolve
  // 01/02/2023, um dia perfeitamente válido que NINGUÉM escreveu. Numa coluna de vigência isso é
  // uma data errada entrando calada, e "vigente desde" é exatamente o campo onde nada pode ser
  // inventado. A conferência de volta é o único jeito de pegar: se o dia, o mês e o ano não
  // sobreviverem à ida e volta, o texto não era uma data.
  const voltou = dt.getUTCDate() === Number(d) && dt.getUTCMonth() === Number(mes) - 1 && dt.getUTCFullYear() === Number(ano);
  return Number.isNaN(dt.getTime()) || !voltou ? null : dt;
}
