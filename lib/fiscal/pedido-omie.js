...
/**
 * ⚠⚠ AUSENTE NÃO É ZERO (achado do Codex, 23/09/2026). `Number(null)`, `Number("")` e
 * `Number(" ")` devolvem **0**, e eu deixava esse zero passar como valor declarado. Com CST 50 e
 * `aliq_ipi` ausente, o item virava "0% declarado" em vez de NAO_AVALIAVEL — e contra uma TIPI que
 * também diz 0% a comparação passava **em silêncio**, que é exatamente a conformidade por falta de
 * dado que a regra 2 do motor existe para proibir.
 */
const num = (v) => {
...
export function lerPedidoOmie(payload, contexto = {}) {
  ...
  const itens = p.det.map((d, i) => {
    ...
    return {
      ...
      // ⚠ O `??` só cai para o total quando a mercadoria é AUSENTE — com o `num` antigo, um
      // `valor_mercadoria` nulo virava 0 e o fallback nunca acontecia.
