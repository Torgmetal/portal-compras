...
import { normalizar, canonico, STATUS } from "@/lib/fiscal/classificacao-produto";
...
const CAMPOS = {
  id: true, codigoProduto: true, codigoNormalizado: true, padraoDescricao: true, padraoNormalizado: true, ncm: true,
...
/**
 * O ESCOPO DE UMA LINHA — o código como foi digitado, e a forma canônica que manda.
 *
 * ⚠⚠ CÓDIGO QUE NORMALIZA PARA VAZIO É RECUSADO (achado do Codex, 23/09/2026). `"---"` sobrevive ao
 * `trim` e seria gravado como específico, mas some na leitura e passa a valer para QUALQUER
 * produto — o oposto de quem digitou. Vazio só pode significar uma coisa: global, e por ausência.
 */
function escopoDoCodigo(bruto) {
  const digitado = bruto?.trim() || null;
  if (!digitado) return { codigoProduto: null, codigoNormalizado: null };
  const canon = canonico(digitado);
  if (!canon) throw new Error("O código do produto não tem letra nem número — deixe vazio para valer para qualquer código.");
  return { codigoProduto: digitado, codigoNormalizado: canon };
}
...
export async function propor(dados, user) {
  ...
  const escopo = escopoDoCodigo(dados.codigoProduto);
