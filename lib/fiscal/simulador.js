...
function fichaDeEmissao({ escolha, ipi, pisCofins, icms }) {
  ...
  const linhas = [
    ...
    // ⚠⚠ O PORTAL NÃO DETERMINA CST DE ICMS. Ele depende do tratamento (tributado, isento,
    // diferido, suspenso, ST), e nenhum deles está estruturado aqui.
    { campo: "CST ICMS", valor: null, descricao: null,
      motivo: icms?.estado === "REFERENCIA"
        ? `A alíquota de referência é ${icms.aliquota}%, mas o CST depende do tratamento da operação — a definir com a contabilidade.`
        : (icms?.motivo ?? "A definir com a contabilidade.") },
