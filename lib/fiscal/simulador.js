...
export function simular(entrada, daTipi, referencia = {}) {
  ...
  // ⚠ A situação das regras que ESTE resultado usa — calculada fora (o motor é puro) e passada
  // pronta. Ausente = nenhuma verificação pedida, e aí nada é bloqueado.
  const validacao = referencia.validacao ?? null;
  const alertas = [];
  const perguntas = [];
  ...
  // ⚠⚠ O ALERTA DE REGRA BLOQUEADA VEM NO TOPO, junto dos outros: é ele que explica a ficha vazia.
  if (validacao?.bloqueada) alertas.push({ nivel: "alto", texto: validacao.motivo });
  ...
  return {
    ...
    // esses campos.
    // ⚠⚠ REGRA CONTESTADA NÃO ENTREGA FICHA (parecer do Codex, 23/09/2026). Tarja isolada não
    // serve: a ficha é justamente o bloco que a pessoa COPIA para o Omie, e ela seria copiada com
    // a tarja para trás. Se a contabilidade disse que o verbete está errado — ou se o portal não
    // conseguiu verificar se disse — a ficha não sai, e o motivo aparece no lugar dela.
    // ⚠ Bloqueia o que DEPENDE da regra; a consulta ao conteúdo e às evidências continua.
    ficha: validacao?.bloqueada ? null : fichaDeEmissao({ escolha, ipi, pisCofins, icms }),
