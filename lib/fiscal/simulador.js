...
import { estimarPisCofins, REGIME } from "@/lib/fiscal/regime";
...
export const NAO_DETERMINADOS = [
  { tributo: "ICMS", motivo: "Depende da UF de destino, de benefícios do estado, de redução de base e de o destinatário ser ou não contribuinte. A alíquota interestadual varia conforme a origem da mercadoria (nacional ou importada) e o destino — não existe um valor único aplicável a toda venda." },
 ...
 *
 * ⚠ A chave da deduplicação é CFOP + papel: a mesma nota citada em duas operações é uma nota só; a
 * 5.949 como "complemento" e como "remessa ao cliente final" são notas diferentes.
 */
...
export function simular(entrada, daTipi, referencia = {}) {
  ...
  return {
    ...
    pisCofins: estimarPisCofins(valor),
