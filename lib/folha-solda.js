...
const un1 = (x) => Math.max(1, Number(x?.qte) || 1);
const kg0 = (x) => Number(x?.pesoTotalKg) || 0;
  ...
  for (const [iB, g] of usaveis.entries()) {
    ...
    for (const it of itens) {
      adicionarLinhaTabela(ws, row, varias
        ? [fmtDiaBR(it._dia), it.opNumero || it.op?.numero || "", it.marca, it.descricao || "", un1(it), Math.round(kg0(it)), ""]
        : [fmtDiaBR(it._dia), it.marca, it.descricao || "", un1(it), Math.round(kg0(it)), ""],
