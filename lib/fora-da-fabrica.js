...
export async function pecasNoTerceiro() {
  const remessas = await prisma.romaneioTerceiro.findMany({
  ...
  const candidatos = await prisma.pecaConjunto.findMany({
  ...
  if (croquiIds.length) {
    const usos = await prisma.conjuntoCroqui.findMany({
