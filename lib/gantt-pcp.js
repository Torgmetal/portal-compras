...
export async function lotesProgramados() {
  ...
  for (const [setor, itens] of Object.entries(filas)) {
    ...
    for (const p of itens) {
      ...
      if (!p.naBancada && !libIds.has(p.id)) continue;
...
export async function aplicarRemanejo(blocos, user) {
  ...
  return prisma.$transaction(banco => gravarRemanejo(destinosFinais(blocos), user, banco), { timeout: 45000 });
}
...
async function gravarRemanejo(blocos, user, banco) {
  ...
  const camposLidos = Object.fromEntries(Object.values(CAMPO).flatMap(c => [c.dia, c.original]).filter(Boolean).map(c => [c, true]));
  const existentes = await banco.pecaConjunto.findMany({ where: { id: { in: ids } }, select: { id: true, ...camposLidos } });
