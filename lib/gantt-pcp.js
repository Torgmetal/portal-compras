...
export async function lotesProgramados() {
  ...
  const foraDaFabrica = chavesNoTerceiro(remessas);
  ...
  const juntar = (setor, recurso, dia, p, custo, adiado, veioDe, over, fila, familia) => {
    ...
    /* ⚠⚠ E A REMESSA VALE SOZINHA. Ver chavesNoTerceiro: a marcação da peça e o romaneio são duas
       portas, e a RT-03 (galvanização da OP-097) provou que dá para emitir o romaneio sem marcar
       nada — 48 peças que já estavam no terceiro seguiam ocupando bancada em três setores. */
    if(noTerceiro(foraDaFabrica, p.op?.numero || p.opNumero, p.marca))return;
