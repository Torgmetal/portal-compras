...
import { conferirRemanejoSaldo } from "./remanejo-saldo";
...
const SELECAO = {
  ...
  pesoTotalKg: true, perfil: true, material: true, tipoPeca: true, _count: {select: {croquiConjuntos:true}}, op: { select: { numero: true, obra: true } },
};
...
export async function lotesProgramados() {
  ...
  const juntar = (setor, recurso, dia, p, custo, adiado, veioDe, over, fila, familia) => {
    if (setor !== "CORTE" && pecaEhCroqui(p)) return;
 ...
 */
export async function aplicarRemanejo(blocos, user, { somenteSaldo = false } = {}) {
  ...
  try {
    return await prisma.$transaction(banco => gravarRemanejo(blocos.some(b => b.fracoes) ? blocos : destinosFinais(blocos), user, banco, somenteSaldo), { timeout: 45000, isolationLevel: "Serializable" });
  } catch (e) {
...
async function gravarRemanejo(blocos, user, banco, somenteSaldo = false) {
  ...
  const existentes = await banco.pecaConjunto.findMany({ where: { id: { in: ids }, ...SO_FABRICACAO, ...OP_VIVA }, select: { id: true, qte: true, tipoPeca: true, ...(somenteSaldo ? {opId:true,marca:true,destino:true} : {}), _count: {select: {croquiConjuntos:true}}, terceirizado: true, terceirizadoRecebidoEm: true, ...camposLidos } });
  ...
  const produzido = somenteSaldo ? await lerProduzidoPorSetor(existentes, undefined, { banco }) : null;
  ...
  for (const b of blocos) {
    ...
    for (const id of new Set(b.ids)) {
      ...
      if (b.setor !== "CORTE" && pecaEhCroqui(p)) throw new Error("Croqui/componente não pode ser programado após o Corte. Atualize o quadro.");
      const anterior = planos.get(k);
      let partes = anterior?.partes || partesAtuais(p, campos, originais.get(k));
      if(somenteSaldo) {
        if(['REVISAO','CANCELADA','AGUARDANDO_MATERIAL','TERCEIRO'].includes(p.destino)) throw new Error('A peça está bloqueada. Confira a pendência antes de remanejar.');
        if(!b.fracoes) throw new Error('Selecione as quantidades pendentes.');
        conferirRemanejoSaldo(partes,produzido(p,b.setor),b.fracoes.filter(f=>f.id===id),{dia:b.dia,recurso:b.recurso},Math.max(1,p.qte||1));
      }
