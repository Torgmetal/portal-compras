...
async function gravarRemanejo(blocos, user, banco) {
  ...
  const existentes = await banco.pecaConjunto.findMany({ where: { id: { in: ids }, ...SO_FABRICACAO, ...OP_VIVA }, select: { id: true, qte: true, tipoPeca: true, _count: {select: {croquiConjuntos:true}}, terceirizado: true, terceirizadoRecebidoEm: true, ...camposLidos } });
