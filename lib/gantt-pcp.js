...
const SELECAO = {
  id: true, opNumero: true, marca: true, qte: true, qteProduzida: true,
...
export async function lotesProgramados() {
  ...
  const juntar = (setor, recurso, dia, p, custo, adiado) => {
    ...
    const feito = Math.min(p.qteProduzida || 0, qte);
  ...
  const pendente = (p) => ({ ...p, qtePendente: Math.max(0, (p.qte || 1) - (p.qteProduzida || 0)) });
  ...
  for (const p of montagem) juntar("MONTAGEM", p.montagemBancada, iso(p.montagemDiaProgramado), p,
    custoMontagem(pendente(p), RITMO_NORMAL), p.montagemAdiado);
  for (const p of solda) juntar("SOLDA", p.soldaBancada, iso(p.soldaDiaProgramado), p,
    custoSolda(pendente(p), RITMO_CONSERVADOR), 0);
