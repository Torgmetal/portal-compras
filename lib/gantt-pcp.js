...
export async function lotesProgramados() {
  ...
  const juntar = (setor, recurso, dia, p, custo, adiado, veioDe, over, fila) => {
  ...
  for (const [setor, itens] of Object.entries(filas)) {
    ...
    for (const p of itens) juntar(setor, p.maquina || null, hoje, { ...p, op: { numero: p.opNumero, obra: p.obra } },
      0, 0, p.dia ? iso(p.dia) : undefined, undefined, true);
  }
