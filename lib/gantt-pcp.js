...
export async function lotesProgramados() {
  ...
  const juntar = (setor, recurso, dia, p, custo, adiado, veioDe, over) => {
  ...
  for (const [setor, itens] of Object.entries(filas)) {
    // ⚠ o corte traz o dia DA IMPORTAÇÃO da peça (pedido do Vitor em 08/09/2026); os demais setores
    //   não trazem `dia` e continuam ancorados em hoje.
    for (const p of itens) juntar(setor, null, p.dia ? iso(p.dia) : hoje, { ...p, op: { numero: p.opNumero, obra: p.obra } }, 0, 0);
  }
