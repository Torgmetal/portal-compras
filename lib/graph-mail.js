...
/**
 * Corpo COMPLETO de uma mensagem, sob demanda (pra ler dentro do portal). Não guardamos
 * o corpo no banco (só snippet) — buscamos aqui na hora que o diretor abre o e-mail.
 * @returns {{ assunto, de, deNome, para:string[], cc:string[], em, contentType:"html"|"text", corpo, webLink }}
 */
export async function lerCorpoMensagem(caixa, mensagemId) {
  const token = await getAccessToken();
  const sel = "subject,from,toRecipients,ccRecipients,receivedDateTime,sentDateTime,body,webLink";
  const url = `${GRAPH}/users/${encodeURIComponent(caixa)}/messages/${mensagemId}?$select=${sel}`;
  const m = await graphGet(url, token);
  const from = extrairEndereco(m.from);
  return {
    assunto: m.subject || "",
    de: from.endereco, deNome: from.nome,
    para: (m.toRecipients || []).map((r) => extrairEndereco(r).endereco).filter(Boolean),
    cc: (m.ccRecipients || []).map((r) => extrairEndereco(r).endereco).filter(Boolean),
    em: m.receivedDateTime || m.sentDateTime || null,
