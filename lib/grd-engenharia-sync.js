...
import { DESTINO_ENGENHARIA } from "./grd-roteiro";
...
export async function sincronizarGrds({ limite = 60 } = {}) {
  ...
  for (const a of pendentes) {
    try {
      ...
      // ─── REMESSA REGISTRADA, DAQUI PARA FRENTE ────────────────────────────────────────────
      // Vitor (31/08/2026): "vamos fazer as próximas assinadas; as antigas já foram, agora vamos
      // fazer isso nas novas".
      //
      // O campo "Recebido:" do FORM 09 está em branco nas 12 GRDs mais recentes que abri — a
      // Engenharia nunca preenche. Então hoje não existe prova de recebimento para mostrar a um
      // auditor. A partir daqui o portal registra a remessa conforme o roteiro definido
      // (Engenharia → Gabriel), com a data em que a GRD entrou.
      //
      // ⚠ SÓ NO REGISTRO NOVO (`create`), nunca no `update`: as 480 já importadas ficam sem, porque
      // foram entregues antes de existir registro — preencher retroativamente inventaria uma
      // remessa que o portal não presenciou. E reprocessar um arquivo alterado não pode reescrever
      // a data da remessa original.
      const remessa = {
        remetidoParaNome: DESTINO_ENGENHARIA.nome,
        remetidoParaEmail: DESTINO_ENGENHARIA.email,
        remetidoEm: new Date(),
      };
      const salva = await prisma.grdEngenharia.upsert({
        where: { itemId: a.id }, create: { ...dados, ...remessa }, update: dados,
        select: { id: true, numero: true, revisao: true, opNumero: true, qtdDocs: true, referencia: true, avisadoEm: true, remetidoParaNome: true, remetidoEm: true },
      });
