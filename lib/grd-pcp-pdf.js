...
export async function gerarGuiaPcpPDF(g) {
  ...
  const assinadas = (g.assinaturas || []).filter((a2) => a2.assinadoEm);
  if (assinadas.length) {
    for (const a2 of assinadas) {
      espaco(30);
      txt(a2.nome || "—", M, y - 9, { f: bold, size: 9 });
      const quando = `Confirmado em ${dataHoraBR(a2.assinadoEm)}${a2.ip ? ` · IP ${a2.ip}` : ""}`;
      txt(quando, M, y - 20, { size: 7.5, color: GRAY });
      if (a2.setor) txt(a2.setor, A4[0] - M - wid(a2.setor, font, 8), y - 9, { size: 8, color: GRAY });
      y -= 32;
    }
    txt("Assinatura eletronica registrada pelo portal.", M, y - 4, { size: 7, color: GRAY });
  } else {
    y -= 30;
    const meia = (W - 20) / 2;
