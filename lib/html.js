...
/**
 * Converte um texto livre (com quebras de linha) em HTML seguro pra E-MAIL,
 * preservando parágrafos/linhas: escapa + troca `\n` por `<br>`. Use isto no
 * lugar de `white-space:pre-wrap` — o Outlook (engine do Word) ignora esse CSS
 * e junta tudo numa linha só; `<br>` é respeitado por todos os clientes.
 * @param {unknown} s
 * @returns {string} HTML seguro (já escapado)
 */
export function textoParaHtml(s) {
