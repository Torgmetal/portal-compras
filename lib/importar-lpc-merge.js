...
export async function importarLpcMerge(parsed, { userId = null, revisao = null, arquivo = null } = {}) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const marca of diff.atualizar) {
      ...
      await tx.pecaConjunto.updateMany({
        ...
        data: {
          ...
          naLPC: true, // quem continua na revisão continua na lista
        },
      });
    }
    ...
    const novosRegistros = diff.adicionar.map((marca) => {
      const d = novas.get(marca);
      return {
        // ⚠⚠ `naLPC: true` É O PERTENCIMENTO À LISTA. Sem ele a peça é "LPC_IMPORT" mas ninguém a acha: o
        // portal do cliente (pecasDaObra: naLPC ou naLE) respondia "esta marca não está nas listas" para
        // as 197 peças da OP-113, e o cliente cobrou "não consta a rastreabilidade nem os certificados"
        // (Alexandre, 14/09/2026). O importador comum sempre gravou a flag; este, da revisão, não.
        id: randomUUID(), opId: op?.id || null, opNumero, marca, status: "PENDENTE", fonte: "LPC_IMPORT", naLPC: true,
