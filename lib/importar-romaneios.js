...
//
// ⚠⚠ ROMANEIO EMITIDO NÃO É ROMANEIO EMBARCADO. O FORM 22 pode ser emitido ANTES de a peça
// existir — foi o caso do romaneio 02 da OP-104 (Vitor 19/08: "o romaneio 2 foi emitido porém
// não foi fabricado ainda ... precisa voltar as peças para op"). Nada no arquivo distingue um
// do outro (a "data de saída" já vem preenchida), então **quem importa ESCOLHE** quais romaneios
// realmente saíram: `somente: [numeros]`. Sem escolha explícita, nenhum é gravado.
//
...
// ⚠⚠ O SEPARADOR DECIMAL É O ÚLTIMO QUE APARECE — nas duas convenções.
//
// A regra anterior era "só trata o ponto como milhar quando há vírgula", e isso quebra em cima do
// formato que o próprio SheetJS produz: com `raw:false` ele devolve o número já formatado pelo SSF
// em convenção AMERICANA, tipo "3,807.26". A vírgula existe, então o ponto era apagado como se
// fosse milhar: "3,807.26" → "3,80726" → **3,80726**. Peso dividido por mil.
//
// Medido em 23/08: **707 dos 5.466 `RomaneioItem` têm peso com mais de 2 casas decimais** — a
// impressão digital do bug. Prova cruzada: a mesma peça aparece com 46,43 kg nos registros limpos
// e 0,04643 nos corrompidos; T60A67 dá 182,45 contra 0,18245. Só pega linha de 1.000 kg para cima,
// que é justamente onde o erro dói.
//
// A regra nova não precisa saber a convenção: o separador mais à DIREITA é o decimal, todos os
// outros são milhar. "1.234,56" e "1,234.56" dão 1234,56; "3807" dá 3807; "1.234" dá 1234
// (grupo de milhar) e "1.2345" dá 1,2345 (decimal, porque não são 3 dígitos).
function numero(v) {
  const s = String(v ?? "").replace(/[^\d.,-]/g, "").trim();
  if (!s) return 0;
  const iPonto = s.lastIndexOf(".");
  const iVirg = s.lastIndexOf(",");
  let limpo;
  if (iPonto < 0 && iVirg < 0) {
    limpo = s;
  } else {
    const iDec = Math.max(iPonto, iVirg);
    const casas = s.length - iDec - 1;
    // separador único com exatamente 3 casas é milhar ("1.234" = mil duzentos e trinta e quatro),
    // a não ser que haja mais de um separador — aí o último é decimal de verdade.
    const soUm = (iPonto < 0) !== (iVirg < 0) && s.split(/[.,]/).length === 2;
    if (soUm && casas === 3) limpo = s.replace(/[.,]/g, "");
    else limpo = s.slice(0, iDec).replace(/[.,]/g, "") + "." + s.slice(iDec + 1);
  }
 ...
 */
export async function importarRomaneiosDaOp(opNumero, { gravar = false, user = null, forcar = false, somente = null } = {}) {
  ...
  // ── GRAVA ────────────────────────────────────────────────────────────────────────────────
  // Só os romaneios ESCOLHIDOS (os que de fato embarcaram). Sem lista, não grava nada — é
  // proposital: o padrão seguro é não marcar peça como expedida por engano.
  const escolhidos = Array.isArray(somente) ? somente.map(String) : [];
  const aGravar = romaneios.filter((r) => escolhidos.includes(String(r.numero)));
  if (!aGravar.length) throw new Error("Escolha quais romaneios realmente embarcaram — romaneio emitido não é romaneio embarcado.");

  let criados = 0, atualizados = 0, itensGravados = 0;
  for (const r of aGravar) {
  ...
  // Marca a peça casada E suas gêmeas (mesma chave frouxa em outra lista) — SÓ dos escolhidos.
  const marcasEmbarcadas = [...new Set(aGravar.flatMap((r) => r.itens.map((i) => i.marca)))];
  const idsExpedidos = [...new Set(marcasEmbarcadas.flatMap((m) => idsPorFrouxa.get(chaveFrouxa(m)) || []))];
  ...
  return { op, pasta, arquivos: arquivos.length, lidos: romaneios.length, semTabela,
    ...
    marcas: marcasVistas.size, pesoKg: Math.round(pesoKg), casadas: casadas.length, semCasar,
    gravado: { romaneiosCriados: criados, romaneiosAtualizados: atualizados, itens: itensGravados, pecasExpedidas: expedidas, escolhidos: aGravar.map((r) => r.numero) } };
}
