...
export async function importarRomaneiosDaOp(opNumero, { gravar = false, user = null, forcar = false } = {}) {
  ...
  for (const p of pecas.filter((x) => !ehItemComprado(x))) {
  ...
  const casadas = [...marcasVistas.keys()].filter((k) => porMarca.has(k));
  const semCasar = [...marcasVistas.values()].filter((v) => !porMarca.has(norm(v.marca))).map((v) => v.marca);
  ...
  for (const r of romaneios) {
    ...
    await prisma.romaneioItem.createMany({
      data: r.itens.map((it) => ({
        ...
        pecaConjuntoId: porMarca.get(norm(it.marca))?.id || null,
  ...
  const idsExpedidos = casadas.map((k) => porMarca.get(k).id);
