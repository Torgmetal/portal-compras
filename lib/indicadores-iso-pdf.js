...
export async function gerarIndicadoresIsoPDF({ titulo = "Indicadores (ISO)", ano, indicadores = [], mesFim = 11 }) {
  ...
  for (const ind of indicadores) {
    ...
    if (ind.periodicidade === "SEMESTRAL") {
      // ⚠ INDICADOR APURADO POR SEMESTRE: uma célula larga por semestre, não o mesmo número
      // repetido em seis colunas — seis células iguais dão a entender seis medições, e a
      // periodicidade é justamente o que esse indicador precisa deixar claro.
      for (let sem = 0; sem < 2; sem++) {
        const mIni = sem * 6, mUlt = Math.min(mIni + 5, mesFim);
        const v = mUlt < mIni ? null : (ind.serie?.[mIni] ?? null);
        if (v == null) continue;
        const cx = xMes0 + mIni * wMes, larg = (mUlt - mIni + 1) * wMes;
        const cor = FAROL_COR[farol(v, ind.meta)];
        page.drawRectangle({ x: cx + 1.5, y: y - rowH + 6, width: larg - 3, height: rowH - 12, color: hex(cor.bg) });
        center(`${sem + 1}º semestre`, cx, larg, y - 13, { size: 6.5, color: hex(cor.fg) });
        center(fmtCell(v, un), cx, larg, y - 23, { size: 9, f: bold, color: hex(cor.fg) });
      }
    } else {
      for (let m = 0; m < 12; m++) {
        ...
        const f = v == null ? null : farol(v, ind.meta, "MES");
