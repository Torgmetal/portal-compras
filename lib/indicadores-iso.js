...
export const INDICADORES_ISO = [
  ...
  { id: "aderencia_prazo_projeto", processo: "ENGENHARIA", nome: "Aderência ao Prazo de Entrega do Projeto",
    oQueMede: "% de projetos entregues na data acordada (cronograma)",
    // ⚠⚠ A DESCRIÇÃO PRECISA BATER COM A CONTA. Ela dizia "÷ concluídas" e o cálculo já somava as
    // tarefas vencidas em aberto no denominador (Vitor, 28/08/2026: "as que estiverem em atraso você
    // deve apontar"). Quem lia a fórmula na tela e via o número não fechava a conta — e é o tipo de
    // divergência que faz um indicador inteiro perder a credibilidade.
    comoSeMede: "(Tarefas de engenharia concluídas dentro da data prevista ÷ (concluídas no mês + vencidas e ainda em aberto)) × 100. Linhas de agrupamento do cronograma não contam.",
    meta: { dir: "min", valor: 92, unidade: "%" }, freq: "Mensal", fonte: "auto", nota: "Pela DATA REAL de conclusão do cronograma. A tarefa vencida e ainda em aberto entra como atraso no mês do prazo — sem isso o índice mediria a pontualidade de quem baixou a tarefa, não a saúde do cronograma. Linhas de agrupamento (summary) ficam de fora: elas resumem as filhas e contariam a mesma entrega duas vezes." },
  ...
  { id: "retrabalho_engenharia", processo: "ENGENHARIA", nome: "Retrabalho gerado pela Engenharia",
    oQueMede: "% do peso retrabalhado por erro de projeto sobre a produção do mês",
    comoSeMede: "(Peso dos apontamentos de retrabalho com origem Engenharia ÷ peso produzido/cortado no mês) × 100. Conta o apontamento do FORM 34 tenha ele virado RNC ou não — a mesma base do retrabalho da Produção.",
    // ⚠⚠ ENTRA POR APONTAMENTO, COM OU SEM RNC. Vitor (29/08/2026): "para esses casos marcar como
    // apontamento e deixar descrito no indicador". A regra anterior (28/08) exigia RNC aberta, e o
    // índice ficava em 0% com 1.980 kg registrados — dois desenhos sem uma chapa, 1,8 t em 14/08.
    // "0% verde" se lê como "a Engenharia não gera retrabalho". Pior: o indicador da PRODUÇÃO
    // sempre contou todo apontamento, então os dois números não eram comparáveis. Agora usam a
    // mesma base, e o detalhamento mostra em cada linha se é RNC ou apontamento.
    meta: { dir: "max", valor: 2, unidade: "%" }, freq: "Mensal", fonte: "parcial", nota: "Peso dos apontamentos de retrabalho (FORM 34) com origem Engenharia ÷ peso cortado no mês — com RNC aberta ou sem, igual ao indicador da Produção. O detalhamento traz cada registro com data, OP, marca, peso, descrição e a origem (RNC ou apontamento), e conta quantos ainda não foram formalizados em RNC. Registro sem peça escolhida não tem peso e não entra no percentual." },
  { id: "erros_projeto", processo: "ENGENHARIA", nome: "Erros de Projeto (RNC)",
    oQueMede: "RNCs de projeto detectadas na fábrica/obra", comoSeMede: "Nº de RNCs com área de Engenharia/Projeto no ano",
    // ⚠ META 8 NO ANO (Vitor, 28/08/2026: "para a meta de RNC de projeto deixar como 8 meta"). É um
    // teto anual, não mensal: o acumulado deste indicador é a SOMA do ano, e é contra ele que a meta
    // vale. Antes era 0, que só existia no papel — meta que ninguém atinge não pauta reunião.
    meta: { dir: "max", valor: 8, unidade: "RNCs", periodo: "ANO" }, freq: "Anual", fonte: "parcial", nota: "Contagem das RNCs de Engenharia/Projeto. A meta (≤8) é do ANO, medida no acumulado; o valor de cada mês é a contagem daquele mês. Para virar o % da ISO (≤1%) falta definir o denominador (o que conta como “projeto liberado”)." },
  ...
  { id: "rnc_cliente", processo: "QUALIDADE", nome: "Índice de RNCs Recebidas do Cliente",
    ...
    // ⚠⚠ FALTAVA O `periodo: "ANO"`. A descrição sempre disse que a meta (≤8) é do ANO, mas sem
    // este campo o `farol()` julgava CADA MÊS contra 8 — e um mês com 3 RNCs de cliente pintava
    // "na meta" mesmo com o ano já estourado. É o mesmo ajuste feito em `erros_projeto` em 28/08,
    // que não foi replicado aqui.
    meta: { dir: "max", valor: 8, unidade: "RNCs", periodo: "ANO" }, freq: "Anual", fonte: "auto" },
 ...
 * @param {{dir,valor,unidade,periodo?}} meta
 * @param {"MES"|"ANO"} contexto o valor julgado é o do MÊS ou o acumulado do ANO
 *
 * ⚠⚠ META DO ANO NÃO JULGA MÊS. Vitor (28/08/2026): "no caso da RNC da engenharia (…) mudar a meta
 * para 8 no ano". Um teto anual pintado em cima de cada mês diz o contrário do que ele significa:
 * 8 RNCs num mês só ficariam verdes e o ano estaria estourado. Nesses indicadores o mês sai sem
 * cor — o número está lá, mas quem responde pela meta é o acumulado.
 */
export function farol(valor, meta, contexto = "ANO") {
  if (valor == null || Number.isNaN(valor)) return null;
  if (meta?.periodo === "ANO" && contexto === "MES") return null;
...
export const metaTexto = (m) => `${m.dir === "min" ? "≥" : "≤"} ${String(m.valor).replace(".", ",")}${m.unidade === "%" ? "%" : " " + m.unidade}${m.periodo === "ANO" ? " no ano" : ""}`;
