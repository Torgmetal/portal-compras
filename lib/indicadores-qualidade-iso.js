...
export async function indicadoresQualidadeIso(prisma, ano) {
  ...
  // RNCs recebidas do cliente — meta ANUAL (≤ 8 no ano). A série mostra o TOTAL ACUMULADO no
  // ano (corrente, mês a mês) pra bater com a meta anual — um mês isolado não faz sentido vs ≤8.
  // Acumulado = total do ano.
  { const mensal = zeros12(); let tot = 0;
    ...
    const s = zeros12(); let run = 0;
    for (let m = 0; m < 12; m++) { run += mensal[m]; s[m] = run; }
