...
export async function indicadoresRhIso(prisma, ano) {
  ...
  // do seu setor em dia (presentes e válidos). Baseado nos documentos REAIS (RH Documentos +
  // regras por setor/CCT), não na avaliação manual. Dispensados (PJ/Diretoria) ficam de fora.
  // Regras dispensáveis (NR-10/12/33/35) que o RH marcou como não-obrigatórias para o
  // funcionário (DocumentoDispensa) NÃO contam — mesma lógica da tela de Compliance; sem isso
  // toda a produção aparecia como "não atende". Snapshot no mês atual.
  { const [fs, dispRows] = await Promise.all([
      prisma.funcionario.findMany({ where: { ativo: true }, select: { id: true, tipoContrato: true, setor: { select: { nome: true } }, documentos: { where: { ativo: true }, select: { tipo: true, dataValidade: true, ativo: true } } } }),
    ...
    for (const f of fs) {
      ...
      const ok = regras.every((rg) => {
        ...
        const st = checarRegraDocumento(rg, f.documentos).status;
        return st === "OK" || st === "VENCENDO";
      });
      if (ok) atende++;
    }
    const val = comRegras > 0 ? r1((atende / comRegras) * 100) : null;
