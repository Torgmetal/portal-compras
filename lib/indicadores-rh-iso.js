...
export async function indicadoresRhIso(prisma, ano) {
  ...
  { const [fs, dispRows] = await Promise.all([
      prisma.funcionario.findMany({ where: { ativo: true }, select: { id: true, tipoContrato: true, dataAdmissao: true, setor: { select: { nome: true } }, documentos: { where: { ativo: true }, select: { tipo: true, dataEmissao: true, dataValidade: true, createdAt: true } } } }),
    ...
    for (let m = 0; m <= mesAtual; m++) {
      ...
      for (const f of fs) {
        ...
        const ok = regras.every((rg) => {
          ...
          if (atual) { const st = checarRegraDocumento(rg, f.documentos).status; return st === "OK" || st === "VENCENDO"; }
          return validoNa(rg, f.documentos, ref);
        });
        if (ok) atende++;
      }
