...
export async function indicadoresRhIso(prisma, ano) {
  ...
  // Snapshot no mês atual (enche conforme o RH sobe os documentos).
  { const fs = await prisma.funcionario.findMany({ where: { ativo: true }, select: { tipoContrato: true, setor: { select: { nome: true } }, documentos: { where: { ativo: true }, select: { tipo: true, dataValidade: true, ativo: true } } } });
    ...
    for (const f of fs) {
      ...
      comRegras++;
      const ok = regras.every((rg) => { const st = checarRegraDocumento(rg, f.documentos).status; return st === "OK" || st === "VENCENDO"; });
      if (ok) atende++;
    }
