...
// Remetentes IGNORADOS — e-mails automáticos (portal Workspace Torg + notificações
// da Microsoft/Copilot), que não são correspondência de projeto. Extra por env.
export function remetentesIgnorados() {
  const extra = (process.env.EMAILS_ENG_IGNORAR || "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
  return new Set(["noreply@torg.com.br", ...extra]);
}

// Padrão de remetente-ruído automático (Microsoft/Copilot/notificações de sistema).
// ⚠⚠ BOT DE REUNIÃO E NOTIFICAÇÃO DE PLATAFORMA NÃO SÃO CORRESPONDÊNCIA DE PROJETO. Vitor
// (29/08/2026): "preciso que ele seja cirúrgico nas informações (...) é para podermos ter base
// para qualquer problema que possamos ter futuramente com o cliente". Dos 170 e-mails que não
// casaram com obra nenhuma, 48 eram destes: read.ai e fireflies.ai mandando resumo de reunião,
// Trimble e ConstruManager mandando notificação de sistema. Eles enchem o histórico da obra de
// linhas que não são conversa com o cliente — e histórico com ruído não serve de prova.
const RX_REMETENTE_RUIDO = /@communication\.microsoft\.com$|@email\.teams\.microsoft\.com$|@microsoftonline\.com$|@sharepointonline\.com$|@e\.read\.ai$|@read\.ai$|@fireflies\.ai$|@otter\.ai$|^trimble-noreply@|^suporte@construmanager\.com\.br$|^naoresponda@construcode\.com\.br$|^(no-?reply|noreply|donotreply|do-not-reply|microsoft365|msonlineservicesteam)@/i;

function ehRemetenteIgnorado(de, ignorados) {
  const e = String(de || "").trim().toLowerCase();
  if (!e) return false;
  return ignorados.has(e) || RX_REMETENTE_RUIDO.test(e);
}
...
async function ingerirCaixaPasta(caixa, pasta, direcao, ignorados) {
  ...
  for (const m of mensagens) {
    ...
    if (ehRemetenteIgnorado(from.endereco, ignorados)) continue; // e-mail automático (portal/Microsoft) — ignora
...
export async function sincronizarEmailsEngenharia() {
  const resultados = [];
  const ignorados = remetentesIgnorados();
  for (const caixa of caixasEngenharia()) {
    for (const { pasta, direcao } of PASTAS) {
      try {
        resultados.push(await ingerirCaixaPasta(caixa, pasta, direcao, ignorados));
      } catch (e) {
