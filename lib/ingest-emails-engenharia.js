...
/** Ingestão de uma caixa+pasta: lê o delta, grava eventos novos, atualiza o deltaLink. */
async function ingerirCaixaPasta(caixa, pasta, direcao) {
  ...
  const { mensagens, deltaLink } = await lerDeltaMensagens(caixa, pasta, sync?.deltaLink || null);
  ...
  // bookkeeping do delta (não-fatal)
  await prisma.obraEmailSync.upsert({
    where: { caixa_pasta: { caixa, pasta } },
    create: { caixa, pasta, deltaLink: deltaLink || null, ultimoEm: new Date(), totalEventos: gravados, ultimoErro: null },
    update: { deltaLink: deltaLink || sync?.deltaLink || null, ultimoEm: new Date(), totalEventos: { increment: gravados }, ultimoErro: null },
  }).catch(() => {});

  return { caixa, pasta, lidos: mensagens.length, gravados };
}
