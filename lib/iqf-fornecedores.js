...
export async function calcularIQF(prisma, { yIni, yFim }) {
  ...
  // Acumulado do ano: % do total comprado no ano feito com fornecedor nível B.
  const totGasto = gastoMes.reduce((a, b) => a + b, 0), totNB = gastoNivelBMes.reduce((a, b) => a + b, 0);
  const acumuladoNivelB = totGasto > 0 ? Math.round((totNB / totGasto) * 1000) / 10 : null;

  return { fornecedores, serieComprasNivelB, acumuladoNivelB, pesos: PESOS };
}
