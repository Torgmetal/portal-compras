...
// ⚠ "CALHA" DE EQUIPAMENTO É FABRICADA. A calha comprada é a de cobertura ("CALHA GALVALUME 0.65MM");
// a "CALHA SAIDA RESIDUOS GROSSOS" da OP-103 (TMSA) é um chute de transportador — pesa 25 kg, tem
// croquis na LPC e é pintada. A regra de cobertura a escondia da produção, do rastreio e do inspetor
// (Vitor, 15/09/2026: a lista de pintura da 103 tem 442, o portal contava 434). Palavras de chute
// desfazem o casamento com "calha".
const RX_CALHA_FABRICADA = /calha.*\b(?:sa[ií]da|res[ií]duo|descarga|chute|bica|aliment|transfer)/;
...
export function ehItemComprado(p) {
  ...
  if (peso > 0) return RX_COBERTURA.test(d) && !d.includes("suporte") && !RX_CALHA_FABRICADA.test(d); // com peso: só cobertura/piso comprado
