...
function parsePintura(txt) {
  ...
  if (/condi[çc][aã]o\s*\d+/i.test(t)) {
    ...
    const rows = parts.map((p) => {
      ...
      return [c1, resto.replace(/\.$/, ""), esp];
    });
    return { modo: "COND", preamble, rows };
  }
  const frases = t.split(/\.\s+/).map(clean1).filter(Boolean);
  const rows = [], pre = [];
  for (const f of frases) { const m = f.match(/^([A-Za-zÀ-ú][^:]{2,32}):\s+(.+)$/); if (m) rows.push([m[1].trim(), m[2].replace(/\.$/, "").trim()]); else pre.push(f); }
...
function parseInspecao(txt) {
  ...
  return { preamble, rows: itens.map(triplaInspecao), nota };
}
  ...
  if (!isFiscal) {
    ...
    if (k.inspecao) {
      ...
      if (ins) {
        ...
        tabela([{ h: "Atividade / etapa", w: W - 200 - 82, a: "l" }, { h: "Critério / método", w: 200, a: "l" }, { h: "Abrangência", w: 82, a: "r" }], ins.rows);
