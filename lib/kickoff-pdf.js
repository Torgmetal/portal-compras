...
// Inspeção em tabela (Atividade / etapa | Abrangência) quando é lista numerada "N)" ou
// separada por ";". Prosa → null (mantém parágrafo). Puxa o preâmbulo (intro antes do 1º item).
function parseInspecao(txt) {
  let t = clean1(txt); if (!t) return null;
  let preamble = "";
  const numIdx = t.search(/\b1\)\s/);
  if (numIdx > 0 && numIdx < 170) { preamble = t.slice(0, numIdx).replace(/[:\s]+$/, "").trim(); t = t.slice(numIdx).trim(); }
  else { const ci = t.indexOf(":"), si = t.indexOf(";"); if (ci > 0 && ci < 130 && (si === -1 || ci < si)) { preamble = t.slice(0, ci).trim(); t = t.slice(ci + 1).trim(); } }
  const itens = t.split(/\s*;\s*|(?=\s\d{1,2}\)\s)/).map((s) => s.replace(/\.\s*$/, "").trim()).filter(Boolean);
  if (itens.length < 2) return null;
  const rows = itens.map((it) => {
    const m = it.match(/[(–\-]\s*(N\.?A\.?|AWS[^,;()]*|\d+\s*%[^;()]*?)\)?\s*$/i);
    if (m) { const ab = m[1].trim(); const at = it.slice(0, m.index).replace(/[\s,;:–\-(]+$/, "").trim(); if (at) return [at, ab]; }
    return [it, ""];
  });
  ...
  if (!isFiscal) {
    ...
    if (k.inspecao) {
      ...
      if (ins) {
        ...
        tabela([{ h: "Atividade / etapa", w: W - 96, a: "l" }, { h: "Abrangência", w: 96, a: "r" }], ins.rows);
