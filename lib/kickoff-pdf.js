...
// Capitaliza a 1ª letra (mantém siglas/normas maiúsculas; não toca µ nem números).
const cap1 = (s) => String(s || "").replace(/[a-zà-ÿ]/i, (c) => c.toUpperCase());
// Idem, mas só se o texto COMEÇA com letra (evita capitalizar "10% - empresa...").
const capIf = (s) => (/^\s*[a-zà-ÿ]/i.test(String(s || "")) ? cap1(s) : String(s || ""));

// Padrão de pintura em tabela quando há estrutura: "Condição N:" (3 col: Condição/Sistema/
// Espessura) ou segmentos "Rótulo: valor" (2 col). Prosa curta → null (mantém parágrafo).
function parsePintura(txt) {
  const t = clean1(txt); if (!t) return null;
  if (/condi[çc][aã]o\s*\d+/i.test(t)) {
    const parts = t.split(/(?=condi[çc][aã]o\s*\d+\s*[:\-–.])/i).map(clean1).filter(Boolean);
    let preamble = ""; if (parts.length && !/^condi[çc]/i.test(parts[0])) preamble = parts.shift();
    const rows = parts.map((p) => {
      const m = p.match(/^(condi[çc][aã]o\s*\d+)\s*[:\-–.]?\s*(.*)$/i);
      const c1 = m ? m[1] : "Condição";
      let resto = m ? m[2] : p, esp = "";
      const me = resto.match(/=\s*([\d.,]+\s*[µu]?m)\s*\.?\s*$/i);
      if (me) { esp = me[1].replace(/\s+/g, ""); resto = resto.slice(0, me.index).replace(/[\s=.]+$/, "").trim(); }
      return [c1, cap1(resto.replace(/\.$/, "")), esp];
    });
    return { modo: "COND", preamble, rows };
  }
  const frases = t.split(/\.\s+/).map(clean1).filter(Boolean);
  const rows = [], pre = [];
  for (const f of frases) { const m = f.match(/^([A-Za-zÀ-ú][^:]{2,32}):\s+(.+)$/); if (m) rows.push([cap1(m[1].trim()), cap1(m[2].replace(/\.$/, "").trim())]); else pre.push(f); }
  if (rows.length >= 2) return { modo: "LABEL", preamble: pre.join(". "), rows };
  return null;
}

// Quebra UM item de inspeção em [atividade, critério/método, abrangência(%)].
function triplaInspecao(item) {
  const it = clean1(item).replace(/^[,;\s]+/, "").replace(/[.\s]+$/, "");
  const par = it.match(/^(.*?)\s*\(([^()]+)\)$/); // "atividade (critério, %)"
  if (par) {
    const atividade = par[1].replace(/[–\-:\s]+$/, "").trim();
    const dentro = par[2].trim();
    const mp = dentro.match(/^(.*?),\s*([^,]*?\d+\s*%[^,]*|N\.?A\.?)$/i);
    if (mp) return [atividade, mp[1].trim(), mp[2].trim()];
    if (/^(\d+\s*%|N\.?A\.?)/i.test(dentro)) return [atividade, "", dentro];
    return [atividade, dentro, ""];
  }
  const mAbr = it.match(/[–-]\s*(\d+\s*%[^–-]*|crit[ée]rio\s+[^–-]*|AWS[^–-]*|N\.?A\.?)\s*$/i); // "atividade – critério – %"
  if (mAbr) {
    const abr = mAbr[1].replace(/^crit[ée]rio\s+/i, "").trim();
    const resto = it.slice(0, mAbr.index).replace(/[–\-\s]+$/, "").trim();
    const mc = resto.match(/^(.*?)\s+[–-]\s+(.+)$/);
    if (mc) return [mc[1].trim(), mc[2].trim(), abr];
    return [resto, "", abr];
  }
  return [it, "", ""];
}

// Inspeção em tabela [Atividade / etapa | Critério | Abrangência]. Detecta a estrutura:
// numerada "N)", separada por ";" OU por vírgula com parênteses "(critério, %)" (formato mais
// comum). Prosa → null (mantém parágrafo). Puxa o preâmbulo (intro) e a nota final (sem item).
function parseInspecao(txt) {
  let t = clean1(txt); if (!t) return null;
  let preamble = "", nota = "";
  const numIdx = t.search(/\b1\)\s/);
  if (numIdx > 0 && numIdx < 180) { preamble = t.slice(0, numIdx).replace(/[:\s]+$/, "").trim(); t = t.slice(numIdx).trim(); }
  else { const ci = t.indexOf(":"); if (ci > 0 && ci < 130) { preamble = t.slice(0, ci).trim(); t = t.slice(ci + 1).trim(); } }
  let itens = null;
  if (/\b\d{1,2}\)\s/.test(t)) {
    itens = t.split(/\s*;\s*|(?=\s\d{1,2}\)\s)/).map((s) => s.trim()).filter(Boolean);
  } else {
    const pars = t.match(/\([^()]+\)/g);
    if (pars && pars.length >= 2) { // vírgula/; com parênteses → ancora nos parênteses
      itens = []; let last = 0;
      for (const m of t.matchAll(/\([^()]+\)/g)) { const end = m.index + m[0].length; itens.push(t.slice(last, end).trim()); last = end; }
      const tail = t.slice(last).replace(/^[,;.\s]+/, "").trim();
      if (tail) nota = tail;
    } else if ((t.match(/;/g) || []).length >= 1) {
      itens = t.split(/\s*;\s*/).map((s) => s.replace(/\.\s*$/, "").trim()).filter(Boolean);
    }
  }
  if (!itens || itens.length < 2) return null;
  return { preamble, rows: itens.map(triplaInspecao).map(([a, c, ab]) => [cap1(a), cap1(c), capIf(ab)]), nota };
}
...
export async function gerarKickoffPDF({ op, kickoff, tipo = "GERAL", itens = [] }) {
  ...
  const novaPagina = (comBanda) => {
    ...
    if (comBanda) {
      ...
      page.drawText(san(`Emitido em ${dataBR(new Date())}`), { x: x0, y: A4[1] - 80, size: 8.5, font, color: rgb(0.66, 0.76, 0.88) });
      y = A4[1] - h - 26;
    } else { y = A4[1] - M; }
  };
  ...
  if (!isFiscal) {
    ...
    if (k.padraoPintura) {
      secao("Padrao de pintura");
      const pp = parsePintura(k.padraoPintura);
      if (pp) {
        if (pp.preamble) { paragrafo(pp.preamble, 10); y -= 2; }
        if (pp.modo === "COND") tabela([{ h: "Condição", w: 84, a: "l" }, { h: "Sistema (fundo + acabamento)", w: W - 84 - 74, a: "l" }, { h: "Espessura", w: 74, a: "r" }], pp.rows);
        else tabela([{ h: "Item", w: 130, a: "l" }, { h: "Especificação", w: W - 130, a: "l" }], pp.rows);
      } else paragrafo(k.padraoPintura, 10);
    }
    if (k.inspecao) {
      secao("Inspecao");
      const ins = parseInspecao(k.inspecao);
      if (ins) {
        if (ins.preamble) { paragrafo(ins.preamble, 10); y -= 2; }
        tabela([{ h: "Atividade / etapa", w: W - 190 - 92, a: "l" }, { h: "Critério / método", w: 190, a: "l" }, { h: "Abrangência", w: 92, a: "l" }], ins.rows);
        if (ins.nota) { y -= 2; paragrafo(ins.nota, 9, italic, GRAY); }
      } else paragrafo(k.inspecao, 10);
    }
