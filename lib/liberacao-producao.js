...
// ⚠ a sugestão do dia mora em lib/liberacao-sugestao.js (sem `server-only`): a TELA precisa
// recalculá-la a cada mudança de filtro, e ir ao servidor a cada clique seria lento demais.
export { POOLS, poolDaPeca, sugerirDoDia } from "@/lib/liberacao-sugestao";
...
// ⚠⚠ `status = "CORTE"` NÃO É "CORTADA" — É "ESTÁ NO CORTE". Eu tinha posto CORTE na lista de
// status concluídos, e o filtro "só as a fazer" passou a esconder exatamente o trabalho que falta:
// medido em 25/08/2026, são 7.751 peças com status CORTE e ZERO com `corteConcluidoEm` — 576
// TONELADAS que a tela dava como prontas. A OP-083 aparecia com "0 peças a fazer" tendo 355 peças
// e 51 t para cortar.
//
// Quem manda é `corteConcluidoEm`: é o que /api/pcp/painel-corte e /api/pcp/fila-corte usam para
// listar o que FALTA (`status: "CORTE", corteConcluidoEm: null`).
const STATUS_PASSOU_DO_CORTE = ["MONTAGEM", "SOLDA", "ACABAMENTO", "JATO", "PINTURA", "EXPEDIDO"];

/** a peça já foi cortada? (mesmo critério do painel de Corte) */
export function pecaCortada(p) {
  if (p.corteConcluidoEm) return true;
  if ((Number(p.qteProduzida) || 0) >= (Number(p.qte) || 1)) return true;
  // status posterior ao corte significa que ela passou por lá; "CORTE" significa que está lá agora.
  return STATUS_PASSOU_DO_CORTE.includes(p.status);
}
