...
import { resolveServidorDriveId } from "@/lib/sharepoint-lpc";
import { prismaDirect } from "@/lib/prisma";
import { parseListaAvancada, revisaoDoNome } from "@/lib/parse-lista-avancada";
import { parseRomaneio } from "@/lib/parse-romaneio";
import { preservarReferenciasRomaneio } from "@/lib/referencias-romaneio";
import { acumularRomaneio, totalExpedido } from "@/lib/expedido-por-romaneio";
import { pesoFaltanteReal } from "@/lib/expedicao-estrutura";
import { alinharCronogramaExpedicao } from "@/lib/expedicao-cronograma";

// Importa a "Lista Avançada Expedição" do SharePoint para a tabela ListaExpedicao.
// A lista fica SEMPRE em {OP}/2. Engenharia/2.6 Lista de expedição — ver
// arquivosListaDaOP. OPs ficam em "01. OP" e as concluídas em "01. OP/Finalizadas".
//
// UM MODELO SÓ: usa o MESMO drive/base que o Vitor usa pra SALVAR (biblioteca
// SERVIDOR via resolveServidorDriveId; fallback = SHAREPOINT_DRIVE_ID). Assim
// leitura (Lista + expedido) e escrita (romaneio) ficam no mesmo lugar e o ciclo
// do "expedido" fecha por construção.
const BASE = process.env.SHAREPOINT_OP_BASE_FOLDER || "/Ordem de Servico/01. OP";
const GRAPH = "https://graph.microsoft.com/v1.0";
const enc = (p) => p.split("/").filter(Boolean).map(encodeURIComponent).join("/");

// Drive resolvido 1× (memoizado) — mesma resolução do sharepoint-lpc/sharepoint-lista.
let _drivePromise = null;
const drive = () => (_drivePromise ||= resolveServidorDriveId());

async function listChildren(token, path) {
  const url = `${GRAPH}/drives/${await drive()}/root:/${enc(path)}:/children?$select=name,id,file,folder,size,lastModifiedDateTime&$top=200`;
  const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  if (!r.ok) return [];
  return (await r.json()).value || [];
}
async function downloadById(token, id) {
  const r = await fetch(`${GRAPH}/drives/${await drive()}/items/${id}/content`, { headers: { Authorization: `Bearer ${token}` } });
...
const ehListaFormatada = (name) => /^LE_OP-\d+_\d{4}-\d{2}-\d{2}\.xlsx$/i.test(name);
const ehListaArquivo = (name) =>
  /\.(xls|xlsx|xlsm)$/i.test(name) &&
  (/[-_\s]le[-_\s]*r?\d/i.test(name) || ehListaFormatada(name)) &&
...
function assinaturaLista(nome) {
  // A data de emissão não cria uma frente nova para a lista consolidada.
  if (ehListaFormatada(nome)) return `OP-${String(nome).match(/OP-(\d+)/i)[1].replace(/^0+/, "")} LE`;
  ...
  // ⚠⚠ LOOKBEHIND, NÃO \b. Em "T97-LE_R00" o "_" é caractere de palavra, então NÃO há fronteira
  // antes do R e o \b não casava: a revisão ficava no nome e "T97-LE_R00.xlsx" virava a frente
  // "T97-LE R00", separada de "T97-LE" (que veio de "T97-LE-R01.xlsx"). Na tela a mesma lista
  // aparecia DUAS VEZES, como se fossem duas frentes, e o peso contratado dobrava.
  // É o mesmo bug que `revisaoDoNome` (lib/parse-lista-avancada) já tinha corrigido com lookbehind
  // — a correção não tinha sido trazida para cá.
  s = s.replace(/(?<![A-Za-z0-9])R(?:ev)?\.?\s*\d{1,3}\b/ig, " ");   // revisão
...
// Acha, entre as subpastas, a primeira cujo nome casa com a regex.
const achaPasta = (kids, rx) => (kids || []).filter((k) => k.folder).find((k) => rx.test(k.name || ""));

/**
 * Onde mora a Lista de Expedição da OP: SEMPRE em
 * `2. Engenharia / 2.6 Lista de expedição` — navega por nome (regex) porque
 * acento/caixa/numeração variam entre as pastas. Só considera o arquivo "LE".
 * (O fallback legado `4. Expedição` + `4.1 Lista de Avançada` foi removido — o
 * time não usa mais esse arquivo.)
 */
async function arquivosListaDaOP(token, opPath) {
  ...
  // A formatada é uma cópia consolidada: só serve de fonte quando não há LE
  // original, para não somar as mesmas peças duas vezes.
  const originais = out.filter((a) => !ehListaFormatada(a.name));
  return originais.length ? originais : out;
}
...
/**
 * Lê os ROMANEIOS emitidos da OP ({OP}/4. Expedição/4.2 Romaneios) e devolve o
 * que já saiu, por marca. É a fonte do "expedido" da lista (Vitor 22/07).
 * Só planilhas — o PDF de cada romaneio é cópia da mesma carga.
 */
async function marcasExpedidasOP(token, opPath) {
  const exp = achaPasta(await listChildren(token, opPath), /^4\.\s*expedi/i);
  if (!exp) return null;
  const rom = achaPasta(await listChildren(token, `${opPath}/${exp.name}`), /4\.\s*2|romaneio/i);
  if (!rom) return null;
  const dir = `${opPath}/${exp.name}/${rom.name}`;
  const arqs = (await listChildren(token, dir)).filter(
    (k) => k.file && /\.(xls|xlsx|xlsm)$/i.test(k.name) && !/obsolet|c[óo]pia|backup|antig/i.test(k.name)
  );
  // ⚠⚠ `porRomaneio` É O QUE FALTAVA: quantas peças da marca saíram em CADA carga. Sem ele, a
  // marca virava um sim/não e a que saiu pela metade contava inteira (ver lib/expedido-por-romaneio.js).
  const porMarca = new Map(); // marca → { peso, romaneios:[nº], datas:[Date], porRomaneio:{nº:qtd} }
  let cargas = 0, pesoRomaneios = 0, falhas = 0;
  for (const a of arqs) {
    try {
      const p = parseRomaneio(await downloadById(token, a.id), a.name);
      if (!p.ok) { falhas++; continue; }
      cargas++;
      pesoRomaneios += p.totais.pesoSomado;
      for (const it of p.itens) {
        const k = String(it.marca).trim().toUpperCase();
        if (!k) continue;
        const cur = porMarca.get(k) || { peso: 0, romaneios: [], datas: [], porRomaneio: {} };
        cur.peso += it.pesoKg || 0;
        acumularRomaneio(cur.porRomaneio, { numero: p.numero, qtd: it.qtd });
        if (p.numero && !cur.romaneios.includes(p.numero)) cur.romaneios.push(p.numero);
        if (p.dataSaida) cur.datas.push(p.dataSaida);
        porMarca.set(k, cur);
      }
    } catch { falhas++; }
  }
  return { porMarca, cargas, pesoRomaneios, falhas, arquivos: arqs.length };
}
...
export async function importarListasOP({ opNumero, opId = null, userId = null }) {
  ...
  if (!arquivos.length) return { ok: false, erro: `Nenhum arquivo "LE" (Lista de Expedição) em "2. Engenharia / 2.6 Lista de expedição" da OP-${opNumero}.` };
  ...
  // romaneios já emitidos da OP (lidos 1× e cruzados com as marcas de cada frente)
  let romaneios = null;
  try { romaneios = await marcasExpedidasOP(token, op.path); } catch { romaneios = null; }
  ...
  for (const [fr, a] of porFrente) {
    try {
      ...
      // "expedido" REAL = marcas desta frente que aparecem nos romaneios emitidos
      if (romaneios?.porMarca?.size) {
        // marca cada peça: expedida ou não + em qual romaneio e quando (vai no export)
        for (const m of parsed.marcas) {
          const r = romaneios.porMarca.get(chaveMarca(m));
          m.expedidoRomaneio = !!r;
          // ⚠ QUANTAS peças saíram, e em qual carga cada uma — é isto que deixa a marca de 2 peças
          // com 1 embarcada aparecer como PARCIAL, e não sumir da lista de quem vai emitir o
          // próximo romaneio (Larissa, 22/09/2026, na OP-067).
          m.expedidoPorRomaneio = r ? { ...r.porRomaneio } : null;
          m.expedidoQtd = r ? totalExpedido(r.porRomaneio) : 0;
          m.romaneio = r?.romaneios?.length ? r.romaneios.join(", ") : null;
          const dts = (r?.datas || []).filter(Boolean).sort((a, b) => a - b);
          m.dataExpedicao = dts.length ? dts[0].toISOString().slice(0, 10) : null;
        }
        const expedidas = parsed.marcas.filter((m) => m.expedidoRomaneio);
        const pesoExp = expedidas.reduce((s, m) => s + (m.pesoTotal || 0), 0);
        dados.pesoExpedido = pesoExp;
        // Faltante = marcas (com peso) NÃO embarcadas. Antes era "contratado − expedido",
        // que subestima quando o contrato registrado é menor que a soma real das peças
        // (ex.: OP-85 mostrava 2.041 kg com ~5.159 de guarda-corpo pendente). Vitor 09/08.
        dados.pesoFaltante = pesoFaltanteReal(parsed.marcas);
        dados.expedidasArquivo = expedidas.length;
      }
      ...
      dados.marcasJson = preservarReferenciasRomaneio(dados.marcasJson, anterior?.marcasJson);
  ...
  // Alinha a linha "Expedição" do cronograma com o expedido recém-importado (best-effort).
  let cronograma = null;
  if (opId) {
    try { cronograma = await alinharCronogramaExpedicao(opId); }
    catch (e) { cronograma = { ok: false, motivo: e.message }; }
  }
  return {
    ok: true, op: String(opNumero), folder: op.folder, finalizada: op.finalizada, resultados, cronograma,
    romaneios: romaneios ? { cargas: romaneios.cargas, arquivos: romaneios.arquivos, falhas: romaneios.falhas, marcas: romaneios.porMarca.size, pesoRomaneios: romaneios.pesoRomaneios } : null,
  };
}
