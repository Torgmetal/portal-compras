...
// Aceita .xls também: na pasta 2.6 há listas no formato antigo do Excel.
// Nomes da 2.6 são "T67-LE-R00.xlsx" / "T64A-LE_R00.xls" — daí o "[-_ ]LE[-_ ]R\d"
// (antes só pegava "LE_R\d" e ignorava o hífen, perdendo a maioria dos arquivos).
const ehListaArquivo = (name) =>
  /\.(xls|xlsx|xlsm)$/i.test(name) &&
  /(lista\s*avan|lista\s*de\s*expedi|[-_\s]le[-_\s]*r?\d)/i.test(name) &&
