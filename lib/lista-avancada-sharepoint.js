  ...
  for (const a of arqs) {
    try {
      ...
      for (const it of p.itens) {
        ...
        if (k) porMarca.set(k, (porMarca.get(k) || 0) + (it.pesoKg || 0));
      }
    } catch { falhas++; }
  }
  ...
  for (const [fr, a] of porFrente) {
    try {
      ...
      if (romaneios?.porMarca?.size) {
        // marca cada peça como expedida ou não (fica no marcasJson, alimenta o export)
        for (const m of parsed.marcas) m.expedidoRomaneio = romaneios.porMarca.has(chaveMarca(m));
