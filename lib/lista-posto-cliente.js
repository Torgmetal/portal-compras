...
export async function baixarListaDoPosto({ setor, recurso, nomePosto, de, ate, periodo }) {
  ...
  if (periodo === "fila") qs.set("fila", "sem-bancada");
  const res = await fetch(`/api/pcp/lista-posto?${qs}`, { cache: "no-store" });
  const d = await res.json();
  if (!res.ok) throw new Error(d.error || "Falha ao montar a lista");
  if (!d.linhas.length && periodo === "fila") throw new Error("Nenhuma peça na fila sem bancada deste setor.");
  ...
  const { workbook, sheet: ws, linhaInicio } = await criarRelatorioTorg({
    ...
    subtitulo: periodo === "fila" ? `${setor} · fila completa sem bancada · todas as datas` : `${setor} · ${periodo === "dia" ? dbr(de) : `${dbr(de)} a ${dbr(ate)}`}`,
    kpis: [
  ...
  let l = linhaInicio;
  adicionarHeaderTabela(ws, l, ["Dia", "OP", "Obra", "Marca", periodo === "fila" ? "Material" : "Descricao", "Perfil", "A fazer", "Peso (kg)"]);
  l++;
