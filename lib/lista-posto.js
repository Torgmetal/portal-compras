...
export async function listaDoPosto(setor, recurso, de, ate) {
  ...
  const pecas = await prisma.pecaConjunto.findMany({
    where: {
      ...SO_FABRICACAO, ...OP_VIVA,
      [campo.recurso]: recurso || null,
      [campo.dia]: { gte: diaUTC(de), lt: fim },
    },
    select: {
      id: true, opId: true, marca: true, descricao: true, perfil: true, qte: true,
      pesoTotalKg: true, tipoPeca: true, [campo.dia]: true,
  ...
  const linhas = pecas.map((p) => {
    ...
    const f = Math.min(feito({ opId: p.opId, marca: p.marca }, setor), qte);
    return {
      ...
      kg: Math.round(((p.pesoTotalKg || 0) / qte) * (qte - f)),
    };
  });
  ...
  return {
    ...
    linhas,
    total: {
      marcas: linhas.length, marcasAbertas: abertos.length,
