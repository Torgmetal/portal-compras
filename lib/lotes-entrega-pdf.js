...
export async function gerarLotesPDF(o, now = new Date()) {
  ...
  kv("Obra:", o.obra);
  y -= 4;
  para("Entrega das peças em lotes, conforme locais e datas abaixo. As quantidades por lote devem ser conferidas no ato do recebimento.", 9, GRAY);
  y -= 6;

  if (!lotes.length) { para("Nenhum lote cadastrado.", 10, GRAY); }

  // ── Lotes ──
  const cols = [0.62 * W, 0.16 * W, 0.22 * W];
  lotes.forEach((lote, li) => {
    ...
    const th = 18;
    const cell = (t, cx, cw, ty, f, color, right) => { const s = fit(t, f, 8.5, cw - 8); page.drawText(s, { x: right ? cx + cw - 5 - f.widthOfTextAtSize(s, 8.5) : cx + 5, y: ty, size: 8.5, font: f, color }); };
    const row = (c0, c1, c2, header) => { espaco(th + 2); page.drawRectangle({ x: M, y: y - th, width: W, height: th, color: header ? LIGHT : WHITE, borderColor: LINE, borderWidth: 0.5 }); cell(c0, M, cols[0], y - th + 5.5, header ? bold : font, DARK); cell(c1, M + cols[0], cols[1], y - th + 5.5, header ? bold : font, DARK, true); cell(c2, M + cols[0] + cols[1], cols[2], y - th + 5.5, header ? bold : font, DARK); y -= th; };
    row("Descrição", "Qtd.", "Unidade", true);
    if (!itens.length) { espaco(th); page.drawText("(sem itens neste lote)", { x: M + 5, y: y - th + 5.5, size: 8.5, font, color: GRAY }); y -= th; }
    else itens.forEach((it) => row(it.descricao || "—", it.qtd || "", it.unidade || ""));
    y -= 16;
  });
