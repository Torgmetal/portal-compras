...
export async function gerarLotesPDF(o, now = new Date()) {
  ...
  // Cabeçalho da página — repetido em TODA página, pois cada lote é impresso
  // sozinho (1 lote por página, facilita a conferência no recebimento).
  const bh = 56;
  ...
  const cabecalho = () => {
    ...
    if (logo) { const lw = 88, lh2 = (logo.height / logo.width) * lw; page.drawImage(logo, { x: M, y: A4[1] - bh / 2 - lh2 / 2, width: lw, height: lh2 }); }
    const tit = "PLANO DE ENTREGAS";
    page.drawText(tit, { x: A4[0] - M - bold.widthOfTextAtSize(tit, 13), y: A4[1] - 26, size: 13, font: bold, color: WHITE });
    const subT = san(`${osNum(o)} · ${dataStr}`);
    page.drawText(subT, { x: A4[0] - M - font.widthOfTextAtSize(subT, 9), y: A4[1] - 42, size: 9, font, color: rgb(0.75, 0.82, 0.92) });
    y = A4[1] - bh - 20;
    kv("Cliente:", o.cliente);
    kv("Obra:", o.obra);
    y -= 6;
  };
  // Linha de conferência no fim de cada lote (checagem/assinatura na impressão).
  const conferencia = () => {
    if (y < 84) return;
    y -= 12;
    page.drawLine({ start: { x: M, y }, end: { x: A4[0] - M, y }, thickness: 0.5, color: LINE });
    y -= 16;
    const campo = (lbl, x1, x2) => { page.drawText(lbl, { x: x1, y, size: 9, font: bold, color: GRAY }); page.drawLine({ start: { x: x1 + bold.widthOfTextAtSize(lbl, 9) + 6, y: y - 1 }, end: { x: x2, y: y - 1 }, thickness: 0.5, color: LINE }); };
    campo("Conferido por:", M, M + 232);
    campo("Data:", M + 250, M + 360);
    campo("Visto:", M + 378, A4[0] - M);
  };

  if (!lotes.length) {
    novaPagina(); cabecalho();
    para("Nenhum lote cadastrado.", 10, GRAY);
    rodape();
    return { bytes: await pdf.save(), filename: `${osNum(o)}-plano-entregas.pdf` };
  }
  ...
  lotes.forEach((lote, li) => {
    novaPagina();          // 1 lote por página (impressão p/ conferência)
    cabecalho();
    if (li === 0) { para("Cada lote é impresso em uma página para facilitar a conferência no recebimento. A coluna Documento traz o arquivo de referência da peça, quando anexado.", 8.5, GRAY); y -= 4; }
    ...
    page.drawText(fit(lote.nome || `Lote ${li + 1}`, bold, 11, W - 170), { x: M + 12, y: y - h + 6.5, size: 11, font: bold, color: WHITE });
    const info = `Lote ${li + 1}/${totalLotes}` + (lote.data ? ` · Entrega: ${lote.data}` : "");
    page.drawText(san(info), { x: A4[0] - M - font.widthOfTextAtSize(san(info), 9) - 10, y: y - h + 7, size: 9, font, color: rgb(0.8, 0.86, 0.94) });
    y -= h + 10;
    // local de entrega
