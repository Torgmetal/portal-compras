...
export function importarLqc(buffer) {
  ...
  if (pc) {
    ...
    const cPc = iCabPc >= 0 ? colunas(pc[iCabPc], { valor: ["valor r$", "valor"] }) : { valor: 6 };
    for (const l of pc) {
      if (!/^subtotal$/.test(norm(l[0]) || norm(l[1]))) continue;
      const v = typeof l[cPc.valor] === "number" ? l[cPc.valor] : numeroBr(String(l[cPc.valor] ?? ""));
      if (v > 0) { precoPlanilha = (precoPlanilha || 0) + v; }
    }
  }
  ...
  return {
    ...
    resumo: {
      ...
      precoMateriaPrima, precoPlanilha,
