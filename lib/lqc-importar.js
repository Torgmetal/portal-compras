...
import { numeroBr, perdaDaEstrutura, CLASSES } from "./lqc";
...
export function importarLqc(buffer) {
  ...
  const montagemLidas = { mdo: 0, despesas: 0, equipamentos: 0, estruturaRsKg: 0, porItem: {}, desmontagem: null };
  ...
  if (pc) {
    ...
    for (const l of pc) {
      ...
      // ⚠ o TOTAL GERAL fecha a planilha e não é linha de bloco nenhum — somá-lo em montagem
      // colocava R$ 2,78 mi de "equipamentos" que não existem. (Auditoria de 05/09/2026.)
      if (/^total geral$/.test(item) || /^total geral$/.test(rot)) { bloco = null; continue; }
      if (/itens comerciais/.test(rot)) { bloco = "comerciais"; continue; }
      // ⚠ ANCORADO NO FIM: "MONTAGEM" é o título do bloco, mas "Montagem das estruturas metálicas"
      // é uma LINHA dele. Com `/^montagem/` toda linha virava cabeçalho e o bloco saía vazio.
      ...
      // ── montagem em campo: o bloco 3 ──
      //
      // ⚠⚠ AQUI SE LÊ O CUSTO, NÃO O PREÇO. A coluna "Unit. R$" da planilha já traz o BDI dentro
      // (R$ 6,86/kg na LQC-253); o que o estudo precisa é o custo, que está nas colunas MONTAGEM
      // (mão de obra) e DESPESAS E CANTEIRO. Importar o 6,86 como custo e deixar o motor aplicar
      // despesas + BDI por cima dobraria o preço da montagem.
      if (bloco === "montagem") {
        const desc = norm(l[cPc.desc]);
        const qtd = numDe(l, cPc.quant);
        const mdo = numDe(l, cPc.valor + 2);        // MONTAGEM
        const desp = numDe(l, cPc.valor + 3);       // DESPESAS E CANTEIRO
        const equip = numDe(l, cPc.valor + 4);      // EQUIPAMENTOS
        montagemLidas.mdo += mdo;
        montagemLidas.despesas += desp;
        montagemLidas.equipamentos += equip;
        if (!qtd || !mdo) continue;
        if (/montagem das estruturas/.test(desc)) montagemLidas.estruturaRsKg = r2(mdo / qtd);
        else if (/desmontagem|remo[cç]/.test(desc)) montagemLidas.desmontagem = { qtd: r2(qtd), preco: r2(mdo / qtd), un: String(l[cPc.un] ?? "m²") };
        else {
          const key = chaveItemComercial(desc);
          if (key) montagemLidas.porItem[key] = r2(mdo / qtd);
        }
        continue;
      }
  ...
  // a proporção despesas/mão de obra é da própria obra — na 253 dá 126%, e é o padrão da casa
  const montagem = blocos.montagem > 0 && montagemLidas.mdo > 0
    ? {
        ativo: true,
        estruturaRsKg: montagemLidas.estruturaRsKg || null,
        porItem: montagemLidas.porItem,
        despesasPct: r2((montagemLidas.despesas / montagemLidas.mdo) * 100),
        equipamentosVb: r2(montagemLidas.equipamentos),
        ...(montagemLidas.desmontagem ? { desmontagem: montagemLidas.desmontagem } : {}),
      }
    : null;
  if (blocos.montagem > 0 && !montagem)
    avisos.push(`A planilha tem R$ ${Math.round(blocos.montagem).toLocaleString("pt-BR")} de MONTAGEM que não consegui destrinchar em mão de obra e canteiro.`);
  ...
  // Preserve os custos efetivamente orçados, inclusive casas decimais da tabela da obra.
  const custosImportados = { precos: { classe: {} }, terceiros: [], faturamento: {} };
  const tintasPorPerda = new Map();
  if (ind) {
    const cab = acharCabecalho(ind, ["descricao", "preco unit"], 8);
    const cols = cab >= 0 ? colunas(ind[cab], {
      item: ["item"], desc: ["descricao"], unidade: ["unid"], preco: ["preco unit"], peso: ["peso total"], subtotal: ["subtotal"],
    }) : { item: 0, desc: 1, unidade: 3, preco: 5, peso: 4, subtotal: 6 };
    let bloco = "", faturamento = "TORG";
    for (const l of ind.slice(cab + 1)) {
      const item = String(l[cols.item] ?? "").trim();
      const desc = String(l[cols.desc] ?? "").trim();
      const rot = norm(desc);
      const valor = l[cols.preco];
      const preco = typeof valor === "number" ? valor : numeroBr(String(valor ?? ""));
      if (item) {
        bloco = item;
        faturamento = String(l[cols.unidade] || "TORG").toUpperCase();
        const chave = { "1.1": "materiaPrima", "1.2": "fixadores", "1.3": "tintas" }[item];
        if (chave && ["TORG", "DIRETO"].includes(faturamento)) custosImportados.faturamento[chave] = faturamento;
        continue;
      }
      if (!Number.isFinite(preco) || valor === "" || valor == null) continue;
      const campo = { "3.1": "fabricacao", "3.2": "pintura", "3.3": "preMont" }[bloco];
      const classe = CLASSES.find(c => norm(c.nome) === rot);
      if (campo && classe) {
        custosImportados.precos.classe[classe.key] ||= {};
        custosImportados.precos.classe[classe.key][campo] = preco;
      }
      if (/^2\.\d+$/.test(bloco) && preco > 0) {
        const num = j => typeof l[j] === "number" ? l[j] : numeroBr(String(l[j] ?? ""));
        const peso = num(cols.peso), subtotal = num(cols.subtotal);
        const pesoObra = resumos.reduce((s,r) => s + r.pesoTotal, 0);
        const ehKg = norm(l[cols.unidade]) === "kg";
        const obraToda = ehKg && Math.abs(peso - pesoObra) < 0.01;
        const areas = [...new Set(resumos.map(r => r.area).filter(Boolean))];
        const candidatas = areas.filter(a => rot.includes(norm(a)) &&
          Math.abs(resumos.filter(r => r.area === a).reduce((s,r) => s + r.pesoTotal,0) - peso) < 0.01);
        const area = candidatas.find(a => rot === norm(a)) || (candidatas.length === 1 ? candidatas[0] : null);
        if (obraToda || (ehKg && area)) {
          custosImportados.terceiros.push({ descricao: desc, base: "kg", precoUnit: preco,
            ...(obraToda ? {} : {area}), faturamento: faturamento === "DIRETO" ? "DIRETO" : "TORG" });
        } else if (subtotal > 0) {
          custosImportados.terceiros.push({ descricao: desc, base: "verba", quantidade: 1, precoUnit: subtotal,
            escopoFixo: true, faturamento: faturamento === "DIRETO" ? "DIRETO" : "TORG" });
          avisos.push(`Serviço ${desc}: subtotal preservado como verba fixa; confira a área e o escopo antes de reduzir o quantitativo.`);
        } else avisos.push(`Serviço ${desc}: base de cobrança não reconhecida; confira o custo antes de emitir.`);
      }
      const perda = /fator de perda:\s*(45|85)\s*%/.exec(rot)?.[1];
      if (bloco === "1.3" && perda && preco > 0) tintasPorPerda.set(Number(perda), preco);
    }
  }
  // Sem destino por cor, não inventar distribuição nem zerar o acabamento. Usa o custo
  // por kg do grupo da planilha, que continua acompanhando o peso do escopo selecionado.
  for (const [perda, precoKg] of tintasPorPerda) {
    const grupo = resumos.filter(r => (r.perda || 45) === perda);
    if (!grupo.some(r => !r.cor) && tintas.some(t => t.perda === perda)) continue;
    let camada = tintas.find(t => t.perda === perda);
    if (!camada) { camada = { perda, camada: "PRIMER", produto: "Custo de tintas da planilha" }; tintas.push(camada); }
    camada.precoKg = precoKg;
    camada.origemPrecoKg = "INDUSTRIALIZACAO";
    avisos.push(`Pintura com perda ${perda}%: custo por kg preservado da planilha; confira os destinos por cor antes de recalcular o consumo.`);
  }
  ...
  return {
    ok: true, resumos, precosPorArea, precoMateriaPrima, fixadoresRsKg, precoPlanilha, bdi, tintas, avisos, custosImportados,
    // os três blocos da PLANILHA COMERCIAL, separados — ver o comentário na leitura
    blocosPlanilha: blocos, itensComerciais, montagem,
    resumo: {
