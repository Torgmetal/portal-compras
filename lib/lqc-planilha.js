...
import { varrerPastasDeLqc, EH_PASTA_DO_MODELO } from "./lqc-sharepoint";
import { abrirXlsx, escreverCelulas, forcarRecalculo, fecharXlsx, adicionarAba } from "./xlsx-patch";
import { calcularLqc, calcularCamadasPintura, numeroBr, fluxoDeCaixa, BDI_CAMPOS, LINHAS_FATURAMENTO, ITENS_COMERCIAIS, CFOPS, ENSAIOS, BASES_ENSAIO, CLASSES, precoPreMontagem } from "./lqc";
...
/**
 * Entre as cópias do modelo achadas no servidor, a que vale (puro — testável sem rede).
 *
 * ⚠⚠ A REGRA MUDOU EM 26/09/2026, E A ANTIGA ERA PERIGOSA. Antes ficava a MAIS RECENTE, e são
 * QUATRO cópias no servidor — medido: `000-26-CLIENTE-OBRA`, `291-26-TESTE-TESTE`,
 * `117-26-TAKRAF-PACOTE-2` e `235-26-MEGASTEAM-UNIPAR-ADITIVO-01`. As três últimas são cópias de
 * trabalho DENTRO da pasta de uma obra: bastava alguém mexer numa delas para ela virar "a mais
 * recente" e o portal passar a exportar o estudo em cima da planilha da TAKRAF. Só dá para
 * distinguir porque a varredura por pasta traz o CAMINHO — a busca do Graph não trazia.
 *
 * ⚠ A preferida é a que está na pasta do próprio modelo (`000-…`). Sem nenhuma lá, volta a regra
 * antiga (a mais recente) em vez de recusar: exportar com uma cópia é ruim, não exportar é pior.
 */
export function escolherModelo(arquivos) {
  const candidatos = (arquivos || []).filter((a) => NOME_MODELO.test(a.nome || ""));
  if (!candidatos.length) return null;
  const naPastaDoModelo = candidatos.filter((a) => /\/0+-\d{2}[-\s]/.test(String(a.caminho || "")));
  const lista = (naPastaDoModelo.length ? naPastaDoModelo : candidatos)
    .slice().sort((a, b) => new Date(b.modificado || 0) - new Date(a.modificado || 0));
  return { escolhido: lista[0], total: candidatos.length, fora: !naPastaDoModelo.length };
}

/**
 * Baixa (e guarda) o modelo em branco do servidor.
 *
 * ⚠⚠ ERA `root/search(q='LQC-000-00')`, e a busca deste drive devolve HTTP 500 desde 22–23/09/2026
 * — criar LQC pelo portal parou de funcionar e ninguém foi avisado, porque isto é tela e não cron.
 * Agora vem da mesma varredura de pastas do importador. Ver `lib/lqc-sharepoint.js`.
 */
export async function baixarModeloLqc(forcar = false) {
  ...
  // ⚠⚠ A PASTA DO MODELO PRIMEIRO: ~8 chamadas em vez das 659 da varredura inteira, que levava
  // 47 s — tempo demais para um clique de "exportar planilha", que tem 60 s de orçamento. A
  // varredura completa fica de reserva, para o dia em que a pasta `000-…` mudar de nome.
  let { arquivos } = await varrerPastasDeLqc(ano, { somentePastas: EH_PASTA_DO_MODELO });
  let achado = escolherModelo(arquivos);
  if (!achado) {
    ({ arquivos } = await varrerPastasDeLqc(ano));
    achado = escolherModelo(arquivos);
  }
  if (!achado) throw new Error("Modelo LQC-000-00-CLIENTE-OBRA-TORG-R00.xlsx não encontrado no servidor.");
  const { escolhido, total, fora } = achado;
  const r = await fetch(`https://graph.microsoft.com/v1.0/drives/${drive}/items/${escolhido.id}/content`, {
    headers: { Authorization: `Bearer ${token}` }, redirect: "follow",
  });
  if (!r.ok) throw new Error(`Download do modelo LQC: HTTP ${r.status}`);
  cacheModelo = {
    em: Date.now(), buffer: Buffer.from(await r.arrayBuffer()), nome: escolhido.nome,
    // ⚠ QUAL FOI USADO VIAJA NO RESULTADO E APARECE NA TELA: escolher em silêncio entre cópias
    // que não são iguais (uma tem 84 partes, outra 82) é como o preço de uma proposta muda sem
    // ninguém saber por quê. `fora` avisa quando nem a pasta do modelo tinha uma.
    caminho: escolhido.caminho || "",
    modificadoEm: escolhido.modificado,
    duplicados: total > 1 ? total : 0,
    foraDaPastaDoModelo: fora,
  };
  return cacheModelo;
}
...
export async function gerarPlanilhaLqc(estudo) {
  ...
  const avisos = [];
  // ⚠ Nenhuma cópia na pasta do modelo: a planilha saiu de dentro da pasta de uma obra, e pode
  // trazer dado dela. Fica em `avisos` e não em `modelo`, porque `avisos` é o que a tela mostra.
  if (modelo.foraDaPastaDoModelo) {
    avisos.push(`O modelo foi lido de "${modelo.caminho}", que não é a pasta do modelo em branco — confira a planilha antes de enviar.`);
  }
  ...
  // Composições livres não cabem na grade fixa do modelo. A memória completa viaja em abas próprias.
  if (c.comerciaisCompra?.versao === 1 || (c.tintas || []).some(t => t.estruturaEscopo != null || t.areaEscopo != null)) {
    adicionarAba(zip, "RESUMO DO PORTAL", [
      ["LQC · VALORES DO ESCOPO ATUAL"], [estudo.obra || estudo.cliente || ""],
      ["Memória do portal; valores calculados na emissão. As abas do modelo conservam suas fórmulas originais."],
      ["Indicador", "Valor"], ["Peso (kg)", res.pesoTotal], ["Área de pintura (m²)", res.areaM2],
      ["Custo total (R$)", res.custo], ["BDI (R$)", res.bdiValor], ["Preço de venda (R$)", res.preco],
      ["Impostos (R$)", res.totalImpostos], ["Itens comerciais (R$)", res.totais.comerciais],
      ["Tintas e diluentes (R$)", res.grupos.tintas.total.subtotal],
    ]);
    adicionarAba(zip, "COMERCIAIS DETALHADOS", [
      ["ITENS COMERCIAIS · PRODUTOS E COMPLEMENTOS"], [estudo.obra || estudo.cliente || ""],
      ["Família", "Item / modelo", "Tipo", "Unidade", "Quantidade", "Preço unitário (R$)", "Subtotal (R$)"],
      ...res.itensComerciais.map(i => [i.familia || i.key, i.rotulo || i.nome, i.tipo || "Produto", i.un, i.qtd, i.preco, i.subtotal]),
      ["TOTAL", "", "", "", "", "", res.totais.comerciais],
    ]);
    const areas = (c.resumos || []).filter(l => l.ativo !== false);
    adicionarAba(zip, "DEMAOS DETALHADAS", [
      ["PINTURA · PRODUTOS POR DEMÃO"], [estudo.obra || estudo.cliente || ""],
      ["Demão", "Produto", "Áreas e estruturas", "Cor", "Área (m²)", "Sólidos (%)", "Película (µm)", "Perda (%)", "Tinta (L)", "Diluente (L)", "Estimativa por consumo (R$)"],
      ...calcularCamadasPintura(c).map((t,i) => [i+1, t.produto || t.camada, t.indices.map(idx => `${areas[idx].area || areas[idx].item || ""} · ${areas[idx].estruturaNome || areas[idx].estrutura || ""}`).join("; "), t.cor || "", t.areaM2, numeroBr(t.solidos), numeroBr(t.peliculaSeca), numeroBr(t.perda ?? 45), t.litros, t.litrosDiluente, t.total]),
    ]);
    avisos.push("Composição livre: consulte RESUMO DO PORTAL, COMERCIAIS DETALHADOS e DEMAOS DETALHADAS. A grade fixa do modelo original não representa todos os modelos e destinos livres.");
  }
  ...
  return { buffer: fecharXlsx(zip), nome: `${nome}.xlsx`, avisos,
    // ⚠ `foraDaPastaDoModelo` viaja junto: se o modelo veio da pasta de uma obra em vez da
    // pasta do modelo, quem exportou precisa saber ANTES de mandar a planilha ao cliente.
    modelo: { nome: modelo.nome, caminho: modelo.caminho, modificadoEm: modelo.modificadoEm, duplicados: modelo.duplicados, foraDaPastaDoModelo: modelo.foraDaPastaDoModelo } };
}
