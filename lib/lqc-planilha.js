...
import { calcularLqc, cenarioFinanceiro, BDI_CAMPOS, LINHAS_FATURAMENTO, ITENS_COMERCIAIS, CFOPS } from "./lqc";
...
export async function gerarPlanilhaLqc(estudo) {
  ...
  // ── INDUSTRIALIZAÇÃO: só faturamento e os preços que a planilha NÃO calcula sozinha.
  {
    ...
    for (const addr of IND.preMontagem) vals[addr] = estudo.preMontagem || "N/A";
  ...
  try {
    const res = calcularLqc({ ...c, demaos: estudo.demaos, preMontagem: estudo.preMontagem });
    const cen = cenarioFinanceiro(res, estudo.cenario || {});
    const linhas = [
      ["CENÁRIO FINANCEIRO"],
      ...
      ["Margem bruta", cen.margemBruta],
      ["Margem líquida", cen.margemLiquida],
      ["Margem líquida (%)", cen.margemLiquidaPct],
      [],
      ["FLUXO DE CAIXA"],
