...
/** A pasta do modelo em branco tem número todo-zero ("000-26-CLIENTE-OBRA") — "001-26" não conta. */
export const EH_PASTA_DO_MODELO = (nome) => /^0+-\d{2}(?![0-9])/.test(String(nome || "").trim());
 ...
 *
 * @param {object} [deps]
 * @param {(nomeDaPasta: string) => boolean} [deps.somentePastas] limita quais pastas de orçamento visitar
 * @returns {Promise<{arquivos: Array, semEstudos: Array<string>, pastas: number}>}
 */
export async function varrerPastasDeLqc(ano, { get, driveId, somentePastas } = {}) {
  ...
  // ⚠ `somentePastas` estreita a varredura quando já se sabe QUAL pasta de orçamento interessa —
  // é o que faz o modelo em branco custar ~8 chamadas em vez das 659 da varredura inteira.
  const interessa = typeof somentePastas === "function" ? somentePastas : () => true;
  const pastasOrc = (await emLotes(pastasDeGrupo, ler))
    .flatMap(({ dir, itens }) => (itens || []).filter((x) => x.folder && interessa(x.name)).map((p) => `${dir}/${p.name}`));
