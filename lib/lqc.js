...
/** Litros e custo de uma camada, dada a área que ela cobre. */
export function custoCamada(t, areaM2) {
  const { teorico, pratico } = rendimentoTinta(t);
  const area = n(areaM2);
  const litros = pratico > 0 ? area / pratico : 0;
  const total = litros * n(t?.precoLitro);
  return { teorico, pratico, litros: r2(litros), total: r2(total) };
}
...
export function calcularLqc(c = {}) {
  ...
  // ⚠ o custo por kg da tinta pode vir digitado OU ser calculado do rendimento — e calculado é
  // melhor, porque acompanha mudança de espessura e de preço do litro sem ninguém lembrar.
  const tintas = (c.tintas || []).map((t) => {
    const pesoTinta = n(t.pesoKg) > 0 ? n(t.pesoKg) : pesoTotal;
    const areaCamada = n(t.areaM2) > 0 ? n(t.areaM2) : areaM2;
    const calc = custoCamada(t, areaCamada);
    const precoKg = n(t.precoKg) > 0
      ? n(t.precoKg)
      : (pesoTinta > 0 ? Math.round((calc.total / pesoTinta) * 10000) / 10000 : 0);
    const li = linha(t.nome || "ESTRUTURA", pesoTinta, precoKg, fat.tintas, "material", t.perda ? `perda ${t.perda}%` : null);
    return { ...li, litros: calc.litros, rendimento: calc.pratico, rendimentoTeorico: calc.teorico, areaM2: r2(areaCamada) };
  });
  ...
  const terceirizados = listaTerceiros.map((t) => {
    ...
    const qtd = base === "kg" ? pesoTotal : base === "m2" ? areaM2 : (n(t.quantidade) || 1);
    const sug = TERCEIROS_SUGESTOES.find((x) => x.chave === t.chave);
    const l = linha(t.descricao || sug?.descricao || "Terceiro", qtd, t.precoUnit, t.faturamento, "servico", BASES_TERCEIRO[base]);
