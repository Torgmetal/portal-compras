import { calcularItensComerciais } from "./lqc-itens-comerciais";
...
export function custoCamada(t, areaM2) {
  ...
  const area = n(areaM2);
  // O rendimento exibido tem duas casas; a conta mantém a precisão da planilha.
  const rendimentoExato = n(t?.solidos) > 0 && n(t?.peliculaSeca) > 0
    ? n(t.solidos) * 10 / n(t.peliculaSeca) * Math.max(0.05, 1 - n(t.perda) / 100) : 0;
  const litros = rendimentoExato > 0 ? area / rendimentoExato : 0;
...
export const ENSAIOS = [
  ...
  // ── INSPETOR NÃO É ENSAIO: É GENTE, E SE COBRA DE TRÊS JEITOS ────────────────────────────────
  // Vitor (05/09/2026): "vamos colocar Inspetor de pintura e dar as opções de colocar por dia, kg
  // ou vb; Inspetor Dimensional N1 SNQC, mesmo caso dos valores; Inspetor Visual de solda SNQC,
  // mesmo caso".
  //
  // ⚠ O ensaio se cobra por AMOSTRA (um a cada tantos kg ou m²). O inspetor se cobra por DIÁRIA
  // quando ele fica na obra, por kg quando entra no preço da estrutura, ou por VERBA quando o
  // cliente exige um profissional dedicado ao contrato. São três negociações diferentes com o
  // mesmo nome, e obrigar as três a caber em "1 a cada X kg" foi o que manteve isso fora do
  // estudo até agora — e o custo do inspetor ia parar no BDI, sem aparecer.
  { key: "INSP_PINTURA", nome: "Inspetor de pintura", norma: "NBR 15239 / plano de pintura", base: "dia", cada: 0, custo: 0, pessoa: true },
  { key: "INSP_DIMENSIONAL", nome: "Inspetor Dimensional N1 SNQC", norma: "SNQC / ABENDI", base: "dia", cada: 0, custo: 0, pessoa: true },
  { key: "INSP_VISUAL_SOLDA", nome: "Inspetor Visual de solda SNQC", norma: "SNQC / ABENDI · AWS D1.1", base: "dia", cada: 0, custo: 0, pessoa: true },
];
export const BASES_ENSAIO = {
  kg: "por kg de estrutura",
  m2: "por m² de pintura",
  dia: "por dia de inspeção",
  vb: "verba — valor fechado",
};
...
export function calcularEnsaios(cfg = {}, pesoKg = 0, areaM2 = 0) {
  const linhas = ENSAIOS.map((e) => {
    ...
    // ⚠ TRÊS CONTAS DIFERENTES no mesmo quadro:
    //   kg/m² → amostragem: um ensaio a cada X (arredonda PRA CIMA, meio ensaio não existe, e a
    //           obra que passa de 500 m² por 1 m² paga o segundo do mesmo jeito);
    //   dia   → diárias: quem digita a quantidade é quem orça, não sai de kg nenhum;
    //   vb    → verba: uma unidade, o custo é o valor fechado.
    const porAmostra = base === "kg" || base === "m2";
    const universo = base === "m2" ? n(areaM2) : porAmostra ? n(pesoKg) : n(c.qtd);
    const qtd = !ativo ? 0
      : base === "vb" ? 1
      : porAmostra ? (cada > 0 ? Math.ceil(universo / cada) : 0)
      : Math.ceil(n(c.qtd));
...
export function cargasPorClasse(pesoPorClasse = {}, capacidades = {}) {
  // ⚠⚠ O PESO SEM CLASSE RECONHECIDA NÃO PODE SUMIR DO FRETE. Auditoria de 05/09/2026: a LQC-186
  // (Orca) tem 7 linhas classificadas como "ESPECIAL" — 38 t que não existem em CLASSES. Elas não
  // entravam em carga nenhuma e o frete saía subdimensionado, sem nada na tela dizendo por quê.
  // Cinco estudos no mesmo caso; a LQC-289 tinha 12 linhas sem classe e ZERO cargas com 2.979 kg.
  // Aqui esse peso vira uma linha própria, com a capacidade do Médio, e sai marcado para a tela
  // cobrar a classificação — contar por baixo é pior que contar por uma média declarada.
  const nomesValidos = new Set(CLASSES.map((c) => c.nome.toUpperCase()));
  const pesoSemClasse = Object.entries(pesoPorClasse || {})
    .filter(([k]) => !nomesValidos.has(String(k).toUpperCase()))
    .reduce((a, [, v]) => a + n(v), 0);
  const rotulosSemClasse = Object.keys(pesoPorClasse || {}).filter((k) => !nomesValidos.has(String(k).toUpperCase()));
  ...
  if (pesoSemClasse > 0) {
    const cap = n(capacidades.MEDIO) > 0 ? n(capacidades.MEDIO) : CAPACIDADE_CARGA.MEDIO;
    const cargas = Math.ceil(pesoSemClasse / cap);
    linhas.push({
      key: "SEM_CLASSE", nome: "Sem classificação", faixa: rotulosSemClasse.join(", ") || "não informada",
      pesoKg: r2(pesoSemClasse), capacidadeKg: cap, cargas,
      ultimaCargaKg: r2(pesoSemClasse - (cargas - 1) * cap),
      semClasse: true,
    });
  }
  return {
    ...
    // o que a tela precisa para cobrar a classificação em vez de deixar passar
    pesoSemClasse: r2(pesoSemClasse),
    rotulosSemClasse,
  };
}
...
// ─── EMBALAGEM (MADEIRA) ──────────────────────────────────────────────────────────────────
//
// Vitor (12/09/2026): "vamos colocar essa parte do material de embalagem no portal (…) pois isso
// pode impactar bem no preço" — depois de medir no simulador de carga o que cada padrão de
// embalagem custa em madeira na OP-118, entregue por nível:
//   econômica 0,14 · padrão 0,18 · reforçada 0,29 · especificada 0,63 R$/kg de aço.
// A Econômica é o REAL de 2026 (R$ 63,5 mil de madeira HM ÷ 464 t expedidas); as outras três são
// o simulador com madeira ao preço da HM (jul–ago/26), a Especificada já com a fumigação (×1,35)
// que a spec TMSA/Vale exige. Calibrar conforme as obras saírem com romaneio.
//
// ⚠ Entra DILUÍDA no R$/kg da estrutura, do lado Torg — madeira é compra nossa. Estudo sem nível
// escolhido fica SEM madeira na composição (zero), e a aba avisa; estudo novo nasce como Padrão.
export const NIVEIS_EMBALAGEM = [
  { key: "ECONOMICA", nome: "Embalagem Econômica", rsKg: 0.14, quando: "obra sem exigência: pacote cintado, calço e caibro, sem proteção de pintura", fonte: "real de 2026: madeira comprada na HM ÷ toneladas expedidas" },
  { key: "PADRAO", nome: "Embalagem Padrão", rsKg: 0.18, quando: "a recomendada: guarda-corpo em engradado, cantoneira sob a cinta na peça pintada, miúdo em caixa de madeira", fonte: "simulador de carga, OP-118 por nível, perfil recomendado" },
  { key: "REFORCADA", nome: "Embalagem Reforçada", rsKg: 0.29, quando: "cliente exigente: tudo embalado, engradado para chapa e guarda-corpo, manta na pintura", fonte: "simulador de carga, perfil exigente" },
  { key: "ESPECIFICADA", nome: "Embalagem Especificada", rsKg: 0.63, quando: "especificação escrita do cliente (Vale, TMSA): engradado de lados abertos, caixa sobre palete, madeira fumigada com certificado", fonte: "simulador de carga, perfil Vale/TMSA (espec. TPR00864)" },
];
export const EMBALAGEM_POR_KEY = Object.fromEntries(NIVEIS_EMBALAGEM.map((e) => [e.key, e]));

/** Madeira de embalagem do escopo: nível escolhido × peso. `rsKg` digitado sobrepõe a tabela. */
export function calcularEmbalagem(cfg = {}, pesoKg = 0) {
  const nivel = EMBALAGEM_POR_KEY[String(cfg?.nivel || "").toUpperCase()] || null;
  const rsKg = nivel ? (cfg.rsKg != null && cfg.rsKg !== "" ? n(cfg.rsKg) : nivel.rsKg) : 0;
  return { nivel: nivel?.key || null, nome: nivel?.nome || "Não informada", rsKg: r2(rsKg), tabelaRsKg: nivel?.rsKg ?? 0, porKg: rsKg, total: r2(n(pesoKg) * rsKg) };
}
...
/** A identidade do tipo acompanha o nome usado pelo cliente sem alterar sua classe técnica. */
export const chaveEstrutura = (l) => l.estruturaTipoId || l.estrutura || "";
export const chaveAreaPintura = (l, i) => l.id || `linha:${i}`;
export const pinturaComDestino = (t) => t.estruturaEscopo != null || t.areaEscopo != null;
const grupoHistoricoDaDemao = (t) => t?.grupoPerda != null ? Number(t.grupoPerda)
  : t && !pinturaComDestino(t) ? Number(t.perda ?? 45) : null;

export function custoHistoricoDaDemao(tintas, indice) {
  const grupo = grupoHistoricoDaDemao(tintas[indice]);
  if (grupo == null) return 0;
  return n(tintas.find((t) => grupoHistoricoDaDemao(t) === grupo && n(t.precoKg) > 0)?.precoKg);
}

/** A troca é explícita: retira todos os preços fechados desse grupo, mantendo o sistema técnico. */
export function usarConsumoDaDemao(tintas, indice) {
  const grupo = grupoHistoricoDaDemao(tintas[indice]);
  if (grupo == null) return tintas;
  return tintas.map((t) => grupoHistoricoDaDemao(t) === grupo ? { ...t, precoKg: null } : t);
}

/** Editar uma demão importada conserva o preço fechado do grupo, quando existente. */
export function atualizarDemao(tintas, indice, patch) {
  const alvo = tintas[indice] || {};
  const mudaDestino = ["perda", "estruturaEscopo", "areaEscopo", "areaM2"].some((k) => Object.hasOwn(patch, k));
  const migrar = mudaDestino && !pinturaComDestino(alvo);
  const perda = Number(alvo.perda ?? 45);
  const grupoFechado = migrar && tintas.some((t) => !pinturaComDestino(t) && Number(t.perda ?? 45) === perda && n(t.precoKg) > 0);
  return tintas.map((t, i) => {
    const mesmoGrupo = !pinturaComDestino(t) && Number(t.perda ?? 45) === perda;
    const base = migrar && (i === indice || (grupoFechado && mesmoGrupo))
      ? { ...t, estruturaEscopo: `legado:${perda}`, ...(grupoFechado ? { grupoPerda: perda } : {}) }
      : t;
    return i === indice ? { ...base, ...patch } : base;
  });
}

export function estruturasDoQuantitativo(c) {
  const tipos = new Map();
  for (const l of (c.resumos || []).filter((l) => l.ativo !== false)) {
    const id = chaveEstrutura(l);
    if (!id) continue;
    if (!tipos.has(id)) tipos.set(id, { id, nome: l.estruturaNome || ESTRUTURA_ROTULO[l.estrutura] || l.estrutura, areas: [] });
    const area = l.area || l.item || "Área não informada";
    if (!tipos.get(id).areas.includes(area)) tipos.get(id).areas.push(area);
  }
  return [...tipos.values()];
}

/** Uma entrada por demão, na ordem original. Estudos sem destino mantêm as regras importadas. */
export function calcularCamadasPintura(c) {
  const areas = (c.resumos || []).map((l, i) => ({ ...l, chaveArea: chaveAreaPintura(l, i) })).filter((l) => l.ativo !== false);
  const norm = (v) => String(v || "").trim().toUpperCase();
  const areaDe = (l) => n(l.areaM2) > 0 ? n(l.areaM2) : (n(l.coef) > 0 ? n(l.coef) : coefSugerido(l.perfil)) * n(l.pesoTotal ?? n(l.quantidade) * n(l.unidades || 1) * n(l.pesoUnit));
  return (c.tintas || []).map((t, indiceOriginal) => {
    const novo = pinturaComDestino(t);
    const indices = areas.flatMap((l, i) => {
      const destino = novo
        ? ((!t.estruturaEscopo || t.estruturaEscopo === "todas" || chaveEstrutura(l) === t.estruturaEscopo || t.estruturaEscopo === `legado:${perdaDaLinha(l)}`)
          && (!t.areaEscopo || t.areaEscopo === "todas" || l.chaveArea === t.areaEscopo))
        : Number(t.perda ?? 45) === perdaDaLinha(l);
      const cor = norm(t.camada) !== "ACABAMENTO" || ((!novo || Boolean(norm(t.cor))) && norm(t.cor) === norm(l.cor));
      return destino && cor ? [i] : [];
    });
    const areaQuantitativo = indices.reduce((v, i) => v + areaDe(areas[i]), 0);
    const areaM2 = novo && !indices.length ? 0 : n(t.areaM2) > 0 ? n(t.areaM2) : areaQuantitativo;
    const calc = custoCamada({ ...t, perda: t.perda ?? 45 }, areaM2);
    // Rateio em centavos/litros: a última área recebe o resíduo, sem criar consumo a mais.
    const distribuicao = {};
    for (const campo of ["litros", "litrosDiluente", "total"]) {
      let anterior = 0, acumulada = 0;
      indices.forEach((i) => {
        acumulada += areaDe(areas[i]);
        const ateAqui = areaQuantitativo > 0 ? r2(calc[campo] * acumulada / areaQuantitativo) : 0;
        distribuicao[i] ||= {};
        distribuicao[i][campo] = r2(ateAqui - anterior);
        anterior = ateAqui;
      });
    }
    return { ...t, ...calc, areaM2: r2(areaM2), areaQuantitativo: r2(areaQuantitativo), indiceOriginal, indices, distribuicao };
  });
}

/** Referência das demãos importadas enquanto o grupo conserva o preço fechado.
 * Não atribui cores às áreas nem altera o total financeiro da LQC.
 */
export function calcularCamadasPinturaConsulta(c) {
  return calcularCamadasPintura(c).map((calc, i) => {
    const t = c.tintas[i];
    if (pinturaComDestino(t) || n(t.areaM2) > 0 || n(t.areaImportada) <= 0
      || custoHistoricoDaDemao(c.tintas, i) <= 0) return calc;
    const grupo = (c.resumos || []).filter(l => perdaDaLinha(l) === Number(t.perda ?? 45));
    const superficie = l => n(l.areaM2) > 0 ? n(l.areaM2)
      : (n(l.coef) || coefSugerido(l.perfil)) * n(l.pesoTotal ?? n(l.quantidade) * n(l.unidades || 1) * n(l.pesoUnit));
    const total = grupo.reduce((s,l) => s + superficie(l), 0);
    const ativa = grupo.filter(l => l.ativo !== false).reduce((s,l) => s + superficie(l), 0);
    const area = total > 0 ? n(t.areaImportada) * ativa / total : 0;
    return { ...calc, ...custoCamada(t, area), areaM2:r2(area), referenciaImportada:true };
  });
}
...
export function calcularLqc(c = {}) {
  ...
  for (const l of resumos) {
    ...
    // ⚠⚠ LINHA SEM CLASSIFICAÇÃO TAMBÉM TEM PESO, E TAMBÉM PRECISA DE CARRETA. Auditoria de
    // 05/09/2026: a LQC-289 tinha 12 linhas em branco e saía com ZERO cargas para 2.979 kg — o
    // peso nem chegava ao cálculo do frete, porque só entrava aqui quem tinha classe. Guardar sob
    // um rótulo próprio faz o peso aparecer no quadro de cargas e cobrar a classificação.
    pesoPorClasse[cl && cl !== "N/A" ? cl : "(sem classificação)"] =
      (pesoPorClasse[cl && cl !== "N/A" ? cl : "(sem classificação)"] || 0) + kg;
  ...
  const camadasCalculadas = calcularCamadasPintura(c);
  const novasCamadas = camadasCalculadas.filter(pinturaComDestino);
  const temDestino = novasCamadas.length > 0;
  const camadasDe = (perda) => (c.tintas || []).filter((t) => !pinturaComDestino(t) && Number(t?.perda ?? 45) === perda);
  ...
  const custosFechadosPorArea = new Map();
  if (temDestino) {
    const pesoDaArea = (i) => n(resumos[i].pesoTotal ?? n(resumos[i].quantidade) * n(resumos[i].unidades || 1) * n(resumos[i].pesoUnit));
    const fechados = new Set(novasCamadas.filter((t) => t.grupoPerda != null && n(t.precoKg) > 0).map((t) => Number(t.grupoPerda)));
    const lotes = new Map();
    for (const t of novasCamadas) {
      const chave = t.grupoPerda != null && fechados.has(Number(t.grupoPerda)) ? `legado:${t.grupoPerda}` : "produtos";
      if (!lotes.has(chave)) lotes.set(chave, []);
      lotes.get(chave).push(t);
    }
    for (const [chave, camadas] of lotes) {
      const indices = [...new Set(camadas.flatMap((t) => t.indices))];
      const kg = indices.reduce((v, i) => v + pesoDaArea(i), 0);
      const manual = chave !== "produtos" ? camadas.find((t) => n(t.precoKg) > 0) : null;
      const subtotal = manual ? r2(kg * n(manual.precoKg)) : r2(camadas.reduce((v, t) => v + t.total, 0));
      const l = linha(manual ? "Custo histórico de produtos" : "Produtos por demão", kg, kg > 0 ? subtotal / kg : 0, fat.tintas);
      // Área pode existir sem peso informado: produtos são cobrados por consumo.
      l.subtotal = subtotal;
      l.icmsPct = 0.18;
      l.icms = String(fat.tintas || "").toUpperCase() === "TORG" ? r2(subtotal * 0.18) : 0;
      l.pisCofins = String(fat.tintas || "").toUpperCase() === "TORG" ? r2(subtotal * l.pisCofinsPct) : 0;
      tintas.push({ ...l, camadas, litros: r2(camadas.reduce((v, t) => v + t.litros, 0)), areaM2: r2(indices.reduce((v, i) => v + (n(resumos[i].areaM2) > 0 ? n(resumos[i].areaM2) : (n(resumos[i].coef) || coefSugerido(resumos[i].perfil)) * pesoDaArea(i)), 0)) });
      if (manual) {
        let pesoAcumulado = 0, custoAnterior = 0;
        for (const i of indices) {
          pesoAcumulado += pesoDaArea(i);
          const custoAteAqui = kg > 0 ? r2(subtotal * pesoAcumulado / kg) : 0;
          const custoArea = r2(custoAteAqui - custoAnterior);
          custoAnterior = custoAteAqui;
          const aplicadas = camadas.filter((t) => t.indices.includes(i));
          const totalCalculado = aplicadas.reduce((v, t) => v + t.distribuicao[i].total, 0);
          let acumulado = 0, anterior = 0;
          for (const [j, t] of aplicadas.entries()) {
            acumulado += totalCalculado > 0 ? t.distribuicao[i].total / totalCalculado : 1 / aplicadas.length;
            const ateAqui = j === aplicadas.length - 1 ? custoArea : r2(custoArea * acumulado);
            custosFechadosPorArea.set(`${i}:${t.indiceOriginal}`, r2(ateAqui - anterior));
            anterior = ateAqui;
          }
        }
      }
  ...
  const demaosPorArea = temDestino ? resumos.map((_, i) => {
    const aplicadas = camadasCalculadas.filter((t) => t.indices.includes(i) && t.camada && String(t.camada).toUpperCase() !== "N/A");
    const novas = aplicadas.filter((t) => pinturaComDestino(t) && t.grupoPerda == null);
    const historicas = aplicadas.filter((t) => !pinturaComDestino(t) || t.grupoPerda != null);
    return Math.max(1, novas.length + new Set(historicas.map((t) => String(t.camada).trim().toUpperCase())).size);
  }) : [];
  const nDemaos = temDestino ? Math.max(1, ...demaosPorArea) : Math.max(1, demaosDeTintas(c.tintas));
  // Até três demãos usa a tabela; acima dela, cada extra soma metade da primeira.
  const precoPintura = (cl, demaos = nDemaos) => demaos <= 3
    ? cl.demaos[Math.max(0, Math.min(2, demaos - 1))]
    : precoPinturaPorDemaos(cl.demaos[0], demaos);
  const precoPinturaDaArea = (cl, l) => n(c.precos?.classe?.[cl.key]?.pintura
    ?? precoPintura(cl, demaosPorArea[resumos.indexOf(l)] || 1));
  ...
  const pintura = porClasse("pintura").map((x) => {
    const custoPorDestino = temDestino ? resumos.filter((l) => String(l.classificacao || "").toUpperCase() === x.cl.nome.toUpperCase())
      .reduce((v, l) => v + n(l.pesoTotal ?? n(l.quantidade) * n(l.unidades || 1) * n(l.pesoUnit)) * precoPinturaDaArea(x.cl, l), 0) : null;
    const preco = temDestino && x.kg > 0 ? custoPorDestino / x.kg : x.preco;
    return linha(x.cl.nome, x.kg, preco, "TORG", "material", temDestino ? "Demãos por destino" : `${nDemaos} ${nDemaos === 1 ? "demão" : "demãos"}`);
  });
  ...
  if (temDestino) {
    // O peso representa as estruturas uma única vez, independentemente do número de demãos.
    const indices = [...new Set(camadasCalculadas.flatMap((t) => t.indices))];
    grupos.tintas.total.pesoKg = r2(indices.reduce((v, i) => v + n(resumos[i].pesoTotal ?? n(resumos[i].quantidade) * n(resumos[i].unidades || 1) * n(resumos[i].pesoUnit)), 0));
  }
  ...
  const pinturaPorArea = resumos.map((l, indiceArea) => {
    ...
    for (const t of novasCamadas.filter((t) => t.indices.includes(indiceArea))) {
      const d = t.distribuicao[indiceArea];
      camadas.push({ id: t.id, camada: t.camada, produto: t.produto || null, cor: t.cor || null,
        solidos: n(t.solidos) || null, peliculaSeca: n(t.peliculaSeca) || null,
        rendimento: t.pratico, litros: d.litros, litrosDiluente: d.litrosDiluente, custo: custosFechadosPorArea.get(`${indiceArea}:${t.indiceOriginal}`) ?? d.total });
    }
    return {
      ...(temDestino ? {item:l.item, estruturaNome:l.estruturaNome || ESTRUTURA_ROTULO[l.estrutura] || l.estrutura} : {}),
  ...
  // ── EMBALAGEM (madeira) — sempre do lado Torg, diluída no R$/kg ──
  const embalagem = calcularEmbalagem(c.embalagem || {}, pesoTotal);
  ...
  const comerciais = calcularItensComerciais(c, ITENS_COMERCIAIS, areasAtivas, { escopoParcial: resumos.length < todas.length });
  const totalComerciais = r2(comerciais.reduce((a, i) => a + i.subtotal, 0));

  // ⚠ a montagem vem DEPOIS dos itens comerciais porque cobra POR ITEM (m² de telha, m de calha),
  // e não só por quilo de estrutura. Ver `calcularMontagem`.
  const montagem = calcularMontagem(c.montagem || {}, { pesoKg: pesoTotal, comerciais });
  ...
  // ⚠ mas NÃO entram na base do BDI: eles têm margem própria (ver MARGEM_COMERCIAIS_PADRAO).
  ...
  // ⚠ montagem em campo é serviço da Torg — mão de obra, canteiro e equipamentos. Nunca é DIRETO:
  // se o cliente monta por conta, a obra não tem o bloco (é o `ativo` do próprio cadastro).
  custoTorg += montagem.total;
  ...
  // Composições novas usam BDI. Estudos antigos conservam sua margem separada até a migração.
  const comerciaisNoTorg = ehDireto("itensComerciais") ? 0 : totalComerciais;
  const comerciaisBdi = c.comerciaisCompra?.versao === 1;
  const margemComerciaisPct = comerciaisBdi ? 0 : c.margemComerciaisPct == null || c.margemComerciaisPct === ""
    ? MARGEM_COMERCIAIS_PADRAO
    : n(c.margemComerciaisPct);
  const margemComerciais = r2(comerciaisNoTorg * (margemComerciaisPct / 100));
  const baseBdi = r2(custoTorg - (comerciaisBdi ? 0 : comerciaisNoTorg));
  const bdiValor = r2(baseBdi * bdi);
  const preco = r2(custoTorg + bdiValor + margemComerciais + custoDireto);
  ...
  return {
    ...
    porArea: (() => {
      const tintaPorArea = temDestino
        ? pinturaPorArea.reduce((m, x) => ({ ...m, [x.area]: r2((m[x.area] || 0) + x.custo) }), {})
        : Object.fromEntries(pinturaPorArea.map((x) => [x.area, x.custo]));
      ...
      const embalagemKg = embalagem.porKg; // madeira rateia por kg em toda área ativa, como o frete diluído
      ...
      for (const l of todas) {
        ...
        g.fabricacao += kg * n(c.precos?.classe?.[cl?.key]?.fabricacao ?? cl?.fabricacao);
        g.pintura += kg * n(c.precos?.classe?.[cl?.key]?.pintura ?? (cl ? (temDestino ? precoPinturaDaArea(cl, l) : precoPintura(cl)) : 0));
        g.preMont += kg * n(c.precos?.classe?.[cl?.key]?.preMont ?? (cl ? precoPreMontagem(cl, pctPre) : 0));
        mapa.set(chave, g);
      }
      // ── ⚠⚠ O QUE NÃO TEM ÁREA RATEIA POR PESO — INCLUSIVE O AÇO ────────────────────────────
      //
      // Matheus (Comercial, 05/09/2026), sobre a planilha do estudo TESTE: "a soma dos valores não
      // está batendo". Subtotal das áreas R$ 143.742,34 + transporte R$ 39.491,20 dava R$ 183.233,54
      // contra um Total geral de R$ 279.866,57 — R$ 96.633 sem explicação na tela.
      //
      // A causa: a matéria-prima tem DOIS modos. Quando alguém digita R$/kg na linha do
      // quantitativo, o aço é da área e entra em `g.aco`. Quando ninguém digita, o estudo precifica
      // POR CATEGORIA DE PERFIL (chapa, viga, tubo…) — e aí o aço não pertence a área nenhuma:
      // `g.aco` ficava zero e R$ 68.512 de aço sumiam da planilha comercial, junto de tudo o mais
      // que fosse lançado sem área.
      //
      // A regra agora é a que o rodapé da tela sempre prometeu: o R$/kg da área carrega TUDO, e o
      // que não tem dono rateia por peso — como fixador e ensaio já faziam. Em vez de listar o que
      // rateia (e esquecer o próximo), calcula-se o RESTO: total do grupo menos o que já foi
      // atribuído a alguma área. Assim a soma fecha por construção, qualquer que seja o modo.
      const areas = [...mapa.values()];
      const somaAtiva = (f) => areas.reduce((a, g) => a + (g.ativo ? f(g) : 0), 0);
      const materialSemDono = material.subtotal - somaAtiva((g) => g.aco) - somaAtiva((g) => n(tintaPorArea[g.area]));
      const indDistribuivel = industrializacao.subtotal - (preMontSeparada ? grupos.preMontagem.total.subtotal : 0);
      const indSemDono = indDistribuivel - somaAtiva((g) => g.fabricacao + g.pintura + (preMontSeparada ? 0 : g.preMont));
      const materialKg = pesoTotal > 0 ? materialSemDono / pesoTotal : 0;
      const indKg = pesoTotal > 0 ? indSemDono / pesoTotal : 0;

      return areas.map((g) => {
        // ⚠ área fora do escopo não recebe rateio: ela não consome fixador, ensaio nem frete.
        const rateio = g.ativo ? g.pesoKg : 0;
        const material = r2(g.aco + (g.ativo ? n(tintaPorArea[g.area]) : 0) + rateio * materialKg);
        const terceiros = r2((g.ativo ? n(terceiroDaArea[g.area]) : 0) + rateio * terceiroKg + rateio * ensaiosKg + rateio * freteKg + rateio * embalagemKg);
        const industrializacao = r2(g.fabricacao + g.pintura + (preMontSeparada ? 0 : g.preMont) + rateio * indKg);
    ...
    grupos, totais: { material, mdo, industrializacao, comerciais: totalComerciais, ensaios: ensaios.total, montagem: montagem.total, embalagem: embalagem.total },
    montagem, embalagem,
    areaM2: r2(areaM2), ensaios, frete, cargas, pinturaPorArea,
    // ⚠ a LISTA de itens comerciais (não só o total): o cronograma prévio precisa saber QUAIS
    // entraram — telha, grade de piso e lanternim têm prazo de compra próprio. Ver
    // lib/cronograma-previo (COMPRAS_POR_MATERIAL).
    itensComerciais: comerciais,
    ...
    comerciais: { total: totalComerciais, margemPct: margemComerciaisPct, margem: margemComerciais, precoComMargem: r2(comerciaisNoTorg + (comerciaisBdi ? comerciaisNoTorg * bdi : margemComerciais)) },
    // ⚠⚠ BDI DE 400% NÃO É ERRO DE FÓRMULA, É LEITURA ERRADA DA ALAVANCA. Auditoria de 05/09/2026:
    // a LQC-268 saiu com BDI 400% — margem 50% + impostos 27% + factoring 3% dão 80% no
    // denominador, e 1/(1−0,80) = 5. A conta está certa; o que falta é alguém avisar que "margem"
    // na LQC é sobre a VENDA, não sobre o custo: 50% ali quintuplica o preço. Quem digitou
    // provavelmente queria 50% sobre o custo, que é margem de 33% sobre a venda.
    bdiAlerta: (() => {
      const p2 = (k) => n(c.bdi?.[k]) / 100;
      const den = p2("impostos") + p2("factoring") + p2("margem") + p2("comissoes");
      if (den >= 1) return { nivel: "erro", den: r2(den * 100), texto: "as alavancas sobre a venda somam 100% ou mais — o preço fica indefinido" };
      if (den > 0.6) return { nivel: "aviso", den: r2(den * 100), texto: "as alavancas sobre a venda somam mais de 60% — confira se a margem foi lançada sobre a venda, não sobre o custo" };
      return null;
    })(),
    ...
    custosExternos: r2(material.subtotal + mdo.subtotal + totalComerciais + frete.total + embalagem.total),
    precoPorKg: pesoTotal > 0 ? r2(preco / pesoTotal) : 0,
  };
}

// ─── CADÊNCIA POR TIPO DE ESTRUTURA ───────────────────────────────────────────────────────────
//
// Vitor (05/09/2026): "na parte de cadência você deveria fazer um resumo de tipos de estrutura, ou
// seja extra leve, leve, pesado, médio, extra pesado e por aí vai".
//
// ⚠⚠ UM NÚMERO SÓ DE kg/mês MENTE, e mente muito. A fábrica que faz 250 t de estrutura pesada não
// faz 250 t de guarda-corpo: a mesma tonelada de extra leve tem várias vezes mais peças, mais
// corte, mais solda e mais área de pintura. Vender prazo por um kg/mês médio é prometer, na obra
// leve, uma data que a fábrica não alcança — e desperdiçar fábrica na obra pesada.
//
// ⚠ E A CASA JÁ SABE A PROPORÇÃO: é a coluna de FABRICAÇÃO da tabela de classes, que a LQC usa para
// precificar (extra leve R$ 5,50/kg contra extra pesado R$ 2,44/kg). Aquele número É o índice de
// dificuldade — custa 2,25× mais porque leva 2,25× mais fábrica por quilo. Então a cadência de
// cada classe sai da cadência de referência pela mesma razão, sem inventar um segundo cadastro.
export const CLASSE_REFERENCIA = "MEDIO";

/**
 * A cadência de cada classe e a da obra, dado o mix do quantitativo.
 *
 * @param {number} cadenciaRefKgMes  o que a fábrica faz por mês na classe de referência (Médio)
 * @param {object} pesoPorClasse     `res.pesoPorClasse` — peso por nome de classe em CAIXA ALTA
 * @returns {{ classes: Array, obraKgMes: number, pesoTotal: number, mistura: string }}
 */
export function cadenciaPorClasse(cadenciaRefKgMes, pesoPorClasse = {}) {
  const ref = CLASSES.find((c) => c.key === CLASSE_REFERENCIA) || CLASSES[2];
  const base = n(cadenciaRefKgMes);
  // ⚠ "MÉDIO" e "MEDIO" são a mesma classe. O quantitativo é digitado à mão e vem dos dois jeitos;
  // casar por texto cru fazia a fatia inteira sumir da conta, em silêncio.
  const semAcento = (t) => String(t || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().trim();
  const pesos = new Map(Object.entries(pesoPorClasse || {}).map(([k, v]) => [semAcento(k), n(v)]));
  const classes = CLASSES.map((c) => {
    const kg = n(pesos.get(semAcento(c.nome)));
    // dificuldade relativa: quanto a fabricação daquela classe custa a mais que a de referência
    const fator = c.fabricacao > 0 ? ref.fabricacao / c.fabricacao : 1;
    const kgMes = r2(base * fator);
    return {
      key: c.key, nome: c.nome, faixa: c.faixa,
      fabricacaoRsKg: c.fabricacao,
      fator: Math.round(fator * 100) / 100,
      kgMes,
      pesoKg: r2(kg),
      // quantos meses de fábrica essa fatia da obra consome
      meses: kgMes > 0 && kg > 0 ? Math.round((kg / kgMes) * 100) / 100 : 0,
    };
  });

  // ⚠ A CADÊNCIA DA OBRA É MÉDIA HARMÔNICA, não aritmética: o que se soma são os MESES de cada
  // fatia, não os kg/mês. Somar kg/mês e dividir por cinco daria uma obra 100% extra leve rodando
  // à velocidade da média — o erro clássico de misturar taxas.
  const pesoTotal = r2(classes.reduce((a, c) => a + c.pesoKg, 0));
  const mesesTotal = classes.reduce((a, c) => a + c.meses, 0);
  const obraKgMes = mesesTotal > 0 ? r2(pesoTotal / mesesTotal) : base;

  const comPeso = classes.filter((c) => c.pesoKg > 0).sort((a, b) => b.pesoKg - a.pesoKg);
  const mistura = comPeso.length
    ? comPeso.map((c) => `${c.nome} ${Math.round((c.pesoKg / pesoTotal) * 100)}%`).join(" · ")
    : "sem classificação no quantitativo";

  return { classes, obraKgMes, pesoTotal, mesesTotal: Math.round(mesesTotal * 100) / 100, mistura, referencia: ref.nome };
}
...
//
// ⚠⚠ ERA O MAIOR BURACO DE ESCOPO DO ESTUDO. A PLANILHA COMERCIAL da LQC tem três blocos —
// fornecimento, itens comerciais e MONTAGEM — e o portal só sabia o primeiro. Na LQC-253
// (A.YOSHII/SESI) a montagem é R$ 5,28 mi de R$ 16,96 mi: 31% da obra. Na 244 (BORA) é 33%. Obra
// com montagem simplesmente não podia ser orçada aqui.
//
// ⚠ A MONTAGEM NÃO SE COBRA SÓ POR QUILO. A estrutura vai por R$/kg, mas telha, calha, rufo, steel
// deck e linha de vida vão pela UNIDADE DELES (m², m) — é o que a planilha faz, linha a linha, e é
// como o cliente compara. Um R$/kg médio para tudo esconde que montar 10 mil m² de telha zipada
// custa mais que a estrutura que a sustenta.
//
// ⚠ E O CANTEIRO NÃO É DETALHE. Na 253, "despesas e canteiro" (R$ 807 mil) é MAIOR que a mão de
// obra de montagem da estrutura (R$ 642 mil) — alojamento, alimentação, transporte, EPI e o
// canteiro em si. Deixá-lo embutido num R$/kg é o jeito mais fácil de perder dinheiro em obra de
// campo. Aqui ele é percentual explícito sobre a mão de obra, com o valor da 253 como padrão.
export const MONTAGEM_PADRAO = { despesasPct: 90, equipamentosVb: 0 };

// ─── O QUE A CASA JÁ COBROU DE MONTAGEM ───────────────────────────────────────────────────────
//
// Vitor (05/09/2026): "para a aba de montagem você precisa trazer as informações para ficar fácil
// para o orçamentista, pois fica ruim ele apenas escrever; precisa ter sentido e lógica".
//
// ⚠⚠ CAMPO EM BRANCO NÃO É NEUTRO: ele empurra o orçamentista a chutar, e chute em montagem é o
// que faz a obra de campo dar prejuízo. Estes números foram MEDIDOS nas 22 LQCs de 2026 que têm o
// bloco de montagem — mediana por faixa de peso, com a amostra à vista na tela.
//
// ⚠ E A ESCALA MANDA. O R$/kg de montagem despenca com o tamanho da obra: 739 t saem por R$ 1,92/kg
// e 25 t por R$ 6,25 — mobilização, canteiro e equipamento não encolhem junto com a estrutura. Uma
// mediana só (R$ 2,46) mentiria nas duas pontas, então a referência é por faixa.
//
// ⚠ E O EQUIPAMENTO CUSTA MAIS QUE A MÃO DE OBRA: R$ 2,42/kg contra R$ 1,89/kg nas obras grandes
// (na LQC-253, R$ 942 mil de guindaste contra R$ 642 mil de montador). Quem esquece o guindaste
// perde a obra inteira, e é o campo que mais fica em branco.
export const MONTAGEM_REFERENCIA = {
  despesasPct: 90, // mediana das 22 obras (mín. 45%, máx. 131%)
  faixas: [
    { de: 300, ate: Infinity, rotulo: "acima de 300 t", estruturaRsKg: 1.89, equipamentosRsKg: 2.42, n: 3 },
    { de: 100, ate: 300, rotulo: "100 a 300 t", estruturaRsKg: 2.08, equipamentosRsKg: 3.09, n: 7 },
    { de: 50, ate: 100, rotulo: "50 a 100 t", estruturaRsKg: 2.46, equipamentosRsKg: 3.54, n: 3 },
    { de: 10, ate: 50, rotulo: "10 a 50 t", estruturaRsKg: 3.34, equipamentosRsKg: 3.85, n: 5 },
    { de: 0, ate: 10, rotulo: "abaixo de 10 t", estruturaRsKg: 14.70, equipamentosRsKg: 10.71, n: 4 },
  ],
  // R$ por unidade do item (m², m ou kg), mediana das obras que montaram cada um
  porItem: {
    TELHA_TERMO: { valor: 22.97, n: 8 }, TELHA_SIMPLES: { valor: 14.43, n: 5 },
    CALHAS: { valor: 15.44, n: 8 }, RUFOS: { valor: 15.44, n: 9 },
    STEEL_DECK: { valor: 16.75, n: 7 }, GRADE_PISO: { valor: 1.89, n: 4 },
    LINHA_VIDA: { valor: 28.86, n: 3 }, VENEZIANAS: { valor: 30.87, n: 2 },
    LANTERNIM: { valor: 308.74, n: 1 }, CHUMBADORES: { valor: 10.05, n: 1 },
  },
};

/** A faixa de referência para o peso desta obra. */
export function referenciaMontagem(pesoKg = 0) {
  const t = n(pesoKg) / 1000;
  return MONTAGEM_REFERENCIA.faixas.find((f) => t >= f.de && t < f.ate) || MONTAGEM_REFERENCIA.faixas[0];
}

// ─── A MARGEM DO ITEM COMERCIAL ───────────────────────────────────────────────────────────────
//
// Vitor (05/09/2026), sobre telha, calha e rufo levarem BDI: "o ideal seria ter um ganho, pois a
// compra se torna uma responsabilidade grande para nós — pode haver aumento de preço da data
// orçada para a data concluída, e por aí vai. Porém, se a margem for grande demais, pode se tornar
// um problema para vender".
//
// ⚠⚠ NÃO É NEM ZERO NEM O BDI CHEIO. Na planilha da LQC-253 o bloco de itens comerciais está com
// BDI zerado — revenda pelo custo. Aqui o motor aplicava o BDI inteiro (30 a 60%), e telha com 60%
// de margem não fecha negócio: o cliente cota direto com o fabricante. Mas zero também está errado:
// entre orçar e comprar a telha pode subir, e quem responde pelo preço é a Torg.
//
// O meio é uma margem PRÓPRIA, menor que a da estrutura e explícita na tela — cobre variação de
// preço e a responsabilidade da compra, sem tirar o item da faixa de mercado.
export const MARGEM_COMERCIAIS_PADRAO = 15;

/**
 * Custo da montagem em campo.
 *
 * @param {object} cfg { ativo, estruturaRsKg, porItem:{key:R$/un}, despesasPct, equipamentosVb, desmontagem:{qtd,preco} }
 * @param {object} obra { pesoKg, comerciais: [{key, rotulo, un, qtd}] }
 */
export function calcularMontagem(cfg = {}, { pesoKg = 0, comerciais = [] } = {}) {
  const ativo = cfg.ativo === true || n(cfg.estruturaRsKg) > 0 || n(cfg.equipamentosVb) > 0;
  const linhas = [];
  if (ativo) {
    const kg = n(pesoKg), rsKg = n(cfg.estruturaRsKg);
    if (kg > 0 && rsKg > 0) linhas.push({ key: "ESTRUTURA", nome: "Montagem das estruturas metálicas", un: "kg", qtd: r2(kg), preco: rsKg, subtotal: r2(kg * rsKg) });
    for (const i of comerciais) {
      if (["frete", "montagem", "complemento"].includes(i.tipo)) continue;
      const preco = n(cfg.porItem?.[i.id || i.key] ?? cfg.porItem?.[i.key]);
      if (!(preco > 0) || !(n(i.qtd) > 0)) continue;
      linhas.push({ key: i.id || i.key, nome: `Montagem de ${i.rotulo || i.nome}`, un: i.un, qtd: r2(n(i.qtd)), preco, subtotal: r2(n(i.qtd) * preco) });
    }
    const des = cfg.desmontagem || {};
    if (n(des.qtd) > 0 && n(des.preco) > 0)
      linhas.push({ key: "DESMONTAGEM", nome: "Desmontagem/remoção existente", un: des.un || "m²", qtd: r2(n(des.qtd)), preco: n(des.preco), subtotal: r2(n(des.qtd) * n(des.preco)) });
  }

  const mdo = r2(linhas.reduce((a, l) => a + l.subtotal, 0));
  const despesas = r2(mdo * (n(cfg.despesasPct ?? MONTAGEM_PADRAO.despesasPct) / 100));
  const equipamentos = r2(n(cfg.equipamentosVb));
  return {
    ativo, linhas, mdo, despesas, equipamentos,
    despesasPct: n(cfg.despesasPct ?? MONTAGEM_PADRAO.despesasPct),
    total: r2(mdo + despesas + equipamentos),
    porKg: pesoKg > 0 ? r2((mdo + despesas + equipamentos) / n(pesoKg)) : 0,
  };
}

// ─── FAZER AQUI OU TERCEIRIZAR ────────────────────────────────────────────────────────────────
//
// Vitor (05/09/2026): "em obras onde temos muita estrutura extra leve precisamos que tenha a opção
// de terceirizar, pois isso pode ser uma solução, porém o custo se torna maior pois tem o valor a
// ser pago para o terceiro, fretes e etc. Como poderíamos projetar isso e tomar uma decisão já no
// comercial?".
//
// ⚠⚠ A CONTA CERTA NÃO É "TERCEIRO É MAIS CARO QUE A MINHA TABELA". A tabela de industrialização
// cobra o que a casa decidiu cobrar; o que a peça REALMENTE custa dentro é o custo mensal da casa
// dividido pela cadência DAQUELA CLASSE. Extra leve, que roda a 143 t/mês, carrega quase o dobro
// de casa por quilo que estrutura pesada — é por isso que ela é a primeira candidata a sair.
//
//     custo real dentro (R$/kg) = custo mensal da casa ÷ cadência da classe
//
// ⚠ E O GANHO NÃO É SÓ DINHEIRO: é FÁBRICA LIBERADA. Terceirizar a fatia extra leve devolve meses
// de fábrica que podem receber outra obra. Por isso a decisão sai em três números — quanto custa a
// mais, quantos meses libera, e quanto esses meses valem se houver carteira para ocupá-los. Sem o
// terceiro número a resposta é sempre "não terceiriza", e a fábrica segue presa fazendo o que ela
// faz pior.
//
// ⚠ O preço do terceiro NÃO é só a nota dele: entra o frete de ida e volta e a perda/retrabalho que
// volta para a casa. Comparar a nota do terceiro com o custo interno é o erro que faz a decisão
// parecer boa e o resultado sair pior.
// ⚠⚠ O PREÇO DO TERCEIRO É POR CLASSE, E É TABELADO. Vitor (05/09/2026): "vamos deixar o valor do
// externo fixo em 6,50 para o extra leve, 6,00 para o leve, o médio em 5,82 como informou". Faz
// sentido pelo mesmo motivo que o custo interno muda por classe: o terceiro também gasta mais
// homem-hora por quilo em estrutura leve. Um preço único para a obra inteira escondia justamente a
// classe onde terceirizar compensa.
//
// ⚠ PESADO E EXTRA PESADO HERDAM O MÉDIO por enquanto — não foram cotados. A tela avisa, para
// ninguém decidir com número que não veio de fornecedor.
export const PRECO_TERCEIRO_CLASSE = {
  EXTRA_LEVE: 6.50,
  LEVE: 6.00,
  MEDIO: 5.82,
  PESADO: 5.82,
...
export const CLASSES_TERCEIRO_COTADAS = new Set(["EXTRA_LEVE", "LEVE", "MEDIO"]);

// ─── QUANTO CADA CLASSE RENDE POR MÊS DE FÁBRICA ──────────────────────────────────────────────
//
// Vitor (05/09/2026): "e se a demanda da fábrica estiver cheia e eu decidir terceirizar média,
// pesada e extra pesada, qual seria a forma de eu ganhar dinheiro com isso?".
//
// ⚠⚠ COM A FÁBRICA CHEIA, A CONTA VIRA DE CABEÇA PARA BAIXO. Com fábrica sobrando, o que decide é
// o custo por quilo: terceiriza-se o que sai mais barato fora. Com fábrica CHEIA, cada quilo feito
// aqui dentro tira o lugar de outro — e o que decide passa a ser MARGEM POR MÊS DE FÁBRICA, não
// margem por quilo. Mantém-se dentro o que rende mais por mês; sai o resto, mesmo custando mais.
//
// ⚠ E A ORDEM SURPREENDE QUEM OLHA SÓ O R$/kg: a estrutura pesada rende mais por mês, porque roda
// 2,25× mais quilos no mesmo tempo. Terceirizar a pesada para segurar a leve dentro é trocar o mês
// que rende mais pelo que rende menos — perde-se nas duas pontas (paga-se mais ao terceiro E
// ocupa-se a fábrica com o que ela faz pior).
//
// A margem sai do PRÓPRIO estudo (preço e custo por área, agrupados pela classe da área), não de
// tabela: é o dinheiro desta obra, com este BDI.
export function margemPorMesDeFabrica(res = {}, cadencia = {}) {
  const semAcento = (t) => String(t || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().trim();
  const porClasse = new Map();
  for (const a of res.porArea || []) {
    if (!a.ativo || !(a.pesoKg > 0)) continue;
    const k = semAcento(a.classificacao);
    const g = porClasse.get(k) || { pesoKg: 0, preco: 0, custo: 0 };
    g.pesoKg += n(a.pesoKg); g.preco += n(a.preco); g.custo += n(a.custo);
    porClasse.set(k, g);
  }
  return (cadencia.classes || []).map((c) => {
    const g = porClasse.get(semAcento(c.nome));
    const margemKg = g && g.pesoKg > 0 ? r2((g.preco - g.custo) / g.pesoKg) : 0;
    return {
      key: c.key, nome: c.nome, cadenciaKgMes: c.kgMes, pesoKg: g?.pesoKg || 0,
      margemKg,
      // é este número que ordena a fila quando a fábrica está cheia
      margemMes: r2(margemKg * c.kgMes),
    };
  }).sort((a, b) => b.margemMes - a.margemMes);
}

export function decisaoTerceirizar({
  cadencia = {}, custoOperacionalMes = 0, precoPorClasse = {}, freteKg = 0, retrabalhoPct = 0, margemPorMes = 0,
} = {}) {
  const casaMes = n(custoOperacionalMes);
  const linhas = (cadencia.classes || [])
    .filter((c) => c.pesoKg > 0)
    .map((c) => {
      const dentroKg = c.kgMes > 0 ? r2(casaMes / c.kgMes) : 0;
      // preço do terceiro daquela classe (o digitado manda sobre a tabela)
      const tabelado = n(precoPorClasse[c.key]) > 0 ? n(precoPorClasse[c.key]) : n(PRECO_TERCEIRO_CLASSE[c.key]);
      const foraKg = r2(tabelado + n(freteKg));
      // a perda volta para a casa: peça terceirizada que retorna fora do padrão é retrabalho aqui
      const foraComPerda = r2(foraKg * (1 + n(retrabalhoPct) / 100));
      const difKg = r2(foraComPerda - dentroKg);
      const mesesLiberados = c.meses;
      return {
        key: c.key, nome: c.nome, pesoKg: c.pesoKg, cadenciaKgMes: c.kgMes,
        precoTerceiro: tabelado, cotado: CLASSES_TERCEIRO_COTADAS.has(c.key),
        dentroKg, foraKg: foraComPerda, difKg,
        dentro: r2(dentroKg * c.pesoKg), fora: r2(foraComPerda * c.pesoKg),
        diferenca: r2(difKg * c.pesoKg),
        mesesLiberados,
        // o que a fábrica liberada rende SE houver obra para pôr no lugar
        ganhoCapacidade: r2(mesesLiberados * n(margemPorMes)),
      };
    })
    .sort((a, b) => b.difKg - a.difKg);

  // o candidato natural é a classe onde terceirizar dói menos por quilo (ou já compensa sozinha)
  const melhor = linhas.length ? linhas[linhas.length - 1] : null;
  return {
    linhas, melhor,
    total: {
      diferenca: r2(linhas.reduce((a, l) => a + l.diferenca, 0)),
      mesesLiberados: Math.round(linhas.reduce((a, l) => a + l.mesesLiberados, 0) * 100) / 100,
      ganhoCapacidade: r2(linhas.reduce((a, l) => a + l.ganhoCapacidade, 0)),
    },
  };
}
