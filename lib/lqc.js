...
//
// ⚠ O PREÇO POR KG DE FABRICAÇÃO JÁ EMBUTE UM RATEIO DE CUSTO OPERACIONAL — e esse rateio supõe
// uma cadência. A fábrica custa o mesmo todo mês, produzindo muito ou pouco: cada mês a mais que a
// obra ocupa é despesa que ninguém cobrou do cliente. É por isso que uma proposta com margem boa
// no papel vira prejuízo quando o prazo estica.
//
// A conta é direta: o prazo previsto sai do peso dividido pela cadência da fábrica; a partir dele,
// cada mês extra come um custo operacional inteiro. O lucro aguenta até acabar.
export function prazoDeFabricacao(pesoKg, lucro, cfg = {}) {
  const capacidade = n(cfg.capacidadeKgMes);
  const custoMes = n(cfg.custoOperacionalMes);
  // fatia da fábrica que a obra ocupa — obra que divide a fábrica divide também a despesa
  const ocupacao = n(cfg.ocupacaoPct) > 0 ? n(cfg.ocupacaoPct) / 100 : 1;
  if (capacidade <= 0) return null;

  const mesesPrevistos = n(pesoKg) / (capacidade * ocupacao);
  const custoMesObra = custoMes * ocupacao;
  // ⚠ mês a mais só corrói quando há custo operacional lançado; sem ele a conta não existe, e
  // fingir que existe seria inventar um limite.
  const mesesExtras = custoMesObra > 0 ? n(lucro) / custoMesObra : null;
  return {
    capacidadeKgMes: capacidade, custoOperacionalMes: custoMes, ocupacaoPct: ocupacao * 100,
    mesesPrevistos: Math.round(mesesPrevistos * 10) / 10,
    custoOperacionalPrevisto: r2(mesesPrevistos * custoMesObra),
    mesesExtras: mesesExtras === null ? null : Math.round(mesesExtras * 10) / 10,
    mesesLimite: mesesExtras === null ? null : Math.round((mesesPrevistos + mesesExtras) * 10) / 10,
    // quanto de lucro cada mês de atraso custa
    custoDoMesDeAtraso: r2(custoMesObra),
  };
}
