...
export function calcularLqc(c = {}) {
  ...
  // ⚠ O ITEM COMERCIAL SAI DA BASE DO BDI e ganha margem própria — ver MARGEM_COMERCIAIS_PADRAO.
  // Sem isso a telha saía com a mesma margem da estrutura e o cliente cotava direto na fábrica.
  const comerciaisNoTorg = ehDireto("itensComerciais") ? 0 : totalComerciais;
  const margemComerciaisPct = c.margemComerciaisPct == null || c.margemComerciaisPct === ""
    ? MARGEM_COMERCIAIS_PADRAO
    : n(c.margemComerciaisPct);
  const margemComerciais = r2(comerciaisNoTorg * (margemComerciaisPct / 100));
  const baseBdi = r2(custoTorg - comerciaisNoTorg);
  ...
  return {
    ...
    comerciais: { total: totalComerciais, margemPct: margemComerciaisPct, margem: margemComerciais, precoComMargem: r2(comerciaisNoTorg + margemComerciais) },
...
export function calcularMontagem(cfg = {}, { pesoKg = 0, comerciais = [] } = {}) {
  ...
  if (ativo) {
    ...
    for (const i of comerciais) {
      const preco = n(cfg.porItem?.[i.key]);
      if (!(preco > 0) || !(n(i.qtd) > 0)) continue;
      linhas.push({ key: i.key, nome: `Montagem de ${i.rotulo || i.nome}`, un: i.un, qtd: r2(n(i.qtd)), preco, subtotal: r2(n(i.qtd) * preco) });
    }
