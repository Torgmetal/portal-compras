...
export function calcularLqc(c = {}) {
  ...
  const terceirizados = listaTerceiros.map((t) => {
    ...
    l.naoAcompanha = base === "verba" && !t.area && resumos.length < todas.length;
    l.base = base;
  ...
  return {
    ...
    // ⚠ CUSTO POR ÁREA — é o que decide QUAL pacote cortar. Vitor (23/08/2026) quer poder tirar
    // pacotes do escopo "até mesmo por conta do tempo". Só que cortar área reduz o prazo E a
    // receita na mesma proporção: se o corte for cego, a relação não melhora em nada. O que
    // melhora é cortar a área com pior R$/kg — e para isso ela precisa estar na tela.
    porArea: (() => {
      const mapa = new Map();
      for (const l of todas) {
        ...
        const g = mapa.get(chave) || { area: chave, ativo: l.ativo !== false, pesoKg: 0, material: 0, fabricacao: 0, pintura: 0 };
        g.pesoKg += kg;
        g.material += kg * n(l.precoKg);
        g.fabricacao += kg * n(cl?.fabricacao);
        g.pintura += kg * n(cl ? precoPintura(cl) : 0);
        mapa.set(chave, g);
      }
      return [...mapa.values()].map((g) => {
        const custo = r2(g.material + g.fabricacao + g.pintura);
        return { ...g, pesoKg: r2(g.pesoKg), material: r2(g.material), fabricacao: r2(g.fabricacao),
          pintura: r2(g.pintura), custo, custoPorKg: g.pesoKg > 0 ? r2(custo / g.pesoKg) : 0 };
      }).sort((a, b) => b.custoPorKg - a.custoPorKg);
    })(),
    ...
    areaM2: r2(areaM2), ensaios, preMontagemPct: pctPre, pinturaPorArea,
