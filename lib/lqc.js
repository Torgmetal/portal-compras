    ...
    for (const p of pag.parcelas) {
      ...
      // receita que não existe e esconde o buraco de caixa exatamente onde ele é maior.
        ...
        for (let m = m1; m <= mN; m++) {
          ...
          entradas[quando(m + atraso)] += v;
          receitaDe.medicao[quando(m + atraso)] += v;
        }
        // ⚠ o que ficou EM ABERTO no fim da obra fecha na entrega — não some
        if (abertoFinal > 0) {
          const v = abertoFinal * precoPorKgFaturado;
          entradas[quando(mN + atraso)] += v;
          receitaDe.medicao[quando(mN + atraso)] += v;
        }
      }
      else if (medindo.length) for (const m of medindo) { entradas[quando(m + atraso)] += valor / medindo.length; receitaDe.medicao[quando(m + atraso)] += valor / medindo.length; }
    }
  }
