...
export function calcularLqc(c = {}) {
  ...
  return {
    ...
    impostos, totalImpostos, splitFaturamento, demaos: nDemaos, bdiCampos: c.bdi || {},
...
export function fluxoDeCaixa(dados, cfg = {}) {
  ...
  const meses = projeto + fabrica;               // prazo do contrato, ponta a ponta
  const m1 = projeto + 1;                        // primeiro mês em que a fábrica corta
  const mN = projeto + fabrica;                  // último mês de fabricação = entrega
  ...
  for (let m = 1; m <= projeto; m++) saidas[m] += n(dados.custoProjetoMes);
  ...
  for (let i = 0; i < mesesCompra; i++) {
    ...
    for (const d of dias) {
      ...
      saidas[quando] += (material / mesesCompra) * fatia;
    }
  }
  ...
  for (let m = m1; m <= mN; m++) saidas[Math.min(horizonte, m)] += porMes;
  ...
  const medindo = [];
  for (let m = m1; m <= mN; m++) if (!semMedicao.has(m - projeto)) medindo.push(m);
  ...
  if (temManual) {
    manual.forEach((v, m) => { if (n(v) > 0) entradas[quando(m)] += n(v); });
  } else {
    for (const p of pag.parcelas) {
      const valor = preco * (n(p.pct) / 100);
      const atraso = Math.round(n(p.dias) / 30);
      if (p.evento === "ASSINATURA") entradas[quando(atraso)] += valor;
      else if (p.evento === "ENTREGA" || p.evento === "POS_ENTREGA") entradas[quando(mN + atraso)] += valor;
      else if (p.evento === "PROJETO") {
        const n0 = Math.max(1, projeto);
        for (let m = 1; m <= n0; m++) entradas[quando(m + atraso)] += valor / n0;
      }
      // ⚠ MEDIÇÃO SÓ NOS MESES QUE MEDEM. Vitor (23/08/2026): "no mês 1 da fabricação não teremos
      // medição, e pode ser que o segundo também não". É o normal: o primeiro mês corta e prepara,
      // e não há peça pronta para medir. Espalhar a medição por igual desde o mês 1 antecipa
      // receita que não existe e esconde o buraco de caixa exatamente onde ele é maior.
      else if (temKg) {
        // ⚠ QUILO PRODUZIDO EM MÊS QUE NÃO MEDE NÃO SE PERDE — ACUMULA. É o que acontece na obra:
        // o que ficou pronto em janeiro e não entrou na medição de janeiro entra na de fevereiro.
        // Zerar seria jogar fora receita que existe; medir mesmo assim seria faturar sem medição.
        let acumulado = 0;
        for (let m = m1; m <= mN; m++) {
          acumulado += kgMes[m] || 0;
          if (medindo.includes(m)) {
            entradas[quando(m + atraso)] += acumulado * precoPorKgFaturado;
            acumulado = 0;
          }
        }
        // o que sobrou sem medir fecha na entrega
        if (acumulado > 0) entradas[quando(mN + atraso)] += acumulado * precoPorKgFaturado;
      }
      else if (medindo.length) for (const m of medindo) entradas[quando(m + atraso)] += valor / medindo.length;
    }
  }
  ...
  for (let m = 0; m <= horizonte; m++) {
    ...
    fluxo.push({
      ...
      fase: m === 0 ? "assinatura" : m <= projeto ? "projeto" : m <= mN ? "fabricação" : "pós-entrega",
    });
  }
  ...
  return {
    ...
    // ⚠ receita digitada que não fecha com o preço é erro de digitação, e some no meio da tabela
    diferencaFaturamento: temManual ? r2(faturado - preco) : 0,
...
export function impostosDoCenario(res, vendaTorg) {
  ...
  const total = r2(linhas.reduce((a, l) => a + l.valor, 0));
  const daVenda = r2(linhas.filter((l) => variavel(l.key)).reduce((a, l) => a + l.valor, 0));
  return {
    linhas, total, fixo: r2(total - daVenda),
...
export function resultadoDoCenario(res, cen, cfg = {}) {
  ...
  const capacidade = n(cfg.capacidadeKgMes);
  const meses = capacidade > 0
    ? Math.round((n(res?.pesoTotal) / capacidade + n(cfg.mesesExtra)) * 10) / 10
    : n(cfg.meses);
  const casa = r2(meses * n(cfg.custoOperacionalMes));
  ...
  return {
    ...
    impostos: imp.total, impostoFixo: imp.fixo, cargaMarginal: imp.cargaMarginal,
...
export function pontoDeEquilibrio(res, r) {
  ...
  if (c >= 1) return null;
  const custoDireto = n(res?.custoDireto);
  const minimo = r2((n(r.impostoFixo) - c * custoDireto + n(r.externos) + n(r.casa) + n(r.financeiro)) / (1 - c));
