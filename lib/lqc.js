...
export const EVENTOS_PAGAMENTO = [
  ...
  { key: "MEDICAO", nome: "Por medição", ajuda: "acompanha a produção, mês a mês" },
...
export function fluxoDeCaixa(dados, cfg = {}) {
  ...
  // ── material: comprado no começo da FABRICAÇÃO, não do contrato ──
  // (a lista de compra sai do detalhamento, que é justamente o que o projeto produz)
  const mesesCompra = Math.max(1, Math.min(fabrica, Math.round(n(cfg.mesesCompraMaterial) || Math.ceil(fabrica * 0.3))));
  ...
  for (let i = 0; i < mesesCompra; i++) {
    const quando = Math.min(horizonte, m1 + i + atrasoPagto);
  ...
  if (temManual) {
    ...
    for (const p of pag.parcelas) {
      ...
      // ⚠ medição acompanha a PRODUÇÃO, então começa em m1 — não no mês 1 do contrato
      else for (let m = m1; m <= mN; m++) entradas[quando(m + atraso)] += valor / fabrica;
    }
  }
