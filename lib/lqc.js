 ...
 */
export function calcularFrete(cfg = {}, pesoKg = 0) {
  const modo = cfg.modo || "kg";
  const peso = n(pesoKg);
  // ⚠ viagem se arredonda PRA CIMA: meia carreta não sai da fábrica, e o excedente de um quilo
  // custa a viagem inteira.
  const capacidade = n(cfg.capacidadeKg) || 27000;
  const viagens = modo === "viagem"
    ? (n(cfg.viagens) > 0 ? Math.ceil(n(cfg.viagens)) : Math.ceil(peso / Math.max(1, capacidade)))
    : 0;
...
export function calcularLqc(c = {}) {
  ...
  // ── FRETE ──
  const frete = calcularFrete(c.frete || {}, pesoTotal);
  ...
  return {
    ...
    areaM2: r2(areaM2), ensaios, frete, preMontagemPct: pctPre, pinturaPorArea,
