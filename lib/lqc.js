...
export function calcularLqc(c = {}) {
  ...
  const baseFaturamento = {
    ...
    INDUSTRIALIZACAO: r2(vendaTorg * (1 - pctProjeto)),
...
export function fluxoDeCaixa(dados, cfg = {}) {
  ...
  const maisTarde = Math.max(...pag.parcelas.map((p) => (p.evento === "POS_ENTREGA" ? meses + Math.ceil(n(p.dias) / 30) : 0)), 0);
  ...
  // ── recebimento pela forma de pagamento negociada ──
  // ⚠ o que manda é QUANDO O DINHEIRO ENTRA: medir no mês 3 e receber em 30 dias é caixa no mês 4.
  ...
  for (const p of pag.parcelas) {
    ...
    else if (p.evento === "ENTREGA" || p.evento === "POS_ENTREGA") entradas[quando(meses + atraso)] += valor;
    else for (let m = 1; m <= meses; m++) entradas[quando(m + atraso)] += valor / meses;
  }
...
export function resultadoDoCenario(res, cen, cfg = {}) {
  ...
  const resultado = r2(preco - imp.total - externos - casa - financeiro);
  return {
    preco, meses,
