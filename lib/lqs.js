...
/* ⚠⚠ A LISTA NÃO ESCREVE POR EXTENSO. Vitor (08/09/2026), sobre a OP-094: "a programação ainda está
   conjunta, preciso que separe por tipo de perfil". Não separava porque o classificador só conhecia
   a palavra ("chapa", "tubo", "perfil") e a LPC escreve na notação da casa: `CH9.50X60`,
   `U200X50X3.75`, `TB100X100X4.75`. Medido em 08/09/2026: de 5.969 peças de corte com perfil
   preenchido, 2.984 não casavam com regra nenhuma — metade da fábrica sem máquina sugerida.
   Só o `CH` colado no número são 1.906 peças.

   ⚠ O QUE NÃO ESTÁ AQUI É DE PROPÓSITO. `FRØ` (ferro redondo, 4.906 peças) e `BRØ` (barra redonda)
   ficaram fora porque Vitor (08/09/2026) disse que a escolha é de disponibilidade — "vamos usar no
   de cantoneira ou de tubo, aí cabe a nós selecionarmos" — e viram FAMÍLIA em lib/familia-corte.js.
   O ferro chato entrou: "laser cantoneira também". O que não se sabe fica fora — o comentário do
   `maquinaDoPerfil` abaixo vale como regra: atribuir à máquina errada erra o custo E o prazo de uma
   vez. Sem regra, devolve null e a tela pergunta. */
export const MAQUINAS = [
  { id: "LASER_PERFIL", nome: "Laser Perfil", rx: /\b(w\s?\d|hp\s?\d|ue?\s?\d|perfil|viga|i\s?laminad|h\s?laminad|ue?\s?dobrad|u\s?laminad)/i },
  // ⚠ O Ø NO MEIO DE NOVO: `TBØ42.40` é tubo redondo e não casava com `tb\s?\d`. Mesmo defeito que
  //   derrubou o `BRØ` no rastreio (lib/casar-omie.js) — a lista escreve o diâmetro colado à sigla.
  { id: "LASER_TUBO", nome: "Laser Tubo", rx: /\b(tubo|tb\s?[ØO]?\s?\d|metalon|quadrad|redond(o|a)\s|din\s?2440)/i },
  // ⚠ FC = ferro chato. Vitor (08/09/2026), sobre as 521 peças `FC2''X1/4''`: "laser cantoneira
  //   também". A regra já aceitava "barra chata" por extenso; faltava a abreviação da lista.
  { id: "LASER_CANTONEIRA", nome: "Laser Cantoneira", rx: /\b(cantoneira|\bl\s?\d|fc\s?\d|barra\s?chata|chato)/i },
  { id: "LASER_CHAPA", nome: "Laser Chapa", rx: /\b(chapa|ch\s?\d|ch\.?\s?#|xadrez|expandida|blank)/i },
];
