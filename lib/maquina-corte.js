...
export function calcularResumoBarras(pecas) {
  ...
  for (const p of pecas) {
    ...
    if (!perfil || perfil.tipo === "CH") {
      // Chapas não contam barras — contam chapas individuais
      r.pecas += p.qte || 1;
      r.pesoTotal += p.pesoTotalKg || 0;
      continue;
    }

    // Agrupa por descrição exata do perfil (ex: "W150X13")
    const chave = p.descricao || "Sem perfil";
    if (!r.perfis[chave]) {
      const compBarra = BARRA_PADRAO_MM[perfil.tipo] || 6000;
      r.perfis[chave] = { qte: 0, compTotalMm: 0, comprimentoBarraMm: compBarra, tipo: perfil.tipo };
    }
    const pf = r.perfis[chave];
    const qtd = p.qte || 1;
    pf.qte += qtd;
    pf.compTotalMm += (p.comprimentoMm || 0) * qtd;
    r.pecas += qtd;
    r.pesoTotal += p.pesoTotalKg || 0;
  }

  // Calcula barras por perfil
  for (const maq of Object.values(resultado)) {
    for (const pf of Object.values(maq.perfis)) {
      pf.barras = pf.compTotalMm > 0 ? Math.ceil(pf.compTotalMm / pf.comprimentoBarraMm) : 0;
    }
  }
 ...
 */
export function alocarBarras(pecas, comprimentoBarraMm) {
  ...
  for (const peca of expandidas) {
    ...
    if (!colocou) {
      barras.push({
        ...
        restante: comprimentoBarraMm - peca.comprimentoMm,
        comprimentoBarraMm,
      });
    }
  }
  ...
  for (const b of barras) {
    ...
    b.aproveitamento = comprimentoBarraMm > 0 ? ((b.usadoMm / comprimentoBarraMm) * 100) : 0;
    delete b.restante;
  }
...
export function gerarProgramaCorte(pecas) {
  ...
  // Alocar barras para cada perfil
  for (const maq of Object.values(programa)) {
    for (const [desc, grupo] of Object.entries(maq.perfis)) {
      const barras = alocarBarras(grupo.pecas, grupo.comprimentoBarraMm);
      grupo.barras = barras;
      ...
      grupo.aproveitamentoMedio = barras.length > 0
        ...
        : 0;
      delete grupo.pecas; // limpar — ja estao dentro de barras
    }
  }
