...
export const MAQUINA_LABEL = {
  ...
  // Corte feito à mão — NÃO entra em MAQUINAS de propósito, pra ficar fora da
  // carga-máquina (carga-corte itera só MAQUINAS). É uma opção atribuível na UI.
...
export const MAQUINA_COR = {
  ...
  CORTE_MANUAL:     { bg: "bg-gray-100",   text: "text-gray-700",    dot: "bg-gray-400" },
};
...
export function parsePerfil(descricao) {
  ...
  let d = descricao.trim().toUpperCase();

  // ─── NORMALIZAÇÃO ────────────────────────────────────────────────────────────────────────────
  // ⚠⚠ O MESMO AÇO CHEGA ESCRITO DE VÁRIOS JEITOS, e cada grafia não reconhecida vira peça SEM
  // MÁQUINA. Medido em 05/09/2026 nas obras vivas: 942 peças em "CHAPA …" contra o "CH…" que o
  // parser já lia, 224 em "REDONDO …", 60 em "TUBO …".
  //
  // ⚠ VÍRGULA DECIMAL: existe "CH6,30X35" ao lado de "CH6.30X35" — a mesma chapa. Os regex usam
  // [\d.]+ e param na vírgula, então sem isto a peça se perde por causa de um caractere.
  d = d.replace(/(\d),(\d)/g, "$1.$2")
       .replace(/^CHAPA\s+/, "CH")
       .replace(/^REDONDO\s+/, "FRØ")
       .replace(/^TUBO\s+(?:RET\.?\s*)?/, "TB");

  // ─── FAMÍLIAS QUE O PARSER NÃO CONHECIA ──────────────────────────────────────────────────────
  // Destinos ditados pelo Vitor (05/09/2026): "HP laser perfil, Z laser tubo, UE tubo, WT perfil
  // ou tubo, BR quad cantoneira ou tubo". Sozinho o HP eram 69,6 t sem máquina — mais que a fila
  // inteira do Laser Perfil.
  //
  // ⚠ WT E UE VÊM ANTES DE W E U, senão o ramo curto engole o longo.
  const mHP = d.match(/^HP\s*(\d+)\s*X\s*([\d.]+)/);
  if (mHP) return { tipo: "HP", bitolaMm: parseInt(mHP[1]), pesoLinearKgM: parseFloat(mHP[2]) };

  const mWT = d.match(/^WT\s*(\d+)\s*X\s*([\d.]+)/);
  if (mWT) return { tipo: "WT", bitolaMm: parseInt(mWT[1]), pesoLinearKgM: parseFloat(mWT[2]) };

  const mUE = d.match(/^UE\s*(\d+)\s*X/);
  if (mUE) return { tipo: "UE", bitolaMm: parseInt(mUE[1]) };

  const mZ = d.match(/^Z\s*(\d+)\s*X/);
  if (mZ) return { tipo: "Z", bitolaMm: parseInt(mZ[1]) };

  // Barra: BRØ1.1/2" (redonda), BR QUAD 1" (quadrada), BRØM16" (rosqueada)
  const mBR = d.match(/^BR\s*(?:QUAD\.?\s*)?[ØO]?\s*M?\s*([\d.]+(?:\.?\d*\/\d+)?)/);
  if (mBR) return { tipo: "BR", bitolaMm: parseFracao(mBR[1]) * 25.4 };

  // Chapa: CHespessuraXlargura — e também só CHespessura.
  //
  // ⚠ A LARGURA É OPCIONAL. Vitor (05/09/2026): "não precisa saber largura, apenas que é chapa, e
  // chapa vai para o Laser Chapa". Exigir o "Xlargura" fazia "CHAPA 4.75" cair em OUTRO e sair sem
  // máquina — 18 peças hoje, por um dado que não muda o destino. A largura segue sendo lida quando
  // vem (ninguém a consome fora daqui hoje, mas ela é informação boa quando existe), e vale 0
  // quando não vem, igual à chapa xadrez logo abaixo.
  const mCH = d.match(/^CH\s*([\d.]+)(?:\s*X\s*([\d.]+))?/);
  if (mCH) {
    return { tipo: "CH", espessuraMm: parseFloat(mCH[1]), larguraMm: mCH[2] ? parseFloat(mCH[2]) : 0 };
  }
  ...
  // ⚠ [\d.]+ e não \d+: existe "L50.8X3.2" no CMR, e o \d+ parava no 50 e falhava o X seguinte.
  const mL2 = d.match(/^L\s*([\d.]+)\s*X/);
  if (mL2) {
    return { tipo: "L", bitolaMm: parseFloat(mL2[1]) };
  }

  // ─── TUBO ────────────────────────────────────────────────────────────────────────────────────
  // ⚠⚠ O RAMO EXISTIA E COBRIA SÓ UMA DAS TRÊS GRAFIAS. Tudo que vem em mm caía em OUTRO e saía
  // SEM MÁQUINA — e o `classificarMaquina` tem ramo de TB desde sempre, esperando por nada. Medido
  // em 05/09/2026 nas obras vivas: 573 peças, 16.187 kg, 19 grafias distintas.
  //
  //   TB 1.1/2"X2.65 · TB 3/4" · TB3.1/2"-SCH40     polegada, às vezes com a norma no fim
  //   TB100X100X4.75 · TB QUAD. 100X100X6.30        quadrado/retangular, em mm
  //   TBØ42.40X2.65 · TBØ1.1/4" - IND               redondo com Ø, em mm ou em polegada
  //
  // ⚠ A BITOLA DO RETANGULAR É O MAIOR LADO: é ele que decide se cabe na máquina. No redondo o
  // segundo número é a parede, e o `Math.max` devolve o diâmetro de qualquer jeito.
  // ⚠ A polegada é testada PRIMEIRO: "TB 3/4"" tem barra de fração, e a regra em mm leria o 3.
  const mTBpol = d.match(/^TB\s*(?:QUAD\.?\s*)?[ØO]?\s*([\d.]+(?:\.?\d*\/\d+)?)\s*(?:''|")/);
  if (mTBpol) {
    return { tipo: "TB", bitolaMm: parseFracao(mTBpol[1]) * 25.4 };
  }
  const mTBmm = d.match(/^TB\s*(?:QUAD\.?\s*)?[ØO]?\s*([\d.]+)(?:\s*X\s*([\d.]+))?/);
  if (mTBmm) {
    const bitola = Math.max(parseFloat(mTBmm[1]) || 0, parseFloat(mTBmm[2]) || 0);
    if (bitola > 0) return { tipo: "TB", bitolaMm: bitola };
  }
...
export function classificarMaquina(descricao, pesoUnitKg, comprimentoMm) {
  ...
  // ⚠⚠ TUBO ATÉ 100mm VAI NA CANTONEIRA. Vitor (05/09/2026): "tubos desse diâmetro será tudo no
  // laser cantoneira (…) considera tubos de até 100mm". A regra anterior mandava tudo até 500mm
  // para o Laser Tubo — o inverso do que a fábrica faz, e nunca chegou a rodar porque o parser não
  // reconhecia tubo. Pelos perfis das obras vivas, os 100mm deixam no Laser Tubo só o TB150X150,
  // o TB200X100 e o TB 8"; todo o resto (a maior parte em peça e em peso) vai para a cantoneira.
  //
  // ⚠ ISSO CARREGA O GARGALO. A cantoneira faz 1.065 kg/dia, a menor das quatro — e passa a receber
  // a maior parte dos 16 t de tubo. A conta de dias nas telas de carga muda junto, de propósito:
  // ela agora diz a verdade sobre onde a fila realmente para.
  if (perfil.tipo === "TB") {
    if (perfil.bitolaMm <= 100) return MAQUINAS.LASER_CANTONEIRA;
    return MAQUINAS.LASER_TUBO;
  }

  // ⚠⚠ DESTINOS DITADOS PELA FÁBRICA. Vitor (05/09/2026): "HP laser perfil, Z laser tubo, UE tubo,
  // WT perfil ou tubo, BR quad cantoneira ou tubo". Onde ele deu duas opções, a preferida virou
  // regra e a outra continua possível — quem troca é o PCP com o Líder, na tela.
  if (perfil.tipo === "HP") return MAQUINAS.LASER_PERFIL;          // o mais pesado da casa
  if (perfil.tipo === "Z" || perfil.tipo === "UE") return MAQUINAS.LASER_TUBO;  // dobrados, leves
  if (perfil.tipo === "BR") return MAQUINAS.LASER_CANTONEIRA;      // barra segue o ferro redondo

  // WT é T cortado de W — mesma régua do W, e por isso "perfil ou tubo" conforme o tamanho.
  if (perfil.tipo === "WT") {
...
export function gerarProgramaCorte(pecas) {
  ...
  for (const [maqCode, maq] of Object.entries(programa)) {
    for (const [_desc, grupo] of Object.entries(maq.perfis)) {
