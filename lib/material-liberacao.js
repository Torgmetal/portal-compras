...
export async function analisarMaterial(opNumero, pecas) {
  ...
  for (const pf of perfis) {
    ...
    const rAmarrado = informado?.rUsado && rsDaOp.has(String(informado.rUsado).trim()) ? String(informado.rUsado).trim() : null;
    if (rAmarrado) {
      ...
      porPerfil.set(k, {
        ...
        porTroca: true, trocaPor: informado.trocadoPorNome || null, trocaMotivo: informado.motivo || null,
      });
      continue;
    }
