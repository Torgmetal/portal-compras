...
export async function analisarMaterial(opNumero, pecas) {
  ...
  // Estoque só passa após escolha e conferência explícitas; nunca pela descrição/FIFO global.
  const rsConferidos = [...trocas.values()].filter((t) => t.estoqueConferido).map((t) => t.r);
  const estoqueConferido = rsConferidos.length ? await prisma.documentoQualidade.findMany({
    where: { categoria: "MATERIAL", importRef: { in: rsConferidos } },
    select: { importRef: true, nome: true, opNumero: true },
  }) : [];
  const estoquePorR = new Map(estoqueConferido.map((e) => [e.importRef, e]));
  ...
  // nenhum: o material já é dela, só está escrito com outro nome. Desde 11/09, R de estoque
  // também pode passar após conferência física explícita registrada pelo Planejamento.
  const rsDaOp = new Set(cmrDaOp.map((c) => String(c.importRef || "").trim()).filter(Boolean));

  const porPerfil = new Map();
  /* O mesmo cálculo para um perfil, seja ele da lista recebida ou de um croqui que compõe um
     conjunto dela — chamado pelo laço abaixo e por `porPerfilDeFora`. */
  function resolverPerfil(pf) {
    const k = String(pf).toUpperCase();
    const informado = jaInformado.get(k);
    const rAmarrado = informado?.r && (rsDaOp.has(informado.r) || (informado.estoqueConferido && estoquePorR.has(informado.r))) ? informado.r : null;
    if (rAmarrado) {
      const ent = cmrDaOp.find((c) => String(c.importRef || "").trim() === rAmarrado) || estoquePorR.get(rAmarrado);
      porPerfil.set(k, {
        ...
        estoqueConferido: !rsDaOp.has(rAmarrado), opOrigem: ent?.opNumero || null,
  ...
  for (const pf of perfis) resolverPerfil(pf);

  /* ⚠⚠ CONJUNTO NÃO CONSOME AÇO — QUEM CONSOME É O CROQUI. Vitor (08/09/2026), diante de 9.931 kg
     marcados como "não comprado" nas coberturas 112/113: "deve ter alguma coisa errada". Tinha: os
     47 itens eram CONJUNTOS (coluna, tesoura, terça), que não têm perfil próprio porque nascem na
     montagem a partir dos croquis. Sem perfil, o casamento não tinha o que procurar e o portal
     concluía "não comprado" — de aço que estava no pátio, com R, pelos 76 croquis que compõem esses
     conjuntos, todos ENTREGUES, com exatamente os mesmos 9.931 kg.
     Cobrar material do conjunto também contaria o mesmo peso duas vezes [[torg_peso_real_op]].
     O conjunto herda dos croquis, como já faz para etapa e prontidão: entregue quando TODOS os
     componentes estão. Sem croqui cadastrado ele continua caindo no fluxo normal. */
  const semPerfil = pecas.filter((p) => p.id && !String(p.perfil || "").trim()).map((p) => p.id);
  const composicao = semPerfil.length
    ? await prisma.conjuntoCroqui.findMany({
        where: { conjuntoId: { in: semPerfil } },
        select: { conjuntoId: true, croqui: { select: { id: true, perfil: true } } },
      })
    : [];
  const croquisDoConjunto = new Map();
  for (const c of composicao) {
    if (!croquisDoConjunto.has(c.conjuntoId)) croquisDoConjunto.set(c.conjuntoId, []);
    croquisDoConjunto.get(c.conjuntoId).push(c.croqui);
  }
  const estadoDoPerfil = (pf) => porPerfil.get(String(pf || "").trim().toUpperCase())
    || porPerfilDeFora(pf);

  /* Os croquis de um conjunto podem não vir na lista que a tela passou (ela manda só os conjuntos).
     Este resolvedor roda a MESMA conta para o perfil deles, em vez de devolver "sem material" por
     ausência — que era exatamente o erro que esta correção existe para tirar. */
  function porPerfilDeFora(pf) {
    const k = String(pf || "").trim().toUpperCase();
    if (!k) return null;
    if (porPerfil.has(k)) return porPerfil.get(k);
    resolverPerfil(k);
    return porPerfil.get(k) || null;
  }

  const porPeca = new Map();
  for (const p of pecas) {
    const v = porPerfil.get(String(p.perfil || "").trim().toUpperCase());
    if (v) { porPeca.set(p.id, v); continue; }
    const partes = croquisDoConjunto.get(p.id);
    if (partes?.length) {
      const estados = partes.map((c) => estadoDoPerfil(c.perfil));
      const todosOk = estados.every((e) => e && (e.estado === "NA_OP" || (e.estado === "ESTOQUE" && e.rInformado)));
      porPeca.set(p.id, todosOk
        ? { perfil: null, estado: "NA_OP", herdadoDosCroquis: partes.length, rs: [], rInformado: null,
            descricaoCmr: null }
        : { perfil: null, estado: "SEM_MATERIAL", herdadoDosCroquis: partes.length, rs: [], rInformado: null,
            ...(estados.find((e) => e?.falta) || {}) });
      continue;
    }
    porPeca.set(p.id, { perfil: p.perfil, estado: "SEM_MATERIAL", rs: [], rInformado: null });
  }
...
export function statusMaterialPlanejamento(v) {
  if (!v) return null;
  if (v.estado === "NA_OP") return "ENTREGUE";
  // ⚠⚠ ESTOQUE COM R DECLARADO PASSA. Vitor (02/09/2026): "para as novas precisamos fazer isso — o
  // almoxarifado informa um R e ele já preenche na programação do planejamento essa informação".
  // Sem isto o Planejamento continuava lendo "não comprado" e barrando a marca mesmo depois de
  // alguém ter escolhido o fardo — e como a tela do PCP só abre DEPOIS da liberação, não havia
  // ordem possível: para descer precisava do R, para informar o R precisava ter descido.
  // ⚠ Só com R. Estoque sem ninguém ter apontado o fardo continua barrado: é a pergunta que o Vitor
  // mandou fazer em 25/08, e deixar passar sem resposta perderia o rastreio justo onde ele é
  // manual. Obra antiga sem declaração não muda de comportamento — nada foi declarado nela.
  if (v.estado === "ESTOQUE" && v.rInformado) return "ENTREGUE";
  return v.falta || "NAO_COMPRADO";
}
