...
import { carimbarNaTx } from "./passagem-auditoria";
...
/**
 * Quantas marcas estão abertas naquele posto.
 *
 * ⚠⚠ ISTO DECIDE, NÃO SÓ INFORMA — e o comentário aqui dizia o CONTRÁRIO até 14/09/2026, quando o
 * Codex leu e apontou. `largarOAnterior` recusa soltar o crachá quando esta contagem é > 0: é a
 * regra inteira do posto exclusivo. Quem acreditasse no comentário antigo moveria a contagem para
 * fora da trava ("é só mensagem") e devolveria a corrida que a trava existe para fechar.
 */
 ...
 */
export async function entrarNoPosto(prisma, { operadorId, recursoId, ambiente }) {
 ...
 */
/**
 * Tudo que pode RECUSAR uma passagem — ou o desfecho de quem chega depois da resposta perdida.
 *
 * ⚠ Separada do corpo da passagem só para caber no teto de complexidade: são quatro perguntas
 * independentes, e a que responde primeiro manda.
 *
 * @returns {Promise<{erro?:string, jaEstava?:boolean, presenca?:object, deQuemSai?:object}>}
 */
async function portaDaPassagem(tx, { deOperadorId, recursoId, dePresencaId, presenca, doQueAssume }) {
  // ⚠⚠ QUEM EXECUTA TAMBÉM PASSA PELO PORTEIRO (achado do Codex, 14/09/2026, e era um furo de
  // verdade). A rota montava o contexto de presença e as ações da passagem simplesmente NÃO o
  // usavam: uma aba antiga podia entregar um vínculo já reaberto, e `assumirPosto` aceitava até
  // operador sem presença no destino. O executor é quem SAI na entrega e quem ASSUME na rendição.
  const deQuemSai = await presencaAtiva(tx, deOperadorId);

  // ⚠⚠ O REENVIO É CONFERIDO ANTES DO PORTEIRO (achado do Codex, 14/09/2026). Na ENTREGA quem
  // executa é quem SAI — e, passada a entrega, a presença dele está encerrada: `exigirPresenca`
  // recusaria o reenvio com "não está mais com o crachá neste posto", exatamente o caminho que a
  // idempotência existe para evitar. Este ramo não muda nada, só relata o que já é verdade.
  const jaAconteceu = (!deQuemSai || deQuemSai.recursoId !== recursoId) && doQueAssume?.recursoId === recursoId;
  if (jaAconteceu) {
    return { jaEstava: true, presenca: doQueAssume, marcasQueSeguemAbertas: await marcasAbertas(tx, recursoId) };
  }

  if (presenca?.operadorId) {
    const recusa = await exigirPresenca(tx, { ...presenca, recursoId });
    if (recusa) return { erro: recusa };
  }

  // ⚠⚠ REENVIO DEPOIS DE RESPOSTA PERDIDA NÃO PODE PARECER ERRO (achado do Codex). Se a passagem já
  // aconteceu — quem sai não está mais no posto e quem assume está —, tocar de novo devolve o que
  // já é verdade, em vez de "não está mais com o crachá neste posto", que o operador leria como
  // defeito logo depois de um gesto que deu certo.
  if (!deQuemSai || deQuemSai.recursoId !== recursoId) {
    return { erro: "Quem você quer render não está mais com o crachá neste posto." };
  }

  // ⚠⚠ O VÍNCULO ESCOLHIDO NA TELA, NÃO SÓ O OPERADOR (achado do Codex). A lista do totem é um
  // retrato: se aquela presença terminou e outra abriu no mesmo posto para a mesma pessoa, o toque
  // antigo encerraria a presença NOVA — justamente a que está trabalhando agora.
  if (dePresencaId && dePresencaId !== deQuemSai.id) {
    return { erro: "Esta lista está desatualizada. Confira quem está no posto e tente de novo." };
  }

  return { deQuemSai };
}

export async function passarPosto(prisma, dados) {
  const { deOperadorId, paraOperadorId, recursoId, dePresencaId = null, presenca = null,
    ambiente, auditoria = null } = dados;
  ...
  return comOPostoDe(prisma, { quem: paraOperadorId, chavesFixas }, async (tx, doQueAssume) => {
    const porta = await portaDaPassagem(tx, { deOperadorId, recursoId, dePresencaId, presenca, doQueAssume });
    if (!porta.deQuemSai) return porta;

    const largou = await largarOAnterior(tx, doQueAssume, recursoId);
    if (largou.erro) return largou;

    const nome = async (id) =>
      String((await tx.mesOperador.findUnique({ where: { id }, select: { nome: true } }))?.nome || "").trim();
    const [nomeQueSai, nomeQueAssume] = [await nome(deOperadorId), await nome(paraOperadorId)];
    await encerrar(tx, porta.deQuemSai.id, `passou para ${nomeQueAssume || "outro operador"}`);
    ...
    const nova = doQueAssume?.recursoId === recursoId
      ? doQueAssume
      : await criar(tx, { operadorId: paraOperadorId, recursoId, ambiente });

    const resultado = {
      presenca: nova,
      saiu: nomeQueSai || "o operador anterior",
      assumiu: nomeQueAssume || null,
      vinculoEncerrado: porta.deQuemSai.id,
      marcasQueSeguemAbertas: await marcasAbertas(tx, recursoId),
      liberou: largou.liberou,
    };
    // ⚠⚠ O CARIMBO VAI DENTRO DA TRANSAÇÃO — passagem sem trilha é o que este módulo existe para
    // impedir; ver `carimbarNaTx`. Falhar aqui desfaz a passagem, e o operador toca de novo.
    return carimbarNaTx(tx, resultado, auditoria);
  });
}
