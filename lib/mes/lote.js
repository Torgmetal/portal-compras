...
export async function encerrarLote(prisma, { recursoId, loteId, operadorId = null, presenca = null }) {
  ...
  return comTravaDe(prisma, chavesDo(presenca, recursoId), async (tx) => {
    ...
    if (!sobrou) {
      // ⚠⚠ O MUNDO VEM DO RECURSO, não de um literal (achado do Codex, 21/09/2026) — era o último
      // `"PROD"` cravado no código. É o recurso e não a sessão porque ESTE evento é do POSTO ("fim
      // do trabalho no posto"), e ele é gravado justamente quando não sobrou sessão nenhuma.
