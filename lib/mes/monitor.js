import "server-only";
// ⚠⚠ ESTA LIB FALA COM OS DOIS BANCOS, E ISSO AGORA ESTÁ ESCRITO (21/09/2026). O parâmetro
// `prisma` que ela recebe é o cliente do MES (schema `mes`) — é dele que saem sessões e
// apontamentos, e é ele que abre as transações. A PROGRAMAÇÃO mora no portal (`PecaConjunto`) e é
// lida pelo cliente do portal, importado aqui de propósito: a fronteira entre os dois sistemas
// precisa aparecer no código, não viver num acordo.
 ...
 */
export function estadoDoCartao(sessoes, evento, agora = new Date()) {
  // ⚠ Recebe a LISTA de trabalhos abertos desde 13/09/2026 (o nesting abre várias marcas de uma vez
  // na mesma máquina). Continua aceitando uma sessão solta para não quebrar quem chama de fora.
  const abertas = Array.isArray(sessoes) ? sessoes : [sessoes].filter(Boolean);
  const sessao = abertas[0] || null;
  const semNoticia = { estado: DESCONHECIDO, desde: sessao ? sessao.abertaEm : null, detalhe: null };
  if (!evento) return semNoticia;

  const doRecurso = !evento.sessaoId;                            // manutenção, sinal do CNC, lote
  const daSessaoAberta = abertas.some((s) => s.id === evento.sessaoId);
  if (!doRecurso && !daSessaoAberta) {
    // Evento de uma sessão que não é nenhuma das abertas. Com trabalho aberto por cima, é dado
    // inconsistente; sem trabalho nenhum, é o rastro da sessão passada — a máquina está livre.
  ...
  return {
    ...
    ...(sessao ? alertaDeSessaoVelha(estado, maisVelha(abertas), agora) : {}),
  };
}

const detalheDoEvento = (evento) => evento.motivo?.descricao || evento.detalhe || null;

/** ⚠ O alerta olha a MAIS VELHA das abertas: é ela que denuncia o turno que foi embora sem encerrar. */
const maisVelha = (abertas) =>
  abertas.reduce((v, s) => (new Date(s.abertaEm) < new Date(v.abertaEm) ? s : v), abertas[0]);
...
export function cartaoDoRecurso(recurso, dados = {}, agora = new Date()) {
  const { sessoes = [], evento = null, somas = new Map(), pecas = new Map() } = dados;
  const { estado, desde, detalhe, alerta = null } = estadoDoCartao(sessoes, evento, agora);
  const principal = sessoes[0] || null;
  return {
    ...
    // ⚠⚠ UM CARD POR POSTO, MESMO COM VÁRIAS MARCAS ABERTAS (pedido do Codex). O card mostra o
    // trabalho principal e diz quantos mais existem; um card por sessão faria a TV mostrar a mesma
    // máquina cinco vezes, em cinco estados que são o mesmo.
    ...trabalhoDoCartao(principal, principal?.pecaId ? pecas.get(principal.pecaId) : null),
    ...numerosDoCartao(principal ? somas.get(principal.id) : null),
    trabalhos: sessoes.length,
    outrasMarcas: sessoes.slice(1).map((s) => s.marca).filter(Boolean),
  };
}
...
export async function panoramaDaFabrica(prisma, { ambiente = "PROD", agora = new Date() } = {}) {
  ...
  const cartoes = recursos.map((r) => cartaoDoRecurso(r, {
    sessoes: porRecurso.get(r.id) || [],
