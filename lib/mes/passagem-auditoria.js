...
export async function carimbarNaTx(tx, resultado, contexto) {
  ...
  // ⚠⚠ `mesAuditoria`, NÃO `auditLog` (Codex, 20-21/09/2026). Desde que o MES tem cliente e schema
  // próprios, um `tx.auditLog.create` aqui seria escrita em OUTRA conexão, em outro schema: a
  // transação da passagem voltaria atrás e o carimbo ficaria de pé, ou o contrário. A garantia que
  // esta função inteira existe para dar — passagem e trilha juntas ou nenhuma das duas — só
  // sobrevive com a trilha no mesmo schema da transação.
  await tx.mesAuditoria.create({
    data: {
      ...
      ambiente: recurso?.ambiente ?? contexto.ambiente ?? "PROD",
    },
  });
  return resultado;
}
