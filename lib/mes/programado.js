...
export async function programadoPara(prisma, recurso, dia = new Date()) {
  ...
  const feito = await produzidoPorMarca(prisma, pecas, ambienteDe(recurso), recurso?.setor?.codigo ?? null);
 ...
 */
async function produzidoPorMarca(prisma, pecas, ambiente, operacao) {
  const feito = new Map();
  ...
  // ⚠⚠ E A OPERAÇÃO (achado do Codex, 22/09/2026). Sem ela, a lista do posto de MONTAGEM mostraria
  // como "já produzida" a marca que foi CORTADA — e a marca sairia da fila de quem ainda tem de
  // montá-la. É o mesmo furo do `saldoDaMarca`, do lado da leitura.
  const sessoes = await prisma.mesSessao.findMany({
    where: { marca: { in: marcas }, ambiente, operacao: operacao ?? null },
...
export async function acharMarca(prisma, termo, limite = 12) {
  ...
  const achadas = await portal.pecaConjunto.findMany({
...
/**
 * O NÚMERO DA OBRA A PARTIR DO ID — a porta de entrada completando o que veio pela metade.
 *
 * ⚠⚠ A OBRA NÃO PODE EXISTIR EM DUAS FORMAS (achado do Codex, 22/09/2026). No MES a identidade da
 * obra é o NÚMERO (`chaveDoTrabalho` em `lib/mes/sessao.js` já era só ele, e `daObra` em
 * `lib/mes/saldo.js` passou a ser estrito). Uma sessão aberta só com `opId` cai num grupo próprio:
 * a mesma marca da mesma obra ganharia DOIS tetos, um para cada forma de escrever a obra.
 *
 * ⚠ Cross-banco de propósito, como o resto desta lib: o `OP` mora no portal e a sessão no `mes`.
 * Custa uma consulta só no caminho em que o número não veio — a tela sempre manda os dois.
 *
 * ⚠ Não achar não impede abrir: a sessão segue com a obra pelo id, que é identidade válida. Recusar
 * o apontamento porque o cadastro não tem a OP seria trocar um teto duplicado por um operador
 * parado.
 */
export async function numeroDaObra(opId) {
  if (!opId) return null;
  const op = await portal.oP.findUnique({ where: { id: String(opId) }, select: { numero: true } });
  return op?.numero || null;
}
