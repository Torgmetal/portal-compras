...
export async function programadoPara(prisma, recurso, dia = new Date()) {
  ...
  const feito = await produzidoPorMarca(prisma, pecas);
  return { lotes: agruparPorObra(pecas, campos.dia, feito), semMapa: false };
}
 ...
 */
async function produzidoPorMarca(prisma, pecas) {
  const feito = new Map();
