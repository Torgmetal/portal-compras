...
export async function abrirSessao(prisma, dados) {
  const { recursoId } = dados;
  if (!recursoId) return { erro: "Informe o recurso." };
  return comTravaDoRecurso(prisma, recursoId, (tx) => abrirNaTransacao(tx, dados));
}
...
export async function apontarQuantidade(prisma, dados) {
  ...
  return comTravaDe(prisma, [dono.recursoId, dono.chaveTrabalho], async (tx) => {
    const { sessao, erro } = await sessaoViva(tx, sessaoId);
    if (erro) return { erro };
...
export async function encerrarSessao(prisma, { sessaoId, operadorId = null, chaveIdem = null }) {
  ...
  return comTravaDoRecurso(prisma, dono.recursoId, (tx) =>
    encerrarNaTransacao(tx, sessaoId, { operadorId, chaveIdem }));
}
