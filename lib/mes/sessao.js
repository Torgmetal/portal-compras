...
import { reconciliarUnidades } from "@/lib/mes/unidade-reserva";
...
// ⚠ Definidos em `lib/mes/estados.js` e reexportados aqui — quem já importava de `sessao.js`
// continua importando dali. A mudança foi para quebrar o ciclo com a reserva da barra.
export { STATUS, ESTADO } from "./estados";
    ...
    const sessao = (novoLote || novaUnidade)
      ? await tx.mesSessao.update({
          ...
          data: {
            ...
            // ⚠⚠ ISTO NÃO É MAIS O TETO — é o registro do que ESTA sessão abriu, para a tela. O
            // teto da marca passou a ser DERIVADO das unidades (`comporTeto`, em lib/mes/saldo.js),
            // porque acumular aqui só consertava a sessão ainda ABERTA: barras produzidas em
            // sequência abrem sessão nova, e a segunda voltava a nascer bloqueada (3ª rodada do
            // Codex, 22/09/2026).
            //
            // ⚠ Continua somando só quando a UNIDADE é nova, para o número da tela não inflar num
            // reenvio com outro id de lote.
...
const contexto = (d) => ({
  ...
  // ⚠⚠ O TOTAL DIGITADO MORA À PARTE (parecer do Codex, 22/09/2026). Só o caminho MANUAL o
  // preenche — no nesting `planejadoQtd` é a quantidade da BARRA, e o teto da marca vem das
  // unidades. Sem esta separação, uma sessão aberta à mão com total 10 que depois recebesse uma
  // barra de 2 veria o teto desabar para 2, bloqueando 8 peças legítimas.
  planejadoManual: d.nestingUnidadeId ? 0 : Number(d.planejadoQtd) || 0,
});
...
export async function encerrarNaTransacao(tx, sessaoId, { operadorId = null, chaveIdem = null, semEvento = false } = {}) {
  ...
  // ⚠⚠ A BARRA VOLTA A FICAR LIVRE AQUI, E SÓ AQUI (parecer do Codex, 22/09/2026). A tela encerra
  // marca a marca, e o lote encerra em bloco — os dois caminhos passam por esta função. Liberar a
  // posse só no `encerrarLote` deixaria reserva de pé sem nenhuma sessão aberta, e a barra ficaria
  // presa a um posto que não a está mais cortando.
  //
  // ⚠ Roda DEPOIS do update: a pergunta é "sobrou alguma sessão ABERTA nesta barra?", e esta
  // sessão já não é uma delas.
  await reconciliarUnidades(tx, { unidades: sessao.nestingUnidades || [], ambiente: sessao.ambiente });
