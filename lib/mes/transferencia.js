...
export async function transferirBarra(prisma, {
  ...
  return comTravaDe(prisma, chavesDa(presenca, antes.recursoId, paraRecursoId), async (tx) => {
    ...
    await encerrarPostoSeVazio(tx, { recursoId: agora.recursoId, operadorId });
    ...
    // ⚠⚠ TRAZER A BARRA NÃO TIRA A MÁQUINA DA PARADA (achado do Codex, 22/09/2026). Eu gravava
    // PRODUCAO direto aqui, enquanto `abrirLote` já perguntava o estado antes: a barra levada para
    // um posto em PARADA, MANUTENÇÃO ou SETUP apagava esse estado, e o tempo dele parava de ser
    // contado no OEE. A regra agora é uma só, em `lib/mes/evento-posto.js`.
    const { estadoPreservado } = await comecarNoPosto(tx, {
      recursoId: paraRecursoId, operadorId, ambiente,
      detalhe: `Barra recebida de outro posto · ${sessoes.length} marca(s)`,
    });
    ...
    return {
      loteId: lote, sessoes, deRecursoId: agora.recursoId,
      encerradasNaOrigem: daOrigem.length, estadoPreservado,
    };
  });
}
...
export async function liberarBarra(prisma, { unidadeId, ambiente, recursoId, motivo, usuario }) {
  ...
  return comTravaDe(prisma, [dona.recursoId, recursoId].filter(Boolean), async (tx) => {
    ...
    if (r.erro) return r;
    await encerrarPostoSeVazio(tx, { recursoId: agora.recursoId });
    await tx.mesAuditoria.create({
      data: {
