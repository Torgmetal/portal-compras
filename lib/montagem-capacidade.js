 ...
 */
// ⚠⚠ ANCORADA EM 230 t/MÊS desde 06/09/2026. Vitor: "montagem e solda pode ser 230 por conta das
// marcas" — a entrega da fábrica é 280 t, e a montagem fica em 230 porque 25% do peso é marca
// avulsa e nunca passa por bancada (medido em 06/09: 22.139 de 89.761 kg no acabamento).
//
// Antes esta curva era só o p75, sem âncora em tonelada — ela rendia 176 t/mês com a mistura das
// obras de hoje. O fator 1,31 a leva a 230 t com essa mesma mistura (5 bancadas × 21 dias).
//
// ⚠ ANCORADA NO MIX DE HOJE, por escolha do Vitor ("vamos para o número de hoje, precisamos
// entregar mais"). Isso significa que em mês de peça miúda a fábrica entrega menos tonelada fazendo
// o mesmo trabalho — ver a nota equivalente em lib/solda-capacidade.js, onde o problema apareceu.
//
// ⚠ Os valores seguem abaixo do melhor dia já registrado (102 contra 234 nas miúdas, 22 contra 249
// na de 60–120): o desafio é sustentar o ritmo, não inventá-lo.
export const RITMO_META = [
  { ate: 25, pecasDia: 102.2 },
  { ate: 60, pecasDia: 23.6 },
  { ate: 120, pecasDia: 22.3 },
  { ate: 300, pecasDia: 17.0 },
  { ate: Infinity, pecasDia: 9.2 },
];
...
/* ⚠⚠ O MAPA FOI CORRIGIDO EM 09/09/2026, E OS QUATRO NOMES ESTAVAM TROCADOS. Em 08/09 o Vitor deu a
   lista ("Jurandir, Adenilson, Vando, Rodrigo") e eu a apliquei NA ORDEM EM QUE VEIO, sem conferir
   contra o apontamento — o erro apareceu quando ele perguntou o peso que o Adenilson montou no dia:
   o Syneco dizia MONTAGEM 3 e o quadro dizia que a 3 era do Vando.

   Agora vem do medido: 90 dias, 1.495 apontamentos.
     MONTAGEM 1 → Edivando Oliveira  100% (255)   ← é o "Vando" da lista dele (Edvando de Oliveira
                                                     Pires, de Montagem Interna; não confundir com
                                                     VANDO MAXIMO, que é soldador da SOLDA 4)
     MONTAGEM 2 → Jurandir Donizeti  100% (431)
     MONTAGEM 3 → Adenilson Alves     99% (283)
     MONTAGEM 4 → Julio Cesar         71% (152)   ← não estava na lista; é o dono da bancada
     MONTAGEM 5 → Rodrigo Aparecido  100% (96)    ← estava rotulada só "Montagem 5"

   ⚠ A CHAVE NÃO MUDA, só o rótulo — "MONTAGEM n" é o que o Syneco aponta e o que está gravado em
   `PecaConjunto.montagemBancada`. Trocar a chave apagaria a atribuição de todas as peças.

   ⚠ FORA DA LISTA, de propósito: a 6 e a 7 giram (Rodrigo 73% na 6; a 7 é 44% "TERCEIRO") e a 9 tem
   5 apontamentos em 90 dias. A MONTAGEM 10 é caso à parte: tem dono claro (Marcos Bahia, 98% de 80
   apontamentos) mas entrar aqui viraria a SEXTA bancada, e a meta de 230 t/mês está ancorada em
   CINCO (ver o aviso do RITMO_META). Decisão do Vitor, não minha. */
export const MONTADOR_DA_BANCADA = {
  "MONTAGEM 1": "Edivando Oliveira",
  "MONTAGEM 2": "Jurandir Donizeti",
  "MONTAGEM 3": "Adenilson Alves",
  "MONTAGEM 4": "Julio Cesar",
  "MONTAGEM 5": "Rodrigo Aparecido",
};

/** O rótulo da bancada na tela: o nome de quem senta nela, ou a própria chave se ninguém foi dito. */
export const nomeDaBancada = (b) => MONTADOR_DA_BANCADA[String(b || "").trim().toUpperCase()] || b || "sem bancada";
 ...
 */
/**
 * ⚠⚠ A META DA BANCADA É 7 TONELADAS POR DIA. Vitor (03/09/2026): "nossa meta é de 7 toneladas para
 * cada banca, claro que peças menores seria por quantidade de peças; aliás tudo deve ser na verdade
 * pela quantidade de peças E kg". Perguntado qual das duas manda, respondeu: "o que garantir que
 * faremos as 7 tons nas bancadas".
 *
 * Então a bancada-dia é o MAIOR dos dois custos — o que estourar primeiro. Peça miúda continua
 * limitada pela contagem (78 peças de 25 kg não dão 7 t e nunca vão dar); peça graúda passa a ser
 * limitada pelo peso antes de a contagem reclamar.
 */
export const META_KG_BANCADA_DIA = 7000;
...
export function custoDoConjunto(c, curva = RITMO_NORMAL) {
  ...
  // ⚠ o peso pendente acompanha a contagem pendente: cobrar o peso das peças já montadas esticaria
  // o prazo do que falta.
  const kgPend = kgUnit * pend;
  return Math.max(pend / ritmoDaPeca(kgUnit, curva), kgPend / META_KG_BANCADA_DIA);
}
