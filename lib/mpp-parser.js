...
function parseXml(xml) {
  ...
  for (const m of taskMatches) {
    ...
    // Predecessoras (vínculos de tarefa). Guarda os UIDs do MS Project; o import
    // mapeia UID→id da CronogramaTarefa e grava em antecessoraIds. Sem isso o
    // vínculo se perde e o Gantt reexportado sai sem as linhas de ligação.
    const predecessorUids = [];
    for (const pm of body.matchAll(/<PredecessorLink>([\s\S]*?)<\/PredecessorLink>/g)) {
      const puid = parseInt(extractTag(pm[1], "PredecessorUID") || "", 10);
      if (Number.isFinite(puid)) predecessorUids.push(puid);
    }
