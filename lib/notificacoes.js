...
export async function criarNotificacao({
  tipo, titulo, mensagem, link, dados, origemUserId, destinatarios, modulos, chaveEvento,
}) {
  try {
    ...
    const data = { tipo, titulo, mensagem, link: link || null, dados: dados || null, origemUserId: origemUserId || null };
    // A mesma entrada pode passar pelo portal, importação e reconciliação. Um só aviso por R.
    const notificacao = chaveEvento
      ? await prisma.notificacao.upsert({ where: { chaveEvento }, create: { ...data, chaveEvento }, update: {} })
