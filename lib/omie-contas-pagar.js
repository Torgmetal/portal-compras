...
import { omieCall as omie } from "./omie-call.js";
...
const numf = (v) => { const n = numeroBR(v ?? "0", NaN); return Number.isFinite(n) ? n : 0; };
 ...
 */
const modos = (incremental) => (incremental ? ["alteracao", "inclusao"] : ["full"]);

/**
 * Coleta as páginas do Omie dentro do orçamento de tempo.
 *
 * ⚠⚠ `parcial` É O QUE IMPEDE BURACO NO HISTÓRICO (achado do Codex, 13/09/2026). A tentação era
 * parar no orçamento e gravar `ultimoSync = agora`: os registros que ficaram para trás NAQUELA
 * janela nunca mais seriam buscados, porque a próxima janela começa onde esta disse ter parado.
 * O `ultimaAlteracao` não salva — ele é o maior carimbo VISTO, não prova de processamento. Então
 * execução parcial não avança marco nenhum: repete a mesma janela na passada seguinte.
 *
 * ⚠ Antes deste conserto o laço de páginas não tinha freio NENHUM — o `orcamentoMs` só guardava a
 * última etapa (enriquecer detalhe). Era por aqui que a função estourava os 120s da Vercel.
 */
async function coletar(listaDeModos, filtroData, noPrazo) {
  const registros = new Map(); // id → registro cru
  for (const modo of listaDeModos) {
    ...
    for (let pg = 1; pg <= 5000; pg++) {
      ...
      if (pg >= Number(d.total_de_paginas || 1)) break;
      if (!noPrazo()) return { registros, parcial: true };
    }
  }
  return { registros, parcial: false };
}

/** Grava os campos core de cada título. Para no orçamento, avisando que parou. */
async function gravarCore(registros, mapas, alteracaoAnterior, noPrazo) {
  let maiorAlteracao = alteracaoAnterior;
  ...
  for (const [, c] of registros) {
    ...
    gravados++;
    if (!noPrazo() && gravados < ids.length) return { ids, gravados, maiorAlteracao, parcial: true };
  }
  return { ids, gravados, maiorAlteracao, parcial: false };
}

// Teto de dias por passada. Três dias cabem folgados no orçamento e recuperam duas semanas de
// atraso em menos de uma semana de execuções diárias.
const JANELA_MAX_DIAS = 3;

export async function sincronizarContasPagar({ incremental = true, maxDetalhe = 120, orcamentoMs = 45000 } = {}) {
  const t0 = Date.now();
  const noPrazo = () => Date.now() - t0 < orcamentoMs;
  const mapas = await getMapas();
  const estado = await prismaDirect.omieSyncState.findUnique({ where: { id: "contapagar" } });

  // ⚠⚠ A JANELA É FECHADA E CURTA, E ISSO É O CONSERTO DE 13/09/2026. Antes ela ia de
  // `ultimoSync - 1 dia` até HOJE, e o `ultimoSync` só era gravado DEPOIS de toda a coleta. Quando
  // a função estourou os 120s da Vercel pela primeira vez (30/08), o marco não avançou — e no dia
  // seguinte a janela tinha um dia a mais, e mais um, e mais um. Em 13/09 eram 14 dias: a espiral
  // se alimentava sozinha, e o cron devolvia 504 todo dia sem nem chegar a gravar o heartbeat
  // (por isso o monitor dizia "sem sucesso há 353h" em vez de "falhou").
  //
  // Com teto de janela, um atraso de duas semanas é recuperado em passadas sucessivas, cada uma
  // cabendo no tempo da função.
  const agora = new Date();
  let janelaAte = agora;
  let filtroData = {};
  if (incremental && estado?.ultimoSync) {
    const desde = new Date(estado.ultimoSync.getTime() - 24 * 60 * 60 * 1000);
    janelaAte = new Date(Math.min(agora.getTime(), desde.getTime() + JANELA_MAX_DIAS * 86400000));
    filtroData = { filtrar_por_data_de: fmtBR(desde), filtrar_por_data_ate: fmtBR(janelaAte) };
  }

  const { registros, parcial: coletaParcial } = await coletar(modos(incremental), filtroData, noPrazo);
  const { ids, gravados, maiorAlteracao, parcial: gravacaoParcial } =
    await gravarCore(registros, mapas, estado?.ultimaAlteracao || null, noPrazo);
  const parcial = coletaParcial || gravacaoParcial;

  // Grava o estado — timestamp + dados core garantidos mesmo que o enriquecimento de detalhe
  // (abaixo) estoure o orçamento de tempo / o limite da função no Vercel.
  //
  // ⚠⚠ O MARCO VAI PARA O FIM DA JANELA, NÃO PARA `agora`. São coisas diferentes desde que a
  // janela passou a ter teto: dizer "sincronizado até agora" depois de varrer só três dias pularia
  // o resto do atraso de uma vez. `janelaAte` é o que foi de fato coberto.
  const total = await prismaDirect.contaPagar.count();
  if (!parcial) {
    await prismaDirect.omieSyncState.upsert({
      where: { id: "contapagar" },
      create: { id: "contapagar", ultimoSync: janelaAte, ultimaAlteracao: maiorAlteracao, totalRegistros: total },
      update: { ultimoSync: janelaAte, ultimaAlteracao: maiorAlteracao, totalRegistros: total },
    });
  }

  // ── Limpeza de órfãos (SÓ no full sync, e SÓ quando ele foi completo) ────────
  ...
  // foi excluída no Omie e deve sair.
  //
  // ⚠⚠ `!parcial` É OBRIGATÓRIO AQUI (achado do Codex, 13/09/2026). A trava dos 90% olha
  // `ids.length`, que é o que foi COLETADO — se a coleta terminou e os upserts pararam no meio,
  // ela passa e o `deleteMany` apaga títulos legítimos que só não tinham sido gravados ainda.
  // A heurística de percentual protege contra listagem parcial do Omie, não contra parada nossa.
  let orfaosRemovidos = 0;
  if (!incremental && !parcial && ids.length >= 1000 && ids.length >= total * 0.9) {
  ...
  // Cada ConsultarContaPagar é sequencial (~0,7s); paramos antes de estourar o tempo.
  let detalhados = 0;
  if (!parcial && noPrazo()) {
  ...
  return { sincronizados: ids.length, gravados, detalhados, total, orfaosRemovidos, parcial, ate: janelaAte };
}
