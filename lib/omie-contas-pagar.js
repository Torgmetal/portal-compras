...
async function omie(url, call, param, tentativa = 0) {
  ...
  if (data.faultstring) {
    if (FAULT_RETRY.test(data.faultstring) && tentativa < 5) {
      // Respeita o tempo que o Omie pede ("Aguarde N segundos" no "Consumo
      // redundante") — o backoff fixo (máx 6s) era menor que o cooldown (~46s)
      // e estourava. Se não vier tempo, usa backoff progressivo.
      const m = data.faultstring.match(/aguarde\s+(\d+)\s*segundo/i);
