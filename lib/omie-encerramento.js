...
async function buscarPagina(flag, { de, ate }, pg) {
  let d;
  try {
...
export async function coletarEncerrados(janela, opts = {}) {
  ...
  for (let pg = 1; pg <= MAX_PAGINAS; pg++) {
    ...
    const { d, motivo } = await buscarPagina(flag, janela, pg);
...
export async function reconciliarEncerramentos(prisma, opts = {}) {
  ...
  if (reabertura.desmarcar.length) {
    await prisma.pedidoOmie.updateMany({ where: { id: { in: reabertura.desmarcar } }, data: { encerradoOmieEm: null } });
  }
  return {
