...
export async function sincronizarProdutos() {
  ...
  try {
    ...
    // Caso retorne vazio (erro 4474 silencioso): não loga como erro — é comportamento
    // esperado nesta conta. Cai no fallback ListarPosEstoque+ObterEstoque.
  } catch (e) {
  ...
  if (listarProdutosOk) {
    ...
    // Une todos, sem duplicatas, max 15 termos por sync (reduzido para caber nos 60s do Vercel)
    const MAX_TERMOS = 15;
    ...
    if (updatesPos.length > 0) {
      // Roda em lotes de 4 para respeitar o pool de conexões (connection_limit=5).
      // Promise.all(tudo) esgotava o pool e causava timeout nas queries em fila.
      const LOTE_DB = 4;
      for (let i = 0; i < updatesPos.length; i += LOTE_DB) {
        await Promise.all(updatesPos.slice(i, i + LOTE_DB));
      }
    ...
    const LOTE_FAMILIA = 30; // 30 por sync: ~13s com Omie 400ms/call, bem dentro dos 60s
  ...
  if (listarProdutosOk && codigosImportados.length > 0) {
    ...
    if (qtdUpdates.length > 0) {
      const LOTE_DB = 4;
      for (let i = 0; i < qtdUpdates.length; i += LOTE_DB) {
        await Promise.all(qtdUpdates.slice(i, i + LOTE_DB));
      }
    }
