...
export async function sincronizarProdutos() {
  ...
  // 3) Desativar produtos que nao vieram do Omie nessa sync (excluidos/inativados la)
  let desativados = 0;
  if (itensImportados.length > 0) {
    const result = await prisma.estoqueItem.updateMany({
      where: { codigoOmie: { notIn: itensImportados }, ativo: true },
      data: { ativo: false },
    });
    desativados = result.count;
  }

  // 4) Enriquecer com categoria via ConsultarProduto (best-effort).
