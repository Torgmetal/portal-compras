...
export async function sincronizarProdutos() {
  ...
  if (listarProdutosOk) {
    ...
    if (updatesPos.length > 0) {
      // Promise.all é mais rápido que $transaction para lotes grandes:
      // cada updateMany vira uma query independente no pool, sem overhead de transação.
      await Promise.all(updatesPos);
  ...
  if (listarProdutosOk && codigosImportados.length > 0) {
    ...
    if (qtdUpdates.length > 0) await Promise.all(qtdUpdates);
