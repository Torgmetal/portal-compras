...
// Escaping seguro para SQL VALUES
...
export async function sincronizarProdutos() {
  ...
  if (listaProdutosOk && Object.keys(prodFamilias).length > 0) {
    ...
    for (const [cod, { cat, label }] of Object.entries(prodFamilias)) {
      ...
      rows.push(`(${esc(cod)}, ${esc(cat || "N/A")}, ${esc(label || null)}, ${isMP(label)})`);
  ...
  return {
    total: pos.size, criados, atualizados, zerados, enriquecidos,
    locais: locais.length,
    fonteUsada: listaProdutosOk ? "ListarProdutos+Família" : "ConsultarProduto",
  };
}
