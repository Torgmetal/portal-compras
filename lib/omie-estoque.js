...
async function omie(url, call, param) {
  ...
  const res  = await fetch(url, {
    ...
    signal: AbortSignal.timeout(10000), // 10s por chamada — evita travar o sync
  });
...
export async function sincronizarProdutos() {
  ...
  // 5c. Aplica famílias no banco — uma única query SQL bulk (ms vs 37s com updateMany individual)
  const todosCodigos = new Set([...pos.keys(), ...fora.keys()]);

  // Helper de escaping seguro para SQL
  const esc = (s) => s == null ? "NULL" : `'${String(s).replace(/'/g, "''")}'`;
  const isMP = (s) => /mat[eé]ria[\s_-]*prima/i.test(s || "");

  if (usouListarProdutos && Object.keys(prodFamilias).length > 0) {
    const rows = [];
    for (const [cod, { cod: cat, label }] of Object.entries(prodFamilias)) {
      if (!todosCodigos.has(cod)) continue;
      rows.push(`(${esc(cod)}, ${esc(cat || "N/A")}, ${esc(label)}, ${isMP(label)})`);
      if (label) enriquecidos++;
    }
    if (rows.length > 0) {
      await prisma.$executeRawUnsafe(`
        UPDATE "EstoqueItem" AS e
        SET "categoriaOmie" = v.cat::text,
            "categoriaLabel" = v.label::text,
            "estoqueTorg"    = v.torg::boolean
        FROM (VALUES ${rows.join(",")}) AS v(cod, cat, label, torg)
    ...
    // 5d. Fallback: ConsultarProduto individual (parâmetro correto: "codigo")
    // API em lotes de 20 paralelos → coleta resultados → bulk SQL por lote
      ...
      const resultados = await Promise.all(lote.map(async ({ codigoOmie }) => {
        try {
          ...
          return {
            ...
            label: String(det.descricao_familia || familiasMap[det.codigo_familia || ""] || "").trim() || null,
          };
        } catch { return { cod: codigoOmie, cat: "N/A", label: null }; }
      }));

      // Bulk SQL para o lote
      const rows = resultados.map(({ cod, cat, label }) =>
        `(${esc(cod)}, ${esc(cat)}, ${esc(label)}, ${isMP(label)})`
      );
      if (rows.length > 0) {
        await prisma.$executeRawUnsafe(`
          UPDATE "EstoqueItem" AS e
