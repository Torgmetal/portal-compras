...
// sincronizarProdutos — 3 fases:
//   FASE 1: ListarProdutos → catálogo completo com família inclusa em cada produto
//   FASE 2: ListarPosEstoque → atualiza qtdAtual e CMC dos produtos importados
//   FASE 3: Marca como ativo=false produtos que sumiram do Omie
//
// Remove as antigas abordagens de ListarFamilias + ConsultarProduto por item
// (eram lentas e frágeis). ListarProdutos já traz descricao_familia diretamente.
export async function sincronizarProdutos() {
  ...
  let total = 0;
  const errosDetalhes = [];
  const codigosImportados = []; // coleta todos os codigos vindos do Omie nessa sync

  // ── FASE 1: ListarProdutos ────────────────────────────────────────────────
  // Fonte primaria: catálogo completo do Omie, com familia inclusa.
  // Pula produtos marcados como inativos (inativo === "S").
  // Auto-classifica estoqueTorg=true para familia "Matéria Prima".
  {
    let pagina = 1;
    const TAMANHO = 100;
    ...
    while (true) {
      ...
      for (const p of lista) {
        ...
        // Auto-classifica como Estoque Torg se familia for Materia Prima
        ...
        const r = await upsertEstoqueItem({
          ...
          estoqueTorg,
          ativo: true,
          // qtdAtual e cmc sao atualizados na Fase 2; preserva o que ja existe no upsert
        });
        if (r === "criado")    criados++;
      ...
      // Suporte a nomes de campo diferentes entre versoes da API Omie
      const totalPaginas =
        Number(data.total_de_paginas) || Number(data.nTotPaginas) || 1;
      // Seguranca extra: se a pagina veio incompleta, e a ultima
  ...
  // Roda SOMENTE se a Fase 1 trouxe pelo menos um produto; se falhou
  // completamente, nao zera o estoque de ninguem.
  if (codigosImportados.length > 0) {
    const hoje = new Date();
    const dDataPosicao = `${String(hoje.getDate()).padStart(2, "0")}/${String(
      hoje.getMonth() + 1
