...
export async function sincronizarProdutos() {
  ...
  if (listarProdutosOk) {
    ...
    // em lotes de 15 por chamada (cada item ~250ms → 15 × 250ms = ~3.5s seguro).
    // Chamadas subsequentes de sync avançam para os próximos 15, e assim por diante.
    const LOTE_FAMILIA = 15;
    const codigosSemFamilia = new Set(
      (await prisma.estoqueItem.findMany({
        where: {
          codigoOmie: { in: codigosImportados },
          OR: [{ categoriaOmie: null }, { categoriaOmie: "" }],
        },
