...
export async function sincronizarProdutos() {
  ...
  const errosDetalhes = [];
  const TAMANHO = 20;  // menor = menos chance de bater em produto zoado do Omie
  const MAX_PAGINAS_CONSECUTIVAS_FALHANDO = 5;

  // Pagina por pagina ate esgotar.
  // Tolerante a erros — se uma pagina falhar (ex: produto especifico
  // com cadastro zoado no Omie), pula pra proxima ate desistir.
  while (true) {
    let data;
    try {
      ...
      paginasFalhadas = 0; // resetou contador
    } catch (e) {
      paginasFalhadas++;
      errosDetalhes.push(`Pagina ${pagina}: ${e.message}`);
      if (paginasFalhadas >= MAX_PAGINAS_CONSECUTIVAS_FALHANDO) {
        // Muitas paginas consecutivas falhando — para de tentar
        break;
      }
      pagina++;
    ...
    const relevantes = lista.filter((p) => {
      const cat = String(p.codigo_familia || p.cCodFamilia || p.descricao_familia || "").trim();
  ...
  return {
    ...
    ...(errosDetalhes.length > 0 ? { erros: errosDetalhes } : {}),
  };
}
