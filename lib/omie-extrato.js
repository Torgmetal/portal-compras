...
const num = (v) => { const n = numeroBR(v ?? "0", NaN); return Number.isFinite(n) ? n : 0; };
 ...
 *
 * ⚠ Havia aqui um `incluirAplicacoes` documentado que o corpo NUNCA leu: quem
 * passasse `true` era ignorado em silêncio. Tirei o parâmetro em vez de deixar a
 * promessa falsa na assinatura. Se a intenção era poder trazer também as contas
 * de aplicação, o lugar é o filtro de `alvo` logo abaixo — mas isso muda o que a
 * tela de Fluxo de Caixa mostra, então fica como decisão de quem cuida do
 * financeiro, não minha.
 *
 * @param {{ de: string, ate: string }} opts — datas "YYYY-MM-DD"
 */
export async function listarFluxoExtrato({ de, ate }) {
