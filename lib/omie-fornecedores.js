...
export async function listarFornecedoresOmie() {
  ...
  for (const c of regs) {
    ...
    // ⚠⚠ O OMIE DEVOLVE VÁRIOS E-MAILS NUM CAMPO SÓ ("a@x.com,b@y.com") — 10 cadastros vieram
    // assim em 25/08/2026 e o envio de cotação recusava todos eles (o servidor valida e-mail).
    // O primeiro vira o `email`; os demais, `emailsAdicionais` (vão em cópia nas cobranças).
    const emails = separarEmails(c.email);
    lista.push({
      nCodOmie: nCod,
...
export async function sincronizarFornecedoresOmie({ dryRun = false } = {}) {
  ...
  // Códigos Omie bloqueados (excluídos manualmente) — não podem voltar.
  const bloqueados = new Set((await prisma.fornecedorOmieBloqueado.findMany({ select: { nCodOmie: true } })).map((b) => so(b.nCodOmie)));
  ...
  let semEmail = 0, inativos = 0, bloqueadosPulados = 0;

  for (const r of omie) {
    if (bloqueados.has(r.nCodOmie)) { bloqueadosPulados++; continue; } // excluído manualmente: não volta
    if (!r.email) semEmail++;
    if (!r.ativo) inativos++;
  ...
  const resumo = {
    ...
    semEmail,
    inativos,
    bloqueados: bloqueadosPulados,   // excluídos manualmente que foram pulados
  ...
  for (let i = 0; i < novos.length; i += 200) {
    const lote = novos.slice(i, i + 200).map((r) => ({
      ...
      email: r.email, emailsAdicionais: r.emailsAdicionais || [], telefone: r.telefone, contato: r.contato,
