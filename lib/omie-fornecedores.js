...
export async function listarFornecedoresOmie() {
  ...
  for (const c of regs) {
    ...
    vistos.add(nCod);
    // Pula funcionários cadastrados como PJ (vêm com a tag "Funcionário" no Omie,
    // geralmente junto com "Fornecedor"). Não são fornecedores de fato.
    const tags = (c.tags || []).map((t) => so(t?.tag).toLowerCase());
    if (tags.some((t) => t.startsWith("funcion"))) { funcionarios.add(nCod); continue; }
...
export async function sincronizarFornecedoresOmie({ dryRun = false } = {}) {
  const { lista: omie, funcionarios } = await listarFornecedoresOmie();

  // Funcionários-PJ que entraram numa carga anterior (antes de passarmos a pulá-los):
  ...
  if (funcionarios.size) {
    const jaImportados = await prisma.fornecedor.findMany({
      where: { nCodOmie: { in: [...funcionarios] } },
