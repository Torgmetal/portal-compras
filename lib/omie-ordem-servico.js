...
const registro = log("omie-ordem-servico");
...
export async function consultarOrdemServico({ numero, codigo } = {}) {
  ...
  try {
    ...
    registro.info("[omie-os] HTTP", res.status, "param", JSON.stringify(param), "keys", Object.keys(data || {}).join(","));
    ...
    if (!cab) {
      registro.erro("[omie-os] cabecalho nao encontrado. Top-level keys:", Object.keys(data || {}));
      return {
    ...
    registro.erro("[omie-os] excecao:", e?.message);
