  ...
  try {
    ...
    if (!res.ok || data.faultstring) {
      return {
        error:
          data.faultstring ||
          `Omie respondeu HTTP ${res.status}. Verifique se o numero da OS existe.`,
        status: res.status,
        raw: data,
      };
    }

    // Estrutura tipica de retorno de OS no Omie
    const cab = data?.Cabecalho || data?.cabecalho;
    const info = data?.InformacoesAdicionais || data?.infoCadastro;
    const det = data?.ServicosPrestados || data?.servicos || [];

    if (!cab) {
      return { error: "OS nao encontrada no Omie.", raw: data };
    }

    const codigoOS = cab.nCodOS || cab.codigo_os;
    const numeroOS = cab.cNumOS || cab.numero_os;
    const etapa = cab.cEtapa || cab.etapa;
    const dataEmissao = cab.dDtPrevisao || cab.dDtEmissao || info?.dInc;
    const valorTotal = Number(cab.nValorTotal || cab.valor_total || 0);
