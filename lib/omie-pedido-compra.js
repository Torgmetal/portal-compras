...
export async function criarPedidoOmie(input) {
  ...
  // (Omie nao gera contas a pagar).
  const numParcelas = nQtdeParc != null ? Number(nQtdeParc) : 1;
  // cParcCustom — texto customizado da parcela. "1P0" = 1 parcela 0 dias (a vista).
  // Aparece como label "1 parcela" no pedido do Omie em vez do numero solto.
  const parcelaCustom = numParcelas === 1 ? "1P0" : `${numParcelas}P30`;
  ...
  const cabecalho = {
    ...
    cParcCustom: numParcelas > 0 ? parcelaCustom : "",
