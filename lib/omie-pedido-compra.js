...
export async function criarPedidoOmie(input) {
  ...
  // Produto generico: usado quando o item do RM nao tem codigo Omie cadastrado.
  // Configure no Omie um produto tipo "Material Diversos" com codigo qualquer
  // (ex: "DIV") e adicione OMIE_PRODUTO_GENERICO_CODIGO no Vercel com esse codigo.
  // O Omie usa o codigo do produto pra resolver internamente; a descricao real
  // do item fica no cDescricao da linha (cada linha do pedido aparece com a
  // descricao correta no Omie).
  const codigoGenerico = String(process.env.OMIE_PRODUTO_GENERICO_CODIGO || "").trim();

  const produtos_incluir = [];
  for (let idx = 0; idx < itens.length; idx++) {
    const item = itens[idx];
    const codigoItem = item.codigo ? String(item.codigo).trim() : "";
    // Usa o codigo do item se houver; senao, cai pro generico.
    const cProduto = codigoItem || codigoGenerico;
    if (!cProduto) {
      return {
        error:
          "Produto genérico não configurado. Cadastre um produto tipo 'Material Diversos' " +
          "no Omie e adicione OMIE_PRODUTO_GENERICO_CODIGO no Vercel com o código dele " +
          "(ex: 'DIV'). Esse produto serve de guarda-chuva pros itens que não estão no " +
          "cadastro do Omie — a descrição real continua aparecendo na linha do pedido.",
      };
    }
