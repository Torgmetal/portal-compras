...
async function buscarProdutoPorDescricao(descricao, appKey, appSecret) {
  ...
  // Extrai primeiras palavras significativas pra busca (evita termos muito longos)
  const palavras = descNorm.split(/[\s\-–—]+/).filter((w) => w.length > 1);
  if (palavras.length === 0) return null;
  // Usa ate 4 palavras pra ser especifico sem exagerar
  const termoBusca = palavras.slice(0, 4).join(" ");
  ...
  try {
    const resp = await fetch(OMIE_PROD_URL, {
      ...
      body: JSON.stringify({
        ...
        param: [{
          ...
          filtrar_apenas_descricao: termoBusca,
        }],
      }),
    });
    const data = await resp.json();
    ...
    // Exige que pelo menos 60% das palavras batam (minimo 2)
    const scoreMinimo = Math.max(2, Math.ceil(palavras.length * 0.6));
    if (melhor && melhorScore >= scoreMinimo) {
      const resultado = {
        codigo: melhor.codigo || String(melhor.codigo_produto || ""),
        unidade: melhor.unidade || "UN",
      };
      produtoDescricaoCache.set(descNorm, resultado);
      return resultado;
    }
  } catch {
    // silencia — segue sem resultado
  }
...
export async function criarPedidoOmie(input) {
  ...
  for (let idx = 0; idx < itens.length; idx++) {
    ...
    if (!cProduto) {
      return {
        error:
          `Nao foi possivel resolver codigo de produto para o item "${String(item.descricao || "").substring(0, 80)}". ` +
