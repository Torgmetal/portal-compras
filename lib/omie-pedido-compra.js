...
export async function criarPedidoOmie(input) {
  ...
  if (localEstoqueCodigo && data.nCodPed) {
    ...
    // Omie aceita o local em multiples lugares/formats — tenta cada um.
    const valorNumerico = Number(localEstoqueCodigo) || 0;
    const tentativas = [
      // 1. produtos_alterar com nCodLocal (numerico) — mais comum
      { tag: "nCodLocal", valor: valorNumerico, em: "produtos" },
      { tag: "nCodLocEst", valor: valorNumerico, em: "produtos" },
      { tag: "nCodLocEstoq", valor: valorNumerico, em: "produtos" },
      // 2. produtos_alterar com codigo string
      { tag: "cCodLocal", valor: String(localEstoqueCodigo), em: "produtos" },
      { tag: "cCodLocEstoq", valor: String(localEstoqueCodigo), em: "produtos" },
      // 3. cabecalho com nCodLocal
    ...
    for (const t of tentativas) {
      ...
      try {
        ...
        // "Tag X não faz parte da estrutura" — tenta proxima
        // Outros erros (valor invalido, etc) — para
