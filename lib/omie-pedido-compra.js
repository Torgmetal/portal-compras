...
export async function criarPedidoOmie(input) {
  ...
  // Conta corrente padrao (Inter) — vem do env var OMIE_CONTA_CORRENTE_PADRAO.
  // Se nao configurado, Omie usa a conta default da empresa no Omie.
  const nCodCCPadrao = Number(process.env.OMIE_CONTA_CORRENTE_PADRAO) || null;

  // nQtdeParc: padrao 1 parcela. Em faturamento direto, passamos 0 explicito
  // (Omie nao gera contas a pagar). Qualquer outro valor se informado.
