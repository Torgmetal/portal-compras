...
// Estrategia: pega o PRIMEIRO produto da listagem (qualquer um serve
// — descricao real fica no cDescricao da linha do pedido).
async function resolverProdutoGenerico(appKey, appSecret) {
  if (produtoGenericoCache) return produtoGenericoCache;

  // Extrai codigo de um produto a partir de qualquer resposta da API
  function extrairCodigo(data) {
    // Tenta todas as chaves conhecidas da API Omie (varia entre versoes)
    const lista =
      data.produto_servico_resumido ||
      data.produto_resumido ||
      data.produto_servico_lista_resumida ||
      data.produto_servico_cadastro ||
      [];
    if (lista.length > 0) {
      const p = lista[0];
      return p.codigo || p.codigo_produto?.toString() || p.cCodigo || "";
    }
    return "";
  }

  // Tentativa 1: ListarProdutosResumido
  try {
    ...
    const cod = extrairCodigo(data);
    if (cod) {
      produtoGenericoCache = cod;
      return produtoGenericoCache;
    }
    console.warn("[resolverProdutoGenerico] ListarProdutosResumido sem resultado. Keys:", Object.keys(data).join(", "));
  } catch (e) {
    console.warn("[resolverProdutoGenerico] ListarProdutosResumido erro:", e?.message);
  }

  // Tentativa 2: ListarProdutos (endpoint completo como fallback)
  try {
    const resp = await fetch(OMIE_PROD_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    console.warn("[resolverProdutoGenerico] ListarProdutos sem resultado. Keys:", Object.keys(data).join(", "));
  } catch (e) {
 ...
 * Usa ListarProdutos com filtrar_apenas_descricao (busca parcial do Omie),
 * depois compara descricao completa pra achar o melhor match.
...
async function buscarProdutoPorDescricao(descricao, appKey, appSecret) {
  ...
  // Funcao interna: busca no Omie e retorna melhor match
  ...
  function melhorMatch(lista) {
    ...
    for (const p of lista) {
      ...
      if (descProdNorm === descItemNorm) {
        return p; // match exato — retorna direto
      }
      // Score: quantas palavras do item aparecem na descricao do produto
      const score = palavras.filter((w) => descProd.includes(w)).length;
      if (score > melhorScore) {
        melhorScore = score;
        melhor = p;
      }
    }
    // Threshold: 40% das palavras (minimo 2). Antes era 60% mas era muito
    // restritivo pra descricoes longas com especificacoes tecnicas (tintas, etc).
    const scoreMinimo = Math.max(2, Math.ceil(palavras.length * 0.4));
    return melhor && melhorScore >= scoreMinimo ? melhor : null;
  }

  try {
    // Tentativa 1: primeiras 4 palavras (especifico)
    const termo1 = palavras.slice(0, 4).join(" ");
    let lista = await tentarBusca(termo1);
    let resultado = melhorMatch(lista);

    // Tentativa 2: se nao achou, busca so com as 2 primeiras palavras (mais amplo)
    if (!resultado && palavras.length > 2) {
      const termo2 = palavras.slice(0, 2).join(" ");
      lista = await tentarBusca(termo2);
      resultado = melhorMatch(lista);
    }

    // Tentativa 3: se ainda nao, busca so com a primeira palavra (nome do produto)
    if (!resultado && palavras.length > 1) {
      const termo3 = palavras[0];
      lista = await tentarBusca(termo3);
      resultado = melhorMatch(lista);
    }

    if (resultado) {
      const res = {
        codigo: resultado.codigo || String(resultado.codigo_produto || ""),
