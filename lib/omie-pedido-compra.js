import { emModoDemo } from "@/lib/modo-demo";
...
// O Omie corrompe caracteres fora do Latin-1 (ex.: travessão "—" vira "¿¿¿").
// Troca traços/aspas tipográficas por ASCII e remove o que sobrar fora do Latin-1.
//
// ⚠ Mora no módulo, não dentro da função: a observação do item precisa dela no laço que monta
// `produtos_incluir`, que roda ANTES do ponto onde ela era declarada — como `const`, isso dava
// ReferenceError por temporal dead zone.
const sanitizarOmie = (s) => String(s ?? "")
  .replace(/[‐-―]/g, "-")
  .replace(/[‘’‚‛]/g, "'")
  .replace(/[“”„‟]/g, '"')
  .replace(/…/g, "...")
...
const registro = log("omie-pedido-compra");
...
async function carregarEstoqueCache(appKey, appSecret) {
  ...
  estoqueCacheLoading = (async () => {
    ...
    try {
      ...
      registro.info(`[estoqueCache] Carregados ${todos.length} produtos de ${totalPags} paginas`);
    } catch (e) {
      registro.erro("[estoqueCache] Erro carregando:", e?.message);
    }
    estoqueCache = todos;
...
async function buscarProdutoPorDescricao(descricao, appKey, appSecret) {
  ...
  try {
    ...
    registro.aviso("[buscarProdutoPorDescricao] erro:", e?.message, "| descricao:", descNorm.substring(0, 60));
  }
...
export async function criarPedidoOmie(input) {
  const {
    ...
    prazoPagamento,
    nCodProj,
    cObs,        // observação EXTERNA (impressa no pedido enviado ao fornecedor)
    cContrato,   // "Nº do Contrato" no Omie
  } = input;
  ...
  // ⚠ DEMO: o pedido "nasce" com número DEMO-… para a tela de Compras chegar ao fim do fluxo sem
  // abrir pedido no ERP de verdade. Ver lib/modo-demo.js.
  if (emModoDemo()) {
    const n = String(Date.now()).slice(-6);
    return { success: true, demo: true, codigo_pedido: `9${n}`, codigo_pedido_integracao: `DEMO-${n}`, numero_pedido: `DEMO-${n}`, nCodFor_resolvido: Number(nCodForInput) || 0, local_aplicado: false, omie_response: {} };
  }
  ...
  for (let idx = 0; idx < itens.length; idx++) {
    ...
    // Unidade: SEMPRE a que está CADASTRADA no Omie para o produto — não a da RM,
    // que vem descritiva ("LATA 2,80L") e virava máscara ("LATA28"). Ordem: produto
    // resolvido por descrição (Omie) → catálogo sincronizado (EstoqueItem) → fallback RM.
    let unidadeOmie = null;
    let descricaoOmie = null; // descrição CADASTRADA no Omie (nunca sobrescrever com a da RM)
    ...
    if (!cProduto && item.descricao) {
      ...
      if (buscaDesc) {
        ...
        unidadeOmie = buscaDesc.unidade || null;
      }
    }
    if (cProduto) {
      const est = await prisma.estoqueItem
        .findUnique({ where: { codigoOmie: String(cProduto).trim() }, select: { unidade: true, descricao: true } })
        .catch(() => null);
      if (est) {
        if (!unidadeOmie && est.unidade) unidadeOmie = est.unidade;
        if (est.descricao) descricaoOmie = est.descricao;
      }
    }
    // Unidade da RM é "descritiva" (não uma sigla limpa) → tem espaço, número ou passa de 4 chars.
    const unidadeRM = String(item.unidade || "").trim();
    const unidadeRM_descritiva = !unidadeRM || unidadeRM.length > 4 || /[\s\d]/.test(unidadeRM);

    if (!cProduto) {
      const descTrunc = String(item.descricao || "").substring(0, 100);
      registro.erro(`[criarPedidoOmie] Produto nao encontrado: "${descTrunc}" | codigo direto: ${codigoItem || "nenhum"}`);
      return {
        error:
    ...
    produtos_incluir.push({
      ...
      cProduto,
      // Descrição CADASTRADA no Omie (não a da RM, que pode vir mascarada). Fallback: a da RM.
      cDescricao: String(descricaoOmie || item.descricao || "").substring(0, 255),
      // Unidade: se a da RM é DESCRITIVA (longa/com número/espaço, ex.: "LATA 2,80L" → virava
      // "LATA28"), usa a cadastrada no Omie VERBATIM (ex.: "LATA"). Se a RM já manda unidade limpa
      // e intencional (ex.: "KG" no aço, cuja qtd vai em peso), mantém — não sobrescreve com o catálogo.
      cUnidade: (unidadeRM_descritiva && unidadeOmie) ? String(unidadeOmie).trim() : normalizeUnidade(item.unidade),
      nQtde: Number(item.qtd) || 0,
      // ⚠⚠ PREÇO LÍQUIDO + IPI DESTACADO (Matheus, 16/09/2026). Antes daqui saía o unitário já
      // multiplicado pela alíquota e o IPI do item ficava ZERADO no Omie — o rufo da T118-006-R00
      // chegou como 42,714 (40,68 × 1,05) com IPI 0,00. `nValorIpi` é o imposto em REAIS, não em
      // porcentagem (confirmado no pedido 2077 corrigido à mão e na documentação da API).
      //
      // ⚠ O TOTAL DO PEDIDO NÃO MUDA: o que era preço cheio virou preço + imposto separados.
      nValUnit: Number(item.precoUnit) || 0,
      ...(Number(item.valorIpi) > 0 ? { nValorIpi: Number(item.valorIpi) } : {}),
      // ⚠ A observação que a engenharia escreveu no item da RM (Matheus, 16/09/2026). É a mesma
      // que o fornecedor lê na cotação — quem recebe a mercadoria vê a mesma instrução.
      ...(item.observacao ? { cObs: sanitizarOmie(String(item.observacao)).substring(0, 255) } : {}),
      nDesconto: 0,
  ...
  const cabecalho = {
    ...
    nCodFor: nCodFor || 0,
    cNumPedido: sanitizarOmie(cNumPedido || ""),
    cCodCateg: categoria,
    cObs: sanitizarOmie(cObs || ""),
    cObsInt: sanitizarOmie(obsCombinada),
    ...(cContrato ? { cContrato: sanitizarOmie(cContrato) } : {}),
