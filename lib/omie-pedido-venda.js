...
const registro = log("omie-pedido-venda");
...
export async function consultarPedidoVenda({ numeroPedido, codigoPedido } = {}) {
  ...
  // Verifica cache primeiro (30s) — evita repetir chamada
  const cached = getCached(param);
  if (cached) {
    registro.info("[omie-pedido-venda] cache HIT", JSON.stringify(param));
    return cached;
  }

  // UMA UNICA chamada ao Omie. Sem retry, sem variacoes — exatamente como
  // era originalmente quando funcionava. O erro REDUNDANT eh tratado mas
  // sem retry forcado (so cacheia o resultado pra evitar repeticao).
  let data = null;
  let res = null;
  try {
    ...
    registro.info("[omie-pedido-venda]", JSON.stringify(param), "→ HTTP", res.status, "keys", Object.keys(data || {}).join(","));
  } catch (e) {
  ...
  try {
    ...
    const rawParcelas = data?.pedido_venda_produto?.lista_parcelas
      || data?.pedido_venda_produto?.listaParcelas
      || data?.lista_parcelas;
    // Omie pode retornar lista_parcelas como objeto { parcela: [...] } ou array direto
    const listaParcelas = Array.isArray(rawParcelas)
      ? rawParcelas
      : Array.isArray(rawParcelas?.parcela)
        ? rawParcelas.parcela
        : [];
