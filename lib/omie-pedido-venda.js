...
export async function consultarPedidoVenda({ numeroPedido, codigoPedido } = {}) {
  ...
  try {
    ...
    // CALCULO DO VALOR JA FATURADO (medido):
    // 1. Soma parcelas com nFatura > 0 ou marcadas como faturadas
    // 2. Se etapa = 60 (Faturado integralmente), assume valor total
    // 3. Senao mantem o total — usuario pode ajustar manualmente
    let valorFaturado = 0;
    for (const p of listaParcelas) {
      ...
      if (ehFaturada) {
        valorFaturado += Number(p.valor || p.nValor || p.valor_parcela || 0);
      }
    }
    // Etapa 60 = Faturado total: se nao achou faturado nas parcelas, usa total
    if (valorFaturado === 0 && etapaNum === 60) {
      valorFaturado = valorTotalPedido;
    }
    // Mensagem informativa pra UI quando nada foi faturado ainda
    const aindaNaoFaturado = valorFaturado === 0 && etapaNum < 50;

    // valorBruto = JA FATURADO (medido). Quando zero, retorna o total pedido
    // mas avisa que ainda nao foi medido — usuario pode editar manualmente.
    const valorBrutoFinal = valorFaturado > 0 ? valorFaturado : valorTotalPedido;
