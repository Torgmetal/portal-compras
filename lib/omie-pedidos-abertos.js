...
// Mapa reverso número-da-OP → código do projeto Omie (cache 10 min).
let opProjCache = { map: null, ts: 0 };

/**
 * Resolve o CÓDIGO do projeto Omie já cadastrado para uma OP, pelo número.
 * Ex.: OP "089" → projeto "OP-089 TMSA Termasa" → codigo 7777709076.
 * Retorna null se não houver projeto correspondente (aí o pedido não vincula
 * projeto, em vez de o Omie criar um novo).
 * @param {string|number} numeroOp
 * @returns {Promise<number|null>}
 */
export async function resolverCodProjetoPorOp(numeroOp) {
  if (numeroOp == null || numeroOp === "") return null;
  const alvo = String(parseInt(numeroOp, 10)).padStart(3, "0");
  if (Number.isNaN(parseInt(numeroOp, 10))) return null;
  if (!opProjCache.map || Date.now() - opProjCache.ts > 600_000) {
    const projMap = await getProjetoMap();
    const rev = new Map();
    for (const [codigo, nome] of projMap) {
      const op = numeroOpDoProjeto(nome);
      if (op && !rev.has(op)) rev.set(op, Number(codigo));
    }
    opProjCache = { map: rev, ts: Date.now() };
  }
  return opProjCache.map.get(alvo) ?? null;
}

const normProj = (s) => String(s || "").toUpperCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/\s+/g, " ").trim();

/**
 * Resolve o CÓDIGO do projeto Omie pelo NOME (ex.: "OP 000 - GERAL TORG").
 * Usado p/ vincular RMs Consumíveis ao projeto geral. Match exato normalizado e,
 * como fallback, "contém". Retorna null se não achar.
 * @param {string} nome
 * @returns {Promise<number|null>}
 */
export async function resolverCodProjetoPorNome(nome) {
  const alvo = normProj(nome);
  if (!alvo) return null;
  const map = await getProjetoMap();
  for (const [codigo, n] of map) if (normProj(n) === alvo) return Number(codigo);
  for (const [codigo, n] of map) if (normProj(n).includes(alvo)) return Number(codigo);
  return null;
}

/**
 * Mapa código-do-projeto (String) → { nome, numeroOp } de TODOS os projetos do
 * Omie. Reusa o cache de getProjetoMap. Usado para enriquecer contas a pagar
 * com o nome da obra e a OP na visão "A pagar por obra".
 * @returns {Promise<Map<string, { nome: string, numeroOp: string|null }>>}
 */
export async function getProjetosInfo() {
  const map = await getProjetoMap();
  const info = new Map();
  for (const [codigo, nome] of map) {
    info.set(String(codigo), { nome, numeroOp: numeroOpDoProjeto(nome) });
  }
  return info;
}
...
async function consultarOmie() {
  ...
  for (let pg = 1; pg <= MAX_PAGINAS; pg++) {
    ...
    for (const p of (d.pedido_venda_produto || [])) {
      ...
      // Pedido ENCERRADO no Omie (cab.encerrado="S") é terminal: NÃO é "a faturar".
      // Ex.: medição faturada parcialmente e encerrada — o saldo não será faturado.
      // A etapa fica "10" (não faturado) mesmo encerrada, por isso caía em aberto.
      const encerrado = cab.encerrado === "S" && !faturado && !cancelado;
      ...
      brutos.push({
        ...
        dataPrevisao: dataPrev, cancelado, faturado, encerrado,
        encMotivo: encerrado ? (cab.enc_motivo || null) : null,
        atrasado: !!(dataPrev && dataPrev < hoje && !faturado && !cancelado && !encerrado),
        situacao: cancelado ? "Cancelado" : faturado ? "Faturado" : encerrado ? "Encerrado" : (ETAPAS[String(cab.etapa || "")] || "Em aberto"),
      });
    }
    if (pg >= Number(d.total_de_paginas || 1)) break;
  }
  ...
  for (const p of brutos) {
    if (!obrasMap.has(p.codProj)) {
      obrasMap.set(p.codProj, {
        ...
        faturado: 0, aFaturar: 0, cancelado: 0, encerrado: 0, atrasado: false,
    ...
    else if (p.encerrado) o.encerrado += p.valor; // terminal — fora do "a faturar"
    ...
    if (!p.cancelado && !p.encerrado) { if (p.faturado) ped.faturado += p.valor; else ped.aFaturar += p.valor; }
    ped.parcelas.push({
