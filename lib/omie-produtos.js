...
/**
 * O cadastro de produtos do Omie lido AGORA, sem gravar nada — para quem precisa do que existe no
 * Omie neste momento e não pode esperar o cache semanal (a planilha de materiais do Tekla, ver
 * lib/materiais-tekla-publicar.js).
 * @returns {Promise<Array<{codigo:string, descricao:string, unidade:string|null, familia:string|null, inativo:boolean}>>}
 */
export async function listarProdutosOmie() {
  ...
  do {
    ...
    totalPaginas = j.total_de_paginas || 1;
    for (const pr of j.produto_servico_cadastro || []) {
      const codigo = String(pr.codigo || "").trim();
      if (!codigo) continue;
      out.push({
        codigo,
        descricao: limpar(pr.descricao),
        unidade: pr.unidade ? String(pr.unidade).trim() : null,
        familia: pr.descricao_familia ? limpar(pr.descricao_familia) : null,
