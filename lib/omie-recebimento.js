...
import { omieCall as omieBase } from "./omie-call.js";
...
async function omieCall(url, call, param) {
  ...
  try {
    return await omieBase(url, call, p);
  } catch (e) {
...
export async function syncEntregas(prisma, opts = {}) {
  ...
  for (const pedido of pedidos) {
    ...
    if (Date.now() - t0 > DEADLINE_MS) { timeboxed = true; break; }
    try {
      let recebido = false;
      let parcial = false;
      let dataRecebimento = null;
      ...
      if (pedido.faturamentoDireto && !recebido) {
        try {
          const rem = await consultarStatusRemessa(pedido.codigoPedido);
  ...
  // preenchido (max 10 por sync pra nao estourar rate limit). Pula se já
  // estourou o tempo (o principal já rodou).
  let itensSync = 0;
  try {
    if (Date.now() - t0 > DEADLINE_MS) throw new Error("timeboxed");
