  ...
  for (const pedido of pedidos) {
    ...
    try {
      ...
      if (recebido || parcial) {
        ...
        // 🚫 DESLIGADO (Vitor, 19/08): "vamos deixar sem o OMIE_SYNC por ora e manter o manual".
        // O recebido passa a vir do CMR (lib/recebimento-cmr.js) + lançamento manual. O Omie
        // marcava recebimento que o CMR não confirma: em 10 grupos de material o OMIE_SYNC diz
        // que chegou MAIS do que o Almoxarifado lançou — na OP-097 são 6.233 kg de perfil H
        // contra 445 kg no CMR. Os 286 lançamentos antigos ficam como histórico (apagá-los faria
        // item já recebido voltar a "aguardando").
        //
        // O resto da sync continua: datas de entrega, atraso e status do PedidoOmie seguem vindo
        // do Omie — o que saiu foi só a baixa do recebimento por item.
        const CRIAR_RECEBIMENTO_DO_OMIE = false;
        for (const rmItem of CRIAR_RECEBIMENTO_DO_OMIE ? pedido.rmItens : []) {
