...
import { omieCall as omieBase, ORCAMENTO_ESGOTADO } from "./omie-call.js";
import { backfillItensOmie } from "./omie-backfill-itens";
import { omiePodeBaixar } from "./recebimento-fonte";
import { log } from "@/lib/log";

const registro = log("omie-recebimento");
...
// Delega ao helper central (com retry em erro transitório do Omie). Aqui o
// `param` já vem como ARRAY (convenção deste módulo) — desembrulha p/ o central,
// que monta `param: [param]`. Preserva o prefixo "Omie <call>:" nos erros.
async function omieCall(url, call, param, opts = {}) {
  const p = Array.isArray(param) ? (param[0] ?? {}) : param;
  try {
    return await omieBase(url, call, p, opts);
  } catch (e) {
    throw new Error(`Omie ${call}: ${e.message}`);
  }
}
 ...
 */
export async function consultarPedidoCompra(nCodPed, opts = {}) {
 ...
 */
export async function buscarNFsPorPedidos(codigosPedidos, opts = {}) {
  ...
  try {
    ...
    registro.aviso("[buscarNFsPorPedidos] erro:", e.message);
  }
 ...
 */
export async function consultarStatusRemessa(nCodPed, opts = {}) {
  try {
 ...
 */
export async function verificarRecebimentoPedido(codigoPedido, opts = {}) {
  if (!codigoPedido) return { recebido: false, parcial: false, etapa: "SEM_CODIGO" };

  try {
    const pedido = await consultarPedidoCompra(codigoPedido, opts);
 ...
 */
export async function syncEntregas(prisma, opts = {}) {
  const apenasPendentes = !!opts.apenasPendentes; // manual: re-checa só os sem entrega (rápido)
  // Limite de tempo do cron (Vercel maxDuration=300s): processa os pedidos até
  // ~210s e para, gravando progresso. A ordenação por `syncEntregaEm` (nunca
  // verificados primeiro, depois os mais antigos) garante ROTAÇÃO — cada execução
  // pega os que estão há mais tempo sem checar, cobrindo todos ao longo do tempo
  // sem estourar o tempo e sem ser morto antes de registrar o heartbeat.
  const t0 = Date.now();
  // Deadline configurável: o caminho MANUAL passa um valor MENOR que o maxDuration da
  // rota, pra a função retornar JSON antes de o Vercel matá-la (senão o client recebe a
  // página de erro HTML → "Unexpected token 'A'"). Cron: 210s (maxDuration 300).
  const DEADLINE_MS = Number(opts.deadlineMs) > 0 ? Number(opts.deadlineMs) : 210_000;
  // ⚠⚠ O PRAZO VIROU INSTANTE ABSOLUTO, E DESCE ATÉ O `fetch` (`ateMs` do `omieCall`). Antes
  // ele só era conferido ENTRE um pedido e outro, e uma chamada iniciada com 1s de orçamento
  // ainda podia gastar 45s × 5 tentativas — ou seja, o "deadline de 60s" do caminho manual não
  // garantia 60s coisa nenhuma, e era exatamente o que fazia a Vercel matar a rota e o
  // navegador receber HTML no lugar de JSON (achado do Codex, 17/09/2026).
  //
  // ⚠ Quem passa `ateMs` manda; `deadlineMs` continua valendo para quem já chamava assim.
  const fim = Number(opts.ateMs) > 0 ? Number(opts.ateMs) : t0 + DEADLINE_MS;
  const acabou = () => Date.now() >= fim;

  // ⚠⚠ `dataEntregaReal` NÃO PODE DESLIGAR O SYNC. Ela é uma ANOTAÇÃO HUMANA — três rotas a
  // escrevem, duas delas sem pedir nota fiscal e sem criar `Recebimento`. Usá-la como filtro de
  // exclusão fazia o pedido sair do sync PARA SEMPRE no instante em que alguém clicasse "entregue".
  //
  // Medido em 23/08: 29 pedidos (R$ 1.441.870) marcados entregues sem um único item recebido, e
  // nenhum com número de NF. Como o `reverter` também não limpa o campo, era definitivo.
  //
  // Quem diz que não há mais nada a receber é o RECEBIMENTO, não o clique. O corte agora é por
  // idade: pedido antigo demais para o Omie ainda ter novidade sai da rotação, e nada mais o tira.
  const limiteIdade = new Date();
  limiteIdade.setMonth(limiteIdade.getMonth() - 12);

  const pedidos = await prisma.pedidoOmie.findMany({
    where: {
      createdAt: { gte: limiteIdade },
      ...
      // Sync MANUAL "sincronizar entregas": re-checa só os SEM entrega (os que podem
      // mudar de status). No cron, `dataEntregaReal` NÃO filtra (ver aviso acima) — mas
      // ela ORDENA: não-entregues primeiro, pra caberem sempre no timebox.
      ...(apenasPendentes ? { dataEntregaReal: null } : {}),
    },
    // Não-entregues primeiro (dataEntregaReal null → nulls:"first"), depois os há mais
    // tempo sem checar. Garante que os 39 pendentes sejam verificados TODA rodada (só ~50s),
    // em vez de a rotação gastar o orçamento re-checando pedidos já entregues.
    orderBy: [
      { dataEntregaReal: { sort: "asc", nulls: "first" } },
      { syncEntregaEm: { sort: "asc", nulls: "first" } },
    ],
    select: {
      id: true,
      codigoPedido: true,
  ...
  // ⚠ Essa varredura roda ANTES do loop, SEM checagem de deadline — se o Omie estiver
  // lento, sozinha já estoura o tempo. No caminho MANUAL (pularNF), a gente pula: o
  // status de entrega não depende dela; o cron diário associa as NFs depois.
  const codigosPedidos = pedidos.map((p) => p.codigoPedido);
  let nfsPorPedido = new Map();
  if (!opts.pularNF) {
    try {
      nfsPorPedido = await buscarNFsPorPedidos(codigosPedidos, { ateMs: fim });
    } catch (e) {
      registro.aviso("[syncEntregas] buscarNFsPorPedidos falhou:", e.message);
    }
  }

  for (const pedido of pedidos) {
    // Para antes do limite de tempo (o restante entra na próxima execução — a
    // ordenação por syncEntregaEm garante que os não-verificados venham primeiro).
    if (acabou()) { timeboxed = true; break; }
    try {
      let recebido = false;
      let parcial = false;
      let dataRecebimento = null;
      ...
      // Recebimento por QUANTIDADE + NF (nQtdeRec do ConsultarPedCompra) é a fonte
      // confiável pra TODO pedido, inclusive FD. Antes o FD desviava só pra remessa
      // e IGNORAVA a NF de entrada + a qtd recebida — pedido mal classificado como
      // FD (ex.: compra de aço com NF de entrada) ficava eternamente "aguardando
      // entrega". Agora a checagem por quantidade roda sempre; a remessa "faturada"
      // do FD entra como sinal ADICIONAL (abaixo), não como substituto.
      const status = await verificarRecebimentoPedido(pedido.codigoPedido, { ateMs: fim });
      // ⚠⚠ "NÃO CONSEGUI PERGUNTAR" NÃO É "NÃO CHEGOU" (achado do Codex, 17/09/2026).
      // `verificarRecebimentoPedido` engole a exceção e devolve `etapa: "ERRO"`; sem esta linha o
      // pedido entrava como PENDENTE, somava em `processados` e a rodada terminava dizendo
      // `timeboxed: false` e `erros: 0` — uma varredura cega se anunciando completa.
      if (status.etapa === "ERRO") throw new Error(status.error || "falha ao consultar o pedido no Omie");
      ...
      const updateData = {};
      if (status.itensOmie?.length > 0) updateData.itensOmie = status.itensOmie;
      if (status.previsaoEntrega && !pedido.prazoEntregaPrevisto) updateData.prazoEntregaPrevisto = status.previsaoEntrega;
      if (Object.keys(updateData).length > 0) {
        await prisma.pedidoOmie.update({ where: { id: pedido.id }, data: updateData });
      }

      // FD: a remessa "faturada" conta como recebido também (sinal adicional).
      if (pedido.faturamentoDireto && !recebido) {
        try {
          const rem = await consultarStatusRemessa(pedido.codigoPedido, { ateMs: fim });
          if (rem.faturada) { recebido = true; if (!dataRecebimento) dataRecebimento = new Date(); }
        } catch { /* não-fatal */ }
      }
      ...
      if (recebido || parcial) {
        ...
        // ⚖️ O Omie só dá baixa no que NÃO passa pelo CMR (Vitor, 19/08: "para deixarmos o Omie
        // apenas para esses itens seria um problema?"). Material que o Almoxarifado lança —
        // aço, tinta, parafuso, arruela — é do CMR (lib/recebimento-cmr.js), porque o Omie marca
        // recebimento que o CMR não confirma: em 10 grupos ele diz que chegou MAIS do que o
        // Almoxarifado lançou; na OP-097, 6.233 kg de perfil H contra 445 kg no CMR.
        //
        // Sobram pro Omie os consumíveis de oficina e serviços: prisioneiro, bico de solda, disco
        // de corte, autobrocante, luva — 75 itens que o CMR nunca vai fechar.
        //
        // O resto da sync não muda: datas de entrega, atraso e status do PedidoOmie seguem vindo
        // do Omie. O que saiu foi só a baixa de recebimento do material rastreado.
        for (const rmItem of (opts.pularNF ? [] : pedido.rmItens)) {
          // Manual (pularNF): não cria Recebimentos (ficariam sem NF) — só atualiza o status do
          // pedido abaixo. O cron diário cria os Recebimentos já com a NF associada.
          if (!(await omiePodeBaixar(rmItem.descricao))) continue;
      ...
      // ⚠ Acabar o orçamento no meio de um pedido é TIMEBOX, não defeito de integração: marcar
      // como erro encheria o relatório de falhas numa rodada que só ficou sem tempo.
      if (String(e?.message || "").includes(ORCAMENTO_ESGOTADO)) { timeboxed = true; break; }
      erros++;
      detalhes.push({
        pedidoId: pedido.id,
        numero: pedido.numeroPedido,
    ...
    // Marca como verificado agora (rotação: próximas execuções pegam primeiro os
    // que estão há mais tempo sem checar). Vale mesmo em erro — evita ficar preso
    // num pedido problemático toda execução.
    await prisma.pedidoOmie.update({ where: { id: pedido.id }, data: { syncEntregaEm: new Date() } }).catch(() => {});
    processados++;
  ...
  // ⚠ O backfill é trabalho de FUNDO — mora em `lib/omie-backfill-itens.js` desde 17/09/2026,
  // quando o prazo passou a ser conferido a cada pedido dele (antes era só na entrada, e ele
  // podia sozinho estourar o orçamento inteiro depois de o laço principal já ter parado).
  const itensSync = await backfillItensOmie(prisma, fim, consultarPedidoCompra);

  return { sincronizados, erros, total: pedidos.length, processados, timeboxed, itensSync, detalhes };
}
