...
export async function verificarRecebimentoPedido(codigoPedido) {
  ...
  try {
    ...
    const cabecalho = pedido.cabecalho || pedido.pedido_compra_produto?.cabecalho || pedido;
    ...
    const recebido = ETAPAS_RECEBIDO.includes(etapa);
    const parcial = ETAPAS_PARCIAL.includes(etapa);

    // Tenta extrair data do recebimento dos itens
    let dataRecebimento = null;
    const itens = pedido.det || pedido.pedido_compra_produto?.det || [];
    for (const item of itens) {
      const prod = item.produto || {};
      if (prod.dDtRecebimento) {
        const parts = prod.dDtRecebimento.split("/");
        if (parts.length === 3) {
    ...
    // Extrai previsão de entrega do Omie (campo dDtPrevisao em cabecalho_consulta)
    let previsaoEntrega = null;
    const cabConsulta = pedido.cabecalho_consulta
      || pedido.pedido_compra_produto?.cabecalho_consulta || {};
    if (cabConsulta.dDtPrevisao) {
      const parts = cabConsulta.dDtPrevisao.split("/");
      if (parts.length === 3) {
        const dt = new Date(`${parts[2]}-${parts[1]}-${parts[0]}`);
    ...
    return { recebido, parcial, etapa, dataRecebimento, previsaoEntrega };
  } catch (e) {
