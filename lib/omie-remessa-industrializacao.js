...
function montarItensMaterial(materiaisResolvidos, numeroRomaneio) {
  const linhas = [];
...
export async function criarPedidoRemessa(romaneio, terceiro = {}, opts = {}) {
  ...
  if (resolvidos && resolvidos.length > 0) {
    ...
    det = montarItensMaterial(resolvidos, romaneio.numero);
  } else {
    ...
    det = montarItensMarca(romaneio, { valorKg: cfg.valorKg });
  }
