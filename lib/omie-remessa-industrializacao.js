...
export async function criarPedidoRemessa(romaneio, terceiro = {}, opts = {}) {
  ...
  const param = {
    ...
    frete: { cTpFrete: cfg.tpFrete }, // modalidade do frete — obrigatória p/ o SEFAZ
    infAdic: {
...
/**
 * CONCLUI/EMITE a NF-e da remessa no Omie (ConcluirRemessa → SEFAZ). AÇÃO IRREVERSÍVEL.
 * Tenta capturar o nº/série/chave da NF da resposta (nomes variam) — se não vier, devolve null.
 * @returns {{ ok:true, nf:{numero,serie,chave}|null, _raw } | { ok:false, erro:string }}
 */
export async function concluirRemessaOmie(nCodRem) {
  if (!nCodRem) return { ok: false, erro: "Remessa sem código no Omie." };
  let res;
  try {
    res = await omieCall(URL_REMESSA_FAT, "ConcluirRemessa", { nCodRem: Number(nCodRem) });
  } catch (e) {
    return { ok: false, erro: e.message };
  }
  const status = String(res.cCodStatus ?? "0");
  // status "0" ok; alguns retornam faultstring já tratado no omieCall
  if (status !== "0" && (res.cDescStatus || res.cDesStatus)) {
    return { ok: false, erro: res.cDescStatus || res.cDesStatus };
  }
  // Captura defensiva do nº/chave da NF (nomes possíveis na resposta do Omie).
  const numero = res.nNF ?? res.cNumeroNfe ?? res.nNfNumero ?? res.numero_nf ?? res.cNF ?? null;
  const serie = res.cSerie ?? res.nSerie ?? res.serie ?? null;
  const chave = res.cChaveNFe ?? res.chave_nfe ?? res.cChave ?? res.cChaveNfe ?? null;
  const nf = (numero || chave) ? { numero: numero != null ? String(numero) : null, serie: serie != null ? String(serie) : null, chave: chave ? String(chave) : null } : null;
  return { ok: true, nf, _raw: res };
}
