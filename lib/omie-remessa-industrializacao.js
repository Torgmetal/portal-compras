...
// (`produtos/remessa` → IncluirRemessa), SEPARADO do Pedido de Venda (`produtos/pedido`) — usar
// o pedido de venda cai na listagem de VENDAS, não na tela "Remessa de Produto".
//
// Estratégia (SEGURA/REVERSÍVEL): o portal só CRIA a remessa (rascunho, NÃO faturada). O Fiscal
// confere no Omie e FATURA lá (emite a NF-e no SEFAZ, via produtos/remessafat). Depois o portal
// registra nº/série/chave da NF. `cabec`/`frete`/`infAdic` conferidos contra uma remessa real da
// conta via ConsultarRemessa — não documentados nos artigos de ajuda (ex.: `nCodProj` mora em
// `infAdic`, não em `cabec`; o bloco `frete` NÃO tem campo de RNTC/RNTRC — não é possível setar
// isso por esta API, nem existe no cadastro do cliente/transportadora consultado).
//
...
export function configRemessa() {
  ...
  // Código de Benefício Fiscal (SP): obrigatório p/ CST com benefício. CST 41 (não tributada)
  // usa SP099999 nas remessas da conta. Override por env (muda junto se mudar o CST).
  const codigoBenef = (process.env.OMIE_REMESSA_CODIGO_BENEF ?? "SP099999").trim();
  return { ok: Boolean(cenario), cenario, categoria, cfopForcado, valorKg, cstIcms, cstPisCofins, cstIpi, ipiTpCalc, origem, tpFrete, codigoBenef };
}
...
function montarProdutos(linhas, cCodIntRem, cfop, cfg) {
  ...
  return linhas.map((l, i) => ({
    ...
    infAdicItem: {
      // Código de Benefício Fiscal — SEFAZ-SP exige p/ CST com benefício (ex.: 41 → SP099999).
      ...(cfg.codigoBenef ? { codigo_benef: cfg.codigoBenef } : {}),
      ...(l.info ? { cInfItemNF: String(l.info).substring(0, 200) } : {}),
    },
  }));
}
...
export async function criarPedidoRemessa(romaneio, terceiro = {}, opts = {}) {
  ...
  if (resolvidos && resolvidos.length > 0) {
    ...
    const valorKg = Number(opts.valorKg) > 0 ? Number(opts.valorKg) : cfg.valorKg; // Fiscal pode informar na tela
    if (!(valorKg > 0)) return { erro: "Defina o valor por kg antes de gerar — a NF não pode sair com valor zero." };
    ...
    linhas = marcas.map((it) => {
      ...
      return { nCodProd: id, nQtde: q, nValUnit: valorKg, info };
    });
  }
  ...
  // ⚠⚠ SEM ISTO O PROJETO FICAVA EM BRANCO NA REMESSA (Matheus, 09/09/2026) — mesmo resolvedor
  // do Pedido de Compra (número da OP → projeto já cadastrado no Omie). Best-effort: sem projeto
  // correspondente, a remessa sai sem vínculo, como sempre saiu.
  let nCodProj = null;
  if (romaneio.opRefNumero) { try { nCodProj = await resolverCodProjetoPorOp(romaneio.opRefNumero); } catch { /* best-effort */ } }
  ...
  const param = {
    ...
    infAdic: {
      ...
      cDadosAdic: [opts.infoAdic ? String(opts.infoAdic).trim() : `Remessa - Romaneio ${romaneio.numero}`, romaneio.opRefNumero ? `OP ${romaneio.opRefNumero}` : "", romaneio.servico ? `Obra/Servico: ${romaneio.servico}` : ""].filter(Boolean).join(" | ").substring(0, 500),
...
export async function statusNfDaRemessa(nCodRem) {
  ...
  } catch { /* sem DANFE ainda */ }
  // DANFE presente = AUTORIZADA (sinal positivo confiável). Ausência de DANFE é AMBÍGUA:
  // pode ser rejeitada OU ainda em processamento (o PDF leva alguns segundos após autorizar).
  // Por isso NÃO afirmamos rejeição aqui — devolvemos PENDENTE.
  return danfeUrl
    ? { estado: "AUTORIZADA", nf: { numero: nf.numero, serie: nf.serie, chave: nf.chave }, danfeUrl }
    : { estado: "PENDENTE", nf: { numero: nf.numero, serie: nf.serie, chave: nf.chave } };
}

/**
 * CONCLUI a remessa no Omie (ConcluirRemessa → SEFAZ) e confirma a autorização REAL via DANFE.
 * Só devolve ok quando o DANFE aparece (prova de autorização). Senão devolve pendente.
 * @returns {{ ok:true, autorizada:true, nf } | { ok:false, pendente?:true, erro:string }}
 */
export async function concluirRemessaOmie(nCodRem) {
  ...
  // Poll da autorização (DANFE) por ~75s — a NF autoriza rápido, mas o DANFE demora a gerar.
  for (let i = 0; i < 15; i++) {
  ...
  return {
    ok: false, pendente: true,
    erro: "NF-e enviada ao SEFAZ, mas a autorização ainda não confirmou aqui. Clique \"Atualizar status\" em ~1 min. Se não autorizar, veja o motivo no Omie (aba Comunicação com a SEFAZ) e reenvie.",
  };
}
