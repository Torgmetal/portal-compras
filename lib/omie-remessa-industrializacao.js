...
export async function criarPedidoRemessa(romaneio, terceiro = {}) {
  ...
  // Regra do negócio (Matheus): manda-se ARMAÇÃO (ARM000001) quando são MARCAS
  // (peças fabricadas — galvanização/pintura/jato). Quando se envia MATÉRIA-PRIMA
  // (perfis/chapas comprados p/ o terceiro fabricar), a NF usa os PRODUTOS REAIS do
  // Omie (que existem porque foram comprados → RM + pedido de compra). Hoje o
  // romaneio guarda só o TEXTO do perfil (ex.: 'TB 3/4" - DIN2440'), sem o código do
  // produto Omie — então esse caminho ainda não pode emitir sozinho. Bloqueia com
  // mensagem clara p/ não emitir uma NF errada (ARM000001 no lugar do produto real).
  if (materiais.length > 0) {
    return { erro: `Este romaneio envia MATÉRIA-PRIMA (${materiais.length} perfil/chapa) — a NF precisa dos produtos reais do Omie (da RM/pedido de compra), não do ARM000001. O vínculo perfil→produto Omie ainda não está implementado; use este fluxo por ora só para remessa de MARCAS (peças p/ galvanização/pintura/jato).` };
  }

  if (!(cfg.valorKg > 0)) return { erro: "Defina o valor da remessa por kg (env OMIE_REMESSA_VALOR_KG) antes de emitir — a NF de marcas não pode sair com valor zero." };
