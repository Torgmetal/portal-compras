...
/**
 * Busca a NF-e AUTORIZADA de uma remessa (nfconsultar → ligada por compl.nIdPedido === nCodRem).
 * Só aparece no nfconsultar quando o SEFAZ autoriza (tem número + chave). Devolve null enquanto
 * não autorizou (em processamento OU rejeitada).
 * @returns {{ numero, serie, chave } | null}
 */
export async function buscarNfDaRemessa(nCodRem) {
  const hoje = new Date();
  const ontem = new Date(hoje.getTime() - 2 * 864e5); // janela de 2 dias (fuso/virada)
  ...
  try {
    for (let pagina = 1; pagina <= 5; pagina++) {
      ...
      for (const nf of lista) {
        ...
        if (c.nIdPedido === alvo) {
          // dCan preenchido = cancelada; cDeneg "S" = denegada — nesses casos não vale como emitida.
          if (ide.dCan || ide.cDeneg === "S") return null;
          return { numero: ide.nNF ? String(ide.nNF) : null, serie: ide.serie ? String(ide.serie) : null, chave: c.cChaveNFe || null };
        }
      }
      if (pagina >= (d.total_de_paginas || 1)) break;
    }
  } catch { /* falha de consulta não deve derrubar a emissão */ }
  return null;
}

/**
 * CONCLUI a remessa no Omie (ConcluirRemessa → envia ao SEFAZ). AÇÃO IRREVERSÍVEL.
 * ATENÇÃO — limitação da API Omie: nenhum endpoint distingue NF AUTORIZADA de REJEITADA
 * (ambas aparecem com número no nfconsultar, sem campo de status SEFAZ). Por isso NÃO
 * marcamos EMITIDA automaticamente aqui: devolvemos "enviado" + o nº sugerido, e o Fiscal
 * confirma no Omie (aba Comunicação com a SEFAZ) — usa "Registrar" se autorizou.
 * @returns {{ ok:true, enviado:true, nfSugerida:{numero,serie,chave}|null } | { ok:false, erro:string }}
 */
export async function concluirRemessaOmie(nCodRem) {
  ...
  try {
    ...
    const status = String(res.cCodStatus ?? "0");
    if (status !== "0" && (res.cDescStatus || res.cDesStatus)) {
      return { ok: false, erro: res.cDescStatus || res.cDesStatus };
    }
  } catch (e) {
    return { ok: false, erro: e.message };
  }
  // dá um tempo pro Omie processar e tenta pegar o nº (só como SUGESTÃO — não é prova de autorização).
  await new Promise((r) => setTimeout(r, 6000));
  const nfSugerida = await buscarNfDaRemessa(nCodRem);
  return { ok: true, enviado: true, nfSugerida };
}
