...
/**
 * CONCLUI/EMITE a NF-e da remessa no Omie (ConcluirRemessa → SEFAZ) e ESPERA a autorização
 * real: só devolve ok quando a NF aparece autorizada no nfconsultar. AÇÃO IRREVERSÍVEL.
 * @returns {{ ok:true, nf:{numero,serie,chave} } | { ok:false, erro:string, pendente?:boolean }}
 */
export async function concluirRemessaOmie(nCodRem) {
  if (!nCodRem) return { ok: false, erro: "Remessa sem código no Omie." };
  // Se já existe NF autorizada (ex.: reenvio após já ter autorizado), usa ela.
  const jaTem = await buscarNfDaRemessa(nCodRem);
  if (jaTem) return { ok: true, nf: jaTem };
  ...
  // SEFAZ é assíncrono → aguarda a autorização real (poll ~35s).
  for (let tentativa = 0; tentativa < 8; tentativa++) {
    await new Promise((r) => setTimeout(r, 4500));
    const nf = await buscarNfDaRemessa(nCodRem);
    if (nf) return { ok: true, nf };
  }
  return {
    ok: false, pendente: true,
    erro: "A NF-e ainda não foi autorizada pelo SEFAZ. Pode ter dado erro (ex.: NCM). Abra a remessa no Omie, veja a aba \"Comunicação com a SEFAZ\", corrija e clique em Emitir de novo.",
  };
}
