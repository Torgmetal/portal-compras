...
export function configRemessa() {
  ...
  return { ok: Boolean(cenario), cenario, categoria, cfopForcado, valorKg };
}
...
function cfopRemessa(cfg, romaneio, uf) {
  if (cfg.cfopForcado) return cfg.cfopForcado;
  if (romaneio?.remessaCfop) return String(romaneio.remessaCfop).trim();
  const u = String(uf || "").toUpperCase();
  return u && u !== UF_TORG ? "6901" : "5901";
}
...
/** Linhas resolvidas → itens do IncluirRemessa (nCodProd + CFOP + qtd + valor + info). */
function montarProdutos(linhas, cCodIntRem, cfop) {
...
export async function criarPedidoRemessa(romaneio, terceiro = {}, opts = {}) {
  ...
  if (resolvidos && resolvidos.length > 0) {
    ...
    for (const m of resolvidos) {
      ...
      linhas.push({ nCodProd: id, nQtde: Number(m.qtd), nValUnit: Number(m.valorUnit), info: m.descricao || null });
    }
  ...
  const param = {
    ...
    produtos: montarProdutos(linhas, cCodIntRem, cfop),
  };
