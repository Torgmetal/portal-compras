  ...
  for (const i of itensComerciais) {
    ...
    if (ehEstrutura && detalheMaterial.length && material > 0) {
      for (const g of detalheMaterial) {
        out.push(monta(g.descricao, "KG", i.quantidade, g.subtotal,
          `Item ${i.item} da planilha comercial · ${g.item} ${g.descricao}${g.precoKg ? ` · R$ ${g.precoKg.toFixed(2)}/kg` : ""}`));
      }
    } else if (material > 0) {
      out.push(monta(i.descricao, i.unidade, i.quantidade, material, `Item ${i.item} da planilha comercial · custo de material`));
    }
    ...
    if (mdo > 0) {
      ...
      out.push(monta(desc, i.verba ? null : i.unidade, i.verba ? 0 : i.quantidade, mdo,
        `Item ${i.item} da planilha comercial · mão de obra terceirizada`, "VERBA"));
    }
  }
  return out;
}
