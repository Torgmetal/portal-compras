...
/**
 * Ano digitado errado na planilha — corrige e AVISA.
 *
 * ⚠⚠ Vitor (30/08/2026): "com certeza essas duas que mencionei é erro de digitação, pode
 * considerar". São a 251-26 (Marko) com envio em 31/07/**2027** e a 103-26 (Inpasa) em
 * 24/04/**2025**. Uma proposta de 2026 não foi enviada em 2027 (ainda não aconteceu) nem em 2025
 * (antes de existir), e o efeito prático é ela sumir do Pipeline do ano — que foi exatamente o
 * problema que ele mandou arrumar.
 *
 * ⚠ A CORREÇÃO SÓ VALE QUANDO O NÚMERO CONFIRMA O ANO. A planilha de 2026 carrega linhas de
 * orçamentos de 2025 (a 265-25, fechada em 2026); nelas uma data de 2025 é o correto, e mexer
 * seria inventar. Por isso a regra olha o sufixo do número: só ajusta "nnn-26" dentro da planilha
 * de 2026.
 *
 * ⚠ E NUNCA EM SILÊNCIO: cada ajuste volta em `correcoes` e aparece no resultado da importação.
 * Corrigir dado do cliente sem deixar rastro é como um erro vira verdade.
 */
export function corrigirAnoDigitado(data, numero, ano) {
  if (!data || !numero || !ano) return { data, correcao: null };
  const aa = String(numero).split("-")[1];
  if (!aa || Number(aa) !== Number(String(ano).slice(-2))) return { data, correcao: null };
  const anoData = data.getUTCFullYear();
  if (anoData === Number(ano)) return { data, correcao: null };
  const corrigida = new Date(Date.UTC(Number(ano), data.getUTCMonth(), data.getUTCDate(), 12));
  return {
    data: corrigida,
    correcao: `${data.toISOString().slice(0, 10)} → ${corrigida.toISOString().slice(0, 10)}`,
  };
}
...
export function linhaParaOrcamento(linha, ano = new Date().getUTCFullYear()) {
  ...
  return {
    ...
    ...(() => {
      // as duas datas passam pela mesma checagem de ano; o que for ajustado sai em `correcoes`
      const correcoes = [];
      const trata = (v, rotulo) => {
        const { data, correcao } = corrigirAnoDigitado(dataUS(v), numero, ano);
        if (correcao) correcoes.push(`${rotulo}: ${correcao}`);
        return data;
      };
      return {
        dataEnvio: trata(col(linha, "Data envio"), "envio"),
        dataFechamento: trata(col(linha, "Data fechamento"), "fechamento"),
        correcoes,
      };
    })(),
...
export function semVazios(o) {
  ...
  for (const [k, v] of Object.entries(o)) {
    if (k === "revisoes" || k === "numero" || k === "correcoes") continue;
