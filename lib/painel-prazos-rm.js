...
export const SITUACAO = {
  ...
  CHEGOU: { rotulo: "Chegou", ordem: 5, cor: "emerald" },
};
...
function fecharLinha(r) {
  ...
  const pendentes = r.pedidos.filter((p) => p.situacao !== "CHEGOU" && p.previsao);
...
export function resumoPorSituacao(linhas) {
  ...
  return {
    ...
    // ⚠ "Pendentes" é o que ainda pode ser resolvido hoje: tudo que não chegou. O que chegou sai
    // da conta mesmo tendo atrasado — cobrar o fornecedor por isso agora não muda nada.
    pendentes: (linhas || []).reduce((s, l) => s + l.pedidos.filter((p) => p.situacao !== "CHEGOU").length, 0),
  };
}
...
export function filtrarLinhas(linhas, filtro) {
  ...
  if (!filtro || filtro === "PENDENTES") return todas.filter((l) => l.situacao !== "CHEGOU");
