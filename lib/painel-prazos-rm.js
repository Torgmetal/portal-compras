...
import { linhaDoTempo, hojeEmSP } from "./acompanhamento-pedido";
import { freteDe } from "./frete-cotacao";
import { propostaPendente } from "./prazo-proposto";
import { origemDaPrevisao, historicoDaPrevisao } from "./historico-previsao";
export { origemDaPrevisao, historicoDaPrevisao };
...
export function situacaoDoPedido(pedido, agora = Date.now()) {
  // ⚠ O "hoje" sai do MESMO `agora` que decide o atraso: com dois relógios, um teste (ou uma
  // virada de dia no meio da conta) podia chamar a etapa de prevista e o prazo de vencido.
  const { previsao, atrasoDias, eventos } = linhaDoTempo(pedido, { hoje: hojeEmSP(new Date(agora)) });
  // ⚠⚠⚠ "MATERIAL RECEBIDO" COM DATA FUTURA NÃO É CHEGADA (24/09/2026). Contando, o pedido virava
  // CHEGOU no ato de alguém anotar uma previsão: saía do vermelho, da lista de atrasados e da
  // COBRANÇA por e-mail — o fornecedor deixaria de ser cobrado por causa de uma promessa dele
  // mesmo. Medido no dia: nenhum caso ainda (os 7 futuros eram "liberado para coleta"); o furo
  // estava aberto, esperando o primeiro.
  const chegou = eventos.some((e) => e.etapa === "MATERIAL_RECEBIDO" && !e.prevista);
...
const pedidoResumido = (p, agora) => ({
  ...
  // ⚠ De quem é a data que está valendo — ver `origemDaPrevisao`.
  previsaoDoFornecedor: origemDaPrevisao(p),
  // ⚠⚠ TODAS as remarcações, internas e do fornecedor — ver `historicoDaPrevisao`.
  historicoPrevisao: historicoDaPrevisao(p),
  // ⚠⚠ DECLARAÇÃO, NÃO FATO: o fornecedor marcou "entregue" no portal e passou a NF. Não muda
  // situação nenhuma; serve para alguém CONFERIR, e o pedido continua cobrável até lá.
  entregaDeclarada: p.fornecedorEntregaEm
    ? { em: p.fornecedorEntregaEm, nfNumero: p.fornecedorNfNumero || null }
    : null,
  // ⚠⚠ PROPOSTA, NÃO PRAZO. O fornecedor sugeriu uma data pelo link público e ela aguarda Compras.
  // ⚠ NÃO entra em `situacaoDoPedido`: o pedido continua atrasado, continua cobrável, e o chip da
  // tela não muda. Se a proposta mudasse a situação, responder qualquer data — inclusive uma
  // impossível — seria o jeito mais barato de sair do vermelho, e o silêncio de Compras viraria
  // aprovação tácita (achado do Codex, 18/09/2026).
  propostaPendente: propostaPendente(p),
  total: Number(p.total || 0),
  criadoEm: p.createdAt || null,
  // ⚠⚠ COM O MESMO `agora` DA SITUAÇÃO. Sem ele, as etapas do cartão usavam o relógio da máquina e
  // a situação o `agora` recebido — dois relógios na mesma linha, e a etapa podia dizer "prevista"
  // ao lado de um chip que já a tratava como feita.
  etapas: linhaDoTempo(p, { hoje: hojeEmSP(new Date(agora)) }).eventos.filter((e) => e.tipo === "etapa"),
 ...
 */
/**
 * As obras que têm RM com pedido, com quantas cada uma tem, da mais nova para a mais antiga.
 *
 * ⚠ Sai de TODAS as linhas, nunca das já filtradas — senão escolher uma obra apagaria as outras
 * da lista de opções, e não haveria como voltar sem recarregar a tela.
 */
export function obrasDasLinhas(linhas) {
  const m = new Map();
  ...
  // ⚠ Ordena pelo NÚMERO extraído, não pelo texto: "OP 9" depois de "OP 118" na ordem alfabética.
