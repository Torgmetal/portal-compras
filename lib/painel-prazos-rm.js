 ...
 *
 * ⚠⚠ NOMES PARECIDOS NÃO SÃO FUNDIDOS. O acervo tem "R SIMIONI" e "R SIMIONI IND E COM LTDA" como
 * dois registros distintos, e é tentador juntá-los por prefixo. Não: a mesma heurística juntaria
 * fornecedores de verdade que começam igual, e aí o filtro mostraria pedido que não é daquele
 * fornecedor — erro silencioso e pior que ver dois nomes na lista. A ordem é alfabética justamente
 * para os parecidos ficarem vizinhos e a pessoa decidir.
 */
...
export function filtrarPorFornecedor(linhas, fornecedor) {
  ...
  return (linhas || [])
    .map((l) => ({ ...l, pedidos: (l.pedidos || []).filter((p) => p.fornecedorNome === fornecedor) }))
