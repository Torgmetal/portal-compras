...
export async function parseCMR(buffer) {
  ...
  // CMR é grande (~17MB) e pesado de imagens/formatação/validações. Só precisamos
  // das células, então pulamos os nós que não são dados — acelera muito o load e
  // reduz memória (evita o parse de >2min que estourava o tempo da função).
  await wb.xlsx.load(buffer, {
    ignoreNodes: [
      "drawing", "picture", "conditionalFormatting", "dataValidations",
      "hyperlinks", "extLst", "sheetProtection", "tableParts", "mergeCells",
    ],
  });
