...
// Nº da OP a partir da coluna OBRA. A planilha escreve "OP 067" e — nas OPs de 3 dígitos —
// "OP 0103" (zero extra). Normaliza pro padrão do portal: tira zeros à esquerda e pad 3 →
// "OP 067"→"067", "OP 0103"→"103". Sem casar isso, o vínculo com a OP some. (Vitor 18/08.)
const soDigitos = (s) => {
...
// "1.234,56" / "7567KG" / "35.64" → número em kg (a coluna PESO/LITRO vem com formato livre).
// ⚠ este estava CERTO, mas era o quinto parser paralelo da mesma conversão. Fonte única agora.
const numeroBr = (s) => { const n = numeroBR(s, NaN); return Number.isFinite(n) ? n : null; };

// "7/20/26" (formato da planilha) ou dd/mm/aaaa → Date; senão null.
const dataBr = (s) => {
...
export async function parseCMR(buffer) {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    linhas.push({
      ...
      // ESTRUTURADOS (além do texto acima) — base do status de compra por OP no PCP.
      pedidoCompra: pedido ? String(pedido).slice(0, 60) : null,
      nfNumero: nf ? String(nf).slice(0, 60) : null,
      dataRecebimento: dataBr(receb),
      pesoKg: numeroBr(peso),
      quantidade: numeroBr(get(rowArr, "quantidade")),
      avisos,
    });
  }
