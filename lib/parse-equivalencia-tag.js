...
// ⚠ A regra do rodapé é UMA só (`lib/linha-de-total.js`). A cópia que estava aqui não normalizava
// acento — ver o comentário de lá sobre como três abrangências diferentes viraram um buraco.
import { ehLinhaDeTotal as ehTotal } from "./linha-de-total";
...
/**
 * Acumula a ocorrência sob a marca.
 *
 * ⚠⚠ A CHAVE É A MARCA EM CAIXA ALTA, A MESMA REGRA DE `chaveMarca` (lib/itens-expedicao).
 * Agrupar pela marca crua faz "105A3" e "105a3" virarem duas marcas, cada uma numerando as
 * unidades a partir de 1 — e `tagsPorUnidade`, que normaliza, faz a segunda SOBRESCREVER a
 * primeira: duas peças saem com a TAG errada e duas sem TAG nenhuma (Codex, 14/09/2026).
 * Não importo `chaveMarca` de lá porque aquele módulo é `server-only` e este roda no teste.
 */
const empilhar = (porMarca, marca, dado) => {
  const daMarca = marca.toUpperCase(); // `chave`, no módulo, é outra coisa: normaliza cabeçalho.
  if (!porMarca.has(daMarca)) porMarca.set(daMarca, []);
  porMarca.get(daMarca).push(dado);
};
...
function lerAba(wb, nome, porMarca, problemas) {
  ...
  for (const l of linhas.slice(iCab + 1)) {
    ...
    empilhar(porMarca, marca, { tag, qtd, aba: nome });
    marcas++; pecas += qtd;
  }
