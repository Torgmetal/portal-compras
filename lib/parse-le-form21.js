...
export function parseFormularioLE(buffer, { opNumeroForcado = null, sheetName = null } = {}) {
  ...
  const cIdx = {
    ...
    observacao: findColIdx(header, ["observ", "obs"]), // opcional: galvanização, terceirização, "pintado pronto", área do parafuso…
  };
  ...
  for (let i = dataStart; i < rows.length; i++) {
    ...
    // Pula o rodapé da planilha ("TOTAL.:", "TOTAL GERAL", "SUBTOTAL", "SOMA"). Ele vinha entrando
    // como peça e DOBRAVA o peso da OP: a qtd dele é a soma geral (não 0), então o descarte por
    // qtd===0 abaixo não pegava. Ex.: OP-071 dava 18.664 = 9.332 real + TOTAL 9.332.
    //
    // ⚠ A regra é a compartilhada. Aqui era `startsWith("total")`, mais estreita que a do
    // importador — e quem importa pelo SharePoint não passa pelo importador (ver linha-de-total.js).
    if (ehLinhaDeTotal(marca)) continue;

    const qtdRaw = r[cIdx.qtd];
    const qtd = numeroBR(qtdRaw ?? "", 0);
    if (qtd === 0) continue; // Pula linhas de totais/sem qte

    const pesoUnit = cIdx.pesoUnit >= 0 ? numeroBR(r[cIdx.pesoUnit] ?? "", 0) : 0;
    const pesoTotal = cIdx.pesoTotal >= 0 ? numeroBR(r[cIdx.pesoTotal] ?? "", 0) : pesoUnit * qtd;
    ...
    pecas.push({
      ...
      observacao: cIdx.observacao >= 0 ? String(r[cIdx.observacao] ?? "").trim() || null : null,
      pesoUnitKg: pesoUnit,
