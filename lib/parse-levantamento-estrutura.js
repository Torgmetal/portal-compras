...
export function parseLevantamentoEstrutura(buffer) {
  ...
  // Rótulos precisam estar em colunas distintas. A instrução "Selecione o
  // Perfil/Bitola..." também contém essas palavras, mas não é o cabeçalho;
  // aceitá-la fazia a linha de títulos ser importada como um material fictício.
  for (let i = 0; i < Math.min(15, rows.length); i++) {
    const row = rows[i];
    const cols = row.map((c) => String(c ?? '').trim().toLowerCase());
    const temPerfil=cols.some(c=>/^(perfil|bitola)(\b|\/)/.test(c));
    const temQuantidade=cols.some(c=>/^(qtd|qtde|quantidade)\b/.test(c));
    const temItem=cols.some(c=>/^item$/.test(c));
    const temPeso=cols.some(c=>/^peso\b/.test(c));
    if (cols.filter(Boolean).length>=3&&((temPerfil&&temQuantidade)||(temItem&&temPeso))) {
      headerRowIdx = i;
      break;
    }
  }
  ...
  for (let i = headerRowIdx + 1; i < rows.length; i++) {
    ...
    if (colA === 'ITEM' || colA.includes("SUBTOTAL") || colA.includes("TOTAL")) continue;
