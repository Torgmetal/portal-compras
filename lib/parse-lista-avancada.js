...
export function revisaoDoNome(nome = "") {
  // lookbehind (não \b) porque "T64A-LE_R00" tem "_" antes do R — e "_" é \w,
  // então \bR não casava e a revisão vinha nula nesses arquivos.
  const r = String(nome).match(/(?<![A-Za-z0-9])R(?:ev)?\.?\s*0*(\d{1,3})\b/i);
...
const COLS = {
  item:      (h) => h === "item",
  marca:     (h) => h === "marca",
  // FORM21 traz "QTD." e "REV." (com ponto) — igualdade exata/"revis" não casava
  // e a qtd caía no default 1, subestimando a contagem de peças.
  qte:       (h) => h.startsWith("qtd") || h.startsWith("qte") || h.startsWith("quant"),
  descricao: (h) => h.startsWith("descric"),
  revisao:   (h) => h.startsWith("rev"),
...
export function parseListaAvancada(buffer, nomeArquivo = "") {
  ...
  const marcas = [];
  let vazios = 0;
  let totalizador = null; // linha "TOTAL.:" do rodapé — NÃO é marca
  for (let i = hRow + 1; i < rows.length; i++) {
    const r = rows[i] || [];
    const marca = r[idx.marca] != null ? String(r[idx.marca]).trim() : "";
    if (!marca) { if (++vazios > 25) break; continue; }
    const mn = norm(marca).replace(/[.:;,\-\s]+$/g, "").trim();
    if (mn === "marca") continue; // cabeçalho repetido
    // O FORM21 fecha com uma linha de TOTAL (ex.: "TOTAL.: | 1115 | … | 131432").
    // Contá-la como marca DOBRAVA o peso da lista — guarda como total declarado.
    if (/^(sub)?total\b|^soma\b/.test(mn)) {
      totalizador = { qte: idx.qte !== undefined ? num(r[idx.qte]) : null, pesoTotal: num(r[idx.pesoTotal]) };
      continue;
    }
    vazios = 0;
  ...
  // FORM21 não declara "Contratado" no topo, mas fecha com a linha TOTAL —
  // usa ela como total declarado (serve de conferência contra a soma das marcas).
  const pesoContratadoReport = acharTotal(/contratad/) ?? totalizador?.pesoTotal ?? null;
