...
function normalize(row) {
  ...
  const qtd = numeroBR(qtdRaw.replace(/[^\d.,]/g, ""), NaN) || 1;
  ...
  const pesoLinear = numeroBR(pesoLinearRaw.replace(/[^\d.,]/g, ""), 0);
  const pesoTotalKey = findExact(["peso", "total"]) || findExact(["peso", "kg"]);
  const pesoTotalRaw = String(row[pesoTotalKey] || "0");
  let peso = numeroBR(pesoTotalRaw.replace(/[^\d.,]/g, ""), 0);
  if (peso === 0 && pesoLinear > 0) {
    const compNum = numeroBR(String(comprimento).replace(/[^\d.,]/g, ""), 0);
