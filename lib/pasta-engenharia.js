...
export async function inventarioEngenharia(opNumero) {
  ...
  if (!base) return { achou: false, erro: "Pasta da OP não encontrada no SharePoint.", pdfs: [], pdfsCliente: [], outrosPdfs: 0, nc1: [], igs: [], listas: [], cliente: [], modelos: 0, ultimo: null };
  ...
  return {
    ...
    // ⚠⚠ O DESENHO DO CLIENTE CONTA COMO DESENHO. Danpower e TMSA mandam o projeto pronto; a
    // Engenharia às vezes deixa em 2.5.5 e nunca copia para 2.5.2. A peça TEM desenho — está no
    // lugar errado, que é um problema de organização, não de emissão. Tratar como "sem desenho"
    // acusaria a OP-067, que tem 1.807 PDFs, de não ter nenhum.
    pdfsCliente: noCli.filter((a) => a.ext === "pdf" && vale(a)).map((a) => ({ ...a, formato: "cliente" })),
...
const TETO_FALTA = 300; // faltantes guardados: o suficiente para agir, não a base inteira
...
export async function conferirPastaDaOp(prisma, opId) {
  ...
  const comPdf = new Set(), soCliente = new Set(), foraPadrao = new Map(), comNc1 = new Set();
  for (const { marca } of marcas) {
    const k = marca.toUpperCase();
    if (inv.pdfs.some((a) => casaMarca(a.nome, marca))) comPdf.add(k);
    else if (inv.pdfsCliente.some((a) => casaMarca(a.nome, marca))) { comPdf.add(k); soCliente.add(k); }
    else {
      const achado = inv.pdfs.find((a) => mencionaMarca(a.nome, marca))
        || inv.pdfsCliente.find((a) => mencionaMarca(a.nome, marca));
  ...
  const temAlgumPdf = inv.pdfs.length > 0 || inv.pdfsCliente.length > 0;
  // ⚠ "só cliente" é estado PRÓPRIO: a peça tem desenho, guardado onde a fabricação não procura.
  // Misturado em "parcial" some; contado como falta, acusa a Engenharia de não ter emitido.
  const veredito =
    !marcas.length && !temAlgumPdf ? "VAZIA"
    : !temAlgumPdf && temMaquina ? "SO_MAQUINA"
    : !temAlgumPdf ? "SEM_DESENHO"
    : !marcas.length ? "SEM_LISTA"
    : semDesenho.length === 0 && soCliente.size > 0 && inv.pdfs.length === 0 ? "SO_CLIENTE"
  ...
  const detalhe = {
    soCliente: soCliente.size,
    pdfsCliente: inv.pdfsCliente.length,
...
export function formatarSalvo(r) {
  ...
  return {
    ...
    arquivos: {
      pdfs: r.pdfs, pdfsCliente: d.pdfsCliente || 0, soCliente: d.soCliente || 0,
