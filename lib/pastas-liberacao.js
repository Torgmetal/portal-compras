...
// ⚠ TODAS as páginas — ver lib/graph-paginas.js (OP-118, 26/09/2026). Exportada para o teste.
export async function listarNaLiberacao(token, path) {
  const { itens, ok } = await todasAsPaginas(`${GRAPH}/drives/${drive()}/root:/${encPath(path)}:/children?$select=id,name,folder,file&$top=999`, token);
  return ok ? itens : [];
}
const listar = listarNaLiberacao;
...
/**
 * O nome do MAÇO no lote de impressão. Igual ao `nomeDePasta`, exceto para CHAPA: aí é a
 * ESPESSURA, sem a largura.
 *
 * Vitor (01/09/2026): "nas chapas não precisa separar por tipo da chapa e medida, pode apenas
 * separar pela espessura delas".
 *
 * ⚠⚠ ERAM 68 MAÇOS PARA A MESMA CHAPA. Medido na base: só de 6,40 existem 68 larguras distintas
 * (`CH6.40X130`, `CH6.40X127`, `CH6.40X135`…), e cada uma virava um PDF e uma pasta. Quem imprime
 * não escolhe largura — corta tudo da mesma chapa —, então a separação por medida só multiplicava
 * arquivo.
 *
 * ⚠ ESPESSURA CONTINUA SEPARANDO. 6,35 e 6,40 são chapas diferentes e juntá-las faria cortar na
 * espessura errada — o erro oposto, e pior. O `nomeDePasta` já cuida das grafias (6,3 / 6.30 /
 * 6,40) antes de a espessura ser lida.
 *
 * ⚠ SÓ CHAPA. Perfil W, L, U, tubo e dobrado seguem com o nome inteiro: neles a medida É o perfil.
 */
export function grupoDoMaco(perfil) {
  const nome = nomeDePasta(perfil);
  // aceita CH, CHAPA, com ou sem espaço; a largura pode vir depois de X ou *
  // ⚠ o qualificador fica no nome: XADREZ é outro produto e não pode cair no mesmo maço da lisa —
  // mas as larguras dele também se juntam ("CHAPA XADREZ 3X710/3X805/3X910" → "CH XADREZ 3").
  const m = nome.match(/^CH(?:APA)?[\s_]*([A-Z]+)?[\s_]*(\d+(?:\.\d+)?)(?:\s*[X*]|$)/);
  if (!m) return nome;
  return m[1] ? `CH ${m[1]} ${m[2]}` : `CH ${m[2]}`;
}
