import { BASE_TOTAL } from "./cotacao-total-pdf";
...
function extractFornecedor(text, formato) {
  ...
  if (formato === "gerdau") {
    // ⚠ O NOME FIXO VEM PRIMEIRO. O bloco "Dados Emissor" da cotação Gerdau traz a TORG nos DOIS
    // lados (emissor e recebedor), então o regex devolvia "TORG METAL LTDA" como fornecedor — o
    // portal exibia a própria Torg cotando para si mesma. Se o documento diz Gerdau, é Gerdau.
...
// Ou sem centro (visto em alguns PDFs) — ⚠ OS NÚMEROS DESTA LINHA DE EXEMPLO NÃO FECHAM
// (472 × 6,48 = 3.058,56, nem líquido nem com IPI dá 2.691,53). Ela documenta o FORMATO sem
// centro, não o contrato do total; foi ela que quase me fez descartar a medição de verdade:
...
//
// ⚠⚠ A ÚLTIMA COLUNA É O TOTAL **COM IPI**, e isso foi MEDIDO, não deduzido (21/09/2026): os dois
// PDFs reais da SOUFER (RM T122-001 e T122-002, 17 linhas somadas) fecham em
// `qtd × preço × (1 + IPI/100)` com erro de 0,000% em TODAS. O da T122-001 tem IPI zero e cai na
// identidade; o da T122-002 tem 3,25% em todas as linhas. Por isso os itens saem daqui carimbados
// com `baseTotal` — ver `lib/cotacao-total-pdf.js`.
const SOUFER_RE =
...
function parseSoufer(text) {
  ...
  while ((m = SOUFER_RE.exec(corpus)) !== null) {
    ...
    itens.push({
      ...
      precoUnit: brNum(m[7]),
      total: brNum(m[10]),
      // ⚠ O total é o IMPRESSO, e vai junto a base em que ele está — quem compara não tem como
      // saber o layout, e adivinhar foi exatamente o defeito.
      baseTotal: BASE_TOTAL.COM_IPI,
      icmsPct: brNum(m[8]),
      ipiPct: brNum(m[9]),
      prazoEntrega: "",
      observacao: "",
...
function parseGerdau(text) {
  ...
  while ((m = GERDAU_RE.exec(corpus)) !== null) {
    ...
    itens.push({
      ...
      precoUnit: brNum(m[4]),
      total: brNum(m[7]),
      // ⚠⚠ AQUI A BASE FICA SEM CARIMBO DE PROPÓSITO (achado do Codex, 21/09/2026). A linha de
      // exemplo acima sugere "com IPI" (544,32 × 5,91 × 1,0325 = 3.321,48, a 0,08% do impresso,
      // contra 3,16% do líquido) — mas sugestão não é medição, e eu não tenho um PDF real da
      // GERDAU para conferir como tive da SOUFER. Sem carimbo, `divergeDoPdf` aceita as DUAS
      // leituras: não acende falso, e continua pegando tudo que erra mais que o IPI.
      // ⚠ E o regex ainda descarta uma TERCEIRA coluna de porcentagem (zero no exemplo); enquanto
      // não se souber o que ela é, carimbar a base seria assumir que só o IPI explica o total.
      icmsPct: brNum(m[5]),
      ipiPct: brNum(m[6]),
      prazoEntrega: "",
      observacao: "",
  ...
  // ⚠⚠ NÃO CORTAR SÓ EM \n. O `unpdf` com `mergePages` devolve o PDF inteiro numa LINHA SÓ, então
  // `[^\n]+` engolia o documento todo — o campo "Condição de pagamento" do formulário do fornecedor
  // era preenchido com mil caracteres de aviso de boleto, CNPJ e telefone do vendedor. O prazo real
  // é curto ("I059-34% 30d / 33% 45d / 33% 60d"); aqui ele para no que vier depois: a próxima linha
  // de item (número + descrição em maiúsculas), o TOTAL, ou o bloco de observações.
  const prazo = text.match(
    /Prazo\s+de\s+Pagamento\s*:\s*(.{0,80}?)(?=\s+\d{2,}\s+[A-ZÁ-Ú]|\s*TOTAL\b|\s*Descarga|\s*Prazo\s+de\s+Pagamento|\n|$)/i
  );
