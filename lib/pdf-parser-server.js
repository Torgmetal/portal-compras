...
//             precoUnit, total, icmsPct, ipiPct, disponibilidade, prazoEntrega,
//             observacao, _centro?}.
...
// → item, codigo, qtd, un, desc, centro, precoUN, ICMS%, IPI%, total
const SOUFER_RE =
  /(\d{1,3})\s+([A-Z0-9][\w-]*)\s+([\d.,]+)\s+(KG|UN|MT|M|PC|PÇ)\s+(\S+)\s+(\d{4})\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)\s+([\d.,]+)/gi;
