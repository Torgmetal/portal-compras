...
function parseSoufer(text) {
  // Ancora depois do header "Item Material Quantidade" e corta na totalização final.
  // O pdfjs junta tudo com espaço, sem newlines, então header e rodapé estariam no
  // mesmo "parágrafo" que os itens e provocariam falsos-positivos.
  ...
  while ((m = SOUFER_ITEM_RE.exec(corpus)) !== null) {
    itens.push({
      item: m[5], // descrição "bunched" do Soufer serve de chave (ex CHPGR12,50X1500...)
      descricao: m[5],
      codigo: m[2],
      qtd: brNum(m[3]),
  ...
  // Condição de pagamento (ex "28DDL") aparece única no cabeçalho original
...
function parseGerdau(text) {
  // Ancora depois do header "Item Descrição Qtd", remove as linhas "Prazo de Pagamento:
  // Z028-28 dias, data Nota Fiscal" que ficam entre itens (viram ruído quando o pdfjs
  // concatena tudo com espaços), e corta no rodapé "TOTAL KG".
  ...
  while ((m = GERDAU_ITEM_RE.exec(corpus)) !== null) {
    const desc = m[2].trim();
    itens.push({
      item: desc,
      descricao: desc,
