...
// LINHA-LIXO do import: a Lista de Expedição termina com uma linha "TOTAL.:" que o parser trouxe
// como se fosse peça — e ela carrega o peso somado da lista inteira. Na OP-060 são 153.647 kg de
// peça fantasma; na 089, 11.376 kg, que apareciam como "um conjunto gigante sem descrição".
// O painel de Liberar já descartava; o motor da TV e das telas de produção não. (Vitor 19/08:
// "deve ter alguma coisa errada pois não temos um único conjunto com esse peso total".)
export function ehLinhaLixo(p) {
  const m = String(p?.marca ?? "").trim();
  return !m || /^(total|soma|subtotal)\b/i.test(m);
}
