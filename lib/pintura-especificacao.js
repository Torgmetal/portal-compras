...
// ⚠ COR SÓ IMPORTA NO ACABAMENTO. Vitor (31/08/2026): "as [cores] de acabamento (…) primer e
// intermediário é melhor considerar sempre cinza".
//
// A especificação do cliente lista cor só na 3ª demão — as de baixo o fabricante entrega no tom de
// fábrica, e na prática é cinza. Deixar em branco não é neutro: a demão sem cor fica de fora da
// conta de acabamento e alguém acaba lançando a cor do cliente nas três, comprando tinta de
// acabamento para cobrir o que ninguém vai ver.
export const COR_BASE = "Cinza";
...
export function lerEspecificacaoPintura(grade) {
  ...
  if (iCab >= 0) {
    for (let r = iCab + 1; r < linhas.length; r++) {
      ...
      const nomeCamada = CAMADAS[Math.min(idx, CAMADAS.length - 1)];
      camadas.push({
        camada: nomeCamada,
        demao: primeira,
        produto: textos[0] || "",
        // a cor do acabamento costuma dizer "conforme cor de acabamento" — não é cor, é remissão
        cor: nomeCamada === "ACABAMENTO"
          ? (textos.slice(1).find((t) => !/conforme|ver se[çc][ãa]o/i.test(t)) || "")
          : COR_BASE,
