...
export async function baixarCadernoPintura(opNumero, ids) {
  ...
  const w2 = workbook.addWorksheet("2. Qual tinta usar");
  w2.columns = [{ width: 11 }, { width: 38 }, { width: 12 }, { width: 11 }, { width: 16 },
    { width: 7 }, { width: 12 }, { width: 12 }, { width: 11 }];
  adicionarHeaderTabela(w2, 1, ["R do CMR", "Produto recebido (nota)", "Lote", "NF", "Fornecedor",
  ...
  const w3 = workbook.addWorksheet("3. Para o pintor");
  w3.columns = [{ width: 26 }, { width: 42 }, { width: 42 }];
  const put = (a, b, c, opt = {}) => {
    const r = w3.addRow([a, b, c]);
