...
export async function baixarCadernoPintura(opNumero, ids) {
  ...
  /* ⚠⚠ TINTA COMPRADA PARA OUTRA OBRA APARECE AQUI, EM CIMA. Vitor (07/09/2026): "tem tinta da
     Induscolor na rastreabilidade da OP 105 e não temos tinta Induscolor para essa obra" — eram
     três R do pedido 1871, da OP-064, lançados na 105 por engano de digitação no CMR. Ele pegou
     lendo; a folha tem de dizer. Some sozinho quando a planilha do Almoxarifado for corrigida. */
  const alertaObra = (d.obraDivergente || []).map((x) =>
    `⚠ R ${x.R} (${x.produto}) foi comprado para a OP-${x.obraPedido}${x.obraNome ? ` — ${x.obraNome}` : ""}, pelo pedido ${x.pedido}. Confira a obra no CMR antes de usar.`);
