...
export async function baixarCadernoPintura(opNumero, ids) {
  ...
  adicionarHeaderTabela(w2, hCab, ["Demao", "Produto do PLP", "Cor", "Esp. seca (um)", "Solidos vol. (%)",
    "Diluicao (%)", "Area (m2)", "Rendimento (m2/L)", "Tinta (L)", "Galoes (3,6 L)", "Diluente (L)", "Umida (um)"]);
  l2++;
  for (const c of d.camadas) {
    ...
    formula(w2, `K${r}`, `IF(OR(I${r}="",F${r}=""),"",I${r}*F${r}/100)`, "#,##0.00");
    // ⚠ úmida = seca × (100 + %diluição) ÷ SV — a mesma fórmula da planilha da Qualidade
    formula(w2, `L${r}`, `IF(OR(D${r}="",E${r}="",F${r}=""),"",ROUND(D${r}*(100+F${r})/E${r},0))`, "#,##0");
    l2++;
  }
  ...
  d.demaos.forEach((m, i) => {
    ...
    w3.getCell(`B${lU}`).value = { formula: `IF(${ref("L")}="","⚠ preencher na folha 2",${ref("L")}&" µm")` };
