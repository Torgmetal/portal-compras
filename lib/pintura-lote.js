...
import { obrasDivergentesDoPedido } from "./cmr-obra-divergente";
...
import { validarFracoesExportacao } from "./exportacao-fracoes";
...
async function pecasDaFila(opNumero, ids) {
  return prisma.pecaConjunto.findMany({
    where: ids?.length ? { id: { in: ids }, op: { numero: opNumero } } : { ...SO_FABRICACAO, op: { numero: opNumero } },
    select: {
 ...
 */
export async function cadernoDePintura(opNumero, ids, fracoes) {
  const [op, plano, originais] = await Promise.all([
  ...
  const selecao = validarFracoesExportacao(fracoes, originais);
  const pecas = selecao ? originais.filter(p => selecao.has(p.id)).map(p => {
    const qte = selecao.get(p.id).reduce((s, f) => s + f.quantidade, 0);
    const proporcao = qte / Math.max(1, p.qte || 1);
    return { ...p, qte, pesoTotalKg: (p.pesoTotalKg || 0) * proporcao,
      areaPinturaM2: p.areaPinturaM2 == null ? null : p.areaPinturaM2 * proporcao };
  }) : originais;
  ...
  const cmr = (await prisma.documentoQualidade.findMany({
    ...
    select: { importRef: true, nome: true, numeroCorrida: true, nfNumero: true, fornecedor: true,
              pedidoCompra: true, quantidade: true, dataRecebimento: true, dataValidade: true, arquivoUrl: true },
  }))
    ...
    .map((c) => ({
      R: c.importRef, produto: c.nome, lote: c.numeroCorrida, nf: c.nfNumero, pedidoCompra: c.pedidoCompra,
  ...
  /* ⚠⚠ TINTA DE OUTRA OBRA NÃO ENTRA CALADA. Vitor (07/09/2026): "tem tinta da Induscolor na
     rastreabilidade da OP 105 e não temos tinta Induscolor para essa obra" — e não era mesmo: os
     três R eram do pedido 1871, da OP-064 PIPE RACK, lançados na 105 por engano de digitação na
     planilha do Almoxarifado. Esta lista só mostra o que o CMR diz ser da obra; sem PLP ela não
     tem com o que comparar a especificação, então o único juiz disponível é o PEDIDO — e o portal
     já o conhece. Avisa; corrigir é de quem lançou. */
  const obraDivergente = await obrasDivergentesDoPedido(prisma, cmr.map((c) => ({
    importRef: c.R, nome: c.produto, opNumero: op.numero, pedidoCompra: c.pedidoCompra, nfNumero: c.nf,
  })));

  return {
    op: { numero: op.numero, obra: op.obra, cliente: op.cliente },
    temPlp: !!plano,
    // ⚠ R cuja NOTA foi comprada para OUTRA obra — some quando a planilha for corrigida
    obraDivergente,
    ...
    quantidade: {
      pecas: selecao ? pecas.reduce((s, p) => s + p.qte, 0) : pecas.length, kg: Math.round(kg), m2: Math.round(m2 * 100) / 100, semArea,
      ...
      /* ⚠ AS DUAS EMBALAGENS. Vitor (07/09/2026): "precisa colocar a opção de galão de 3,6 L e 18 L,
         isso é importante para trazer as quantidades". O fornecedor vende num ou noutro, e o CMR já
         recebeu as duas — quem compra escolhe. As duas saem da mesma conta em litros. */
      baldes: r?.total.litros != null ? Math.ceil(r.total.litros / 18) : null,
    ...
    ).map(c => selecao ? { ...c, pecas: pecas.reduce((s, p) => {
      const cor = corDaPeca(p.descricao, Array.isArray(plano?.itens) ? plano.itens : []);
      return s + ((cor.cor || `⚠ definir (${cor.origem})`) === c.rotulo ? p.qte : 0);
    }, 0) } : c),
  };
}
