 ...
 *   usuario — quem emite
 */
export async function gerarPitExcel({ op = {}, padrao = "PINTURA", revisao = "0", usuario = null, responsaveis = null, cliente = null }) {
  ...
  ws.getRow(l).height = 16;
  l++;
  // ⚠ O NOME VEM DO CADASTRO DA OBRA, não de quem clicou. Vitor (26/08/2026): "deixar o campo de
  // elaborado e verificado para poder ser preenchido". Antes saía o usuário logado em "ELABORADO
  // POR" — o mesmo plano mudava de autor a cada emissão.
  const quem = (r, alt) => so(r?.nome) || so(alt) || "";
  const dt = (r) => (r?.assinadoEm ? `Assinado em ${fmtD(r.assinadoEm)}` : null);
  const blocos = [
    { papel: "ELABORADO POR", nome: quem(responsaveis?.elaborado, usuario), data: dt(responsaveis?.elaborado), ci: 1, cf: 3 },
    { papel: "VERIFICADO POR", nome: quem(responsaveis?.verificado), data: dt(responsaveis?.verificado), ci: 4, cf: 5 },
    { papel: "INSPETOR DO CLIENTE / QUALIDADE", nome: so(cliente) || "", data: null, ci: 6, cf: 8 },
  ];
  for (const b of blocos) {
    ...
    // ⚠ quem já assinou eletronicamente leva a data impressa; o campo em branco continua para quem
    // assina à caneta no papel.
    p4.value = b.data || "Data: ____ / ____ / ________";
