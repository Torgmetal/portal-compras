...
export async function gerarPlanoAcaoPDF(p) {
  ...
  const chrome = () => {
    ...
    if (p.indicador && p.valor != null && p.metaValor != null) {
      const bateu = Number(p.valor) >= Number(p.metaValor);
      const num = (v) => (Number.isInteger(Number(v)) ? String(v) : Number(v).toFixed(1).replace(".", ","));
      const linha = `${p.processo ? `${p.processo} · ` : ""}${p.mes ? `${String(p.mes).padStart(2, "0")}/${p.ano}` : p.ano || ""}`;
  ...
  for (const it of itens) {
    ...
    const cells = COLS.map((c) => {
      const val = c.sit ? situacaoItemLabel(sit) : c.data ? fmtD(it[c.key]) : (it[c.key] || "");
