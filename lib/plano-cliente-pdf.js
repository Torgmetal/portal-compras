...
export async function gerarPlanoClientePDF({ snapshot = {}, assinaturas = null, minuta = false }) {
  ...
  const tabela = (cols, linhas, { tam = 7.5, agrupar = false } = {}) => {
    ...
    const cabec = () => {
      ...
      let cx = M + 4;
      for (const c of cols) {
        const dx = c.al === "c" || c.al === "n" ? Math.max(0, (c.w - 7 - bold.widthOfTextAtSize(san(c.t), 6.8)) / 2) : 0;
        page.drawText(san(c.t), { x: cx + dx, y: y - 11, size: 6.8, font: bold, color: WHITE });
        cx += c.w;
      }
      y -= 16;
    };
  ...
  if (doc === "PIT") {
    ...
    // ⚠ ITEM e os PERCENTUAIS centrados; critério em "n" (centra só valor curto, como "1" e "0.1",
    // e deixa "VISUAL E ESPESSURA 100% · ADERÊNCIA NBR 11003" à esquerda). Vitor (27/08/2026): "o %
    // por exemplo como centralizado".
    const centro = snapshot.snqc ? { 0: "c", 4: "c" } : { 0: "c", 3: "c", 4: "c", 5: "n" };
    const cols = nomes.map((t, i) => ({ t, w: (pesos[i] / soma) * W, al: centro[i] }));
    secao(`Escopo de inspeção — ${snapshot.nomePadrao || ""}`);
    tabela(cols, snapshot.linhas || [], { agrupar: true });
  ...
  const blocos = [
    ...
    { papel: "INSPETOR DO CLIENTE / QUALIDADE", nome: doCliente?.nome || r.cliente?.nome, em: doCliente?.assinadoEm || r.cliente?.assinadoEm, ip: doCliente?.ip },
  ];
  const larg = W / 3;
  blocos.forEach((b, i) => {
    const x = M + i * larg;
