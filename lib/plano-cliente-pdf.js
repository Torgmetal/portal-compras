...
export async function gerarPlanoClientePDF({ snapshot = {}, assinaturas = null }) {
  ...
  if (doc === "PIT") {
    ...
    secao("Preparação de superfície");
    const prep = [
      ["Método", nz(snapshot.preparoMetodo)],
      ["Grau de limpeza", nz(snapshot.grauLimpeza)],
      ...
      ["Método de aplicação", nz(snapshot.metodoAplicacao)],
    ];
    prep.forEach(([r, v], i) => {
      if (i % 2 === 0) espaco(24);
      const x = M + (i % 2) * (W / 2);
    ...
    if ((snapshot.itens || []).length) {
      secao("Itens da estrutura e cores");
      tabela([
        { t: "ITEM", w: W * 0.42 }, { t: "SISTEMA", w: W * 0.14 }, { t: "COR", w: W * 0.2 }, { t: "OBSERVAÇÃO", w: W * 0.24 },
      ], snapshot.itens.map((i) => [nz(i.item), nz(i.sistema), nz(i.cor), nz(i.obs)]), { tam: 8 });
    }
  ...
  // ── o aceite ──
  //
  // ⚠ O QUADRO SAI IMPRESSO MESMO SEM NINGUÉM TER ACEITO: é ele que diz, na folha, que este
  // documento depende do aceite do cliente. Folha sem o quadro passa por documento já valendo.
  espaco(70);
  secao("Aceite do cliente");
  if (assinaturas && assinaturas.length) {
    const ac = [{ t: "Nome", w: W * 0.28 }, { t: "Empresa / função", w: W * 0.24 }, { t: "Aceite / data", w: W * 0.28 }, { t: "IP", w: W * 0.2 }];
    espaco(16);
    let cx = M + 4;
    for (const c of ac) { page.drawText(c.t, { x: cx, y, size: 7, font: bold, color: GRAY }); cx += c.w; }
    y -= 13;
    for (const a of assinaturas) {
      ...
      if (a.assinadoEm) page.drawText(san(`Aceito em ${fmtDT(a.assinadoEm)}`), { x: cx, y, size: 7.5, font: bold, color: GREEN });
      else page.drawText("Aguardando aceite", { x: cx, y, size: 7.5, font, color: ORANGE });
      cx += ac[2].w;
    ...
    page.drawText("Documento sujeito ao aceite do cliente. O aceite é registrado eletronicamente (confirmação + data/hora + IP).", { x: M, y, size: 8.5, font, color: GRAY });
    y -= 14;
  }
