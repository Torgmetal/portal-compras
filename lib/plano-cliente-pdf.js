...
export async function gerarPlanoClientePDF({ snapshot = {}, assinaturas = null, minuta = false }) {
  ...
  if (doc === "PIT") {
    ...
    const comEspec = (snapshot.demaos || []).filter((d) => d.lote || d.diluicao || d.camadaUmida || d.secagem);
    if (comEspec.length) {
      secao("Especificações das tintas");
      tabela([
        { t: "ESPECIFICAÇÃO", w: W * 0.16 }, { t: "PRODUTO", w: W * 0.24 }, { t: "FABRICANTE", w: W * 0.14 },
        { t: "LOTE / R", w: W * 0.12 }, { t: "DILUIÇÃO", w: W * 0.13 }, { t: "CAMADA ÚMIDA", w: W * 0.09 },
        { t: "TEMPO DE SECAGEM", w: W * 0.12 },
      ], comEspec.map((d) => [nz(d.nome), nz(d.produto), nz(d.fabricante), nz(d.lote), nz(d.diluicao), nz(d.camadaUmida), nz(d.secagem)]), { tam: 8 });
    }
