...
export async function responsaveisDoPlano(prisma, doc, opNumero, { revisao = null } = {}) {
  const r = await prisma.planoResponsavel.findUnique({ where: { opNumero_doc: { opNumero, doc } } }).catch(() => null);
  const base = {
  ...
  for (const a of envio?.assinaturas || []) {
    const alvo = /verific/i.test(a.setor || "") ? "verificado" : "elaborado";
    if (a.assinadoEm) base[alvo].assinadoEm = a.assinadoEm;
    if (!base[alvo].nome) base[alvo].nome = a.nome;
  }
  return base;
}
...
export async function statusDosPlanos(prisma, opNumero) {
  ...
  const daEtapa = (doc, etapa) => {
    const meus = envios.filter((e) => e.tipo === tipoDoEnvio(doc, etapa));
    ...
    return {
      ...
      assinantes: ass.map((a) => ({ nome: a.nome, email: a.email, papel: a.setor, assinadoEm: a.assinadoEm })),
...
export async function excelDoPlano(prisma, doc, opNumero, { snapshot = null, usuario = null } = {}) {
  ...
  // ⚠ os responsáveis saem do SNAPSHOT quando existe envio: o documento que o cliente recebeu não
  // pode trocar de elaborador porque alguém editou o cadastro depois.
  const responsaveis = snapshot?.responsaveis || await responsaveisDoPlano(prisma, doc, opNumero);
  const inspetor = snapshot?.aceitoPorCliente || null;
  ...
  if (doc === "PIT") {
    ...
    return {
      ...
      bytes: await gerarPitExcel({ op, padrao, revisao: snapshot?.revisao || op.pitRevisao || "0", usuario, responsaveis, cliente: inspetor }),
    };
  }
  ...
  if (!plp) return null;
  return { nome: `PLP-T${opNumero}.xlsx`, bytes: await gerarPlpExcel({ plp, op, usuario, responsaveis, cliente: inspetor }) };
}
...
//
// ⚠ O ARQUIVO É O EXCEL **E** O PDF. O Excel é o documento controlado (campos de assinatura, é o que
// a Qualidade edita se precisar revisar); o PDF é o que se lê e o que o Data Book monta. Guardar só
// um dos dois obriga alguém a converter à mão depois — e é aí que a versão errada entra.
...
export async function arquivarPlano(prisma, doc, opNumero, { snapshot = null, usuario = null } = {}) {
  ...
  const arq = await excelDoPlano(prisma, doc, opNumero, { snapshot: snap, usuario });
  const pdf = await gerarPlanoClientePDF({ snapshot: snap, assinaturas });

  const subidos = [];
  if (arq) {
    subidos.push(await uploadFileToFolder({
      folderPath: pasta, fileName: `${nomeBase}.xlsx`, buffer: Buffer.from(arq.bytes),
  ...
  return { ok: true, pasta, arquivos: subidos.map((x) => x?.name).filter(Boolean), documentoId: registro.id, url: doPdf.webUrl || null };
}
