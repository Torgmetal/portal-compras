...
export async function responsaveisDoPlano(prisma, doc, opNumero, { revisao = null } = {}) {
  ...
  // ⚠ O CONTATO DO CLIENTE também segue a obra, não o documento: quem aceita o PIT aceita o PLP.
  // Cadastrado num, vale no outro — mesma economia de digitação dos outros dois papéis.
  let cli = { nome: r?.clienteNome || null, email: r?.clienteEmail || null };
  if (!cli.nome && !cli.email) {
    const outro = await prisma.planoResponsavel.findFirst({
      where: { opNumero, doc: { not: doc }, OR: [{ clienteNome: { not: null } }, { clienteEmail: { not: null } }] },
      orderBy: { atualizadoEm: "desc" }, select: { clienteNome: true, clienteEmail: true },
    }).catch(() => null);
    if (outro) cli = { nome: outro.clienteNome || null, email: outro.clienteEmail || null };
  }
  const base = {
  ...
  // quem aceitou pelo cliente (etapa CLIENTE) — para o quadro de aprovações mostrar a data
  const envioCli = await prisma.envioAssinatura.findFirst({
    where: { tipo: tipoDoEnvio(doc, "CLIENTE"), opNumero, ...(revisao === null ? {} : { revisao }) },
    orderBy: { enviadoEm: "desc" },
    select: { assinaturas: { select: { email: true, nome: true, assinadoEm: true } } },
  }).catch(() => null);
  const aceitou = (envioCli?.assinaturas || []).find((a) => a.assinadoEm) || (envioCli?.assinaturas || [])[0] || null;
  if (aceitou) {
    base.cliente.assinadoEm = aceitou.assinadoEm || null;
    if (!base.cliente.nome) base.cliente.nome = aceitou.nome || null;
    if (!base.cliente.email) base.cliente.email = aceitou.email || null;
  }
  return base;
}
...
export async function comResponsaveis(prisma, doc, opNumero, snapshot) {
  ...
  if (!op) return snap;
  const atual = await responsaveisDoPlano(prisma, doc, op).catch(() => null);
  if (!atual) return snap;
  const s = snap.responsaveis || {};

  // ⚠⚠ NOME DO ENVIO, DATA DE VERDADE. O snapshot congela QUEM assina (trocar o verificador depois
  // não reescreve o documento que já saiu), mas ele foi gravado ANTES de qualquer assinatura — com
  // `assinadoEm: null`. Devolver o snapshot inteiro deixava o quadro de aprovações eternamente em
  // "Data: __/__/__", mesmo com todo mundo já tendo assinado. A data vem sempre do registro vivo.
  //
  // ⚠ E o papel que o snapshot NÃO TEM entra do cadastro de hoje — é o caso do contato do cliente,
  // que passou a existir em 28/08/2026: envio antigo não guardou, e um campo que ainda não existia
  // não é uma verdade a preservar.
  const juntar = (a, b) => ({
    nome: a?.nome || b?.nome || null,
    email: a?.email || b?.email || null,
    assinadoEm: b?.assinadoEm || a?.assinadoEm || null,
  });
  const outros = (Array.isArray(s.outros) && s.outros.length ? s.outros : atual.outros || []).map((o) => {
    const vivo = (atual.outros || []).find((x) => String(x.email).toLowerCase() === String(o.email).toLowerCase());
    return { ...o, assinadoEm: vivo?.assinadoEm || o.assinadoEm || null };
  });
  return {
    ...snap,
    responsaveis: {
      elaborado: juntar(s.elaborado, atual.elaborado),
      verificado: juntar(s.verificado, atual.verificado),
      cliente: juntar(s.cliente, atual.cliente),
      outros,
    },
  };
}
...
export async function arquivarPlano(prisma, doc, opNumero, { snapshot = null, _usuario = null } = {}) {
  ...
  // ⚠⚠ PODE NÃO HAVER ENVIO AO CLIENTE. Desde que o aceite dele virou opcional, o arquivamento
  // dispara na última assinatura da verificação INTERNA — e a busca por `tipo: doc` voltava vazia,
  // o que gerava um PDF sem conteúdo nenhum (snapshot {}). O documento é o da verificação interna;
  // o envio ao cliente só entra quando existe E é da mesma revisão (aceite da R00 não arquiva a R01).
  const envios = await prisma.envioAssinatura.findMany({
    where: { tipo: { in: [doc, tipoDoEnvio(doc, "INTERNA")] }, opNumero },
    orderBy: { enviadoEm: "desc" },
    select: { id: true, tipo: true, snapshot: true, revisao: true },
  });
  const interno = envios.find((e) => e.tipo !== doc) || null;
  const aoCliente = envios.find((e) => e.tipo === doc && (!interno || e.revisao === interno.revisao)) || null;
  const envio = aoCliente || interno;
  // ⚠ as datas de elaborado/verificado vêm do registro VIVO — o snapshot foi gravado antes de
  // qualquer assinatura, e sem isto o documento arquivado sairia com o quadro em "Data: __/__/__"
  // justamente agora, que a verificação interna é a única aprovação que ele carrega.
  const snap = await comResponsaveis(prisma, doc, opNumero, snapshot || envio?.snapshot || {});
  const assinaturas = aoCliente
    ? await prisma.assinaturaDocumento.findMany({
        where: { envioId: aoCliente.id },
  ...
  const doPdf = await uploadFileToFolder({
    ...
    // substitui: a MESMA revisão é arquivada duas vezes — na verificação interna e, se vier, no
    // aceite do cliente. Com "rename" a pasta ficaria com duas cópias do R00.
    conflict: "replace",
  });
...
export async function tudoAprovado(prisma, doc, opNumero) {
  ...
  // ⚠⚠ O ACEITE DO CLIENTE NÃO É MAIS OBRIGATÓRIO. Vitor (28/08/2026): "para o PLP e PIT tire a
  // opção de ter que a assinatura do cliente obrigatoriamente". Muito cliente não devolve o plano
  // assinado, e o documento ficava fora da pasta da Qualidade e fora do Data Book por causa de uma
  // assinatura que nunca vem — o dossiê acabava sem o plano justamente na obra que já fechou. Quem
  // responde pelo documento é a casa: elaborado + verificado assinados já valem. Se o cliente
  // aceitar depois, isto roda de novo e o PDF é substituído COM o aceite dele.
  return !!st?.[doc]?.interna?.aceito;
}
