...
export async function gerarPlpExcel({ plp = {}, op = {}, tintas = [], revisoes = [], usuario = null }) {
  ...
  // ══ FOLHA 1 — capa ══════════════════════════════════════════════════════════
  {
    ...
    l = rotuloValor(ws, l, [["EMPREENDIMENTO", so(op.obra) || "", "A", "J"]]);
    l = rotuloValor(ws, l, [
      ["CLIENTE", so(op.cliente) || "", "A", "D"],
      ["UNIDADE", so(op.unidade) || "", "E", "G"],
      ["LOCAL", so(op.local) || "", "H", "J"],
    ]);
    l = rotuloValor(ws, l, [
    ...
    l = tabela(ws, l, [
      ...
      [1, "Sistemas de Pintura do Empreendimento"],
  ...
  // ══ FOLHA 2 — sistemas e especificações ═════════════════════════════════════
  {
    ...
    l = titulo(ws, l, "1 — SISTEMAS DE PINTURA DO EMPREENDIMENTO");
