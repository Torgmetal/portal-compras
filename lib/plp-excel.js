 ...
 */
export async function gerarPlpExcel({ plp = {}, op = {}, tintas = [], revisoes = [], usuario = null, responsaveis = null, cliente = null }) {
  ...
  // ══ FOLHA 1 — capa ══════════════════════════════════════════════════════════
  {
    ...
    // ⚠ ELABORADO / VERIFICADO, os mesmos papéis do PIT. Vitor (26/08/2026): "deixar o campo de
    // elaborado e verificado para poder ser preenchido (…) isso vale para o PLP também". Antes eram
    // "REALIZADO POR" (com o usuário logado) e "APROVADO POR" — dois nomes para o mesmo fluxo, e o
    // primeiro trocava de pessoa a cada emissão.
    const quem = (r, alt) => so(r?.nome) || so(alt) || "";
    const dtAss = (r) => (r?.assinadoEm ? fmtD(r.assinadoEm) : null);
    l = assinaturas(ws, l, [
      { papel: "ELABORADO POR", nome: quem(responsaveis?.elaborado, usuario), data: dtAss(responsaveis?.elaborado) || hoje },
      { papel: "VERIFICADO POR", nome: quem(responsaveis?.verificado), data: dtAss(responsaveis?.verificado) },
      // ⚠ Vitor (26/08/2026): "tanto o PIT quanto o PLP precisam ser assinados pelo inspetor do
      // cliente ou algum responsável da qualidade". Sem esse campo no papel, o plano é uma
      // declaração nossa; com ele, é um acordo.
      { papel: "INSPETOR DO CLIENTE / QUALIDADE", nome: so(cliente) || "", data: null },
    ]);
