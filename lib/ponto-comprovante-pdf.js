...
// COMPROVANTE DE CONFERÊNCIA DE CARTÃO DE PONTO (pdf-lib) — prova de que cada
// colaborador visualizou e confirmou (deu ciência) o próprio cartão do mês, com
// data, hora e IP. Uma linha por funcionário da competência. A4 paisagem.
// Espelha o comprovante de holerites.
...
const STATUS = { CONFIRMADO: "Confirmado", VISUALIZADO: "Visualizado", PENDENTE: "Pendente" };

export async function gerarComprovantePontoPDF(competencia, itens, now = new Date()) {
  ...
  const cabecalho = () => {
    ...
    const tit = "COMPROVANTE DE CONFERÊNCIA DE CARTÃO DE PONTO";
    page.drawText(san(tit), { x: PW - M - wid(tit, bold, 12), y: PH - 24, size: 12, font: bold, color: WHITE });
  ...
  const dataRow = (h) => {
    ...
    const ty = y - th + 5.5;
    cell(h.nome || h.funcionario?.nome || "—", 0, ty, font, DARK);
  ...
  y -= 14;
  page.drawText(fit("Este documento comprova a conferência (ciência) dos cartões de ponto pelos funcionários, registrada eletronicamente no portal com data, hora e endereço IP no ato da confirmação.", font, 8.5, W), { x: M, y, size: 8.5, font, color: GRAY });
  y -= 18;

  headerRow();
  if (!lista.length) { page.drawText("Nenhum cartão nesta competência.", { x: M + 5, y: y - th + 5.5, size: 8.5, font, color: GRAY }); y -= th; }
  ...
  return { bytes, filename: `comprovante-ponto-${competencia}.pdf` };
}
