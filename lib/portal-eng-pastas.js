 ...
 */
// ⚠⚠ O QUE PODE IR AO CLIENTE, EM QUALQUER TIPO. Vitor (03/09/2026): "só leve para o portal
// arquivos pdf, dwg e excel; word ou txt você deve ignorar". Desenho, lista e modelo saem daqui
// carimbados; Word e txt na pasta de projeto são rascunho, ata e anotação — coisa nossa, que nunca
// foi feita para sair, e que aparecia no seletor pedindo para ser marcada por engano.
const EXTENSOES_PUBLICAVEIS = ["pdf", "dwg", "dxf", "xlsx", "xls", "xlsm", "ifc", "step", "stp"];
const publicavel = (nome) => EXTENSOES_PUBLICAVEIS.includes((String(nome).split(".").pop() || "").toLowerCase());
...
export async function conteudoDoTipo(tipoId, raizes, caminho, listar) {
  ...
  const exts = (TIPO_ENG[tipoId]?.extensoes || null)?.map((e) => e.toLowerCase()) || null;
  ...
  for (const alvo of alvos) {
    ...
    for (const x of filhos) {
      ...
      // ⚠ o tipo pode restringir a extensão (memorial: só PDF) — ver TIPOS_ENGENHARIA
      if (exts && !exts.includes((String(x.name).split(".").pop() || "").toLowerCase())) continue;
      if (!publicavel(x.name)) continue;
