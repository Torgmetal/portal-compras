...
export const LISTAS = {
  ...
  // ⚠⚠ FILTRA POR PERTENCIMENTO, NÃO POR PROCEDÊNCIA. Vitor (02/09/2026): "precisamos consertar
  // isso, não pode ser feito gambiarra mais". Estas listas perguntavam `fonte`, que guarda QUEM
  // criou a linha — e a mesma marca pode estar nas duas listas numa linha só. Quem importasse por
  // último levava a marca embora da outra lista; foi assim que o portal da OP-113 anunciou ao
  // cliente 76 marcas "excluídas" que nunca saíram de lugar nenhum.
  LPC: { campo: "naLPC", so: null, comCroquis: true, nome: "Lista de Producao (LPC)", secao: "LPC" },
  LE: { campo: "naLE", so: null, nome: "Lista de Expedicao (LE)", secao: "LE" },
};
...
export function revisaoParaOCliente(rev, comPeso, { clienteViuEm = null } = {}) {
  if (!rev) return null;
  ...
  return {
    ...
    //
    // ⚠⚠ E PRECISA DE ALGUÉM QUE TENHA VISTO A ANTERIOR. Vitor (26 e 27/08/2026): "o aviso da
    // revisão permanece, mesmo que o cliente nunca nem tenha acessado" e, depois de mandarmos o
    // link para ele mesmo, "acessei o link e novamente o aviso de revisão está aparecendo".
    //
    // A primeira tentativa foi exigir o ENVIO antes da revisão. Não bastou, e o log mostra por quê:
    // o link foi enviado 23:39:44, ele abriu 23:43:22 — e é a PRÓPRIA ABERTURA que tira a foto da
    // lista (`sincronizarRevisao` roda na visita). A revisão nasceu 23:43:22 e o acesso ficou
    // registrado 23:43:26, quatro segundos depois: "publicada depois do envio" era verdade, e
    // mesmo assim ninguém tinha a lista anterior — as fotos de antes eram só dos nossos previews.
    //
    // A pergunta certa é outra: O CLIENTE JÁ TINHA ABERTO O PORTAL ANTES DESTA REVISÃO EXISTIR? Só
    // aí existe "a lista que ele tem". Vale o primeiro acesso de um DESTINATÁRIO (quem entrou pelo
    // `?d=` do e-mail que mandamos) — o nosso preview nunca grava isso.
    mudou: rev.seq > 1
      && (rev.nIncluidas > 0 || rev.nExcluidas > 0 || rev.nAlteradas > 0)
      && !!clienteViuEm && new Date(clienteViuEm) < new Date(rev.publicadaEm),
