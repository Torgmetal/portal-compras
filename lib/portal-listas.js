...
export const LISTAS = {
  // `fonte` é o que separa as duas na PecaConjunto; `so` filtra o tipo de peça que interessa ao
  // cliente na LPC (conjunto, não croqui — croqui é peça de fábrica, não se expede).
  LPC: { fonte: "LPC_IMPORT", so: "CONJUNTO", nome: "Lista de Producao (LPC)", secao: "LPC" },
...
export function revisaoParaOCliente(rev, comPeso) {
  ...
  return {
    seq: rev.seq,
    rotulo: rev.rotulo || `Revisão ${rev.seq}`,
