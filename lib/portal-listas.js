...
export function revisaoParaOCliente(rev, comPeso, { enviadoAoClienteEm = null } = {}) {
  if (!rev) return null;
  ...
  return {
    ...
    //
    // ⚠⚠ E PRECISA DE ALGUÉM QUE TENHA VISTO A ANTERIOR. Vitor (26/08/2026): "o aviso da revisão
    // permanece, mesmo que o cliente nunca nem tenha acessado".
    //
    // O aviso diz "mudou desde a que você tem" — e o que o cliente tem é a lista de quando MANDAMOS
    // o link. Antes do envio não existe "a que ele tem": é só conteúdo se formando. Na OP-112 o
    // portal anunciava "150 marcas incluídas" a um cliente que nunca recebeu o link — as 150 eram
    // as subpeças que a LPC passou a listar naquela tarde, mudança nossa, de tela.
    //
    // ⚠ NÃO SERVE OLHAR ACESSO: `primeiroAcessoEm` do portal é gravado pelo NOSSO "Ver como o
    // cliente vê" (nos quatro portais publicados ele está a segundos do `publicadoEm`). Quem
    // responde "ele podia ter esta lista?" é o ENVIO, não o acesso.
    mudou: rev.seq > 1
      && (rev.nIncluidas > 0 || rev.nExcluidas > 0 || rev.nAlteradas > 0)
      && !!enviadoAoClienteEm && new Date(enviadoAoClienteEm) < new Date(rev.publicadaEm),
