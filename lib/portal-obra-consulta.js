...
export async function pecaParaCliente({ opId, opNumero, marca }) {
  ...
  // ── rastreabilidade ──
  // ⚠⚠ O Nº DA RASTREABILIDADE (R), CORRIDA, CERTIFICADO E NORMA. Vitor (04/09/2026): "não está
  ...
  // conferir. Fornecedor, nota fiscal, pedido de compra e peso comprado continuam FORA: são dados da
    ...
    if (minhas[0].tipoPeca === "CONJUNTO") {
      ...
      for (const l of linhas) {
        for (const u of l.usadas || []) {
          ...
          rastreio.push({ r: u.rastreio || null, material: l.perfil || null, corrida: u.corrida || null, certificado: u.certificado || null, norma: u.norma || null });
        }
      }
    } else {
      ...
      for (const p of minhas) {
        ...
        for (const u of r?.usadas || []) {
          if (!u?.corrida && !u?.certificado) continue;
          rastreio.push({ r: u.rastreio || null, material: p.perfil || null, corrida: u.corrida || null, certificado: u.certificado || null, norma: u.norma || null });
        }
      }
    }
