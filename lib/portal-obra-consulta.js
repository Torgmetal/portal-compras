...
async function pecasDaObra(opId) {
  return prisma.pecaConjunto.findMany({
    ...
    select: { id: true, marca: true, perfil: true, qte: true, pesoTotalKg: true, tipoPeca: true },
  });
}
 ...
 */
export async function pecaParaCliente({ opId, opNumero, marca }) {
  ...
  // ── rastreabilidade ──
  // ⚠⚠ SÓ CORRIDA, CERTIFICADO E NORMA. É o que um data book mostra e o que o cliente tem como
  // conferir. Fornecedor, nota fiscal, pedido de compra e peso comprado ficam FORA: são dados da
  // nossa cadeia de suprimentos, não da peça dele. O `linhaCmr` traz tudo junto (ver
  // lib/rastreio-peca) — por isso a escolha aqui é campo a campo, nunca espalhamento do objeto.
  //
  ...
  try {
    if (minhas[0].tipoPeca === "CONJUNTO") {
      ...
      for (const l of linhas) {
        for (const u of l.usadas || []) {
          ...
          rastreio.push({ material: l.perfil || null, corrida: u.corrida || null, certificado: u.certificado || null, norma: u.norma || null });
        }
      }
    } else {
      ...
      for (const p of minhas) {
        ...
        for (const u of r?.usadas || []) {
          if (!u?.corrida && !u?.certificado) continue;
          rastreio.push({ material: p.perfil || null, corrida: u.corrida || null, certificado: u.certificado || null, norma: u.norma || null });
        }
      }
    }
    ...
    rastreio = rastreio.filter((r) => {
      const k = `${r.material}|${r.corrida}|${r.certificado}`;
  ...
  return {
    ...
    etapa: expedida ? "expedida" : setor === "Pintura" ? "pronta" : setor ? `em ${setor.toLowerCase()}` : SEM,
    expedicao: expedida
