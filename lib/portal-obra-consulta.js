...
export async function pecaParaCliente({ opId, opNumero, marca }) {
  ...
  //
  // ⚠ FORNECEDOR E PEDIDO DE COMPRA CONTINUAM FORA. Ele pediu NF e peso, não a cadeia inteira: o
  // nome do fornecedor e o nº do pedido são a nossa negociação, e o `linhaCmr` traz tudo junto —
    ...
    if (minhas[0].tipoPeca === "CONJUNTO") {
      ...
      for (const l of linhas) {
        for (const u of l.usadas || []) {
          ...
          rastreio.push({ r: u.rastreio || null, material: l.perfil || null, corrida: u.corrida || null, certificado: u.certificado || null, norma: u.norma || null, nf: u.nf || null, compradoKg: u.pesoKg || null });
        }
      }
    } else {
      ...
      for (const p of minhas) {
        ...
        for (const u of r?.usadas || []) {
          if (!u?.corrida && !u?.certificado) continue;
          rastreio.push({ r: u.rastreio || null, material: p.perfil || null, corrida: u.corrida || null, certificado: u.certificado || null, norma: u.norma || null, nf: u.nf || null, compradoKg: u.pesoKg || null });
        }
      }
    }
