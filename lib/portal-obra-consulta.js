...
/**
 * A etapa de cada marca da obra, já contando o CONJUNTO pelos croquis dele.
 *
 * ⚠⚠ QUEM É APONTADO NO CORTE É O CROQUI, NÃO O CONJUNTO. Vitor (05/09/2026): "me parece ter muito
 * mais peças preparadas do que essas que você mostrou no filtro" — e tinha. Na OP-112 o Syneco
 * registra 67 marcas cortadas (850 peças), todas croquis e avulsas: **nenhum conjunto** tem
 * apontamento próprio. Como o modelo 3D mostra CONJUNTOS, o filtro casava só as avulsas e a obra
 * parecia parada. Contando pelos croquis, os 47 conjuntos da 112 estão em preparação — que é o que
 * a fábrica está fazendo.
 *
 * ⚠ O apontamento PRÓPRIO do conjunto manda: quando ele é montado, soldado ou pintado, o setor dele
 * é esse. A herança pelos croquis só preenche o vazio do começo da vida da peça — e nunca passa de
 * "Preparação", porque croqui não vai além disso: ele é cortado e vira parte do conjunto.
 */
export async function etapaDasMarcas(opId, marcas) {
  const proprio = await setorDasMarcas(marcas);
  if (!opId) return proprio;
  try {
    const conjuntos = await prisma.pecaConjunto.findMany({
      where: { opId, tipoPeca: "CONJUNTO" },
      select: { marca: true, conjuntoCroquis: { select: { croqui: { select: { qteProduzida: true, corteConcluidoEm: true } } } } },
      take: 6000,
    });
    for (const c of conjuntos) {
      if (!c.marca || proprio.has(c.marca)) continue;
      const emPreparo = (c.conjuntoCroquis || []).some(
        (x) => (Number(x?.croqui?.qteProduzida) || 0) > 0 || x?.croqui?.corteConcluidoEm,
      );
      if (emPreparo) proprio.set(c.marca, "Preparação");
    }
  } catch { /* sem a estrutura, fica só o apontamento próprio */ }
  return proprio;
}
