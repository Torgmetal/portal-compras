...
export async function gerarPosicaoCronogramaPDF(opId) {
  const op = await prisma.oP.findUnique({
    where: { id: opId },
    select: { id: true, numero: true, cliente: true, obra: true, refCliente: true, clienteEmail: true, clienteContatos: true },
  });
  ...
  const [envios, tarefas, correspondencia, revisoes] = await Promise.all([
    ...
    prisma.obraEmailEvento.findMany({
      ...
      // ⚠ o `snippet` é o TEXTO — sem ele as seções 8b e 8c saem só com o assunto, e elas existem
      // justamente para carregar o que a outra parte escreveu.
      select: { direcao: true, de: true, deNome: true, para: true, cc: true, conversationId: true,
                assunto: true, snippet: true, recebidoEm: true, enviadoEm: true, tipoGatilho: true,
                temAnexo: true, anexos: true },
    }),
  ...
  // ⚠⚠ TRÊS PARTES, NÃO DUAS. Vitor (29/08/2026): "o Jonas é um calculista nosso; para termos
  // envolvido ele, foi uma solicitação que partiu da engenharia ou do cliente — preciso que vincule
  // tudo, preciso que seja cirúrgico".
  //
  // A regra antiga era "não é @torg.com.br ⇒ cliente", e isso jogava o calculista da Torg dentro de
  // "o que o cliente respondeu" — bem na seção que precisa ser exata, porque é a única em que a
  // outra parte fala. Agora o CLIENTE é definido pelo cadastro da OP (clienteEmail +
  // clienteContatos): quem escreve daquele domínio é o cliente; o resto que não é Torg é TERCEIRO,
  // e vai para uma seção própria.
  const dominio = (e) => String(e || "").toLowerCase().split("@")[1] || "";
  const domCliente = new Set(
    [op.clienteEmail, ...(Array.isArray(op.clienteContatos) ? op.clienteContatos.map((c) => c?.email) : [])]
      .map(dominio).filter(Boolean),
  );
  const parte = (endereco) => {
    const d = dominio(endereco);
    if (!d) return "DESCONHECIDO";
    if (d === "torg.com.br") return "TORG";
    if (domCliente.has(d)) return "CLIENTE";
    return "TERCEIRO";
  };

  // ⚠ QUEM TROUXE O TERCEIRO. Está no registro: a primeira mensagem da thread que o coloca em
  // "para" ou "cc". Se saiu da Torg, o envolvimento partiu da Engenharia; se veio do cliente,
  // partiu dele. É a resposta que hoje se busca na memória de quem participou.
  const quemChamou = (endereco, conversationId) => {
    const alvo = String(endereco || "").toLowerCase();
    const naThread = correspondencia
      .filter((x) => x.conversationId && x.conversationId === conversationId)
      .sort((a, x) => new Date(a.recebidoEm || a.enviadoEm || 0) - new Date(x.recebidoEm || x.enviadoEm || 0));
    const primeira = naThread.find((x) => {
      const dest = [...(Array.isArray(x.para) ? x.para : []), ...(Array.isArray(x.cc) ? x.cc : [])]
        .map((d) => String(d?.email || d || "").toLowerCase());
      return dest.includes(alvo);
    });
    if (!primeira) return null;
    return {
      em: primeira.recebidoEm || primeira.enviadoEm,
      por: primeira.deNome || primeira.de,
      lado: parte(primeira.de),
    };
  };
  ...
  y -= 54;
  const nRespCli = correspondencia.filter((e) => e.direcao === "ENTRADA" && parte(e.de) === "CLIENTE").length;
  txt(`Somadas, as tarefas paradas acumulam ${diasEspera} dias de espera. ${nRespCli} respostas do cliente e ${correspondencia.filter((e) => e.direcao === "ENTRADA" && parte(e.de) === "TERCEIRO").length} de terceiros, no periodo ${dt(janela._min.recebidoEm)} a ${dt(new Date())}.`,
    40, 8, f, CINZA);
  y -= 24;
  ...
  const respostasCliente = correspondencia.filter((e) => e.direcao === "ENTRADA" && parte(e.de) === "CLIENTE");
  ...
  // ─── 8c. TERCEIROS NA CONVERSA ──────────────────────────────────────────────
  // ⚠⚠ NEM TODO MUNDO DE FORA É O CLIENTE. Calculista, projetista contratado e fornecedor entram
  // na thread e não podem ser lidos como "o cliente respondeu" — na OP-089 é o calculista da
  // própria Torg. E o dossiê registra QUEM PEDIU o envolvimento de cada um, que é a pergunta que
  // hoje se responde de memória.
  const deTerceiros = correspondencia.filter((e) => e.direcao === "ENTRADA" && parte(e.de) === "TERCEIRO");
  if (deTerceiros.length) {
    sec("8c. TERCEIROS NA CONVERSA");
    const porEndereco = new Map();
    for (const e of deTerceiros) {
      const k = String(e.de).toLowerCase();
      if (!porEndereco.has(k)) porEndereco.set(k, []);
      porEndereco.get(k).push(e);
    }
    for (const [endereco, msgs] of porEndereco) {
      espaco(44);
      const m0 = msgs[0];
      const chamou = quemChamou(endereco, m0.conversationId);
      txt(String(m0.deNome || endereco).slice(0, 42), 42, 8.5, b);
      txt(endereco.slice(0, 42), 200, 7.5, f, CINZA);
      txt(`${msgs.length} mensagem(ns)`, 470, 7.5, f, CINZA);
      y -= 12;
      txt(chamou
        ? `Incluido na conversa em ${dt(chamou.em)} por ${String(chamou.por).slice(0, 34)} (${chamou.lado === "TORG" ? "a pedido da Engenharia" : chamou.lado === "CLIENTE" ? "a pedido do cliente" : "origem nao identificada"})`
        : "Momento da inclusao na conversa nao identificado no registro.",
        50, 7.5, f, chamou ? ESCURO : CINZA);
      y -= 13;
      for (const e of msgs) {
        espaco(24);
        txt(dt(e.recebidoEm || e.enviadoEm), 50, 7.5, b);
        txt(String(e.assunto || "").slice(0, 84), 100, 7.5, f, CINZA);
        y -= 10;
        const corpo = String(e.snippet || "").replace(/\s+/g, " ").trim();
        for (const l of (corpo.match(/.{1,100}(\s|$)/g) || []).slice(0, 3)) {
          espaco(11); txt(l.trim(), 58, 7.5); y -= 10;
        }
      }
      y -= 8;
    }
