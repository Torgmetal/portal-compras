...
import { marcasEntreguesAExpedicao, entregueAExpedicao } from "./entregue-expedicao";
import { STATUS_OP_MORTA } from "./op-viva";
import { pecasNoTerceiro } from "./fora-da-fabrica";

// A fonte de peças da OP é a LPC — conjuntos, croquis e avulsas. Só ela.
//
// ⚠⚠ AQUI HAVIA UM FALLBACK PARA A LE, E ELE SAIU EM 07/09/2026. A regra era "LPC se existir, senão
// a LE tratando cada peça como solo". Parecia generosa e era mentirosa: obra sem LPC ainda não foi
// liberada para produzir, e mesmo assim entrava no painel do PCP somando peso como se estivesse na
// fábrica. Medido no dia: a OP-118 (ENC 336 – Caldeira Uruçuí) entrava com 1.641 peças e
// **136.942 kg** de LE no "Fluxo da fábrica" — uma obra que nunca teve um apontamento sequer.
//
// Vitor (07/09/2026): "tire o fallback, sem LPC não se fala em produção". É a mesma regra que ele
// já tinha dado sobre o Gantt e sobre as filas — ver [[torg_producao_e_lpc]] e lib/lista-pecas.js.
//
// Consequência combinada: OP sem NENHUMA peça de LPC sai das três telas que leem esta lib (o painel
// do PCP, /planejamento/prioridades e /pcp/dashboard-prioridades). Não é peça escondida: é obra que
// ainda não existe para a produção.
//
// Continuam de fora, como antes: a linha "TOTAL.:" do import da LE (peso da lista inteira somado
// numa peça só — sem isso a OP-060 aparecia com 153 t fantasma) e os ITENS COMPRADOS
// (parafuso/porca/arruela/…), que não são feitos por nós.
function selecionarUniverso(pecas) {
  return pecas
    .filter((p) => p.fonte === "LPC_IMPORT")
    .filter((p) => !ehLinhaLixo(p) && !ehItemComprado(p));
}
...
export async function carregarPrioridadesPorObra() {
  ...
  // ⚠⚠ OBRA ENCERRADA SAI, VENHA DE ONDE VIER. Das quatro fontes acima, só a da programação
  // conferia o status — cronograma, OP fixada à mão e "enviada para produção" traziam obra morta
  // de volta. Medido em 07/09/2026, logo depois de encerrar as OPs terminadas: 083, 084, 089, 102,
  // 104, 106 e 115 continuavam no painel do PCP e na TV de prioridades.
  //
  // A regra é a mesma de lib/op-viva.js e mora aqui uma vez só, no fim, em vez de repetida em cada
  // fonte — foi exatamente a repetição que deixou três delas passarem.
  const mortas = new Set(
    (await prisma.oP.findMany({
      where: { id: { in: opIds }, status: { in: STATUS_OP_MORTA } },
      select: { id: true },
    })).map((o) => o.id)
  );
  ...
  // FIM DE LINHA DO PCP: peça já em romaneio (prévio ou emitido) sai das raias — a Expedição
  // assumiu. Vitor (19/08): "fez o romaneio prévio, ou emitiu o romaneio, sim essa peça deve sair
  // do portal do PCP e a responsabilidade passa a ser do próximo setor". Mesma regra do painel de
  // despacho, pela mesma lib — se divergissem, a TV e o painel mostrariam contas diferentes da
  // mesma obra.
  const entreguesPorOp = new Map(
    await Promise.all(opIds.map(async (id) => [id, await marcasEntreguesAExpedicao(prisma, id)]))
  );

  // ⚠ "TEM LPC?" SE MEDE ANTES do corte de entregues, e por isso não pode sair do `pecasPorOp`.
  // Uma obra que já embarcou tudo tem universo vazio mas TEM LPC — ela continua se comportando como
  // antes. Quem sai é só a obra que nunca teve lista de produção nenhuma.
  const opsComLpc = new Set(pecasRaw.filter((p) => p.fonte === "LPC_IMPORT").map((p) => p.opId));

  /* ⚠⚠ O QUE ESTÁ NO TERCEIRO SAI DA FRENTE DO PCP. Vitor (08/09/2026): "a 97B foi enviada para
     terceiros, porém ela ainda aparece no painel do PCP, precisa tirar ela da frente".
     A trava por `terceirizado` (logo abaixo, em prioridades-setor.js) só alcança o CONJUNTO — na
     OP-097 os 65 conjuntos T97B estão marcados, mas os 262 CROQUIS deles não, e eram eles que
     continuavam nas raias: 260 PENDENTE e 2 em CORTE. O romaneio é a autoridade e desce até o
     croqui [[torg_romaneio_terceiro]]. */
  const noTerceiro = await pecasNoTerceiro();

  const pecasPorOp = new Map();
  for (const p of pecasRaw) {
    if (noTerceiro.has(p.id)) continue;
    if (entregueAExpedicao(p, entreguesPorOp.get(p.opId) || new Set())) continue;
  ...
  const porObra = obras.filter((o) => opsComLpc.has(o.opId) && !mortas.has(o.opId)).map((o) => {
