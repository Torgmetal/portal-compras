...
function selecionarUniverso(pecas) {
  ...
  return base.filter((p) => !ehItemComprado(p));
}
...
export async function carregarPrioridadesPorObra() {
  ...
  for (const c of cronos) {
    ...
    obras.push({ opId: c.op.id, opNumero: c.op.numero || c.opNumero, obra: c.op.obra || c.titulo || c.opNumero, cliente: c.op.cliente || null, refCliente: c.op.refCliente || null, entrega, datasSetorCrono: datasSetorDoCronograma(c.tarefas) });
    opIds.push(c.op.id);
  }
  ...
  if (fixadas.length) {
    ...
    const opsFix = await prisma.oP.findMany({ where: { OR: [{ id: { in: idsFix } }, { numero: { in: numsFix } }] }, select: { id: true, numero: true, obra: true, cliente: true, refCliente: true } });
    ...
    for (const f of fixadas) {
      ...
      obras.push({ opId: op.id, opNumero: op.numero, obra: op.obra || op.numero, cliente: op.cliente || null, refCliente: op.refCliente || null, entrega: null, datasSetorCrono: {}, manual: true });
      opIds.push(op.id);
      vistos.add(op.id);
    }
  }
  ...
  const pecasRaw = await prisma.pecaConjunto.findMany({
    where: { opId: { in: opIds } },
    select: {
      opId: true, marca: true, descricao: true, tipoPeca: true, perfil: true, pesoTotalKg: true, status: true, fonte: true,
  ...
  for (const p of pecasRaw) {
    ...
    arr.push({
      marca: p.marca, descricao: p.descricao, tipoPeca: p.tipoPeca, perfil: p.perfil, pesoTotalKg: p.pesoTotalKg, status: p.status, fonte: p.fonte,
      ...
      terceirizado: p.terceirizado, destinoTerceirizado: p.destinoTerceirizado, terceirizadoRecebidoEm: p.terceirizadoRecebidoEm, terceiroRetornoPrevisto: p.terceiroRetornoPrevisto,
    });
