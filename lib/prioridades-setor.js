...
// Setores pós-corte de uma peça solo (avulsa): do CORTE vai direto ao JATO.
//
// ⚠ ACABAMENTO SAIU DAQUI. Vitor (25/08/2026): "no jato as peças avulsas ficam disponíveis para
// jateamento assim que sai do corte". Medido nas 3.858 ordens de avulsa casadas à LPC: Corte 43%,
// Pintura 33%, Jato 15% e Acabamento só 9% — ou seja, passar pelo acabamento é a exceção, não a
// rota. Contá-lo inflava o TOTAL do Acabamento com peça que nunca chega lá, e o setor aparecia com
// mais pendência do que tem (era o de menor entrega medida: 48 t/mês).
const POS_CORTE_SOLO = ["JATO", "PINTURA", "EXPEDICAO"];

// ⚠⚠ FONTE ÚNICA DA ROTA. Esta regra estava escrita em TRÊS lugares — aqui, em
// app/api/producao/prioridades e em app/api/pcp/despacho — e ao corrigir só este, em 25/08/2026,
// duas telas do portal passaram a discordar: a TV dizia que o Acabamento não tinha avulsa e a tela
// dos setores continuava listando 261 avulsas / 22.435 kg lá. Quem exporta a regra é este arquivo;
// os outros importam.
export const ROTA_SOLO = ["CORTE", ...POS_CORTE_SOLO];
export const ROTA_COMPOSTA = [...POS_CORTE_COMPOSTO];
/** a peça avulsa/solo passa por este setor? */
export const soloPassaNoSetor = (setor) => !setor || ROTA_SOLO.includes(setor);

export function pecaEhCroqui(p) {
  // O vínculo de componente é mais confiável que classificações legadas da importação.
  const pais = p?._count?.croquiConjuntos ?? p?.croquiConjuntos?.length ?? 0;
  return p?.tipoPeca === "CROQUI" || Number(pais) > 0;
}
// Conjunto composto = conjunto da LPC que realmente tem sub-peças (croquis) listadas.
export function pecaEhComposta(p) {
  const croquis = p?.croquiCount ?? p?._count?.conjuntoCroquis ?? p?.conjuntoCroquis?.length ?? 0;
  return !pecaEhCroqui(p) && p?.tipoPeca === "CONJUNTO" && Number(croquis) > 0;
}
...
//
// ⚠ `PecaConjunto.status` diz ONDE A PEÇA ESTÁ (ou aguarda), NÃO o que ela concluiu: status
// "MONTAGEM" = saiu do corte e está na fila da montagem — ela NÃO foi montada. Tratar o status
// como setor concluído dava Montagem 100% em obra sem um único apontamento de montagem (OP-106:
// 7 conjuntos, todos status MONTAGEM, zero apontamento, e o portal dizia 100%). O concluído é o
// setor ANTERIOR ao do status. (Vitor 19/08: "isso na verdade está acontecendo em várias".)
// EXPEDIDO é o único terminal — ali tudo foi concluído.
function realIdxDe(p, map) {
  let r = idxOf(map.get(p.marca));
  const iStatus = idxOf(p.status);
  const s = iStatus >= 0 ? iStatus - 1 : -1; // status = onde ESTÁ → concluiu o anterior
  if (s > r) r = s;
  if (p.status === "EXPEDIDO") r = idxOf("EXPEDICAO");
  // No terceiro (ainda NÃO recebido de volta): a peça saiu da fábrica e volta no setor
  // `destinoTerceirizado`. Conta como ADIANTADA até o setor anterior ao de retorno — sai da fila
  // do setor de onde saiu (ex.: Montagem) e fica PENDENTE só no setor de retorno, com a data do
  // terceiro. Ao ser recebida (terceirizadoRecebidoEm), o status normal volta a mandar.
  if (p.terceirizado && !p.terceirizadoRecebidoEm && p.destinoTerceirizado) {
    const back = idxOf(p.destinoTerceirizado) - 1;
    if (back > r) r = back;
  }
  // Encaminhada MANUALMENTE pra um setor (painel de Liberar): pula as etapas anteriores e fica
  // pendente no setor escolhido (ex.: direto pro Jato) — mesma mecânica do terceiro.
  if (p.encaminhadoSetor) {
    const e = idxOf(p.encaminhadoSetor) - 1;
    if (e > r) r = e;
  }
  return r;
}

// A peça está fisicamente no terceiro agora (foi enviada e ainda não voltou)?
export function noTerceiroAgora(p) {
  return !!(p && p.terceirizado && !p.terceirizadoRecebidoEm && p.destinoTerceirizado);
}
...
export function progressoPorSetor(pecas, realSetorByMarca) {
  ...
  // ⚠⚠ CONTA PEÇA TAMBÉM, NÃO SÓ KG. Vitor (01/09/2026), depois de a montagem cair de 4.312 para
  // 1.425 kg/dia com o MESMO ritmo de peças: a montagem em diante se mede em peça, não em tonelada.
  // As abas de setor da tela do PCP mostravam kg em todos os sete — e na montagem isso mente.
  // `un` = unidades (a soma de `qte`); `itens` = quantas marcas distintas passam pelo setor.
  const acc = FLUXO_SETORES.map((s) => ({ ...s, totalKg: 0, feitoKg: 0, totalUn: 0, feitoUn: 0, totalItens: 0, feitoItens: 0 }));
  ...
  for (const p of pecas || []) {
    ...
    const un = Math.max(1, Number(p.qte) || 1);

    if (pecaEhCroqui(p)) {
      // Sub-peça: só existe no Corte (Preparação).
      const c = byKey.CORTE;
      const fr = fracaoCortada(p);
      c.totalKg += kg; c.feitoKg += kg * fr;
      c.totalUn += un; c.feitoUn += un * fr;
      c.totalItens += 1; if (fr >= 1) c.feitoItens += 1;
      continue;
    }
    ...
    if (pecaEhComposta(p)) {
      ...
      for (const key of POS_CORTE_COMPOSTO) {
        const s = byKey[key];
        const feito = rIdx >= idxOf(key);
        s.totalKg += kg; s.totalUn += un; s.totalItens += 1;
        if (feito) { s.feitoKg += kg; s.feitoUn += un; s.feitoItens += 1; }
      }
      continue;
    }
    ...
    // realIdxDe = último setor CONCLUÍDO. Concluiu o CORTE (>=) → cortada 100%; senão a fração
    // apontada. (Era `>`, que com a semântica nova deixava de fora a peça que acabou de sair do
    // corte — o status "MONTAGEM" numa avulsa significa exatamente isso. Vitor 19/08.)
    const frSolo = rIdx >= idxOf("CORTE") ? 1 : fracaoCortada(p);
    c.feitoKg += kg * frSolo;
    c.totalUn += un; c.feitoUn += un * frSolo;
    c.totalItens += 1; if (frSolo >= 1) c.feitoItens += 1;
  ...
  return acc.map((s) => {
    ...
    const totalUn = Math.round(s.totalUn);
    const feitoUn = Math.round(Math.min(s.feitoUn, s.totalUn));
    return {
      setor: s.key, label: s.label, totalKg, feitoKg,
      pendenteKg: totalKg - feitoKg,
      totalUn, feitoUn, pendenteUn: totalUn - feitoUn,
      totalItens: s.totalItens, feitoItens: s.feitoItens, pendenteItens: s.totalItens - s.feitoItens,
...
export function croquisCortadosPorConjunto(pecas, realSetorByMarca, links) {
  ...
  for (const p of pecas || []) {
    // O conjunto CONCLUIU o corte (>= CORTE): seus croquis foram cortados, mesmo sem apontamento
    // escrito neles. Um conjunto que só está na FILA da montagem já passou pelo corte.
...
export function temDetalheCorte(pecas) {
  // Croqui é o caso comum (LPC com conjunto+sub-peças), MAS há obra que é só peça AVULSA: a
  // Engenharia manda só a LE e cada marca já é a peça de corte. Sem isso a OP ficava eternamente
  // em "aguardando lista de corte da Engenharia" — caso da OP-108, que tem 2 marcas avulsas de
  // 5.134 kg pra cortar. (Vitor 19/08: "a 108 só terá lista LE pois são peças avulsas".)
  return (pecas || []).some((p) => pecaEhCroqui(p) || (pecaEhSolo(p) && !!(p.perfil && String(p.perfil).trim())));
}
...
export function pecasPendentesNoSetor(pecas, realSetorByMarca, setorKey, montaveis = null) {
  ...
  for (const p of pecas || []) {
    ...
    // "No terceiro" só marca a fila do SETOR DE RETORNO (destinoTerceirizado) — é onde a peça
    // volta e a data importa. Nos setores seguintes ela é trabalho normal futuro (já terá voltado).
    const terceiro = noTerceiroAgora(p) && p.destinoTerceirizado === setorKey;
    out.push({ marca: p.marca, kgPend: Math.round(kgPend), prioridade: p.prioridade ?? null, ordem: p.ordemCampo ?? null, montavel, terceiro, retornoPrevisto: terceiro ? (p.terceiroRetornoPrevisto || null) : null });
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// SEMEAR AS DATAS POR SETOR A PARTIR DO CRONOGRAMA.
//
// Vitor (25/08/2026): "hoje temos que selecionar a OP e pedir para sugerir uma data; podemos deixar
// como padrão que, assim que o cronograma for criado, isso já é criado e sugerido com a data que
// ficou do cronograma aprovado... seria após o cronograma de fato ser preenchido e definido".
//
// ⚠⚠ NUNCA SOBRESCREVE DATA INFORMADA. Só preenche setor que está VAZIO. A data digitada aqui
// SOBREPÕE o cronograma na TV de Prioridades — é decisão de quem planeja, e regerar o cronograma
// não pode desfazer essa decisão pelas costas. É a mesma regra do `apenasSemData` do Gerar Datas.
//
// ⚠ E não inventa setor que o cronograma não descreve: se não há tarefa de Jato, Jato fica vazio.
// Melhor um campo em branco, que a pessoa vê e preenche, do que uma data plausível e errada.
/**
 * @returns {Promise<{preenchidos: string[], mantidos: string[]}>}
 */
export async function semearDatasSetor(prisma, { opNumero, opId, tarefas }) {
  if (!opNumero) return { preenchidos: [], mantidos: [] };
  const doCrono = datasSetorDoCronograma(tarefas);
  if (!Object.keys(doCrono).length) return { preenchidos: [], mantidos: [] };

  const atual = await prisma.solicitacaoProducao.findUnique({ where: { opNumero }, select: { datasSetor: true } });
  const jaTem = (atual?.datasSetor && typeof atual.datasSetor === "object") ? atual.datasSetor : {};

  const novo = { ...jaTem };
  const preenchidos = [], mantidos = [];
  for (const [k, v] of Object.entries(doCrono)) {
    if (jaTem[k]) { if (jaTem[k] !== v) mantidos.push(k); continue; }
    novo[k] = v;
    preenchidos.push(k);
  }
  if (!preenchidos.length) return { preenchidos: [], mantidos };

  await prisma.solicitacaoProducao.upsert({
    where: { opNumero },
    update: { datasSetor: novo, ...(opId ? { opId } : {}) },
    create: { opNumero, opId: opId || null, datasSetor: novo, status: "SOLICITADA", prioridade: "MEDIA" },
  });
  return { preenchidos, mantidos };
}
