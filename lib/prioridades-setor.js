...
// Conjuntos "liberados pra montar": todos os croquis vinculados já foram cortados
// (Syneco/apontamento de corte). Conjunto cujo croqui ainda não foi cortado não dá pra
// montar — fica preso no corte. links: [{ conj: <marcaConjunto>, croqui: <marcaCroqui> }].
export function conjuntosMontaveis(pecas, links) {
  ...
  for (const lk of links || []) {
    ...
    if (cr && fracaoCortada(cr) >= 1) a.cortados++;
    agg.set(lk.conj, a);
  }
  const out = new Set();
...
// Progresso da MONTAGEM contando SÓ os conjuntos montáveis (croquis já cortados) — os que
// "dá pra montar agora". Conjunto com croqui pendente fica fora do escopo (aparece no Corte).
// Mesmo shape de progressoPorSetor; devolve também o Set `montaveis` (reuso na fila detalhada).
export function progressoMontagemMontavel(pecas, realSetorByMarca, links) {
  ...
  for (const p of pecas || []) {
    if (!pecaEhComposta(p) || !montaveis.has(p.marca)) continue;
    const kg = Number(p.pesoTotalKg) || 0;
    if (kg <= 0) continue;
    totalKg += kg;
...
export function pecasPendentesNoSetor(pecas, realSetorByMarca, setorKey, montaveis = null) {
  ...
  for (const p of pecas || []) {
    ...
    if (pecaEhCroqui(p)) {
      ...
      // Montagem: só conjuntos montáveis (croquis já cortados) — os que dá pra montar agora.
      if (setorKey === "MONTAGEM" && montaveis && !montaveis.has(p.marca)) continue;
    ...
    out.push({ marca: p.marca, kgPend: Math.round(kgPend), prioridade: p.prioridade ?? null, ordem: p.ordemCampo ?? null });
  }
