...
const GRADE_PATTERNS = [
  ...
  // Soufer escreve "A572" sozinho na descrição, sem o GR50 — assume A572-GR50
  // (a grade mais comum de A572 pra estrutural no Brasil)
  { re: /\bA\s*572\b(?!\s*\d)/i, label: "A572-GR50" },
  { re: /\bA\s*36\b/i, label: "A36" },
  { re: /CIVIL\s*300/i, label: "A572-GR50" }, // sinônimo brasileiro de A572
  { re: /\bCV\s*300/i, label: "A572-GR50" }, // abreviação visto em PDFs
];
...
export function normalizeProduto(desc, mat = "") {
  ...
  // PERFIL W (W150X13) — checa antes de PERFIL_DOB porque "PERFIL"
  // sozinho é ambíguo. Sem \b antes do W porque o Soufer escreve
  // tudo junto ("PERFILW150X13,0-12M") e não há word boundary entre L e W.
  // O padrão estrutural (W + 2-4 dígitos + X + dígitos) é específico o
  // bastante pra evitar falsos positivos.
  const wm = cleaned.match(/W\s*(\d{2,4})\s*X\s*(\d+(?:[,.]\d+)?)/);
  if (wm) {
    return {
      categoria: "PERFIL_W",
  ...
  // PERFIL DOBRADO A FRIO (DBFQ no Soufer, "DOBRADO/DOBRADA" em geral)
  // Match SÓ por espessura (lateral varia por fabricante; pode estar em
  // polegadas ou mm). "1/4" é polegadas → convertido. "2,65" é mm.
  if (/DBFQ?|DOBRAD[OA]/.test(cleaned)) {
    // 1) número logo após DBFQ
    let m = cleaned.match(/DBFQ?\s*(\d+(?:[,.]\d+)?(?:\/\d+)?)/);
    // 2) fallback: primeiro número standalone
    if (!m) m = cleaned.match(/(?:^|[\s,])(\d+(?:[,.]\d+)?(?:\/\d+)?)/);
    if (m) {
      const raw = m[1];
      let esp;
      if (raw.includes("/")) {
        ...
        esp = parseFloat(raw.replace(",", ".")); // assume mm
      }
      if (isFinite(esp) && esp > 0) {
        return { categoria: "PERFIL_DOB", dim1: Math.round(esp * 100) / 100, dim2: null, grade };
      }
    }
  }
...
export function matchProdutos(a, b, opts = {}) {
  ...
  if (a.categoria === "PERFIL_DOB") {
    // Perfil dobrado: match SÓ por espessura, com tolerância pequena.
    // Lateral varia muito entre fornecedores e fica fora do critério.
    return Math.abs(a.dim1 - b.dim1) <= (opts.tolPerfilDob ?? 0.3);
  }
  return false;
}
