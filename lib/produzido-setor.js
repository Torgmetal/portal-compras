 ...
 */
export async function lerProduzidoPorSetor(pecas, setores, { deduzirRota = true, banco = prisma } = {}) {
  ...
  const ordens = await banco.mesOrdem.findMany({
  ...
  const baixadas = await banco.pecaConjunto.findMany({
  ...
  const expedidos = deduzirRota
    ? await banco.romaneioItem.findMany({
