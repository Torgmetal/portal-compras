...
// ⚠ CROQUI FICA DE FORA. O croqui é o desenho da PEÇA solta — numa OP média são centenas
// (na OP-067 eram 429 contra 1336 conjuntos) e o data book incha a ponto de não fechar o PDF.
// O comentário do topo sempre disse que croqui não entra, mas `coletarPdfs` varria as
// subpastas de Conjunto sem filtrar e eles entravam pela porta dos fundos. (Vitor, 22/08/2026)
const ehCroqui = (nome) => /croqui/i.test(String(nome || ""));
...
async function coletarPdfs(driveId, path) {
  ...
  const itens = kids.filter((c) => ehPdf(c) && !ehCroqui(c.name)).map((f) => ({ f, rev: false }));
  for (const s of kids.filter((c) => c.folder && !/obsolet/i.test(c.name || "") && !ehCroqui(c.name))) {
    const sk = await listChildrenByPath(driveId, `${path}/${s.name}`).catch(() => []);
    const rev = /revis/i.test(s.name || "");
    for (const f of sk.filter((c) => ehPdf(c) && !ehCroqui(c.name))) itens.push({ f, rev });
  }
