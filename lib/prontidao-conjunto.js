...
// ⚠⚠ "CORTADO" É UMA REGRA SÓ NO PORTAL INTEIRO — `croquiCortado` (lib/prioridades-setor): apontamento
// do Syneco (`qteProduzida ≥ qte`) OU `corteConcluidoEm` OU baixa de corte (`baixaSetores.CORTE`).
// OP-113 (14/09/2026): a lista do PCP dizia "2/2 pronto" (croquiCortado) e a liberação recusava os
// mesmos conjuntos com "não desceu — 1/2 croquis cortados", porque esta função só olhava o Syneco. O
// croqui T113A-P53 tinha 0/28 no Syneco e `corteConcluidoEm` gravado pela baixa — cortado de verdade.
// Duas contas para a mesma pergunta é exatamente a divergência que este arquivo existe para evitar.
//
// ⚠ Quem chama precisa TRAZER os campos que a regra lê — use `CROQUI_PRONTIDAO_SELECT` no Prisma.
// Select só com `qteProduzida` volta a contar pelo Syneco e reabre o bug sem nenhum erro.
export const CROQUI_PRONTIDAO_SELECT = { marca: true, qte: true, qteProduzida: true, corteConcluidoEm: true, baixaSetores: true };

// quantidade cortada "efetiva": Syneco, ou a baixa de corte, ou tudo quando o corte foi concluído
function produzidoNoCorte(c, necessario) {
  const syneco = Number(c.qteProduzida) || 0;
  const bx = c.baixaSetores && typeof c.baixaSetores === "object" ? c.baixaSetores.CORTE : null;
  const baixa = bx ? (bx.qtd != null ? Number(bx.qtd) || 0 : necessario) : 0;
  const produzido = Math.max(syneco, baixa);
  return croquiCortado(c) ? Math.max(produzido, necessario) : produzido;
}
...
export function calcularProntidao(conjunto) {
  ...
  for (const rel of croquis) {
    ...
    const produzido = produzidoNoCorte(c, necessario);
