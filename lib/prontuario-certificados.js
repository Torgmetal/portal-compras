...
export async function escanearCertificados() {
  ...
  // Desce em paralelo (concorrência limitada). Lê os certificados de TODAS as pastas de
  // treinamento do nível. Só desce nas subpastas quando ainda NÃO achou treinamento aqui
  // (níveis de agrupamento) ou quando a subpasta é intermediária genérica (VMI/TORG) —
  // nunca nas folhas de certificado (A.S.O's, Exames…). Corta ~800→~150 chamadas e mantém
  // os 512 (inclusive quem aninha em <Colaborador>/VMI/Treinamentos).
  async function walk(path, colab, depth) {
    if (depth > 5) return;
    let items;
    try { items = await ls(path); } catch { return; }
    const pastas = items.filter((it) => it.folder);
    const treinos = pastas.filter((it) => ehTreino(it.name));
    await Promise.all(treinos.map(async (tr) => {
      let arqs;
      try { arqs = await ls(`${path}/${tr.name}`); } catch { return; }
      for (const a of arqs) {
        if (!a.file) continue;
        out.push({ colab, arquivo: a.name, nr: nrDoArquivo(a.name), data: dataArquivo(a) });
      }
    }));
    const descer = pastas.filter((it) => !ehTreino(it.name) && (treinos.length === 0 || GENERICO.test(it.name)));
...
export async function documentosDeProntuarioSeguro(funcionarios, timeoutMs = 25000) {
  ...
  try {
    ...
    console.error("Prontuário indisponível (cai p/ docs do RH):", err?.message);
