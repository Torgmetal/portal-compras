...
// A descrição da obra sai do PDF como um parágrafo único de mil caracteres — ilegível no campo da
// OP. Vitor (19/08): "ainda está bugado… na parte da descrição precisa melhorar, colocar as
// informações da proposta aqui na descrição e deixar melhor isso".
//
// Aqui ela vira texto com estrutura: cada subtítulo numerado (1.2.1 ESTRUTURA METÁLICA,
// 1.2.2.1 TELHAS…) vira um título, e cada item vira uma linha com marcador.
//
// ⚠ O corte dos itens NÃO pode ser em todo hífen: "esp.: 0,65mm - NATURAL" e "chapa 24 (0,65mm) -
// NATURAL" quebrariam no meio. Só corta em hífen seguido de palavra Capitalizada ou número —
// "- Área:", "- Comprimento:", "- Telhas trapézio" — deixando " - NATURAL" e " - esp.:" na linha.
function formatarDescricao(bruta) {
  const txt = String(bruta || "").replace(/\s+/g, " ").trim();
  if (!txt) return null;

  // parte nos subtítulos numerados, guardando o título de cada bloco
  const partes = txt.split(/(?=\d+(?:\.\d+){1,3}\s+[A-ZÁÂÃÉÊÍÓÔÕÚÇ])/).map((x) => x.trim()).filter(Boolean);
  const linhas = [];
  for (const parte of partes) {
    const m = parte.match(/^(\d+(?:\.\d+){1,3})\s+([A-ZÁÂÃÉÊÍÓÔÕÚÇ][A-ZÁÂÃÉÊÍÓÔÕÚÇ0-9 ,."'’-]{2,70}?)(?=\s[-–]\s|\s[A-ZÁ-Ú][a-zá-ú]|$)/);
    let corpo = parte;
    if (m) {
      linhas.push(`${m[2].replace(/[ .,-]+$/, "").trim()}`);
      corpo = parte.slice(m[0].length);
    }
    for (const item of corpo.split(/\s[-–]\s+(?=[A-ZÁÂÃÉÊÍÓÔÕÚÇ][a-zá-úâêîôû]|\d)/)) {
      const t = item.replace(/^[-–\s]+/, "").replace(/[;\s]+$/, "").trim();
      if (t.length <= 2) continue;
      // ⚠ Subtítulo SEM numeração ("COBERTURA", "FECHAMENTOS") vem colado no fim do item anterior:
      // "Largura: 15,50m. FECHAMENTOS". Separa só nesse caso — depois de ponto e em CAIXA ALTA.
      //
      // 🚫 NÃO tentar detectar isso por lookahead com `\b`: em regex JS o \b é ASCII e acha limite
      // DENTRO de "METÁLICA" (entre Á e L), o que partia a palavra em "MET" e "LICA".
      const mSub = t.match(/^(.*?[.;])\s+([A-ZÁÂÃÉÊÍÓÔÕÚÇ]{4,}(?:\s+[A-ZÁÂÃÉÊÍÓÔÕÚÇ]{2,})*)$/);
      if (mSub) { linhas.push(`  • ${mSub[1].trim()}`); linhas.push(`  ${mSub[2].trim()}`); }
      else linhas.push(`  • ${t}`);
    }
  }
  return linhas.join("\n").slice(0, 3000) || null;
...
export async function lerProposta(pdfBytes, nome = null) {
  ...
  const descricao = mDesc ? formatarDescricao(mDesc[1]) : null;
