...
import { tabelaDeEtapas, tabelaDeEntregas } from "./proposta-tabela-cronograma";
...
export async function montarPropostaDocx({ tipo = "PTC", comMontagem = false, selecao = {}, dados = {} }) {
  ...
  if (moldeTituloSecao && moldeLinhaTexto) {
    for (const id of BLOCOS_INSERIDOS) {
      ...
      // ⚠ `tabela` estava declarado em TEXTOS desde a modularização e nunca era desenhado — o bloco
      // saía só com as frases. Agora o quadro entra logo depois das linhas, no estilo do modelo.
      if (t.tabela === "cronograma" && dados.cronograma) {
        const etapas = tabelaDeEtapas(dados.cronograma);
        if (etapas) novos.push(etapas);
        const entregas = tabelaDeEntregas(dados.cronograma);
        if (entregas) {
          novos.push(paragrafoComoOutro(moldeLinhaTexto, "Quadro de entregas:"));
          novos.push(entregas);
        }
      }
      corpo.splice(pos + 1, 0, ...novos);
