...
import { tabelaDePreco, frasesDeFaturamento } from "./proposta-tabela-preco";
...
export async function montarPropostaDocx({ tipo = "PTC", comMontagem = false, selecao = {}, dados = {} }) {
  ...
  if (dados.resultado && querBloco("PLANILHA_PRECO")) {
    ...
    if (pos >= 0) {
      ...
      if (moldeLinha) for (const f of frasesDeFaturamento(dados.resultado)) novos.push(paragrafoComoOutro(moldeLinha, f));
      if (novos.length) {
