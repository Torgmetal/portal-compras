...
export function numeroPtcDe(o) {
  return `PTC-${String(o.numero || 0).padStart(3, "0")}-26-R${String(o.revisao || 0).padStart(2, "0")}`;
}
...
export function dadosProposta(o, now) {
  ...
  // Histórico de revisões (sempre com a Emissão inicial na R00). Se ainda não
  // há histórico gravado (prévia antes do 1º envio), sintetiza a R00 de hoje.
  let revs = Array.isArray(o.revisoes) ? o.revisoes : [];
  if (!revs.length) revs = [{ num: 0, data: now, motivo: "Emissão inicial" }];
  const revisoes = revs.map((r) => ({ rev_num: String(r.num ?? 0).padStart(2, "0"), rev_data: fmtData(r.data), rev_motivo: r.motivo || "" }));
  ...
  return {
    ...
    cq,
    dias: o.pagamentoPrazo || String(o.diasPagamento || 15),
    inclusos: Array.isArray(o.inclusos) && o.inclusos.length ? o.inclusos : DEFAULT_INCLUSOS,
    exclusos: Array.isArray(o.exclusos) && o.exclusos.length ? o.exclusos : DEFAULT_EXCLUSOS,
    servicos,
    valorTotal: fmtBRL(valorTotal),
    valorTotalNum: valorTotal,
    valoresNum: valores, // valor numérico por serviço (chave = CORTE_FURACAO...) p/ gerar OP
  };
}
