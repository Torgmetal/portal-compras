...
const W = PW - ML - MR;
const SIZE = 11;                            // corpo
const LH = Math.round(SIZE * 1.5);         // entrelinha 1,5
...
export async function gerarPropostaPDF(o, now = new Date()) {
  ...
  const para = (texto, { justify = true, indent = INDENT, size = SIZE, cor = DARK, x0 = ML, gapAfter = 6 } = {}) => {
  ...
  const secao = (num, titulo) => { espaco(LH + 18); y -= 8; page.drawText(san(`${num}  ${titulo}`).toUpperCase(), { x: ML, y, size: 12, font: bold, color: NAVY }); y -= 5; page.drawLine({ start: { x: ML, y }, end: { x: ML + W, y }, thickness: 1.2, color: ORANGE }); y -= LH; };
  const sub = (num, titulo) => { espaco(LH + 4); page.drawText(san(`${num}  ${titulo}`), { x: ML, y, size: 11.5, font: bold, color: NAVY }); y -= Math.round(LH * 0.9); };
