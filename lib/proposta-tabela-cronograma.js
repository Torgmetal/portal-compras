...
export function tabelaDeEtapas(cron) {
  ...
  // ⚠⚠ SEM DATA É O PADRÃO. Vitor (05/09/2026): "conseguiria fazer um gantt sem informar data por
  // hora, pois não sabemos se ele vai fechar mesmo ou não? Apenas com a representação da duração".
  // Proposta em aberto com data no papel vira cobrança de data — e a data depende de quando o
  // contrato é assinado. Sem data, as colunas passam a contar DIAS a partir da assinatura.
  const comData = !!cron.resumo?.dataInicio;
  const cab = comData
    ? COL_ETAPA.map((c) => c.r)
    : ["Item", "Etapa", "Duração", "Começa no dia", "Termina no dia"];
  const linhas = [linha(COL_ETAPA.map((c, i) => celula({ w: c.w, texto: cab[i], al: "center", negrito: true })))];
  ...
  cron.fases.forEach((f, i) => {
    linhas.push(linha([
      ...
      celula({ w: COL_ETAPA[3].w, texto: comData ? dataBR(f.dataInicio) : inteiro(corridos(f.inicio)), al: "center" }),
      celula({ w: COL_ETAPA[4].w, texto: comData ? dataBR(f.dataFim) : inteiro(corridos(f.fim)), al: "center" }),
    ]));
  });
  ...
  linhas.push(linha([
    ...
    celula({ w: COL_ETAPA[3].w, texto: comData ? dataBR(cron.resumo.dataInicio) : "0", al: "center", negrito: true }),
    celula({ w: COL_ETAPA[4].w, texto: comData ? dataBR(cron.resumo.dataFim) : inteiro(cron.resumo.totalCorridos), al: "center", negrito: true }),
  ]));
...
export function tabelaDeEntregas(cron) {
  ...
  // sem data, o "evento" não repete o número que a coluna Prazo já dá

  const eventos = [
    ["1", `Primeira carga${comData ? ` (${quando(ent[0])})` : ""}`, inteiro(r.primeiraEntregaCorridos), ent[0]?.kg],
  ];
  if (r.cargas > 1) {
    eventos.push(["2", `Demais cargas — ${r.ritmoEntregas}`, `+${inteiro(r.intervaloEntregasCorridos)}`, r.pesoPorCarga]);
