...
const registro = log("qualidade-doc-backup");
...
export async function backupISODocumentoQualidade(doc, userId) {
  ...
  try {
    ...
    registro.erro("[qualidade-doc-backup] falha SharePoint:", e?.message);
    await prisma.auditLog
...
export async function moverDocumentoParaObsoleto(doc, userId) {
  ...
  try {
    ...
    registro.erro("[qualidade-doc-backup] falha mover Obsoleto:", e?.message);
    await prisma.auditLog
