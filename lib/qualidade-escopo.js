...
// "GERAL" fica de fora: é o registro solto do inspetor, não um ensaio contratado —
// não faz sentido uma obra "não ter" registro geral.
export const TIPOS_ESCOPAVEIS = TIPOS_RELATORIO.filter((t) => t.id !== "GERAL");
...
/** O tipo pode ser usado nesta obra? "GERAL" sempre pode. */
...
/** Os tipos que a tela deve oferecer, já com o GERAL no fim. */
export function tiposDaOP(op) {
  const { tipos } = escopoDaOP(op);
  return TIPOS_RELATORIO.filter((t) => t.id === "GERAL" || tipos.includes(t.id));
}
