...
//
// Estoque conferido explicitamente no Planejamento (11/09) também pode atender a obra.
// Antes dessa confirmação, continua valendo a regra: R DA PRÓPRIA OBRA. Vitor (25/08/2026): "não vamos criar uma maneira de burlarmos e
...
export async function amarracoesDaOp(opNumero) {
  ...
  const trocas = await prisma.trocaRastreabilidade
    .findMany({ where: { opNumero: num }, select: { perfil: true, rUsado: true, trocadoPorNome: true, motivo: true, estoqueConferido: true } })
    .catch(() => []);
  return new Map(
    trocas.filter((t) => t.rUsado && t.perfil)
      .map((t) => [String(t.perfil).trim().toUpperCase(), { r: String(t.rUsado).trim(), por: t.trocadoPorNome || null, motivo: t.motivo || null, estoqueConferido: t.estoqueConferido === true }])
  );
}
