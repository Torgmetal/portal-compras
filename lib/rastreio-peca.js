...
export const ehMateriaPrimaDePeca = (nome) => !["TINTA", "CONSUMIVEL"].includes(classificarMaterial(nome));
...
export async function rastreioDaOp(opNumero, opId) {
  ...
  const [cmrBruto, pecas, ordens, trocas] = await Promise.all([
  ...
  for (const g of grupos.values()) {
    ...
    for (const p of g.pecas) {
      ...
      const cTroca = troca?.rUsado
        ...
        : null;
      if (!cTroca) { pendentes.push(p); continue; }
