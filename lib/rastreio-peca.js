...
// ⚠ ACABAMENTO (telha, cumeeira, calha, silicone) entra pelo mesmo motivo da tinta: cobertura e
// vedação não são origem de peça cortada. (Vitor, 05/09/2026.)
export const ehMateriaPrimaDePeca = (nome) => !["TINTA", "CONSUMIVEL", "ABRASIVO", "ACABAMENTO"].includes(classificarMaterial(nome));
...
export async function rastreioDaOp(opNumero, opId) {
  ...
  const [cmrBruto, pecasBrutas, ordens, trocas] = await Promise.all([
    ...
    prisma.pecaConjunto.findMany({
      ...
      // é a da LPC.
      // ⚠ material/descricao/pesoUnitKg entram por causa do `ehItemComprado` abaixo
      select: { marca: true, perfil: true, qte: true, pesoTotalKg: true, pesoUnitKg: true, tipoPeca: true, fonte: true, corteConcluidoEm: true, dataProducao: true, material: true, descricao: true },
    }),
  ...
  // ⚠⚠ ITEM COMPRADO PRONTO NÃO TEM R DE MATÉRIA-PRIMA. A grade de piso vem do Tekla com um perfil
  // FALSO de chapa ("CH30.00X1292" é o painel, material GS_A4_304 — ver lib/item-comprado) e nunca
  // casa com o CMR: na OP-089 eram 16 "peças sem material" (2.350 kg) que ninguém tinha como
  // resolver, inflando o "sem R" do data book e do carimbo (Vitor, 15/09/2026: "307 projetos sem
  // R"). Mesma regra da fila de corte e da TV: comprado fica fora do fluxo de fábrica.
  const pecas = dedupLpcLe(pecasBrutas).filter((p) => !ehItemComprado(p));
  ...
  // ── CORTE PROVADO PELA ETAPA SEGUINTE ─────────────────────────────────────────────────────
  //
  // ⚠⚠ Vitor (24/09/2026): "temos mais materiais, sem R no data book da OP-102". O ferro redondo
  // (FRØ3/4") de 25 croquis nunca teve o corte apontado no Syneco — e os conjuntos onde ele entra
  // já estavam montados, soldados e jateados. O data book dizia "aguardando corte" sobre peça
  // pintada. Regra da casa (Vitor, 09/09/2026): "se existir apontamento na frente você deve dar
  // baixa nos setores anteriores" — a peça não chega à montagem sem ter sido cortada.
  //
  // Sem data própria de corte, vale o PRIMEIRO dia com apontamento de uma etapa posterior — da
  // própria peça ou de um conjunto que a leva: ela foi cortada ATÉ esse dia. É teto, não a data
  // exata; por isso a linha sai marcada (`corteInferido`) e o FIFO usa só o que tinha chegado até ali.
  const temDataPropria = (p) => !!(cortadoEm.get(p.marca) || p.corteConcluidoEm || p.dataProducao);
  const semData = pecas.filter((p) => !temDataPropria(p)).map((p) => p.marca);
  const corteInferido = semData.length ? await corteProvadoPelaEtapaSeguinte(opId, semData) : new Map();
  const dataDeCorte = (p) => cortadoEm.get(p.marca) || p.corteConcluidoEm || p.dataProducao || corteInferido.get(p.marca) || null;
  const doCorte = (p) => ({ cortadoEm: soDia(dataDeCorte(p)), ...(!temDataPropria(p) && corteInferido.has(p.marca) ? { corteInferido: true } : {}) });
  ...
  for (const p of semMaterial) {
    porMarca.set(p.marca, { situacao: "SEM_MATERIAL", perfil: p.perfil, ...doCorte(p), usadas: [], candidatas: [] });
  ...
  for (const g of grupos.values()) {
    ...
    for (const p of g.pecas) {
      ...
      porMarca.set(p.marca, {
        situacao: "R_DEFINIDO", perfil: p.perfil, ...doCorte(p), balanco,
    ...
    for (const p of cortadas) {
      ...
      const kg = Number(p.pesoTotalKg) || 0;
      const base = { perfil: p.perfil, ...doCorte(p), balanco };
      ...
      // ⚠⚠ POR DIA, no fuso de Brasília. As duas datas são de dia inteiro: o CMR grava o
      // recebimento ao meio-dia UTC (4.153 de 4.153 linhas) e a ordem do Syneco começa à meia-noite
      // de Brasília (03:00 UTC). Comparando o horário, a peça cortada NO DIA em que o aço chegou
      // caía em "cortada antes da entrega" — na OP-102, T102A-P61 e P65 (W200, cortadas e recebidas
      // em 13/08). "Até o dia do corte" é a regra; a hora nunca foi dado.
      const diaCorte = diaBRT(corte);
      const disp = g.entradas.filter((e) => !e.c.dataRecebimento || diaBRT(e.c.dataRecebimento) <= diaCorte);
      if (!disp.length) {
...
/**
 * marca → primeiro dia com apontamento de uma etapa POSTERIOR ao corte (montagem em diante), da
 * própria peça ou de um conjunto que a leva. Só para peças sem data de corte — ver
 * "CORTE PROVADO PELA ETAPA SEGUINTE" em `rastreioDaOp`.
 */
async function corteProvadoPelaEtapaSeguinte(opId, marcas) {
  const rel = await prisma.conjuntoCroqui.findMany({
    where: { croqui: { opId, marca: { in: marcas } } },
    select: { croqui: { select: { marca: true } }, conjunto: { select: { marca: true } } },
  }).catch(() => []);
  const donos = new Map(marcas.map((m) => [m, new Set([m])])); // a própria peça também conta
  for (const r of rel || []) {
    const c = r?.croqui?.marca, j = r?.conjunto?.marca;
    if (c && j && donos.has(c)) donos.get(c).add(j);
  }
  const itens = [...new Set([...donos.values()].flatMap((s) => [...s]))];
  const ordens = await prisma.mesOrdem.findMany({
    where: {
      opId, item: { in: itens }, produzidoUn: { gt: 0 },
      NOT: [{ setor: { contains: "orte", mode: "insensitive" } }, { setor: { contains: "repara", mode: "insensitive" } }],
    },
    select: { item: true, dataInicio: true },
  }).catch(() => []);
  ...
  for (const o of ordens || []) {
    if (!o.item || !o.dataInicio) continue;
    const d = primeiro.get(o.item);
    if (!d || o.dataInicio < d) primeiro.set(o.item, o.dataInicio);
  }
  const out = new Map();
  for (const [marca, set] of donos) {
    let d = null;
    for (const item of set) { const x = primeiro.get(item); if (x && (!d || x < d)) d = x; }
    if (d) out.set(marca, d);
  }
  return out;
}
