...
export async function rastreioDaOp(opNumero, opId) {
  ...
  for (const g of grupos.values()) {
    ...
    for (const p of cortadas) {
      ...
      // ⚠ TROCA REGISTRADA VENCE TUDO, INCLUSIVE A DATA. Alguém separou o material e declarou qual
      // fardo saiu — isso é fato, e fato não se corrige com regra de consumo. A checagem vem antes
      // do FIFO porque é também o que resolve o material de estoque: a entrada de origem pode ter
      // sido recebida DEPOIS do corte desta peça (foi comprada noutra obra), e a regra de "não
      // pode ter saído de aço que não tinha chegado" mandaria a peça pra ESTOQUE de novo.
      ...
      if (cTroca) {
        ...
        porMarca.set(p.marca, {
          situacao: "R_DEFINIDO", ...base, criterio: "troca",
          semCorrida: usadas.some((u) => !u.corrida), saldoEsgotado: false, usadas,
        ...
        if (usadas.some((u) => !u.corrida)) resumo.semCorridaNoCmr++;
