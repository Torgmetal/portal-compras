...
export async function rastreioDaOp(opNumero, opId) {
  ...
  const [cmr, pecas, ordens, trocas] = await Promise.all([
    ...
    prisma.trocaRastreabilidade.findMany({ where: { opNumero: num }, select: { perfil: true, rIndicado: true, rUsado: true, trocadoPorNome: true, createdAt: true } }).catch(() => []),
  ]);
  ...
  for (const g of grupos.values()) {
    ...
    for (const p of cortadas) {
      ...
      let criterio = disp.length === 1 ? "unica" : "fifo";
      // TROCA REGISTRADA vence o FIFO: alguém separou o material e disse qual fardo saiu.
      const troca = trocaPorPerfil.get(String(p.perfil).trim().toUpperCase());
      if (troca?.rUsado) {
        const c = entradaPorR.get(troca.rUsado) || g.entradas.find((e) => e.c.importRef === troca.rUsado)?.c;
        if (c) {
          usadas = [{ ...linhaCmr(c), consumidoKg: Math.round(kg * 10) / 10 }];
          criterio = "troca";
        }
      }
      ...
      porMarca.set(p.marca, {
        ...
        troca: criterio === "troca" ? { rIndicado: troca.rIndicado || null, por: troca.trocadoPorNome || null, em: troca.createdAt ? troca.createdAt.toISOString() : null } : undefined,
      ...
      if (criterio === "troca") resumo.porTroca = (resumo.porTroca || 0) + 1;
...
export async function rastreioDoConjunto(opNumero, opId, marcaConjunto) {
  ...
  return links
    .map((l) => ({ marca: l.croqui.marca, perfil: l.croqui.perfil, qte: l.croqui.qte, qtdNoConjunto: l.qtdNoConjunto, pesoTotalKg: l.croqui.pesoTotalKg, ...(porMarca.get(l.croqui.marca) || { situacao: "SEM_MATERIAL", usadas: [], candidatas: [] }) }))
