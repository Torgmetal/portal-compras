...
export async function custoTransformacaoOP(opId, serie) {
  ...
  for (const m of Object.keys(porMes).sort()) {
    ...
    total += custo;
    kgTotal += porMes[m];
    // share = fatia da OP no total da fábrica no mês (adimensional — NÃO expor
    // kg·setor na tela; o peso mostrado é sempre o da lista de expedição).
    detalhe.push({ mes: m, custo, share: s[m]?.totalKg ? porMes[m] / s[m].totalKg : 0 });
  }
