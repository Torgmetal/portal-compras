...
export const REGRAS_DOCUMENTOS = [
  ...
  {
    tipo: "NR_20",
    nome: "NR-20 (Inflamáveis e Combustíveis)",
    categoria: "TREINAMENTO",
    obrigatorioPara: "SETOR",
    setores: ["pintura"], // todo o setor de Pintura fica exposto a tintas/solventes inflamáveis
    validadeMeses: 24, // reciclagem (classe intermediária) — ajustar se a operação exigir outra
    descricao: "Obrigatório para quem trabalha com inflamáveis/combustíveis — setor de Pintura (tintas e solventes). Reciclagem periódica.",
    referenciaCCT: "NR-20 — Segurança e Saúde no Trabalho com Inflamáveis e Combustíveis",
    dispensavel: true, // o RH pode dispensar por funcionário dentro do setor
  },
  ...
  // ── FORMAÇÃO / QUALIFICAÇÃO (por cargo, conforme Matriz de Competências) ──
  {
    tipo: "FORMACAO",
    nome: "Formação (Técnico ou Superior)",
    categoria: "TREINAMENTO",
    obrigatorioPara: "CARGO",
    // cargos cuja matriz exige "técnico OU superior" — o comprovante (diploma
    // técnico OU superior) satisfaz o item; casa por substring do cargo.
    cargos: ["analista de pcp", "analista de suprimentos", "coordenador de producao", "orcamentista", "projetista"],
    validadeMeses: null, // diploma/comprovante de formação não vence
    descricao: "Comprovante de formação exigida na matriz do cargo — diploma/certificado técnico OU superior.",
    referenciaCCT: "Matriz de Competências (FORM-11) — escolaridade obrigatória",
    dispensavel: true, // o RH pode dispensar por funcionário (exceção justificada)
  },
  {
    tipo: "REGISTRO_CLASSE",
    nome: "Registro em Órgão de Classe",
    categoria: "TREINAMENTO",
    obrigatorioPara: "CARGO",
    cargos: ["orcamentista", "projetista"], // só engenharia/projeto (CREA/CRT/CFT…)
    validadeMeses: 12, // anuidade / renovação anual
    descricao: "Registro ativo no órgão de classe (CREA/CRT/CFT/CRA…) com anuidade em dia.",
    referenciaCCT: "Matriz de Competências (FORM-11) — registro profissional",
    dispensavel: true,
  },
...
/** Retorna regras aplicáveis a um funcionário, dado o nome do setor e do cargo.
 *  Regras "CARGO" casam por substring do cargo normalizado (ex.: "pintor"). */
export function regrasParaFuncionario(nomeSetor, nomeCargo = "") {
  const producao = isSetorProducao(nomeSetor);
  const setor = norm(nomeSetor);
  const cargo = norm(nomeCargo);
  return REGRAS_DOCUMENTOS.filter((r) => {
    ...
    if (r.obrigatorioPara === "MONTAGEM_EXTERNA") return setor.includes("montagem externa");
    if (r.obrigatorioPara === "SETOR") return (r.setores || []).some((s) => setor.includes(norm(s)));
    if (r.obrigatorioPara === "CARGO") return (r.cargos || []).some((c) => cargo.includes(norm(c)));
    return false;
  });
}
