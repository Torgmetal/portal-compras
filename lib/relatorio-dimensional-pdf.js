...
export async function gerarDimensionalPDF({ rel, assinaturas = null, desenhoBytes = null, cliente = null, obra = null }) {
  ...
  for (let gi = 0; gi < total; gi++) {
    ...
    const cols = [
      { t: "Dimensão de Projeto", w: wTab * 0.38 },
      { t: "Tolerâncias", w: wTab * 0.26 },
      { t: "Dimensão Encontrada", w: wTab * 0.36 },
    ];
    let cx = M;
    cols.forEach((c, i) => {
    ...
    for (let i = 0; i < maxLinhas; i++) {
      ...
      if (l) {
        ...
        const proj = [l.descricao, l.projetoMm != null ? `${l.projetoMm}` : null].filter(Boolean).join("  ");
        page.drawText(fit(proj || l.marca || "", font, 7.5, cols[0].w - 8), { x: x + 5, y: ly - 12, size: 7.5, font, color: DARK });
        x += cols[0].w;
    ...
    if (g.desenho && typeof desenhoBytes === "function") {
      const bytes = await desenhoBytes(g.desenho);
      if (bytes) {
        let emb = null;
