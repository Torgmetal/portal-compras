...
export async function gerarDimensionalPDF({ rel, fotos = [], assinaturas = null, desenhoBytes = null, cliente = null, obra = null, refCliente = null }) {
  ...
  const totalPaginas = grupos2.reduce((s, g) => s + g.partes.length, 0);

  let paginaAtual = 0;
  for (let gi = 0; gi < grupos2.length; gi++) {
    const g = grupos2[gi];

    // ⚠ o desenho é baixado e recortado UMA VEZ por grupo — reaproveitado em todas as folhas dele.
    // Repetir por folha multiplicaria a mesma chamada de rede e o mesmo parse de PDF à toa.
    let embGrupo = null;
    if (g.desenho && typeof desenhoBytes === "function") {
      ...
      if (bruto) {
        ...
        // (rota /vetor), para o relatório impresso nunca mostrar um pedaço diferente do que a
      ...
      if (bytesDesenho) { try { [embGrupo] = await pdf.embedPdf(bytesDesenho, [0]); } catch { embGrupo = null; } }
    }
  ...
  for (let sp = 0; sp < g.partes.length; sp++) {
    ...
    {
      // ⚠ `embGrupo` já foi baixado e recortado uma vez, antes do laço de folhas deste grupo (ver
      // acima) — a mesma imagem se repete em cada folha, sem baixar nem recortar de novo.
      const emb = embGrupo;
      if (emb) {
          ...
          for (const l of rel.resultados?.linhasOcultasDesenho || []) {
            ...
            page.drawLine({
              ...
              // ⚠ mais grosso que o traço original, senão sobra fiapo nas bordas. A parte
              // proporcional (× esc) importa mais agora que o desenho é impresso maior: o traço do
              // Tekla cresce junto com a escala, e uma espessura fixa deixaria de cobri-lo.
          ...
          for (const m of layoutCotas(g.cotasG, emb.width, emb.height)) {
            if (!m) continue;
    ...
    page.drawText(san(`${rel.codigo} · OP-${rel.opNumero} · folha ${paginaAtual} de ${totalPaginas}`), { x: M, y: 10, size: 6.2, font, color: GRAY });
