...
export async function gerarDimensionalPDF({ rel, fotos = [], assinaturas = null, desenhoBytes = null, cliente = null, obra = null, refCliente = null }) {
  ...
  for (let gi = 0; gi < total; gi++) {
    ...
    for (const [i, p] of papeis.entries()) {
      ...
      if (a?.assinadoEm) {
        ...
        if (img) {
          const maxL = wA - 22, maxA = hAss - 40;
          const esc = Math.min(maxL / img.width, maxA / img.height);
          page.drawImage(img, { x: x + 8, y: y - 16 - img.height * esc, width: img.width * esc, height: img.height * esc });
        }
        const yNome = comImagem ? y - 62 : y - 26;
        page.drawText(fit(a.nome, bold, 8, wA - 18), { x: x + 8, y: yNome, size: 8, font: bold, color: DARK });
        const d = new Date(a.assinadoEm);
        page.drawText(san(`assinado eletronicamente em ${d.toLocaleDateString("pt-BR")} ${d.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}`), { x: x + 8, y: yNome - 10, size: 5.8, font, color: GREEN });
        if (a.ip) page.drawText(san(`IP ${a.ip}`), { x: x + 8, y: yNome - 18, size: 5.8, font, color: GRAY });
      } else {
        const yLinha = comImagem ? y - 58 : y - 34;
        page.drawLine({ start: { x: x + 8, y: yLinha }, end: { x: x + wA - 10, y: yLinha }, thickness: 0.5, color: LINE });
        page.drawText("Controle de Qualidade", { x: x + 8, y: yLinha - 10, size: 6.4, font, color: GRAY });
      }
    }
    y -= hAss;
  ...
  if (Array.isArray(fotos) && fotos.length) {
    ...
    await paginaDeFotos({ pdf, font, bold, logo }, rel, fotos, {
      cliente, obra, assinaturas, paginas: total,
