    ...
    if (g.desenho && typeof desenhoBytes === "function") {
      ...
      if (bytes) {
        ...
        if (emb) {
          ...
          const ox = M + wTab + 4 + (dispW - emb.width * esc) / 2;
          const oy = yCorpo - hCorpo + 4 + (dispH - emb.height * esc) / 2;
          page.drawPage(emb, { x: ox, y: oy, width: emb.width * esc, height: emb.height * esc });

          // ── AS COTAS MARCADAS, POR CIMA ──────────────────────────────────────────────────────
          //
          // Elas vêm nas coordenadas da própria vista (é lá que a pessoa clicou, grudando no
          // traço), então basta aplicar o mesmo deslocamento e a mesma escala do desenho.
          //
          // ⚠ A LINHA APONTA, NÃO MEDE: o valor da tabela é a especificação, não o comprimento
          // desta linha. É o que dispensa descobrir a escala do PDF.
          for (const c of cotasG) {
            if (c.ax == null || c.bx == null) continue;
            const ax = ox + c.ax * esc, ay = oy + c.ay * esc;
            const bx = ox + c.bx * esc, by = oy + c.by * esc;
            page.drawLine({ start: { x: ax, y: ay }, end: { x: bx, y: by }, thickness: 1.1, color: ORANGE });
            // pé de cota nas duas pontas, para não se confundir com traço do desenho
            const ang = Math.atan2(by - ay, bx - ax) + Math.PI / 2;
            const dx = Math.cos(ang) * 3, dy = Math.sin(ang) * 3;
            for (const [px, py] of [[ax, ay], [bx, by]]) {
              page.drawLine({ start: { x: px - dx, y: py - dy }, end: { x: px + dx, y: py + dy }, thickness: 1.1, color: ORANGE });
            }
            const mx = (ax + bx) / 2, my = (ay + by) / 2;
            page.drawCircle({ x: mx, y: my, size: 5.5, color: ORANGE });
            const lt = String(c.letra || "");
            page.drawText(lt, { x: mx - bold.widthOfTextAtSize(lt, 7) / 2, y: my - 2.5, size: 7, font: bold, color: rgb(1, 1, 1) });
          }
        }
      }
    }
