...
export async function gerarDimensionalPDF({ rel, assinaturas = null, desenhoBytes = null, cliente = null, obra = null, refCliente = null }) {
  ...
  for (let gi = 0; gi < total; gi++) {
    ...
    linhaInfo([["OP:", `OP-${rel.opNumero}`, 0.28], ["OBRA:", obra || "", 0.44], ["REF. CLIENTE:", refCliente || "—", 0.28]]);
    const primeira = g.linhas[0] || {};
    linhaInfo([
      // ⚠ marca E TIPO. Vitor: "aqui trazer o tipo da peça — coluna, viga, tesoura, etc — conforme
      // descrito na lista". Só "T89A1" não diz nada a quem lê o relatório meses depois; "T89A1 ·
      // COLUNA" diz. O tipo é gravado na criação do relatório (ver a nota lá).
      ["IDENTIFICAÇÃO DA PEÇA:", [marca, rel.resultados?.tiposPeca?.[String(marca).toUpperCase()]].filter(Boolean).join(" · "), 0.4],
