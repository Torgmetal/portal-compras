...
export async function gerarDimensionalPDF({ rel, fotos = [], assinaturas = null, desenhoBytes = null, cliente = null, obra = null, refCliente = null }) {
  ...
  if (rascunho) {
    for (const pg of pdf.getPages()) {
      ...
      const txt = "RASCUNHO";
      const tam = Math.min(pw, ph) * 0.17;
      // ⚠ opacidade baixa: precisa ser inconfundível de longe sem impedir a leitura dos números —
      // quem imprime o rascunho está justamente conferindo medida nele.
      pg.drawText(txt, {
        x: pw * 0.16, y: ph * 0.30,
