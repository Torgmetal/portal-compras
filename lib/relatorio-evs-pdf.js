...
export async function paginaDeFotos(doc, rel, fotos, { cliente, obra, assinaturas, paginas = 1, titulo = null }) {
  const lista = Array.isArray(fotos) ? fotos.filter(Boolean) : [];
  if (!lista.length) return false;

  const comImagem = await embutirFotos(doc.pdf, lista.slice(0, 8));
  const f = novaFolha(doc);
  f.cabecalho({
    titulo: titulo || "REGISTRO FOTOGRÁFICO",
    codigo: rel.codigo, revisao: rel.revisao ? `R${String(rel.revisao).padStart(2, "0")}` : null,
    emitidoEm: rel.emitidoEm, folha: paginas + 1, total: paginas + 1,
  });
