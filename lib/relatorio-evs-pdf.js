...
import { abrirDocumento, novaFolha, embutirFotos, M, san, DARK, GRAY, LINE, SOFT, GREEN, RED, ORANGE } from "./relatorio-form-pdf";
...
export async function gerarEVSPDF({ rel, fotos = [], assinaturas = null, cliente = null, obra = null, refCliente = null }) {
  ...
  for (let p = 0; p < paginas; p++) {
    ...
    // ── identificação ──
    f.linhaInfo([["OP:", `OP-${rel.opNumero}`, 0.22], ["CLIENTE:", cliente || "", 0.42], ["OBRA:", obra || "", 0.36]]);
    f.linhaInfo([
      ["DESENHO TORG:", res.desenho || (Array.isArray(rel.marcas) ? rel.marcas.join(", ") : ""), 0.32],
      ["REV.:", res.revisaoDesenho || "-", 0.10],
      ["DESENHO CLIENTE:", res.desenhoCliente || refCliente || "", 0.42],
      ["REV.:", res.revisaoCliente || "-", 0.16],
    ]);
    ...
    f.linhaInfo([
      ...
      ["REF. CLIENTE:", refCliente || "—", 0.350],
    ]);
    // ⚠⚠ A JUNTA SOLDADA CRESCE, NÃO CORTA (23/09/2026). Um EVS pode juntar peças de processos
    // diferentes — no EVS-102-001, GMAW e SMAW —, e o cabeçalho lista as duas EPS: com `linhaInfo`
    // saía "EPS 001/2025, E…", a mesma reticência que apagava peças da relação (ver linhaInfoCresce).
    f.linhaInfoCresce([
      ["METAL BASE:", res.metalBase || "", 0.25],
      ["METAL DE ADIÇÃO:", res.metalAdicao || "", 0.25],
      ["COND. SUPERFICIAIS:", res.condicoes || "", 0.25],
      ["ILUMINAÇÃO:", res.iluminacao || "", 0.25],
    ]);
    f.linhaInfoCresce([
      ["PROCESSO DE SOLDAGEM:", res.processoSolda || "", 0.24],
      ["EPS (WPS):", res.eps || "", 0.27],
      ["RQS (PQR):", res.rqs || "", 0.27],
      ["TIPO DE JUNTA:", res.tipoJunta || "", 0.22],
    ]);
    f.linhaInfo([
      ["TÉCNICA:", res.tecnica || "", 0.24],
      ["PROCEDIMENTO:", res.procedimento || "", 0.32],
      ["CRITÉRIO DE ACEITAÇÃO:", res.criterio || CRITERIO_PADRAO, 0.44],
    ]);
    ...
    for (let i = 0; i < LINHAS_POR_FOLHA; i++) {
      ...
      for (const c of COLS) {
        const larg = W * c.w;
        // ⚠ a EPS da junta é gravada pelo CÓDIGO da pasta ("EPS-RQPS 01"), e o cabeçalho cita o número
        // do documento ("EPS 001/2025"). Na mesma folha, o mesmo documento com dois nomes faz quem
        // confere procurar duas EPS — a junta sai com o número, como o cabeçalho (23/09/2026).
        const v = c.k === "soldador"
          ? soldadorCurto(l.soldador, l.sinete)
          : c.k === "eps" && l.eps
          ? rotuloEps(completarEps({ codigo: String(l.eps) }))
    ...
    if (p === paginas - 1) {
      ...
      await f.blocoAssinaturas(assinaturas, ["Realizado por", "Aprovado por", "Cliente / Fiscalização"], { inspetor: rel.inspetor });
    }
  }
...
export async function paginaDeFotos(doc, rel, fotos, { cliente, obra, assinaturas, paginas = 1, titulo = null, papeis = null }) {
  ...
  for (let i = 0; i < folhas; i++) {
    ...
    await f.blocoAssinaturas(assinaturas, papeis || ["Realizado por", "Aprovado por", "Cliente / Fiscalização"], { inspetor: rel.inspetor });
  }
  return true;
}
