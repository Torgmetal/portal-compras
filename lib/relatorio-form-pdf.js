...
export function novaFolha({ pdf, font, bold, logo }, { paisagem = false } = {}) {
  ...
  const f = {
    ...
    /**
     * Como `linhaInfo`, mas a caixa CRESCE para caber o valor inteiro, em até `maxLinhas`.
     *
     * ⚠⚠ Vitor (04/09/2026): "quando informamos as peças precisa que sejam listadas todas elas,
     * você não pode deixar ela com '…', precisa sair 100%". `linhaInfo` encolhe a fonte até 6pt e
     * depois CORTA com reticência — numa relação de peças isso apaga justamente a informação que o
     * documento existe para dar, e ninguém que lê o relatório sabe quantas peças sumiram.
     *
     * Devolve, por campo, se ainda assim sobrou linha — para o relatório mandar o resto ao anexo em
     * vez de fingir que coube.
     */
    linhaInfoCresce(campos, { alt = 16, maxLinhas = 3, tam = 8 } = {}) {
      const dobras = campos.map(([r, v, frac]) => {
        const dx = bold.widthOfTextAtSize(san(r), 6.4) + 12;
        return f.quebrar(nz(v), bold, tam, W * frac - dx - 8);
      });
      const usadas = Math.min(maxLinhas, Math.max(1, ...dobras.map((d) => d.length)));
      const altura = alt + (usadas - 1) * (tam + 2.5);
      f.caixa(M, f.y, W, altura, SOFT);
      let x = M;
      campos.forEach(([r, , frac], i) => {
        if (i > 0) page.drawLine({ start: { x, y: f.y }, end: { x, y: f.y - altura }, thickness: 0.7, color: LINE });
        page.drawText(san(r), { x: x + 7, y: f.y - 11, size: 6.4, font: bold, color: GRAY });
        const dx = bold.widthOfTextAtSize(san(r), 6.4) + 12;
        dobras[i].slice(0, usadas).forEach((ln, j) => {
          page.drawText(ln, { x: x + dx, y: f.y - 11 - j * (tam + 2.5), size: tam, font: bold, color: DARK });
        });
        x += W * frac;
      });
      f.y -= altura;
      return dobras.map((d) => d.length > usadas);
    },
    ...
    blocoFotos(fotos, { colunas = 2, altura = 168 } = {}) {
      ...
      fotos.forEach((foto, i) => {
        ...
        // ⚠ LEGENDA EM ATÉ 3 LINHAS, não cortada. Vitor (16/09/2026) liberou a legenda escrita pelo
        // inspetor; numa linha só, com `fit`, ela virava "Trinca na solda do fla…". A faixa cresce
        // com o texto e a foto encolhe o equivalente — a célula continua do mesmo tamanho, então
        // a folha não muda de layout.
        const cap = [foto.marca, foto.observacao].filter(Boolean).join(" · ") || `Foto ${i + 1}`;
        const linhasCap = f.quebrar(cap, bold, 6.6, wCel - 14).slice(0, 3);
        if (linhasCap.length === 3 && f.quebrar(cap, bold, 6.6, wCel - 14).length > 3) linhasCap[2] = f.fit(linhasCap[2] + "…", bold, 6.6, wCel - 14);
        const hFaixa = 6 + linhasCap.length * 9;
        page.drawRectangle({ x, y: yTopo - hFaixa, width: wCel, height: hFaixa, color: SOFT });
        page.drawRectangle({ x, y: yTopo - hFaixa, width: wCel, height: hFaixa, borderColor: LINE, borderWidth: 0.7 });
        linhasCap.forEach((ln, li) => page.drawText(ln, { x: x + 7, y: yTopo - 10.5 - li * 9, size: 6.6, font: bold, color: GRAY }));
        if (foto.img) {
          const dispW = wCel - 12, dispH = altura - hFaixa - 12;
          const esc = Math.min(dispW / foto.img.width, dispH / foto.img.height);
          page.drawImage(foto.img, {
            x: x + 6 + (dispW - foto.img.width * esc) / 2,
            y: yTopo - hFaixa - 6 - dispH + (dispH - foto.img.height * esc) / 2,
     ...
     *
     * ⚠⚠ QUEM VAI EM QUAL QUADRO NÃO SE DECIDE AQUI: é `quadrosDeAssinatura`
     * (lib/assinatura-quadros.js), a MESMA regra do dimensional. Com cada desenho casando do seu
     * jeito, o conserto de um não chegou ao outro — e os RPM saíram com o Geraldo em dois quadros
     * (Vitor, 23/09/2026: "alguns estão com o Geraldo duplicando a assinatura").
     *
     * @param {{inspetor?:string|null}} [opts] quem executou a inspeção: se for quem aprova, os dois
     *   quadros viram um, com a assinatura uma vez
     */
    ...
    async blocoAssinaturas(assinaturas, papeis = ["Inspetor Torg Metal", "Fiscalização Torg Metal", "Inspetor Cliente"], { inspetor = null } = {}) {
      ...
      // ⚠ COM IMAGEM, O CAMPO É UM CAMPO DE ASSINATURA DE VERDADE: a assinatura ocupa a largura
      // útil e fica CENTRADA, com nome e data centrados embaixo. Vitor (28/08/2026): "centralize
      // melhor e deixe mais preenchido no campo da assinatura" — encostada à esquerda e pequena,
      // ela parecia um selo perdido, não a assinatura de quem respondeu pelo documento.
      // A altura vem daí: um carimbo de 5cm no papel só cabe inteiro num campo alto.
      const comImagem = (assinaturas || []).some((a) => a?.assinadoEm && a?.imagemUrl);
      const hAss = comImagem ? 118 : 54;
      const hImg = hAss - 52;          // faixa da imagem, entre o rótulo e o nome
      const topo = f.bloco(hAss);
      const wA = W / papeis.length;
      // centraliza o texto no quadro quando o bloco é o das assinaturas desenhadas
      const escrever = (txt, xCol, y, fnt, size, color, larg = wA) => {
        const t = f.fit(txt, fnt, size, larg - 16);
        const px = comImagem ? xCol + (larg - fnt.widthOfTextAtSize(t, size)) / 2 : xCol + 8;
        page.drawText(t, { x: px, y, size, font: fnt, color });
      };

      for (const q of quadrosDeAssinatura(assinaturas, papeis, { inspetor })) {
        const x = M + q.inicio * wA, larg = q.colunas * wA;
        if (q.inicio > 0) page.drawLine({ start: { x, y: topo }, end: { x, y: topo - hAss }, thickness: 0.7, color: LINE });
        page.drawText(`${san(q.rotulo)}:`, { x: x + 8, y: topo - 12, size: 6.8, font: bold, color: GRAY });
        const a = q.assinatura;
        if (a?.assinadoEm) {
          const img = await imagemAssinada(pdf, a);
          if (img) {
            // ocupa o campo: cresce até bater na largura útil ou na faixa de altura, o que vier
            // primeiro — assinatura larga enche a largura, carimbo quadrado enche a altura.
            const esc = Math.min((larg - 14) / img.width, hImg / img.height);
            const lg = img.width * esc, al = img.height * esc;
            page.drawImage(img, { x: x + (larg - lg) / 2, y: topo - 18 - (hImg - al) / 2 - al, width: lg, height: al });
          }
          const yNome = comImagem ? topo - hAss + 22 : topo - 30;
          escrever(a.nome || "", x, yNome, bold, 8.5, DARK, larg);
          escrever(`assinado em ${dataBR(a.assinadoEm)}`, x, yNome - 11, font, 6.4, GREEN, larg);
        } else {
          const yLinha = comImagem ? topo - hAss + 34 : topo - 34;
          page.drawLine({ start: { x: x + 8, y: yLinha }, end: { x: x + larg - 12, y: yLinha }, thickness: 0.6, color: LINE });
          escrever("nome / assinatura / data", x, yLinha - 10, font, 6.2, GRAY, larg);
        }
      }
    },
  };
