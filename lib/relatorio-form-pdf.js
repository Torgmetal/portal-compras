...
export function novaFolha({ pdf, font, bold, logo }, { paisagem = false } = {}) {
  ...
  const f = {
    ...
    async blocoAssinaturas(assinaturas, papeis = ["Inspetor Torg Metal", "Fiscalização Torg Metal", "Inspetor Cliente"]) {
      ...
      // centraliza o texto na coluna quando o bloco é o das assinaturas desenhadas
      const escrever = (txt, xCol, y, fnt, size, color) => {
        const t = f.fit(txt, fnt, size, wA - 16);
        const px = comImagem ? xCol + (wA - fnt.widthOfTextAtSize(t, size)) / 2 : xCol + 8;
        page.drawText(t, { x: px, y, size, font: fnt, color });
      };
      // ⚠⚠ QUEM NÃO CASA PELO NOME DO PAPEL ENTRA NA COLUNA QUE SOBRAR. O convite pede um papel
      // livre ("Torg Metal", "Inspetor", "Cliente") e a folha tem os nomes do modelo ("Inspetor de
      // Qualidade", "Qualidade / Documentação", "Cliente / Fiscalização"): quem escrevia "Torg
      // Metal" não casava com coluna nenhuma e a assinatura simplesmente não saía no documento —
      // a pessoa assinava e o PDF continuava com a linha em branco.
      const restantes = (assinaturas || []).filter((s) => s?.assinadoEm || s?.nome);
      const usados = new Set();
      const casar = (rot) => {
        const alvo = norm(rot);
        const i = restantes.findIndex((s, k) => {
          if (usados.has(k)) return false;
          const st = norm(s.setor);
          return st === alvo || (st && (alvo.includes(st) || st.includes(alvo)));
        });
        if (i >= 0) { usados.add(i); return restantes[i]; }
        return null;
      };
      // primeiro todos os que casam pelo nome; só depois as sobras preenchem as colunas vagas
      const daColuna = papeis.map((rot) => casar(rot));
      papeis.forEach((_, i) => {
        if (daColuna[i]) return;
        const k = restantes.findIndex((_s, j) => !usados.has(j));
        if (k >= 0) { usados.add(k); daColuna[i] = restantes[k]; }
      });
      ...
      for (const [i, p] of papeis.entries()) {
        ...
        if (a?.assinadoEm) {
          ...
          if (img) {
            ...
            page.drawImage(img, { x: x + (wA - lg) / 2, y: topo - 18 - (hImg - al) / 2 - al, width: lg, height: al });
          }
          const yNome = comImagem ? topo - hAss + 22 : topo - 30;
          escrever(a.nome || "", x, yNome, bold, 8.5, DARK);
          escrever(`assinado em ${dataBR(a.assinadoEm)}`, x, yNome - 11, font, 6.4, GREEN);
        } else {
          ...
          escrever("nome / assinatura / data", x, yLinha - 10, font, 6.2, GRAY);
        }
      }
    },
  };
