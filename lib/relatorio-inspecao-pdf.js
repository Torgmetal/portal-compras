...
export async function gerarRelatorioInspecaoPDF({ rel, fotos = [], assinaturas = null, desenhoBytes = null }) {
  ...
  const colW = (W - 14) / 2;
  const imgH = 150;
  // ⚠ LEGENDA EM ATÉ 3 LINHAS (Vitor, 16/09/2026): a altura do par de fotos é a da legenda mais
  // comprida das duas — antes `slice(0, 80)` cortava a observação numa linha.
  const linhasObs = (f) => (f?.observacao ? quebrar(f.observacao, font, 7.5, colW).slice(0, 3) : []);

  for (let i = 0; i < fotos.length; i += 2) {
    const par = [fotos[i], fotos[i + 1]].filter(Boolean);
    const nLinhas = Math.max(0, ...par.map((f) => linhasObs(f).length));
    const blocoH = imgH + 26 + (nLinhas ? 2 + nLinhas * 9.5 : 8);
    espaco(blocoH + 6);
    const yTopo = y;
    for (const [k, f] of [[0, fotos[i]], [1, fotos[i + 1]]]) {
      ...
      if (img) {
        ...
        page.drawText("—", { x: x + 8, y: yTopo - imgH / 2, size: 8, font, color: GRAY });
      }
      ...
      linhasObs(f).forEach((ln, li) => page.drawText(ln, { x, y: yTopo - imgH - 33 - li * 9.5, size: 7.5, font, color: DARK }));
    }
    y = yTopo - blocoH - 8;
  }
  ...
  if (instrumentos.length) {
    ...
    for (const e of instrumentos) {
      ...
      // ⚠ NUNCA "sem validade" NUM DOCUMENTO QUE O CLIENTE LÊ. É a mesma frase que saiu do PDF do
      // data book em 23/08: certificado de material não vence, e num registro de qualidade a frase
      // se lê como instrumento sem controle. Célula vazia é dado que não se aplica; frase é
      // confissão. [[torg_nao_declarar_furo]]
      const val = e.validade ? san(String(e.validade).split("-").reverse().join("/")) : "—";
