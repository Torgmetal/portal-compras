...
export async function gerarRelatorioInspecaoPDF({ rel, fotos = [], assinaturas = null }) {
  ...
  // ── Identificação ──
  const linhas = [
    ["Obra", `OP-${rel.opNumero}`],
    ["Inspeção", TIPO_LABEL[rel.tipo] || rel.tipo],
    ["Inspetor", rel.inspetor || rel.criadoPorNome || "—"],
    ["Registros", `${fotos.length} foto(s)`],
  ];
  ...
  for (const [rot, val] of linhas) {
    ...
    page.drawText(san(val), { x: cx, y: y - 34, size: 10, font: bold, color: DARK });
    cx += W / linhas.length;
  }
  y -= 66;
