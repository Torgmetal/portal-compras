...
export async function vincularNoDataBook(rel, arquivoUrl) {
  ...
  if (documentoId) {
    ...
    // data book até alguém reenviar para assinatura. Enquanto relatório enviado era somente leitura
    // isso não acontecia; desde 22/09/2026 ele é editável (achado da varredura de 23/09/2026).
    const { arquivoUrl: _semArquivo, ...soOsDados } = dados;
    await prisma.documentoQualidade.update({ where: { id: documentoId }, data: arquivoUrl ? dados : soOsDados }).catch(() => {});
  } else {
