...
const baseDoPortal = () => {
  ...
  return process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "https://workspace.torg.com.br";
};
...
export async function anexarRevisaoNoDataBook(rel, revisaoFechada) {
  ...
  const dados = {
    ...
    // ⚠ URL ABSOLUTA: o data book monta o PDF no SERVIDOR e busca o arquivo por fetch — caminho
    // relativo ali não resolve. E aponta para `?revisao=N`, que reconstrói a folha daquela rodada a
    // partir do snapshot, sem precisar guardar um binário por revisão.
    arquivoUrl: `${baseDoPortal()}/api/qualidade/inspecoes/${rel.id}/pdf?revisao=${revisaoFechada.revisao ?? 0}`,
    ativo: true,
  };
  ...
  await prisma.dataBookSecaoDoc.createMany({ data: [{ secaoId: secao.id, documentoId: doc.id }], skipDuplicates: true });
  if (secao.estado !== "ANEXADO") {
    await prisma.dataBookSecao.update({ where: { id: secao.id }, data: { estado: "ANEXADO" } });
  }
  return { vinculado: true, secao: numeroSecao, documentoId: doc.id, rotulo: rot };
}
