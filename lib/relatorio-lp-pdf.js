import "server-only";
import { abrirDocumento, novaFolha, M, san, GRAY, LINE, SOFT, RED, GREEN } from "./relatorio-form-pdf";
...
export async function gerarLPPDF({ rel, fotos = [], assinaturas = null, cliente = null, obra = null, refCliente = null }) {
  ...
  for (let p = 0; p < paginas; p++) {
    ...
    f.linhaInfo([
      ["DESENHO TORG (TORG Drawing):", res.desenho || (Array.isArray(rel.marcas) ? rel.marcas.join(", ") : ""), 0.34],
      ["REV.:", res.revisaoDesenho || "-", 0.12],
      ["DESENHO CLIENTE (Client Drawing):", res.desenhoCliente || "", 0.38],
    ...
    f.linhaInfo([
      ["EPS (WPS):", res.eps || "", 0.25],
      ["RQS (PQR):", res.rqs || "", 0.25],
      ["TIPO DE JUNTA (Joint Type):", res.tipoJunta || "", 0.50],
    ]);
    ...
    if (p === paginas - 1) {
      ...
      await f.blocoAssinaturas(assinaturas, ["Identif. do inspetor / Nível", "Aprovado por", "Cliente / Fiscalização"], { inspetor: rel.inspetor });
    }
  }
