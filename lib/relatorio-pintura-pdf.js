...
export async function gerarPinturaPDF({ rel, fotos = [], assinaturas = null, cliente = null, obra = null, refCliente = null }) {
  ...
  const temFotos = sobras.length > 0;
  ...
  const totalFolhas = 2 + folhasAnexo + folhasFotos;
  ...
  f.linhaInfoCresce([
    ["PEÇAS PINTADAS:", cabeNaLinha ? textoPecas : `${itensAnexo.length} peças — relação completa na folha 3`, 0.72],
  ...
  for (let i = 0; i < areas.length; i++) {
    ...
    const doEnsaio = daArea(areas[i].k);
    // "· 2 de 3": quem lê a folha sabe que existem outras daquele mesmo ensaio, na folha seguinte
    const rot = doEnsaio.length > 1 ? `${areas[i].rot} · 1 de ${doEnsaio.length}` : areas[i].rot;
    f2.rotulo(x + 7, yTopo - 10, rot, 6.4);
    const img = fotosGrade.find((f) => (f.evidencia || null) === areas[i].k)?.img;
    if (img) {
  ...
  for (let pg = 0; pg < folhasAnexo; pg++) {
    ...
    fa.cabecalho({
      ...
      emitidoEm: rel.emitidoEm, folha: 3 + pg, total: totalFolhas,
    });
  ...
  if (sobras.length) {
    ...
    await paginaDeFotos(doc, rel, sobras, { cliente, obra, assinaturas, paginas: 2 + folhasAnexo });
  }
