...
import { mediaRugosidade, CAMPOS_AMBIENTE, leiturasAmbientais, ambientePorEtapa } from "./pintura-campos";
...
export async function gerarPinturaPDF({ rel, fotos = [], assinaturas = null, cliente = null, obra = null, refCliente = null }) {
  ...
  const textoPecas = detalhadas ? detalhadas.join(", ") : res.pecas || marcas.join(", ");
  ...
  const itensAnexo = cabeNaLinha ? [] : (detalhadas || (marcas.length ? marcas : textoPecas.split(/[,;]/).map((t) => t.trim()).filter(Boolean)));
  ...
  f.linhaInfoCresce([
    ["PEÇAS PINTADAS:", cabeNaLinha ? textoPecas : `${itensAnexo.length} marcas — relação completa na folha ${1 + (temFolha2 ? 1 : 0) + 1}`, 0.72],
  ...
  f.linhaInfo([
    ["UMIDADE NO JATO (URA):", res.prepUmidade || "", 0.25],
  ...
  // ⚠⚠ A FOLHA 1 TEM DE FECHAR DENTRO DO PAPEL. Varredura de 23/09/2026: no RIP-106-002 (4
  // instrumentos + 3 assinaturas com imagem) as datas das assinaturas foram desenhadas em y = −2,2
  // pt, FORA da folha, e em outros quatro a 8 pt da borda — na faixa que a impressora não imprime.
  // `bloco()` não confere margem, e o que vem depois das tabelas (laudo, observações, instrumentos
  // e o quadro de assinaturas, que com imagem tem 118 pt) varia de relatório para relatório.
  //
  // Então a ALTURA DA LINHA das duas tabelas é o que cede: calcula-se o que o rodapé vai precisar e
  // as 25 linhas dividem o resto, entre 9,5 e 12,4 pt. Encolher a linha custa pouco — o texto é de
  // 6,6 pt —; cortar a assinatura custa o documento.
  const comImagem = (assinaturas || []).some((a) => a?.assinadoEm && a?.imagemUrl);
  const nInstr = Math.max(1, Array.isArray(rel.equipamentos) ? rel.equipamentos.length : 0);
  const rodape = 24 /* laudo */ + 30 /* observações */ + (26 + nInstr * 9) /* instrumentos */ + (comImagem ? 118 : 54);
  const fixoTabelas = 13 * 4; // dois títulos e dois cabeçalhos de tabela
  const linhasTabelas = APLICACAO.length + 6;
  const hL = Math.max(9.5, Math.min(12.4, (f.y - M - rodape - fixoTabelas) / linhasTabelas));
  // a base do texto acompanha a linha (8,6 pt numa linha de 12,4)
  const baseTxt = hL * 0.69;
  ...
  // a leitura ambiental de cada demão (a dela, ou a herdada do jateamento), calculada uma vez
  const ambienteDaDemao = ["1", "2", "3"].map((n) => leiturasAmbientais(res, n));
  APLICACAO.forEach(([rot, k], i) => {
    ...
    page.drawText(f.fit(rot, font, 6.4, wRot - 12), { x: M + 7, y: ly - baseTxt, size: 6.4, font, color: GRAY });
    for (let d = 0; d < 3; d++) {
      ...
      const v = EH_AMBIENTE.has(k) ? ambienteDaDemao[d][k] : bloco?.[k];
      if (v == null || v === "") continue;
      const x = M + wRot + d * wDem;
      const txt = f.fit(String(v), bold, 6.6, wDem - 8);
      page.drawText(txt, { x: x + (wDem - bold.widthOfTextAtSize(txt, 6.6)) / 2, y: ly - baseTxt, size: 6.6, font: bold, color: DARK });
    }
  });
  ...
  for (let i = 0; i < 6; i++) {
    ...
    page.drawText(ehMedia ? "Média geral" : `Leitura ${i + 1}`, { x: M + 7, y: ly - baseTxt, size: 6.4, font: ehMedia ? bold : font, color: GRAY });
    for (let d = 0; d < 3; d++) {
      ...
      const x = M + wRot + d * wDem;
      const txt = String(v);
      page.drawText(txt, { x: x + (wDem - bold.widthOfTextAtSize(txt, 6.8)) / 2, y: ly - baseTxt, size: 6.8, font: bold, color: DARK });
    }
  }
  ...
  // ⚠⚠ O LAUDO É O RESULTADO DA INSPEÇÃO, e o documento não sabia disso. Vitor (05/09/2026): "o
  // laudo final desse relatório não está como aprovado". Estava — no relatório: o RIP-106-002 tem
  // `resultadoInspecao: APROVADO`, marcado por quem inspecionou. Só que esta folha lia um campo
  // ANTIGO do formulário (`resultados.laudo`), que a tela não preenche mais, e imprimia as duas
  // caixas vazias num relatório aprovado, assinado e enviado ao cliente.
  const laudo = String(res.laudo || rel.resultadoInspecao || "").toUpperCase();
  ["Aprovado", "Reprovado"].forEach((o, i) => {
    const x = M + 90 + i * 90;
    const on = laudo === o.toUpperCase();
  ...
  if (laudo === "REC") {
    page.drawText(san(RESULTADO_LABEL.REC), { x: M + 90 + 2 * 90, y: topoLau - 15, size: 7, font: bold, color: DARK });
  }
  ...
  // ⚠⚠ E A CONFERÊNCIA É POR ETAPA. Julgar só o jateamento deixava passar o caso que mais importa:
  // um fundo aplicado com 92% de umidade num dia em que o jato foi perfeito. O aviso nomeia a
  // etapa — "na 2ª demão" —, senão quem lê procura o erro no lugar errado.
  const foraDoPO = ambientePorEtapa(res).filter((e) => e.avaliacao.avaliado && !e.avaliacao.permitido);
  if (foraDoPO.length) {
    const texto = foraDoPO.map((e) => `na ${e.curto}: ${e.avaliacao.impedimentos.join(" ")}`).join(" | ");
    page.drawText(f.fit(`Fora do PO-05 (5.4) ${texto}`, font, 6, W - 14),
  ...
  await f.blocoAssinaturas(assinaturas, PAPEIS, { inspetor: rel.inspetor });
  ...
  for (let i = 0; i < areasComFoto.length; i++) {
    ...
    // "· 1 de 8": quem lê a folha sabe que existem outras daquele mesmo ensaio, na folha seguinte —
    // e lá elas continuam a MESMA contagem, porque a legenda das duas folhas sai da mesma função.
    const naMoldura = [...naGrade].find((ft) => (ft.evidencia || null) === areasComFoto[i].k);
    const rot = naMoldura?.legenda || areasComFoto[i].rot;
    f2.rotulo(x + 7, yTopo - 10, f2.fit(rot, bold, 6.4, wCel - 14), 6.4);
  ...
  await f2.blocoAssinaturas(assinaturas, PAPEIS, { inspetor: rel.inspetor });
  }
  ...
  for (let pg = 0; pg < folhasAnexo; pg++) {
    ...
    const rot = folhasAnexo > 1
      ? `RELAÇÃO DE PEÇAS PINTADAS (${pg + 1} de ${folhasAnexo}) — ${itensAnexo.length} marcas`
      : `RELAÇÃO DE PEÇAS PINTADAS — ${itensAnexo.length} marcas`;
    ...
    doPg.forEach((m, i) => {
      ...
      const texto = san(m);
      const tamanho = Math.min(7, (wCol - 10) / Math.max(1, font.widthOfTextAtSize(texto, 1)));
      fa.page.drawText(texto, { x: x + 5, y, size: tamanho, font, color: DARK });
    });
    await fa.blocoAssinaturas(assinaturas, PAPEIS, { inspetor: rel.inspetor });
  }
  ...
  if (sobras.length) {
    ...
    await paginaDeFotos(doc, rel, sobras, { cliente, obra, assinaturas, papeis: PAPEIS, paginas: 1 + (temFolha2 ? 1 : 0) + folhasAnexo });
  }
