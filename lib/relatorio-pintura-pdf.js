...
export async function gerarPinturaPDF({ rel, fotos = [], assinaturas = null, cliente = null, obra = null, refCliente = null }) {
  ...
  f.cabecalho({ titulo: "RELATÓRIO DE INSPEÇÃO DE PINTURA", codigo: rel.codigo, revisao: rel.revisao ? `R${String(rel.revisao).padStart(2, "0")}` : null, emitidoEm: rel.emitidoEm, folha: 1, total: 2 });
  ...
  const grau = (rot, opcoes, escolhido) => {
    ...
    opcoes.forEach((o, i) => {
      const x = x0 + i * passo;
      f.marcar(x, topo - 3.5, String(escolhido || "").toUpperCase() === o.toUpperCase(), GREEN);
  ...
  f.blocoAssinaturas(assinaturas, ["Inspetor de Qualidade", "Qualidade / Documentação", "Cliente / Fiscalização"]);

  // ── FOLHA 2: registro fotográfico ────────────────────────────────────────────────────────────
  const f2 = novaFolha(doc);
  f2.cabecalho({ titulo: "RELATÓRIO DE INSPEÇÃO DE PINTURA", codigo: rel.codigo, revisao: rel.revisao ? `R${String(rel.revisao).padStart(2, "0")}` : null, emitidoEm: rel.emitidoEm, folha: 2, total: 2 });
  ...
  f2.blocoAssinaturas(assinaturas, ["Inspetor de Qualidade", "Qualidade / Documentação", "Cliente / Fiscalização"]);
