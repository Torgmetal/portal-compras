...
export async function gerarRelatorioStatusPDF(rel) {
  ...
  const codigo = rel.numero ? `REL-${String(rel.numero).padStart(3, "0")}` : rel.opNumero ? `REL-${rel.opNumero}` : "REL";
  ...
  const desenharFotos = async (fotos) => {
    ...
    for (let i = 0; i < fotos.length; ) {
      ...
      for (const c of cells) {
        ...
        else page.drawText("—", { x: x + 10, y: y - boxH / 2, size: 8, font, color: GRAY });
  ...
  if (rel.refCliente) rowCapa("REFERÊNCIA DO CLIENTE", rel.refCliente);
