...
const COLS = [
  ...
  { t: "Compr.\nInspec. (mm)", k: "inspecionado", w: 6, meio: true },
...
export async function gerarUSPDF({ rel, fotos = [], assinaturas = null, cliente = null, obra = null, refCliente = null }) {
  ...
  const linhas = linhasTabelaUS(rel);
  ...
  for (let p = 0; p < paginas; p++) {
    ...
    f.linhaInfo([
      ...
      ["TIPO DE JUNTA:", res.tipoJunta || res.junta || "", 0.28],
    ...
    for (let k = 0; k < LINHAS_POR_FOLHA; k++) {
      ...
      for (const c of COLS) {
        ...
        if (v) {
          ...
          // ⚠ encolhe até 5,4 pt antes de cortar com reticência: "Sem indicação reprovável" não cabe
          // inteiro a 6,2 pt na coluna de observação
          // (a folga de 3% é do arredondamento: no corpo exato, o `fit` ainda cortava a última letra)
          const tam = Math.max(5.4, Math.min(6.2, (0.97 * 6.2 * (larg - 4)) / Math.max(1, fnt.widthOfTextAtSize(san(v), 6.2))));
          const txt = f.fit(v, fnt, tam, larg - 4);
          const px = c.meio ? cx + (larg - fnt.widthOfTextAtSize(txt, tam)) / 2 : cx + 3;
    ...
    if (p === paginas - 1) {
      ...
      await f.blocoAssinaturas(assinaturas, ["Inspetor", "Controle de Qualidade", "Cliente"], { inspetor: rel.inspetor });
    }
  }
