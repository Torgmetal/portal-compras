...
// ─── QUANTAS VEZES UM TOKEN PODE ESCREVER ────────────────────────────────────
//
// ⚠⚠ AS TRAVAS DE AVISO NÃO PROTEGEM O BANCO (achado do Codex, 18/09/2026). Elas limitam o
// E-MAIL; a ESCRITA continuava sem teto. Quem tem um token pode alternar data e motivo à vontade:
// cada chamada abre transação, grava linha de auditoria e gera uma proposta nova — e ainda faz a
// tela de Compras colher 409 atrás de 409, porque a proposta que ela leu muda a cada vez.
//
// ⚠ Os tetos são FOLGADOS de propósito. Um fornecedor de verdade responde uma vez e, no pior
// caso, corrige duas ou três; 10 por hora e 30 por dia nunca encostam nele. Isto é anti-abuso,
// não controle de fluxo.
export const ESCRITAS_MAX_HORA = 10;
export const ESCRITAS_MAX_DIA = 30;

/** As ações que contam como escrita do fornecedor. */
const ACOES_ESCRITA = ["FORNECEDOR_PROPOS_PRAZO", "FORNECEDOR_DECLAROU_ENTREGA", "FORNECEDOR_ATUALIZOU_PRAZO"];

/**
 * Este token já escreveu demais?
 *
 * ⚠⚠ FALHA ABERTO, ao contrário de `podeAvisar`. Lá, não conseguir ler o limite significa no
 * máximo um aviso atrasado; aqui significaria **o fornecedor não conseguir responder** — e a
 * resposta dele é o propósito inteiro do link. Se a leitura falha, a gravação logo abaixo
 * provavelmente falha também, então fechar não protegeria nada e só quebraria quem é legítimo.
 */
export async function podeEscrever(prisma, pedidoId) {
  const desde = new Date(Date.now() - 24 * 60 * 60_000);
  const escritas = await prisma.auditLog.findMany({
    where: { action: { in: ACOES_ESCRITA }, entityId: pedidoId, createdAt: { gte: desde } },
    select: { createdAt: true },
  }).catch((e) => {
    registro.erro("não consegui ler o histórico de escritas:", e?.message);
    return null;
  });
  if (escritas === null) return { ok: true };
  if (escritas.length >= ESCRITAS_MAX_DIA) {
    return { ok: false, motivo: "Muitas respostas para este pedido hoje. Tente novamente amanhã ou fale com compras@torg.com.br." };
  }
  const umaHora = Date.now() - 60 * 60_000;
  const naHora = escritas.filter((e) => new Date(e.createdAt).getTime() >= umaHora).length;
  if (naHora >= ESCRITAS_MAX_HORA) {
    return { ok: false, motivo: "Muitas respostas em pouco tempo. Aguarde alguns minutos e tente novamente." };
  }
  return { ok: true };
}
...
export function textoDoAviso(pedido, resposta) {
  ...
  // ⚠⚠ "PROPÔS", E A AÇÃO É APROVAR — não "já está com a data nova". Desde 18/09/2026 a data do
  // fornecedor NÃO vale sozinha: ela fica pendente até Compras aprovar. Um aviso dizendo que a
  // tela já mudou faria ninguém clicar em nada, e a proposta morreria esperando.
  return {
    titulo: `${quem} propôs nova previsão — ${ref}`,
    linha: `${quem} propôs uma nova previsão de entrega para o ${ref}: ${fmt(resposta.prazoNovo)}`
      + (resposta.prazoAnterior ? ` (a data combinada é ${fmt(resposta.prazoAnterior)})` : "") + ".",
    acao: "A data só passa a valer depois que Compras aprovar, na tela Prazos das RMs."
      + " Até lá o pedido continua na situação de hoje e segue sendo cobrado.",
  };
}
...
export async function avisarResposta(prisma, pedido, resposta) {
  ...
  await criarNotificacao({
    ...
    modulos: ["COMPRAS"],
    dados: {
      pedidoId: pedido.id, entregue: !!resposta.entregue, nfNumero: resposta.nfNumero || null,
      proposto: !!resposta.proposto,
    },
