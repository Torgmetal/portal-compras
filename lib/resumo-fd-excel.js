...
export async function montarResumoFDWorkbook({ op, fornecedores = [], totalGeral = 0 }) {
  ...
  for (const f of fornecedores) {
    ...
    banda(ws, row, headers.length, nome, { fill: NAVY, cor: "FFFFFF", size: 12, height: 26 }); row++;
    ...
    // Respiro entre blocos de fornecedor — separa um "pedido" do outro pra leitura
  ...
  // Sem auto-filtro: com vários blocos de fornecedor ele só cairia no último e
  // confundiria a leitura. Esta planilha é um resumo pra ler, não pra filtrar.
  ws._torgFiltro = null;
