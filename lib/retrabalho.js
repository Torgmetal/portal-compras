...
import { apontamentosDaRnc } from "./rnc-apontamentos";
...
export async function retrabalhoDoAno(prisma, ano) {
  ...
  const rncs = await prisma.naoConformidade.findMany({
    ...
    select: {
      ...
      apontamentos: true, tipo: true, pertinente: true, disposicao: true, respostaCliente: true,
    },
    orderBy: { data: "asc" },
  });
  ...
  /* ⚠⚠ CADA APONTAMENTO DA RNC SOMA NO SETOR QUE O GEROU. Vitor (09/09/2026), aprovando os
     apontamentos: "deve ser somando cada tipo de apontamento". Uma RNC como a 012/26 tem três
     apontamentos que podem ter donos diferentes — jogar o peso todo no `setorRetrabalho` da RNC
     diria que a Engenharia gerou um retrabalho que foi da Solda.
     ⚠ RNC antiga cai aqui igual: `apontamentosDaRnc` devolve um item só, derivado dos campos de
     sempre, e o resultado é idêntico ao de antes — nenhum número de 2025/2026 se mexe.
     ⚠ O peso das PEÇAS ainda manda quando existe, e ele é da RNC inteira: nesse caso não há como
     repartir por apontamento, então o peso vai para o setor dominante, como antes. */
  for (const r of rncs) {
    const base = { fonte: "RNC", data: r.data, marca: r.desenhoProjetoMarca, opNumero: r.opNumero, categoria: "RETRABALHO" };
    const kgPecas = pesoDasPecas(r.pecas);
    if (kgPecas != null) {
      somar({ ...base, id: r.id, setor: setorDaRnc(r), kg: kgPecas, descricao: r.descricao, numeroRnc: `${r.numero}/${r.ano}` });
      continue;
    }
    const itens = apontamentosDaRnc(r).filter((a) => a.procedente && a.decisao === "RETRABALHAR");
    if (!itens.length) { somar({ ...base, id: r.id, setor: setorDaRnc(r), kg: r.pesoRetrabalhoKg ?? 0, descricao: r.descricao, numeroRnc: `${r.numero}/${r.ano}` }); continue; }
    for (const a of itens) {
      somar({
        ...base, id: `${r.id}:${a.id}`,
        marca: a.referencia || r.desenhoProjetoMarca,
        setor: setorDaRnc({ setorRetrabalho: a.setor, processoArea: r.processoArea }),
        kg: a.pesoKg ?? 0, descricao: a.descricao || r.descricao,
        numeroRnc: `${r.numero}/${r.ano}`,
      });
    }
  }
