...
export async function abrirRNCdeReprovacao(rel, { userId = null, elaborador = null } = {}) {
  ...
  // ⚠ NUMERAÇÃO COM RETENTATIVA. Ler o último número e somar 1 é uma corrida: dois inspetores
  // reprovando no mesmo instante leem o mesmo "15" e tentam gravar "16". O índice único no banco
  // barra o segundo, e aqui ele tenta o próximo em vez de estourar na cara de quem está medindo.
  const criar = async (tentativa = 0) => {
    ...
    try {
      return await montar((ultima?.numero || 0) + 1);
    } catch (e) {
