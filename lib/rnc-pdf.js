...
export async function gerarRncPDF(a, plano) {
  ...
  if (a.tipo === "CLIENTE") {
    const proc = a.pertinente !== false;
    const rot = proc ? "PROCEDENTE" : "IMPROCEDENTE";
    const lw = wid(rot, bold, 9) + 16;
    page.drawRectangle({ x: A4[0] - M - lw, y: A4[1] - 95, width: lw, height: 15, color: proc ? rgb(0.72, 0.22, 0.22) : rgb(0.32, 0.38, 0.46) });
  ...
  if (a.setorRetrabalho) ident.push(["Setor que gerou o retrabalho", SETOR_RETRABALHO[a.setorRetrabalho]?.nome || a.setorRetrabalho]);
  ...
  // fica neutro.
  const justif = (a.respostaCliente || "").trim();
  if (justif) {
    secao(a.tipo === "CLIENTE"
      ? (a.pertinente !== false ? "JUSTIFICATIVA DA PROCEDÊNCIA" : "JUSTIFICATIVA DA IMPROCEDÊNCIA")
      : "JUSTIFICATIVA");
    paragrafo(justif, M, W, { size: 9, lh: 12.5 });
    y -= 4;
  }
  ...
  if (a.pesoRetrabalhoKg != null) disp.push(["Peso de retrabalho", `${Number(a.pesoRetrabalhoKg).toLocaleString("pt-BR", { maximumFractionDigits: 2 })} kg`]);
