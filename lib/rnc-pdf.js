...
export async function gerarRncPDF(a, plano) {
  ...
  txt("FORM 20", A4[0] - M - wid("FORM 20", font, 8), A4[1] - 92, { size: 8, color: rgb(0.72, 0.79, 0.88) });
  y = A4[1] - 128;
  ...
  /* Causa raiz */
  if (a.causas || (Array.isArray(a.cincoPorques) && a.cincoPorques.length)) {
    ...
    const pqs = (a.cincoPorques || []).filter((p) => (p.resposta || "").trim());
    if (pqs.length) {
      txt("Ferramenta dos 5 porquês", M, y - 8, { size: 7.5, color: GRAY }); y -= 14;
      pqs.forEach((p, i) => { espaco(12); txt(`${i + 1}º`, M, y - 8, { f: bold, size: 8.5, color: NAVY }); for (const ln of wrap(p.resposta, font, 9, W - 20)) { txt(ln, M + 20, y - 8, { size: 9 }); y -= 11; } y -= 2; });
    }
    y -= 4;
  }
  ...
  pages.forEach((p, i) => {
    ...
    p.drawText(san(`${numRNC(a.numero, a.ano)} · Torg Metal · FORM 20 · relatório de não conformidade · documento controlado (ISO 9001)`), { x: M, y: 19, size: 7, font, color: GRAY });
