 ...
 */
export async function gerarRomaneioForm22({ op, romaneio, itens, historico, rotuloColG }) {
  ...
  // "ajustar em 1 página" (largura A–J e altura numa folha só, com as linhas ocultas).
  ...
  if (Array.isArray(historico) && historico.length) {
    ...
    wsH.addRow(["HISTÓRICO DE REVISÕES"]);
    wsH.addRow([`Romaneio ${romaneio?.numero || ""}`, `OP ${op?.numero || ""}`, op?.obra || "", ""]);
    wsH.addRow([]);
    const head = wsH.addRow(["Revisão", "Emitido em", "O que mudou", "Por"]);
    head.font = { bold: true };
    for (const h of historico) {
      wsH.addRow([`R${String(h.revisao ?? 0).padStart(2, "0")}`, fmtDT(h.emitidoEm), h.mudanca || "", h.porQuem || ""]);
    }
    wsH.getRow(1).font = { bold: true, size: 12 };
