...
export async function gerarRomaneioTerceiroForm22(rom) {
  ...
  const paramsOp = {
    ...
    obra: op?.obra || (rom.servico ? `Terceiro · ${rom.servico}` : "Material em terceiro"),
  ...
  const romaneio = {
    numero: `RT-${String(rom.numero).padStart(3, "0")}`,
  ...
  const itens = (Array.isArray(rom.itens) ? rom.itens : []).map((it) => ({
    marca: it.marca,
    descricao: it.descricao || "",
    qtd: it.qte ?? null,
    pesoKg: it.pesoTotal != null ? it.pesoTotal : (Number(it.qte) || 0) * (Number(it.pesoUn) || 0),
  }));
