...
export async function gerarRomaneioTerceiroForm22(rom, opts = {}) {
  ...
  const itens = material
    // MATERIAL: descrição e CÓDIGO saem do cadastro do OMIE (casado do perfil na RM da OP) — o
    // terceiro e o fiscal precisam do item oficial, não do perfil interno. Sem match, cai no
    // perfil/material de sempre. (Vitor 18/08.)
    ? (Array.isArray(rom.materiais) ? rom.materiais : []).map((m) => ({
        marca: m.perfil,
        codigo: m.codigoOmie || null,
        descricao: [m.descricaoOmie || m.descricao, m.unidade].filter(Boolean).join(" · ") || m.perfil || "",
        qtd: m.qtd ?? null,
  ...
  // O modelo é o do FORM 22, mas o documento emitido é o FORM 26 (Romaneio Terceiros) — e é o
  // documento, não o nome do arquivo, que a ISO 9001 §7.5.2 manda identificar.
  return gerarRomaneioForm22({
    op: paramsOp, romaneio, itens,
    rotuloColG: material ? "Cód. Omie" : undefined,
    carimboForm: refFORM(26, { formato: "planilha" }),
  });
}
