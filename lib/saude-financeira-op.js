import "server-only";
import { margemOrcadaLqc } from "./lqc-op-margem";
...
export async function saudeFinanceiraOP(opId) {
  const op = await prisma.oP.findUnique({
    ...
    select: {
      ...
      receitas: { select: { aditivoId: true, valor: true, icmsPct: true, ipiPct: true, pisPct: true, cofinsPct: true, issPct: true, irrfPct: true, csllPct: true } },
  ...
  const margemLqc = margemOrcadaLqc(op);

  // ── MARGEM: três cenários ────────────────────────────────────────────────────────────────
  // Só a do estudo é "previsão"; as outras duas medem o quanto a obra se afastou dela.
  const margem = margemLqc ? { estudo: null, estudoPct: null, prevista: null, previstaPct: null, corrente: null, correntePct: null } : {
  ...
  if (!estudo && !margemLqc) alertas.push({ nivel: "info", texto: "OP sem planilha de estudo vinculada — não há custo do Comercial pra comparar." });
