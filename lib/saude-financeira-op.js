...
function realizadoDaExpedicao(listas) {
  ...
  let areaM2 = 0, pesoKg = 0, marcas = 0, marcasComArea = 0;
  for (const l of listas) {
    for (const m of Array.isArray(l.marcasJson) ? l.marcasJson : []) {
      ...
      if (a > 0) { marcasComArea++; areaM2 += a; }
      pesoKg += Number(m.pesoTotal) || 0;
    }
  }
  return {
    areaM2, pesoKg, marcas, marcasComArea,
    coberturaPct: marcas > 0 ? (marcasComArea / marcas) * 100 : null,
 ...
 */
function confronto({ rotulo, unidade, itens, estimadoEstudo, real, verba, realizado, cobertura }) {
  const doItem = itens.find((i) => String(i.unidade || "").toUpperCase().replace("2", "²") === unidade && Number(i.qtdContratada) > 0);
  const estimado = doItem ? Number(doItem.qtdContratada) : (Number(estimadoEstudo) || 0);
  const fonte = doItem ? "item do contrato" : estimadoEstudo ? "planilha de estudo" : null;

  // unidade divergente é informação, não silêncio: alguém lançou tinta em kg e ninguém viu
  const unidadeErrada = !doItem && itens.length > 0 && !estimadoEstudo
    ? [...new Set(itens.map((i) => i.unidade).filter(Boolean))].join(", ")
    : null;

  if (!estimado || !real) {
    return unidadeErrada ? { rotulo, unidade, semComparacao: true, unidadeErrada } : null;
  }
...
export async function saudeFinanceiraOP(opId) {
  ...
  const cobertura = { marcas: daLista.marcas, comArea: daLista.marcasComArea, pct: daLista.coberturaPct, listas: daLista.listas };
  ...
  const confrontos = [
    confronto({
      ...
      real: daLista.areaM2, verba: fTinta.estimado, realizado: fTinta.realizado, cobertura,
    }),
    confronto({
      rotulo: "Peso da estrutura", unidade: "KG",
      itens: itensPorCat.MATERIA_PRIMA || [],
      estimadoEstudo: op.estudoDados?.aco?.pesoKg || null,
      real: daLista.pesoKg, verba: fAco.estimado, realizado: fAco.realizado,
      // peso vem de TODA marca da lista, não só das que têm área
      cobertura: { ...cobertura, comArea: daLista.marcas, pct: daLista.marcas > 0 ? 100 : null },
    }),
  ].filter(Boolean);
  ...
  for (const c of confrontos) {
    ...
    if (c.desvioPct != null && Math.abs(c.desvioPct) >= 10) {
      ...
      const nota = !parcial ? ""
        : c.desvioPct > 0
          ? ` A lista tem ${c.cobertura.comArea} de ${c.cobertura.marcas} marcas medidas — o desvio real é maior.`
          : ` A lista tem só ${c.cobertura.comArea} de ${c.cobertura.marcas} marcas medidas: parte da diferença pode ser medição que falta, não orçamento sobrando.`;
  ...
  return {
    ...
    margem,
    confrontos,
    expedicao: daLista.marcas > 0 ? { ...cobertura, areaM2: daLista.areaM2, pesoKg: daLista.pesoKg } : null,
    alertas,
  };
}
