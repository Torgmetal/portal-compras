...
/**
 * Guard do portal de autoatendimento (/meu-rh). Exige um usuário do tipo
 * FUNCIONARIO com vínculo a um registro de RH. Retorna { ...user, funcionarioId }.
 */
export async function requireFuncionario() {
  const user = await requireUser();
  if (user.tipo !== "FUNCIONARIO" || !user.funcionarioId) {
    throw new Error("Forbidden");
  }
  return user;
}
...
/**
 * Gate do painel /admin e das rotas /api/admin — allowlist por e-mail, e aqui nem ADMIN burla.
 * Vitor (05/09/2026): Caio, Guilherme e Fabrine seguem com acesso full ao portal, sem o painel de
 * administração nem os alertas de cron. Ver lib/admin-portal.js.
 */
export async function requireAdminDoPortal() {
  const user = await requireUser();
  if (!ehAdminDoPortal(user.email)) throw new Error("Forbidden");
  return user;
}
