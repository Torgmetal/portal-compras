...
// ⚠ "Preparação" faltava e o Syneco tem 18.484 ordens nela — sem isso, ela caía como desconhecida
// (índice -1) e ordenava ANTES do corte. É a etapa entre Corte e Montagem.
export const FLUXO_VISUAL = [
  "Corte", "Preparação", "Dobra", "Montagem", "Solda",
...
// ⚠⚠ SETOR SAI DO BANCO EM ORDEM ARBITRÁRIA. Uma lista crua vira "Acabamento, Jato, Corte", que
// faz parecer que a peça foi ao acabamento antes de cortar — Vitor (25/08/2026): "no status de por
// onde passou o que seria? pois está confuso". Ordenar pelo fluxo é o que devolve sentido à linha.
const IDX_FLUXO = new Map(FLUXO_VISUAL.map((s, i) => [normSetor(s), i]));

/** Ordena setores pelo fluxo produtivo. Desconhecido vai para o fim, não para o começo. */
export function ordenarSetores(lista) {
  return [...new Set((lista || []).filter(Boolean))].sort(
    (a, b) => (IDX_FLUXO.get(normSetor(a)) ?? 99) - (IDX_FLUXO.get(normSetor(b)) ?? 99)
  );
}

/** O setor mais adiantado da lista — "até onde a peça chegou". */
export function setorMaisAvancado(lista) {
  const ord = ordenarSetores(lista);
