...
export async function listarPasta(get, driveId, caminho) {
  ...
  let url = `${GRAPH}/drives/${driveId}/root:${encodeURI(caminho)}:/children`
    + "?$select=id,name,file,folder,size,webUrl,lastModifiedDateTime&$top=999";
  while (url) {
 ...
 * @returns {Promise<{arquivos: Array<{id,name,size,modificadoEm,webUrl,caminho,pasta,relativo}>, pastas: number}|null>}
...
/**
 * Os arquivos com EXATAMENTE este nome sob uma pasta.
 *
 * ⚠⚠ ERA UM `search` DO GRAPH, que devolve HTTP 500 neste drive desde 22–23/09/2026. Ela é o 4º
 * degrau da escada do Data Book: o certificado que o Almoxarifado moveu de subpasta só é
 * reencontrado por aqui (ver `lib/databook-arquivo.js`). Com a busca de pé o degrau existia; com
 * ela morta, o livro voltava a sair sem o certificado, e a pendência ficava num canto da tela.
 *
 * ⚠ A raiz é estreita de propósito (o 2º nível do caminho antigo, ex. `/Almoxarifado/01.
 * Rastreabilidade`) — larga o bastante para achar o arquivo que mudou de subpasta, estreita o
 * bastante para não pegar homônimo de outro setor. Medido em 26/09: 32 pastas, 1.925 PDF, 3,6 s.
 *
...
export async function procurarArquivoPorNome(driveId, caminhoRaiz, nome) {
  ...
  const raiz = String(caminhoRaiz || "/").replace(/\/+$/, "") || "/";
  const r = await varrerPasta(driveId, raiz, { arquivo: (n) => String(n).trim().toLowerCase() === alvo });
  // ⚠ Raiz inexistente vira lista vazia: quem chama já trata "não achei" como não achei, e um
  // erro aqui derrubaria a geração do livro inteiro por causa do último degrau da escada.
  return (r ? r.arquivos : []).map((v) => ({ id: v.id, name: v.name, webUrl: v.webUrl || null, parentPath: v.caminho }));
}
