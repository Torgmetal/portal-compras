...
async function moverAtuaisParaObsoleto(driveId, folderPath, token, filtro = null) {
  try {
    const clean = "/" + folderPath.replace(/^\/+/, "").replace(/\/+$/, "");
    ...
    // só arquivos (a subpasta Obsoleto fica de fora); filtro opcional por nome (ex.: só o mesmo romaneio)
    const arquivos = (data.value || []).filter((x) => x.file && (!filtro || filtro(x.name)));
...
export async function salvarListaNoServidor({ tipo, opNumero, fileNome, buffer }) {
  ...
  // (histórico). Vale pra revisão nova E pra reenvio da mesma revisão.
  // LPC é por FASE: obsoleta só os arquivos da MESMA fase (T83F não mexe em T83D).
  // LE segue unificada (obsoleta todos).
  let filtroObsoleto = null;
  if (tipo === "LPC") {
    const fase = String(fileNome || "").toUpperCase().match(/T\d+[A-Z]*/)?.[0];
    if (fase) filtroObsoleto = (name) => String(name).toUpperCase().match(/T\d+[A-Z]*/)?.[0] === fase;
  }
  await moverAtuaisParaObsoleto(driveId, folderPath, token, filtroObsoleto);
 ...
 * @returns {Promise<{ webUrl:string, nome:string, pastaOp:string, caminho:string }>}
 */
export async function salvarRomaneioNoServidor({ opNumero, fileNome, buffer, moverPrefixo }) {
  ...
  // Revisão: move o(s) arquivo(s) do MESMO romaneio (mesmo prefixo) pra Obsoleto
  // antes de salvar a nova versão. Outros romaneios (R2, R3…) ficam intactos.
  if (moverPrefixo) {
    const pref = moverPrefixo.replace(/[\\/:*?"<>|]/g, "-");
    await moverAtuaisParaObsoleto(driveId, folderPath, token, (nome) => nome.startsWith(pref));
  }
