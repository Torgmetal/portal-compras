...
export async function salvarListaNoServidor({ tipo, opNumero, fileNome, buffer }) {
  ...
  const token = await getAccessToken();
  // replace: subir de novo a mesma revisão (mesmo nome de arquivo) SUBSTITUI o
  // anterior em vez de criar "arquivo (1)". Revisão nova = nome novo (R02) →
  // convive com a R01 (histórico de revisões na pasta).
