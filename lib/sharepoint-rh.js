...
const registro = log("sharepoint-rh");
...
export async function syncContratacao(func) {
  try {
    ...
    registro.erro("[SharePoint RH] Erro sync contratação:", e.message);
    return { success: false, error: e.message };
  }
}

// Versão em lote da contratação: abre a planilha UMA vez e anexa todas as linhas
// (usado pela importação em massa por Excel — evita 1 download/upload por funcionário).
export async function syncContratacaoLote(funcs) {
  if (!funcs?.length) return { success: true, total: 0 };
  try {
    await withWorkbook((wb) => {
      const sheet = wb.getWorksheet(SHEET_BASE);
      if (!sheet) throw new Error("Sheet BASE FUNCIONÁRIOS não encontrada");
      ...
      for (const func of funcs) {
        ...
        appendHistorico(wb, {
          ...
          cargoNovo: func.cargo?.nome || "",
          salarioNovo: func.salario,
          motivo: "Importação em massa via Portal",
        });
      }
    });
    return { success: true, total: funcs.length };
  } catch (e) {
    registro.erro("[SharePoint RH] Erro sync lote:", e.message);
    return { success: false, error: e.message };
  }
}
...
export async function syncAjuste(func, antes, depois, motivo) {
  try {
    await withWorkbook((wb) => {
      ...
      if (!rowNum) {
        registro.aviso("[SharePoint RH] Funcionário não encontrado na planilha:", func.nome);
        return;
      }
    ...
    registro.erro("[SharePoint RH] Erro sync ajuste:", e.message);
    return { success: false, error: e.message };
  }
}
...
export async function syncDesligamento(func, dados) {
  try {
    ...
    registro.erro("[SharePoint RH] Erro sync desligamento:", e.message);
    return { success: false, error: e.message };
  }
}

// Propaga a renomeação de um cargo para a planilha BASE FUNCIONÁRIOS: o cargo é
// gravado como TEXTO na linha de cada funcionário (célula 3), então renomear o
// cargo no portal não atualiza sozinho a planilha. Abre o arquivo UMA vez, casa
// cada funcionário por matrícula (col 1) e, na falta, por nome (col 2), e grava
// o novo nome. `funcionarios` = [{ nome, matricula }] (ativos que usam o cargo).
export async function syncCargoRename({ nomeAntes, nomeNovo, funcionarios }) {
  if (!nomeNovo || nomeNovo === nomeAntes) return { success: true, atualizados: 0 };
  const lista = funcionarios || [];
  if (lista.length === 0) return { success: true, atualizados: 0 };
  try {
    let atualizados = 0;
    await withWorkbook((wb) => {
      const sheet = wb.getWorksheet(SHEET_BASE);
      ...
      // Índices numa única passada (mantém a 1ª ocorrência, como findRowByNome)
      const porMatricula = new Map();
      const porNome = new Map();
      for (let r = 2; r <= sheet.rowCount; r++) {
        const mat = (sheet.getRow(r).getCell(1).value ?? "").toString().trim();
        const nome = (sheet.getRow(r).getCell(2).value ?? "").toString().toUpperCase().trim();
        if (mat && !porMatricula.has(mat)) porMatricula.set(mat, r);
        if (nome && !porNome.has(nome)) porNome.set(nome, r);
      }

      for (const f of lista) {
        const mat = (f.matricula ?? "").toString().trim();
        const nome = (f.nome ?? "").toString().toUpperCase().trim();
        const rowNum = (mat && porMatricula.get(mat)) || porNome.get(nome);
        if (!rowNum) continue;
        const row = sheet.getRow(rowNum);
        row.getCell(3).value = nomeNovo;
        row.commit();
        atualizados++;
      }
      ...
      if (atualizados > 0) {
        appendHistorico(wb, {
          tipo: "Ajuste",
          nome: `${atualizados} funcionário(s)`,
          cargoAnterior: nomeAntes || "",
          cargoNovo: nomeNovo,
          motivo: "Correção de nome de cargo via Portal",
        });
      }
    });
    return { success: true, atualizados };
  } catch (e) {
    registro.erro("[SharePoint RH] Erro sync renomeação de cargo:", e.message);
    return { success: false, error: e.message };
  }
}
