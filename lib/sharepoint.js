...
export async function baixarCmrAtual(ano = new Date().getFullYear()) {
  ...
  const res = await fetch(`${GRAPH}/drives/${driveId}/root/search(q='${encodeURIComponent(`CMR TORG-${ano}`)}')?$select=id,name&$top=20`, { headers: auth });
  ...
  for (const a of achados) {
    const r = await fetch(`${GRAPH}/drives/${driveId}/items/${a.id}?$select=id,name,size,lastModifiedDateTime,parentReference`, { headers: auth });
  ...
  const dl = await fetch(`${GRAPH}/drives/${driveId}/items/${alvo.id}/content`, { headers: auth, redirect: "follow" });
