...
import { completarEps } from "./eps-casa";
...
/**
 * A EPS que corresponde a um processo.
 *
 * ⚠ EPS É POR PROCESSO, CQS É POR SOLDADOR. Vitor (21/08/2026): "a EPS tem uma numeração específica
 * para cada soldador? exemplo, no relatório aparece EPS - 002_25". Não — 002/25 é o número da CQS
 * (qualificação do Daniel em FCAW). São cinco EPS para nove soldadores; se fossem por pessoa,
 * seriam nove. O que liga as duas é o PROCESSO: qualificado em FCAW pela CQS-002/25, ele solda sob
 * a EPS-RQPS 02 FCAW.
 */
export function epsDoProcesso(lista, processo) {
  if (!processo) return null;
  const alvo = String(processo).toUpperCase().trim();
  return (lista || []).find((e) => String(e.processo || "").toUpperCase().trim() === alvo) || null;
}
...
export async function listarEPS(forcar = false) {
  ...
  const lista = filhos
    ...
    .filter(Boolean)
    // ⚠ o nome do arquivo não diz tudo — as EPS 03, 04 e 05 nem trazem o processo. A ficha
    // (lib/eps-casa.js) completa número do documento, processo, arame e RQPS.
    .map(completarEps)
