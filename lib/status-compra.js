...
export async function materialPorPerfil(opNumero, perfis) {
  ...
  // ⚠⚠ A AMARRAÇÃO À MÃO TAMBÉM VALE AQUI. Vitor (26/08/2026): "os materiais estão sem o R".
  // O R do Z da OP-105 foi amarrado por `TrocaRastreabilidade` e a liberação passou a enxergar —
  // esta função, que é a que o PCP usa, continuava casando SÓ por descrição. Resultado: o mesmo
  // aço aparecia entregue no Planejamento e "sem R" no PCP, que é onde alguém vai separar o fardo.
  //
  // ⚠ Duas telas lendo o mesmo material por critérios diferentes é o erro que mais custa aqui: a
  // conversa vira sobre em qual acreditar, não sobre qual barra cortar.
  const { amarracoesDaOp } = await import("./r-amarrado");
  const amarrado = await amarracoesDaOp(num);
  ...
  if (!entradas.length && !amarrado.size) return out;
  ...
  // ⚠⚠ O R AMARRADO PODE SER DE OUTRA OBRA — E QUASE SEMPRE É. Vitor (01/09/2026): "já definimos o
  // R, feche esse problema em todas as abas". A amarração existe justamente para apontar MATERIAL
  // DE ESTOQUE, cuja nota foi lançada em outra OP: na OP-112 os R amarrados são 260103 (OP-067),
  // 261066 (OP-097), 260083 (OP-074) e 260027 (OP-008). Procurando só dentro do CMR desta OP, o
  // resolvedor não achava nenhum deles, IGNORAVA A AMARRAÇÃO EM SILÊNCIO e a tela dizia "sem R" —
  // com o R já definido no banco. Silêncio é o pior: quem olha conclui que ninguém amarrou.
  const rsAmarrados = [...new Set(
    lista.map((p) => amarrado.get(String(p).trim().toUpperCase())?.r).filter(Boolean)
  )];
  const deFora = new Map();
  if (rsAmarrados.length) {
    const faltantes = rsAmarrados.filter((r) => !entradas.some((e) => String(e.importRef || "").trim() === r));
    if (faltantes.length) {
      const extra = await prisma.documentoQualidade.findMany({
        where: { categoria: "MATERIAL", importRef: { in: faltantes } },
        select: {
          importRef: true, nome: true, nfNumero: true, dataRecebimento: true, pesoKg: true, quantidade: true,
          numeroCorrida: true, numeroDocumento: true, norma: true, fornecedor: true, pedidoCompra: true, opNumero: true,
        },
      });
      for (const e of extra) deFora.set(String(e.importRef).trim(), e);
    }
  }

  for (const perfil of lista) {
    // ⚠ o R amarrado vem PRIMEIRO: alguém disse, com nome e motivo, qual fardo é. Isso ganha de um
    // casamento por texto, que é palpite comparado a uma decisão humana registrada.
    const am = amarrado.get(String(perfil).trim().toUpperCase());
    if (am) {
      const e = entradas.find((x) => String(x.importRef || "").trim() === am.r) || deFora.get(am.r);
      if (e) {
        out.set(perfil.toUpperCase(), {
          recebido: true, ...rastro(e), entradas: [rastro(e)],
          porTroca: true, trocaPor: am.por || null,
          // ⚠ de qual obra veio o fardo — material de estoque não é da OP, e a tela precisa poder dizer
          deOutraOp: e.opNumero && String(e.opNumero) !== num ? String(e.opNumero) : null,
        });
        continue;
      }
    }
