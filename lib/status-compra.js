...
export async function materialPorPerfil(opNumero, perfis) {
  ...
  const trocas = await prisma.trocaRastreabilidade.findMany({
    where: { opNumero: num }, select: { perfil: true, rUsado: true, trocadoPorNome: true },
  }).catch(() => []);
  const amarrado = new Map(trocas.filter((t) => t.rUsado).map((t) => [String(t.perfil).trim().toUpperCase(), t]));
  ...
  for (const perfil of lista) {
    ...
    if (am) {
      const e = entradas.find((x) => String(x.importRef || "").trim() === String(am.rUsado).trim());
      if (e) {
        out.set(perfil.toUpperCase(), {
          recebido: true, ...rastro(e), entradas: [rastro(e)],
          porTroca: true, trocaPor: am.trocadoPorNome || null,
        });
        continue;
      }
    }
