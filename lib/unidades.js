...
export function conversaoDoItem({ qtdDoc, precoDoc, fator, unidadeCotada, unidadeRM }) {
  ...
  const convertido = converterParaRM({ qtd: qtdDoc, preco: precoDoc, fator: f });
  if (!totalBate(doDocumento, convertido.total)) {
    return { erro: `A conversão mudaria o total da proposta (R$ ${doDocumento.toFixed(2)} → R$ ${convertido.total.toFixed(2)}). Confira a unidade e o fator.` };
  }
  return { ok: true, ...convertido, unidadeCotada: unidadeCanonica(unidadeCotada), unidadeRM: unidadeCanonica(unidadeRM) };
}
