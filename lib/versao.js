...
/**
 * Número de build = quantidade de commits do repositório, gravada em versao-build.json
 * pelo hook de pre-commit. Incrementa a cada commit que sobe pro Vercel, então serve
 * de "carimbo" pra saber qual atualização está no ar.
 */
export const BUILD_NUMERO = process.env.NEXT_PUBLIC_BUILD_NUMERO || null;

/** Título do commit publicado — o "o que mudou" da versão que está no ar. */
export const BUILD_TITULO = process.env.NEXT_PUBLIC_BUILD_TITULO || null;

/** Rótulo curto pro rodapé: "v2.0.0 · build 2773" (ou só a versão, se não houver número). */
export const VERSAO_LABEL = BUILD_NUMERO
  ? `v${VERSAO_ATUAL} · build ${BUILD_NUMERO}`
  : `v${VERSAO_ATUAL}`;
