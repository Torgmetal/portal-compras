...
export async function vetoresDaVista(pdfBytes, { min = 1.5 } = {}) {
  ...
  try {
    ...
    ol = await (await doc.getPage(1)).getOperatorList();
  } catch { return null; }
  ...
  const unicos = segs.filter((s) => { const k = s.join(","); if (vistos.has(k)) return false; vistos.add(k); return true; });

  return { largura: vista.largura, altura: vista.altura, segs: unicos };
}
