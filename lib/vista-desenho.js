...
export async function recortarVista(pdfBytes) {
  ...
  // ── SÓ RECORTA FOLHA DE CONJUNTO (A3) ───────────────────────────────────────────────────────
  //
  // Vitor reclamou do "projeto todo" olhando o desenho de CONJUNTO: A3, com SIMBOLOGIA DE FUROS,
  // QUADRO DE PENAS, LISTA DE MATERIAIS, carimbo e três vistas. Ali o recorte vale muito.
  //
  // O croqui é outra folha: A4, e a peça JÁ ocupa quase tudo — só há uma faixa de tabelas em cima e
  // o carimbo embaixo. Tentei recortar ali também e o resultado foi pior que o original: o
  // agrupamento escolhia a cadeia de cotas, ou o logo da Torg (que é vetor pesado e ganha de uma
  // peça inteira em quantidade de traço). Devolver `null` faz o relatório usar a folha inteira, que
  // no croqui é o certo. Se um dia incomodar, o caminho é a Engenharia exportar DXF — lá a vista é
  // uma entidade nomeada e não se adivinha nada.
  if (vp.width < 900) return null;
  ...
  const fora = (x, y) => {
    ...
    if (!reg.ehCroqui && x >= reg.carimbo.x0 && y <= reg.carimbo.y1) return true;
    if (!reg.ehCroqui && x >= reg.lista.x0 && y >= reg.lista.y0) return true;
    return false;
  };
  ...
  const principal = reg.ehCroqui
    ? (() => {
        const xs = dentro.map((p) => p[0]), ys = dentro.map((p) => p[1]);
        return { bb: { left: Math.min(...xs), right: Math.max(...xs), bottom: Math.min(...ys), top: Math.max(...ys) } };
      })()
    : vistaPrincipal(dentro, reg.margem);
  ...
  if (reg.ehCroqui) {
    if (reg.faixaInferior) tapar(reg.faixaInferior);
    if (reg.faixaSuperior) tapar(reg.faixaSuperior);
  } else {
    tapar(reg.carimbo);
    tapar(reg.lista);
    tapar(reg.esquerda);
