 ...
 */
function segsDoConteudo(ol, OPS, min) {
  const segs = [];
  let ctm = [1, 0, 0, 1, 0, 0];
  const pilha = [];
  const põe = (a, b) => {
...
export async function vetoresDaPagina(pdfBytes, { min = 1.5 } = {}) {
  ...
  const textos = mapeiaTextos(txt).filter((t) => t.x >= -2 && t.x <= vp.width + 2 && t.y >= -2 && t.y <= vp.height + 2);
