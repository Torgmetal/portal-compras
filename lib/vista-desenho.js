...
import {
  mul, aplicar, espacoDaPagina, caixaParaCru, caixaDoTexto, transladar, recorteAproveitavel, IDENTIDADE,
} from "./geometria-pagina";
 ...
 */
function pontosDaPagina(ol, OPS, passo = 5, base = IDENTIDADE) {
  const pts = [];
  let ctm = [...base];
  const pilha = [];
  const traco = (p1, p2) => {
  ...
  for (let i = 0; i < ol.fnArray.length; i++) {
    ...
    else if (f === OPS.restore) ctm = pilha.pop() || [...base];
    else if (f === OPS.transform) ctm = mul(ctm, a);
    // ⚠⚠ O FORM XOBJECT TEM `/Matrix` PRÓPRIA, e ela não vem como `transform`. O pdf.js emite a
    // COLOCAÇÃO do form como `transform` (que o laço acima já trata) e a matriz interna aqui, à
    // parte. Sem isto, o desenho que mora dentro de um form saía em coordenadas CRUAS enquanto
    // `segsDoConteudo` — que já tratava — o via no lugar certo: as duas metades do módulo
    // discordavam sobre onde está a peça. Medido com `/Matrix [2 0 0 2 50 30]`: vertical em x=10
    // no lugar de x=70 (achado do Codex, confirmado em 18/09/2026).
    else if (f === OPS.paintFormXObjectBegin) { pilha.push([...ctm]); ctm = mul(ctm, a[0]); }
    else if (f === OPS.paintFormXObjectEnd) { ctm = pilha.pop() || [...base]; }
    else if (f === OPS.constructPath) {
      const [ops, args] = a;
      let ai = 0, cur = null, inicio = null;
      for (const op of ops) {
        if (op === OPS.moveTo) { cur = aplicar(ctm, args[ai], args[ai + 1]); pts.push(cur); inicio = cur; ai += 2; }
        ...
        // ⚠⚠ `closePath` NÃO TEM ARGUMENTO, e fecha o subcaminho (achado 18/09/2026). Dois
        // defeitos vinham daqui: o lado de FECHAMENTO era perdido — moldura escrita como
        // `m l l l h` (que é como o pdf-lib e vários CADs escrevem um retângulo) chegava com três
        // lados — e o `else ai += 2` consumia dois argumentos que não existem, dessincronizando
        // TODAS as coordenadas seguintes do mesmo traçado. Num caminho com vários subcaminhos
        // (`m l l h m l l h`), tudo depois do primeiro `h` virava número sem sentido.
          if (cur && inicio) traco(cur, inicio);
          cur = inicio;
        } else ai += 2;
      }
    }
...
/**
 * A borda esquerda do CARIMBO — a divisória entre o desenho e a coluna de tabelas — e até onde
 * ela sobe.
 *
 * ⚠⚠ ELA NASCE NA MOLDURA DE BAIXO, NA METADE DIREITA DA FOLHA — sem exigir as duas coisas, a
 * "divisória" podia ser uma linha do PRÓPRIO DESENHO. Vitor (23/09/2026), nos RID-084 da OP-84:
 * "parece que os desenhos estão ficando zuado". Era "a vertical mais longa da faixa", e isso só
 * acerta no A3: o carimbo do Tekla tem altura FIXA (~234 pt no A3 e no A2), mas o desenho cresce com
 * a folha. No A3 a borda do carimbo é 29% da altura e ganhava; no A2 cai para 20%, e uma linha de
 * cota da própria coluna de 12 m (308 pt, 26%) passava na frente. Com a divisória no meio do
 * desenho, as máscaras de carimbo e lista cobriam tudo à direita dela: T84A2/A3 saíam com um pedaço
 * da cabeça da coluna, o T84A5 com um retalho de 65 x 69 pt, o T84A4 com o CORTE A-A no lugar da
 * peça. Medido: os T89A1/A2/A3 (A2) também pegavam linha do desenho (x = 881, 954, 792 em vez de
 * 1129) e só davam certo porque a vista principal estava inteira à esquerda dela.
 * ⚠ Só "nascer na moldura de baixo" não basta: no T84A4 o CORTE B-B, na metade ESQUERDA, desce até
 * a moldura e tem traço mais longo que o carimbo.
 *
 * ⚠ O TOPO é o fim da CORRENTE de traços que sobe da borda sem interrupção, não o do primeiro
 * trecho: no desenho da TMSA (RPM-103) a coluna da direita é uma caixa só, do carimbo até as
 * premissas, escrita em trechos (28–193, 190–200, 200–240 … 656–676), e a lista de materiais dela
 * mora nessa caixa. No Tekla da Torg a corrente acaba no próprio carimbo (a lista começa muito
 * acima), e o topo é o mesmo de antes.
 *
 * @returns {{x:number, topo:number}|null} `null` quando a folha não tem essa borda
 */
function bordaDoCarimbo(verticais, { meioX, baseF, dir, alturaPg }) {
  const borda = verticais
    .filter((v) => v.x > meioX && v.x < dir && Math.abs(v.y1 - baseF) <= 6 &&
      v.y2 - v.y1 > alturaPg * 0.15 && v.y2 - v.y1 < alturaPg * 0.7)
    .sort((a, b) => (b.y2 - b.y1) - (a.y2 - a.y1))[0];
  if (!borda) return null;
  let topo = borda.y2;
  const mesmaLinha = verticais.filter((v) => Math.abs(v.x - borda.x) < 1).sort((a, b) => a.y1 - b.y1);
  for (const v of mesmaLinha) if (v.y1 <= topo + 2 && v.y2 > topo) topo = v.y2;
  return { x: borda.x, topo };
}
...
function regioes(pts, larguraPg, alturaPg, verticaisLongas, horizontaisLongas) {
  ...
  // a divisória da coluna direita: a borda esquerda do carimbo (ver `bordaDoCarimbo`)
  const div = bordaDoCarimbo(verticaisLongas, { meioX, baseF, dir: margem.dir, alturaPg });
  ...
  const topoCarimbo = div ? div.topo : alturaPg * 0.33;
  ...
  return {
    ehCroqui,
    // ── FAIXAS: SÓ NA FOLHA DE CROQUI ──────────────────────────────────────────────────────────
    //
    // Elas cobrem a LARGURA INTEIRA, e isso só é verdade no croqui (A4), onde as tabelas ocupam uma
    // faixa de ponta a ponta. No conjunto (A3) as tabelas ficam na coluna da direita, e quem cuida
    // delas é `lista`/`carimbo`, que respeitam essa coluna.
    //
    // ⚠ Sem esta condição, a faixa superior era montada TAMBÉM no conjunto — as linhas da lista de
    // materiais (uma a cada ~14 pt, no alto da coluna direita) viravam "tabela no topo" e a máscara
    // varria a folha toda naquela altura. No T89A3 passou despercebido porque a peça termina abaixo
    // dali; no T89A1 a peça sobe um pouco mais e o topo dela foi cortado — foi o que o Vitor viu
    // ("já nesse relatório novo você cortou o desenho").
    faixaInferior: ehCroqui && topoCarimboFaixa != null ? { x0: 0, x1: larguraPg, y0: 0, y1: topoCarimboFaixa + 2 } : null,
    faixaSuperior: ehCroqui && fundoTabelasTopo != null ? { x0: 0, x1: larguraPg, y0: fundoTabelasTopo - 2, y1: alturaPg } : null,
 ...
 * @param {{caixaManual?: {left:number,right:number,bottom:number,top:number}}} opts
 *   `caixaManual` vem em pontos da folha ORIGINAL (mesmo espaço que `vetoresDaPagina` devolve) e,
 *   quando presente, vale por cima do algoritmo inteiro de achar a vista sozinho — ver nota abaixo.
 * @returns {Promise<{bytes:Uint8Array, largura:number, altura:number}|null>}
 */
export async function recortarVista(pdfBytes, opts = {}) {
  const { getDocumentProxy, getResolvedPDFJS } = await import("unpdf");
  const { OPS } = await getResolvedPDFJS();

  let pg, ol, esp, textos;
  try {
    const doc = await getDocumentProxy(new Uint8Array(pdfBytes));
    pg = await doc.getPage(1);
    ol = await pg.getOperatorList();
    esp = espacoDaPagina(pg);
    textos = (await pg.getTextContent()).items.filter((i) => String(i.str).trim());
  } catch { return null; }

  // ⚠⚠ TUDO DAQUI PARA BAIXO VIVE NO ESPAÇO DA FOLHA ORIENTADA. Ver `lib/geometria-pagina.js`:
  // o viewport do PDF.js já vem girado e a operator list não, e comparar um com o outro era o que
  // descartava metade do desenho numa folha com `/Rotate 90`.
  const vp = { width: esp.largura, height: esp.altura };
  const pts = pontosDaPagina(ol, OPS, 5, esp.M);
  ...
  // ⚠ A caixa sai dos QUATRO cantos, pela direção da própria matriz — ver `caixaDoTexto`.
  const pontosTexto = textos.map((t) => {
    const m = mul(esp.M, t.transform);
    return { ...caixaDoTexto(m, t.width || 0, t.height || 8), x: m[4], y: m[5] };
  });

  // verticais longas, para achar a divisória da coluna direita
  const vs = verticais(ol, OPS, esp.M);
  const hs = horizontais(ol, OPS, esp.M);
  ...
  // ⚠⚠ RECORTE MANUAL, por cima do algoritmo inteiro. Vitor (03/09/2026): "quero poder colocar o
  // projeto dentro do relatório e poder mover ele dentro para mostrar apenas o que eu selecionar" —
  // o automático abaixo acerta a maioria das peças, mas uma folha com várias vistas parecidas (um
  // diagrama de montagem tem isso aos montes) dá margem para escolher a vista errada, e não havia
  // correção nenhuma. Quando o relatório guarda uma caixa escolhida à mão (mesmo espaço de pontos
  // que `vetoresDaPagina` devolve, ou seja, a folha ORIGINAL), ela vale sozinha: só falta encaixar
  // nos limites da folha, sem achar vista nem cortar corte nenhum.
  // ⚠⚠ RECORTE ANTIGO NÃO É REAPLICADO EM FOLHA GIRADA (achado do Codex, 18/09/2026). Ele foi
  // escolhido sob outro contrato de coordenadas; aplicá-lo calado moveria o recorte e levaria as
  // cotas junto. Onde a matriz é identidade os dois contratos coincidem e nada muda.
  const aproveita = recorteAproveitavel(opts.caixaManual, esp);
  const manual = aproveita.ok ? opts.caixaManual : null;
  const recorteAntigoIgnorado = !!opts.caixaManual && !aproveita.ok;

  let caixa;
  if (manual) {
    const m = manual;
    caixa = {
      left: Math.max(0, Math.min(m.left, m.right)),
      right: Math.min(vp.width, Math.max(m.left, m.right)),
      bottom: Math.max(0, Math.min(m.bottom, m.top)),
      top: Math.min(vp.height, Math.max(m.bottom, m.top)),
    };
    if (caixa.right - caixa.left < 20 || caixa.top - caixa.bottom < 20) return null;
  } else {
    // moldura, carimbo e tabelas — à parte do limite dos cortes, que precisa saber o que é desenho
    const foraDaFolha = (x, y) => {
    ...
    // que o agrupamento precisa ter (a cota fica longe do traço, e apertar a folga quebrava a
    // própria vista em pedaços). O sinal que resolve é SEMÂNTICO — o desenho ROTULA cada corte. No
    // T89A3 os rótulos estão em y=525 e y=295, e a vista principal fica inteira acima deles.
    //
    // ⚠ O rótulo fica ACIMA do corte que ele nomeia, então o que vale é o MAIS ALTO: dali para
    // baixo é tudo secundário.
    const yCortes = textos
      .filter((t) => /^\s*(CORTE|DETALHE|SEÇÃO|SECAO)\b/i.test(String(t.str)))
      .map((t) => mul(esp.M, t.transform)[5]);
    ...
    if (yCortes.length) {
      ...
      // ⚠⚠ E SÓ SE O LIMITE PASSAR NUM VAZIO. Vitor (23/09/2026), nos RID-084 da OP-84: "parece que
      // os desenhos estão ficando zuado". Cortar tudo abaixo do rótulo supõe o layout do T89A3 —
      // vista em cima, cortes EMBAIXO. Nas colunas de 12 m da OP-84 (T84A2/A3/A5) os cortes ficam
      // AO LADO das elevações, na altura do meio delas: o limite ATRAVESSAVA a coluna e jogava fora
      // três quartos dela, e o relatório saía só com a cabeça. O sinal que separa um caso do outro é
      // traço vertical do desenho cruzando a linha do limite. Medido nas folhas reais: T84A2/A3/A5
      // têm 4, 18 e 17; T84A1 e T89A1/A2/A3 (cortes embaixo) têm zero — para eles nada muda.
      // ⚠ A partir de DOIS. Um traço só é aresta solta, não peça fatiada: no T97A1 é a da VISTA 3D,
      // ao lado, e o recorte dele continua o de antes.
      const atravessam = vs.filter((v) =>
        v.y1 < limite - 12 && v.y2 > limite + 12 && !foraDaFolha(v.x, limite)).length;
      if (acima > pts.length * 0.2 && atravessam < 2) corteAcimaDe = limite;
    }
    const fora = (x, y) => (corteAcimaDe != null && y <= corteAcimaDe) || foraDaFolha(x, y);
    ...
    // No croqui (A4) o corte é por FAIXA — a folha é da peça e o que atrapalha são os andares de
    // cima e de baixo. No conjunto (A3) é por ilha, porque ali há várias vistas na mesma folha.
    let principal;
    if (ehFolhaCroqui) {
    ...
    for (const t of pontosTexto) {
      ...
      if (t.right < bb.left - perto || t.left > bb.right + perto) continue;
      if (t.top < bb.bottom - perto || t.bottom > bb.top + perto) continue;
      bb.left = Math.min(bb.left, t.left);
      bb.right = Math.max(bb.right, t.right);
      bb.bottom = Math.min(bb.bottom, t.bottom);
      bb.top = Math.max(bb.top, t.top);
    }
  ...
  // ── DO ESPAÇO DA FOLHA ORIENTADA DE VOLTA AO CRU, que é onde o pdf-lib recorta ─────────────
  //
  // ⚠⚠ `embedPage` NÃO CONHECE `/Rotate`. A caixa acima está no espaço orientado (o mesmo que a
  // tela usou para escolher o recorte); entregá-la crua ao pdf-lib fazia o corte cair noutro
  // lugar da folha. A conversão passa pelos QUATRO cantos — sob 90° o canto de baixo vira o de
  // cima, e converter só dois devolvia caixa invertida (achado do Codex, 18/09/2026).
  const larguraSaida = caixa.right - caixa.left;
  const alturaSaida = caixa.top - caixa.bottom;
  const caixaCrua = esp.girada ? caixaParaCru(caixa, esp.inv) : caixa;

  const origem = await PDFDocument.load(pdfBytes, { updateMetadata: false });
  const out = await PDFDocument.create();
  const emb = await out.embedPage(origem.getPage(0), caixaCrua);
  // ⚠ A página de saída tem as dimensões do espaço ORIENTADO e `/Rotate 0`: a tela desenha as
  // cotas sobre `largura`/`altura` devolvidos aqui, e o PDF final carimba nas mesmas coordenadas.
  // Deixar a rotação para a página de saída manteria o conteúdo cru e a divergência de pé.
  const page = out.addPage([larguraSaida, alturaSaida]);

  if (esp.girada) {
    // O XObject nasce transladado pela origem da caixa crua. Para o ponto cair onde a folha
    // orientada diz: desfaz essa translação, aplica a matriz da página, e traz para a origem do
    // recorte. Uma vez só, dentro de save/restore, para não vazar para as máscaras e cotas.
    const D = mul(transladar(-caixa.left, -caixa.bottom),
      mul(esp.M, transladar(caixaCrua.left, caixaCrua.bottom)));
    page.pushOperators(pushGraphicsState(), concatTransformationMatrix(D[0], D[1], D[2], D[3], D[4], D[5]));
  ...
  //
  // ⚠⚠ NUNCA NO RECORTE MANUAL. Vitor (03/09/2026): "recriei o relatório novamente e mesmo assim
  // ele está cortado depois de feito o recorte" — no diagrama de montagem, que não segue o layout
  // do "conjunto" do Tekla (moldura + coluna de tabela num canto fixo), `reg.carimbo`/`reg.lista`
  // saem de uma heurística que procura esse layout e pode achar um retângulo qualquer sem relação
  // nenhuma com esta folha — daí cobrir de branco um pedaço do que a pessoa ESCOLHEU mostrar.
  // Recorte automático precisa desta limpeza porque o próprio corte é um palpite; recorte manual
  // não: a pessoa já enquadrou exatamente o que quer, e mais nada deveria mexer nisso.
  if (!ehFolhaCroqui && !manual) {
    const tapar = (r) => {
  ...
  // ⚠ As dimensões devolvidas são as do espaço ORIENTADO, não as de `emb` (que são da caixa crua
  // e, numa folha girada, vêm com largura e altura trocadas).
  return { bytes: await out.save(), largura: larguraSaida, altura: alturaSaida, recorteAntigoIgnorado };
}
 ...
 */
/**
 * Anda a operator list e devolve os segmentos de traço, já em ponto do usuário. Usado tanto pela
 * vista recortada (que embute um Form XObject, com matriz própria) quanto pela folha inteira.
 */
function segsDoConteudo(ol, OPS, min, base = IDENTIDADE) {
  const segs = [];
  let ctm = [...base];
  const pilha = [];
  const põe = (a, b) => {
  ...
  for (let i = 0; i < ol.fnArray.length; i++) {
    ...
    // o Vitor viu na tela ("veio bugado"). A folha inteira não tem esse XObject, mas os mesmos
    // opcodes não fazem mal nenhum quando não aparecem.
    else if (f === OPS.paintFormXObjectBegin) { pilha.push([...ctm]); ctm = mul(ctm, a[0]); }
    else if (f === OPS.paintFormXObjectEnd) { ctm = pilha.pop() || [...base]; }
    else if (f === OPS.constructPath) {
      const [ops, args] = a;
      let ai = 0, cur = null, inicio = null;
      for (const op of ops) {
        if (op === OPS.moveTo) { cur = aplicar(ctm, args[ai], args[ai + 1]); inicio = cur; ai += 2; }
        else if (op === OPS.lineTo) {
          const p = aplicar(ctm, args[ai], args[ai + 1]); ai += 2;
          if (cur) põe(cur, p);
          cur = p;
          ...
          // ⚠ ver a nota em `pontosDaPagina`: sem argumento, e fecha o subcaminho.
          if (cur && inicio) põe(cur, inicio);
          cur = inicio;
        } else ai += 2;
      }
    }
...
/**
 * O texto da página, no formato que a tela usa (posição, tamanho, se está girado).
 *
 * ⚠⚠ A MATRIZ DO TEXTO ENTRA INTEIRA, não só a posição (achado do Codex, 18/09/2026). Numa folha
 * girada, transformar apenas `x`/`y` deixaria o rótulo no lugar certo mas deitado para o lado
 * errado — e `v` (que manda a tela girar o texto) sairia invertido.
 */
function mapeiaTextos(txt, base = IDENTIDADE) {
  return txt.map((i) => {
    const m = mul(base, i.transform);
    return {
      s: String(i.str),
      x: Math.round(m[4] * 2) / 2,
      y: Math.round(m[5] * 2) / 2,
      t: Math.round((i.height || 6) * 10) / 10,
      // largura do texto: é o que permite APAGAR o item depois, cobrindo exatamente a área dele
      w: Math.round((i.width || 0) * 2) / 2,
      // cota vertical vem com o texto girado; a tela precisa girar junto para não sair sobreposta
      v: Math.abs(m[1]) > 0.3 || Math.abs(m[2]) > 0.3,
    };
  });
}

export async function vetoresDaVista(pdfBytes, { min = 1.5, caixaManual = null } = {}) {
  const vista = await recortarVista(pdfBytes, { caixaManual });
  ...
  // ⚠ SEM MATRIZ AQUI, DE PROPÓSITO: `recortarVista` já entrega a vista com `/Rotate 0` e origem
  // em (0,0) — a rotação da folha original foi assada no desenho. Aplicar a matriz de novo giraria
  // duas vezes.
  const segs = segsDoConteudo(ol, OPS, min);
  ...
  return { largura: vista.largura, altura: vista.altura, segs: unicos, textos,
    recorteAntigoIgnorado: !!vista.recorteAntigoIgnorado };
}

/**
 * A FOLHA INTEIRA EM VETOR — para escolher o recorte manual vendo tudo, não só o que o algoritmo
 * automático já isolou.
 *
 * Vitor (03/09/2026): "quero poder colocar o projeto dentro do relatório e poder mover ele dentro
 * para mostrar apenas o que eu selecionar". `vetoresDaVista` só devolve o que `recortarVista` já
 * decidiu ser a vista; esta função devolve a folha original inteira, no mesmo formato, para a tela
 * desenhar um retângulo arrastável por cima e o resultado virar `caixaManual` de `recortarVista`.
 *
 * @returns {Promise<{largura:number, altura:number, segs:number[][], textos:Array}|null>}
 */
export async function vetoresDaPagina(pdfBytes, { min = 1.5 } = {}) {
  const { getDocumentProxy, getResolvedPDFJS } = await import("unpdf");
  const { OPS } = await getResolvedPDFJS();
  ...
  try {
    ...
    const pg1 = await doc.getPage(1);
    ol = await pg1.getOperatorList();
    esp = espacoDaPagina(pg1);
    txt = (await pg1.getTextContent()).items.filter((i) => String(i.str).trim());
  } catch { return null; }

  // ⚠⚠ COM A MATRIZ DA PÁGINA. Sem ela, numa folha girada o traço saía em coordenadas cruas e o
  // filtro abaixo o descartava em silêncio — a tela de recorte manual recebia a folha já
  // incompleta, e era por isso que "ajustar recorte" não salvava o desenho cortado (18/09/2026).
  const vp = { width: esp.largura, height: esp.altura };
  const segs = segsDoConteudo(ol, OPS, min, esp.M);
  const dentroDaPagina = ([x1, y1, x2, y2]) =>
    x1 >= -2 && x1 <= vp.width + 2 && y1 >= -2 && y1 <= vp.height + 2 &&
    x2 >= -2 && x2 <= vp.width + 2 && y2 >= -2 && y2 <= vp.height + 2;
  const vistos = new Set();
  const unicos = segs.filter(dentroDaPagina).filter((s) => { const k = s.join(","); if (vistos.has(k)) return false; vistos.add(k); return true; });

  const textos = mapeiaTextos(txt, esp.M).filter((t) => t.x >= -2 && t.x <= vp.width + 2 && t.y >= -2 && t.y <= vp.height + 2);

  return { largura: vp.width, altura: vp.height, segs: unicos, textos };
}
