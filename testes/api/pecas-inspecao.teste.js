...
it('consulta a lista pelo vínculo da OP sem gravar sugestões no relatório',async()=>{
 ...
 expect(mockPrisma.pecaConjunto.findMany).toHaveBeenCalledWith({where:{opId:'obra-106',OR:[{tipoPeca:'CONJUNTO'},{tipoPeca:null}]},select:{marca:true,qte:true,fonte:true,naLE:true,tipoPeca:true,pesoTotalKg:true}});
