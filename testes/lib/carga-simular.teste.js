...
describe("modelo de carga em PDF", () => {
  it("gera documento em A4 paisagem com vistas, volumes, camadas e separação", async () => {
    ...
    for (const page of doc.getPages()) { expect(page.getWidth()).toBeCloseTo(841.89); expect(page.getHeight()).toBeCloseTo(595.28); }
    const { extractText } = await import("unpdf");
    const { text } = await extractText(bytes, { mergePages: true });
    for (const item of lista) expect(text).toContain(item.marca);
