...
describe("compararLista", () => {
  ...
  it("arquivo no servidor e nada no portal: nunca importada", () => {
    const r = compararLista([arq("T105-LE-R01.xlsx", "2026-09-10T12:00:00Z")], []);
...
// ─── Achados da revisão do Codex sobre o cron (15/09/2026) ───────────────────

describe("obra com mais de uma frente", () => {
  // ⚠⚠ O CASO REAL DA OP-085, e foi falso positivo na primeira execução. O portal tem duas
  // frentes — T85-LE (T85-LE-R01.xls) e T85-LE-R01 GALV (T85-LE-R01_GALV.xls) — e o servidor
  // tem só o primeiro. Comparando contra a importação MAIS RECENTE (a do GALV), o arquivo que já
  // estava importado aparecia como "revisão nova".
  const registros085 = [
    { frente: "T85-LE-R01 GALV", arquivo: "T85-LE-R01_GALV.xls", fileModificado: new Date("2026-09-01T10:00:00Z"), importadoEm: new Date("2026-09-01T11:00:00Z") },
    { frente: "T85-LE", arquivo: "T85-LE-R01.xls", fileModificado: new Date("2026-08-17T10:00:00Z"), importadoEm: new Date("2026-08-17T11:00:00Z") },
  ];

  it("o arquivo do servidor já importado por OUTRA frente está em dia", () => {
    const r = compararLista([arq("T85-LE-R01.xls", "2026-08-17T10:00:00Z")], registros085);
    expect(r.situacao).toBe(SITUACAO.EM_DIA);
    expect(r.noPortal).toBe("T85-LE-R01.xls");
  });

  it("mas troca desse mesmo arquivo continua sendo pega", () => {
    const r = compararLista([arq("T85-LE-R01.xls", "2026-09-14T10:00:00Z")], registros085);
    expect(r.situacao).toBe(SITUACAO.ARQUIVO_TROCADO);
  });

  it("arquivo que nenhuma frente importou é revisão nova", () => {
    const r = compararLista([arq("T85-LE-R02.xls", "2026-09-14T10:00:00Z")], registros085);
    expect(r.situacao).toBe(SITUACAO.REVISAO_NOVA);
  });

  it("o nome casa ignorando caixa e espaço nas pontas", () => {
    const r = compararLista([arq(" t85-le-r01.XLS ", "2026-08-17T10:00:00Z")], registros085);
    expect(r.situacao).toBe(SITUACAO.EM_DIA);
  });
});
