...
var ORDEM_VEIC = ["hr", "tresquartos", "toco", "truck", "carreta"];
var FRETE = { hr: 35, tresquartos: 45, toco: 55, truck: 70, carreta: 100, carreta14: 140 };
...
var TEMPO = { movimento: 4, movimentoLeve: 3, emPe: 4, madeiraPorCamada: 5, amarracao: 30, conferencia: 15 };
...
var ehDelicadoPlano = (u) => u.gc || u.grade || u.plano || u.tipo === "PALLET" || u.tipo === "CAIXA" || /GRADE/i.test(u.rotulo || u.desc || "");
...
function expandirPecas(lista, geometria2 = {}, ajustes = {}) {
  ...
  for (const p of lista) {
    ...
    for (let i = 0; i < (p.qtd || 1); i++) {
      ...
      pecas.push(o ? { ...base, ...o, facesApoio: !aj?.orientacao && !manual ? g?.apoioSuperior?.faces : void 0, topoVazado: g?.apoioSuperior ? !!aj?.orientacao || !!manual || !g.apoioSuperior.plana : void 0, temGeo: !manual && !!g?.temGeo, aproximada: !!manual || !g?.obb, estimada: !!estimada, manual: !!manual } : { ...base, semCaixa: true });
    }
  }
...
var temFacesApoio = (u) => !!u.facesApoio || !!u.membros?.some((m) => m.facesApoio);
...
var CAMPOS = ["x", "y", "z", "sobre", "girada", "fx", "fz", "nivelPilha", "camada", "pilha", "semLugar", "calcos"];
...
function avaliarPosicao(c, u, perfil, ctx, pos, tetoUtil, pref, melhor) {
  ...
  if (topo > 0) {
    ...
    const candidato = { ...u, x: c.zonaGC + ix * c.cel + FOLGA / 2, z: iz * c.cel + FOLGA / 2, y, fx: pos.fx, fz: pos.fz, girada: pos.girada };
    const usaContato = temFacesApoio(u) && baixo.some(temFacesApoio);
    if (usaContato) {
      calcos = planejarCalcos(candidato, baixo);
      if (!calcos) return null;
    }
    const candidatos = baixo.filter((b) => Math.abs(b.y + b.A - topo) <= 2 && !b.baseEmV && (calcos || !b.topoVazado));
    ...
    if (!calcos && !cobertura.suficiente) return null;
    custo += (1 - cobertura.fracao) * 1e4;
  }
  return { ...pos, y, custo, baixo, calcos, nivel: topo > 0 ? nivelBase : 0 };
}
function tentaPor(c, u, perfil, ctx, pref = "chao") {
  ...
  const { ix, iz, y, cx, cz, fx, fz, girada, baixo, nivel, calcos } = melhor;
  Object.assign(u, { x: c.zonaGC + ix * cel + FOLGA / 2, z: iz * cel + FOLGA / 2, y, girada, fx, fz, calcos, nivelPilha: nivel, sobre: baixo.map((d) => d.id), pilha: false });
  ...
  for (let a = ix; a < ix + cx; a++) for (let b = iz; b < iz + cz; b++) {
    ...
    c.dono[k] = u;
    c.nivel[k] = nivel + 1;
    if (u.nadaEmCima || u.baseEmV || u.topoVazado && !temFacesApoio(u)) c.tampa[k] = 1;
...
// lib/carga/arranjo.js
var familia = (u) => u.tipo === "PECA" ? `z${u.desc || ""}` : u.rotulo.replace(/~.*$/, "");
var exigePiso = (u) => !!(u.soChao || u.baseEmV && u.almaVertical !== false || u.tipo === "PALLET");
var ESTRATEGIAS = {
  ...
  largura: (a, b) => a.classe - b.classe || b.L - a.L || b.C - a.C,
  // peça larga (pórtico, painel) é a mais difícil de encaixar: vai primeiro
...
function meio(u) {
  return u.classe === 3 ? 1.5 : u.classe;
}
var ordemSequencia2 = (a, b) => a.nivel - b.nivel || a.tipoSeq - b.tipoSeq || a.classe - b.classe || area(b) - area(a) || b.kg - a.kg;
var ordemSequenciaComp2 = (a, b) => a.nivel - b.nivel || a.tipoSeq - b.tipoSeq || a.classe - b.classe || b.C - a.C || area(b) - area(a) || b.kg - a.kg;
var ordemGrades = (a, b) => area(b) - area(a) || b.kg - a.kg || a.nivel - b.nivel;
var ordemGradesComp = (a, b) => b.C - a.C || area(b) - area(a) || b.kg - a.kg;
var freteDe = (ctx, k) => (ctx.frete || FRETE)[k];
function menorVeiculo(carga, perfil, ctx, ordem, pref) {
  ...
  for (const k of ORDEM_VEIC) {
    if (freteDe(ctx, k) >= freteDe(ctx, carga.veicKey)) break;
    const v = ctx.veiculos[k];
    if (!v) continue;
    if (carga.peso > v.pesoMax) continue;
    const FOLGA2 = 60;
    if (carga.itens.some((u) => u.A > v.alturaUtil || !(u.C + FOLGA2 <= v.C && u.L + FOLGA2 <= v.L) && !(u.L + FOLGA2 <= v.C && u.C + FOLGA2 <= v.L))) continue;
...
function melhorArranjo(lista, perfil, ctx, pilha, veicBase = "carreta") {
  let melhor = null;
  const avaliar = (cargas) => {
    ...
    const alertas = cargas.reduce((t, c) => t + c.itens.filter((u) => u.sobre?.length && u.sobre.some((id) => {
      const b = ctx.porId.get(id);
      return b && u.kg > b.kg * 1.5;
    })).length, 0);
    const score = semLugar * 1e6 + cargas.length * 1e4 + frete * 10 + alertas * 3 + Math.max(0, ...cargas.map((c) => c.altura)) / 500;
    return { score, frete };
  };
  const estrategias = ctx.refinarOrdens || (ctx.modoGrupo === "grades" ? { "grades-area": ordemGrades, "grades-comp": ordemGradesComp } : ctx.sequencia ? { sequencia: ordemSequencia2, "sequencia-comp": ordemSequenciaComp2 } : ESTRATEGIAS);
  ...
  for (const [nomeOrdem, cmp] of Object.entries(estrategias)) for (const pref of prefs) for (const reserva of reservas) {
    ...
    if (!ctx.sequencia && cargas.length > 1) {
      ...
      if (cargas2.length < cargas.length) {
        cargas = cargas2;
        ordem.splice(0, ordem.length, ...ordem2);
      } else restaurar(snap);
    }
  ...
  if (!ctx.refinarOrdens && !ctx.sequencia && !pilha.length && ctx.cel > 50 && lista.length <= 80 && melhor.cargas.length > 1 && lista.reduce((s, u) => s + u.kg, 0) <= ctx.veiculos[veicBase].pesoMax) {
  ...
  ctx.reservaTopo = 0;
  restaurar(melhor.snap);
  return melhor;
}
function montarCargas(unidades, perfil, gcModo, ctx) {
  ...
  const montarGrupo = (lista, nome, veicBase = "carreta") => {
    ...
    const arranjo = melhorArranjo(resto, perfil, ctx, pilha, veicBase);
    ctx.modoGrupo = "";
    for (const c of arranjo.cargas) {
      c.grupo = nome;
      c.estrategia = arranjo.nome;
    }
    return arranjo.cargas;
  };
  ...
  if (longas.length) {
    ...
    const c14 = montarGrupo(longas.concat(cand), "carreta 14 m (colunas + pe\xE7as de 12,4 a 14 m)", "carreta14").filter((c) => c.itens.some((u) => longas.includes(u)));
  ...
  if (!ctx.sequencia && cargas.length >= 2) {
    ...
    if (est.length && del.length) {
      const rebaixados = perfil.delicadoSobreDelicado ? del.filter((u) => u.classe === 3 && !u.emPe) : [];
      for (const u of rebaixados) u.classe = 2;
      const cargasB = montarGrupo(est, "estrutura").concat(montarGrupo(del, "delicados"));
      for (const u of rebaixados) u.classe = 3;
      if (cargasB.length <= cargas.length && frete(cargasB) <= frete(cargas)) cargas = cargasB;
      else restaurar(snapA);
    }
  }
  return { cargas, especiais };
}

// lib/carga/madeira.js
var hyp = (a, b) => Math.hypot(a, b);
var COBERTURA_CAIXA = 0.6;
function madeiraDaUnidade(u, veic) {
  const m = { caibro: 0, sarrafo: 0, tabua: 0 };
  const along = (u.fx || (u.girada ? u.L : u.C)) / 1e3, across = Math.min((u.fz || (u.girada ? u.C : u.L)) / 1e3, veic.L / 1e3);
  m.nCaibro = Math.max(2, Math.round(along / 1.5) + 1);
  m.compCaibro = across;
  m.caibro += m.nCaibro * across;
  if (u.calcos?.length) {
    m.nCaibro = 0;
    m.compCaibro = 0;
    m.calcos = u.calcos.map((c) => ({ C: c.C, L: c.L, A: c.A }));
    m.caibro = u.calcos.reduce((s, c) => s + c.A / 1e3, 0);
  }
  const C = u.C / 1e3, L = u.L / 1e3, A = u.A / 1e3, emb = u.embalagem?.tipo;
  if (u.emPe && emb === "engradado") {
    m.sarrafo += 2 * (2 * C + 2 * A) + 2 * hyp(C, A) + 4 * L + 3 * C;
    m.caibro += 2 * C + 3 * L;
  } else if (emb === "engradado" && u.embalagem?.deitado) {
    m.sarrafo += 2 * (2 * C + 2 * L) + 4 * A;
    m.tabua += C * L;
    m.caibro += 3 * L;
  } else if (u.tipo === "CAIXA") {
    m.tabua += COBERTURA_CAIXA * 2 * (C * L + C * A + L * A);
    m.sarrafo += 4 * A + 2 * (2 * C + 2 * L);
    m.caibro += 3 * L;
  }
  return m;
}
var pecasDeMadeira = (m) => ({ caibro: Math.ceil(m.caibro / 3), sarrafo: Math.ceil(m.sarrafo / 3), tabua: Math.ceil(m.tabua / 0.9) });
function madeiraDaCarga(itens, veic) {
  const tot = { caibro: 0, sarrafo: 0, tabua: 0 };
  for (const u of itens) {
    const m = madeiraDaUnidade(u, veic);
    for (const k in tot) tot[k] += m[k];
  }
  return { metros: tot, pecas: pecasDeMadeira(tot) };
}

// lib/carga/simular.js
var tipoVolume = (u) => u.embalagem?.rotulo || (u.tipo === "PACOTE" ? u.gc ? "pacote de guarda-corpo cintado" : u.grade ? u.degrau ? "pacote de degraus cintado" : "pacote de grade cintado" : "feixe cintado" : u.tipo === "CAIXA" ? "caixa de madeira" : "pe\xE7a solta cal\xE7ada");
function marcarAlertas(cargas, porId) {
  for (const c of cargas) for (const u of c.itens) {
    if (u.pilha || !u.sobre?.length) continue;
    const baixo = u.sobre.map((id) => porId.get(id)).filter(Boolean);
    if (u.tipo === "CAIXA") {
      u.alerta = "caixa empilhada: conferir refor\xE7o e resist\xEAncia da embalagem inferior";
      continue;
    }
    const leve = baixo.find((b) => u.kg > b.kg * 1.5), delic = baixo.find((b) => b.classe === 3 && !b.emPe && !((u.tipo === "PALLET" || u.tipo === "CAIXA") && b.tipo === "PALLET"));
    if (delic) u.alerta = "em cima de pe\xE7a delicada";
    else if (leve) u.alerta = `mais pesada (${Math.round(u.kg)} kg) que a de baixo (${Math.round(leve.kg)} kg)`;
  }
}
function saidaDaCarga(c, perfil) {
  const niveis = [...new Set(c.itens.filter((u) => !u.pilha).map((u) => Math.round(u.y / 50) * 50))].sort((a, b) => a - b);
  for (const u of c.itens) if (!u.pilha) u.camada = c.emCamadas ? u.nivelPilha || 0 : niveis.indexOf(Math.round(u.y / 50) * 50);
  ...
  const nCam = c.itens.filter((u) => u.y > 0).length * 0.4;
  const chao = Math.round(100 * c.itens.filter((u) => u.y === 0).reduce((t, u) => t + u.C * u.L, 0) / (c.veic.C * c.veic.L));
  ...
  const ordem = [...c.itens].sort((a, b) => a.y - b.y || a.x - b.x || a.z - b.z);
  ordem.forEach((u, j) => {
    u.volume = j + 1;
  });
  const romaneio = ordem.map((u) => {
    const mad = madeiraDaUnidade(u, c.veic);
    return { volume: u.volume, id: u.id, tipo: tipoVolume(u), rotulo: u.rotulo, marcas: [...new Set((u.membros || [u]).map((m) => m.marca))], pecas: (u.membros || [u]).length, kgLiquido: Math.round((u.membros || [u]).reduce((t, m) => t + m.kg, 0)), kgBruto: Math.round(u.kg), dimsCm: [u.C, u.L, u.A].map((v) => Math.round(v / 10)), madeira: { ...pecasDeMadeira(mad), calcos: mad.calcos, nCaibro: mad.nCaibro, compCaibro: Math.round(mad.compCaibro * 10) / 10 } };
  });
  const itens = c.itens.map((u) => ({ ...u, membros: (u.membros || [u]).map((m) => ({ id: m.id, marca: m.marca, desc: m.desc, kg: m.kg, nivel: m.nivel, nivelNome: m.nivelNome, C: m.C, L: m.L, A: m.A, perm: m.perm, giro: m.giro, dx: m.dx || 0, dy: m.dy || 0, dz: m.dz || 0, temGeo: m.temGeo, facesApoio: m.facesApoio })) }));
  return {
    versaoMontagem: 7,
    veiculo: c.veic,
    grupo: c.grupo,
    ...
    itens,
    peso: Math.round(c.peso),
    altura: c.altura,
    chao,
    usa,
    passos,
    alertas: c.itens.filter((u) => u.alerta).length,
    profundidade: Math.max(1, ...c.itens.map((u) => (u.nivelPilha || 0) + 1)),
    camadas: [...new Set(c.itens.map((u) => u.camada || 0))].length,
    volumes: ordem.length,
    romaneio,
    madeira: madeiraDaCarga(c.itens, c.veic),
    tempo: { movimentos: c.itens.length, minutos }
  };
}
function simularCarga({ lista, geometria: geometria2 = {}, perfil: chavePerfil = "recomendado", prefixo = "", opcoes = {}, ajustes = {} }) {
  const perfil = PERFIS[chavePerfil] || PERFIS.recomendado;
  const pecasBase = expandirPecas(lista, geometria2, ajustes || {});
  ...
  const rodar = (gcModo) => {
    const ctx = novoContexto({ ...opcoes, prefixo });
    ctx.gradeSeparada = !!opcoes.gradeSeparada;
    ...
    return {
      gcModo,
      cargas: saida,
      especiais: especiais.map((u) => ({ id: u.id, rotulo: u.rotulo, transporte: u.transporte, aviso: u.aviso, C: u.C, L: u.L, A: u.A, kg: Math.round(u.kg), marcas: [...new Set((u.membros || [u]).map((m) => m.marca))] })),
  ...
  let r;
  if (gc === "melhor" || gc === "melhor2") {
    const cands = ["topo", "engradado"].map(rodar);
    r = cands.reduce((m, c) => c.resumo.frete < m.resumo.frete || c.resumo.frete === m.resumo.frete && c.resumo.viagens < m.resumo.viagens ? c : m);
  } else r = rodar(gc);
...
// tmp/analisar102.js
var ps = JSON.parse(import_node_fs.default.readFileSync("/tmp/carga102/previos.json"));
var geometria = JSON.parse(import_node_fs.default.readFileSync("/tmp/carga102/geometria.json"));
for (const p of ps) {
  const lista = p.itens.map((i) => ({ marca: i.marca, desc: i.descricao, qtd: Math.max(1, Number(i.qte) || 1), kgUn: Number(i.pesoTotal) / Math.max(1, Number(i.qte) || 1) }));
  const entrada = { lista, geometria, prefixo: "T102", perfil: p.numero === 1 ? "economico" : "recomendado" };
  import_node_fs.default.writeFileSync(`/tmp/carga102/entrada-${p.numero}.json`, JSON.stringify(entrada));
  const r = simularCarga(entrada);
  import_node_fs.default.writeFileSync(`/tmp/carga102/antes-${p.numero}.json`, JSON.stringify(r));
  console.log(p.numero, r.resumo);
  console.log("EM PE", r.cargas.flatMap((c) => c.itens).filter((u) => u.emPe || u.A > u.L * 2).map((u) => ({ marca: u.rotulo, C: u.C, L: u.L, A: u.A, tipo: u.embalagem?.tipo })));
}
