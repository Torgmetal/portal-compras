...
export function planejarCalcos(u,baixo){
 ...
 for(const opcoes of linhas){let escolhido=null;
   for(const opcao of opcoes){const x=u.x+opcao.rx*fx,calcos=[];let fundo=Infinity;for(let dx=-50;dx<=50;dx+=10)for(let dz=0;dz<=fz;dz+=10){const h=alturaContato(u,x+dx,u.z+dz,false);if(h!==null)fundo=Math.min(fundo,h);}if(!Number.isFinite(fundo))continue;const topoTravessa=fundo,baseTravessa=topoTravessa-100;let valido=true;
