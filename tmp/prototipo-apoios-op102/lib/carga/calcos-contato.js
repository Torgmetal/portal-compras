...
import { temGiroSemBaseConfirmada } from './apoios';
...
export function alturaContato(u,x,z,superior){
  ...
  let a=x-u.x,b=z-u.z;if(u.girada)[a,b]=[b,u.L-a];
...
export function planejarCalcos(u,baixo){
 ...
 for(const opcoes of linhas){let escolhido=null;
   for(const opcao of opcoes){const x=u.x+opcao.rx*fx,calcos=[];let valido=true;
