...
export function montarCaibros(u, madeira, baixo = []) {
  ...
  if(u.calcos?.length){
    for(const c of u.calcos){const m=new THREE.Mesh(new THREE.BoxGeometry(mm(c.C),mm(c.A),mm(c.L)),matMad);m.position.set(mm(c.x+c.C/2),mm(c.y+c.A/2),mm(c.z+c.L/2));m.userData.pilhaDe=u.id;m.castShadow=true;out.push(m);}return out;
  }
