...
import { coberturaDaBase, baseCompativelParaCaixa } from "./apoios";
import { planejarCalcos, temFacesApoio } from "./calcos-contato";
import { ehQuadroVazado } from "./classificar";

const { MADEIRA, FOLGA, CEL } = MEDIDAS;
export const CAMPOS = ["x", "y", "z", "sobre", "girada", "fx", "fz", "nivelPilha", "camada", "pilha", "semLugar", "calcos"];
...
export const novoContexto = (op = {}) => ({ prefixo: op.prefixo || "", sequencia: !!op.sequencia, janela: op.janela || MEDIDAS.JANELA, cel: op.cel || CEL, modoGrupo: "", reservaTopo: 0, veiculos: op.veiculos || VEICULOS, frete: op.frete });
...
function avaliarPosicao(c, u, perfil, ctx, pos, tetoUtil, pref, melhor) {
  ...
  if (topo > 0 && (u.soChao || u.tipo === "PALLET" || (u.baseEmV && u.almaVertical !== false) || nivelBase + 1 > perfil.niveisMax)) return null; // ajuste "no chão": só no assoalho
  ...
  for (let a = ix; a < ix + cx; a++) for (let b = iz; b < iz + cz; b++) { const k = a * NZ + b; nCel++; if (Math.abs(topo - c.alt[k]) <= 2) nTopo++; if (c.dono[k]) donos.add(c.dono[k]); }
  const baixo = [...donos]; let calcos;
  if (topo > 0) {
    ...
    // A grade faz a triagem; a projeção real dos retângulos exige 80% e cobertura nos quatro quadrantes.
    ...
    if(usaContato){calcos=planejarCalcos(candidato,baixo);if(!calcos)return null;}
    ...
    // Prefere pilhas alinhadas: avançar sobre o vazio não pode ganhar só por ficar mais à frente.
    custo += (1-cobertura.fracao)*10000;
  }
...
export function tentaPor(c, u, perfil, ctx, pref = "chao") {
  ...
  for (let a = ix; a < ix + cx; a++) for (let b = iz; b < iz + cz; b++) { const k = a * NZ + b; c.alt[k] = y + u.A; c.dono[k] = u; c.nivel[k] = nivel + 1;
    if (u.nadaEmCima || u.baseEmV || (u.topoVazado && !temFacesApoio(u))) c.tampa[k] = 1; // ajuste "nada em cima"
