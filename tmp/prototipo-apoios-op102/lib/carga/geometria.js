...
export function expandirPecas(lista, geometria = {}, ajustes = {}) {
  ...
  for (const p of lista) {
    ...
    for (let i = 0; i < (p.qtd || 1); i++) {
      ...
      pecas.push(o ? { ...base, ...o, facesApoio: !aj?.orientacao && !manual ? g?.apoioSuperior?.faces : undefined, topoVazado: g?.apoioSuperior ? (!!aj?.orientacao || !!manual || !g.apoioSuperior.plana) : undefined, temGeo: !manual && !!g?.temGeo, aproximada: !!manual || !g?.obb, estimada: !!estimada, manual: !!manual } : { ...base, semCaixa: true });
    }
  }
