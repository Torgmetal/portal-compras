...
export async function gerarModeloCargaPDF({ op, previo, carga, indice = 0, total = 1, perfilNome = "", prefixo = "", imagens = {}, estimadas = [], ajustes = {}, logo: logoBytes = null }) {
  ...
  for (const u of ordem) {
    ...
    texto(apoios.length?`Sobre ${apoios.length===1?"o volume":"os volumes"} ${apoios.map(n=>String(n).padStart(2,"0")).join(", ")}, com caibros.`:u.y>0?"Apoio não identificado. Conferir antes de carregar.":"No piso da carroceria, sobre caibros.",direita,192,dw,11,bold,NAVY,2);
    const md=rom.madeira||{};
    texto(md.calcos?.length?`${md.calcos.length} calços de contato · conferir pontos no 3D e dimensionar os apoios antes de carregar`:md.compCaibro?`${md.nCaibro||2} caibros de ${String(md.compCaibro).replace(".",",")} m${md.sarrafo?` · ${md.sarrafo} sarrafos`:""}${md.tabua?` · ${md.tabua} tábuas`:""}`:"Madeira: conferir a preparação na seção 03.",direita,158,dw,10,font,GRAY,2);
