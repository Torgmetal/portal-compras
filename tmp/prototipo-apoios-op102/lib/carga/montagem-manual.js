...
export function recalcularMontagem(carga, madeira=100) {
  ...
  for(const u of itens){
    u.camada=niveis.indexOf(Math.round(u.y));u.sobre=[];delete u.alerta;delete u.calcos;
    ...
    if(u.topoVazado)aviso('geometria',u,`Volume ${u.volume}: topo irregular ou vazado. Empilhar apenas com contatos e calços conferidos; validar resistência e amarração no carregamento.`);
    if(u.y>1){

      const apoios=itens.filter(b=>b.id!==u.id&&inter(u,b,'x','fx')>1&&inter(u,b,'z','fz')>1&&Math.abs(topo(b)+madeira-u.y)<=2);
      u.sobre=apoios.map(b=>b.id);
      if(temFacesApoio(u))u.calcos=planejarCalcos(u,apoios)||undefined;
      ...
      if(!u.calcos && (!coberturaDaBase(u,apoios.filter(b=>!b.baseEmV&&!b.topoVazado&&!temGiroSemBaseConfirmada(b))).suficiente||temGiroSemBaseConfirmada(u)))aviso('apoio',u,`Volume ${u.volume}: a base não tem apoio suficiente identificado na altura escolhida. Reposicione ou confira os apoios reais.`);
  ...
  const chao=Math.round(100*itens.filter(u=>u.y<=1).reduce((s,u)=>s+u.fx*u.fz,0)/(carga.veiculo.C*carga.veiculo.L));
  const romaneio=itens.map(u=>{const r=(carga.romaneio||[]).find(v=>v.id===u.id)||{id:u.id,volume:u.volume,rotulo:u.rotulo,pecas:u.membros?.length||1,dimsCm:[u.C,u.L,u.A].map(v=>Math.round(v/10))};const mad=madeiraDaUnidade(u,carga.veiculo);return {...r,madeira:{...pecasDeMadeira(mad), calcos: mad.calcos,nCaibro:mad.nCaibro,compCaibro:arred(mad.compCaibro)}};});
