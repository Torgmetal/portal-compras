...
export function caixaOrientada(pos) {
  ...
  // Detecta a base em V pela malha: as duas extremidades acima do trecho central.
  // Restrita a longarinas na orientação; não muda vigas retas pela proporção da caixa.
  ...
  if (iC !== 1) {
    ...
    for (let i=0;i<pos.length;i+=3) {
      ...
      const t=q[iC]/obb.dimsEixos[iC], f=t<.2?0:t>.8?2:t>.4&&t<.6?1:-1;
      if(f>=0) faixa[f]=Math.min(faixa[f],q[1]);
    }
    const limiar=Math.max(80,obb.dimsEixos[1]*.2);
...
export function orientarPeca(desc, dimsEixos, obb, orientacao = "") {
  ...
  if (!chapa) {
    ...
    if (d[iA] > 1.5 * d[iL] && (d[iA] > 800 || (!orientacao && !/VIGA|COLUNA|PERFIL|LONGARINA/i.test(desc)))) { const t = iA; iA = iL; iL = t; }
...
export function expandirPecas(lista, geometria = {}, ajustes = {}) {
  ...
  for (const p of lista) {
    ...
    for (let i = 0; i < (p.qtd || 1); i++) {
      ...
      pecas.push(o ? { ...base, ...o, facesApoio: !aj?.orientacao && !manual ? g?.apoioSuperior?.faces : undefined, ocupacao: !aj?.orientacao && !manual ? g?.ocupacao : undefined, topoVazado: g?.apoioSuperior ? (!!aj?.orientacao || !!manual || !g.apoioSuperior.plana) : undefined, temGeo: !manual && !!g?.temGeo, aproximada: !!manual || !g?.obb, estimada: !!estimada, manual: !!manual } : { ...base, semCaixa: true });
    }
  }
