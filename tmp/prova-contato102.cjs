...
var frenteDe = (u, ctx2) => faseDaMarca(u.marca, ctx2.prefixo) + (ctx2.sequencia ? `/N${u.nivel || 0}` : "");
var cabeEmPe = (u, tipo, veic) => {
  const e2 = EMB[tipo], alt = e2.base + u.L + e2.tampa;
  return alt <= veic.alturaUtil && alt + veic.assoalho <= LEGAL.alturaTotalMax && u.kg <= e2.kgMax && u.C + 2 * e2.quadro <= veic.C;
};
function porEmPe(u, tipo) {
  const e2 = EMB[tipo], alturaPainel = u.L, espessura = u.A, largura = Math.max(espessura + 2 * e2.quadro, e2.largMin || 0);
  Object.assign(u, {
    emPe: true,
    posicao: "em p\xE9",
    ...
    C: u.C + 2 * e2.quadro,
    L: largura,
    A: e2.base + alturaPainel + e2.tampa,
    kg: u.kg + e2.kgProprio,
    embalagem: { tipo, rotulo: e2.rotulo, quadro: e2.quadro, base: e2.base, tampa: e2.tampa, alturaPainel, espessura, kgProprio: e2.kgProprio },
    classe: tipo === "engradado" ? 0 : 1
  });
  u.aviso = `em p\xE9 em ${e2.rotulo} ${m3(u)}`;
}
function porEngradadoDeitado(u) {
  const e2 = EMB.engradado;
  Object.assign(u, {
    tipo: "ENGRADADO",
    C: u.C + 2 * e2.quadro,
    L: u.L + 2 * e2.quadro,
    A: e2.base + u.A + e2.tampa,
    kg: u.kg + e2.kgProprio,
    classe: 0,
    embalagem: { tipo: "engradado", deitado: true, rotulo: "engradado refor\xE7ado (deitado)", quadro: e2.quadro, base: e2.base, tampa: e2.tampa, kgProprio: e2.kgProprio }
  });
...
function pacotesGuardaCorpo(pecas, ctx2, gcModo, unidades, seq) {
  const espMax = gcModo === "engradado" ? PAC_GC.alturaMax : PAC_GC.alturaMax, kgMax = PAC_GC.kgMax;
  const g = agrupar(pecas.filter(ehGC).sort((a, b) => b.A - a.A || b.C - a.C), (u) => `${frenteDe(u, ctx2)}|${Math.ceil(u.C / 2e3) * 2}`);
...
function pacotesGrade(pecas, ctx2, unidades, seq) {
  const g = agrupar(pecas.filter(ehGrade).sort((a, b) => b.A - a.A || b.C - a.C || b.L - a.L), (u) => `${frenteDe(u, ctx2)}|${ehDegrau(u) ? "DEG" : "GR"}|${Math.ceil(u.C / 1e3)}|${Math.ceil(u.L / 500) * 0.5}`);
  for (const [kk, lista] of g) {
...
function pacotesPlanosEFeixes(pecas, ctx2, PAC_P, unidades, seq, forcaFeixe = []) {
  const barra = (u) => ehBarra(u) && u.C <= (PAC_P.compMax || Infinity);
  const planas = agrupar(pecas.filter(ehPlana).sort((a, b) => b.A - a.A || b.C - a.C), (u) => `${frenteDe(u, ctx2)}|${u.desc || "CHAPA"}|${Math.ceil(u.C / 2e3) * 2}`);
  ...
  const barras = agrupar(pecas.filter(barra).concat(forcaFeixe).sort((a, b) => b.L - a.L || b.A - a.A || b.C - a.C), (u) => `${frenteDe(u, ctx2)}|${u.desc || "BARRA"}|${Math.ceil(u.C / 2e3) * 2}`);
...
function caixasDeMiudos(pecas, ctx2, perfil, unidades, seq, entra = ehPequena) {
  ...
  for (const { CM, sel } of niveisCaixa) {
    ...
    for (const [m, lista] of porMarca) {
      if (!perfil.compartilharMiudos && lista.length >= (perfil.minPorMarca ?? CM.minPorMarca)) {
        ...
        grupos.push({ frente: frenteDe(lista[0], ctx2), rotulo: `${m} \u2014 ${lista[0].desc || ""} (${lista.length} p\xE7${niv.length > 1 ? `, ${niv.length} n\xEDveis` : ""})`, lista });
      } else for (const u of lista) {
        const k = frenteDe(u, ctx2);
...
function montarUnidades(pecasBase, perfil, gcModo, ctx2) {
  ...
  pacotesGuardaCorpo(modeladas, ctx2, gcModo, unidades, seq);
  pacotesGrade(modeladas, ctx2, unidades, seq);
  const barra = pacotesPlanosEFeixes(modeladas, ctx2, PAC_P, unidades, seq, forcaFeixe);
  for (const lista of agrupar(perfisCurtos, (p) => `${frenteDe(p, ctx2)}|${p.desc}`).values()) {
    let grupo = [];
    for (const p of lista.sort((a, b) => b.C - a.C)) {
      if (grupo.length && p.C < grupo[0].C * 0.8) {
        pacotesPlanosEFeixes([], ctx2, PAC_P, unidades, seq, grupo);
        grupo = [];
      }
      grupo.push(p);
    }
    if (grupo.length) pacotesPlanosEFeixes([], ctx2, PAC_P, unidades, seq, grupo);
  }
  caixasDeMiudos(modeladas, ctx2, perfil, unidades, seq, (p) => ehPequena(p) && !curtos.has(p));
  if (estimadas.length) caixasDeMiudos(estimadas, ctx2, perfil, unidades, seq, () => true);
  ...
  classificarTransporte(unidades, perfil, gcModo, ctx2.veiculos?.carreta || VEICULOS.carreta);
  aplicarAjustesNasUnidades(unidades, ctx2.veiculos?.carreta || VEICULOS.carreta);
  return unidades;
}
...
var fotografar = (us2) => us2.map((u) => [u, Object.fromEntries(CAMPOS.map((c) => [c, u[c]]))]);
var restaurar = (snap) => {
...
function novaCarga(veic, zona, ctx2) {
  const cel = ctx2.cel, nx = Math.floor((veic.C - zona) / cel), nz = Math.floor(veic.L / cel);
  return { veicKey: veic.chave, veic, zonaGC: zona, nx, nz, cel, emCamadas: ctx2.modoGrupo === "grades", alt: new Float32Array(nx * nz), dono: new Array(nx * nz).fill(null), tampa: new Uint8Array(nx * nz), nivel: new Uint8Array(nx * nz), itens: [], peso: 0, altura: 0, pilhas: [] };
}
...
function avaliarPosicao(c, u, perfil, ctx2, pos, tetoUtil, pref, melhor) {
...
function tentaPor(c, u, perfil, ctx2, pref = "chao") {
  const veic = c.veic, NX = c.nx, NZ = c.nz, cel = c.cel, delicada = u.classe === 3 && !u.emPe;
  const tetoUtil = c.emCamadas ? Math.min(veic.alturaUtil, GRADE_CARGA.teto) : veic.alturaUtil - (delicada ? 0 : ctx2.reservaTopo || 0);
  ...
  for (const [fx2, fz2, girada2] of opcoes) {
    ...
    for (let ix2 = 0; ix2 + cx2 <= NX; ix2++) for (let iz2 = 0; iz2 + cz2 <= NZ; iz2++) {
      const r = avaliarPosicao(c, u, perfil, ctx2, { ix: ix2, iz: iz2, cx: cx2, cz: cz2, fx: fx2, fz: fz2, girada: girada2 }, tetoUtil, pref, melhor);
...
function empacotar(ordem, veicKey, perfil, ctx2, pilha = [], pref = "chao", consolidarDepois = true) {
  const veic = ctx2.veiculos[veicKey];
  for (const u of ordem) limpar(u);
  for (const u of pilha) limpar(u);
  const cargas = [];
  let carga = pilha.length ? novaCarga(veic, Math.max(...pilha.map((u) => u.C)) + FOLGA * 2, ctx2) : novaCarga(veic, 0, ctx2);
  if (pilha.length) colocarPilha(carga, pilha);
  for (const u of ordem) {
    let ok = false;
    const abertas = ctx2.sequencia ? cargas.slice(-ctx2.janela).concat([carga]) : cargas.concat([carga]);
    for (const c of abertas) {
      if (c.peso + u.kg > veic.pesoMax) continue;
      if (tentaPor(c, u, perfil, ctx2, pref)) {
        ok = true;
        break;
      }
    }
    if (!ok) {
      ...
      carga = novaCarga(veic, 0, ctx2);
      if (!tentaPor(carga, u, perfil, ctx2, pref)) u.semLugar = true;
    }
  }
  if (carga.itens.length) cargas.push(carga);
  return consolidarDepois ? consolidar(cargas, perfil, ctx2, pref) : cargas;
}
...
function consolidar(cargas, perfil, ctx2, pref) {
  if (!ctx2.sequencia) return cargas;
  for (let i = cargas.length - 1; i >= 0; i--) {
    const c = cargas[i];
    if (cargas.length < 2 || c.peso > 0.3 * c.veic.pesoMax || c.itens.some((u) => u.pilha)) continue;
    const alvos = cargas.filter((x, j) => x !== c && Math.abs(j - i) <= ctx2.janela);
    ...
    for (const u of itens) {
      ...
      for (const x of alvos) {
        ...
        if (tentaPor(x, u, perfil, ctx2, pref)) {
          posto = true;
          break;
        }
      }
    ...
    restaurar(fot);
    const vizinhas = cargas.map((x, j) => ({ x, d: Math.abs(j - i) })).filter(({ x, d }) => x !== c && d <= ctx2.janela && x.veicKey === c.veicKey && !x.itens.some((u) => u.pilha) && x.peso + c.peso <= c.veic.pesoMax).sort((p, q) => p.d - q.d);
    for (const { x } of vizinhas) {
      ...
      for (const cmp of [ordemSequencia, ordemSequenciaComp]) for (const pr of ["chao", "empilhar"]) {
        const nova = empacotar([...uniao].sort(cmp), c.veicKey, perfil, ctx2, [], pr, false);
...
var us = montarUnidades(expandirPecas(e.lista, e.geometria), PERFIS.economico, "topo", ctx);
console.time("pack");
var cs = empacotar(us.sort((a, b) => b.C * b.L - a.C * a.L), "carreta", PERFIS.economico, ctx, [], "empilhar");
console.timeEnd("pack");
console.log(cs.map((c) => ({ n: c.itens.length, emp: c.itens.filter((u) => u.y > 0).length, cal: c.itens.filter((u) => u.calcos).length })));
import_node_fs.default.writeFileSync("/tmp/carga102/prova.json", JSON.stringify(cs));
