...
for (const n of [1, 2]) {
  ...
  for (const [k, cmp] of Object.entries({ area: (a, b) => b.C * b.L - a.C * a.L, comp: (a, b) => b.C - a.C, larg: (a, b) => b.L - a.L })) {
    ...
    console.time(n + k);
    const cs = encaixarNoPiso(u.sort(cmp), VEICULOS.carreta);
    console.timeEnd(n + k);
    console.log(cs.map((c) => c.itens.length));
    import_node_fs.default.writeFileSync(`/tmp/carga102/piso-${n}-${k}.json`, JSON.stringify(cs));
  }
}
