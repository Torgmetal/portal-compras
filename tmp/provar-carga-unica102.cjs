    ...
    for (let ci = 0; ci <= cargas.length; ci++) {
      ...
      if (!melhor && u.tipo === "PECA" && u.facesApoio && !u.soChao && !u.emPe) {
        for (const b of c.itens) {
          ...
          for (const g of [false, true]) for (const rx of [0, 0.5, 1]) for (const rz of [0, 0.5, 1]) {
            if (posto) continue;
            const fx = g ? u.L : u.C, fz = g ? u.C : u.L;
            if (fx > b.fx || fz > b.fz) continue;
            const x = Math.ceil((b.x + (b.fx - fx) * rx) / CEL2) * CEL2, z = Math.ceil((b.z + (b.fz - fz) * rz) / CEL2) * CEL2;
...
function alturasLocais(u, girada = false, virada = false) {
  ...
  for (const r of rs) {
    ...
    for (let a = Math.floor(x / CEL2); a < Math.ceil((x + C) / CEL2); a++) for (let b = Math.floor(z / CEL2); b < Math.ceil((z + L) / CEL2); b++) {
...
function alturaDeEncaixe(u, x, z, girada, itens, nx, nz) {
  ...
  for (const b of itens) for (const r of alturasLocais(b, b.girada, b.rotacaoManual?.y === 180)) {
    const a = Math.floor(b.x / CEL2) + r.a, c = Math.floor(b.z / CEL2) + r.b;
    if (a >= 0 && a < nx && c >= 0 && c < nz) alt[a * nz + c] = Math.max(alt[a * nz + c], b.y + r.max);
  }
  let y = 0;
  for (const r of alturasLocais(u, girada)) {
    const a = Math.floor(x / CEL2) + r.a, b = Math.floor(z / CEL2) + r.b;
...
for (const n of [1, 2]) {
  ...
  const orig = montarUnidades(expandirPecas(e.lista, geometria), PERFIS.economico, "topo", novoContexto());
  for (const C of [12400, 14e3]) for (const L of [2450, 2850, 3e3]) {
    const v = { ...VEICULOS.carreta, C, L };
    let best = 99, counts;
    for (const [k, cmp] of Object.entries({ area: (a, b) => b.C * b.L - a.C * a.L, comp: (a, b) => b.C - a.C, larg: (a, b) => b.L - a.L })) {
      const us = orig.map((u) => ({ ...u, ocupacao: geometria[u.marca || u.rotulo]?.ocupacao, facesApoio: void 0 }));
      const cs = encaixarNoPiso(us.sort(cmp), v);
      const score = cs.length + us.filter((u) => u.semLugar).length * 10;
      if (score < best) {
        best = score;
        counts = cs.map((c) => c.itens.length);
        import_node_fs.default.writeFileSync(`/tmp/carga102/unica-${n}-${C}-${L}.json`, JSON.stringify(cs));
      }
    }
