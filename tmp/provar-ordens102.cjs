...
for (const n of [1, 2]) {
  ...
  for (const [nome, cmp] of Object.entries({ area: (a, b) => b.C * b.L - a.C * a.L, comp: (a, b) => b.C - a.C, peso: (a, b) => b.kg - a.kg, larg: (a, b) => b.L - a.L })) {
    const ctx = novoContexto(), us = montarUnidades(expandirPecas(e.lista, e.geometria), PERFIS[e.perfil], "topo", ctx).filter((u) => u.transporte === "normal");
    const inicio = Date.now();
    const cs = empacotar(us.sort(cmp), "carreta", PERFIS[e.perfil], ctx, [], "empilhar");
    console.log(n, nome, cs.length, Date.now() - inicio);
    import_node_fs.default.writeFileSync(`/tmp/carga102/prova-${n}-${nome}.json`, JSON.stringify(cs));
  }
}
