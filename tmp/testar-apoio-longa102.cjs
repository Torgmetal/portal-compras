...
function alturaFaces(faces, x, z, superior) {
  ...
  for (const t of bins.get(`${Math.floor(x / 200)},${Math.floor(z / 200)}`) || []) {
    ...
    const a2 = ((bz - cz) * (x - cx) + (cx - bx) * (z - cz)) / den, b2 = ((cz - az) * (x - cx) + (ax - cx) * (z - cz)) / den;
    if (a2 >= -1e-6 && b2 >= -1e-6 && a2 + b2 <= 1.000001) h = h === null ? y : superior ? Math.max(h, y) : Math.min(h, y);
  }
...
function alturaContato(u, x, z, superior) {
  ...
  let a2 = x - u.x, b2 = z - u.z;
  if (u.rotacaoManual?.y % 360 === 180) {
    a2 = (u.fx || u.C) - a2;
    b2 = (u.fz || u.L) - b2;
  }
  if (u.girada) [a2, b2] = [b2, u.L - a2];
  const h = alturaLocal(u, a2, b2, superior);
...
function contatosSuperiores(u, rx, lado) {
  ...
  for (const rz of lado === 0 ? [0.15, 0.1, 0.2, 0.05, 0.25, 0.3, 0.35, 0.4, 0.45] : [0.85, 0.9, 0.8, 0.95, 0.75, 0.7, 0.65, 0.6, 0.55]) {
    ...
    const alturas = [[0, 0], [-20, -20], [20, -20], [-20, 20], [20, 20]].map(([a2, b2]) => alturaContato({ ...u, x: 0, y: 0, z: 0 }, x + a2, z + b2, false));
...
function planejarCalcos(u, baixo) {
  if (temGiroSemBaseConfirmada(u) || u.rotacaoManual && (u.rotacaoManual.x !== 0 || u.rotacaoManual.z !== 0 || u.rotacaoManual.y % 180 !== 0)) return null;
  const fx = u.fx || u.C, fz = u.fz || u.L, n2 = 2, out = [];
  ...
  if (!linhas) {
    linhas = [];
    for (let linha = 0; linha < n2; linha++) {
      const alvo = 0.15 + 0.7 * linha / (n2 - 1), opcoes = [];
      ...
      for (const lado of [0, 1]) {
        ...
        for (const rz of lado === 0 ? [0.1, 0.05, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45] : [0.9, 0.95, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55]) {
          ...
          for (const b2 of baixo) {
            ...
            const alturas = pontos.map(([a2, c]) => alturaContato(b2, x + a2, z + c, true));
            ...
            if (baixo.some((o) => o !== b2 && pontos.some(([a2, c]) => {
              const h = alturaContato(o, x + a2, z + c, true);
              return h !== null && h > y + 1;
            }))) continue;
...
// tmp/testar-apoio-longa102.js
var s = JSON.parse(import_node_fs.default.readFileSync("/tmp/carga102/config.json")).sims[1];
var g = JSON.parse(import_node_fs.default.readFileSync("/tmp/carga102/geometria.json"));
var us = s.cargas.flatMap((c) => c.itens);
var b = { ...us.find((u) => u.rotulo === "T102B45"), x: 50, z: 50, y: 0, facesApoio: g.T102B45.apoioSuperior.faces };
var a = { ...us.find((u) => u.rotulo === "T102A16"), facesApoio: g.T102A16.apoioSuperior.faces, rotacaoManual: { x: 0, y: 0, z: 0 }, girada: false };
var n = 0;
for (let x = 100; x < 5e3; x += 100) for (let z = 350; z < 700; z += 25) {
  const u = { ...a, x, y: 400, z }, cs = planejarCalcos(u, [b]);
  if (cs) {
    console.log(x, z, cs);
    import_node_fs.default.writeFileSync("/tmp/carga102/apoio-longa.json", JSON.stringify({ ...u, calcos: cs, sobre: [b.id] }));
    n++;
    break;
  }
  if (n) break;
